...
TL;DR: This paper introduces GenSeC-PC, an advanced method for point cloud transmission that utilizes cross-modal semantic communication combining images and point clouds, demonstrating enhanced efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: The need for efficient point cloud transmission in the context of autonomous driving and extended reality due to increasing data demands.

Method: GenSeC-PC employs a semantic encoder for fusing images and point clouds, utilizes a redesigned decoder based on PointDif, and incorporates a channel-adaptive joint semantic-channel coding architecture.

Result: GenSeC-PC achieves high compression efficiency, superior reconstruction performance, and operates in real-time, demonstrating robustness under various transmission conditions.

Conclusion: The proposed framework improves transmission reliability and efficiency while supporting fully analog transmission, distinguishing it from previous methods.
...
TL;DR: This paper presents the Sol Video Inference Engine, an acceleration framework for video diffusion models that optimizes inference performance while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: The increasing complexity and size of video diffusion models lead to higher inference costs, necessitating efficient acceleration methods that adapt to the specific instance of model, hardware, and configuration.

Method: The Sol Video Inference Engine employs five techniques—cache, sparse attention, token pruning, quantization, and kernel fusion—within an agentic stack, where parallel agents optimize each technique for specific deployment targets and a human validator ensures quality.

Result: The framework achieves over 2x end-to-end acceleration on three different video models while preserving near-lossless generation quality.

Conclusion: The agentic optimization framework effectively reduces inference time and resource usage in video diffusion models with minimal manual intervention.
...
TL;DR: Reflective VLA addresses limitations of reactive VLA models by using observation-action-consequence triplets for improved generalization in embodied control tasks.


<details>
  <summary>Details</summary>
Motivation: Current VLA models often fail to generalize due to reliance on single observations, missing vital context from embodiment-specific factors.

Method: Reflective VLA uses context from triplets of observations, actions, and their consequences, leveraging shared attention to improve reasoning during decision-making.

Result: The model achieves better performance in varying environments, showing a 5.4% and 4.2% improvement in success rates over reactive models under distribution shifts.

Conclusion: Reflective VLA enhances generalization across environments by incorporating action consequences, outperforming traditional reactive policies.
...
TL;DR: This paper critiques the notion that video generation equates to world modeling, arguing instead that it results in only a partial spatiotemporal model. It emphasizes the need for counterfactual controllability in video generation to achieve self-evolving world models.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing literature that equates video generation with comprehensive world modeling, which neglects elements like controllability and intervention.

Method: The paper proposes a framework focused on autonomous video generation that prioritizes counterfactual controllability, allowing models to assess the effects of actions and interventions on generated scenarios.

Result: It argues that video generation models, while capable of producing realistic scenarios, lack a full understanding of controllable variables and constraints; thus, they do not constitute a fully functional world model.

Conclusion: The study suggests that incorporating counterfactual controllability is crucial for advancing autonomous video generation toward developing self-evolving world models.
...
TL;DR: This paper introduces a novel generative model for spatio-temporal interpolation of tropical cyclone microwave imagery from multiple sources to address the limitations of long revisit times and data heterogeneity.


<details>
  <summary>Details</summary>
Motivation: To enhance monitoring of tropical cyclone precipitation and intensity by overcoming the challenges of long revisit times and the heterogeneous nature of microwave satellite data.

Method: The authors developed a generative model applied to irregularly-sampled microwave and infrared data, training it using a self-supervised task where a source is masked and reconstructed.

Result: The model achieved significant improvements in Continuous Ranked Probability Score and outperformed traditional methods by effectively utilizing both infrared and microwave data.

Conclusion: This model represents a significant advancement in interpolating cyclone microwave imagery, integrating multiple instruments and allowing for better spatial and temporal resolution.
...
TL;DR: The paper introduces open-vocabulary BEV segmentation (OVBS) using vision-language models to improve 3D geometric consistency in BEV perception for autonomous driving.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current closed-set BEV segmentation methods and enhance recognition in unpredictable real-world scenarios.

Method: OVBEVSeg is proposed as a geometry-aware OVBS framework, incorporating efficient Gaussian splatting and three progressive stages: pseudo-labeling, per-scene optimization, and geometric distillation.

Result: On the nuScenes dataset, OVBEVSeg achieves state-of-the-art performance with a 15.3 mIoU improvement over closed-set methods for unseen categories, faster inference, and reduced memory consumption.

Conclusion: OVBEVSeg demonstrates significant advancements in BEV segmentation, maintaining efficiency and accuracy, even without novel-class ground-truth labels.
...
TL;DR: This paper presents PCFM, a flow matching approach for medical point cloud completion using continuous-time generative modeling, evaluated on several skull datasets.


<details>
  <summary>Details</summary>
Motivation: The need for effective anatomical reconstruction and clinical workflows highlights the importance of medical point cloud completion, which is currently underexplored in generative modeling.

Method: PCFM utilizes PTv3 in a flow matching approach, integrates deterministic encoder-decoder models, and instantiates diffusion completion with PVCNN and PTv3 denoisers.

Result: PCFM achieves competitive performance compared to deterministic models and state-of-the-art generative quality with fewer sampling steps, also demonstrating significant speed improvements.

Conclusion: The study indicates that higher point resolution and model scaling yield consistent performance improvements, underscoring PCFM's advantages in medical point cloud completion.
...
TL;DR: This paper proposes EERLoss, a loss function designed to optimize biometric verification models directly based on the Equal Error Rate (EER), demonstrating superior performance and faster convergence on keystroke dynamics verification tasks.


<details>
  <summary>Details</summary>
Motivation: The study addresses the misalignment between optimization objectives and EER in biometric verification training, aiming to enhance model accuracy and efficiency.

Method: EERLoss, a subdifferentiable approximation to EER, is introduced to train deep biometric models, validated with extensive experiments on keystroke dynamics.

Result: EERLoss outperforms existing loss functions, converges faster, and achieves a significant EER reduction of approximately 30% compared to state-of-the-art architectures.

Conclusion: EERLoss serves as an effective, task-aligned training objective for biometric traits with high variability, validated through rigorous benchmarking.
...
### [9] [Error-Aware TF-IDF Retrieval-Augmented Generation for ASR Error Correction](https://arxiv.org/abs/2606.24915)
*Mohammad Aref Jafari-Raddani*

Main category: cs.CL

TL;DR: The paper presents a novel error-aware framework for automatic speech recognition that enhances accuracy by addressing phonetic misrecognitions and hallucinations, achieving significant performance improvements with low latency.


<details>
  <summary>Details</summary>
Motivation: To improve automatic speech recognition in low-resource languages by reducing errors related to rare entities and domain-specific terms, which are commonly hallucinated by existing systems.

Method: The proposed framework integrates a symmetric text normalization module and an error-aware term frequency-inverse document frequency algorithm, utilizing a sparse diagonal penalty matrix based on historical errors to prioritize corrective documents.

Result: The approach significantly increased the error-aware hit rate from 53.7% to 90.9% and reduced the final word error rate from 23.06% to 18.83% on the Persian subset of the FLEURS dataset.

Conclusion: The new framework enhances the performance of speech recognition systems with minimal latency, proving effective in mitigating hallucination errors.
...
### [10] [Automatic Generation of Highlights for Academic Paper Via Prompt-based Learning](https://arxiv.org/abs/2606.25253)
*Yi Xiang,Chengzhi Zhang,Heng Zhang*

Main category: cs.CL

TL;DR: This study explores prompt-based learning for automatic extraction of academic paper highlights, achieving performance akin to supervised methods without extensive labeled data.


<details>
  <summary>Details</summary>
Motivation: The lack of highlights in many journals hampers effective literature retrieval and analysis; thus, automating highlight generation is essential.

Method: The authors utilize prompt-based learning, designing specific templates and inputting paper abstracts into various language models, including GPT-2, T5, and ChatGPT.

Result: ChatGPT with prompt templates matches prior supervised methods' performance without needing task-specific labeled data and outperforms them when limited examples are included.

Conclusion: This approach offers a novel methodology for highlight generation, effective for papers without domain-specific training data, facilitating further text mining and bibliometric studies.
...
### [11] [Memory Makes the Difference: Evaluating How Different Memory Roles Shape Conversational Agents](https://arxiv.org/abs/2606.25361)
*Yuxin Wang,Paul Thomas,Zhiwei Yu,Yuan Gao,Saeed Hassanpour,Soroush Vosoughi,Robert Sim,Nick Craswell*

Main category: cs.CL

TL;DR: This paper investigates the influence of different memory types on response quality in RAG-based conversational systems, revealing that specific memory roles can improve response accuracy and personalization.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand how various memory roles in RAG-based systems impact the quality of responses, addressing a gap in existing research that mainly focuses on storage and retrieval aspects of memory.

Method: The authors create a taxonomy of conversational memory types, categorize retrieved memories based on their functional roles, and establish a user-centric evaluation framework to assess their effects on agents' responses across different contexts.

Result: Through experiments using long-term datasets and advanced LLMs, the analysis shows that certain memory types enhance response quality, such as clarifying memory increasing accuracy and personalization, while irrelevant memory detracts from topic relevance.

Conclusion: The paper concludes that leveraging specific memory types can lead to more personalized conversational responses, highlighting the potential for further research in enhancing memory mechanisms for conversational agents.
...
### [12] [Three Buddhist Vocabularies: Computational Stylometry of the English Pali Canon across Sutta, Vinaya, and Abhidhamma](https://arxiv.org/abs/2606.25372)
*Joy Bose*

Main category: cs.CL

TL;DR: This study performs a comprehensive computational stylometric analysis of the Tipitaka's English translations, revealing patterns in lexical diversity and vocabulary overlap across different Pitakas.


<details>
  <summary>Details</summary>
Motivation: To enhance understanding of the linguistic characteristics and relationships within the Tipitaka's translations, building upon previous analyses focused solely on the Sutta Pitaka.

Method: The analysis utilizes Zipf rank-frequency distributions to assess vocabulary consistency, Moving Average TTR for lexical diversity evaluation, and compares vocabulary overlap using Jaccard and Szymkiewicz-Simpson coefficients across various texts.

Result: Findings indicate consistent Zipf distributions across all corpora, high lexical diversity in the Sangaha corpus, significant numeral-word density there, shared vocabulary between Theravada and Mulasarvastivada Vinaya, and notable differences between English translations of the same Vinaya source text.

Conclusion: The study establishes clear patterns of lexical diversity and shared vocabulary in the Tipitaka translations, reflecting both historical continuities and nuances in translation across time.
...
### [13] [Is GraphRAG Needed? From Basic RAG to Graph-/Agentic Solutions with Context Optimization](https://arxiv.org/abs/2606.25656)
*Long Chen,Ryan Razkenari,Yuxuan Zhou,Yuan Tian,Rahul Ghosh,Venkatesh Pappakrishnan,Disha Ahuja,Vidya Sagar Ravipati*

Main category: cs.CL

TL;DR: This paper introduces a framework for evaluating advanced Retrieval-Augmented Generation (RAG) models, including GraphRAG and Agentic RAG, across different scenarios with a focus on semi-structured knowledge bases.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of when and how to effectively utilize advanced RAG variants in real-world applications.

Method: Developed a framework for evaluating multiple RAG scenarios and implemented 9 standardized scenarios, alongside a novel context engineering method to enhance performance and manage memory issues.

Result: Experiments showed a significant reduction in token usage (19%-53%) and identified a retrieval-generation gap indicating that improved retrieval does not correlate directly with generation quality.

Conclusion: The findings provide actionable insights into optimizing the use of RAG systems in production environments.
...
### [14] [BitNet Text Embeddings](https://arxiv.org/abs/2606.25674)
*Zhen Li,Xin Huang,Liang Wang,Nan Yang,Ting Song,Yan Xia,Xun Wu,Shaohan Huang,Huishuai Zhang,Furu Wei,Dongyan Zhao*

Main category: cs.CL

TL;DR: This paper presents BITEMBED, a low-bit framework for efficient LLM-based text embedding that reduces both computational and storage costs while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: To address the high costs of deploying LLM-based text embedders due to slow inference and substantial storage overhead from full-precision embeddings.

Method: BITEMBED transforms pretrained LLMs into BitNet-style encoders with ternary weights and quantized activations, using continual contrastive pre-training and supervised contrastive fine-tuning to adapt the model for efficient representation learning.

Result: Experiments demonstrate that BITEMBED achieves performance comparable to full precision teacher embedders on MMTEB datasets while allowing flexible output embedding storage precision.

Conclusion: BITEMBED provides an effective trade-off between performance and storage costs, making it suitable for various large-scale indexing scenarios.
...
### [15] [Beyond Function Calling: Benchmarking Tool-Using Agents under Tool-Environment Unreliability](https://arxiv.org/abs/2606.25819)
*Yang Tian,Zhengpeng Shi,Bo Zhao*

Main category: cs.CL

TL;DR: ToolBench-X is a benchmark designed to evaluate agents' performance in unreliable tool environments by injecting recoverable reliability hazards into task settings.


<details>
  <summary>Details</summary>
Motivation: To address the gap in evaluating tool-use efficiency under non-ideal conditions, as existing benchmarks assume stable tool environments.

Method: ToolBench-X incorporates five structured hazard types into executable multi-step tasks across various domains to assess agents' abilities to recover from reliability hazards.

Result: Experiments showed a significant reliability gap, with agents performing poorly under recoverable hazards despite success in reliable environments; failures stem from limited hazard recognition and poor recovery mechanisms.

Conclusion: Evaluating tool-use should focus on task completion in unreliable environments rather than merely on accuracy of function calls.
...
### [16] [How Reliable Is Your Jailbreak Judge? Calibration and Adversarial Robustness of Automated ASR Scoring](https://arxiv.org/abs/2606.25487)
*Yang Gao*

Main category: cs.CL

TL;DR: The paper critiques the reliability of automated judges used in assessing attack-success rates in LLM jailbreaks, revealing significant discrepancies in their performance versus human evaluations. It highlights the robustness and vulnerabilities of different judge types, calling for more rigorous reporting standards in future research.


<details>
  <summary>Details</summary>
Motivation: To assess the reliability of automated judges in reporting attack-success rates (ASR) in LLM jailbreaks and prompt injection, given the importance of accurate evaluation in AI safety.

Method: The study compares the performance of two types of judges—dedicated classifiers and LLM-based judges—against human majority votes using a dataset of 596 human-labeled completions. It evaluates precision, recall, and robustness under adversarial conditions.

Result: The dedicated classifier exhibited high recall but over-flags responses, while LLM judges maintained better precision but had erratic recall. They showed vulnerability to adversarial attacks, with significant variations in their ASR depending on the judging method used.

Conclusion: Due to the unreliability of ASR derived from LLM judges, the paper recommends new standards for reporting judge performance metrics and conducting adversarial checks to ensure accurate results.
...
### [17] [Detect, Unlearn, Restore: Defending Text Summarization Models Against Data Poisoning](https://arxiv.org/abs/2606.26036)
*Poojitha Thota,Shirin Nilizadeh*

Main category: cs.CL

TL;DR: The study addresses training-time data poisoning in fine-tuning large language models for text summarization, proposing a post-hoc defense framework that enables detection and remediation of such attacks with high precision and minimal performance loss.


<details>
  <summary>Details</summary>
Motivation: The paper seeks to tackle the vulnerability of large language models to data poisoning during the fine-tuning phase, which can lead to biased and harmful summarization outcomes, despite standard evaluation metrics appearing unaffected.

Method: A unified post-hoc defense framework is introduced for detecting and remediating data poisoning, utilizing influence-function analysis and behavioral auditing across white-box and black-box settings.

Result: The experiments reveal that poisoned document-summary pairs have high training influence, allowing for effective poison detection; the defenses achieve detection precision of 85-92% and restore significant model behavior with low utility loss.

Conclusion: The findings demonstrate that poisoning during fine-tuning results in detectable structural artifacts, allowing for feasible post-deployment recovery of summarization models without needing full retraining.
...
### [18] [Evaluating LLM Usage for Efficient and Explainable Numerical and Classified Implicit Sentiment Analysis of Product Desirability](https://arxiv.org/abs/2606.23701)
*Sherri Weitl-Harms,John Hastings*

Main category: cs.CL

TL;DR: A framework using large language models quantifies product desirability from qualitative feedback, achieving high correlation with expert sentiment assessments.


<details>
  <summary>Details</summary>
Motivation: To enhance the measurement of implicit sentiment in qualitative product feedback, which is often nuanced and difficult to quantify.

Method: The framework utilizes large language models to generate numerical sentiment scores and categorical classifications from qualitative responses, validated with two annotated datasets.

Result: The LLMs significantly matched expert labels with high Pearson correlations (up to 0.97) and classification accuracy (up to 94%), outperforming lexicon-based methods and cheaper models achieving competitive performance.

Conclusion: The approach supports efficient and interpretable product feedback analysis, fostering product evaluation and development insights, thereby enabling more effective marketing strategies.
...
TL;DR: This paper explores Emergent Misalignment (EM) in language models, focusing on self-generated text recognition (SGTR) finetuning as an effective intervention for both reversing and preventing EM.


<details>
  <summary>Details</summary>
Motivation: The study is motivated by the idea that EM is linked to misaligned persona vectors rather than direct learning of harmful content, emphasizing the need for targeted interventions.

Method: The authors conduct two-stage finetuning experiments across three language models (GPT-4.1, Qwen2.5-32B-Instruct, Seed-OSS-36B-Instruct) using various EM datasets to evaluate SGTR finetuning against benign baselines.

Result: The results show that while all interventions can reverse EM effects, SGTR finetuning is the only method that consistently reduces EM misalignment without negative consequences, effectively fortifying the model's character.

Conclusion: The findings suggest that EM destabilizes rather than develops a coherent misaligned persona, highlighting the importance of character maintenance in preventing misalignment.
...
### [20] [Do LLM Attribution Metrics Transfer? Auditing Retrieval-Augmented Generation Evaluation Across Datasets and Constructs](https://arxiv.org/abs/2606.23915)
*Tianyu Ding,Aditya Nannapaneni,Juan Pablo De la Cruz Weinstein*

Main category: cs.CL

TL;DR: The paper evaluates eight automatic scoring metrics for attribution in LLM retrieval-augmented generation, finding none consistently performed well across multiple datasets. A prompt-based LLM judge showed promise but comes with higher costs and unpredictability.


<details>
  <summary>Details</summary>
Motivation: To assess the reliability and interchangeability of various automatic metrics for attribution in LLM retrieval-augmented generation by determining their performance stability across different datasets.

Method: The study audited eight scoring metrics across three constructs, analyzing their performance on multiple datasets and comparing the results using statistical measures like Kendall tau and AUROC.

Result: No single automatic scorer maintained high performance across all datasets, with significant metric ranking inversions observed and an average regret metric revealing flaws in the 'best-on-average' selection approach.

Conclusion: Selecting an appropriate scoring metric should be based on the specific target dataset, as automatic metrics may prove unstable; a prompt-based LLM judge provides an alternative but incurs additional costs and variability.
...
### [21] [When Retrieval Metrics Mislead: Measuring Policy Signal in Long-Horizon Tool-Use Agents](https://arxiv.org/abs/2606.23937)
*Tianyu Ding,Juan Pablo De la Cruz Weinstein*

Main category: cs.CL

TL;DR: The study evaluates the effectiveness of exact-match retrieval recall in pre-action policy classification using Qwen2.5-3B/7B classifiers, highlighting that exact-match performance may underestimate the utility of retrieved policies.


<details>
  <summary>Details</summary>
Motivation: To assess whether exact-match retrieval recall accurately reflects the usefulness of retrieved policies for downstream decision-making processes, particularly in the context of pre-action policy classification.

Method: The authors use tau-bench to test Qwen2.5-3B/7B classifiers under various configurations, comparing macro-F1 scores when using gold-policy versus retrieved clauses in decision-time context.

Result: While the top-ranked retrieved clause achieves macro-F1 scores of 0.58 versus 0.60 for gold clauses, the results show no significant macro-F1 difference in this comparison, with mismatched-policy and no-policy controls scoring lower.

Conclusion: Exact-match clause recall may not fully capture the utility of retrieved policies, suggesting the need to incorporate retrieved policies into the classification loop instead of relying solely on recall metrics.
...
### [22] [Does My Embedding Reflect That $A = B$? Evaluating Mathematical Equivalence in Embedding Models](https://arxiv.org/abs/2606.23959)
...
TL;DR: This paper evaluates current embedding models on their ability to capture mathematical equivalence across different terminologies and languages, introducing the MELD dataset as a tool for improving representation learning.


<details>
  <summary>Details</summary>
Motivation: There is a need for better tools that facilitate the navigation between varying mathematical terminologies due to the abstract nature of mathematics, which can lead to misunderstandings or overlooked connections between fields.

Method: The paper introduces the MELD Dataset comprising mathematically equivalent statements with different phrasings and proposes a contrastive learning approach to improve embeddings of mathematical text, focusing on aligning informal and formal statements.

Result: Experiments reveal that the proposed contrastive embedding method enhances retrieval performance for both informal-formal tasks and the MELD dataset, indicating better alignment of statements independent of lexical differences.

Conclusion: The findings highlight the limitations of current embedding models in capturing mathematical equivalence and suggest an effective method to improve this alignment, aiding in the inter-field connectivity of mathematical concepts.
...
### [23] [RASC+: Retrieval-Constrained LLM Adjudication for Clinical Value Set Authoring](https://arxiv.org/abs/2606.23992)
*Sumit Mukherjee*

Main category: cs.CL

TL;DR: This paper presents a stage-wise approach to improving clinical value set completion by enhancing information retrieval and using LLM adjudicators, leading to significant improvements in recall and macro F1 scores.


<details>
  <summary>Details</summary>
Motivation: The need for accurate clinical code retrieval for quality measurement, phenotyping, and decision support, in light of LLM limitations in zero-shot scenarios.

Method: An optimized candidate pool for recall followed by constrained LLM adjudication, specifically utilizing Qwen3 for retrieval and GPT-5 for adjudication.

Result: Achieved increased candidate-pool recall and improved macro F1 scores (0.549 full-test, 0.533 held-out-publisher) compared to baseline methods.

Conclusion: Retrieval-constrained LLMs can significantly enhance clinical value set completion while ensuring that only codes from an auditable pool are returned.
...
TL;DR: The paper critiques the effectiveness of 'caveman style' prompt simplification in AI, showing that while output compression reduces costs, input compression increases costs and affects accuracy negatively.


<details>
  <summary>Details</summary>
Motivation: To evaluate the efficacy of input versus output compression in AI models and their real impact on inference costs and accuracy.

Method: A two-channel evaluation protocol called 'Cavewoman' assesses eight models across five datasets on task accuracy, cost per item, and alignment with reference texts.

Result: Output compression yields significant cost savings (1.4-3x), while input compression raises costs (1.15-2.7x) and degrades accuracy.

Conclusion: Balancing prompt and response styles is crucial, as input compression detrimentally impacts cost and accuracy, contradicting common assumptions.
...
TL;DR: PORTER introduces a vocabulary-independent model for electronic health records (EHR) that enhances flexibility and performance across clinical tasks by integrating event descriptions and numeric values while maintaining representation quality.


<details>
  <summary>Details</summary>
Motivation: The limitation of traditional EHR models stems from their reliance on fixed vocabularies, restricting their ability to encode unseen concepts or combinations, thus affecting transferability between institutions and pipelines.

Method: PORTER utilizes a frozen text encoder for event descriptions, implements a dedicated pathway for numeric integration, and employs an autoregressively pretrained temporal backbone to learn clinical dynamics.

Result: PORTER matched the performance of a fixed-vocabulary model on 74 clinical prediction tasks, effectively transferred across unseen event descriptions, and outperformed traditional models when faced with unseen tokens, achieving a mean AUROC of 97.1%.

Conclusion: This research demonstrates that PORTER can function as a vocabulary-independent EHR foundation model, facilitating cross-task reuse and reducing the need for vocabulary harmonization while maintaining strong performance.
...
TL;DR: The paper proposes a unified computational architecture for Indic languages based on Pānini's grammar, aiming to improve natural language processing in these languages through shared benchmarks.


<details>
  <summary>Details</summary>
Motivation: The fragmentation in NLP resources for Indic languages leads to inefficiency and underdevelopment, necessitating a unifying framework.

Method: The paper introduces a four-part benchmark suite based on the morphosyntactic architecture derived from Pānini's grammar.

Result: The proposed benchmarks will enhance accuracy, data efficiency, and transferability of NLP systems for Indic languages by integrating disparate resources.

Conclusion: This unifying framework not only aims to improve computational performance but also raises questions about the interpretability of neural models in relation to Pānini's linguistic categories.
...
### [27] [BehaviorBench: Benchmarking Foundation Models for Behavioral Science Tasks](https://arxiv.org/abs/2606.24162)
*Jin Huang,Yutong Xie,Wanli Song,Xingjian Zhang,Walter Yuan,Matthew O. Jackson,Qiaozhu Mei*

Main category: cs.CL

TL;DR: BehaviorBench is a benchmark for evaluating foundation models in behavioral science, focusing on four core capabilities: prediction, decision-making, trait inference, and knowledge application. It highlights the gap in performance between general-purpose models and behaviorally fine-tuned models like Be.FM-1.5, which offers better distributional alignment.


<details>
  <summary>Details</summary>
Motivation: To systematically evaluate the performance of foundation models in various behavioral science tasks and identify their applicability across different contexts and populations.

Method: Introduction of BehaviorBench for evaluating models on behavior prediction, strategic decision-making, trait inference, and knowledge application, measured at individual and distributional levels. Development of Be.FM-1.5 model, fine-tuned on behavioral data.

Result: Proprietary models excel in individual-level tasks, while Be.FM-1.5 shows superior distributional alignment, effectively bridging the performance gap with proper adaptation.

Conclusion: The study emphasizes the necessity of distributional evaluation for behavioral validity and positions BehaviorBench and Be.FM-1.5 as valuable resources for developing AI in behavioral science.
...
### [28] [Harmonic: Hierarchical State Space Models for Efficient Long-Context Language Modeling](https://arxiv.org/abs/2606.24650)
*Petr Nyoma*

Main category: cs.CL

TL;DR: Harmonic is a hierarchical state space model for language modeling, outperforming comparable Transformer and Mamba models in efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: To improve language modeling by utilizing a hierarchical state space architecture that processes prediction errors across multiple levels, addressing the limitations of attention mechanisms in large-scale models.

Method: Harmonic employs three recurrent levels at varying timescales, incorporating input from lower levels' prediction errors instead of hidden states, with a focus on minimizing compute complexity and extending sequence lengths.

Result: Harmonic achieves superior performance to a 28M parameter Transformer and Mamba at various token counts, maintaining efficient training even at 64K tokens and demonstrating robustness across different benchmarks.

Conclusion: Harmonic's architecture and approach allow it to overcome the limitations of traditional attention mechanisms, highlighting its potential for scalable language modeling with improved performance and efficiency.
...
### [29] [AI-PAVE-Br: Leveraging Large Language Models for Enhanced Product Attribute Value Extraction through a Golden Set Approach](https://arxiv.org/abs/2606.24655)
...
TL;DR: This paper presents AI-PAVEBr, a high-accuracy system for Product Attribute Value Extraction in Brazilian e-commerce, alongside a new dataset, the Golden Set, for reproducible research.


<details>
  <summary>Details</summary>
Motivation: The need for effective structured information extraction from complex product data in the Brazilian e-commerce sector, particularly given the linguistic challenges of Portuguese descriptions.

Method: The introduction of AI-PAVEBr, utilizing Large Language Models and targeted prompt engineering for product attribute extraction, along with the creation of the Golden Set dataset.

Result: AI-PAVEBr significantly outperforms traditional Named Entity Recognition methods in product attribute value extraction tasks.

Conclusion: The paper offers a valuable tool for Brazilian e-commerce while contributing a high-quality dataset to enhance future research in NLP.
...
TL;DR: The paper introduces L3Cube-MahaPOS, a gold-standard POS tagging dataset for Marathi, addressing the language's under-resourcing and unique computational challenges. It details the dataset's creation, benchmarks various models, and achieves promising accuracy rates, with the aim of advancing Marathi NLP research.


<details>
  <summary>Details</summary>
Motivation: There is a significant lack of annotated resources and evaluation benchmarks for Marathi, despite being a widely spoken language. Addressing this gap can enhance various NLP applications.

Method: The dataset comprises 32,354 manually annotated sentences from news text using a Universal Dependencies-aligned tagging scheme. A structured preprocessing pipeline ensures label consistency, and benchmarks are established across multiple model families including HMM and BiLSTM.

Result: The best-performing model achieved an 88.67% token-level accuracy and a macro-F1 score of 81.67% across 15 tag classes, demonstrating the dataset's effectiveness for POS tagging in Marathi.

Conclusion: The release of L3Cube-MahaPOS, along with guidelines and model checkpoints, aims to stimulate further research and development in Marathi NLP.
...
### [31] [EvoFlock: evolved inverse design of multi-agent motion](https://arxiv.org/abs/2606.25280)
*Craig Reynolds*

Main category: cs.NE

TL;DR: This paper presents a method to automatically tune multi-agent motion models, enhancing simulations of group behaviors like bird flocks and traffic using a genetic algorithm.


<details>
  <summary>Details</summary>
Motivation: The complexity of interactions within multi-agent systems leads to difficulties in adjusting parameters for desired group behavior.

Method: An inverse design approach is adopted, where group behavior is measured by a user-defined objective function and optimized via a genetic algorithm.

Result: The method successfully identifies parameter adjustments that yield desired behaviors, with the spacing between agents found to be pivotal for alignment in flocks.

Conclusion: Optimizing agent interactions through an objective function allows for more effective simulations of group behaviors in dynamic environments.
...
### [32] [Fifty Years of Specification Completeness: What Aviation Certification Tells AI Governance About Epoch Limits, Proof Surfaces, and the Structural Gap](https://arxiv.org/abs/2606.25120)
*Christo Zietsman*

Main category: cs.SE

TL;DR: The paper critiques current AI governance documents for lacking essential structural properties enforced in aviation software certification, proposing a framework to address these shortcomings.


<details>
  <summary>Details</summary>
Motivation: There is a gap in AI governance that fails to enforce structural properties established in aviation software certification, which are critical for ensuring the reliability and traceability of software systems.

Method: The paper maps key aspects of aviation software certification (DO-178C and DO-330) to identify deficiencies in AI governance documentation and proposes the PromptQ framework to integrate required structural properties.

Result: The analysis reveals that many AI governance documents (37%) do not meet the structural quality threshold necessary for effective governance, highlighting a critical flaw in current practices.

Conclusion: A framework built on aviation's structural requirements can provide a necessary foundation for improving the quality and reliability of AI governance documents.
...
### [33] [LLM4MTLs: Automated Generation and Empirical Evaluation of Model Transformation Languages](https://arxiv.org/abs/2606.25193)
*Bowen Jiang,Nathan Hagel,Haowei Cheng,Benedikt Jutz,Arne Lange,Weixing Zhang,Rahul Sharma,Ralf Reussner,Anne Koziolek*

Main category: cs.SE

TL;DR: This paper introduces LLM4MTLs, an automated workflow for enhancing LLM-generated model transformation languages (MTLs) through effective prompting strategies and an empirical evaluation across different MTLs and language models.


<details>
  <summary>Details</summary>
Motivation: The complexity of developing correct model transformations motivates the use of LLMs for MTL code generation, addressing challenges with syntax and semantics due to limited training data.

Method: The workflow explores combinations of few-shot prompting, grammar prompting, and helper method inclusion for generating MTL code, evaluated through syntactic and semantic metrics across four MTLs and three LLMs.

Result: Few-shot prompting improves syntactic quality in MTL code, while semantic correctness varies by language. Grammar prompting stabilizes generation but is context-dependent, and helper methods can enhance outcomes.

Conclusion: The choice of LLM affects syntactic correctness in certain MTLs, but has limited influence on semantic correctness, suggesting the need for tailored prompting strategies.
...
### [34] [Tensor-Based Batch Fuzzing with Adaptive Perturbation Scaling for Deep Neural Networks](https://arxiv.org/abs/2606.25239)
*Guanqin Zhang,Yulei Sui*

Main category: cs.SE

TL;DR: This paper introduces a novel framework for batch fuzzing of deep neural networks (DNNs) that enhances testing efficiency and exploration of input spaces by adapting perturbation sizes based on specification-defined ranges, achieving substantially higher throughput and more effective counterexample exploration than traditional methods.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of model reliability assessment in deep neural networks, particularly in safety-critical applications, and to enhance the efficiency of existing fuzzing methodologies which suffer from limitations in input exploration and throughput.

Method: A specification-aware batch fuzzing framework with adaptive perturbation scaling is developed. It derives mutation step sizes based on the feasible ranges defined by the specifications and processes multiple specification instances in one iteration, embedding input constraints and output checks as non-trainable layers in the model.

Result: The framework outperforms traditional fuzzing methods by achieving up to 40X higher throughput and identifying 4X more violations on benchmarks involving six networks and over 400 specifications.

Conclusion: The proposed fuzzing framework significantly improves the effectiveness of specification-guided testing for deep neural networks, facilitating better exploration of heterogeneous input spaces and increasing the reliability assessment of DNNs.
...
### [35] [How Do Developers Maintain and Evolve Their Agents' Instructions? An Empirical Study](https://arxiv.org/abs/2606.25257)
*Gianmario Voria,Alfonso Cannavale,Andrea De Lucia,Yutaro Kashiwa,Gemma Catolino,Fabio Palomba*

Main category: cs.SE

TL;DR: The paper investigates the evolution of Agent Context Files (ACFs) in autonomous coding, focusing on their classification, impact on code quality, and temporal patterns during development.


<details>
  <summary>Details</summary>
Motivation: The shift towards AI-driven software development raises questions about governance and control, necessitating a study of ACFs to understand their maintenance and evolution.

Method: The research employs a large-scale mining study of repositories, analyzing ACF changes at the commit level and their association with code quality metrics.

Result: The study aims to classify ACF changes and analyze their impact on code quality, revealing patterns and informing future ACF designs.

Conclusion: Findings will enhance understanding of ACF evolution and govern the use of autonomous coding agents effectively.
...
### [36] [Semantic Code Clone Detection: Are We There Yet?](https://arxiv.org/abs/2606.25272)
*Zhiwei Xu,Weixian Deng,Xuyang Liu,Xiaolin Peng,Jiabao Gao,Tian Qiu,Hai Wan,Xibin Zhao*

Main category: cs.SE

TL;DR: This paper investigates the generalizability of state-of-the-art semantic code clone detectors, revealing performance degradation in real-world scenarios despite high benchmark results.


<details>
  <summary>Details</summary>
Motivation: The study aims to determine whether the high performance of semantic clone detectors in benchmark settings is genuine or due to dataset-specific patterns.

Method: The authors propose a clone operator framework with eight transformation operators to create semantically equivalent Type-4 clone instances and evaluate 11 detectors on the BigCloneBench dataset.

Result: The evaluation shows substantial performance degradation for all detectors when applied to real-world instances, indicating reliance on shortcut learning.

Conclusion: The findings highlight the limited generalizability of SOTA semantic code clone detectors and suggest directions for future research.
...
### [37] [LibEvoBench: Probing Temporal Knowledge Stratification in Code Generation Models](https://arxiv.org/abs/2606.25402)
*Daniele Cipollone,Sergey Titov,Maliheh Izadi,Egor Bogomolov,Arie van Deursen*

Main category: cs.SE

TL;DR: This paper introduces LibEvoBench, a benchmark to evaluate language models' ability to handle multiple versions of Python library APIs, revealing significant limitations in current models' version-specific reasoning.


<details>
  <summary>Details</summary>
Motivation: The need for language models to maintain knowledge of multiple, evolving library API versions in large software projects creates a challenge for effective code generation.

Method: LibEvoBench is introduced as a multi-task benchmark that assesses performance across multiple library versions, coupled with the Software Evolution Understanding Score (SEUS) metric for evaluating model consistency.

Result: State-of-the-art models exhibit version-oblivious behavior, performing worse on evolving API tasks while remaining stable on non-evolving APIs; specifying a target version does not improve outcomes, yet relevant documentation enhances accuracy.

Conclusion: Current training paradigms are inadequate for version-specific reasoning in code generation, indicating the necessity for new approaches to incorporate temporally grounded knowledge.
...
### [38] [Project-wise Comparison of Software Birthmarks Using Weighted Partial Similarity](https://arxiv.org/abs/2606.25418)
*Nikolay Fedorov,Akito Monden,Hiroki Inayoshi,Haruaki Tamada,Masateru Tsunoda*

Main category: cs.SE

TL;DR: This paper presents a project-level framework for detecting code reuse through software birthmarks, addressing challenges of partial reuse and incidental similarity with a novel weighting scheme and partial similarity method, demonstrating improved performance over existing techniques.


<details>
  <summary>Details</summary>
Motivation: To enhance the detection of code plagiarism and independent software development at the project level, particularly given the inadequacies of module-level similarity measures in scenarios of partial reuse.

Method: The authors propose a framework for project-wise birthmark comparison that aggregates module-level similarities symmetrically, complemented by a weighting scheme for larger modules and a partial similarity method focusing on highly similar pairs.

Result: Evaluation across 35 open-source Java projects shows the new methodology consistently outperforms existing approaches, particularly in identifying partial code reuse while reducing false positives from incidental similarities.

Conclusion: The proposed framework and mechanisms provide a robust means of detecting project-level code reuse, thus contributing to the reliability of software birthmark technology.
...
### [39] [Unlocking Model Potentials Through Adaptive Multi-Agent Scaffolding for Efficient Issue Resolution](https://arxiv.org/abs/2606.25514)
*Yang Chen,Aliya Ahmad,Yiheng Zhou,Reyhaneh Jabbarvand*

Main category: cs.SE

TL;DR: icat-agent is a decentralized multi-agent system that improves bug resolution efficiency through event-based messaging, outperforming existing models in accuracy and cost-effectiveness.


<details>
  <summary>Details</summary>
Motivation: To address challenges with ambiguous bug descriptions and inefficient context management in software debugging workflows.

Method: Introduces icat-agent, which utilizes event-based message passing instead of shared context, along with a rubric-based issue quality check to guide parallel patching and validation processes.

Result: icat-agent outperforms existing systems such as SWE-agent and Claude Code across various difficulty levels, showing improvement in resolution rates and significantly reducing costs per instance.

Conclusion: icat-agent reveals the potential of existing models when supported by a robust scaffolding, achieving a superior problem resolution rate compared to current best practices.
...
### [40] [The impact of artificial intelligence on enterprise software user roles](https://arxiv.org/abs/2606.25525)
*Isabel Unger,Elizangela Valarini,Martin Schrepp,Nina Hollender,Gabriela Rocha,Erik Bertram*

Main category: cs.SE

TL;DR: The study analyzes the impact of AI on software development roles within SAP's BTP, revealing shifts in responsibilities and the need for updated frameworks and governance.


<details>
  <summary>Details</summary>
Motivation: To understand how AI is reshaping tasks and roles in software development, particularly in the context of SAP's Business Technology Platform.

Method: A qualitative approach using expert interviews (n=20) and a participatory workshop (n=24) to gather insights from professionals in the field.

Result: Findings indicate a significant change in development responsibilities due to increased automation, enhanced collaboration with AI, and reliance on agentic AI systems.

Conclusion: The study emphasizes the need for revised role frameworks, governance structures, and design strategies to accommodate the evolving landscape of AI in enterprise software development.
...
### [41] [Evaluating LLMs on Real-World Software Performance Optimization](https://arxiv.org/abs/2606.25530)
*Ezgi Sarıkayak,Wenchao Gu,Hesham Ghonim,Chunyang Chen*

Main category: cs.SE

TL;DR: SWE-Pro benchmark reveals LLMs' shortcomings in software optimization compared to expert implementations, with negligible gains in runtime and memory.


<details>
  <summary>Details</summary>
Motivation: To address the lack of realistic benchmarks for software performance optimization that consider trade-offs, noise, and variability found in real-world codebases.

Method: Introduction of SWE-Pro, a repository-level benchmark using 102 expert optimizations, incorporating parameterized tests for runtime, memory, and TWMU under varying conditions.

Result: Current LLMs show minimal improvements in runtime and memory optimization, while expert optimizations yield significant performance enhancements.

Conclusion: There is a notable performance gap between LLMs and expert engineers, highlighting the limitations of LLMs in achieving expert-level software performance optimizations.
...
### [42] [On the Viability of Requirements Generation From Code: An Experience Report](https://arxiv.org/abs/2606.25550)
*Alexander Korn,Jone Bartel,Max Unterbusch,Andreas Vogelsang*

Main category: cs.SE

TL;DR: This study explores using Large Language Models to generate requirements from source code, revealing shortcomings in generating non-implemented requirements and ensuring quality, ultimately suggesting the need for human oversight in dataset generation.


<details>
  <summary>Details</summary>
Motivation: Address the lack of datasets pairing source code with corresponding requirements in Requirements Engineering.

Method: Utilized an LLM-based and RAG-supported approach to generate requirements and verify their status with human oversight, while introducing synthetic requirements smells.

Result: Encountered issues where LLMs failed to generate reliable non-implemented requirements, produced low-quality requirements, and could not effectively introduce synthetic requirements smells, with inadequate detection of such smells by LLMs or individual human assessors.

Conclusion: Generation of code-to-requirements datasets via LLMs is currently unfeasible without significant human supervision, highlighting the need for improved quality assurance in the process.
...
### [43] [IntentTester: Intent-Driven Multi-agent Framework for Cross-Library Test Migration](https://arxiv.org/abs/2606.25588)
*Yi Gao,Ziyuan Zhang,Xing Hu,Xiaohu Yang,Xin Xia*

Main category: cs.SE

TL;DR: IntentTester is a multi-agent framework that enables intent-driven test reuse by abstracting tests into a Test Description Language and aligning them with semantically related entities, allowing for successful cross-library and cross-language migration of unit tests.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of test reuse across libraries and languages, as existing migration techniques struggle with divergent designs and lead to non-executable tests.

Method: IntentTester abstracts tests into a language-agnostic Test Description Language (TDL), aligns them with related entities in a repository graph, and synthesizes executable tests through LLM-guided reasoning and iterative validation.

Result: IntentTester generated 2,776 syntactically correct tests with an 85% correctness rate, outperforming baselines that achieved 51% and 43%. 2,410 tests executed successfully, yielding a 74% effectiveness rate, and some tests revealed previously unknown defects.

Conclusion: Intent-driven migration enhances the practicality of test reuse across libraries and languages, improving test quality and revealing implementation flaws.
...
### [44] [CodeChat-Eval: Evaluating Large Language Models in Multi-Turn Code Refinement Dialogues](https://arxiv.org/abs/2606.25747)
*Guoxiang,Guo,Kla Tantithamthavorn,Neelofar Neelofar,Yuanyuan Qi,Aldeida Aleti*

Main category: cs.SE

TL;DR: The paper presents CodeChat-Eval, an evaluation framework that addresses the challenge of assessing functional correctness in multi-turn code refinement dialogues for Large Language Models (LLMs).


<details>
  <summary>Details</summary>
Motivation: To evaluate whether LLMs can maintain functional correctness during multi-turn code refinement, a setting largely overlooked in existing benchmarks.

Method: The authors developed CodeChat-Eval, which creates evaluation sessions from multi-turn refinement dialogues using a dynamic instruction selection algorithm.

Result: An empirical study revealed a significant decline in functional correctness, with LLMs showing decreases between 19.2% to 69.2% across different models during multi-turn refinements.

Conclusion: LLMs face challenges in maintaining functional correctness during multi-turn dialogues, necessitating new benchmarks for evaluating refinement that preserves functionality.
...
### [45] [Automated Detection of Configuration-Specific Security Vulnerabilities via Patch Analysis](https://arxiv.org/abs/2606.25863)
...
TL;DR: The paper presents PatchLens, a tool that identifies configuration-specific vulnerabilities in C/C++ systems by formalizing the Vulnerability Impact Condition (VIC) to help automate CVE enrichment and improve vulnerability assessments.


<details>
  <summary>Details</summary>
Motivation: The need for improved identification and assessment of vulnerabilities in highly configurable software systems.

Method: PatchLens utilizes static analysis to match AST-level patch hunks with source-level presence conditions, enhanced by lightweight analysis of build systems to derive VICs.

Result: Evaluated on 1,192 Linux kernel, 289 FFmpeg, and 100 PHP patches, yielding compact VICs and revealing that only a small proportion of vulnerabilities are system-wide, which also correlate to higher CVSS scores.

Conclusion: PatchLens aids in automating CVE descriptions, and its applications can significantly enhance CI processes, vulnerability triage, testing, and risk assessment in configurable software environments.
...
### [46] [LLM-Based Discovery of Latent Requirements from Stakeholder Conversations: Preliminary Results from Industry](https://arxiv.org/abs/2606.25867)
*Mithila Sivakumar,Martin Lochner,Shiva Nejati,Mehrdad Sabetzadeh*

Main category: cs.SE

TL;DR: LENS is a method for extracting both explicit and implicit requirements from stakeholder interviews using LLMs, achieving high accuracy and perceived value in its results.


<details>
  <summary>Details</summary>
Motivation: To address the gap in identifying latent requirements that stakeholders may not articulate directly during interviews.

Method: LENS analyzes interview transcripts, extracts explicit requirements, and infers latent ones by reasoning over stakeholder statements and contextual organizational information.

Result: LENS achieves an F1-score of 84.4% for explicit requirements extraction; 75% of inferred latent requirements are deemed useful by domain experts.

Conclusion: LENS effectively captures both explicit and implicit requirements, enhancing the requirements elicitation process.
...
### [47] [The Web4 Agent Economy: A Large-Scale Empirical Study of the Landscape, Challenges, and Opportunities](https://arxiv.org/abs/2606.25876)
*Yuhan Jin,Shuohan Wu,Chong Chen,Lingfeng Bao,Xiaohu Yang,Jiachi Chen*

Main category: cs.SE

TL;DR: The paper examines the emerging Web4 ecosystem driven by autonomous agents, identifying their operational challenges and the necessity of robust infrastructure to support growth.


<details>
  <summary>Details</summary>
Motivation: To explore the largely underinvestigated ecosystem of Web4 agents and their operational realities, especially in the face of rapid growth and emerging protocols.

Method: Conducted a large-scale empirical study analyzing 99,448 multi-chain identity registrations, over 317 million transaction logs, 341 MCP project source codes, and 349 GitHub issues.

Result: The study finds an active machine-to-machine payment economy but highlights vulnerabilities in identity practices, cross-environment functionalities, and payment interoperability.

Conclusion: There is a significant gap between the growth of the Web4 agent economy and its fragile infrastructure, necessitating improvements for a secure and effective ecosystem.
...
### [48] [The Dependency Black Hole](https://arxiv.org/abs/2606.25949)
*Ehsan Zabardast,Bhuwan Paudel,Javier Gonzalez-Huerta*

Main category: cs.SE

TL;DR: The paper studies dependencies in microservice architectures, proposing a 'dependency black hole' theory based on a case study of 267 microservices, highlighting a core of tightly coupled services and suggesting hypotheses for dependency evolution.


<details>
  <summary>Details</summary>
Motivation: To understand the concentration of dependencies in large microservice systems, which contradicts the intended loose coupling, and to devise ways to manage these dependencies effectively.

Method: An exploratory industrial case study involving the analysis of compile-time, run-time, and task dependencies among 267 microservices, validated through iterative discussions with practitioners.

Result: Identification of a recurring macro-structure in the dependency network resembling a black hole, with a core of dependency magnets and outer regions of loosely connected services, leading to the formulation of the dependency black hole theory.

Conclusion: The dependency black hole theory provides insights into the growth of dependencies and governance of microservices, with hypotheses to explain dependency concentration and implications for further research.
...
### [49] [Helpful or Harmful? Evaluating LLM-Assisted Vulnerability Patching via a Human Study](https://arxiv.org/abs/2606.25973)
*Giulian Biolo,Michael Tezza,Yuanjun Gong,Fabio Massacci*

Main category: cs.SE

TL;DR: The paper investigates the effectiveness of LLM-assisted vulnerability remediation compared to manual debugging, analyzing speed and efficacy in real-world scenarios.


<details>
  <summary>Details</summary>
Motivation: To assess if LLM assistance can efficiently patch software vulnerabilities, despite concerns over potential hallucinations or insecure code.

Method: A controlled experiment with a Balanced Crossover design, including a WebApp for code execution and hidden Ghost Tests to validate patch integrity.

Result: The experiment evaluates remediation speed, efficacy in functionality and security tests, and participant perception. Pilot study findings inform the main study.

Conclusion: This research will enhance understanding of LLM's role in vulnerability remediation and its implications for software security.
...
### [50] [EnerInfer: Energy-Aware On-Device LLM Inference](https://arxiv.org/abs/2606.23001)
*Bohua Zou,Nian Liu,Binqi Sun,Matteo Mascherin,Debayan Roy,Yutao Liu,Yu Peng,Ning Jia,Haibo Chen*

Main category: cs.SE

TL;DR: EnerInfer framework optimizes on-device LLM inference by balancing energy efficiency and thermal performance, achieving significant energy savings without compromising quality of experience.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance on-device LLM inference for better energy efficiency and thermal management, addressing the limitations of current systems that prioritize speed over efficiency and thermal comfort.

Method: EnerInfer utilizes disaggregated model-structure-aware predictions and online feedback to optimize energy and thermal performance, adapting configurations based on runtime conditions and predicting throughput and power for various NPU/DDR settings.

Result: EnerInfer achieves energy efficiency improvements of up to 65%, 12%, and 24% on phones, laptops, and development boards, respectively, while maintaining quality of experience.

Conclusion: The proposed framework demonstrates that on-device LLM inference can leverage configuration slack to optimize performance without sacrificing user experience, suggesting a shift in how such systems are designed.
...
### [51] [MANGO: Automated Multi-Agent Test Oracle Generation for Vision-Language-Action Models](https://arxiv.org/abs/2606.24815)
*Pablo Valle,Shaukat Ali,Aitor Arrieta,Lionel Briand*

Main category: cs.SE

TL;DR: MANGO is a framework that automates the generation of fine-grained oracles for testing Vision-Language-Action robotic systems, addressing limitations in manual oracle construction.


<details>
  <summary>Details</summary>
Motivation: To overcome the challenges of manually constructed symbolic test oracles for VLA models, which are costly, require domain expertise, and offer limited insights into task execution.

Method: MANGO creates a library of atomic tasks, generates simulator-grounded oracle definitions, and produces executable fine-grained oracles by breaking down complex tasks into atomic actions with associated oracles, using collaborative agents for iterative refinement.

Result: MANGO demonstrates similar failure detection capabilities as traditional symbolic oracles while providing enhanced localization and diagnostic information, validated through evaluations on specific benchmarks.

Conclusion: The study validates the feasibility and effectiveness of automating test oracle generation for VLA-enabled robots, improving scalability and insight into robotic task performance.
...
### [52] [Toward Self-Evolution-Ready Workflow Harnesses: A Reversible Migration Path and Convertibility Taxonomy for Expert LLM Pipelines](https://arxiv.org/abs/2606.24598)
*Yimo Lin,Zhen Zhang,Yibin Li*

Main category: cs.SE

TL;DR: This paper introduces a migration framework for adapting static expert-validated workflows into dynamic systems that can respond to feedback.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current expert-validated workflows that do not adapt and the challenges of migrating existing legacy systems.

Method: The authors propose a reversible migration path using a Strangler-Fig approach, refactoring workflows into composable stages based on a three-tier convertibility taxonomy that assesses workflow readiness.

Result: The framework enhances the adaptability of legacy workflows by providing a structured method for their migration to more flexible systems.

Conclusion: This migration strategy facilitates the transformation of static workflows into dynamic, adaptable systems that can better utilize domain knowledge.
...
### [53] [Programmable Probabilistic Computer with 1,000,000 p-bits](https://arxiv.org/abs/2606.25313)
...
TL;DR: This paper presents a scalable architecture for networking FPGAs into a programmable one million p-bit probabilistic computer for Ising model applications, overcoming limitations of single-chip systems.


<details>
  <summary>Details</summary>
Motivation: The need for hardware accelerators in sampling and optimizing Ising models with improved capacity and memory bandwidth.

Method: Networking multiple FPGAs to create a large-scale Ising machine capable of performing Gibbs sampling quickly while managing memory efficiently.

Result: The distributed machine operates at over a trillion flips per second with effective boundary state management, demonstrating a scalable architecture and identifying a critical timing ratio for performance.

Conclusion: Findings provide a universal guideline for building larger probabilistic computers while revealing a throughput-accuracy tradeoff in partitioned systems.
...
### [54] [CrossPool: Efficient Multi-LLM Serving for Cold MoE Models through KV-Cache and Weight Disaggregation](https://arxiv.org/abs/2606.24506)
*Zhuoren Ye,Tianyu Wo,Dinghao Xue,Mingming Zhang,Yuchen Teng,Chunming Hu,Renyu Yang*

Main category: cs.DC

TL;DR: CrossPool is a serving engine designed to optimize GPU memory usage for cold Mixture of Experts (MoE) models by separating weight and KV-cache pools, thereby improving response times for long-context requests.


<details>
  <summary>Details</summary>
Motivation: To address GPU memory inefficiencies caused by cold MoE models that lead to underutilization of KV-cache and static weight storage in a monolithic memory pool.

Method: CrossPool separates FFN weights and KV-cache into distinct GPU memory pools and implements a KV-cache planner, a pipeline scheduler for hidden-state transfers, and persistent kernels for reduced CPU-GPU overhead.

Result: CrossPool significantly enhances GPU memory utilization and response time, outperforming existing systems by reducing P99 TBT by up to 10.4 times.

Conclusion: The separation of memory pools in CrossPool allows for more efficient handling of bursty long-context requests, providing a better solution for serving cold MoE models.
...
### [55] [Bridging the Post-discharge Gap: A Traceable Multi-agent Framework for Safe and Continuous Care](https://arxiv.org/abs/2606.25334)
...
TL;DR: Healink is a multi-agent AI framework designed to enhance post-discharge clinical follow-up by providing traceable, prescription-grounded responses that improve patient care while overcoming traditional challenges in healthcare continuity.


<details>
  <summary>Details</summary>
Motivation: To address issues in post-discharge follow-up caused by workforce shortages, fragmented patient histories, and information silos that can lead to health risks.

Method: The Healink framework features a triage routing mechanism, a memory enhancement module with a relational database, and a retrieval-augmented generation engine to ensure effective inter- and intra-patient matching and prevent drug conflicts.

Result: Healink showed superior performance compared to human physicians in authoritativeness and clinical safety during evaluations on 400 continuous and 85 complex follow-up cases, as well as on the webMedQA benchmark.

Conclusion: Healink creates a scalable and safe approach for intelligent patient management, significantly improving healthcare outcomes by providing a structured, transparent evidence chain.
...
### [56] [Rate-Aware Quantum-Inspired Trajectory Learning for Interference-Limited Multi-UAV Networks](https://arxiv.org/abs/2606.25480)
*Khaoula Khaled,Muhammad Afaq,Ali Arshad Nasir,Zeeshan Kaleem*

Main category: cs.MA

TL;DR: The paper presents RA-QAGC, a scheme for optimizing UAV trajectory in interference-limited environments, achieving significant throughput improvements over existing methods.


<details>
  <summary>Details</summary>
Motivation: To address trajectory optimization challenges in UAVs under interference-limited conditions and large search spaces, which complicate real-time coordination.

Method: The Rate-Aware Quantum-Annealed Graph Condensation (RA-QAGC) integrates rate-aware graph abstraction with decentralized reinforcement learning for scalable UAV coordination.

Result: RA-QAGC achieved 59.4 Mbps total throughput and 23.9 Mbps priority-user throughput, outperforming existing schemes by approximately 15% and 34%, respectively.

Conclusion: The proposed scheme effectively balances network capacity and maintains QoS requirements in UAV communications.
...
### [57] [Manipulation Is Task-Dependent: A Multi-Axis, Multi-Environment Evaluation of Frontier LLMs](https://arxiv.org/abs/2606.25899)
*Adeeb Zaman,Erik Nordby,Fred Heiding*

Main category: cs.MA

TL;DR: This study assesses manipulative behavior in language models across various tasks, revealing that manipulation tendencies vary significantly across different environments and influencers.


<details>
  <summary>Details</summary>
Motivation: To investigate how manipulative behaviors manifest in language models across diverse scenarios, highlighting the inadequacy of current benchmarks that focus on single axes within isolated environments.

Method: An evaluation of six language models through 13,590 scenarios across six environments, measuring manipulation rates based on framing, incentive structure, and task difficulty.

Result: Models were ranked by manipulation rates, revealing low Spearman rank correlations (average $ρ= 0.055$) across environments, indicating inconsistency in manipulative behavior across different tasks.

Conclusion: The study emphasizes the necessity of multi-dimensional evaluations for understanding manipulative tendencies, as influences differ significantly based on the environment.
...
### [58] [Multi-Agent Goal Recognition with Team- and Goal-Conditioned Reinforcement Learning and Factorized Branch-and-Bound](https://arxiv.org/abs/2606.25978)
*Thiago Thomas,Gabriel de Oliveira Ramos,Felipe Meneguzzi*

Main category: cs.MA

TL;DR: MAGR-BB enhances multi-agent goal recognition by utilizing a shared team- and goal-conditioned policy within a branch-and-bound framework, achieving superior efficiency compared to exhaustive search.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of inferring agent teamwork and objectives solely from trajectory data in applications like drone surveillance and collaborative robotics, where hypothesis spaces are large.

Method: MAGR-BB employs a shared policy conditioned on teams and goals, integrated into a factorized branch-and-bound search to efficiently rank hypotheses.

Result: In tests on a multi-agent Blocksworld benchmark, MAGR-BB achieved the same top-ranked hypothesis as exhaustive search with significantly reduced hypothesis materialization and recognition time.

Conclusion: MAGR-BB is a more efficient approach to multi-agent goal recognition, maintaining effectiveness while greatly improving computational efficiency.
...
### [59] [Unprivileged Topology Certificates for Cloud GPU Attestation](https://arxiv.org/abs/2606.24934)
*Faruk Alpay,Taylan Alpay*

Main category: cs.CR

TL;DR: This paper introduces a software-only attestation method that allows cloud GPU tenants to verify the physical characteristics of their computing resources without direct access to the hardware.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable cloud GPU tenants to ascertain the identity and characteristics of the physical accelerators running their jobs, ensuring transparency and security in a cloud environment.

Method: The method involves a CUDA probe that measures latency metrics, generating a certificate containing sufficient statistics and evidence that can be verified without GPU access.

Result: The results show that the latency map acts as a stable fingerprint of the GPU, providing 100% accuracy in distinguishing different dies, and successfully reconstructing hardware topology, as well as verifying location within 44 km of the claimed datacentre using network landmarks.

Conclusion: The findings demonstrate that it is possible to securely validate cloud GPU identity, class, and approximate location without requiring privileged access, thereby enhancing trust in cloud computing services.
...
### [60] [Quantum-Resilient Decentralized AI Economies: Proof-of-Useful-Work and Post-Quantum Security](https://arxiv.org/abs/2606.24942)
*Connor Barbaccia,Sudip Vhaduri,Sayanton Dibbo*

Main category: cs.CR

TL;DR: This paper proposes a decentralized AI economy that rewards nodes for machine learning tasks instead of traditional Proof-of-Work hashing. It introduces a three-layer architecture and a closed-loop token economy to facilitate honest participation and addresses quantum security concerns.


<details>
  <summary>Details</summary>
Motivation: To replace the inefficient hashing used in Proof-of-Work blockchains with a system that rewards productive machine learning tasks, enhancing both economic value and security against quantum threats.

Method: The paper proposes a three-layer architecture for separating computation, validation, and economic coordination, formalized through a closed-loop token economy and a derived sufficient-stake condition for honest participation.

Result: The proposed model offers economic benefits by rewarding useful work and provides advantages in quantum security compared to traditional hashing methods and existing quantum algorithms like Grover's and Shor's.

Conclusion: The paper concludes that a useful-work consensus mechanism not only enhances the economic efficiency of blockchains but also mitigates risks associated with quantum computing.
...
### [61] [What Does It Mean to Break a Distillation Defense?](https://arxiv.org/abs/2606.25059)
*Lena Libon,Pura Peetathawatchai,Michael Aerni,Daniel Paleka,Florian Tramèr*

Main category: cs.CR

TL;DR: This paper critiques the lack of a standardized threat model for output perturbation defenses against distillation attacks on black-box LLMs, proposing a framework for evaluation based on attacker capabilities.


<details>
  <summary>Details</summary>
Motivation: To address vulnerabilities in black-box LLMs and the inadequacies of current defenses that do not offer a consensus threat model, which could lead to ineffective security measures.

Method: A threat model framework is proposed that categorizes attackers based on query budget, data budget, and interface profile for API interactions. The model is explored through a case study on antidistillation sampling.

Result: The effectiveness of antidistillation defenses varies significantly based on the underlying threat model assumptions, highlighting the need for explicit definitions of attacker capabilities.

Conclusion: Future research and regulatory frameworks should incorporate well-defined threat models to enhance security and trust in distillation defenses.
...
### [62] [SoK: AI Secure Code Generation: Progress, Pitfalls, and Paths Forward](https://arxiv.org/abs/2606.25195)
*Rupam Patir,Keyan Guo,Haipeng Cai,Hongxin Hu*

Main category: cs.CR

TL;DR: This paper examines the effectiveness of AI systems in generating secure code, identifying progress, failings, and future directions in the field.


<details>
  <summary>Details</summary>
Motivation: To address the shortcomings and understand the capabilities of AI systems in secure code generation, as well as to identify paths for improvement.

Method: The study introduces a three-level framework assessing AI's understanding of secure coding, its code-generation capabilities, and the gaps between the two. This framework is applied to various models and benchmarks for analyzing security at multiple levels.

Result: The research finds a strong correlation between models' understanding of security principles and code-level outcomes, revealing significant gaps where models understand principles but fail to implement them correctly in code.

Conclusion: The findings provide insights into the current state of AI secure code generation and recommend focused paths for advancing the field through principle-based methodologies.
...
### [63] [Homomorphic Encryptions for Privacy Preserving Vision](https://arxiv.org/abs/2606.25216)
*Preey Shah,Rohan Virani,Sanjari Srivastava*

Main category: cs.CR

TL;DR: This paper presents a method for performing privacy-preserving inference tasks in Computer Vision using fully homomorphic encryption (FHE), which allows operations on encrypted data. The study focuses on adapting convolutional neural networks (CNNs) to classify encrypted images with minimal accuracy loss compared to traditional methods.


<details>
  <summary>Details</summary>
Motivation: Organizations face legal constraints in sharing sensitive data, hindering their use of cloud-based ML solutions. This project aims to address privacy concerns in inference tasks by exploring FHE techniques.

Method: The paper modifies conventional CNN layers to work with fully homomorphic encryption, leveraging Microsoft SEAL for encryption. The study uses various datasets, starting with MNIST and extending to more complex datasets like CIFAR-10, to evaluate the performance of these adapted CNNs.

Result: The research successfully demonstrated minimal drops in classification accuracy when using encrypted data across multiple datasets, indicating effective adaptation of CNNs for privacy-preserving predictions.

Conclusion: The findings confirm that CNNs can be effectively trained for classifying encrypted images with comparable accuracy to non-encrypted counterparts, thereby providing a viable solution for privacy-preserving machine learning in sensitive data scenarios.
...
### [64] [Sponsored Group Signature and its Application to Privacy-preserving Guest Access in Smart Environments](https://arxiv.org/abs/2606.25248)
*Sepideh Avizheh,Reihaneh Safavi-Naini,Shiwei Sun*

Main category: cs.CR

TL;DR: The paper introduces SPonsored Group Signature (SPGS), a two-level group signature scheme that allows one group member to sponsor another while ensuring accountability and anonymity, suitable for applications like guest access in smart environments.


<details>
  <summary>Details</summary>
Motivation: A flexible mechanism for privacy preserving access in smart environments is needed to enhance security and accountability when integrating new members into a group.

Method: The paper presents SPGS, where level 1 members can sponsor level 2 members, making their signatures linkable and allowing the sponsor to be accountable for their actions. The construction combines group signatures, commitment schemes, and zero-knowledge proofs.

Result: The SPGS scheme is formally defined, its security is established using a game-based approach, and an instantiation is provided. An application for guest access in smart buildings is demonstrated with the introduction of the Anonymous Guest Access Token (AGAT).

Conclusion: SPGS allows for efficient guest access while maintaining anonymity and accountability, showcasing its practical utility in smart environments.
...
### [65] [Decoupling Reconnaissance and Exploitation: Measuring the Capability Boundaries of LLM-Based Web Penetration Testing](https://arxiv.org/abs/2606.25332)
*Liwei Yu,Shuo Li,Ming Zhou,Ge Chu,Yan Guo*

Main category: cs.CR

TL;DR: This paper presents a two-stage decoupled evaluation framework for assessing Large Language Models in penetration testing, mitigating errors from reconnaissance failures and isolating exploitation performance.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of error cascading in existing black-box evaluations of LLMs for penetration testing, which can obscure their true exploitation capabilities.

Method: A two-stage evaluation framework is proposed that separates exploit execution from reconnaissance, utilizing ground-truth injection and ablation techniques across 70 web vulnerability testbeds.

Result: Evaluation of five open-source penetration-testing agents reveals a functional success rate of up to 90.0% with accurate vulnerability context, contrasted with a plateau of approximately 50.0% for autonomous reconnaissance due to parsing failures.

Conclusion: The study establishes a detailed benchmarking protocol and highlights distinct performance capabilities among different architectures, providing a foundation for future developments in automated offensive security.
...
### [66] [Can Trustless Agents Be Trusted? An Empirical Study of the ERC-8004 Decentralized AI Agent Ecosystem](https://arxiv.org/abs/2606.26028)
*Xihan Xiong,Zelin Li,Wei Wei,Qin Wang,William Knottenbelt,Zhipeng Wang*

Main category: cs.CR

TL;DR: This study empirically examines the ERC-8004 protocol for AI agents' trustworthiness across three chains, revealing significant flaws in its identity and reputation systems and providing recommendations for improvement.


<details>
  <summary>Details</summary>
Motivation: To evaluate the effectiveness and trustworthiness of the ERC-8004 protocol's identity and reputation mechanisms in decentralized AI agent transactions.

Method: An empirical study analyzing on-chain Identity and Reputation data, off-chain files, and x402 payment transactions across Ethereum, BNB Smart Chain, and Base, from protocol inception to three years post-deployment.

Result: The study finds that most identity registrations are inactive, the current reputation system lacks reliability and is vulnerable to manipulation, and a high percentage of reviewers engage in Sybil behavior, undermining the trustworthiness of feedback.

Conclusion: The findings indicate critical design flaws in the ERC-8004 protocol, leading to recommendations for future protocol enhancements to improve trust and usability in AI agent economies.
...
### [67] [General Techniques for Reducing Key-Switching Overhead in Privacy-Preserving Two-Party Transformer Inference](https://arxiv.org/abs/2606.25349)
*Wenshao Yang,Zhenhua Liu,Dongdong Yao*

Main category: cs.CR

TL;DR: The paper presents a method to optimize secure two-party Transformer inference by reducing key-switching overhead in Fully Homomorphic Encryption (FHE) through preprocessing-assisted techniques, trade-offs between storage and communication, and a novel fused key-switch approach.


<details>
  <summary>Details</summary>
Motivation: Existing FHE optimizations rely on packing-specific algorithms that limit their applicability and suffer from high computational costs due to key switching during secure inference.

Method: The authors propose a preprocessing-assisted approach to divide attention into precomputable parts and online interactions, and introduce storage-communication trade-off techniques along with a fused key-switch method to minimize computation costs.

Result: The analytical evaluations indicate significant reductions in online key-switch overhead and provide flexible trade-offs between storage needs and communication demands in secure inference scenarios without changing current packing methods.

Conclusion: The proposed techniques improve the efficiency of secure Transformer inference, demonstrating potential for broader applications in FHE-based systems.
...
### [68] [Representation Matters: An Empirical Study of Program Representations for LLM Vulnerability Reasoning](https://arxiv.org/abs/2606.25356)
*Andrew Stoltman,Johnathan Tang,Haipeng Cai*

Main category: cs.CR

TL;DR: The paper introduces RepBench, an empirical benchmark for evaluating how different program representations affect LLM-based vulnerability detection, finding that structural representations lead to better accuracy than raw code.


<details>
  <summary>Details</summary>
Motivation: To understand how program structures and semantics influence LLM-based reasoning for vulnerability detection, challenging the assumption that more source context improves model performance.

Method: RepBench compares multiple program representations, including raw source, ASTs, CFGs, PDGs, their combinations, and enriched PDGs using a curated dataset of Joern-based test cases.

Result: The best representation, AST+PDG, achieved 83.2% accuracy in vulnerability detection, while raw source code yielded only 53.5%. Graph-only prompts were more effective than source-inclusive prompts.

Conclusion: Selective structural representations improve LLM performance over raw code, suggesting that static analysis can enhance prompt construction for security-focused reasoning.
...
### [69] [Security and Privacy in Retrieval-Augmented Generation: Architectures, Threats, Defenses, and Future Directions for Building Trustworthy Systems](https://arxiv.org/abs/2606.25533)
*Balamurugan Palanisamy,G S S Chalapathi,Vikas Hassija,Rajkumar Buyya*

Main category: cs.CR

TL;DR: This survey examines privacy and security challenges in Retrieval-Augmented Generation systems, highlighting risks from integration of retrieval mechanisms with generative models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the emerging privacy and security issues in RAG systems that arise from their architecture and operational procedures.

Method: The paper reviews RAG systems across various paradigms (centralized, on-device, federated, and hybrid), providing a taxonomy of threat surfaces and analyzing different attack classes and potential defenses.

Result: The analysis reveals multiple security threats including membership inference, index inference, and more, while discussing available defenses and their implications on privacy and utility.

Conclusion: The study concludes by identifying open research challenges necessary to create more secure and trustworthy RAG systems for practical use.
...
### [70] [CrypFormBench: Benchmarking Formal Analysis Capability of Large Language Models for Cryptographic Schemes](https://arxiv.org/abs/2606.25561)
...
TL;DR: This paper presents CrypFormBench, a benchmark for assessing large language models (LLMs) in the context of cryptographic scheme analysis, identifying strengths and weaknesses while providing guidance for improvements.


<details>
  <summary>Details</summary>
Motivation: To address the labor-intensive nature of manual formal analysis in cryptography and evaluate the potential of LLMs as a viable alternative.

Method: The authors introduce CrypFormBench, a benchmark that examines LLM capabilities in five areas related to cryptographic schemes, using a comprehensive dataset of 700 instances from various formal verification languages.

Result: The evaluation of nine state-of-the-art LLMs indicated strong performance in interpretation and completion tasks, but limited effectiveness in generation, transformation, and correction, with the top performance achieving only 48.7 out of 100.

Conclusion: The benchmark serves as a tool for understanding LLM capabilities in cryptographic analysis and offers guidance to enhance their utility in producing tool-usable outputs.
...
### [71] [An Approach for a Supporting Multi-LLM System for Automated Certification Based on the German IT-Grundschutz](https://arxiv.org/abs/2606.25608)
*Lea Roxanne Muth,Marian Margraf*

Main category: cs.CR

TL;DR: This paper introduces a semi-automated system for BSI IT-Grundschutz certification using a MultiLarge Language Model and Hybrid Retrieval-Augmented Generation to improve efficiency and reduce costs amid growing certification demands.


<details>
  <summary>Details</summary>
Motivation: To tackle the challenges posed by the NIS2 directive, such as a shortage of specialists and high implementation costs, while ensuring quality in security certification processes.

Method: The approach integrates Large Language Models and Knowledge Graphs to assist in various stages of the certification process.

Result: The system enhances efficiency, reduces costs, and facilitates the management of security concepts to meet the rising need for certifications among newly affected businesses.

Conclusion: The proposed architecture effectively addresses the digital security challenges related to NIS2, streamlining the certification process.
...
### [72] [Probabilistic Agents in Deterministic Audits: Evaluating Multi-Agent Systems for Automated Audits Based on the German IT-Grundschutz](https://arxiv.org/abs/2606.25622)
*Lea Roxanne Muth,Marian Margraf*

Main category: cs.CR

TL;DR: The paper explores a Multi-Agent System for automating IT-Grundschutz certification, highlighting its enhancements and evaluating its performance in compliance tasks.


<details>
  <summary>Details</summary>
Motivation: To address the resource-intensive nature of IT-Grundschutz certification for SMEs amid the NIS-2 Directive requirements.

Method: The paper implements and evaluates a Multi-Agent System architecture with novel contributions including a Hypothesis-Verification Loop and a Decoupled Reasoning Pipeline.

Result: The system shows high efficacy in the Structural Analysis and Modeling phases but faces challenges in logical reasoning phases, revealing limitations in compliance rigor due to LLMs' probabilistic nature.

Conclusion: While the MAS improves automation and reduces manual effort, it still struggles with the stringent logical reasoning demands of IT-GS certification.
...
### [73] [Taxonomy of Risks on Automated Fact-Checking Systems Considering its Propagation](https://arxiv.org/abs/2606.25645)
*Jun Yajima,Tatsuya Oka,Takao Okubo*

Main category: cs.CR

TL;DR: The paper addresses the growing issue of fake news on social media and highlights the importance of automated fact-checking systems, while categorizing the associated risks.


<details>
  <summary>Details</summary>
Motivation: The proliferation of fake news on social networking services necessitates effective fact-checking mechanisms.

Method: The paper categorizes risks in automated fact-checking systems using a three-stage risk propagation approach, analyzing factors leading to hazards and potential harm, and applies this framework to assess the DEFAME system.

Result: The analysis identifies 32 specific risks in automated fact-checking systems and demonstrates that some risks elude conventional security assessment methods like STRIDE.

Conclusion: The categorized risks provide valuable insights for enhancing the safety and reliability of automated fact-checking systems.
...
### [74] [Tracing Target Answers in Poisoned Retrieval Corpora via Token Influence Attribution](https://arxiv.org/abs/2606.25721)
*Yan-Lun Chen,Pin-Yu Chen,Chia-Mu Yu,Ying-Dar Lin,Yu-Sung Wu,Wei-Bin Lee*

Main category: cs.CR

TL;DR: TRACE is a lightweight detection framework for identifying corpus poisoning attacks in RAG systems by tracing influential tokens without significant computational overhead.


<details>
  <summary>Details</summary>
Motivation: To address vulnerabilities in Retrieval-Augmented Generation (RAG) systems from corpus poisoning attacks that manipulate model outputs via malicious documents, while minimizing computational costs associated with existing detection methods.

Method: TRACE utilizes token influence attribution to identify frequent influential keywords in retrieved documents and conducts a secondary verification to ascertain their impact on model predictions.

Result: Experiments across three QA benchmarks and six LLMs show that TRACE achieves strong detection performance and reveals attacker-specified target answers effectively.

Conclusion: TRACE provides an efficient alternative for detecting corpus poisoning in RAG systems, emphasizing the balance between robust detection and computational efficiency.
...
### [75] [Shoot the Honey, Cloak the Player: Towards Zero-Runtime-Overhead Proactive Defense and Detection for Visual Game Cheating](https://arxiv.org/abs/2606.25734)
*Jianing Wang,Chuqi Zhang,Yuancheng Jiang,Adil Ahmad,Shanqing Guo*

Main category: cs.CR

TL;DR: AimTrap is introduced as an effective end-to-end defense against visual aimbots in FPS games, combining real-time protection and post-game detection using adversarial textures, with impressive evaluation results.


<details>
  <summary>Details</summary>
Motivation: The emergence of visual aimbots poses significant challenges to match integrity in FPS games, as existing defenses lack real-time protection and are ineffective against advanced cheating methods.

Method: AimTrap employs two adversarial texture techniques: Adversarial Camouflage Textures (ACT) to conceal real players from aimbots, and Adversarial Honeypot Textures (AHT) to attract aimbots to fake targets, culminating in a robust detection and attribution framework.

Result: In evaluations against a leading visual aimbot, AimTrap achieved an 85.1% success rate for ACT and 96.9% for AHT, demonstrating a significant reduction in false positives and minimal runtime overhead compared to previous methods.

Conclusion: AimTrap represents a comprehensive and efficient solution for defending against visual aimbots, enhancing security in FPS games through innovative adversarial techniques.
...
### [76] [RAS: Measuring LLM Safety Through Refusal Alignment](https://arxiv.org/abs/2606.25750)
*Chang-Chieh Huang,Yan-Lun Chen,Chia-Mu Yu,Wei-Bin Lee*

Main category: cs.CR

TL;DR: SafeVec offers a new white-box safety evaluation method for large language models (LLMs) that focuses on internal representations rather than output responses, yielding faster and more reliable safety measurements.


<details>
  <summary>Details</summary>
Motivation: Traditional output-level safety evaluations for LLMs are costly and subjective, leading to the need for a more efficient and reliable method to assess model safety.

Method: SafeVec extracts layer-wise refusal directions from a reference model, identifies stable areas where behaviors diverge, and scores target models based on their alignment with these refusal directions when prompted with unsafe inputs. This is summarized with the Refusal Alignment Score (RAS).

Result: SafeVec's RAS effectively differentiates LLMs based on safety alignment and correlates with the output-level attack success rate across various model families, while being significantly faster than traditional evaluation methods.

Conclusion: Refusal alignment is a promising approach to provide an efficient and compact signal for evaluating LLM safety in a white-box context.
...
### [77] [Color Matters: Trigger Color Affects Success in Federated Backdoor Attacks](https://arxiv.org/abs/2606.25858)
*Kavindu Herath,Joshua C. Zhao,Saurabh Bagchi*

Main category: cs.CR

TL;DR: The paper investigates a semantics-driven backdoor attack in federated learning, using natural visual accessories as triggers and analyzing their effectiveness based on trigger color and placement.


<details>
  <summary>Details</summary>
Motivation: To understand vulnerabilities in federated learning systems due to backdoor attacks using semantic triggers, which can manipulate model behavior while maintaining performance on benign tasks.

Method: The authors propose a framework that uses visual accessories (like masks and sunglasses) as triggers, altering only their color. They study the impact of these triggers in a federated learning setup with both benign and malicious clients and analyze performance under different objectives.

Result: Experimentation revealed that trigger color influences attack success rates, with variations based on the target class (e.g., white triggers for blond class, black triggers for black class). This holds true even under robust aggregation conditions.

Conclusion: Trigger color is a critical factor in the effectiveness of semantic backdoor attacks in federated learning, impacting operation and persistence, highlighting the need for more secure models against such vulnerabilities.
...
### [78] [A Tattered Cloak of Invisibility: Measuring Anonymity Loss in Railgun on Ethereum](https://arxiv.org/abs/2606.25926)
*Kanan Huseynov,Ali Shahzaib,István András Seres,János Tapolcai*

Main category: cs.CR

TL;DR: This paper investigates privacy leakage in cryptocurrency mixers, specifically Railgun on Ethereum, highlighting how user behavior and transaction patterns impact anonymity, despite robust cryptographic protocols.


<details>
  <summary>Details</summary>
Motivation: To analyze how user behavior and transaction patterns compromise privacy in blockchain financial systems, particularly in cryptocurrency mixers.

Method: The study examines five sources of behavioral leakage affecting anonymity in Railgun: timing patterns, address reuse, transaction graph proximity, amount fingerprints, and knapsack-type matches. Heuristic estimates of user linkages between deposit and withdrawal transactions were computed.

Result: The heuristics linked 17.65% of Railgun withdrawals to deposits, and a knapsack solver revealed a median anonymity loss of 3.42 bits for withdrawals.

Conclusion: The findings demonstrate that substantial anonymity loss occurs in privacy systems due to user behavior, informing safer practices and design principles for anonymity pools and mixers.
...
### [79] [BlowLive: Blow-Based Multi-Factor Biometrics with Liveness Detection and Revocability](https://arxiv.org/abs/2606.25998)
*Eyasu Getahun Chekole,Howard Halim,Daniël Reijsbergen,Jianying Zhou*

Main category: cs.CR

TL;DR: This paper presents BlowLive, a multi-factor biometric framework that combines blow-acoustic signals and facial biometrics to enhance security against spoofing and privacy concerns in biometric authentication systems.


<details>
  <summary>Details</summary>
Motivation: Existing biometric systems face limitations such as susceptibility to spoofing attacks, privacy leakage, accuracy issues due to variability, and challenges in revocation and long-term safeguarding of biometrics.

Method: BlowLive integrates blow-acoustic signals with facial biometrics, using advanced spectral feature extraction, multimodal fusion, and a fuzzy-extractor-based authentication scheme paired with a Doppler shift-based liveness detection mechanism.

Result: The framework demonstrates high authentication accuracy (99.56% for blow-acoustics, 100% for facial and fusion), robust liveness detection at 99.42%, and effective template protection and revocability in practical tests with 50 participants.

Conclusion: BlowLive offers a comprehensive and effective solution to the limitations of traditional biometric systems, ensuring high accuracy, security, usability, and long-term application.
...
### [80] [Do (Not) Tell Me About My Insecurities: Assessing the Status Quo of Coordinated Vulnerability Disclosure in Germany Amid New EU Cybersecurity Regulations](https://arxiv.org/abs/2606.25950)
*Sebastian Neef,Cenk Schlunke,Anne Hennig*

Main category: cs.CR

TL;DR: Study on CVD program adoption among DAX companies shows significant growth and challenges in implementation.


<details>
  <summary>Details</summary>
Motivation: Highlight the importance of CVD programs in enhancing IT security and enabling vulnerability reporting.

Method: Longitudinal study involving public information analysis and questionnaires sent to 40 DAX companies between 2023 and 2025.

Result: Adoption rates of CVD programs increased from 50% in 2023 to over 90% in 2025, with new programs and files introduced by several companies.

Conclusion: Findings provide insights for SMEs, help enhance security practices, and inform policymakers about industry challenges.
...
### [81] [Privacy Vulnerabilities of Attention Layers in Tabular Foundation Models and Protection of High-Risk Queries](https://arxiv.org/abs/2606.26021)
*Tânia Carvalho,Maxime Cordy*

Main category: cs.CR

TL;DR: This paper reveals that tabular foundation models using in-context learning can leak sensitive information through the attention mechanism, enabling effective Membership Inference Attacks (MIAs). The authors present AMIA, a novel attack method, and propose a defense inspired by $k$-anonymity principles to mitigate this vulnerability.


<details>
  <summary>Details</summary>
Motivation: To address privacy concerns posed by tabular foundation models employing in-context learning, where sensitive records may be included during inference.

Method: The authors introduce AMIA, a shadow-model-free attack that exploits transformer attention patterns. They also propose a defense method that targets high-risk queries based on AMIA scores to reduce information leakage.

Result: The AMIA attack shows increased success rates, with a 7.7% gain over traditional methods, particularly in low false-positive scenarios, and the proposed defense reduces leakage by 50% against AMIA and 25% against confidence-based attacks, while minimally affecting performance.

Conclusion: Fine-tuning can exacerbate privacy risks, as it can make samples whose prediction confidence improves more vulnerable to MIAs, revealing the impact of model training processes on privacy.
...
### [82] [Time-dependent weighted directed networks of cryptocurrency interaction from high-frequency returns](https://arxiv.org/abs/2606.25466)
*Shubhangam Shukla,Mahesh Peyyala,Abhijit Chakraborty*

Main category: q-fin.TR

TL;DR: The study analyzes the dynamic structure of cryptocurrency interactions using a network framework based on high-frequency data from 2020-2025, revealing significant influence disparities among assets.


<details>
  <summary>Details</summary>
Motivation: To understand how interactions among cryptocurrencies evolve over time and identify influential assets in the market.

Method: Construct directed and weighted networks from Granger causal relationships in cryptocurrency log-returns, analyzing the structure and dynamics of these networks.

Result: Identified heavy-tailed distribution of normalized returns, significant heterogeneity in links and nodes, and established a ranking of cryptocurrencies based on their influence, with Ethereum as the most influential and Bitcoin's importance declining.

Conclusion: The cryptocurrency market displays a competitive and unstable hierarchy of influence, with shifting dynamics among various assets.
...
### [83] [Hierarchical Graph Learning for Calendar Spread Strategies in Commodity Futures Markets](https://arxiv.org/abs/2606.25811)
*Yoonsik Hong,Diego Klabjan*

Main category: q-fin.TR

TL;DR: This paper proposes a hierarchical graph learning method for calendar spread strategies in commodity futures, addressing gaps in machine learning application in this area.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the need for learning-based methods for calendar spread strategies in futures markets and the importance of considering maturity-dependent relationships among commodities.

Method: The method involves a hierarchical graph learning approach that incorporates maturity-dependent interrelationships to predict futures price movements and convert predictions into calendar spread positions.

Result: Empirical results indicate that the proposed method outperforms benchmark models in prediction and trading performance on the Chicago Mercantile Exchange.

Conclusion: The study concludes that maturity-dependent interrelationships are crucial for accurate predictions and that the developed method is effective for statistical arbitrage in commodity futures.
...
### [84] [TabClean: Reusable LLM-Synthesized Programs for Tabular Data Cleaning](https://arxiv.org/abs/2606.25388)
*Yibo Wang,Riteng Zhang,Yinghao He,Yongye Su,Bharat Bhargava,Chunwei Liu*

Main category: cs.DB

TL;DR: TabClean is a model-training-free system that creates reusable cleaning programs for tabular data, improving efficiency and precision compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of cleaning dirty tabular data without the need for extensive model training or high resource consumption.

Method: TabClean profiles the input table, diagnoses issues, synthesizes Python transformation programs, validates these candidates using feedback, and creates reusable cleaning methods based on specific evidence-backed rules.

Result: TabClean demonstrates high precision and improved F1 scores across six benchmarks, outperforming rule-based, learning-based, and LLM-based systems while significantly reducing runtime and costs.

Conclusion: TabClean effectively minimizes setup and operational costs by compiling LLM reasoning into deterministic cleaning programs, facilitating easier deployment for data cleaning tasks.
...
### [85] [TokenMinds: Pretrained User Tokens and Embeddings for User Understanding in Large Recommender Systems](https://arxiv.org/abs/2606.25147)
...
TL;DR: TokenMinds is a novel industrial-scale recommender system that generates both discrete SID-based user tokens and dense user embeddings, improving user modeling in recommender systems by combining the strengths of both representation types.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of existing user representation methods in industrial recommender systems, particularly the constraints of dense embeddings and the challenges posed by text-based user tokens generated by LLMs.

Method: The authors propose TokenMinds, which extends the PLUM framework using an encoder-decoder architecture from pre-trained LLMs to simultaneously generate discrete SID-based user tokens and dense embeddings.

Result: TokenMinds demonstrated enhanced user modeling capabilities, validated through offline experiments and live implementations across multiple YouTube platforms, effectively serving billions of users and reducing training and serving costs.

Conclusion: The study concludes that TokenMinds successfully integrates discrete SID-based user representations with dense embeddings, offering complementary advantages and confirming practical viability at an industrial scale.
...
### [86] [Extreme Meta-Classification for Large-Scale Zero-Shot Retrieval](https://arxiv.org/abs/2606.25237)
...
TL;DR: The paper presents EMMETT, a framework for efficiently handling novel items in large-scale retrieval tasks, with the algorithm IRENE boosting zero-shot retrieval accuracy and ad click-through rates.


<details>
  <summary>Details</summary>
Motivation: To address limitations of conventional Siamese-style approaches in large-scale retrieval tasks where novel items continuously arrive.

Method: Development of EMMETT, which synthesizes classifiers on-the-fly for novel items and IRENE, a specific instance for large-scale deployment; also a theoretical framework for analyzing generalization performance.

Result: IRENE improves zero-shot retrieval accuracy by up to 15% in Recall@10 and boosts ad click-through rates by 4.2% in a large-scale test.

Conclusion: The proposed methods enhance the efficiency and accuracy of retrieval tasks in the presence of novel items, validating through extensive experiments.
...
### [87] [Adaptive Re-Ranking](https://arxiv.org/abs/2606.25249)
*Ata Cinar Genc,Emir Kaan Korukluoglu,James Allan*

Main category: cs.IR

TL;DR: The paper presents an Adaptive Re-Ranking framework that improves the efficiency of Information Retrieval systems by utilizing a utility-based approach for per-query routing, significantly reducing latency while maintaining competitive retrieval effectiveness.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the inefficiency of the traditional 'retrieve-then-rerank' approach in IR systems, which applies heavy re-ranking models indiscriminately, leading to excessive latency and resource usage, particularly for simpler queries.

Method: The method involves an adaptive routing classifier that leverages three retrieval strategies: sparse retrieval (BM25), dense re-ranking (MiniLM-L6-v2), and heavy neural re-ranking (BGE-v2-m3), to optimize query processing based on complexity.

Result: The proposed method achieves median latency reductions of 1.15-53 times and mean latency reductions of 1.11-5.22 times compared to the BGE method across various datasets, while maintaining competitive nDCG@10 scores.

Conclusion: The findings indicate that the proposed utility-based query routing framework provides a scalable solution for enhancing computational efficiency and lowering latency in Information Retrieval systems.
...
### [88] [TheoremGraph: Bridging Formal and Informal Mathematics](https://arxiv.org/abs/2606.25363)
...
TL;DR: TheoremGraph integrates informal and formal mathematical knowledge by creating a statement-level dependency graph, enhancing the structure and retrieval of mathematical statements across varied sources.


<details>
  <summary>Details</summary>
Motivation: To provide a unified structure for mathematical knowledge that bridges the gap between informal citations and formal dependencies, facilitating improved search and reasoning in mathematics.

Method: The authors developed TheoremGraph by parsing informal theorem-like statements from arXiv and formal ones from Lean, generating and linking a vast number of directed dependencies and declaration nodes, respectively, and embedding them in a shared semantic space for retrieval.

Result: The unified graph comprises 11.7M informal theorem-like environments leading to 18.3M candidate dependencies and includes 388,105 formal declaration nodes with 11.3M typed edges from Lean, with effective matches demonstrated between informal and formal mathematics using LLM validation.

Conclusion: The results indicate a high accuracy in linking informal and formal mathematical knowledge, providing tools for improved mathematical search and reasoning, and the infrastructure developed is made publicly accessible.
...
### [89] [S2-CAR: Segmentation-Supervised Complexity-Adaptive Recommendation](https://arxiv.org/abs/2606.25415)
*Linjiang Guo,Nitin Bisht,Shiqing Wu,Xianzhi Wang,Guandong Xu*

Main category: cs.IR

TL;DR: This paper introduces S2-CAR, a new framework for sequential recommendation that adapts to user intent segmentation by modeling preferences as a continuous latent energy state, improving upon existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing sequential recommendation models struggle with complex and heterogeneous user behavior due to uniform treatment of interaction histories and rigid time-window segmentation, leading to misalignment with true user intent.

Method: S2-CAR implements a segmentation-supervised, complexity-adaptive approach, using a Context-Aware Soft Temporal Point Process (Soft-TPP) to segment user intent based on the natural decay of latent-state energy, rather than fixed time intervals.

Result: Extensive experiments on benchmark datasets demonstrate that S2-CAR outperforms 13 state-of-the-art methods in movie, e-commerce, and gaming domains across all metrics.

Conclusion: The energy-based segmentation can be integrated as a plug-and-play module into existing sequential recommendation systems, consistently enhancing their performance.
...
### [90] [Recommendation as Generation: Unifying Personalized Video Generation and Recommendation at Industrial Scale](https://arxiv.org/abs/2606.25496)
...
TL;DR: Introduction of Recommendation-as-Generation (RaG) for personalized video creation based on user interests, which shows significant revenue improvements in A/B testing.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of traditional short-video recommendation systems in capturing dynamic user preferences and interests.

Method: RaG generates videos on demand by using shared semantic IDs to model user interest and control video generation, incorporating Video Generation Agents for hierarchical planning and refinement.

Result: Implementation of RaG led to a 1.87% increase in ad revenue in an A/B test compared to a strong baseline, indicating its effectiveness in revenue generation.

Conclusion: RaG demonstrates the potential of closed-loop generative systems for enhanced personalized video recommendations, resulting in significant economic benefits.
...
### [91] [A Stochastic Epidemiological Model of Latent Tuberculosis in a Radiation Exposed Mars Colony](https://arxiv.org/abs/2606.25728)
*Teddy Lazebnik*

Main category: cs.IR

TL;DR: The paper develops a stochastic model for latent tuberculosis reactivation in a Mars colony, assessing the impact of radiation on immune function and disease transmission. It highlights the emergence of tuberculosis without initial infections and suggests adaptive control strategies for managing health risks.


<details>
  <summary>Details</summary>
Motivation: To address the biological consequences of human settlement on Mars, focusing on the risks of latent infections in a closed and radiation-exposed environment.

Method: A stochastic model was created linking cosmic radiation exposure to immune competence and latent tuberculosis reactivation, evaluated through simulations that included decision-making strategies for medical interventions.

Result: The simulations indicated that tuberculosis could arise endogenously, influenced by latent reservoir size and immune responses, and showed that adaptive control strategies effectively managed infectious risks.

Conclusion: The framework provides a basis for testing health intervention strategies for Mars missions, ensuring preparedness for handling infectious diseases in isolated environments.
...
### [92] [How Large Language Models Source Brand Reputation Across Languages and Markets](https://arxiv.org/abs/2606.25787)
*Dmitrij Zatuchin*

Main category: cs.IR

TL;DR: The study analyzes AI brand visibility through citations in LLM-generated answers, revealing that most citations come from third-party sources, primarily Wikipedia, with notable market-specific variations.


<details>
  <summary>Details</summary>
Motivation: To investigate the sources that large language models (LLMs) use to generate brand-related answers, focusing on the significance of citations in shaping responses.

Method: The study merges three datasets covering 128 brands across 12 markets and analyzes 167,551 URL-grounded citations to classify them by domain and source type.

Result: Findings indicate that 85.7% of citations are from third-party sources, with a concentration of reference sites, primarily Wikipedia, dominating across most languages and markets.

Conclusion: The research highlights the reliance on third-party domains for brand information in LLM responses and suggests the source mix is influenced by regional differences.
...
### [93] [AutoRelAnnotator: Calibrated Model Cascades for Cost-Efficient Relevance Evaluation in Sponsored Search](https://arxiv.org/abs/2606.25871)
*Md Omar Faruk Rokon,Shasvat Desai,Hong Yao,Kuang-chih Lee*

Main category: cs.IR

TL;DR: The paper presents a calibrated model cascade for high-quality relevance annotations at scale, reducing costs and improving efficiency in search ranking systems.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of slow and inaccurate human relevance annotation in training data preparation and evaluation for search systems.

Method: A calibrated model cascade that routes queries through incrementally larger fine-tuned classifiers, optimizing for both accuracy and cost, along with per-class isotonic calibration for improved results.

Result: The approach achieved a notable increase in accuracy (20 points from fine-tuning) and reduced compute costs by half with cascading while introducing a small accuracy boost (+0.6 points) through isotonic calibration.

Conclusion: The proposed methodology shows promise for scalable, efficient offline annotation pipelines, successfully validating improvements across multiple use cases.
...
### [94] [SPORT: Spherical-PSNR-Optimized tRuncaTion for Power-Efficient 360-Degree Video Systems](https://arxiv.org/abs/2606.24916)
...
TL;DR: The paper presents SPORT, a memory optimization framework for VR headsets that significantly reduces power consumption by selectively storing pixel data based on viewer gaze.


<details>
  <summary>Details</summary>
Motivation: To address the high power consumption of memory bandwidth in VR headsets, which accounts for 30-40% of total power, by optimizing how frame data is stored and accessed.

Method: Introducing Spherical-PSNR Optimized truncation (SPORT), which employs weighted-to-spherically-uniform PSNR for optimization and gaze-predictive tile classification to minimize latency and enhance classification accuracy.

Result: The SPORT framework achieves up to 51.6% power saving and substantial bandwidth reduction while providing full quality in the user's field of view and maintaining a high level of fidelity across regions.

Conclusion: Validated through silicon testing, SPORT demonstrates effective memory power savings and improved efficiency, making it a viable solution for enhancing VR headset performance without compromising user experience.
...
### [95] [SafeGen: LLM-Driven Assertion Generation and Fault Criticality Evaluation for Functional Safety](https://arxiv.org/abs/2606.25296)
*Xuanyi Tan,Arjun Chaudhuri,Rubin Parekhji,Krishnendu Chakrabarty*

Main category: cs.AR

TL;DR: SafeGen is an LLM-driven framework for assessing fault criticality in automotive chip design, enhancing functional safety through formal verification and semantic assertions.


<details>
  <summary>Details</summary>
Motivation: Functional safety is essential in automotive chip design due to advancements in autonomous driving and electric vehicles, and traditional fault analysis methods are often inadequate.

Method: SafeGen uses large language models and a Hyper Knowledge Graph to extract specifications, generate Functional Safety Assertions, and implement a fault-mapping mechanism supported by formal property verification.

Result: SafeGen produces higher-quality assertions with better semantic understanding than existing frameworks, leading to improved fault criticality assessments compared to traditional methods.

Conclusion: The experimental validation shows SafeGen's superiority in assertion generation and fault criticality assessment, benefiting the functional safety of automotive systems.
...
### [96] [Cache-Resident LLM Inference in GB-Scale Last-Level Caches](https://arxiv.org/abs/2606.25353)
*Wanning Zhang,Tongzhou Gu,Marco Canini,Ceyu Xu,Jian Weng*

Main category: cs.AR

TL;DR: The paper presents a cache-resident execution model for efficient LLM inference, leveraging GB-scale last-level caches to optimize performance.


<details>
  <summary>Details</summary>
Motivation: To address the challenges posed by data movement in LLM inference due to memory hierarchy and increase performance by keeping model weights on-chip.

Method: A cache-resident execution model separates weight-centric operations from attention and KV-cache management, allowing independent scaling of resources and reducing synchronization overhead.

Result: The proposed model achieves significant speedups in time-per-output-token, with a 2.04x-11.51x improvement on specific configurations, and up to 13.9x in a validated analytical model across various conditions.

Conclusion: The results demonstrate that efficient LLM inference is feasible on commodity CPUs with large caches when properly organized for cache residency and dependency management.
...
### [97] [Energy-Efficient CNN Acceleration with MSDF Digit-Serial Arithmetic on FPGA](https://arxiv.org/abs/2606.25562)
*Muhammad Usman,Yousef Sadegheih,Dorit Merhof*

Main category: cs.AR

TL;DR: This paper proposes a merged multiply-add architecture for hardware acceleration of U-Net's convolutional layers on FPGA, enhancing throughput and energy efficiency.


<details>
  <summary>Details</summary>
Motivation: To improve energy efficiency and reduce latency in hardware implementations of U-Net for image segmentation on FPGA.

Method: The proposed method integrates multiplication and addition operations into a single pipeline, reducing startup delays and improving overall throughput in comparison to traditional methods.

Result: The MMA architecture demonstrated significantly higher performance, achieving up to 15.14 GOPS/W, outperforming CPU-based designs and offering substantial energy savings over MSDF FPGA implementations.

Conclusion: The merged arithmetic approach is effective for enhancing performance in resource-constrained, latency-sensitive applications in medical imaging and computer vision.
...
### [98] [Croc: Training the Next Generation Chip Designers on Domain-Specific End-to-End Open Source Silicon](https://arxiv.org/abs/2606.25673)
*Enrico Zelioli,Philippe Sauter,Thomas Benz,Hannah Pochert,Luisa Wüthrich,Beat Muheim,Frank K. Gürkaynak,Luca Benini*

Main category: cs.AR

TL;DR: The paper presents an open-source SoC design and fabrication flow using the customizable RISC-V platform Croc, addressing the need for practical VLSI education.


<details>
  <summary>Details</summary>
Motivation: The increase in demand for domain-specific SoCs in AI, robotics, and automotive sectors necessitates hands-on VLSI design skills, which are often hindered by proprietary tools and designs.

Method: An end-to-end open-source design flow for SoCs was developed using Croc, integrating SystemVerilog IP blocks and a 130nm open PDK, allowing for extensive customization in design.

Result: In the initial course, 65 students completed 33 projects, resulting in 30 manufacturable layouts, 18 tapeout candidates, and five fabricated chips.

Conclusion: The first fabricated chip demonstrated comparable functionality and metrics to similar closed-source products, confirming the efficacy of the open-source approach in VLSI design education.
...
### [99] [BFMTrack: Latent Sequence Optimization for Physics-Based Motion Tracking with Behavioral Foundation Models](https://arxiv.org/abs/2606.25056)
*Thomas Rupf,Agon Serifi,David Müller,Sammy Christen,Ruben Grandia,Espen Knoop,Moritz Bächer*

Main category: cs.RO

TL;DR: This paper introduces Latent Sequence Optimization (LSO) to enhance Behavioral Foundation Models (BFMs) for precise motion tracking in character control, addressing limitations in capturing time-varying objectives.


<details>
  <summary>Details</summary>
Motivation: The goal is to improve BFMs for tasks requiring dynamic motion tracking, which current methods struggle with due to inadequate latent space support for time-varying objectives.

Method: The proposed LSO combines simulation rollouts and policy gradient updates to optimize a sequence of latents, employing temporally correlated noise for guiding the optimization process.

Result: The approach shows effectiveness in dense tracking, sparse keyframing, and direct application on a real humanoid robot, outperforming existing methods in motion tracking.

Conclusion: LSO extends BFM capabilities for precise motion tracking without the need for complex reward engineering, offering a more effective solution for dynamic character control.
...
TL;DR: The paper presents an advanced approach to extended pose estimation in articulated systems using an invariant framework, improving upon existing methods in terms of performance metrics.


<details>
  <summary>Details</summary>
Motivation: To improve pose estimation accuracy for IMU-equipped articulated systems, addressing challenges such as inter-body coupling and joint constraints.

Method: The authors introduce the relative L-extended pose, a Lie group representation, and develop an iterated IEKF (IterIEKF) that incorporates joint constraints as pseudo-measurements.

Result: The IterIEKF outperforms traditional EKF methods, showing faster convergence, reduced variability, and lower RMSE, with at least a 50% improvement over the second-best filter in all tested scenarios.

Conclusion: The proposed method provides a robust solution for pose estimation in articulated systems, demonstrating significant performance enhancements in practical applications.
...
### [101] [ADM-Fusion: Adaptive Deep Multi-Sensor Fusion for Robust Ego-Motion Estimation in Diverse Conditions](https://arxiv.org/abs/2606.25111)
*Hasan Moughnieh,Ibrahim Ghaddar,Hadi Elham,Imad H. Elhajj,Daniel Asmar*

Main category: cs.RO

TL;DR: Introduction of ADM-Fusion, a deep learning method for adaptive multi-sensor fusion that enhances ego-motion estimation under variable environments.


<details>
  <summary>Details</summary>
Motivation: Enhancing reliability in autonomous systems by dynamically balancing sensor inputs to improve performance in diverse and degraded environments.

Method: ADM-Fusion uses a mixture-of-experts framework for adaptive weight assignment of sensor inputs and employs a cross-task attention mechanism for information sharing between translation and rotation branches.

Result: Effective simulation-to-real transfer validated through training on CARLA-LOC and fine-tuning on KITTI, outperforming existing methods under degraded conditions.

Conclusion: ADM-Fusion proves to be robust and maintains competitive performance, adapting to environmental changes and sensor reliability fluctuations.
...
### [102] [SurveilNav: Collaborative Object Goal Navigation with Robot and Surveillance System](https://arxiv.org/abs/2606.25119)
*Ming-Ming Yu,Qunbo Wang,Rongtao Xu,Yanghong Mei,Yirong Yang,Longteng Guo,Wenjun Wu,Jing Liu*

Main category: cs.RO

TL;DR: This paper introduces SurveilNav, a framework for robot navigation that leverages multi-view surveillance data for improved efficiency, and presents a new dataset for testing these systems.


<details>
  <summary>Details</summary>
Motivation: The need for collaborative robot navigation in large-scale environments, utilizing multiple surveillance perspectives to enhance task execution efficiency.

Method: The paper presents SurveilNav, which features active camera scheduling, 2D/3D mapping, VLM-based value estimation, and collaborative target verification to optimize navigation.

Result: SurveilNav outperforms existing methods in exploration efficiency and navigation success, as demonstrated in experiments with the HM3D dataset.

Conclusion: The proposed framework shows strong potential for various applications including large-scale search and rescue missions, addressing the limitations of single-robot navigation.
...
### [103] [AeroCast: Probabilistic 3D Trajectory Prediction for Non-Cooperative Aerial Obstacles via Transformer-MDN Architecture](https://arxiv.org/abs/2606.25122)
*Syed Izzat Ullah,Jose Baca*

Main category: cs.RO

TL;DR: AeroCast is a novel trajectory prediction framework for autonomous aerial vehicles that predicts future positions of non-cooperative obstacles using a combination of a Transformer encoder and a Mixture Density Network, significantly improving prediction accuracy over existing methods.


<details>
  <summary>Details</summary>
Motivation: To improve trajectory prediction of non-cooperative obstacles in shared airspace, enhancing collision avoidance capabilities for autonomous aerial vehicles.

Method: The AeroCast framework integrates a Transformer encoder and a Mixture Density Network to predict per-timestep Gaussian mixture distributions of future 3D displacements, addressing input design and mode-degeneracy issues with specific techniques.

Result: AeroCast achieved a roughly 50% reduction in Average Displacement Error and Final Displacement Error compared to baseline methods, while also achieving superior statistical performance metrics.

Conclusion: The framework's efficient inference and improved prediction quality make it suitable for real-time deployment in autonomous aerial systems.
...
### [104] [RGB: RL Guided Whole-Body MPPI for Humanoid Control](https://arxiv.org/abs/2606.25123)
*Yunsoo Seo,Sol Choi,Euncheol Im,Myo Taeg Lim,Yisoo Lee*

Main category: cs.RO

TL;DR: Proposes a framework combining RL with MPPI for humanoid robot control, enhancing precision and stability in real-time environments without retraining.


<details>
  <summary>Details</summary>
Motivation: To improve the robustness and precision of humanoid robot control in contact-rich environments while minimizing retraining efforts when incorporating new feedback objectives.

Method: An RL-guided MPPI framework acts as an add-on feedback controller, utilizing a pretrained RL policy as a sampling prior for the MPPI, which specifies task objectives through cost terms and corrects the RL output continuously without retraining.

Result: Simulations showed stable high-rate control at an average of 280 Hz, with improvements in task-level precision over a pure RL controller, addressing systematic drift and tracking reference signals effectively.

Conclusion: The proposed method successfully combines RL and MPPI, enabling dynamic adjustments in humanoid robot behavior in real-time, enhancing performance in executing diverse tasks.
...
### [105] [Causality-Based Parametric Control Barrier Function for Safe Multi-Vehicle Interaction](https://arxiv.org/abs/2606.25134)
*Yiwei Lyu,Caleb Chang,John M. Dolan*

Main category: cs.RO

TL;DR: This paper develops an adaptive safety-critical controller for autonomous vehicles using a Causality-based Parametric-Control Barrier Function to address inter-vehicle influences and improve task efficiency in multi-vehicle interactions.


<details>
  <summary>Details</summary>
Motivation: To enhance safety in autonomous driving by accurately modeling the behavior of surrounding vehicles and improving response mechanisms without being overly conservative.

Method: The paper extends the Parametric-Control Barrier Function framework by incorporating causality inference to reason about inter-vehicle influences and dynamics effectively.

Result: The proposed controller allows the ego vehicle to adaptively react to surrounding vehicles, which significantly improves task efficiency in complex interaction scenarios.

Conclusion: The method leads to safer and more efficient interactions among autonomous vehicles by providing a better understanding of causality in vehicle behaviors.
...
### [106] [Memory Retrieval in Visuomotor Policies for Long-Horizon Robot Control](https://arxiv.org/abs/2606.25136)
*Rutav Shah,Yisu Li,Femi Bello,Yuke Zhu,Roberto Martín-Martín*

Main category: cs.RO

TL;DR: HALO is a visuomotor policy with attention-based memory retrieval for robots in partially observable environments, addressing spurious correlations and memory errors to enhance long-horizon control.


<details>
  <summary>Details</summary>
Motivation: To enable general-purpose robots to operate autonomously in dynamic environments, effective memory management is essential for recalling important past information.

Method: HALO employs attention-based memory retrieval, using vision-language model priors to generate Q&A pairs from past demonstrations and implementing sparse attention to limit memory retrieval to relevant history.

Result: The approach successfully reduces errors in robotic decision-making and improves reliability in long-horizon tasks by managing memory effectively.

Conclusion: By integrating these techniques, HALO provides a robust framework for enhancing robot autonomy in complex, partially observable environments.
...
### [107] [SwarmFly: A simulation platform for UAV swarm experiment design and validation](https://arxiv.org/abs/2606.25146)
*Abhishek Phadke,Karthik Kumar Vasudeva,Abhishek Joshi*

Main category: cs.RO

TL;DR: SwarmFly is a new MATLAB-based simulation platform for multi-UAV swarms, addressing shortcomings of existing tools by providing modularity, various swarm coordination modes, and comprehensive testing capabilities.


<details>
  <summary>Details</summary>
Motivation: There is a crucial need for a comprehensive and modular simulation platform for UAV swarms, given the limitations of existing open-source tools.

Method: SwarmFly uses a real-time operational map, supports four coordination modes, and employs a plugin architecture for extensibility, facilitating a range of internal and external disruptions.

Result: The platform was evaluated through eight experiments assessing formation accuracy, wind tolerance, fault recovery, energy endurance, and airspace compliance, demonstrating its effectiveness.

Conclusion: SwarmFly is fully MATLAB-based, easily extensible for hardware-in-the-loop testing, and available as an open-source tool, enhancing UAV swarm research.
...
### [108] [Toward Low-Latency Vision-Language Models with Doubly-Correct Predictions in Egocentric Visual Understanding](https://arxiv.org/abs/2606.25160)
*Qitong Wang,Fan Du,Pranav Maneriker,Jihui Jin,Christopher Rasmussen*

Main category: cs.RO

TL;DR: This paper explores a new pruning strategy for Vision-Language Models (VLMs) in egocentric visual understanding, focusing on improving both accuracy and evidential grounding in predictions for safe human-robot interaction.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the need for low-latency inference in HRC tasks, where efficient VLMs must provide accurate, evidentially grounded predictions to ensure safe human interaction and user trust.

Method: The paper introduces a rationale-informed pruning strategy that prioritizes alignment between evidence localization and correct predictions, addressing limitations of existing pruning strategies.

Result: Experiments reveal that the proposed method achieves superior prediction accuracy and outperforms existing methods in achieving doubly-correct predictions on egocentric video datasets.

Conclusion: The study emphasizes the importance of developing efficient and reliable VLMs that enhance transparency, auditability, and safety in human-robot interactions.
...
### [109] [fARfetch: Enabling Collocated AR-HRC in Large Visually Diverse Environments with VLM-Driven AR Content Adaptation](https://arxiv.org/abs/2606.25162)
*Christian Fronk,Hanting Ye,David Hunt,Miroslav Pajic,Maria Gorlatova*

Main category: cs.RO

TL;DR: fARfetch is an AR-HRC system enhancing collaboration by visualizing robot states and intents in large outdoor environments, demonstrating improved efficiency and reduced mental workload in user studies.


<details>
  <summary>Details</summary>
Motivation: The paper addresses challenges in human-robot collaboration in visually diverse outdoor environments, focusing on improving interaction and legibility of content at a distance.

Method: fARfetch integrates shared environment mapping, a world-in-miniature representation for path authoring, and vision-language model-driven AR content management, implemented with a Meta Quest 3 headset and Unitree Go2 robot, verified through a user study.

Result: User study results showed fARfetch reduced task completion time by 66% compared to a non-AR baseline and decreased mental workload across multiple dimensions: mental demand (-43%), temporal demand (-34%), and frustration (-66%).

Conclusion: The study concludes that fARfetch effectively enhances AR-HRC by maintaining content legibility and improving collaboration efficiency in large outdoor environments.
...
### [110] [Learning Perceptive Platform Adaptive Locomotion Controllers for Quadrupedal Robots](https://arxiv.org/abs/2606.25179)
*David Rytz,Kim Tien Ly,Ioannis Havoutis*

Main category: cs.RO

TL;DR: The paper investigates the integration of perception into quadrupedal locomotion control, emphasizing the challenges posed by diverse morphologies and noise sensitivity in perception.


<details>
  <summary>Details</summary>
Motivation: The aim is to enhance the performance of quadrupedal locomotion across various robot morphologies by effectively incorporating perception into the control policies.

Method: The authors utilize a reinforcement learning framework, specifically MorAL, to develop morphology-specific universal controllers. This is tested through adaptive terrain curricula on both simulations and real hardware (ANYmal).

Result: Comparative evaluations reveal that a critic-only perception approach (MorAL+) offers better robustness and tracking consistency than a blind baseline and demonstrates greater stability in the presence of noise compared to fully perceptive policies (PPAL).

Conclusion: Effective perception placement and curriculum design are crucial for the advancement of scalable and adaptable locomotion strategies in quadrupedal robots.
...
### [111] [RAVEN: Long-Horizon Reasoning & Navigation with a Visuo-Spatio-Temporal Memory](https://arxiv.org/abs/2606.25206)
*Yixun Hu,Zhicheng Zheng,Lihan Zha,Chunwei Xing,Rajdeep Singh,Omar Hossain,Antonio Loquercio,Dhruv Shah*

Main category: cs.RO

TL;DR: RAVEN is a memory system designed for long-term robotic navigation and question answering, offering efficient storage and retrieval of visual data without loss of detail compared to traditional caption systems.


<details>
  <summary>Details</summary>
Motivation: The need for a compact and scalable memory system that accurately preserves visual semantics and enables efficient spatial and temporal query responses in robotic applications.

Method: RAVEN uses a vector database to store visual embeddings linked with time and pose, grounding retrieval within a spatial map to facilitate navigation and answering queries directly from visual data.

Result: RAVEN outperforms traditional caption-based memory systems and matches leading vision-language models on long-horizon tasks while reducing retrieval costs significantly.

Conclusion: RAVEN's practical deployment on a Unitree Go1 robot demonstrates its capability for effective long-horizon navigation and goal-reach in various indoor environments.
...
### [112] [Wear-Clearance-Impact Coupling in the Jansen Linkage: A Gait-Durability-Optimized Design Slows Joint Loosening](https://arxiv.org/abs/2606.25208)
*Jichao Wang*

Main category: cs.RO

TL;DR: This study improves the modeling of the Jansen walking linkage by incorporating joint clearance and its effects on wear and durability.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of previous studies that modeled revolute joints as ideal, aiming to better predict in-service degradation in the Jansen leg.

Method: Developed a forward-dynamic model treating revolute joints as clearance joints, using a continuous normal contact law and Ambrosio friction, coupled with the Archard wear law.

Result: The model reveals that neglecting clearance significantly underestimates peak joint load and wear, indicating that designs must be statistically compared due to the sensitivity of wear patterns to clearance conditions.

Conclusion: The findings emphasize the need for a more refined model incorporating joint clearance and demonstrate the continuing durability advantage of clearance-free joints, necessitating empirical testing of predictions.
...
### [113] [RigPI: Dynamic Parameter Identification of Rigid Body via VLM-Seeded Differentiable Simulation](https://arxiv.org/abs/2606.25212)
*Xincheng He,Rongrong Zhang,Wei Jiang,Wenqiang Xu*

Main category: cs.RO

TL;DR: RigPI is a framework for accurately identifying dynamic parameters of rigid bodies in robotic manipulation, enhancing performance through vision-based priors and differentiable simulations.


<details>
  <summary>Details</summary>
Motivation: To improve robotic manipulation and create accurate digital twins by overcoming challenges in identifying inertial and frictional properties due to noise and modeling errors.

Method: RigPI utilizes a combination of vision-based semantic priors, force-torque measurements, motion observations, and a differentiable simulation pipeline to identify parameters through a two-stage optimization process.

Result: RigPI demonstrated accurate and stable parameter estimates in real-world experiments, effectively reproducing manipulation trajectories on a robot with predictive validity.

Conclusion: The framework proves effective and robust for real-world robotic system identification tasks, addressing common challenges in the field.
...
### [114] [Swazure: Swarm Measurement of Pose for Flying Light Specks](https://arxiv.org/abs/2606.25222)
*Hamed Alimohammadzadeh,Shahram Ghandeharizadeh*

Main category: cs.RO

TL;DR: This study introduces the Swazure technique for enabling miniature drones, called Flying Light Specks (FLSs), to accurately track each other and illuminate 3D shapes despite limitations of their sensors.


<details>
  <summary>Details</summary>
Motivation: The goal is to create a 3D multimedia display using FLSs that can localize and illuminate shapes by overcoming sensor range limitations.

Method: Swazure is a novel technique that enables FLSs to cooperate and share data to track poses accurately, even outside their sensor's sweet range, while maintaining physical data independence.

Result: Swazure allows medium-sized FLSs to position all their neighbors accurately; the Move Obstructing heuristic effectively resolves about 30% of pose obstructions in challenging scenarios.

Conclusion: The proposed methods enhance the functionality of FLSs in complex tracking scenarios, improving their utility for 3D displays.
...
### [115] [Spatio-Temporal Retrieval-based Priors for Adaptive Computational Teaching in Driving](https://arxiv.org/abs/2606.25224)
*Deepak Edakkattil Gopinath,Xiongyi Cui,Jonathan DeCastro,Avinash Balachandran,Guy Rosman*

Main category: cs.RO

TL;DR: This paper proposes an imitation learning model for adaptive teaching in motor skill training, integrating a temporal reasoning module to enhance performance using limited data.


<details>
  <summary>Details</summary>
Motivation: Existing automated coaching systems struggle with adaptation due to their focus on local reasoning without considering long-term learning dynamics.

Method: The proposed model employs imitation learning with a temporal reasoning module, utilizing nearest neighbor retrieval and cross-attention on a subset of similar past interactions in a concurrent teaching framework.

Result: The model was validated using a novel semi-synthetic dataset and a small real-world dataset, demonstrating superior performance compared to a non-adaptive baseline and other adaptive models.

Conclusion: The approach effectively enhances adaptive teaching in complex motor tasks, proving advantageous in scenarios with limited training data.
...
### [116] [GRAFT: Graph-Based Affordance Transfer via Part Correspondence](https://arxiv.org/abs/2606.25241)
*Mengying Lin,Utkarsh Mishra,Ajay Mandlekar,Danfei Xu*

Main category: cs.RO

TL;DR: This paper presents GRAFT, a geometry-aware framework that facilitates zero-shot robotic manipulation transfer using only one demonstration per object.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance robotic manipulation of unseen objects, addressing shortcomings in learning-based methods that require multiple demonstrations and struggle in few-shot scenarios.

Method: GRAFT uses part-based graphs to represent objects, employing both part-level and vertex-level descriptors for retrieving functionally and geometrically similar instances and matching contact points.

Result: The approach demonstrates improved performance in zero-shot manipulation by effectively utilizing geometric information alongside functional semantics.

Conclusion: GRAFT enables more efficient and accurate robotic manipulation of new objects, contributing to advancements in few-shot learning and manipulation techniques.
...
### [117] [An Integrated Hardware-Software Design for Low-Data Spatial Defect Detection in Robotic Visual Inspection with Hybrid Optoelectronic Neural Networks](https://arxiv.org/abs/2606.25277)
*Chaoqing Tang,Jiaxuan Li,Huanze Zhuang,Guiyun Tian,Chao Wang,Yihao Ouyang,Wenzhong Liu*

Main category: cs.RO

TL;DR: This paper presents a novel optoelectronic architecture for efficient robotic visual inspection, reducing data dependency and annotation needs through innovative sensor integration and a low-data processing approach.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance robotic visual inspection by addressing challenges of data overload and the inefficiency of shape-level annotations.

Method: A hardware-software integrated approach utilizing a Digital Micromirror Device as an optical convolutional layer, combined with block-based compressed sensing and natural language descriptions to guide feature alignment in neural networks.

Result: The proposed system successfully detects defects in transparent materials, achieving a 90% reduction in data volume while maintaining accuracy comparable to traditional imaging methods and decreasing computational workload by 60%.

Conclusion: This low-data paradigm offers an effective and efficient solution for industrial automation, particularly in scenarios characterized by large data streams and limited resources.
...
### [118] [DynaMOMA: Instantaneous Prediction of Grasp Poses for Mobile Manipulation of Dynamic Objects](https://arxiv.org/abs/2606.25295)
...
TL;DR: This paper presents a dynamic mobile manipulation framework that combines grasp trajectory prediction with whole-body control for enhancing robot interaction with dynamic objects.


<details>
  <summary>Details</summary>
Motivation: The need to improve mobile manipulation of dynamic objects due to the challenges in coordinating a robot's base and arm in constantly changing environments.

Method: Utilization of an anchor-based diffusion model for predicting short-horizon grasp trajectories, along with a whole-body reinforcement learning policy that incorporates an anticipation-guided reward system.

Result: The proposed method demonstrates strong performance in dynamic object manipulation in simulations and shows high generalizability in real-world experiments.

Conclusion: The framework effectively improves the robot's ability to dynamically grasp objects by integrating trajectory prediction with reinforcement learning.
...
### [119] [WaveForward: An Omnidirectional Passive Wheeled Quadruped Robot with Casters](https://arxiv.org/abs/2606.25299)
*Chuanlin Zhao,Qifeng Zheng,Shuhan Wang,Tiancheng Ma,Weixian Lin,Xin Luo*

Main category: cs.RO

TL;DR: This paper presents a low-cost, passive wheeled-legged robot utilizing standard casters for omnidirectional movement, controlled by an asymmetric actor-critic method.


<details>
  <summary>Details</summary>
Motivation: The goal is to create an efficient robot for varied terrains without the complexity of conventional actuated designs.

Method: A passive wheeled quadruped with casters and an asymmetric actor-critic control method was developed, enabling posture adjustments based on velocity commands.

Result: The robot demonstrated versatility in omnidirectional movement and achieved a reduction in the cost of transport by up to 89.1% compared to traditional legged robots.

Conclusion: This innovative design offers a significant improvement in mobility efficiency for long-distance transportation applications.
...
TL;DR: This paper investigates the role of an embodied AI agent in coaching for motor-skill development, presenting a framework that enhances human learning outcomes through strategic scaffolding and shared control.


<details>
  <summary>Details</summary>
Motivation: The motivation is to optimize human performance in skill-based tasks by preventing over-reliance on AI through effective coaching methods.

Method: The study formalizes the AI coaching as a non-cooperative dynamic game and develops a reinforcement learning framework that integrates adaptive shared control with probabilistic models of skill influence.

Result: A user study demonstrated that the proposed coaching framework significantly improved learning outcomes compared to existing AI coaching methods.

Conclusion: The findings suggest that strategically designed AI coaching can enhance motor-skill acquisition while fostering independent competence in learners.
...
### [121] [Self Capacitive Tactile Sensor System designed for Companion Robots](https://arxiv.org/abs/2606.25348)
*Mohsin Ali,Hidenobu Sumioka,Shuhei Ikemoto*

Main category: cs.RO

TL;DR: This paper presents a simple, scalable tactile sensing system for humanoid robots using a self-capacitance principle, addressing issues of complexity and cost.


<details>
  <summary>Details</summary>
Motivation: To improve physical interaction and manipulation capabilities in humanoid robots through effective tactile sensing.

Method: A tactile sensing system using a single conductive fabric layer and a 100-point sensor array on a flexible printed circuit, paired with a decision-tree classifier on an FPGA.

Result: The system was tested at 100 Hz and 1000 Hz, successfully identifying various interaction types without latency issues, while a lower frequency of 10 Hz was inadequate.

Conclusion: The proposed design is optimal for full-body tactile sensing in humanoid robots like HIRO-chan, balancing performance, scalability, and power efficiency.
...
### [122] [Decoupling Semantics and Geometric Grounding: Spatial Visual Prompts for Language-Conditioned Imitation Learning](https://arxiv.org/abs/2606.25360)
*Yanzhe Tang,Xinyu Shao,Yuxuan Hu,Siyu Chen,Bowen Yang,Yajun Gao,Tongtong Cao,Xiu Li,Long Zeng*

Main category: cs.RO

TL;DR: SVP-IL introduces a decoupled architecture for robotic manipulation that enhances spatial reasoning and control in vision-language-action tasks.


<details>
  <summary>Details</summary>
Motivation: To alleviate the alignment bottleneck in traditional end-to-end VLA models, which hampers precise target disambiguation in data-scarce imitation learning settings.

Method: SVP-IL decouples spatial visual grounding from action generation using vision-language models to create geometric masks that serve as Spatial Visual Prompts (SVP), integrating these prompts into a continuous action generator via direct feature-level fusion.

Result: SVP-IL significantly outperforms existing VLA and visuomotor approaches, improving average success rates on ambiguous language tasks from 24.0% to 39.5%, and achieving 67.8% on standard benchmarks with minimal training data.

Conclusion: The architecture demonstrates robustness and data efficiency through practical robotic experiments in unstructured environments, validating its effectiveness in enhancing manipulation tasks.
...
### [123] [Reliability-Asymmetric Spacecraft Autonomy: Co-Designing a Capable Learned GNC Stack with a Verified, Adaptation-Aware Runtime Shield](https://arxiv.org/abs/2606.25366)
*Alireza Shojaei*

Main category: cs.RO

TL;DR: The paper presents AMPLE-GNC, a robust three-tier autonomy framework for deep-space missions, combining command execution, fault tolerance, and safety measures.


<details>
  <summary>Details</summary>
Motivation: To address the need for autonomous systems in deep-space missions that are both capable and certifiable, overcoming the limitations of traditional rule-based and learned autonomy.

Method: The proposed system includes a foundation-model commander for language to action mapping, a verifier for constraint screening, and a fault-adaptive controller, supported by a runtime shield with verified temporal logic invariants.

Result: The system demonstrated high output-validity guarantees, effective fault adaptation, and robust performance under various conditions, with significant generalization capabilities.

Conclusion: AMPLE-GNC effectively balances capability and certifiability in autonomous systems for space missions, maintaining high autonomy while ensuring safety through adaptive mechanisms.
...
### [124] [Commerge: Communication-Efficient, Robust, and Fast LiDAR Map Merging Framework for Multi-Robot Coordination in Resource-Constrained Scenarios](https://arxiv.org/abs/2606.25386)
*Hogyun Kim,Jiwon Choi,Juwon Kim,Geonmo Yang,Seokhwan Jeong,Hyungtae Lim,Younggun Cho*

Main category: cs.RO

TL;DR: Commerge is a communication-efficient framework for multi-robot LiDAR map merging that significantly reduces data exchange while preserving alignment accuracy.


<details>
  <summary>Details</summary>
Motivation: To address communication bottlenecks in robot teams during LiDAR map merging, especially in resource-constrained environments.

Method: Commerge uses graph-theoretic approaches to selectively exchange a small subset of key scans between robots, minimizing inter-robot communication through a three-stage optimization on an exchange graph.

Result: The framework achieves up to 5,000x reduction in communication costs and up to 99.98% reduction in data exchange size while maintaining alignment accuracy across various datasets.

Conclusion: Commerge can operate efficiently across different platforms, showcasing its practical applicability in diverse environments.
...
### [125] [Large-Scale Tunnel Air--Ground Collaboration With FLISP: Fast LiDAR-IMU Synchronized Path Planne](https://arxiv.org/abs/2606.25393)
*Fenghe Guo,Runjie Shen,Chenyang Sun,Junrui Zhang,Quanxi Zhan,Yongchun Wang,Junjie Zhang*

Main category: cs.RO

TL;DR: FLISP is a novel framework enhancing hydropower tunnel inspections using cooperative UGV-UAV systems, achieving significant efficiency and safety improvements over traditional methods.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiencies and hazards of manual hydropower tunnel inspections.

Method: FLISP incorporates a unified UGV-mounted LiDAR-IMU system for coordinated path planning, employs enhanced solvers for obstacle avoidance and UAV flight, and utilizes a hierarchical refinement strategy for kinematic feasibility without drift.

Result: FLISP demonstrates a 100% success rate with ~7 ms latency in a 1.2 km operational tunnel, significantly outperforming traditional map-based inspection methods.

Conclusion: FLISP offers a scalable, efficient, and safe solution for robotic inspection in challenging infrastructure environments.
...
### [126] [MAPL: Multi-Objective Preference Learning for Robot Locomotion](https://arxiv.org/abs/2606.25398)
*Xiyue Chen,Muhan Lin,Shuyang Shi,Joseph Campbell*

Main category: cs.RO

TL;DR: MAPL framework uses LLM-generated preferences for robot locomotion reward design, achieving performance similar to expert rewards without manual engineering.


<details>
  <summary>Details</summary>
Motivation: To address the bottleneck of reward design in reinforcement learning for robot locomotion, which often relies on finely tuned, task-specific rewards.

Method: MAPL leverages a large language model to compare locomotion trajectories based on natural language objectives across multiple criteria, resulting in a scalar reward for training policies.

Result: MAPL trains policies in quadruped locomotion environments using solely LLM-generated preferences, yielding performance on par or superior to expert-designed rewards.

Conclusion: MAPL effectively eliminates the need for manual reward engineering while achieving competitive or superior locomotion performance.
...
### [127] [HEART: Coordination of Heterogeneous Expert Agents for Physically Grounded Robotic Task Planning](https://arxiv.org/abs/2606.25404)
*Junho Lee,Seabin Lee,Wonjong Lee,Nayoung Kim,Moonjeong Kang,Changjoo Nam*

Main category: cs.RO

TL;DR: HEART is a multi-LLM framework that enhances robotic task planning by decomposing complex instructions and utilizing specialized agents to ensure feasibility and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based planners struggle with physical and spatial constraints in robotic task planning, leading to invalid or incomplete action sequences.

Method: HEART decomposes instructions into atomic tasks, allocates them to specialized LLM agents, and uses constraint-driven plan synthesis to validate the feasibility of the plans under computational constraints.

Result: HEART outperforms single-LLM and rule-based planners by successfully producing executable plans across various household tasks, through the collaboration of heterogeneous agents.

Conclusion: The framework demonstrates that utilizing multiple specialized LLMs can significantly enhance robotic task planning efficiency and effectiveness in real-world scenarios.
...
### [128] [Delta-Position Estimation-Based IMU Odometry: A Comparison of MLP and Kolmogorov-Arnold Networks](https://arxiv.org/abs/2606.25454)
*Osman Tokluoglu,Emin Keresteci*

Main category: cs.RO

TL;DR: This study compares two neural network models for estimating incremental displacement in inertial odometry, finding that a Kolmogorov-Arnold Network (KAN) with learnable B-spline activations outperforms a Multi-Layer Perceptron (MLP) in terms of error and stability.


<details>
  <summary>Details</summary>
Motivation: To improve the accuracy and stability of inertial odometry using learning-based methods applied to raw IMU data from the EuRoC MAV benchmark.

Method: Models are trained to estimate incremental displacement over a 50 ms sliding window, utilizing a KAN with learnable B-spline activations and comparing it to a standard MLP.

Result: KAN has significantly fewer parameters than MLP and results in 44% lower error in cumulative drift on the test trajectory, as well as more stable long-term error performance.

Conclusion: Learnable B-spline activations in KAN can effectively reduce error accumulation in inertial odometry tasks.
...
### [129] [Generative AI for Safe and Photorealistic Drone Light Shows](https://arxiv.org/abs/2606.25458)
*Pascal Reinhold,Alexander Gräfe,Sebastian Trimpe*

Main category: cs.RO

TL;DR: SWAN is a novel AI-driven pipeline that automates drone choreography for light shows, translating text prompts into photorealistic animations while ensuring collision-free operation.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of labor-intensive manual animation in drone light shows and to leverage generative AI for automation.

Method: SWAN synthesizes drone choreographies from text using an adaptive point-tracking algorithm that maintains spatial coherence and a safety filter for collision avoidance.

Result: The system effectively orchestrated simulated formations of 2,000 drones and validated on a real-world swarm of 49 quadcopters, all using standard consumer hardware.

Conclusion: This research demonstrates the potential of generative AI in automating multi-robot choreography, enhancing accessibility in drone light shows.
...
### [130] [SAGE-Nav: Leveraging LLM Planning and Alignment Fusion for Hierarchical Scene Graph-Guided Navigation](https://arxiv.org/abs/2606.25497)
*Hao Su,Yuehao Huang,Yukai Ma,Yong Liu,Jiajun Lv*

Main category: cs.RO

TL;DR: SAGE-Nav introduces a hierarchical navigation framework for embodied agents, utilizing Large Language Models and scene graphs to enhance long-horizon reasoning and generalizability across diverse environments.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the limitations of existing ObjNav methods that struggle with long-distance planning and generalization to new environments.

Method: SAGE-Nav integrates LLMs for global semantic planning, uses a Hierarchical Scene Graph Encoder for embedding, and employs a Goal-aware Alignment-Fusion Network for real-time perception fusion.

Result: SAGE-Nav achieves state-of-the-art performance in navigation efficiency and zero-shot generalization in i-THOR and RoboTHOR environments.

Conclusion: The proposed framework maintains low control latency, making it suitable for practical robotic applications.
...
### [131] [AISPO: Enhancing Depth Reliability for Robotic Manipulation of Non-Lambertian Objects via Affine-Invariant Shape Prior](https://arxiv.org/abs/2606.25503)
*Zhiming Chen,Linfang Zheng,Kun Zhang,Hyung Jin Chang,Wei Zhang,Hongyu Yu,Hua Chen*

Main category: cs.RO

TL;DR: AISPO is a depth completion framework that enhances depth perception reliability for robotic manipulation of non-Lambertian objects by using RGB-D feature fusion and a shape prior.


<details>
  <summary>Details</summary>
Motivation: The need for reliable depth perception in robotics is crucial, especially when dealing with challenging surfaces like transparent or specular materials that often yield unreliable depth data.

Method: AISPO leverages multi-scale RGB-D feature fusion and incorporates an affine-invariant shape prior to ensure geometric consistency in depth maps.

Result: Benchmark evaluations indicate that AISPO achieves competitive performance and demonstrates strong generalization capabilities to novel objects and scenes.

Conclusion: Real-world tests reveal that improved depth reliability leads to higher success rates in robotic manipulation, particularly for challenging transparent objects.
...
### [132] [GROVE: Grounded Pedestrian Simulation via Natural Language for Interactive Social Robot Navigation](https://arxiv.org/abs/2606.25504)
*Duc Tai Nguyen,Volodymyr Shcherbyna,Anh Do Duc,Zhengcheng Shen,Teham Buiyan,Linh Kästner*

Main category: cs.RO

TL;DR: GROVE is a text-to-scenario pedestrian simulation framework enhancing social robot navigation by generating customizable, realistic scenarios without manual data generation.


<details>
  <summary>Details</summary>
Motivation: To address the rigidity and manual data requirements of current pedestrian simulation systems for social robot navigation.

Method: GROVE combines various modules that generate long, medium, and short-horizon pedestrian behaviors and interactions, allowing user customization through presets or independent prompts.

Result: A validated framework producing high-fidelity pedestrian simulations that present complex challenges to social robot navigation in diverse settings.

Conclusion: GROVE significantly improves the realism of pedestrian behavior simulations, thereby facilitating better training for social robots in social environments.
...
### [133] [ASSCG: Just-Right Gating over Chattering for Fast-Slow LLM Planning in Autonomous Driving](https://arxiv.org/abs/2606.25509)
*Sining Ang,Yuan Chen,Liu Haiyan,Xuanyao Mao,Jason Bao,Xuliang,Bingchuan Sun,Yan Wang*

Main category: cs.RO

TL;DR: The proposed Adaptive Slow-System Control Gate (ASSCG) enhances autonomous driving planning efficiency through optimal management of slow-system invocation, leading to improved performance and reduced latency in two benchmark architectures.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of costly online querying in large language models for autonomous driving and the limitations of existing fast-slow planners.

Method: ASSCG formulates slow-system invocation as a resource-aware decision problem, using an RWKV backbone for efficient gating and employing supervised and reinforcement fine-tuning.

Result: ASSCG improves performance metrics in two architectures: AsyncDriver shows a score increase of 2.28 with a 60% latency reduction, and RecogDrive demonstrates a 0.6 increase in PDMS alongside a 25% speed boost.

Conclusion: The ASSCG approach effectively optimizes autonomous driving systems by dynamically managing slow guidance, offering significant performance enhancements and latency reductions.
...
### [134] [One Body, Two Minds: Variable Autonomy Approach for a Co-embodied Robotic Hand](https://arxiv.org/abs/2606.25575)
*Piotr Koczy,Yuchong Zhang,Danica Kragic,Michael C. Welle*

Main category: cs.RO

TL;DR: The paper introduces co-embodiment with variable autonomy for assistive robotic systems, facilitating collaboration through shared control in a single physical body.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of fully autonomous and fully user-controlled systems in assistive robotics.

Method: The study presents a co-embodied robotic hand utilizing a visuomotor diffusion policy for autonomous grasping and enabling human control through gestures.

Result: User study with 44 participants showed 23.3% improvement in completion times and a task success rate of 93.6%, with high acceptance ratings.

Conclusion: Co-embodiment with variable autonomy is a promising approach for enhancing collaboration and usability in assistive robotics.
...
### [135] [WOLF-VLA: Whole-Body Humanoid Optimal Locomotion Framework for Vision-Language-Action Learning](https://arxiv.org/abs/2606.25591)
*Melya Boukheddimi,Omar Adjali,Daniel Sontag,Frank Kirchner*

Main category: cs.RO

TL;DR: This paper presents WOLF-VLA, a framework for humanoid locomotion using Vision-Language-Action models, addressing data scarcity and optimality issues in contact-rich environments.


<details>
  <summary>Details</summary>
Motivation: To explore the application of VLA models to humanoid locomotion, overcoming limitations like lack of data and dynamic consistency.

Method: A unified framework combines whole-body optimal-control motion synthesis with a large-scale dataset of humanoid trajectories, using multi-modal inputs.

Result: The resulting VLA model generates robust locomotion policies from natural-language instructions, demonstrating strong performance across tasks and environmental variations.

Conclusion: The framework and dataset will facilitate reproducible research in humanoid locomotion, enhancing instruction-driven policy transfer.
...
### [136] [1000 Rallies: An Event-Camera Dataset and Real-Time Learned Ball-State Estimation for Robotic Table Tennis](https://arxiv.org/abs/2606.25620)
*Raphaela Kreiser,Asude Aydin,Yin Bi,Claudio Fanconi,Peter Dürr,Naoya Takahashi*

Main category: cs.RO

TL;DR: This paper presents the first large-scale event-camera dataset for table tennis, which addresses the need for accurate ball state estimation in robotic table tennis using event-based perception. A convolutional neural network trained on this dataset improves trajectory prediction, and the system integrates with a robotic arm for real-time play.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to enhance robotic perception in fast-paced sports, particularly table tennis, where traditional cameras struggle with frame rate limitations affecting the estimation of fast-moving objects.

Method: The authors introduce a novel dataset created from 1000 rallies, utilizing event cameras and high-speed frame-based cameras. A convolutional neural network is employed to estimate ball position and velocity from event data, augmented by a Kalman filter to improve prediction accuracy.

Result: Using the proposed methodology, the system achieves a 36% reduction in bounce-point prediction error compared to a position-only baseline, demonstrating improved accuracy in trajectory prediction.

Conclusion: The integration of event-based perception with a robotic arm facilitates real-time human-robot interactions in table tennis, showcasing the potential of event cameras in dynamic environments.
...
### [137] [Event-Adaptive Motion Planning with Distilled Vision-Language Model in Safety-Critical Situations](https://arxiv.org/abs/2606.25629)
*Zhenwei Huang,Changsheng You,Shuai Wang,Chao Zhou,Wei Xu,Yi Gong*

Main category: cs.RO

TL;DR: Proposes a new framework (EAMP) for robot navigation that integrates VLM capabilities without compromising real-time efficiency, addressing challenges posed by dynamic agents in safety-critical environments.


<details>
  <summary>Details</summary>
Motivation: To enhance robot navigation in safety-critical scenarios where unexpected behaviors of dynamic agents lead to collisions.

Method: Introduces event-adaptive motion planning (EAMP) with a prompt-configurable semantic event trigger (PC-SET) for monitoring behavioral anomalies, followed by an event-triggered distilled SemNav-VLM for decision-making and a semantic model predictive control (SMPC) for dynamic adjustments.

Result: EAMP shows improved dynamic safety margins in logistics settings compared to existing methods, maintaining real-time operation.

Conclusion: The approach effectively bridges high-level reasoning and low-level control, enhancing safety in robotic navigation.
...
### [138] [Calousel: Extrinsic Calibration of Non-overlapping Multi-camera Systems from Pure Rotation](https://arxiv.org/abs/2606.25646)
*Gwanhyeong Song,Chaehyeon Song,Ayoung Kim*

Main category: cs.RO

TL;DR: This paper presents a new method for calibrating multi-camera systems with non-overlapping fields of view (FOVs) using a single static calibration board and pure rotational motion, addressing challenges in accuracy and usability in previous methods.


<details>
  <summary>Details</summary>
Motivation: The need for effective calibration of multi-camera systems in robotics, particularly when cameras do not share overlapping fields of view, while minimizing complexity and overhead.

Method: The method involves sequentially observing a single calibration target from all cameras under a shared geometric reference, integrated through a global optimization framework using a latent turntable and 3D error on SE(3).

Result: The proposed method was validated on both a controlled camera rig and a full-scale vehicle platform, demonstrating competitive accuracy without requiring specialized hardware.

Conclusion: The approach proves robust for realistic deployments, indicating strong applicability in diverse scenarios within the field.
...
### [139] [Learning to Adapt: Reptile-D-Learning for Robust and Efficient Control Under Parametric Uncertainty](https://arxiv.org/abs/2606.25659)
*Haipeng Cao,Zhaolong Shen,Quan Quan*

Main category: cs.RO

TL;DR: Reptile-D-learning enhances LLC by enabling better generalization and adaptation in nonlinear systems under parameter uncertainties.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of traditional LLC and D-learning, specifically the issues of model inaccuracies and performance degradation with parameter shifts.

Method: Reptile-D-learning utilizes the Reptile meta-learning algorithm to learn from multiple tasks, facilitating a shared dynamical understanding and improving Lyapunov network initialization.

Result: Experiments show that Reptile-D-learning outperforms traditional methods in generalization and rapid adaptation to new parameter settings in various nonlinear control scenarios.

Conclusion: The proposed framework successfully provides a robust approach to Lyapunov control in nonlinear systems, mitigating retraining costs and enhancing stability guarantees.
...
### [140] [Power-Budgeted Underwater Vehicle Control via Constrained Reinforcement Learning](https://arxiv.org/abs/2606.25680)
*Yinuo Wang,Gavin Tao,Yuze Liu*

Main category: cs.RO

TL;DR: This paper presents an energy-efficient control method for underwater vehicles by formulating the control problem as a constrained Markov decision process, addressing the limitations of traditional reinforcement learning methods.


<details>
  <summary>Details</summary>
Motivation: The need to optimize underwater vehicle energy consumption while maintaining task accuracy and avoiding oscillatory behavior in control actions.

Method: The study utilizes a constrained Markov decision process with an explicit thruster power budget, solved using a PPO-Lagrangian algorithm, to achieve energy efficiency without manual weight tuning.

Result: In experiments with three vehicles and four tasks, the proposed energy-constrained policy consistently reduced power consumption by 14-65% compared to a task-only baseline and an energy-reward baseline, while ensuring smooth control and task accuracy in most scenarios.

Conclusion: By treating energy as an explicit constraint, this approach simplifies achieving energy-efficient underwater control without the need for manual weight adjustments tailored to specific vehicles or tasks.
...
### [141] [SA-LIVO: Efficient LiDAR-Inertial-Visual Odometry with Subspace-Aware Degeneracy Handling](https://arxiv.org/abs/2606.25699)
*Yinong Cao,Xin He,Yuwei Chen,Shijie Liu,Chunlai Li,Jianyu Wang*

Main category: cs.RO

TL;DR: SA-LIVO enhances LiDAR-visual-inertial odometry by addressing independent sensor failure modes through direction-selective fusion and efficient processing, achieving superior accuracy and resource efficiency.


<details>
  <summary>Details</summary>
Motivation: To mitigate the limitations of existing LiDAR-visual odometry systems subjected to independent failure modes, ensuring robust performance in various conditions.

Method: SA-LIVO employs the Subspace-Aware Information Fusion (SAIF) framework to eigendecompose the joint information matrix, applying direction-selective soft gating and optimizing sensor residuals in a shared InEKF loop with pre-computed photometric Jacobians.

Result: Experiments demonstrate that SA-LIVO achieves competitive accuracy and approaches bounded drift across multiple benchmarks, outperforming existing systems in concurrent-degradation scenarios while maintaining lower processing time and memory usage.

Conclusion: SA-LIVO shows promising results in enhancing robustness and efficiency of LiDAR-visual-inertial odometry, making it suitable for real-time applications.
...
### [142] [Learning Asynchronous Upper-body Task-space Trajectory Tracking Policy for Humanoid Robots](https://arxiv.org/abs/2606.25706)
*Yumeng Liu,Dongqi Wang,Jiyu Yu,Yijun Fan,Rong Xiong,Yue Wang*

Main category: cs.RO

TL;DR: This paper presents an asynchronous upper body task-space tracking framework for humanoid robots to address the inconsistency between low-rate planning outputs and high-frequency control execution.


<details>
  <summary>Details</summary>
Motivation: To tackle the challenges of temporal asynchrony and structural incompleteness in humanoid robot control, specifically focusing on improving tracking performance despite reduced update rates.

Method: The framework employs teacher-student distillation to initialize a student policy, which is conditioned on future trajectories and an execution-time index. It uses a sliding-window global reward for training and incorporates an MPC module for task-specific post-training.

Result: Experiments demonstrate that the proposed framework improves tracking under low update rates, outperforms synchronous and decoupled methods, and ensures safer adaptation to out-of-distribution motions.

Conclusion: The proposed asynchronous framework effectively bridges the gap between task-space planning and full-body control, enhancing overall humanoid performance and adaptability.
...
### [143] [Stage-Aware and Roughness-Constrained Diffusion Policy for Multi-Stage Robotic Polishing](https://arxiv.org/abs/2606.25754)
*Shuai Ke,Jiexin Zhang,Huan Zhao,Zhiao Wei,Yikun Guo,Tiange Wu,Guoqiang Guo,Haoyuan Zhou,Jie Pan,Han Ding*

Main category: cs.RO

TL;DR: This paper introduces the Stage-Aware and Roughness-Constrained Diffusion Policy (SRDP) for enhancing robotic polishing processes in high-end manufacturing, addressing key challenges related to long-horizon dependencies and process parameter regulation.


<details>
  <summary>Details</summary>
Motivation: The necessity for improved surface quality in high-end manufacturing, particularly in aerospace, driven by the impact on component performance and reliability.

Method: The paper proposes SRDP, which utilizes multimodal observation to infer process stages and employs a roughness-oriented diffusion sampling method to regulate polishing parameters during operation.

Result: Experimental results demonstrate that SRDP leads to improved stability in stage transitions, consistency in process parameters, and enhanced surface quality across polishing tasks.

Conclusion: The proposed SRDP effectively overcomes existing limitations in robotic polishing, showcasing significant advancements in process control and final output quality.
...
### [144] [StairMaster: Learning to Conquer Risky Hollow Stairs for Agile Quadrupedal Robots](https://arxiv.org/abs/2606.25765)
*Xincheng Tang,Youhan Xie,Zhengjie Shu,Wanyu Li,Lai Jiang,Wenkang Hu,Yitong Li,Ruigang Yang*

Main category: cs.RO

TL;DR: StairMaster is a three-stage reinforcement learning framework designed for quadruped robots to navigate challenging hollow stairs, incorporating advanced sensor modeling and memory mechanisms for improved stability and precision.


<details>
  <summary>Details</summary>
Motivation: To address the difficulties quadruped robots face when climbing hollow stairs, including leg trapping, depth sparsity, and high-frequency noise.

Method: A three-stage RL framework employing Cross-Attention for feature extraction, Spatial-aware Recurrent Unit for memory maintenance, and a sim-to-real depth sensor modeling pipeline, combined with a reward structure for active perception and precise movement.

Result: StairMaster was deployed on a Unitree Go2 robot, successfully enabling it to climb hollow stairs with inclines of up to 55$^	ext{o}$ using zero-shot transfer.

Conclusion: This approach represents a significant advancement in RL-based policies for real-world steep hollow stair climbing.
...
### [145] [MIL-LC: A Robust Magnetometer-Inertial-LiDAR Fusion Multimodal Localization Framework](https://arxiv.org/abs/2606.25796)
*Qiyang Lyu,Zhenyu Wu,Wei Wang,Hongming Shen,Danwei Wang*

Main category: cs.RO

TL;DR: The paper proposes a multimodal localization framework (MIL-LC) for autonomous mobile robots (AMRs) combining magnetometer, inertial, and LiDAR data, addressing limitations in challenging environments where traditional methods fail.


<details>
  <summary>Details</summary>
Motivation: To improve localization reliability for AMRs in GNSS-denied and challenging environments, where existing multimodal approaches fall short due to reliance on geometric features or infrastructure.

Method: The MIL-LC framework integrates magnetometer, inertial, and LiDAR data to create a robust localization system that operates effectively under conditions of geometric degradation and magnetic map changes.

Result: Extensive experiments show that the MIL-LC framework provides accurate and robust localization in both simulated and real-world environments, outperforming existing methods.

Conclusion: The proposed framework represents a significant advancement in AMR localization technology, offering a flexible and infrastructure-free solution suitable for various challenging scenarios.
...
### [146] [Beyond a Shadow of a Doubt: Close Proximity Geometry Reconstruction Using FMCW Radar Shadow Effects](https://arxiv.org/abs/2606.25829)
*Felix de Trogoff du Boisguezennec,Benjamin Ramtoula,Daniele De Martini*

Main category: cs.RO

TL;DR: The paper proposes a method for recovering the 3D inclination of slender vertical objects using millimetre-wave FMCW radar by analyzing shadows cast by vehicle chassis, overcoming limitations in adverse conditions.


<details>
  <summary>Details</summary>
Motivation: To address the challenges faced by autonomous systems in perceiving objects accurately under poor lighting or weather, leveraging the robustness of FMCW radar.

Method: The method employs an analytical, closed-form mapping to relate radar return boundaries to the opening angle of occluded slender vertical objects, without assumptions about the environment.

Result: Validation through simulation and experiments demonstrates the capability to estimate inclinations, although the segmentation of objects in radar scans presents a significant challenge.

Conclusion: Chassis shadows are identified as a novel geometric cue, broadening the application of 2D rotating radar from localization to 3D scene reconstruction.
...
### [147] [TacVerse: A Multi-Sensor Dataset and Benchmark for Cross-Sensor Vision-Based Tactile Perception](https://arxiv.org/abs/2606.25877)
*Lan Wei,Gurmeher Khurana,Sirine Bhouri,Wenhao Hong,Zeyuan Xin,Qingzheng Cong,Wen Fan,Yanzheng Xiang,Dandan Zhang*

Main category: cs.RO

TL;DR: TacVerse is a dataset and benchmark for evaluating vision-based tactile sensors (VBTSs), comprising 106,800 images from seven sensors, addressing cross-sensor generalization in shape classification, grating classification, and force regression tasks.


<details>
  <summary>Details</summary>
Motivation: To improve understanding of cross-sensor generalization in vision-based tactile perception due to the limited knowledge in adapting performances across different VBTS designs.

Method: The study introduced the TacVerse dataset with various settings for training and evaluation: within-sensor training, zero-shot cross-sensor transfer, and few-shot adaptation, while analyzing performance across three tasks.

Result: Within-sensor training showed strong performance, but cross-sensor transfer caused significant degradation; shape classification was more robust compared to grating classification and force regression, with few-shot adaptation improving force regression but not reaching within-sensor levels.

Conclusion: TacVerse serves as a valuable resource for advancing research on sensor adaptability, data-efficient learning, and self-supervised methods in tactile perception.
...
### [148] [A 3D-Printable Dataset for Fair Testing and Comparisons of Tactile Sensors](https://arxiv.org/abs/2606.25886)
*Dexter R. Shepherd,Nicolas Herzig,Phil Husbands,Andrew Philippides,Chris Johnson,William Kimbell*

Main category: cs.RO

TL;DR: The paper introduces a 3D-printable dataset of mathematically defined textures for tactile sensing, addressing the limitations of existing datasets. It evaluates the reproducibility of these textures across 3D printers and filament types, revealing the impact of print quality on tactile variance and offering a benchmark for tactile sensor comparison.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of existing texture datasets, which focus on specific sensor readings and impair reproducibility and comparison among tactile sensors.

Method: A dataset of six parametrically generated surface patterns was created using sine-wave and Fourier functions. The reproducibility was evaluated across three 3D printers and various filament types, measuring tactile variance with an optical TacTip sensor.

Result: Print quality significantly influences tactile variance, with higher-end printers yielding more consistent tactile signatures. Classification experiments indicate better generalisation within printers compared to cross-printer scenarios due to geometric inconsistencies.

Conclusion: The work provides an openly available and reproducible 3D-printed texture benchmark, facilitating fair comparisons of tactile sensors.
...
### [149] [DeformGen: Dynamics-Based Topology Augmentation for Deformable Manipulation Policy Learning](https://arxiv.org/abs/2606.25939)
...
TL;DR: DeformGen is a framework for enhancing data acquisition in deformable manipulation by addressing high-dimensional state space and trajectory transfer challenges, leading to improved policy learning.


<details>
  <summary>Details</summary>
Motivation: The need for cost-efficient data acquisition in deformable manipulation scenarios, which are limited by high-dimensional constraints and non-equivariant trajectory transfer.

Method: DeformGen expands the valid state distribution through localized physical disturbances and dynamic simulation, while it transfers manipulation trajectories using deformation-field warping to adapt to deformed geometries.

Result: DeformGen enhances topology-coherent and physically plausible states, improving the augmentation of both state distribution and manipulation behaviors in experiments.

Conclusion: Experiments demonstrate that DeformGen significantly enhances policy learning compared to both the original demonstrations and existing rigid-style augmentation methods.
...
### [150] [DSP-SLAM++: A Unified Framework for Multi-Class, High-Fidelity Object SLAM in the Wild](https://arxiv.org/abs/2606.25953)
*Ahmad Kourani,Ghina Daoud,Daniel Asmar,Imad Elhajj*

Main category: cs.RO

TL;DR: DSP-SLAM++ improves existing SLAM systems by integrating real-time performance, multi-class object support, and high-fidelity mapping using an asynchronous mapping pipeline.


<details>
  <summary>Details</summary>
Motivation: To resolve the trade-off in SLAM systems between real-time performance, multi-class capabilities, and the quality of object models.

Method: DSP-SLAM++ enhances the DSP-SLAM framework with asynchronous mapping and sensor fusion for a fisheye-LiDAR setup.

Result: The system reduces object processing latency by up to 70d in comparison to the state-of-the-art, generating fine-grained, geometrically-complete shapes for multiple object classes.

Conclusion: This advancement facilitates the practicality of high-fidelity SLAM in applications such as autonomous driving and robotic manipulation.
...
### [151] [From Rubble Simulation to Active Magnetic Mapping: Quantum Sensing for Disaster Response](https://arxiv.org/abs/2606.25957)
*Samuel Tovey,Stefan Prestel,Hiroshi Yamauchi*

Main category: cs.RO

TL;DR: The paper presents a drone-based quantum magnetometry approach for locating survivors in building collapses, focusing on a simulation of a parking garage collapse to validate the technique.


<details>
  <summary>Details</summary>
Motivation: Accurate and timely location of survivors in building collapses is crucial, with existing methods providing limited data.

Method: A simulation pipeline was developed using Unreal Engine and magnetic field computation, integrating a Gaussian Process Regression with active spatial reconstruction using a three-sensor array.

Result: The research demonstrates recoverable magnetic structures in the sub-pT to sub-nT range and validates the effectiveness of the method with peak correlation in about 100 samples.

Conclusion: Quantum magnetometry has the potential to enhance drone-based structural analysis and aid in detecting voids in collapsed buildings.
...
### [152] [Mixture-of-Experts RL for Fault-Tolerant Legged Locomotion](https://arxiv.org/abs/2606.25965)
*Giulio Turrisi,Ozan Pali,Luca Oneto,Claudio Semini*

Main category: cs.RO

TL;DR: This paper presents a fault-aware modular control architecture for legged robots that improves locomotion performance in the presence of actuator failures.


<details>
  <summary>Details</summary>
Motivation: To enable legged robots to maintain reliable locomotion in remote environments despite actuator failures and challenging terrains.

Method: A modular control architecture that utilizes fault-diagnosis information to activate specialized control experts for distinct actuator failure modes.

Result: Experimental results demonstrate that the modular policies outperform monolithic policies, achieving better locomotion performance across various failure scenarios.

Conclusion: The proposed architecture remains effective even with reduced network capacity, making it suitable for compute-constrained robotic platforms used in space exploration.
...
### [153] [A Sensorised Lattice Footplate for a Semi-Active Prosthetic Foot](https://arxiv.org/abs/2606.25966)
*Jinze Ge,Jingcheng Sun,Chengxu Zhou*

Main category: cs.RO

TL;DR: This paper presents a prototype of a low-cost semi-active prosthetic foot with embedded magnetic plantar sensing, demonstrating the feasibility of internal force estimation and adjustable damping.


<details>
  <summary>Details</summary>
Motivation: The study aims to explore the integration of magnetic plantar sensing within a prosthetic foot to enhance functionality and reduce external components, improving semi-active control.

Method: The authors developed a sensorized 3D-printed lattice footplate combined with a servo-adjustable hydraulic damper, evaluating the damper's performance and conducting compression tests to analyze stiffness and load distribution.

Result: Experimental results indicate that the embedded sensor accurately tracks plantar forces and that the adjustable damping mechanism can mimic ankle motion trends during walking, albeit with limitations in generating active push-off.

Conclusion: The findings confirm that integrating sensing within the compliant structure of a prosthetic foot is viable and can effectively contribute to semi-active damping control.
...
### [154] [Action ControlNet: A Lightweight Delay-Aware Adapter for Smooth Asynchronous Control in Vision-Language-Action Models](https://arxiv.org/abs/2606.25985)
*Tiecheng Guo,Meng Guo*

Main category: cs.RO

TL;DR: ACNet enhances VLA models for robot manipulation by addressing inference latency with a lightweight, delay-aware adapter that improves action execution robustness.


<details>
  <summary>Details</summary>
Motivation: To overcome the inference latency in VLA models for stable high-frequency robot control without needing extensive retraining or custom runtime logic.

Method: ACNet is a lightweight adapter that employs executed motion suffixes as residual conditions for a mostly frozen action head, compatible with various generative action heads.

Result: ACNet demonstrates improved robustness against inference delays and produces smoother asynchronous trajectories compared to direct chunk stitching, while maintaining a lightweight framework.

Conclusion: The introduction of ACNet provides an effective solution for enhancing VLA model performance in real-time robotic manipulation tasks.
...
### [155] [Emcar: Embodied Controller for Animating Robots](https://arxiv.org/abs/2606.26008)
*Carlos Gomez Cubero,Elizabeth Jochum*

Main category: cs.RO

TL;DR: EMCAR is a no-code software tool designed for programming robot motion inspired by artistic practices, enabling artists and non-technical individuals to engage in robotics.


<details>
  <summary>Details</summary>
Motivation: To create an inclusive platform that allows artists and individuals without technical expertise to program robots, thereby expanding creative applications in human-robot interaction (HRI).

Method: EMCAR integrates artistic practices, such as puppetry and drawing, into a no-code interface for programming robots.

Result: The tool empowers artists to design and program robotic interactions, enhancing creativity in collaborations with robots.

Conclusion: EMCAR democratizes robotics by making it accessible to non-experts, fostering artistic contributions to robotic research and development.
...
### [156] [FORCE: Efficient VLA Reinforcement Fine-Tuning via Value-Calibrated Warm-up and Self-Distillation](https://arxiv.org/abs/2606.26006)
...
TL;DR: FORCE is a 3-stage framework enhancing Vision-Language-Action models by addressing issues in sample inefficiency and stabilization during Reinforcement Learning training, achieving significant performance improvements in autonomous robotic tasks.


<details>
  <summary>Details</summary>
Motivation: VLA models face limitations due to sub-optimal training data and inefficiencies inherent to RL fine-tuning, necessitating a robust solution to improve both training effectiveness and autonomous performance.

Method: FORCE comprises three stages: (1) a Value-Calibrated Warm-Up phase using on-policy rollouts to stabilize the Q-function; (2) an online stage where the calibrated Q-function filters action proposals and expert data, focusing on high-value actions for policy updates; (3) implementation and evaluation on various tasks to assess performance.

Result: FORCE demonstrates a 79% absolute improvement in success rates, outperforms existing RL methods by 10%, and increases training speed by 32.5% across simulation and real-world tasks.

Conclusion: FORCE achieves enhanced performance and robustness in training VLA models for robotic agents, successfully eliminating the need for human intervention and addressing issues commonly faced in Reinforcement Learning.
...
### [157] [FAR-LIO: Enabling High-Speed Autonomy through Fast, Accurate, and Robust LiDAR-Inertial Odometry](https://arxiv.org/abs/2606.26010)
*Maximilian Leitenstern,Marcel Weinmann,Patrick Haft,Tobias Lasser,Dominik Kulmer,Markus Lienkamp*

Main category: cs.RO

TL;DR: FAR-LIO is a CUDA-accelerated LiDAR-inertial odometry framework designed to improve odometry accuracy and reduce latency in autonomous racing environments. It utilizes a voxel hashmap and a sparsity-aware ICP algorithm to enhance performance. Testing shows significant reductions in positional error and runtime compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenges of odometry estimation in dynamic environments, particularly in autonomous racing, where low latency is critical for performance.

Method: FAR-LIO employs a CUDA-optimized voxel hashmap for parallel processing and implements a sparsity-aware Generalized Iterative Closest Point algorithm integrated with an Extended Kalman Filter to fuse LiDAR and IMU data.

Result: FAR-LIO demonstrates a 6.9% reduction in positional error and a 38.4% decrease in runtime compared to state-of-the-art methods under various sensor setups.

Conclusion: The framework showcases computational efficiency and effectiveness in enhancing odometry performance and is made available as open-source software.
...
### [158] [G2DP: Diffusion Planning with Spatio-Temporal Grid Guidance](https://arxiv.org/abs/2606.26017)
...
TL;DR: The paper introduces G2DP, a grid-guided diffusion planner for autonomous driving that enhances safety and effectiveness in motion planning by using dense environmental constraints during trajectory generation.


<details>
  <summary>Details</summary>
Motivation: The need for improved safety and situational awareness in autonomous driving, particularly in dense and interactive traffic, driven by limitations of existing diffusion-based planners.

Method: G2DP constructs a differentiable spatio-temporal cost volume that combines probabilistic future occupancy distributions with a route-progress map to guide trajectory generation during the denoising process.

Result: G2DP significantly outperforms traditional methods, increasing reactive score by +7.2 points and improving collision avoidance by +10.15 in comparative benchmarks.

Conclusion: The implementation of spatio-temporal cost grids effectively enhances robustness in guidance for diffusion-based motion planning in autonomous driving.
...
### [159] [In-Context World Modeling for Robotic Control](https://arxiv.org/abs/2606.26025)
*Siyin Wang,Junhao Shi,Senyu Fei,Zhaoyang Fu,Li Ji,Jingjing Gong,Xipeng Qiu*

Main category: cs.RO

TL;DR: This paper presents In-Context World Modeling (ICWM), a framework that enhances Vision-Language-Action models' ability to generalize to new environments by autonomously inferring system variables through self-generated interactions, reducing the need for fine-tuning.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the limitations of existing VLA models that struggle with generalization to novel setups due to their reliance on fixed execution contexts and the need for extensive fine-tuning.

Method: The authors introduce ICWM, which treats system identification as an in-context adaptation process, allowing robots to infer necessary system variables from a history of task-agnostic interactions instead of relying solely on specified demonstrations.

Result: ICWM enables models to adapt to novel configurations without changes in parameters, improving generalization capabilities significantly over standard VLA baselines, particularly under varied camera viewpoints.

Conclusion: The experimental evaluation indicates that ICWM substantially enhances the adaptability and performance of VLA models in real-world scenarios and simulations, proving its effectiveness in novel setups.
...
### [160] [RoboAtlas: Contextual Active SLAM](https://arxiv.org/abs/2606.26046)
*Alexander Schperberg,Shivam K. Panda,Abraham P. Vinod,M. K. Jawed,Stefano Di Cairano*

Main category: cs.RO

TL;DR: RoboAtlas is a contextual Active SLAM framework that enhances exploration and semantic navigation using a scalable semantic mapping system, achieving high success rates in large environments.


<details>
  <summary>Details</summary>
Motivation: To improve robotic navigation by balancing exploration and semantic reasoning in large-scale environments through a contextual framework.

Method: RoboAtlas integrates frontier exploration, global semantic-map reasoning, and egocentric VLM-based reasoning via a contextual multi-armed bandit model.

Result: In simulations and on a Unitree Go2 robot, RoboAtlas achieved a 100% task success rate and state-of-the-art performance with a 90.6% success rate on the GOAT-Bench benchmark.

Conclusion: Using large-scale 3D semantic maps significantly enhances contextual Active SLAM, demonstrating the advantages of grounding foundation models in rich semantic information.
...
### [161] [Learning Robot Visual Navigation in Crowds via Intention-Aware Scene Representations](https://arxiv.org/abs/2606.26047)
*Han Bao,Bingyi Xia,Hanjing Ye,Yu Zhan,Hao Cheng,Baozhi Jia,Wenjun Xu,Jiankun Wang*

Main category: cs.RO

TL;DR: iCrowdNav enhances robot crowd navigation by using intention-aware scene representations and deep reinforcement learning, outperforming existing methods in realistic environments.


<details>
  <summary>Details</summary>
Motivation: Existing deep reinforcement learning approaches for robot crowd navigation fail to fully leverage the structural and visual context of environments by representing pedestrians as mere 2D points.

Method: iCrowdNav employs a spatio-temporal encoder for scene occupancy and an Intent-Interact Former to extract and interpret human motion intentions from visual data, which is then integrated into a compact state for DRL policy training.

Result: Experiments indicate that iCrowdNav outperforms baseline methods in various scenarios, showcasing effective understanding of human intentions in navigation tasks.

Conclusion: The proposed method not only improves navigation performance in simulated environments but also shows practical effectiveness in real-world applications.
...
### [162] [Deep Reinforcement Learning-Enhanced Event-Triggered Data-Driven Predictive Control for a 3D Cable-Driven Soft Robotic Arm](https://arxiv.org/abs/2606.26048)
*Cheng Ouyang,Moeen Ul Islam,Kaixiang Zhang,Zhaojian Li,Xiaobo Tan,Dong Chen*

Main category: cs.RO

TL;DR: This paper presents a reinforcement-learning-based event-triggered predictive control framework for soft robots that reduces optimization frequency and maintains performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the real-time control of soft robots under resource limitations, addressing the computational demands of traditional data-enabled predictive control (DeePC).

Method: The proposed method, RL-ET-DeePC, integrates a model-free reinforcement learning policy that determines optimal event triggers for invoking the DeePC optimizer based on system state representations.

Result: Simulation results demonstrate a reduction in optimization calls by up to 66% and hardware experiments show a 34% optimization frequency reduction while preserving tracking accuracy.

Conclusion: The framework delivers improved performance and efficiency in soft robotic control, outperforming static event-triggered approaches.
...
### [163] [ForceBand: Learning Forceful Manipulation with sEMG](https://arxiv.org/abs/2606.26093)
...
TL;DR: ForceBand is a low-cost wrist-worn sEMG system that enhances robot manipulation learning by capturing muscle activity and contact forces, demonstrating significant improvement in force prediction accuracy and task success rates.


<details>
  <summary>Details</summary>
Motivation: To improve robot manipulation policies by integrating critical contact forces into human demonstrations, which are often missed in standard data sources.

Method: The authors develop ForceBand, an sEMG system that records human muscle activity alongside video and other sensors, then uses this data to train an EMG2Force model for predicting fingertip forces during manipulation tasks.

Result: ForceBand reduces force prediction error by over 50% compared to vision-based approaches and achieves an 87% success rate in various object manipulation tasks.

Conclusion: The integration of ForceBand and EMG2Force significantly enhances the quality of human demonstrations for robot learning by incorporating essential force data.
...
### [164] [Verifiable Foundation Models for Robot Safety](https://arxiv.org/abs/2606.23754)
*Davide Corsi,Kyungmin Kim,Roy Fox*

Main category: cs.RO

TL;DR: FEARL is a framework that modularizes robot control by separating policy into a large Controller for perception and reasoning and a small Safety module for verification, enabling practical sim-to-real transfer.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of verifying complex foundation models used in robot control, which are often opaque and difficult to analyze with existing tools.

Method: FEARL decomposes the control policy into a Controller for high-dimensional tasks and a Safety module that ensures safety through formal verification of low-dimensional observations.

Result: FEARL was evaluated on three simulated robotic domains using multiple Controller architectures and successfully transferred a learned policy to a physical robot, demonstrating effective sim-to-real capabilities.

Conclusion: The modular architecture of FEARL allows for tractable formal analysis of safety requirements while maintaining the expressive power necessary for complex task reasoning.
...
### [165] [Learning Action Priors for Cross-embodiment Robot Manipulation](https://arxiv.org/abs/2606.26095)
*Dong Jing,Tianqi Zhang,Jiaqi Liu,Jinman Zhao,Zelong Sun,Li Erran Li,Zhiwu Lu,Mingyu Ding*

Main category: cs.RO

TL;DR: The paper proposes a two-stage training framework for Vision-Language-Action (VLA) models, enhancing the action module with pre-trained motion priors to improve performance and convergence in cross-embodiment tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models struggle with learning actionable physical motion due to lack of explicit motion priors, leading to challenges in simultaneous discovery of action dynamics and cross-modal alignment.

Method: A two-stage framework where Stage 1 pretrains an action module using unconditioned action trajectories to learn temporal motion structure; Stage 2 aligns visual-language features with action embeddings for VLA training.

Result: The approach demonstrated faster convergence, higher success rates, and better performance on real-world tasks across 13 diverse cross-embodiment tasks compared to traditional VLA training.

Conclusion: Scaling the action data during pretraining enhances generalizability of action priors, positively affecting downstream VLA performance.
...
### [166] [RE4: Transformation-aware Imitation of Object Interactions Using Manipulation Modes](https://arxiv.org/abs/2606.24403)
*Arsh Chawla,Rahul Shome*

Main category: cs.RO

TL;DR: This paper proposes the RE4 framework for object interaction tasks, enhancing imitation learning by combining interpretability with performance through a structured approach using self-supervised pose estimation and mode-aware retrieval methods.


<details>
  <summary>Details</summary>
Motivation: To address the challenges in object interaction tasks within imitation learning, particularly the trade-off between performance and interpretability in end-to-end methods.

Method: The RE4 framework consists of lightweight model-free pose estimation, mode-aware demonstration retrieval, transformation, and replan steps to effectively leverage demonstration data.

Result: The RE4 framework was evaluated on state-based and image-based benchmarks, showing superior performance in robustness, especially in low-data regimes.

Conclusion: The introduced RE4 framework demonstrates that simple and interpretable components can effectively facilitate the learning of manipulation skills.
...
### [167] [InSight: Self-Guided Skill Acquisition via Steerable VLAs](https://arxiv.org/abs/2606.24884)
*Maggie Wang,Lars Osterberg,Stephen Tian,Ola Shorinwa,Jiajun Wu,Mac Schwager*

Main category: cs.RO

TL;DR: InSight enhances vision-language-action models by allowing them to autonomously acquire manipulation skills through primitive-action steerability.


<details>
  <summary>Details</summary>
Motivation: To address limitations in VLA models’ capabilities due to bounded training data, enabling continuous skill learning without human input.

Method: InSight uses an automated segmentation pipeline to decompose demonstrations into actionable primitives and a data flywheel to autonomously identify and integrate missing skill demonstrations.

Result: InSight was tested on various real-world tasks, showing successful acquisition of manipulation skills without human demonstrations, enabling composition of learned primitives for new tasks.

Conclusion: The framework provides a robust approach to enhance VLA model capabilities through primitive steerability, facilitating ongoing skill development.
...
### [168] [Systematic Exploration of 4-Expert Heterogeneous Mixture-of-Experts via Automated Pipeline Search](https://arxiv.org/abs/2606.23739)
*Yashkumar R Lukhi,Harsh Rameshbhai Moradiya,Radu Timofte,Dmitry Ignatov*

Main category: cs.LG

TL;DR: The study introduces an automated pipeline for optimizing heterogeneous Mixture-of-Experts architectures in the LEMUR dataset, identifying a coverage bias in model generation and suggesting a stratified random sampling solution for improved performance.


<details>
  <summary>Details</summary>
Motivation: To automate the design process of heterogeneous MoE architectures and improve model efficiency within the LEMUR dataset ecosystem.

Method: A deterministic code-assembly generator systematically combines base architectures from LEMUR into ensembles, utilizing a convolutional gating network and advanced training techniques for model evaluation.

Result: Generated 4,463 models with 1,021 successfully evaluated, revealing a coverage bias due to reliance on a single architecture family (AirNet), which affected exploration of the model space.

Conclusion: The highest accuracy ensembles emerged from AirNet, ShuffleNet, and MobileNetV3; FractalNet and MNASNet were deemed low-yield candidates requiring exclusion in future iterations.
...
### [169] [Weight-Space Geometry of Offline Reasoning Training](https://arxiv.org/abs/2606.23740)
*Aleksandr Nikolich,Igor Kiselev,Vladimir Platonov,Karina Romanova*

Main category: cs.LG

TL;DR: This study evaluates six offline reinforcement-learning methods (SFT, RFT, DFT, RIFT, Offline GRPO, DPO) to determine if they exhibit mechanistic differences in weight updates, finding variations in weight delta similarities and accuracy.


<details>
  <summary>Details</summary>
Motivation: The paper investigates whether various offline reinforcement-learning losses produce distinct weight updates or converge towards similar mechanisms, despite their comparable performance metrics.

Method: The authors trained six methods on identical math rollouts from a single model, analyzing weight deltas using cosine similarity, principal-angle subspace analysis, linear mode connectivity, and CKA metrics.

Result: Results indicated that SFT, RFT, and RIFT produced nearly colinear weight deltas, with DFT diverging more than the others; Offline GRPO's deltas were orthogonal to SFT, while DPO occupied a near-orthogonal subspace, achieving the highest accuracy metrics with a smaller learning rate.

Conclusion: The findings emphasize distinct weight updates among methods, particularly DPO, highlighting the influence of loss-function and optimizer choices on performance; further exploration is suggested for learning-rate-matched comparisons.
...
### [170] [A Survey on Federated Causal Discovery and Inference](https://arxiv.org/abs/2606.23741)
*Xianjie Guo,Yuwei Wang,Guodu Xiang,Xiaoli Tang,Kui Yu,Han Yu,Qiang Yang*

Main category: cs.LG

TL;DR: This paper systematically reviews federated causal discovery (FCD) and inference (FCI), connecting them as complementary stages, while addressing methodologies, federation topologies, and practical challenges in a privacy-preserving context.


<details>
  <summary>Details</summary>
Motivation: The interdisciplinary nature and lack of comprehensive surveys in federated causal reasoning hinder researchers' entry into the field.

Method: The paper provides a systematic review using multi-dimensional taxonomies based on core design decisions in FCD, organizing methods along methodological paradigms, federation topology, and structural scopes.

Result: It categorizes methods for FCI by target estimand and estimation strategy, highlighting the connection between FCD and FCI as parts of a unified pipeline.

Conclusion: The paper identifies shared challenges in privacy, communication efficiency, and theoretical guarantees, while outlining open research challenges for the future.
...
### [171] [The Inference-Compute Frontier and a Latency-Efficient Architecture for Limit Order Book Prediction](https://arxiv.org/abs/2606.25986)
*C. Evans Hedges*

Main category: cs.LG

TL;DR: The study explores the scaling-law behavior in predicting limit order books (LOBs) and identifies a power-law relationship between predictive loss and compute work, asserting the performance of FastBiNLOB, a new architecture that outperforms existing ones.


<details>
  <summary>Details</summary>
Motivation: To investigate the relationship between predictive loss and computational resources in limit order book prediction and to understand scaling laws in model performance.

Method: Analyzed various models, including decision trees and neural architectures, to derive a power-law relationship and introduced FastBiNLOB, focusing on efficiency in computations through innovative mixing operations.

Result: The empirical results demonstrate a power-law relationship with high predictive accuracy ($R^2=0.941$) for LOB models, while FastBiNLOB achieved better performance metrics with lower latency compared to state-of-the-art architectures.

Conclusion: FastBiNLOB represents a significant advancement in LOB prediction by maximizing efficiency and performance, highlighting the importance of understanding scalability in predictive modeling.
...
### [172] [Low-power analogue neural networks with trainable nonlinear connections for continuous control](https://arxiv.org/abs/2606.23742)
...
TL;DR: This paper explores the use of trainable nonlinear functions on connections in physical neural networks, enhancing efficiency for specific tasks, but showing limited benefits for classification-boundaries.


<details>
  <summary>Details</summary>
Motivation: To enhance low-power machine learning by utilizing analogue device physics effectively in neural network architectures.

Method: Introducing trainable nonlinear functions on connections in physical neural networks, implemented as analogue band-pass filters on field-programmable analogue arrays.

Result: Demonstrated task-dependent advantages in smooth target representation with fewer nodes/ connections compared to multilayer perceptrons, but no efficiency gain for classification tasks.

Conclusion: The advantages arise from the implementation of trainable nonlinearity on connections, applicable across various hardware, and suggest potential for energy-efficient neural networks.
...
### [173] [GCT-MARL: Graph-Based Contrastive Transfer for Sample-Efficient Cooperative Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2606.25073)
*Animesh Animesh,Satheesh K Perepu,Kaushik Dey*

Main category: cs.LG

TL;DR: The paper presents GCT-MARL, a transfer learning framework designed to enhance cooperative multi-agent reinforcement learning by reducing training costs and time for different environments through adaptable alignment and a two-phase training approach.


<details>
  <summary>Details</summary>
Motivation: To address the challenges and costs associated with training agents from scratch in various cooperative multi-agent reinforcement learning scenarios.

Method: GCT-MARL involves a multi-view graph contrastive backbone with an adaptively weighted alignment loss and a structured two-phase training protocol aimed at facilitating transfer learning across diverse agent populations.

Result: The framework significantly speeds up convergence on target tasks compared to traditional from-scratch training across both homogeneous and heterogeneous scenarios.

Conclusion: GCT-MARL provides a coherent strategy for improving transfer learning in MARL, while also accommodating continual learning through its adaptable transfer protocol.
...
### [174] [Synergizing Physically Constrained MCMC and Chemical-Informed Gaussian Processes for Reaction Network Discovery](https://arxiv.org/abs/2606.23757)
*Runzhe Liu,Zihao Wang,Wenbo Yang,Shengyang Tao*

Main category: cs.LG

TL;DR: The paper presents a novel gray-box workflow, PC-MCMC-CIGP, for extracting interpretable chemical governing equations from sparse, noisy time-series data, integrating sampling, screening, and a Gaussian Process model, achieving improved experimental outcomes.


<details>
  <summary>Details</summary>
Motivation: The need for interpretable governing equations in chemical processes despite challenges in data quality and parameter coupling.

Method: PC-MCMC-CIGP integrates spike-and-slab topology sampling, hard conservation, and a CIGP residual model within a physically constrained workflow.

Result: The confined sampler successfully differentiates genuine radical pathways in experiments and improves yields by 12.5% compared to a baseline approach.

Conclusion: The study reveals the varying efficacy of different optimization strategies, with PC-EI reducing low-yield suggestions, while EI criteria enhance final yield performance.
...
### [175] [Low Variance Trust Region Optimization with Independent Actors and Sequential Updates in Cooperative Multi-agent Reinforcement Learning](https://arxiv.org/abs/2606.25526)
*Bang Giang Le,Viet Cuong Ta*

Main category: cs.LG

TL;DR: This paper addresses the high variance in advantage function estimation during cooperative multi-agent reinforcement learning and introduces a clipping objective to stabilize convergence and reduce variance.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenges posed by high variance in advantage function estimates within sequential update frameworks in cooperative multi-agent reinforcement learning, which can lead to unstable convergence.

Method: The authors analyze the variance of the advantage function and propose a clipping objective that controls its fluctuation, enabling a monotonic bound with sub-linear convergence to ε-Nash Equilibria. Two practical algorithms based on this objective are developed.

Result: The introduced clipping objective provides a stable convergence in the learning process and effectively reduces the variance of advantage estimation, outperforming tested baselines in various environments according to experimental results.

Conclusion: This research contributes a novel method that enhances stability and performance in cooperative multi-agent reinforcement learning, with availability of source code for further use and reproducibility.
...
### [176] [Exploring Dualistic Meta-Learning to Enhance Domain Generalization in Open Set Scenarios](https://arxiv.org/abs/2606.23758)
*Xiran Wang,Jian Zhang,Lei Qi,Yang Gao,Yinghuan Shi*

Main category: cs.LG

TL;DR: This paper introduces MEDIC, a novel meta-learning approach that addresses label mismatch in open set domain generalization by balancing decision boundaries for unseen classes.


<details>
  <summary>Details</summary>
Motivation: To address label mismatches between source and target domains in open set domain generalization, where current methods struggle with classifying outliers effectively.

Method: MEDIC employs a dualistic meta-learning strategy that focuses on implicit gradient matching across inter-domain and inter-class task splits simultaneously.

Result: MEDIC outperforms previous methods in handling open set scenarios and demonstrates competitive performance in close set generalization.

Conclusion: The proposed method effectively balances decision boundaries, improving classification accuracy for unseen classes in varied domains.
...
### [177] [One Ruler: A Same-Hands Re-Evaluation of Bivariate Causal Direction on Tuebingen, with a Parameter-Free Compression Baseline](https://arxiv.org/abs/2606.23767)
*Wietse Stienstra*

Main category: cs.LG

TL;DR: This paper critiques the comparison of cause-effect pair accuracies across studies, emphasizing the need for a uniform reevaluation protocol. A baseline method achieves notable accuracy, revealing discrepancies in previous results and common practices that inflate reported figures.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the inconsistencies in the reported accuracies of cause-effect pairs across different studies and to provide a fair comparison using a standardized approach.

Method: A uniform reevaluation where all methods are applied to the same 102 pairs without any tuning and with a forced decision on each pair is employed. The baseline method using sorted-conditional compression and an off-the-shelf compressor is introduced.

Result: The study finds that under the common benchmark, methods cluster in low-to-mid 70s for accuracy, with the zero-parameter compressor tying the most effective method. It reveals significant discrepancies in published accuracies due to inflated figures from practices such as model selection and significance gating.

Conclusion: The results show that the original published accuracies are misleading, and the research exposes methodological flaws while providing standardization benchmarks for future comparisons.
...
### [178] [Deciphering Fingerprints of 3D Molecular Surfaces for Accurate Epitope Prediction](https://arxiv.org/abs/2606.23830)
*Fang Wu,Weihao Xuan,Jure Leskovec,Yejin Choi,Li Erran Li*

Main category: cs.LG

TL;DR: SurfBind is a novel framework for predicting antibody-antigen epitopes by using molecular surface representations and advanced learning techniques, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Traditional epitope prediction methods are limited as they often rely on sequences or backbone structures, failing to effectively identify discontinuous surface-driven epitopes essential for antibody-antigen recognition.

Method: SurfBind employs a Transformer-based architecture that integrates geometric and physicochemical information through patch-level surface modeling and binder-aware cross-attention, utilizing a hierarchical approach for predictions.

Result: SurfBind showcases state-of-the-art performance and robust generalization capabilities on various epitope identification benchmarks, such as SAbDab and DB5.5.

Conclusion: The study emphasizes the importance of interaction-aware surface modeling in enhancing the understanding of protein-protein interactions.
...
### [179] [Reconstructing GRACE Terrestrial Water Storage with Spatio-Temporal Graph Neural Networks: An Application to South America](https://arxiv.org/abs/2606.23833)
*Lukas Arzoumanidis,Lara Johannsen,Klara Middendorf,Annette Eicker,Youness Dehbi*

Main category: cs.LG

TL;DR: This paper presents a deep learning method using a multi-variate time series graph neural network (MTGNN) to reconstruct terrestrial water storage (TWS) anomalies from 1940 to 2002 by leveraging meteorological data, achieving superior accuracy with fewer predictors compared to traditional methods.


<details>
  <summary>Details</summary>
Motivation: To address the short record of GRACE observations since 2002 and enable climate-scale analyses of terrestrial water storage changes influenced by climate variability and human activities.

Method: A multi-variate time series graph neural network (MTGNN) is adapted to reconstruct monthly TWS anomalies by correlating daily ERA5 meteorological data with monthly GRACE observations, utilizing a hybrid adjacency matrix for encoding spatial dependencies.

Result: The reconstruction yields a Pearson correlation of 0.69 at grid-cell level and 0.94 at basin-mean level, with near-zero bias, successfully capturing significant climate events like El Niño and La Niña, while also demonstrating competitive performance against established reconstruction models with fewer predictors.

Conclusion: The MTGNN method provides a robust approach to reconstruct historical TWS anomalies, showing potential for addressing gaps in satellite geodesy data while highlighting challenges in arid regions.
...
### [180] [The Degeneracy Distillery](https://arxiv.org/abs/2606.23838)
*T. Lucas Makinen,Deaglan J. Bartlett,Niall Jeffrey,Benjamin D. Wandelt*

Main category: cs.LG

TL;DR: The paper introduces the degeneracy distillery, a method for detecting and resolving degenerate parameter combinations in models, improving label prediction and other tasks.


<details>
  <summary>Details</summary>
Motivation: Identifying degeneracies in parameters is crucial for distinguishing data, enhancing machine learning and probabilistic sampling efforts.

Method: The method employs estimation and flattening of the Fisher information matrix to analyze parameter combinations symbolically and automatically without requiring realized data.

Result: The approach leads to the discovery of coordinate transformations that reduce simulation budgets by up to 10 times while providing insights into the physical system.

Conclusion: The degeneracy distillery improves efficiency in parameter estimation and enhances understanding of physical models by characterizing degeneracies as intrinsic properties.
...
### [181] [Machine Learning Modeling for Real-Time Melt Pool Monitoring in Laser Powder Bed Fusion Additive Manufacturing: A Hybrid Approach](https://arxiv.org/abs/2606.23851)
*Inioluwa Emmanuel,Zhuo Yang,Ho Yeung,Xinyao Zhang*

Main category: cs.LG

TL;DR: This research develops a binary image classification framework using AI/ML to monitor melt pool images in laser powder bed fusion additive manufacturing, achieving high accuracy and efficiency through a hybrid model combining EfficientNetB0 and Random Forest.


<details>
  <summary>Details</summary>
Motivation: To enhance real-time monitoring of melt pool anomalies in LPBF additive manufacturing, ensuring quality control and process reliability in data-constrained environments.

Method: Implemented a binary classification framework using transfer learning architectures and Random Forest on a dataset of 1,200 images, with a focus on accuracy, inference latency, and resource usage.

Result: The hybrid EfficientNetB0-plus-Random Forest achieved an F1 score of 0.9451, accuracy of 0.9458, and AUC of 0.9904, with rapid inference times, outperforming purely deep learning models.

Conclusion: Combining convolutional features with classical methods provides an efficient solution for monitoring melt pool anomalies in additive manufacturing.
...
### [182] [Sesame: Structure-Aware Molecular Generation via Spatial Density-Map Conditioning](https://arxiv.org/abs/2606.23856)
*Konstantin Yatsenko,Arvind Thiagarajan*

Main category: cs.LG

TL;DR: Sesame is a diffusion-based molecular generation model designed for drug design, featuring a unique spatial pairformer module for understanding molecular structures and optimizing ligand interactions.


<details>
  <summary>Details</summary>
Motivation: The need for advanced generative models that can effectively handle de novo molecular generation and lead optimization in drug design.

Method: Sesame incorporates a spatial pairformer to condition on partial molecular structures and protein environments, augmented by a diffusion framework for denoising atom and bond types, with trajectory finetuning for enhanced output quality.

Result: The model successfully allows for both de novo molecule generation and fragment-based lead optimization, enhancing medicinal chemists' ability to develop effective drug candidates.

Conclusion: Sesame demonstrates significant potential in improving the drug design process by integrating spatial awareness and diffusion techniques for more effective molecular generation.
...
### [183] [Are Safety Guarantees in Neural Networks Safe? How to Compute Trustworthy Robustness Certifications](https://arxiv.org/abs/2606.23858)
*Merkouris Papamichail,Konstantinos Varsos,Giorgos Flouris,João Marques-Silva*

Main category: cs.LG

TL;DR: This paper addresses adversarial examples in AI by proposing an efficient method to compute robustness certifications using the apothem measure, which improves upon existing volume-centric methods.


<details>
  <summary>Details</summary>
Motivation: The existence of adversarial examples in AI poses significant challenges for neural network reliability, prompting the need for effective robustness certifications to ensure accurate predictions under input perturbations.

Method: The paper introduces the apothem measure for computing apothem-optimal certifications in a linear number of oracle calls, alongside the concept of dual certifications to provide bounds on robustness.

Result: The proposed method yields certifications with at least double the minimum edge length compared to previous methods on MNIST datasets, showcasing significant efficiency improvements.

Conclusion: Through the development of ParallelepipedoNN and dual certification concepts, the paper provides a promising approach to generate more efficient robustness certifications for neural networks.
...
### [184] [Exact Schur-Sylvester Dimensionality Reductions for Non-Smooth Stochastic Complexity and Manifold Sampling](https://arxiv.org/abs/2606.23867)
*Trenton Lau,Gary P. T. Choi*

Main category: cs.LG

TL;DR: This paper presents an efficient method for computing the Normalized Maximum Likelihood (NML) codelength for non-smooth estimators, reducing computational complexity significantly.


<details>
  <summary>Details</summary>
Motivation: The traditional approach to compute NML codelength faced challenges due to high computational overhead, particularly in the evaluation of projection operators and volume factors.

Method: The authors introduce a new formulation using the block Schur complement and Sylvester's determinant identity, which reduces the complexity of key operations involved in the computation.

Result: The computational complexity of necessary operations is reduced from O(N^3) to O(k^3 + N^2 k), and this reduction is generalized to other non-smooth estimators like Sparse SVMs and Elastic Net.

Conclusion: Empirical benchmarks demonstrate a speedup of over 14,100 times while preserving numerical precision, making non-smooth NML estimation feasible for large datasets.
...
### [185] [Federated Survival Analysis in Healthcare: A Multi-Model Evaluation on Cross-Institutional Heterogeneous Breast Cancer Data](https://arxiv.org/abs/2606.23871)
*Natalia Moreno-Blasco,Anusha Ihalapathirana,Pekka Siirtola,Miguel Fernandez-de-Retana*

Main category: cs.LG

TL;DR: The paper evaluates federated learning for survival analysis in breast cancer cohorts, comparing models and optimization strategies to enhance performance without compromising patient privacy.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of limited data availability and privacy concerns in clinical survival analysis.

Method: A systematic evaluation of federated learning using Cox Proportional Hazards, DeepSurv, and Random Survival Forest models, with assessments of three optimization strategies on a heterogeneous breast cancer cohort.

Result: Federated learning consistently outperforms local training and matches or exceeds centralized performance, particularly with Random Survival Forest showing robustness.

Conclusion: Guidelines for model and training-choice decisions in federated survival analysis are proposed, emphasizing client distribution diversity and optimization strategy effectiveness.
...
### [186] [MGI: Member vs Generated Inference](https://arxiv.org/abs/2606.23872)
*Bihe Zhao,Michel Meintz,Juangui Xu,Franziska Boenisch,Adam Dziedzic*

Main category: cs.LG

TL;DR: The paper addresses the challenge of distinguishing between true training data and generated outputs in generative models (MGI) using a novel method called Data Circuit Breaker (DCB).


<details>
  <summary>Details</summary>
Motivation: With generative models blurring the lines between human-created and machine-generated content, it is increasingly important to discern the source of samples to understand model behavior and privacy implications.

Method: The proposed Data Circuit Breaker (DCB) is a three-stage method that integrates signals from a generative model's autoencoder and latent generator to effectively differentiate between training members and generated samples.

Result: DCB shows consistent improvement over existing membership inference and attribution methods, effectively identifying true training samples even when models create near-duplicates.

Conclusion: The method generalizes well to various generative models and maintains effectiveness in complex scenarios where new models are trained on data produced by prior models.
...
### [187] [GRACE: Gated Refinement for Accurate Causal Edge Discovery in High-Dimensional Time Series](https://arxiv.org/abs/2606.23880)
*Mohammad Fesanghary,Abhinav Havaldar*

Main category: cs.LG

TL;DR: GRACE is a novel method for causal edge discovery that combines the strengths of constraint-based and score-based approaches while overcoming their limitations. It utilizes gated refinement with $L_0$ regularization to enhance the accuracy and efficiency of causal inference in complex datasets.


<details>
  <summary>Details</summary>
Motivation: The increasing complexity and scale of modern time-series datasets with numerous interacting variables complicates causal discovery, necessitating efficient and robust methodologies.

Method: GRACE employs Hard Concrete gates and $L_0$ regularization to create independent gates for candidate edges, enhancing binary decision-making through bimodal separation. It combines a fast linear CI skeleton for high-recall candidates with a gated model for pruning false positives based on improved prediction.

Result: Experimental results demonstrate that GRACE substantially improves F1 scores compared to constraint-based methods while maintaining high precision, surpassing the performance of attention-based and score-based alternatives. It accomplishes this with significantly reduced computational cost.

Conclusion: GRACE effectively recovers causal relationships in complex datasets, evidenced by results on both synthetic benchmarks and a real-world river flow dataset, showing high accuracy and a marked reduction in false positives.
...
### [188] [ARIA: Adaptive Region-Based Importance Allocation for Conditional Diffusion Distillation](https://arxiv.org/abs/2606.23898)
*Loay Mualem,Vinh Tong,Samir Darouich,Mathias Niepert*

Main category: cs.LG

TL;DR: ARIA is a framework for adaptive training of conditional diffusion models that optimizes the allocation of effort across the conditioning space to handle knowledge distillation effectively.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenges of transferring knowledge from large to small conditional diffusion models, especially when conditioned on a broad range of inputs.

Method: ARIA adaptively allocates training resources across different regions of the conditioning space by estimating teacher-student discrepancy in real-time.

Result: Empirical evaluations demonstrate that ARIA outperforms previous methods (such as RC) in various architectures and significantly benefits unseen and underrepresented cases.

Conclusion: The theoretical framework shows ARIA's tracking mechanism effectively manages evolving discrepancies during the training process.
...
### [189] [Closing the Loop: Formally Verified Law as a Reward Signal for Self-Improving Legal AI](https://arxiv.org/abs/2606.23913)
*Armin Heydari,Torben Leowald*

Main category: cs.LG

TL;DR: This paper presents an architecture for training legal AI with a verifiable reward signal, ensuring formal correctness in legal reasoning.


<details>
  <summary>Details</summary>
Motivation: The need for a reliable method in legal AI that assures the formal verifiability of reward signals amidst the complex nature of legal reasoning.

Method: An architecture is proposed integrating LLM-driven autoformalization into a formal legal framework, along with a verification kernel and proof trace explanations.

Result: The architecture demonstrates provable correctness in legal computations and guarantees structural integrity in legal argumentation through various legal scenarios.

Conclusion: This architecture enhances legal AI training by providing verifiable outcomes, bridging gaps in traditional reinforcement-learning approaches in legal contexts.
...
### [190] [Catastrophic Compositional Generation: Why Vanilla Diffusion Models Fail to Extrapolate](https://arxiv.org/abs/2606.23920)
*Duncan Soiffer,Chandler Squires,Yuan Guan,Jason Hartford,Pradeep Ravikumar*

Main category: cs.LG

TL;DR: The paper examines challenges in compositional generation using conditional diffusion models, highlighting that existing techniques may be ineffective in certain scenarios, particularly when target distributions differ substantially from the training data.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the limitations of conditional generative models in producing accurate samples from complex, compositionally-defined target distributions, particularly when training data is limited.

Method: The authors present theoretical arguments and conduct experiments on synthetic and realistic datasets to evaluate the performance of existing generative models and highlight the influence of score estimation error on output accuracy.

Result: Findings indicate that while recent corrective methods can reduce certain approximation errors, they do not adequately address the issue of score estimation errors, especially when dealing with out-of-distribution targets.

Conclusion: The results suggest a need for new approaches in compositional generation tasks, as current inference-time techniques fail to handle cases where the target distribution is significantly different from the training distributions.
...
### [191] [KLip-PPO: A per-sample KL perspective on PPO-Clip](https://arxiv.org/abs/2606.23932)
*Riccardo Colletti,Robin Holzinger*

Main category: cs.LG

TL;DR: This paper clarifies the relationship between the two forms of Proximal Policy Optimization (PPO) by demonstrating that the clipped surrogate gradient is equivalent to a Kullback-Leibler surrogate with a variable sample coefficient, revealing their structural similarities and implications for future research.


<details>
  <summary>Details</summary>
Motivation: To address the confusion in literature regarding the two forms of PPO and their treatment as separate, while establishing their underlying equivalence and potential for further development.

Method: The authors derive a Kullback-Leibler surrogate with a per-sample coefficient that reproduces the clipped surrogate gradient of PPO, analyzing its behavior at every minibatch step and validating it through experiments.

Result: Empirical results on five MuJoCo benchmarks show that the training curves from both PPO forms are indistinguishable, confirming their equivalence in practical applications.

Conclusion: The study highlights a structural feature of the clipped surrogate that is not obvious, proposing a new perspective for the design of PPO algorithms and suggesting avenues for future research.
...
### [192] [DREG: A Layer-Wise Jacobian Regularization as a General-Purpose Penalty](https://arxiv.org/abs/2606.23942)
*Rowan Martnishn*

Main category: cs.LG

TL;DR: This study analyzes the efficacy of the Derivative Regularization penalty (DREG) across 960 experiments and finds it consistently outperforms other regularizers in terms of accuracy, especially in data-scarce environments.


<details>
  <summary>Details</summary>
Motivation: To isolate and examine the effectiveness of DREG in regularizing neural networks across various settings, thus providing insights into its operational dynamics and advantages.

Method: Conducted a large-scale empirical study with 960 experiments, varying activations, regularizers, datasets, and random seeds to assess DREG's performance compared to other regularizers.

Result: DREG achieved the highest accuracy among evaluated regularizers, was second in noise robustness, and excelled under data scarcity, suggesting it is effective in modern deep learning applications.

Conclusion: DREG is a potent regularization method that outperforms traditional techniques in specific conditions, particularly where data is limited, and can be deployed with minimal tuning, reinforcing its utility in complex model architectures.
...
### [193] [Learning the Koopman Operator using Attention Free Transformers](https://arxiv.org/abs/2606.23957)
*Mohammed Nagdi,Evangelos-Marios Nikolados,Alexey Yermakov,Mars Gao,Nathan Kutz,Filippo Menolascina*

Main category: cs.LG

TL;DR: Introducing robust Koopman predictors through attention-free latent memory and dynamic re-encoding to improve long-horizon predictions.


<details>
  <summary>Details</summary>
Motivation: To address the issue of drift and errors in long-horizon rollouts of Koopman operators in dynamical systems.

Method: Implemented an attention-free latent memory block and dynamic re-encoding triggers to maintain prediction accuracy.

Result: Demonstrated significant reductions in long-horizon error across benchmark systems compared to existing approaches.

Conclusion: The proposed method proves effective in maintaining predictions on learned manifolds while enhancing speed and compactness.
...
### [194] [Certification of Machine Learning Models via Directional Sharpness](https://arxiv.org/abs/2606.25004)
*Gefei Tan,Adria Gascon,Sarah Meiklejohn,Mariana Raykova*

Main category: cs.LG

TL;DR: The paper introduces directional sharpness, a novel metric for assessing model generalization in machine learning, which proves to be more reliable and efficient compared to existing metrics.


<details>
  <summary>Details</summary>
Motivation: The need for reliable model certification in machine learning to ensure trustworthiness and quality, particularly regarding generalization performance.

Method: The authors propose a new metric called directional sharpness, which is easy to compute and provides a more accurate indication of generalization capabilities.

Result: Directional sharpness shows a stronger correlation with generalization compared to existing metrics and identifies models with poor generalization more effectively.

Conclusion: Directional sharpness offers a reliable and efficient alternative to assess model generalization, addressing limitations of current metrics.
...
TL;DR: Nexus Sampling improves token retention in LLM inference by using iterative scoring and probabilistic sampling, outperforming traditional deterministic methods.


<details>
  <summary>Details</summary>
Motivation: The need to manage KV cache within fixed memory limits in long-context LLM tasks which leads to the loss of subtly important tokens during inference.

Method: Nexus Sampling combines Nexus scoring, which identifies important tokens through an iterative direct attention process, with weighted reservoir sampling that allows for probabilistic retention of tokens.

Result: At 80% KV cache eviction, Nexus Sampling shows performance within 1% of dense attention on LongBench and outperforms top-$K$ methods on retrieval tasks while significantly reducing cache memory usage.

Conclusion: Nexus Sampling is a more effective approach for token eviction in LLM inference, ensuring better retention of important information compared to deterministic methods.
...
### [196] [Speculative Decoding at Temperature Zero: A Scoped Safety-Invariance Screen with a 48,072-Sample Expansion](https://arxiv.org/abs/2606.25097)
*Sahil Kadadekar*

Main category: cs.LG

TL;DR: The paper investigates the safety implications of speculative decoding in machine learning models, particularly at temperature zero, using a method called Typical-Acceptance Invariance Screen (TAIS).


<details>
  <summary>Details</summary>
Motivation: To address safety concerns regarding speculative decoding, especially the potential leakage of draft model behavior into safety-scored outputs.

Method: The paper introduces TAIS, a behavioral-equivalence screen that tests matched outputs for safety using byte-identity evidence, TOST equivalence criteria, and Cohen's h effect size measurements across a large sample of outputs.

Result: Safety divergence between draft and target outputs at temperature zero was not detected; the largest Cohen's h observed was 0.024, indicating trivial differences, with most comparisons passing the equivalence margin.

Conclusion: Overall, the TAIS framework suggests that speculative decoding does not compromise safety outputs under the tested conditions, though the study acknowledges limitations regarding temperature sampling and untested models.
...
### [197] [3D Masked Autoencoders are Robust Learners of Volumetric and Multimodal Cellular Representations for Microscopy](https://arxiv.org/abs/2606.23964)
*Amirhossein Kardoost,Lion Gleiter,Tingying Peng,Carsten Marr*

Main category: cs.LG

TL;DR: This paper compares 2D and 3D masked autoencoders in fluorescence microscopy, showing that 3D models significantly outperform 2D counterparts in downstream tasks.


<details>
  <summary>Details</summary>
Motivation: The study addresses the limitations of using 2D projections for inherently 3D cellular data in fluorescence microscopy.

Method: The authors systematically compare 2D and 3D masked autoencoders on volumetric microscopy data, using aligned visual representations with a pretrained protein language model (ESM2) for cross-modal supervision.

Result: MAE-3D consistently outperforms 2D methods on single-cell tasks, achieving a ROC--AUC of 0.865 and state-of-the-art metrics for protein localization.

Conclusion: The findings underscore the benefits of 3D modeling and multimodal alignment for improved representation learning in single-cell microscopy.
...
### [198] [A Comparative Study of Bayesian Contextual Bandits for Real-Time Warehouse Sorter Optimization](https://arxiv.org/abs/2606.23977)
*Tina Dongxu Li,Mouhacine Benosman,Ken Meszaros,Trevor Dardik*

Main category: cs.LG

TL;DR: The paper presents a method for optimizing sorter diversion in material handling systems using hybrid machine learning frameworks.


<details>
  <summary>Details</summary>
Motivation: To improve operational efficiency in automated material handling systems by addressing inadequacies in static cost functions.

Method: Three hybrid machine learning frameworks were compared: Linear Regression with Gradient Descent Optimization, XGBoost with Bayesian Optimization, and Bayesian Contextual Bandits, using a physics-aware emulator for model training.

Result: The Bayesian Contextual Bandits framework achieved a 2.03% reward uplift compared to a heuristic baseline, demonstrating higher overall performance and various advantageous characteristics.

Conclusion: The findings support the effectiveness of the BCB approach for real-time optimization, suggesting a path for operational deployment.
...
### [199] [Leaking Circuit Secrets: Gradient Leakage Attacks on Graph Neural Networks](https://arxiv.org/abs/2606.25589)
*Rupesh Raj Karn,Johann Knechtel,Ozgur Sinanoglu*

Main category: cs.LG

TL;DR: The paper evaluates gradient leakage attacks (GLAs) on GNNs used in circuit design and hardware security, demonstrating vulnerabilities and assessing defenses.


<details>
  <summary>Details</summary>
Motivation: To address the overlooked security and privacy risks of GNNs in circuit design and hardware security tasks, specifically focusing on gradient leakage attacks (GLAs).

Method: The study assesses various GNN architectures (GraphSAGE, GCN, GIN, GAT) on known netlist benchmarks to evaluate their vulnerability to GLAs and tests several defense techniques.

Result: GLAs can reveal sensitive information, influenced by architectural features, with attention mechanisms increasing risks and injective aggregation offering better resilience; defense techniques showed limited effectiveness and potential performance trade-offs.

Conclusion: The findings underscore the need for stronger defenses in GNNs for privacy preservation in circuit design, emphasizing a balanced approach between security and model performance.
...
### [200] [Offline Reinforcement Learning for Warehouse SLAM Throughput Control](https://arxiv.org/abs/2606.23978)
*Tina Dongxu Li,Mouhacine Benosman,Rajat Kumar,Kevin Tan,Ken Meszaros,Trevor Dardik*

Main category: cs.LG

TL;DR: A framework using offline reinforcement learning optimizes SLAM throughput in warehouses, improving efficiency and stability.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of optimizing SLAM throughput which affects congestion and operational efficiency in warehouse environments.

Method: An offline RL framework is proposed, implementing history-informed state representation, action space abstraction, and a reward function capturing operational metrics, using various RL algorithms.

Result: CQL policy showed a 22.97% improvement in system health and a 3.18% reduction in average throttling duration compared to other policies.

Conclusion: The study illustrates the effectiveness of offline RL in enhancing warehouse throughput control safely and scalably.
...
### [201] [TL++: Accuracy and Privacy Preserving Traversal Learning for Distributed Intelligent Systems](https://arxiv.org/abs/2606.25627)
*Erdenebileg Batbaatar,Young Yoon*

Main category: cs.LG

TL;DR: TL++ is a two-mode framework for training distributed intelligent systems that enhances mini-batch gradient behavior while maintaining data privacy through improved communication efficiency and secure activation sharing.


<details>
  <summary>Details</summary>
Motivation: The need for training across data silos without centralizing data while addressing issues with existing federated and split learning protocols.

Method: TL++ enables construction of virtual batches with explicit synchronization, exchanging cut-layer activations and gradients, and employs secret sharing for privacy in a semi-honest setting.

Result: TL++ demonstrated superior accuracy on CIFAR-10 and PubMedQA, exceeding non-TL++ approaches by over 12 percentage points and significantly reducing communication requirements per training step.

Conclusion: TL++ achieves near-centralized training performance with enhanced communication efficiency and added privacy through activation-level secret sharing.
...
### [202] [EMAgnet: Parameter-Space EMA Regularization for Policy Gradient Self-Play in Large Games](https://arxiv.org/abs/2606.23995)
*Tristan Maidment,JB Lanier,Chase McDonald,Nathan Tsang,Eugene Vinitsky,Roy Fox,Albert Wang,Wesley N. Kerr*

Main category: cs.LG

TL;DR: EMAgnet introduces an adaptive regularization approach for policy gradient methods in self-play, enhancing performance in two-player zero-sum imperfect-information games.


<details>
  <summary>Details</summary>
Motivation: To improve policy regularization in self-play settings, where existing methods like uniform distribution fail to consider action viability.

Method: EMAgnet uses an exponential moving average (EMA) of policy parameters as a dynamic regularization target, adapting to the agent's strategy changes.

Result: EMAgnet outperforms PPO self-play with uniform-magnet regularization in terms of lower exploitability across various test environments, particularly in games with dominated strategies.

Conclusion: The adaptive nature of EMAgnet leads to consistent performance improvements, making it a more effective regularization method for self-play in complex game scenarios.
...
### [203] [Learning to Trigger: Reinforcement Learning at the Large Hadron Collider](https://arxiv.org/abs/2606.23993)
...
TL;DR: The paper presents a reinforcement learning approach to optimize real-time event filtering in high-throughput scientific facilities, specifically the Large Hadron Collider, by dynamically tuning trigger thresholds to maximize signal efficiency and track background rates.


<details>
  <summary>Details</summary>
Motivation: Static and hand-tuned trigger menus can become suboptimal over time due to varying detector conditions and background composition, necessitating a more adaptive approach to optimize event filtering.

Method: The authors frame threshold tuning as a sequential decision-making problem, employing a reinforcement learning agent that processes streaming data to update trigger thresholds using a novel adaptation of Group-Filtered Policy Optimization (GFPO) with two variants. These variants ensure background rate feasibility during training.

Result: In simulated collider operation benchmarks, the RL agent improved in-tolerance time intervals by 48% for the total transverse energy trigger and 28% for the anomaly-detection trigger. Additionally, cumulative signal efficiency gained was up to 2% within tolerance.

Conclusion: The RL-based trigger control approach successfully enhanced in-tolerance rates and signal efficiency when applied to real collision data from the Large Hadron Collider, achieving significant improvements without fine-tuning, marking a pioneering effort in this domain.
...
### [204] [Cyclic Denoising Reveals Ultrastable Memories in Diffusion Models](https://arxiv.org/abs/2606.24000)
*Rishabh Sharma,Stefano Martiniani*

Main category: cs.LG

TL;DR: This paper presents cyclic denoising as a novel extraction attack on image diffusion models, highlighting its effectiveness in exposing previously inaccessible regions of the learned distribution and its ability to identify memorized training images.


<details>
  <summary>Details</summary>
Motivation: To develop an effective method for extracting information from image diffusion models, addressing gaps in standard sampling techniques and exploring the dynamics of generative models.

Method: Cyclic denoising employs repeated forward and reverse diffusion processes at controlled noise levels to extract samples from diffusion models without requiring additional knowledge or complex prompts.

Result: The method successfully reveals memorized images from training data and demonstrates consistent behavior across different diffusion model architectures, showcasing the intricate dynamics of generative landscapes.

Conclusion: Cyclic denoising serves both as a probing method for generative model landscapes and a tool for assessing memorization, with significant implications for privacy and copyright concerns.
...
### [205] [Fast and Slow Variational Continual Learning](https://arxiv.org/abs/2606.24007)
*Subarnaduti Paul,Yohan Jung,Mohammad Emtiyaz Khan,Siddharth Swaroop,Thomas Möllenhoff,Martin Mundt*

Main category: cs.LG

TL;DR: The paper proposes the Continual IVON (CoVON) optimizer, which integrates fast and slow adaptation mechanisms into the VCL framework to enhance continual learning in deep networks.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of contemporary optimizers in facilitating continual learning, particularly the need for mechanisms that balance stability and plasticity in adapting to new information.

Method: The VCL framework is utilized to merge past posteriors into a slow adaptation process, which stabilizes knowledge, while fast-weight updates are performed using the merged posterior as prior in the VCL updates implemented in the IVON optimizer.

Result: The CoVON optimizer demonstrates consistent improvements over existing VCL optimizers and outperforms traditional weight-regularization methods in various continual learning scenarios, including domain-incremental learning and fine-tuning of large language models.

Conclusion: The introduction of the CoVON optimizer significantly enhances continual learning capabilities in deep networks, enabling better performance across multiple applications compared to existing methods.
...
### [206] [You Don't Need to Run Every Eval](https://arxiv.org/abs/2606.24020)
*Yuchen Zeng,Dimitris Papailiopoulos*

Main category: cs.LG

TL;DR: The paper introduces BenchPress, a method for efficiently predicting model scores across multiple benchmarks using a rank-2 approximation, significantly reducing the number of evaluations needed.


<details>
  <summary>Details</summary>
Motivation: To determine whether all evaluations are necessary for assessing model scores and improve efficiency in model evaluation processes.

Method: The authors compile a public score matrix for 84 models across 133 benchmarks and utilize a rank-2 matrix completion method, BenchPress, to predict scores for held-out evaluations and establish a confidence layer for predictions.

Result: BenchPress successfully predicts held-out scores with a margin of 4.6 points, identifying key benchmark subsets that further tighten this margin to 3.93 points and 4.55 points for different sets.

Conclusion: The study suggests that model evaluation can be streamlined through strategic selection of benchmarks, as demonstrated by BenchPress, which predicts scores effectively with fewer evaluations.
...
### [207] [Information-Theoretic Classifier-Free Guidance with Adaptive Schedule Optimization](https://arxiv.org/abs/2606.24025)
*Haobo Chen,Xiangxiang Xu,Yuheng Bu*

Main category: cs.LG

TL;DR: The paper presents an information-theoretic framework for optimizing classifier-free guidance (CFG) in diffusion models to balance condition consistency and diversity in generation tasks.


<details>
  <summary>Details</summary>
Motivation: The study addresses the inconsistency in managing the trade-off between condition consistency and diversity when using CFG in diffusion models for various generation tasks.

Method: An information-theoretic framework is proposed to optimize CFG schedules based on a clean endpoint reference that defines the desired consistency-coverage trade-off, allowing for selective allocation of guidance across noise levels.

Result: The proposed method yields competitive or improved trade-offs in generation quality on ImageNet-512 and COCO datasets compared to constant guidance techniques.

Conclusion: The optimized CFG schedules enhance the generation process by balancing consistency and diversity more effectively than traditional methods.
...
### [208] [RoPE-Aware Bit Allocation for KV-Cache Quantization](https://arxiv.org/abs/2606.24033)
*Fengfeng Liang,Yuechen Zhang,Jiaya Jia*

Main category: cs.LG

TL;DR: Block-GTQ introduces a block-wise bit allocation method for key-cache quantization in RoPE, significantly improving fidelity and performance compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing low-bit KV-cache quantizers overlook the position-dependent contributions of cached keys under RoPE, treating keys as flat vectors, which negatively impacts quantization accuracy.

Method: Block-GTQ calculates energy scores for RoPE blocks and allocates bit widths based on marginal gain, optimizing the quantization process for each layer and KV head.

Result: Block-GTQ demonstrates substantial improvements in preserving RoPE query-key logits, achieving a 32-80% reduction in MAE and outperforming uniform TQ-MSE across all comparisons. In downstream tasks, it significantly boosts long-context performance metrics.

Conclusion: Block-GTQ not only enhances quantization fidelity but also enables efficient KV-cache serving, achieving impressive performance and memory reductions, making it a valuable tool for large-scale models.
...
### [209] [Rapid FinFET Modelling Using an Autoencoder](https://arxiv.org/abs/2606.24046)
*Amit Sarkar Suman Sau,Swagata Mandal*

Main category: cs.LG

TL;DR: This paper introduces a machine learning framework utilizing an autoencoder for modeling FinFET devices based on calibrated BSIM-CMG model data.


<details>
  <summary>Details</summary>
Motivation: To enhance the modeling of FinFET by leveraging machine learning and reduce reliance on extensive datasets.

Method: A calibrated BSIM-CMG model generated a dataset of current-voltage characteristics which trained an autoencoder. Key device physics were encoded in a low-dimensional latent space, with the incorporation of drain to source voltage as an input feature for capturing bias dependencies.

Result: The model achieved successful reconstruction of full I-V curves and extraction of important metrics like threshold voltage, subthreshold slope, and peak transconductance.

Conclusion: Data-driven compact models from actual device characterization can deliver high accuracy with limited training data, aiding in device characterization and circuit simulation.
...
### [210] [RAVEN: A Regime-Aware Variable-context Expert Network for Financial Time Series Forecasting](https://arxiv.org/abs/2606.24062)
*Cheng He,Zhenyu Guan,Xijie Liang,Defu Lian,Jiajia Li,Enhong Chen,Patrick P. C. Lee,Geng Hu,Zehao Chen*

Main category: cs.LG

TL;DR: RAVEN addresses the limitations of fixed context windows in financial time series forecasting by establishing adaptive temporal contexts using a Mixture-of-Experts approach.


<details>
  <summary>Details</summary>
Motivation: To tackle the non-stationarity and regime-dependent characteristics of financial time series that traditional models inadequately address due to their reliance on fixed look-back windows.

Method: RAVEN utilizes a Mixture-of-Experts framework with nested contiguous windows, applying a Cumulative Importance Thresholding mechanism to adaptively determine look-back lengths and a Global Compressed Representation for global temporal coherence. Correlation-Aware Weighting is introduced to align expert outputs.

Result: RAVEN significantly outperforms state-of-the-art models in cumulative log-return prediction and fund sales forecasting, demonstrating a 9.2% improvement in Pearson correlation on HS300 and a 20.2% on S&P500, alongside an 18.2% reduction in MSE for fund sales.

Conclusion: The proposed RAVEN framework not only addresses the issues related to fixed context windows in financial time series but also establishes a new benchmark in prediction accuracy across various metrics.
...
### [211] [Blockwise Policy-Drift Gating for On-Policy Distillation](https://arxiv.org/abs/2606.24084)
*Liwen Zheng,Haiyun Jiang*

Main category: cs.LG

TL;DR: This paper presents blockwise policy-drift gating to enhance on-policy distillation (OPD) for long-horizon reasoning tasks, demonstrating improved performance in a six-variant math reasoning benchmark.


<details>
  <summary>Details</summary>
Motivation: The aim is to address the fragility of sampled-token OPD in long-horizon tasks by introducing a method that enhances its stability and effectiveness.

Method: Blockwise policy-drift gating is implemented as a controller that computes log-probability shifts between the behavior student and the current student, aggregates these over fixed blocks, and uses mean-normalized gates to adjust OPD losses without altering teacher targets or rollout policy.

Result: The method improves the mean pass@8 metric from 0.4978 to 0.5160 in a math reasoning benchmark, showcasing its effectiveness across multiple datasets.

Conclusion: Local old-current policy drift serves as a viable control signal for OPD, and block-level gating is recommended for increased robustness in solving rates.
...
### [212] [FedUP: One-Shot Federated Unlearning via Centroid-Guided Plug-in Filters](https://arxiv.org/abs/2606.24113)
*Feihong Nan,Zhengyi Zhong,Pan Wang,Weidong Bao,Xiongtao Zhang,Quan Wen,Ji Wang*

Main category: cs.LG

TL;DR: FedUP is a federated unlearning framework that minimizes knowledge loss and request latency using lightweight filters, achieving rapid unlearning.


<details>
  <summary>Details</summary>
Motivation: The necessity for federated unlearning arises from compliance with legal requirements such as the right to be forgotten, driving the need for more efficient unlearning methods.

Method: FedUP employs lightweight pluggable filters to screen out target data while maintaining model performance, freezing model parameters, and training filters on server-side DP-protected class centroids, eliminating complex retraining and multi-round communication.

Result: The framework significantly reduces unlearning latency from minutes to seconds and maintains high unlearning precision while preserving non-target knowledge.

Conclusion: FedUP demonstrates effectiveness in diverse applications, offering efficient federated unlearning while ensuring that knowledge can be restored when necessary.
...
### [213] [NeuroSonic: Conditional Flow Matching for EEG-to-Speech Reconstruction](https://arxiv.org/abs/2606.24087)
*Wenhao Gao,Yifan Wang,Yijia Ma,Carl Yang,Wen Li,Chenyu You*

Main category: cs.LG

TL;DR: NeuroSonic introduces a new framework for reconstructing speech from EEG data, significantly improving perceptual quality by utilizing a deterministic probability-flow velocity field and a gated Transformer to process EEG and audio data.


<details>
  <summary>Details</summary>
Motivation: Reconstructing continuous speech from EEG is challenging due to the weak spatial resolution and variability of EEG measurements compared to the structured nature of speech.

Method: NeuroSonic employs a conditional flow-matching approach, learning a velocity field that transports corrupted acoustic states to clean speech, using a time-conditioned gated Transformer to model the transport dynamics without relying on stochastic methods.

Result: NeuroSonic outperformed baseline methods (GANs, diffusion models, and mean-flow) on the CineBrain and EAV benchmarks, achieving up to a 26.3% improvement in overall perceptual quality, particularly in artifact-heavy segments.

Conclusion: The study demonstrates that deterministic conditional transport is an effective and stable approach for EEG-driven speech reconstruction, improving both fidelity and realism in generated speech.
...
### [214] [When Top-1 Fails: Calibrating LoRA Monitors for Masked Diffusion LMs](https://arxiv.org/abs/2606.24119)
*Lucky Verma,Pratik Yadav*

Main category: cs.LG

TL;DR: DLM fine-tuning diagnostics show top-1 argmax concentration as a poor collapse warning; maximum LoRA gradient norm is a better predictor for identifying successful configurations.


<details>
  <summary>Details</summary>
Motivation: This study aims to evaluate the effectiveness of current diagnostics used in fine-tuning discrete diffusion language models, specifically examining collapse warnings and their reliability.

Method: The authors tested the top-1 argmax concentration metric across various LoRA/PEFT configurations to determine its effectiveness as a collapse warning and evaluated max LoRA gradient norm for better predictive capabilities.

Result: Top-1 argmax concentration was found to have zero precision in predicting actual collapses; however, max LoRA gradient norm showed a precision of 0.68 and F1 score of 0.79 when identifying successful configurations.

Conclusion: The study recommends replacing top-1 argmax concentration as a collapse alarm with a focus on max-gradient logging early in training to better calibrate thresholds for different DLM families.
...
### [215] [Holistic Data Scheduler for LLM Pre-training via Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2606.24133)
*Chenhao Dang,Jing Ma,Mingjie Liao*

Main category: cs.LG

TL;DR: The paper introduces the Holistic Data Scheduler (HDS), an online data mixing framework that utilizes reinforcement learning to optimize training data composition for Large Language Models (LLMs), resulting in improved efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: The motivation of this paper is to address the limitations of existing online data mixing methods that rely on a singular optimization approach, which does not fully explore the complexities of LLM pre-training with diverse data compositions.

Method: The proposed approach, Holistic Data Scheduler (HDS), employs reinforcement learning to adaptively manage data mixing. It uses the Soft Actor-Critic algorithm to navigate a continuous control space and features a multi-objective reward function that encompasses data quality, inter-domain influence, and model weight norms.

Result: HDS outperformed traditional methods by reaching validation perplexity on The Pile benchmark with 44% fewer training iterations and improved performance on the MMLU 0-shot task by 7.2%, along with consistent improvements on other benchmarks.

Conclusion: The HDS framework demonstrates significant enhancements in both training efficiency and the overall capability of Large Language Models, validating its design through extensive experimentation.
...
### [216] [A Time-Reparameterized Cumulative Intensity Extrapolation Sampler for Discrete Flow Matching](https://arxiv.org/abs/2606.24140)
*Feiyang Fu,Hehe Fan*

Main category: cs.LG

TL;DR: The paper presents a new sampling method called TR-CIE for discrete flow matching (DFM) models, designed to enhance sampling quality under restricted function evaluations.


<details>
  <summary>Details</summary>
Motivation: To improve the efficiency and quality of sampling in DFM when the number of function evaluations is limited.

Method: The TR-CIE sampler consists of a time reparameterization based on schedule and a cumulative-intensity extrapolation rule, which enhances the accuracy of intensity approximations without increasing the evaluation cost.

Result: The new sampler requires only one function evaluation per step and outperforms the standard τ-leaping method in terms of sampling quality across various tasks.

Conclusion: TR-CIE effectively addresses sampling challenges in DFM by optimizing NFE utilization, validated through experiments in diverse applications.
...
### [217] [AsyncOPD: How Stale Can On-Policy Distillation Be?](https://arxiv.org/abs/2606.24143)
...
TL;DR: The paper investigates on-policy distillation (OPD) with a focus on the challenges posed by stale data in asynchronous training settings, proposing solutions such as improved KL divergence approaches and a new training pipeline, AsyncOPD, which enhances efficiency without sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the bottleneck in on-policy distillation during large language model post-training caused by the dominance of rollouts in training time, and to explore how staleness affects performance.

Method: A systematic study of staleness in asynchronous OPD is conducted, including comparisons of KL divergence methods and evaluation of techniques from asynchronous reinforcement learning.

Result: The findings reveal that teacher-weighted forward KL is robust to staleness, while student-weighted reverse KL is vulnerable, and propose that recomputing the reverse-KL signal and using multi-sample Monte Carlo techniques effectively mitigate issues associated with stale data. The AsyncOPD pipeline demonstrates improved training throughput compared to synchronous methods.

Conclusion: AsyncOPD significantly boosts training efficiency (1.6x to 3.8x faster) while maintaining comparable accuracy, providing a practical solution for overcoming the challenges of staleness in asynchronous OPD.
...
### [218] [Lightweight Transformer Models for On-Device Fault Detection: A Benchmark Study on Resource-Constrained Deployment](https://arxiv.org/abs/2606.24173)
*Disha Patel*

Main category: cs.LG

TL;DR: This paper benchmarks lightweight transformer architectures against traditional ML methods for on-device fault detection, considering accuracy, latency, and model size across three datasets.


<details>
  <summary>Details</summary>
Motivation: The goal is to enable real-time fault detection on resource-constrained devices without relying on cloud resources, addressing challenges in model deployment and performance.

Method: The study evaluates classification performance (F1-score, AUC) of various traditional ML methods and lightweight transformer architectures on three public datasets, with a focus on model size and CPU inference latency, while also incorporating INT8 dynamic quantization and an adaptive inference pipeline.

Result: Lightweight transformers can match traditional ML performance on well-separated data but at significantly greater model sizes and latencies. TinyBERT-4L shows promise for deployment with reduced size and latency, while quantization techniques and an adaptive pipeline optimize inference efficiency.

Conclusion: The findings reveal issues with extreme class imbalance affecting both traditional and transformer methods in fault detection, underscoring the need for further advancements in this area.
...
### [219] [Project Ariadne: Prompt-Conditioned Route Generation for Synthesis Planning](https://arxiv.org/abs/2606.24184)
*Anton Morgunov,Victor S. Batista*

Main category: cs.LG

TL;DR: Ariadne is a decoder-only model for retrosynthetic planning that improves route generation by integrating prompts for constraints, significantly outperforming existing methods in efficiency and some accuracy metrics.


<details>
  <summary>Details</summary>
Motivation: To enhance retrosynthetic planning by streamlining the route generation process and reducing the need for separate models to handle different planning specifications.

Method: Ariadne uses a decoder-only architecture that generates retrosynthetic routes by incorporating the target molecule, constraints, and route within a single prompt-completion format.

Result: Ariadne achieves significant improvements over traditional methods, including a 13.7 point increase for depth constraints and a 31.2 point increase for required-leaf constraints in the RetroCast/PaRoutes benchmark. It also demonstrates faster performance, completing tasks in 24 GPU-minutes compared to 6.8 GPU-hours for previous planners.

Conclusion: While Ariadne shows promise in structural route generation and offers improved efficiency, the lack of defined route checkers remains a challenge for practical applications in experimental chemistry.
...
### [220] [Managing Task Execution for Unknown Workloads in Batteryless IoT: A Hardware-Agnostic Evaluation](https://arxiv.org/abs/2606.24340)
*Samer Nasser,Henrique Duarte Moura,Ritesh Kumar Singh,Maarten Weyn,Jeroen Famaey*

Main category: cs.LG

TL;DR: The paper proposes two dynamic scheduling strategies for batteryless IoT systems that improve energy management under unpredictable workloads, evaluated against existing methods.


<details>
  <summary>Details</summary>
Motivation: The shift towards energy-harvesting IoT systems necessitates efficient energy management due to the volatility of stored energy and increasing complexity of edge applications.

Method: Two novel dynamic scheduling strategies: a model-free Reinforcement Learning agent and an on-the-fly Approximated Prediction method, evaluated against AsTAR and optimized static thresholds in a simulation framework.

Result: The AP method achieves lightweight, near-oracle task throughput; the RL agent balances survival and execution, while AsTAR effectively manages execution pacing during energy gaps.

Conclusion: Advanced strategies enhance resilience in devices with limited energy storage, while simpler static policies are sufficient for devices with larger energy buffers.
...
### [221] [Parallel Manifold Steering: Efficient Adaptation of Large Associative Memories via Residual Energy Shaping](https://arxiv.org/abs/2606.24396)
*Kanishk Awadhiya*

Main category: cs.LG

TL;DR: H-Res (Hierarchical Residual Steering) improves adaptation of large Transformer models for new tasks by managing their energy landscape without altering global dynamics or increasing length, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Addressing the Plasticity-Stability dilemma in adapting frozen memory systems without causing catastrophic interference or compromising associative recall capacity.

Method: H-Res modulates the effective energy landscape of Transformers by steering token trajectories into task-specific basins of attraction, treating adaptation as a control problem on the activation manifold.

Result: H-Res outperforms global weight modification by 26% in associative retrieval tasks and reduces the computational burden associated with prompt-based adaptation methods.

Conclusion: H-Res maintains attention entropy and supports Neural Collapse, indicating its effectiveness for structured domains.
...
### [222] [Natural Identifiers for Privacy and Data Audits in Large Language Models](https://arxiv.org/abs/2606.24408)
*Lorenzo Rossi,Bartłomiej Marek,Franziska Boenisch,Adam Dziedzic*

Main category: cs.LG

TL;DR: This paper introduces natural identifiers (NIDs) as a solution to the challenges of auditing the privacy of large language models (LLMs) through post-hoc differential privacy and dataset inference.


<details>
  <summary>Details</summary>
Motivation: There is a significant need to assess the privacy of LLMs, but current methods require either special canary data for auditing or access to private datasets, making them impractical for real-world application.

Method: The study proposes natural identifiers (NIDs), structured random strings derived from common LLM training datasets, which can serve as alternative canaries for privacy audits and as held-out data for dataset inference.

Result: The evaluation shows that NIDs facilitate post-hoc auditing of differential privacy without retraining and enable dataset inference for suspect datasets without accessing private reference datasets.

Conclusion: NIDs represent a promising approach to enable scalable privacy auditing of LLMs, overcoming the limitations of existing methods.
...
### [223] [Data Augmentation: A Fourier Analysis Perspective](https://arxiv.org/abs/2606.24418)
*Behrooz Tahmasebi,Melanie Weber,Stefanie Jegelka*

Main category: cs.LG

TL;DR: The paper investigates whether partial data augmentation can match the statistical benefits of full data augmentation in learning tasks, demonstrating that it can achieve similar performance with fewer resources when using a random subset of transformations.


<details>
  <summary>Details</summary>
Motivation: To address the computational challenges of full data augmentation by exploring the effectiveness of partial augmentation in learning tasks under symmetry constraints.

Method: The authors develop a framework utilizing Fourier analysis and representation theory of finite groups to analyze the effects of partial data augmentation.

Result: Partial augmentation achieves comparable minimax rates to full augmentation, with errors decreasing as the subset size increases, while a strict subset cannot achieve exact invariance when the hypothesis space is expressive.

Conclusion: The study provides insights into the trade-offs between computational efficiency and statistical performance in data augmentation techniques, affirming the viability of partial augmentation while highlighting the necessity of full group averaging for exact invariance.
...
### [224] [An LLM-based Two-Stage Transformer Framework for Cross-Domain Bearing Fault Diagnosis with Limited Data](https://arxiv.org/abs/2606.24459)
*Jinghan Wang,Feng Cheng,Wentao Wu,Hang Li,Gaoliang Peng,Tianchen Liu*

Main category: cs.LG

TL;DR: This paper presents a novel two-stage transfer learning framework using a lightweight GPT-2-style Transformer for effective bearing fault diagnosis, addressing challenges of dataset heterogeneity and limited labeled data.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of existing methods that tackle issues like dataset heterogeneity and operating condition variations separately and rely on implicit feature alignment.

Method: The proposed framework utilizes a lightweight GPT-2-style Transformer with causal self-attention for hierarchical feature extraction, combining multi-source learning, prototype-based knowledge modulation, and taxonomy-adaptive classification.

Result: Achieved 92.61% average accuracy with only 10% labeled target data, outperforming existing methods by 17.24 percentage points.

Conclusion: The framework establishes an effective and practical approach for predictive maintenance in Industry 4.0, significantly enhancing fault diagnosis performance.
...
### [225] [A Fair Evaluation of Graph Foundation Models for Node Property Prediction](https://arxiv.org/abs/2606.24509)
*Oleg Platonov,Gleb Bazhenov,Dmitry Eremeev,Liudmila Prokhorenkova*

Main category: cs.LG

TL;DR: This study reevaluates 9 recent Graph Foundation Models (GFMs) focused on node property prediction, comparing them to strong Graph Neural Network (GNN) baselines, highlighting performance disparities and costs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of a unified evaluation standard for Graph Foundation Models used in node property prediction, which hampers reliable comparisons in the field.

Method: The study conducts a rigorous reevaluation of 9 GFMs alongside strong GNN baselines, using a consistent evaluation framework.

Result: Only the latest GFMs based on the Prior-data Fitted Networks paradigm showed superior predictive performance compared to well-tuned GNNs, but with increased inference costs.

Conclusion: The findings emphasize the need for standardized evaluation in GFMs and highlight trade-offs between performance and inference costs.
...
### [226] [Reasoning as Attractor Dynamics: Latent Memory Retrieval via Gibbs-Weighted Energy Minimization](https://arxiv.org/abs/2606.24543)
*Kanishk Awadhiya*

Main category: cs.LG

TL;DR: This paper explores Large Language Models as Dense Associative Memories, examining the energy landscape of mathematical reasoning and introducing a retrieval mechanism to enhance reasoning performance based on attractor dynamics.


<details>
  <summary>Details</summary>
Motivation: To better understand how LLMs can improve mathematical reasoning by modeling inference as a dynamic settling process in attractor basins instead of traditional next-token prediction.

Method: The authors propose a retrieval mechanism that uses Gibbs measure and spectral entropy to sample multiple reasoning paths, effectively approximating the equilibrium distribution of the model's outputs.

Result: The proposed method enhances performance on the GSM8K benchmark, improving Microsoft Phi-3.5 by 5.38% from 84.7% to 90.1%.

Conclusion: Modeling inference as a dynamic process into attractor basins leads to improved reasoning performance in LLMs.
...
### [227] [QC-SMOTE: Quality-Controlled SMOTE for Imbalanced Classification](https://arxiv.org/abs/2606.24625)
*Parth Upman,Shreyank N Gowda*

Main category: cs.LG

TL;DR: QC-SMOTE is a novel framework addressing class imbalance in classification by generating reliable synthetic samples and replacing low-quality ones with original minority duplicates, leading to improved classification performance.


<details>
  <summary>Details</summary>
Motivation: Class imbalance in classification leads to poor performance due to the generation of low-quality synthetic samples in noisy regions.

Method: QC-SMOTE employs a neighbourhood trustworthiness score for estimating minority sample reliability, using an adaptive strategy to generate synthetic candidates while adjusting for local data geometry.

Result: In experiments on 30 imbalanced datasets, QC-SMOTE outperformed other oversampling methods with the highest average AUC-ROC and Macro F1 scores, especially under moderate to severe imbalance.

Conclusion: The study highlights the significance of implementing a quality-aware, geometry-adaptive approach to synthetic sampling for improved robustness in imbalanced classification.
...
### [228] [Grad Detect: Gradient-Based Hallucination Detection in LLMs](https://arxiv.org/abs/2606.24790)
*Anand Kamat,Daniel Blake,Brent M. Werness*

Main category: cs.LG

TL;DR: Grad Detect is a gradient-based method for identifying hallucinations in LLMs during inference, leveraging internal gradient structures for improved reliability without reliance solely on output signals.


<details>
  <summary>Details</summary>
Motivation: To address the issue of hallucinations in Large Language Models, critical for their deployment in high-stakes applications.

Method: Grad Detect employs a gradient-based approach to predict hallucinations, analyzing layer-wise gradient patterns from a single forward-backward pass during inference.

Result: Grad Detect outperforms confidence-based and sampling-based methods on various Q&A benchmarks, demonstrating superior predictive accuracy for hallucination detection and model abstention.

Conclusion: The method reveals that the last five layers contain the majority of the discriminative gradient information, allowing for efficient use in models while maintaining high performance, and provides insights into model reliability.
...
### [229] [FlowPipe: LLM-Enhanced Conditional Generative Flow Networks for Data Preparation Pipeline Construction](https://arxiv.org/abs/2606.24679)
*Kunyu Ni,Lei Cao,Jie He,Xiaotong Zhang,Jianfeng Jin,Junyu Dong,Yanwei Yu*

Main category: cs.LG

TL;DR: FlowPipe is a new framework that enhances the automation of data preparation pipelines in machine learning by utilizing C-GFlowNets to improve performance and efficiency in feature transformation.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the computational challenges in automatically constructing data preparation pipelines, highlighting limitations in existing Multi-DQN methods and aiming to enhance credit assignment, policy context, and exploration efficiency.

Method: FlowPipe reformulates pipeline synthesis as conditional probabilistic flow generation using C-GFlowNets, employing a Trajectory Balance objective and Deep Semantic Modulation via FiLM to effectively incorporate dataset semantics into decision-making.

Result: FlowPipe significantly outperformed state-of-the-art baselines in benchmark tests, achieving an average accuracy improvement of 11.96% and accelerating training convergence by 12.5 times.

Conclusion: The results indicate FlowPipe's effectiveness in improving data pipeline automation, making it a valuable tool for enhancing machine learning data quality.
...
### [230] [Real vs. Complex Spectral Bases for Neural Operators: The Role of Green's Function Alignment](https://arxiv.org/abs/2606.24851)
*Jason Sulskis,Sathya Ravi*

Main category: cs.LG

TL;DR: The study introduces the Hartley Neural Operator (HNO) as a real-valued alternative to the Fourier Neural Operator (FNO) for solving partial differential equations (PDEs).


<details>
  <summary>Details</summary>
Motivation: To enhance the representation of real-valued PDE solutions by utilizing the Discrete Hartley Transform, which offers a more efficient parameterization without complex arithmetic.

Method: A comparative analysis of HNO and FNO based on training both operators on various PDE classes, initial conditions, and boundary conditions.

Result: HNO shows superiority for self-adjoint elliptic operators, while FNO is more effective for time-dependent operators with significant phase content; a monotone split is observed correlating with operator phase content.

Conclusion: The choice of spectral basis should align with the symmetry of the solution operator rather than attributing superiority to one method universally.
...
### [231] [When Do Conservation Laws Survive Learned Representations? Certified Horizons for Latent World Models](https://arxiv.org/abs/2606.24945)
*Hongbo Wang*

Main category: cs.LG

TL;DR: This paper investigates the certifiability of conservation laws in physical models after latent representation learning, focusing on how to maintain the integrity of these laws despite potential model defects.


<details>
  <summary>Details</summary>
Motivation: The need to understand how conservation laws can be preserved in learned representations of physical models to ensure reliable predictions and behaviors in simulations.

Method: The authors derive shell-horizon certificates based on the decoded physical invariant, breaking down the errors into representation, readout, and latent-dynamics defects, and evaluate these across different observational settings.

Result: The study shows that while conservation certificates can endure through learned representation, their effectiveness varies based on the geometric priors applied, with notable results on soft invariants in learned settings and traditional symplectic structures yielding limited robustness.

Conclusion: A decoded physical invariant serves as a key component for verifying conservation laws in representation-learned models, emphasizing its role over latent Hamiltonians.
...
### [232] [Conformal Orbit-Valid Trust Horizons for Equivariant World Models](https://arxiv.org/abs/2606.24946)
*Hongbo Wang*

Main category: cs.LG

TL;DR: The paper examines trust-horizon certification for latent world models with symmetries, presenting methods that account for rollout errors and establishing that their calibration achieves conservative bounds in audits, particularly when structures are equivariant.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of learned world models by ensuring their rollout errors are controlled over specified horizons, facilitating reliable predictions in stochastic environments.

Method: The authors develop a certification approach using a raw horizon curve and a split-conformal calibration that adapts the trust-horizon based on known group symmetries and equivariance properties.

Result: After conducting 50 audits, they achieved zero anti-conservative violations, indicating that their method reliably produces conservative estimates, with small errors observed in empirical models, supporting the proposed structural results.

Conclusion: The study confirms that under specific conditions, the trust-horizon certification is effective, suggesting that model equivalence can enhance robustness and assist in achieving reliable performance without extensive recalibrations.
...
### [233] [Swarm-Inspired Generation of Collective Behaviors in Graph Dynamical Systems](https://arxiv.org/abs/2606.24958)
*Ji Chen,Song Chen,Chengzhang Gong,Li Fan,Chao Xu*

Main category: cs.LG

TL;DR: The paper introduces the Swarm-Inspired Emergent Synchronizer (SIES), a graph-dynamical framework that learns local interaction rules for collective behavior, leading to synchronization in various systems and improved performance in graph representation learning.


<details>
  <summary>Details</summary>
Motivation: The aim is to explain and design local interaction rules that enable desired collective behavior and its generalization across different scenarios.

Method: SIES uses a graph-dynamical framework whereby each node acts as an agent with state and task cue, employing adaptive coupling for synchronization control and message passing for graph learning.

Result: SIES learns a coupling operator that efficiently creates synchronization patterns across varying conditions and outperforms existing methods in both synchronization control for robotic locomotion and heterophilous node classification tasks.

Conclusion: SIES demonstrates effectiveness as a learning and generalizable framework for managing synchronization and graph representation challenges in dynamic systems.
...
### [234] [Memory-Efficient Policy Libraries with Low-Rank Adaptation in Reinforcement Learning](https://arxiv.org/abs/2606.25700)
*Samuel Valland Lyngset,Tor Viljen Raanaas,Gard Sveipe,Eirik Møller Nilsen,Jim Torresen,Kai Olav Ellefsen,Tobias Lømo*

Main category: cs.LG

TL;DR: The paper investigates the effectiveness of Parameter-Efficient Fine-Tuning (PEFT) techniques, specifically Low Rank Adaptation (LoRA), for optimizing memory usage and computational performance in robotic applications using Reinforcement Learning (RL).


<details>
  <summary>Details</summary>
Motivation: To determine if PEFT approaches can enhance memory efficiency and computational performance in robotics and RL, particularly in multi-task settings where memory management is crucial.

Method: The authors employed a Proximal Policy Optimization (PPO) algorithm to fine-tune a baseline model across various tasks using LoRA, assessing the effects on memory usage and task success rates.

Result: LoRA reduced memory usage by 20-160 times compared to full fine-tuning of all layers, leading to 90-95% storage savings for deploying multiple specialized policies.

Conclusion: No significant difference in success rates was found between full fine-tuning and LoRA fine-tuning, indicating that LoRA provides an effective method for memory-efficient training in robotics without compromising performance.
...
### [235] [ROAD-VLA: Robust Online Adaptation via Self-Distillation for Vision-Language-Action Models](https://arxiv.org/abs/2606.25800)
*Kejing Wang,Toan Nguyen,Minh Hoang Nguyen,Simon Khan,Flora D. Salim*

Main category: cs.LG

TL;DR: ROAD-VLA is an advantage-guided self-distillation approach that enhances online adaptation of VLA models by transforming sparse rewards into denser token-level supervision.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of effective online adaptation for VLA models due to sparse rewards offering weak supervision.

Method: The proposed method, ROAD-VLA, uses advantage-guided self-distillation to create a proximal teacher in action space, improving training signals by perturbing action-token logits with calibrated advantage estimates.

Result: ROAD-VLA outperforms PPO in seven robotic manipulation environments, both in-distribution and out-of-distribution, showcasing its efficacy in robust online adaptation of VLA models.

Conclusion: The findings demonstrate that ROAD-VLA effectively converts sparse rewards into dense supervision, enabling more successful adaptation of VLA models in varied environments.
...
### [236] [Zero-Shot Neural Priors for Generalizable Cross-Subject and Cross-Task EEG Decoding](https://arxiv.org/abs/2606.23706)
*Baimam Boukar Jean Jacques,Brandone Fonya,Nchofon Tagha Ghogomu,Pauline Nyaboe,Kipngeno Koech*

Main category: eess.SP

TL;DR: This study presents a zero-shot cross-subject EEG decoding framework utilizing a fine-tuned Transformer model to improve generalization in brain-computer interfaces, achieving better performance compared to conventional models.


<details>
  <summary>Details</summary>
Motivation: The aim is to develop robust EEG decoding models that generalize well across different subjects and tasks for effective brain-computer interfaces and neural biomarkers in mental health, addressing issues of inter-subject variability and non-stationary signals.

Method: A zero-shot cross-subject decoding framework using a large-scale EEG dataset, benchmarking various models including a convolutional neural network, a hybrid LSTM, and a Transformer-based model with a progressive unfreezing strategy to enhance adaptation.

Result: The baseline model yielded an normalized root mean square error (nRMSE) of 0.9991, while the fine-tuned Transformer model achieved a significantly improved nRMSE of 0.9799 on unseen subjects.

Conclusion: This research contributes to scalable and calibration-free EEG decoding methods applicable in computational psychiatry and behavioral prediction.
...
### [237] [Coordinate-Queryable Neural Field Reconstruction for EEG Spatial Super-Resolution with Unseen-Electrode Generation](https://arxiv.org/abs/2606.23707)
*Hongjun Liu,Leyu Zhou,Zijianghao Yang,Chao Yao*

Main category: eess.SP

TL;DR: This paper addresses EEG spatial super-resolution (EEGSR) challenges due to random channel missingness and device variability by reformulating it to learn a shared conditional scalp field using partially observed channels, achieving significant improvements in accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of existing EEGSR methods that rely on fixed channel mappings, rendering them ineffective under variable missing channel conditions during testing.

Method: The proposed approach involves a position-guided encoder to summarize available EEG channels into a latent condition, and a conditional implicit neural representation decoder to reconstruct signals at unobserved electrode coordinates, using a fidelity-preserving channel corruption training strategy.

Result: The framework shows effective performance in random missing-channel reconstruction and unseen-electrode signal generation, achieving a 37.5% reduction in NMSE and a 2.12 dB improvement in SNR over the best baseline in strict held-out-electrode scenarios.

Conclusion: The method effectively synthesizes EEG signals at electrode locations that were not included during training, demonstrating its robustness and applicability in real-world EEGSR scenarios.
...
### [238] [WiFi-Based People Counting Using Beam-Steerable Antennas: A Test-bed Study](https://arxiv.org/abs/2606.23710)
*Riccardo Bersan,Anay Ajit Deshpande,Sanaz Kianoush,Daniele Piazza,Stefano Savazzi*

Main category: eess.SP

TL;DR: RF signals can be utilized for personalized services like smart living and healthcare, leveraging Wi-Fi and cellular networks for innovative sensing tools.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of RF signals in enabling personalized services and innovative sensing applications, utilizing existing communication infrastructure.

Method: Analysis of chip designs for Channel State Information (CSI) extraction using MIMO and OFDM technologies in Wi-Fi, particularly focusing on the capabilities introduced by the IEEE 802.11be standard.

Result: The ability of Wi-Fi 7 to support a wider subcarrier bandwidth enhances CSI extraction, facilitating improved human tracking and re-identification based on signal variations.

Conclusion: Wi-Fi signals present significant opportunities for diverse applications in sensing, notably in tracking human behavior and enabling innovative services.
...
### [239] [Low-rank Updates in Slowly Time-varying Graphs for Spatial-Temporal Signal Interpolation](https://arxiv.org/abs/2606.24011)
*Saghar Bagheri,Gene Cheung,Tim Eadie,Antonio Ortega*

Main category: eess.SP

TL;DR: This paper presents a novel method for enhancing graph signal processing by modeling dynamic changes in a graph's adjacency matrices, leveraging low-rank approximations to facilitate improved signal interpolation for spatial-temporal data.


<details>
  <summary>Details</summary>
Motivation: The authors identify that traditional static graphs are inadequate for representing evolving node similarities in spatial-temporal data, necessitating a new approach to graph modeling.

Method: The approach models the differences between two adjacency matrices as a low-rank matrix, using joint optimization to interpolate signals and estimate the changing graph structure across time. The algorithm alternates between updating the signal and the graph using a linear system and proximal gradient descent.

Result: The proposed method improves signal interpolation in comparison to existing time-varying graph models, demonstrating effectiveness in capturing the relationships between nodes over time.

Conclusion: The creation of a neural network architecture through the unrolling of optimization iterations allows for better parameter tuning, enhancing the performance of graph signal processing tasks.
...
### [240] [PROTECT-90: A Fault Dataset for Power System Protection](https://arxiv.org/abs/2606.24298)
*Julian Oelhaf,Georg Kordowich,Christian Bergler,Andreas Maier,Johann Jäger,Siming Bayer*

Main category: eess.SP

TL;DR: PROTECT-90 is an open dataset of 9,022 high-voltage fault simulation episodes designed for reproducible evaluation in power system protection studies.


<details>
  <summary>Details</summary>
Motivation: There is a lack of standardized, publicly available datasets for transparent evaluation of data-driven power system protection methods.

Method: The dataset includes 9,022 short-circuit simulation episodes using EMT simulations on a standardized 90 kV topology, with synchronized voltage and current waveforms measured at eight locations, and comprehensive metadata.

Result: PROTECT-90 provides consistent measurements and detailed documentation, enhancing the reproducibility and comparability of research in power system protection.

Conclusion: The dataset serves as a foundational benchmark for signal processing and learning-based methods in high-voltage fault studies.
