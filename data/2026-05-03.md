...
TL;DR: This paper presents a dual-stream Transformer architecture for automating the detection of mutual gaze and joint attention in caregiver-infant interactions from dual-camera recordings, outperforming existing models and providing a scalable tool for researchers.


<details>
  <summary>Details</summary>
Motivation: The need to automate the labor-intensive process of analyzing mutual gaze and joint attention in developmental psychology.

Method: A dual-stream Transformer architecture utilizing frozen gaze-aware backbones (GazeLLE) and a custom token fusion mechanism for effective detection from synchronized dual-camera recordings.

Result: The proposed model demonstrates superior performance on an ecologically valid dataset, surpassing a convolutional baseline and state-of-the-art multimodal LLMs.

Conclusion: By openly sharing the model and pre-trained weights, the authors aim to support behavioral scientists in applying this technology across various laboratory settings.
...
TL;DR: RecGen is a generative framework that improves the reconstruction of complex multi-object scenes from sparse RGB-D images, achieving superior performance with fewer training resources.


<details>
  <summary>Details</summary>
Motivation: The challenge of accurately reconstructing complex multi-object scenes from limited observations is critical for advancements in computer vision and robotic simulations.

Method: RecGen employs a generative framework for probabilistic joint estimation of object and part shapes, as well as their poses, utilizing compositional synthetic scene generation and strong 3D shape priors.

Result: RecGen demonstrates state-of-the-art performance on heavily occluded datasets, effectively managing occlusions, symmetry, and intricate geometrical and textural challenges while requiring significantly fewer training meshes than SAM3D.

Conclusion: Despite using 80% fewer training meshes, RecGen outperforms the previous state of the art in geometric quality, texture reconstruction, and pose estimation, showcasing its efficiency and effectiveness.
...
TL;DR: InterPartAbility is a novel TI-ReID method that enhances interpretability through explicit part-wise matching and phrase-region grounding, improving retrieval explanations and maintaining competitive accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current TI-ReID methods that lack interpretable decisions and depend on inadequate interpretability approaches.

Method: Introduces InterPartAbility, which uses a new patch-phrase interaction module (PPIM) for training with concept-level guidance and constrains self-attention in CLIP ViT for better spatial activation alignment.

Result: InterPartAbility achieves state-of-the-art interpretability metrics while maintaining robust retrieval accuracy, demonstrated on benchmarks such as CUHK-PEDES and ICFG-PEDES.

Conclusion: This method offers significant improvements in interpretability for TI-ReID tasks, validating its utility through quantitative measures.
...
TL;DR: The paper presents a refined model for livestock monitoring that enhances accuracy while significantly reducing resource demands.


<details>
  <summary>Details</summary>
Motivation: To make precision livestock farming (PLF) more accessible by reducing the memory requirements of AI models for edge devices.

Method: Distillation of a large perception encoder into a smaller model using techniques such as a multi-scale student design, a specialized distillation loss, and memory management strategies.

Result: The resulting model achieved high accuracy metrics and drastically reduced both the parameter count and peak VRAM usage, fitting within the constraints of low-resource edge devices.

Conclusion: The compressed pipeline can effectively facilitate individual livestock monitoring on commodity hardware while also enabling potential longitudinal data collection for health monitoring.
...
TL;DR: Knowledge distillation enhances smaller models for plant species and disease recognition, maintaining performance while reducing computational demands.


<details>
  <summary>Details</summary>
Motivation: To address the high computational cost and deployment challenges of large models in resource-constrained environments for plant recognition tasks.

Method: Knowledge distillation is employed to transfer knowledge from large pretrained models to smaller architectures, evaluated on two benchmarks with varying training regimes.

Result: Knowledge distillation consistently improves performance, allowing smaller models to match larger models' accuracy while significantly reducing computational cost.

Conclusion: Knowledge distillation techniques can facilitate efficient and scalable deployment of plant recognition systems in practical applications.
...
### [6] [FineState-Bench: Benchmarking State-Conditioned Grounding for Fine-grained GUI State Setting](https://arxiv.org/abs/2604.27974)
*Fengxian Ji,Jingpu Yang,Zirui Song,Yuanxi Wang,Zhexuan Cui,Yuke Li,Qian Jiang,Xiuying Chen*

Main category: cs.CV

TL;DR: The paper introduces FineState-Bench, a benchmark to evaluate the capacity of large vision-language models for fine-grained GUI interaction, highlighting the challenges and limitations in current evaluations.


<details>
  <summary>Details</summary>
Motivation: To address the inadequacies in evaluating LVLMs for precise state-conditioned GUI interactions, particularly in diagnosing failures in achieving exact target states.

Method: FineState-Bench, comprising 2,209 interaction instances across multiple platforms, introduces FineState-Metrics for detailed performance evaluation and a Visual Diagnostic Assistant for enhanced visual grounding diagnostics.

Result: The exact goal-state success rates are low, with fine-tuning from the VDA yielding significant improvements in success rates for the Gemini-2.5-Flash model, but overall accuracy remains inadequate.

Conclusion: The study reveals a critical need for improved visual grounding in LVLMs to achieve reliable interaction in fine-grained state-conditioned scenarios.
...
TL;DR: The paper introduces HQ-UNet, a hybrid quantum-classical U-Net for semantic segmentation in remote sensing, which utilizes a compact quantum circuit to improve feature representation under quantum hardware limitations, outperforming classical U-Net.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of classical deep learning architectures like U-Net in semantic segmentation for remote sensing, particularly their high parameter requirement and the challenges of applying QML to high-dimensional images.

Method: The hybrid architecture HQ-UNet incorporates a non-pooling quantum convolutional module at the bottleneck of a classical U-Net, making the quantum component shallow and parameter-efficient while enhancing encoder feature representation.

Result: HQ-UNet achieved a mean IoU of 0.8050 and overall accuracy of 94.76% on the LandCover.ai dataset, surpassing the performance of the classical U-Net baseline.

Conclusion: The findings indicate that incorporating compact quantum components can significantly improve feature representation in remote sensing image segmentation, suggesting hybrid designs as a viable approach for dense prediction tasks while adhering to quantum hardware limitations.
...
TL;DR: This paper investigates the influence of attributes like BMI, pose, and gender on transformer-based person re-identification systems, revealing that these attributes are encoded differently across layers and modalities.


<details>
  <summary>Details</summary>
Motivation: The research aims to address fairness and generalization concerns in person re-identification systems by analyzing how specific attributes are represented in learned features.

Method: The authors propose a framework that quantifies the expressivity of attributes using mutual information, applying it to three transformer-based ReID models on a large dataset and extending the analysis to cross-spectral identification.

Result: Findings indicate that BMI has the highest expressivity, particularly in deeper layers, with the ranking of attributes showing variations across layers and training epochs.

Conclusion: The study concludes that transformer-based ReID embeddings encapsulate a hierarchy of implicit attributes, with morphometric information prominently represented and a notable contribution of pose under cross-spectral conditions.
...
TL;DR: The paper discusses a modular workflow for mapping linear woody features like hedges using Earth observation data, improving ecosystem services in agricultural areas.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance ecosystem services and biodiversity in agricultural landscapes by managing and preserving linear woody features through systematic mapping.

Method: The method involves a modular workflow that consolidates diverse Earth observation data into a binary vegetation mask and utilizes a deep neural network for shape differentiation, producing maps without needing retraining.

Result: The workflow successfully generates national-scale linear woody feature maps for Germany from multiple data sources, showing competitive results compared to existing maps and evaluation standards.

Conclusion: The design's modularity supports scalability and generalizability for mapping linear woody features beyond Germany, addressing methodological challenges in varied landscapes.
...
TL;DR: The paper introduces VTBench, a framework for multimodal time-series classification (TSC) that combines raw numerical data with chart-based visualizations for improved interpretability and accuracy.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of chart-based visualizations in time-series classification, addressing limitations of traditional numerical input models and previous visual encoding methods.

Method: VTBench employs a modular architecture allowing various fusion strategies of raw sequences and several types of visual representations (line, area, bar, scatter), systematically evaluating across 31 UCR datasets.

Result: Experiments show that chart-only models can be competitive, combining multiple chart types enhanced accuracy, and multimodal models can retain performance or degrade accuracy depending on redundancy of information.

Conclusion: VTBench provides a comprehensive framework for interpretable multimodal TSC, offering guidelines for effective chart selection and fusion strategies.
...
TL;DR: The paper presents an improved student behavior recognition model, ALC-YOLOv8s, addressing challenges in classroom settings, achieving better performance in detecting student behavior.


<details>
  <summary>Details</summary>
Motivation: To enhance teaching quality analysis through reliable student behavior recognition despite challenges in real classroom environments, such as occlusions and imbalanced data.

Method: The ALC-YOLOv8s model is developed by improving YOLOv8s, utilizing SPPF-LSKA for feature extraction, CFC-CRB and SFC-G2 for feature fusion, and ATFLoss for addressing underrepresented classes.

Result: The model demonstrates performance improvements of 1.8% in mAP50 and 2.1% in mAP50-95 over the baseline, showing superior performance against other detection methods.

Conclusion: The proposed model effectively addresses the complexities of automatic student behavior recognition in classrooms, supporting enhanced classroom participation assessments.
...
TL;DR: YOSE is a novel fine-tuning framework for video object removal that enhances efficiency by reducing inference time during the processing of masked regions.


<details>
  <summary>Details</summary>
Motivation: To address the high inference latency in DiT-based video object removal methods, exemplified by MiniMax Remover's low FPS due to dense computations.

Method: YOSE introduces Batch Variable-length Indexing (BVI) for adaptive token selection and a Diffusion Process Simulator (DiffSim) for approximating the diffusion process of unmasked tokens, enabling variable-length token processing.

Result: YOSE achieves up to 2.5X acceleration in inference speed for 70% of cases while preserving visual quality comparable to existing methods.

Conclusion: The proposed methods allow for significant reduction in computation related to unmasked regions, demonstrating efficiency without sacrificing quality.
...
TL;DR: The paper presents a training-free framework for improving zero-shot web content classification by refining category definitions using feedback from misclassifications.


<details>
  <summary>Details</summary>
Motivation: Existing web filtering systems struggle with accurate classification due to dynamic web content, leading to misclassification from poor definitions.

Method: An adaptive iterative definition refinement framework using LLMs to optimize category definitions, with three strategies: example-guided, confusion-aware, and history-aware.

Result: Experiments show that the proposed methods enhance classification performance across various embedding models, highlighting the importance of definition quality.

Conclusion: The study establishes definition refinement as a pivotal factor in zero-shot classification, creating a benchmark dataset for future research.
...
TL;DR: JI-ADF is a trimodal deep learning framework for skin lesion classification that integrates dermoscopic images, clinical photographs, and patient metadata to improve diagnostic accuracy in clinical settings.


<details>
  <summary>Details</summary>
Motivation: To improve skin lesion classification by utilizing multimodal evidence beyond just dermoscopic images, addressing a significant gap in existing computer-aided systems.

Method: JI-ADF combines multimodal representation learning with modality-specific supervision and an adaptive decision fusion mechanism, enhanced by a multimodal fusion attention (MMFA) module for better cross-modal reasoning.

Result: JI-ADF shows strong performance on the MILK10k benchmark, enhancing sensitivity and Dice score while ensuring high specificity and good calibration across various lesion types.

Conclusion: JI-ADF establishes a reliable groundwork for multimodal skin lesion classification, suitable for real-world clinical applications.
...
TL;DR: This paper presents a high-precision gait recognition framework that improves biometric identification by fusing gait dynamics with body shape characteristics using a multi-branch deep learning approach.


<details>
  <summary>Details</summary>
Motivation: To enhance gait recognition performance by addressing challenges such as viewpoint variation, clothing change, and carrying conditions that hinder accurate identification.

Method: Utilizes a High-Resolution Network for keypoint estimation and constructs three complementary feature branches (body proportion, gait velocity, skeletal motion) integrated through a Multi-Branch Feature Fusion module based on a ResNet-50 backbone.

Result: Achieves a Rank-1 accuracy of 94.52% under normal walking conditions, demonstrating superior performance in skeleton-based recognition, especially for subjects wearing coats.

Conclusion: The proposed framework effectively captures rich biometric cues from gait, outperforming existing methods under various conditions, thus advancing the field of biometric recognition.
...
TL;DR: CasLayout is a cascaded diffusion framework for realistic 3D indoor scene generation that addresses data scarcity and structural consistency by decomposing the process into four sub-stages, enhancing controllability and integrating LLMs and VLMs.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to tackle the challenges of generating realistic 3D indoor scenes, focusing on data scarcity and the need for coherent architectural constraints and semantic consistency.

Method: CasLayout employs a cascaded diffusion framework, decomposing scene generation into four stages: predicting furniture elements, refining sizes/embeddings, modeling spatial relationships, and generating Oriented Bounding Boxes. It introduces a sparse relation graph and utilizes a VAE for latent space encoding.

Result: Experiments show that CasLayout outperforms existing methods in terms of fidelity and diversity, while also improving controllability for practical applications.

Conclusion: CasLayout not only reduces data requirements but also effectively integrates advanced models for enhanced scene generation, allowing for better compliance with physical and semantic constraints.
...
TL;DR: The DSCC framework improves hyperspectral image classification by integrating spatial coherence through a dual-stage process of clustering and classification, yielding superior performance.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of existing superpixel-based methods in hyperspectral image classification, which fail to ensure regional consistency and boundary alignment due to inherent contradictions between clustering and pixel-wise classification.

Method: The DSCC framework decouples clustering from classification by forming spectral supertokens from pixel groups based on spectral similarity and spatial proximity, followed by token-level classification. Key components include multi-criteria feature distance computation, locality-aware assignment regularization for boundary-preserving tokens, density-isolation for center selection, and a soft-label scheme for mixed-class robustness.

Result: The framework achieves a CF1 score of 0.728 at a speed of 197.75 FPS on the WHU-OHS dataset, demonstrating an improved accuracy-efficiency trade-off over existing methods.

Conclusion: The proposed dual-stage paradigm effectively enhances hyperspectral image classification performance, validated through extensive experiments.
...
TL;DR: CriticVLA is a two-stage framework that improves driving decision-making in autonomous vehicles by integrating a VLA-based critic to refine actions after initial trajectory generation.


<details>
  <summary>Details</summary>
Motivation: To enhance the performance of vision language action models in autonomous driving by incorporating a critic mechanism that refines decision-making in complex scenarios.

Method: CriticVLA operates in two stages: it first generates a rough trajectory and then refines it using multimodal evaluation and optimization, guided by a VLA-based critic.

Result: CriticVLA achieved a 73.33% total success rate and a 30% improvement in challenging scenarios during extensive closed-loop experiments on the Bench2Drive benchmark.

Conclusion: The introduction of a critic in the VLA framework significantly enhances the quality of driving behaviors in autonomous driving applications.
...
TL;DR: VeraRetouch is a lightweight, fully differentiable multi-task photo retouching framework utilizing a Vision-Language Model and a novel dataset, achieving high performance with reduced parameters.


<details>
  <summary>Details</summary>
Motivation: To improve photo retouching by overcoming limitations of existing methods that are non-differentiable and prone to optimization issues.

Method: We propose a fully differentiable framework using a Vision-Language Model for retouching plans and a Retouch Renderer for end-to-end training, supported by the AetherRetouch-1M+ dataset and DAPO-AE strategy.

Result: VeraRetouch achieves state-of-the-art performance across benchmarks, with reduced system requirements suitable for mobile use.

Conclusion: This framework enhances photo retouching capabilities significantly, proving efficient for real-time applications and generalization.
...
TL;DR: The paper introduces COHERENCE, a benchmark for evaluating Multimodal Large Language Models' understanding of interleaved image-text contexts, addressing the lack of such assessments in multimodal comprehension.


<details>
  <summary>Details</summary>
Motivation: The motivation is to assess MLLMs' capabilities in understanding complex interleaved multimodal contexts, which is crucial for tasks like document reading.

Method: COHERENCE benchmark comprises 6,161 high-quality questions based on interleaved image-text content across four domains, along with a detailed error analysis.

Result: The benchmark allows for systematic evaluation of MLLMs' abilities to recognize, align, and reason over multimodal information in interleaved contexts.

Conclusion: COHERENCE fills a critical gap in existing multimodal benchmarks and provides insights into the limitations of current MLLMs in fine-grained image-text understanding.
...
TL;DR: This study investigates the transferability of physical adversarial attacks across various vision-language models used in autonomous driving, revealing high effectiveness rates in real-world scenarios.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand the robustness of vision-language models against physical adversarial attacks, especially their transferability across different architectures, in the context of autonomous driving.

Method: The study involved a systematic cross-architecture evaluation of three VLMs (Dolphins, OmniDrive, LeapVAD) using physical patches on roadside structures in both crosswalk and highway scenarios to assess adversarial transferability.

Result: The results showed significant adversarial transferability, with transfer rates between 73-91% and effective manipulation during critical decision-making windows (64.7-79.4%).

Conclusion: The findings indicate that physical adversarial attacks pose a substantial risk to VLMs in autonomous vehicles, emphasizing the need for improved model robustness.
...
TL;DR: The paper presents a novel 3D sparse-view synthesis framework designed for unconstrained real-world scenarios, prioritizing effective rendering despite the presence of distractors.


<details>
  <summary>Details</summary>
Motivation: Current methods mainly focus on constrained environments or enhance 3D representations using dense collections, which limits effectiveness in real-world applications with transient elements.

Method: The authors introduce a reference-guided view refinement technique utilizing a diffusion model with a transient mask and reference image, coupled with pseudo-view generation and a sparsity-aware Gaussian replication strategy to improve 3D representations.

Result: Experiments show significant performance improvement over existing methodologies, with specific metrics indicating PSNR increases of 17.2%, SSIM of 10.8%, and LPIPS of 4.0%.

Conclusion: The proposed framework achieves high-fidelity 3D rendering in unconstrained environments, offering a more accessible solution for 3D representation without intensive data acquisition.
...
TL;DR: Softmax-GS improves 3D Gaussian Splatting by addressing view inconsistencies and boundary diffusion through softmax-based competition, enhancing reconstruction quality and efficiency.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting suffers from artifacts due to overlapping Gaussians and diffuse boundaries, affecting view consistency and edge sharpness.

Method: Softmax-GS introduces a softmax-based competition between overlapping Gaussians, allowing control over color blending and boundary definition while preserving order invariance and output transmittance during rendering.

Result: Softmax-GS was validated through ablation experiments and real-world benchmarks, demonstrating improvements in reconstruction quality and parameter efficiency, achieving state-of-the-art performance.

Conclusion: The proposed method effectively resolves issues inherent in traditional 3D Gaussian Splatting, enabling enhanced visual fidelity and efficiency in novel view synthesis.
...
TL;DR: CatSignal is a probabilistic framework for inferring intent in non-speaking agents, achieving high accuracy by combining spatial context and behavioral observations.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of inferring intents from non-verbal agents in complex environments where communication is limited or unclear.

Method: CatSignal uses a Bayesian-inspired approach with context-gated Product-of-Experts to model intent distribution, combining spatial context, pose dynamics, and acoustic cues.

Result: The method achieved an accuracy of 77.72% in intent inference, outperforming simpler feature concatenation and late-fusion strategies, and reduced context-driven prediction errors.

Conclusion: The proposed framework demonstrates superior intent inference in ambiguous scenarios, providing a robust solution for understanding non-speaking embodied agents.
...
TL;DR: This paper proposes LA-Pose, a camera pose estimation method leveraging inverse-dynamics pretraining for improved performance with minimal labeled data.


<details>
  <summary>Details</summary>
Motivation: The motivation is to challenge the reliance on fully supervised training with 3D annotations by using scalable self-supervised learning techniques.

Method: The method employs inverse- and forward-dynamics models to extract latent action representations, which are then repurposed for camera pose estimation.

Result: LA-Pose demonstrates over 10% higher pose accuracy on Waymo and PandaSet benchmarks compared to recent feed-forward methods, using significantly less labeled data.

Conclusion: The work is pioneering in showing the effectiveness of inverse-dynamics self-supervised learning in pose estimation.
...
TL;DR: EdgeFM is a lightweight framework for optimizing vision-language model (VLM) inference on edge devices, enhancing performance and cross-platform adaptability.


<details>
  <summary>Details</summary>
Motivation: Existing frameworks for VLMs either lack efficiency due to general-purpose designs or are tied to closed-source ecosystems, hindering adaptability and increasing latency.

Method: EdgeFM leverages agent-tuned kernel optimizations, modularizing reusable skills and reducing latency by removing non-essential features, while enabling direct invocation of optimizations.

Result: EdgeFM outperforms conventional vendor-specific toolchains, showing up to 1.49 times speedup over TensorRT-Edge-LLM on the NVIDIA Orin platform, improving inference performance significantly.

Conclusion: EdgeFM offers an open-source solution that enhances inference performance for industrial edge applications, supporting various platforms and improving portability.
...
TL;DR: Uni-HOI is a unified framework for modeling 4D human-object interaction (HOI) using large language models and variational autoencoders to integrate text, human motion, and object motion.


<details>
  <summary>Details</summary>
Motivation: To address the lack of a unified framework for diverse HOI tasks reliant on task-specific architectures.

Method: A two-stage training strategy involving multi-task learning on a large-scale dataset followed by task-specific fine-tuning, utilizing LLMs and VQ-VAEs for converting heterogeneous motion data.

Result: Uni-HOI demonstrates significant performance improvements across various HOI tasks including text-driven HOI generation and motion prediction.

Conclusion: Uni-HOI provides a comprehensive approach to 4D HOI, effectively integrating multi-modal data for enhanced model performance.
...
TL;DR: The paper introduces the IRON dataset, the first large-scale annotated infrared dataset for off-road freespace detection, and proposes IRONet, a framework that enhances temporal perception for autonomous driving using this dataset, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: To address the limitations in off-road nighttime driving caused by unreliable visible-light perception and the lack of annotated infrared datasets for temporally consistent freespace detection.

Method: The authors present IRONet, which utilizes a memory-attention mechanism and a mask decoder to combine historical context for improved freespace detection, leveraging the newly created IRON dataset.

Result: IRONet achieves state-of-the-art performance on the IRON dataset, reaching 82.93% IoU and 90.66% F1 score during real-time inference, while also generalizing well to RGB modalities on other datasets.

Conclusion: The establishment of the IRON dataset and the IRONet framework lays a foundation for advancing all-day off-road autonomous driving and further research in infrared temporal perception.
...
TL;DR: REVIVE 3D is a two-stage pipeline that generates 3D assets from flat images, addressing the challenge of limited 3D cues in 2D inputs.


<details>
  <summary>Details</summary>
Motivation: To improve the generation of voluminous 3D assets from flat images, which is a significant challenge due to limited 3D information in these inputs.

Method: The method involves a two-stage process: (1) Inflated Prior construction for global volume recovery and part-aware detail capture, and (2) 3D Latent Refinement to denoise the inflated latent using the geometric cues of the prior.

Result: The proposed framework supports image-conditioned 3D editing and introduces two metrics, Compactness and Normal Anisotropy, validated through a user study, indicating alignment with human perception.

Conclusion: REVIVE 3D achieves state-of-the-art performance on a challenging dataset, demonstrating its effectiveness and quality in generating 3D assets from flat images.
...
TL;DR: This paper presents Edit-R1, a novel framework for image editing using a reasoning verifier-based reward model to improve edit quality in Reinforcement Learning from Human Feedback (RLHF).


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance image editing tasks in RLHF, addressing the limitations of existing edit reward models that deliver biased evaluations without considering diverse instruction requirements.

Method: The method involves developing a chain-of-thought verifier-based reasoning reward model (Edit-RRM) that breaks down editing instructions, evaluates images against these criteria, and aggregates results into fine-grained, interpretable rewards; combining supervised fine-tuning with Group Contrastive Preference Optimization for reinforcement learning.

Result: Edit-RRM outperforms established vision language models (VLMs) such as Seed-1.5-VL and Seed-1.6-VL in image editing tasks and shows improved performance with increased model size, yielding better results for editing models like FLUX.1-kontext.

Conclusion: The results indicate that Edit-R1 effectively enhances image editing quality through a detailed and interpretable reward system, demonstrating significant performance gains in comparison with existing models.
...
TL;DR: The paper challenges the Spatial Funnel Hypothesis by introducing a new inversion framework that reveals how classifiers in CNNs operate through destructive interference, leading to precise channel selection for effective classification.


<details>
  <summary>Details</summary>
Motivation: To test the Spatial Funnel Hypothesis in CNN interpretability, which suggests that deep encoders suppress background noise, yet this remains unverified due to the limitations of existing visualization tools.

Method: An inversion framework based on magnitude-phase decoupling and Local Adjoint Correctors is introduced, which ensures reconstructions are derived from genuinely active channels and provides a geometric probe for analysis.

Result: The findings demonstrate that per-channel inversions show strong superposition in vision encoders; destructive interference is proven to play a role in classification, thus contradicting the initial Spatial Funnel Hypothesis.

Conclusion: A new covariance-volume channel selection algorithm is developed, showing the relationship between OOD failure and covariance volume, and the framework is applicable to attention mechanisms without retraining.
...
TL;DR: This paper investigates self-supervised learning (SSL) for automated plant recognition and finds that traditional SSL augmentations are ineffective; it proposes alternative strategies that enhance model performance.


<details>
  <summary>Details</summary>
Motivation: To improve automated plant recognition for biodiversity monitoring using self-supervised learning, addressing limitations of expert-labeled data in existing methods.

Method: The study evaluates various augmentations for SSL, identifies ineffective strategies, and proposes alternative transformations. It trains a model (SimDINOv2) on the iNaturalist 2021 Plantae subset and compares performance against ImageNet-1K.

Result: The proposed augmentations improve representation learning, showcasing stronger results on SSL with domain-specific data, and achieving performance comparable to or exceeding supervised methods in few-shot scenarios.

Conclusion: Domain-adapted augmentation strategies and careful dataset selection are vital for enhancing self-supervised learning models for plant recognition, offering a pathway for scalable biodiversity monitoring.
...
TL;DR: This study enhances 3D Gaussian splatting for cone-beam computed tomography by integrating wavelet transforms, proposing Residual Gaussian Splatting to address spectral bias and improve detail preservation.


<details>
  <summary>Details</summary>
Motivation: Conventional photometric optimization in 3D Gaussian splatting struggles with spectral bias under ultra sparse-view conditions, resulting in loss of detail.

Method: The proposed method, Residual Gaussian Splatting, uses a novel spectrally-decoupled Gaussian representation to decompose the volumetric field, facilitating a physically consistent optimization between geometric and high-frequency components.

Result: RGS significantly improves image reconstruction, capturing fine geometric textures and handling the artifact-detail trade-off better than existing methods.

Conclusion: The approach enhances visual fidelity in complex anatomical structures in clinical datasets, outperforming traditional neural rendering techniques.
...
TL;DR: The paper examines how text styles affect the descriptions provided by Large Visual Language Models (LVLMs) for concepts despite the independent meaning of words.


<details>
  <summary>Details</summary>
Motivation: To explore the impact of visual text styles on the semantic descriptions generated by LVLMs, particularly in distinguishing between functional and decorative styles.

Method: Experiments where LVLMs identify concepts from visual text rendered in different styles to assess influence on attribute-based descriptions.

Result: Findings indicate that the text style, whether functional or decorative, affects LVLMs' attribute descriptions, even when concept identification is accurate.

Conclusion: The results highlight the need for style-aware evaluations in LVLM systems due to the observed style leakage influencing semantic inferences.
...
TL;DR: RIHA is a framework for automatic radiology report generation that aligns visual and textual data at multiple levels to improve report accuracy.


<details>
  <summary>Details</summary>
Motivation: To reduce radiologists' workload and errors by automatically generating structured diagnostic reports from medical images.

Method: RIHA uses a hierarchical alignment approach with a Visual Feature Pyramid (VFP) and Text Feature Pyramid (TFP), integrated through a Cross-modal Hierarchical Alignment module, utilizing optimal transport for alignment and Relative Positional Encoding for improved token-level mapping.

Result: RIHA outperforms existing state-of-the-art models on benchmark datasets, achieving better natural language generation and clinical metrics.

Conclusion: The work addresses significant alignment challenges in radiology report generation, leading to enhanced accuracy and effectiveness in generating diagnostic reports.
...
### [36] [Machine Unlearning for Class Removal through SISA-based Deep Neural Network Architectures](https://arxiv.org/abs/2604.27804)
...
TL;DR: This paper presents a modified SISA framework for class-level unlearning in CNNs to address data privacy concerns in AI, demonstrating effective unlearning while maintaining model performance.


<details>
  <summary>Details</summary>
Motivation: The study addresses ethical and legal challenges surrounding data privacy and user consent due to the use of proprietary user data in AI model training.

Method: The research uses a modified SISA framework for class-level unlearning, incorporating a reinforced replay mechanism and a gating network to facilitate selective forgetting.

Result: Experimental results show that the modified SISA framework allows effective class unlearning without compromising model performance, while also minimizing retraining efforts.

Conclusion: The findings suggest that SISA-based unlearning can be effectively integrated into privacy-sensitive AI applications.
...
TL;DR: World2Minecraft converts real-world environments into Minecraft for improved embodied intelligence, addressing data issues with a new automated dataset.


<details>
  <summary>Details</summary>
Motivation: Existing simulation environments for embodied intelligence face challenges with data contamination and flexibility.

Method: The paper proposes World2Minecraft, which uses 3D semantic occupancy prediction to transform real-world scenes into structured Minecraft environments and introduces a scalable data pipeline for occupancy dataset creation.

Result: The authors developed MinecraftOcc, a dataset with 100,165 images from 156 indoor scenes, enhancing occupancy prediction and complementing current datasets.

Conclusion: World2Minecraft enables customizable research in embodied AI and poses a challenge to state-of-the-art methods in occupancy prediction.
...
TL;DR: This paper introduces Fake3DGS, a dataset for 3D fake detection, highlighting the inadequacy of 2D detectors against manipulated 3D images and proposing a novel 3D detection method.


<details>
  <summary>Details</summary>
Motivation: To address the emerging security concerns associated with manipulated 3D content due to advances in neural rendering techniques.

Method: The paper introduces the Fake3DGS dataset and a new 3D-aware detection method that uses multi-view coherence and Gaussian splatting features.

Result: Current state-of-the-art 2D detectors fail to effectively differentiate between original and manipulated 3D rendered images.

Conclusion: The study shows significant improvement in recognizing modified 3D content and emphasizes the need for 3D authenticity assessments, sharing resources for further research.
...
TL;DR: The paper introduces the CURVAS-PDACVI Dataset and Challenge to improve preoperative assessment of vascular invasion in pancreatic ductal adenocarcinoma (PDAC) using AI, highlighting the limitations of existing methods in handling diagnostic ambiguities.


<details>
  <summary>Details</summary>
Motivation: To improve the accuracy of preoperative staging and surgical planning for PDAC by addressing challenges related to vascular invasion assessment and inter-rater variability among radiologists.

Method: An open benchmark dataset (CURVAS-PDACVI) is introduced, featuring five expert annotations per scan, along with a multi-metric evaluation framework that includes probabilistic calibration for AI methods in assessing vascular invasion.

Result: Evaluation of six AI methods reveals that those optimized for binary segmentation struggle with high-complexity cases and inter-rater disagreement, while methods accounting for such disagreements yield better-calibrated and more robust results.

Conclusion: The study emphasizes the need for uncertainty-aware probabilistic models in PDAC preoperative decision-making, as traditional volumetric accuracy does not reliably indicate surgical utility at critical tumor-vessel interfaces.
...
TL;DR: SECOS enhances open-world semi-supervised learning by predicting textual labels from a candidate set directly, using external knowledge for semantic alignment and achieving improved effectiveness over existing methods.


<details>
  <summary>Details</summary>
Motivation: There is a need for OWSSL models to accurately classify samples from both known and novel classes without relying solely on post-processing techniques.

Method: SECOS employs external knowledge to extract and align semantic representations, directly predicting labels for samples from a candidate set.

Result: SECOS outperforms existing OWSSL methods by up to 5.4% in direct label prediction, even under lenient evaluation conditions.

Conclusion: The proposed SECOS model effectively addresses the limitations of current OWSSL methods by providing explicit supervision and semantic alignment.
...
TL;DR: The paper presents ClipTBP, a framework for video moment retrieval that enhances multimodal alignment by accounting for semantic relationships among answer segments and improving boundary prediction accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing video retrieval models that fail to consider multi-segment relationships and struggle with irrelevant segments due to visual similarities.

Method: ClipTBP employs a clip-pair temporal boundary prediction framework and introduces a clip-level alignment loss to learn semantic relationships, alongside predicting accurate temporal boundaries using main and auxiliary losses.

Result: ClipTBP shows consistent performance improvements across various models and exhibits robust boundary predictions in ambiguous query situations.

Conclusion: The proposed framework effectively enhances video moment retrieval by incorporating boundary-aware learning, leading to better alignment and exclusion of irrelevant segments.
...
### [42] [Decoding Scientific Experimental Images: The SPUR Benchmark for Perception, Understanding, and Reasoning](https://arxiv.org/abs/2604.27604)
...
TL;DR: SPUR is a benchmark for evaluating scientific image perception and reasoning, consisting of 4,264 QA pairs from 1,084 curated images, highlighting limitations in current AI models.


<details>
  <summary>Details</summary>
Motivation: The need for a robust benchmark to assess the capabilities of MLLMs in understanding and interpreting scientific images accurately and effectively.

Method: SPUR evaluates MLLMs using three innovations: fine-grained perception across visual dimensions, understanding complex cross-panel relations, and assessing expert-level qualitative and quantitative reasoning.

Result: 20 MLLMs and four MCoT methods were tested, revealing a significant gap between model performance and expert interpretation requirements.

Conclusion: The findings indicate a critical limitation of current AI models in scientific image analysis, highlighting the need for advancements in AI for Science.
...
### [43] [Robust Lightweight Crack Classification for Real-Time UAV Bridge Inspection](https://arxiv.org/abs/2604.27617)
*Wei Li,Haisheng Li,Weijie Li,Jiandong Wang,Kaichen Ma,Luming Yang*

Main category: cs.CV

TL;DR: This paper proposes a lightweight deep learning framework for UAV-based bridge inspection, addressing challenges like weak crack features and class imbalance, while achieving high accuracy and speed.


<details>
  <summary>Details</summary>
Motivation: The need for effective crack detection in bridge structural health monitoring using UAVs, despite existing challenges in features, imaging, class imbalance, and computational constraints.

Method: A unified lightweight convolutional neural network framework comprising a lightweight backbone, Convolutional Block Attention Module (CBAM), robust augmentation strategy, and Focal Loss for tackling class imbalance.

Result: The method demonstrates an inference speed of 825 FPS, with an improvement in F1-score by 2.51% and recall by 3.95% over the baseline model, while maintaining only 11.21M parameters and 1.82G FLOPs.

Conclusion: The study offers a practical solution for real-time UAV bridge inspections, balancing accuracy, speed, and robustness, with the potential for further application in ground-station assisted deployments.
...
### [44] [SpaAct: Spatially-Activated Transition Learning with Curriculum Adaptation for Vision-Language Navigation](https://arxiv.org/abs/2604.27620)
*Pengna Li,Kangyi Wu,Shaoqing Xu,Fang Li,Hanbing Li,Lin Zhao,Kailin Lyu,Long Chen,Zhi-Xin Yang,Nanning Zheng*

Main category: cs.CV

TL;DR: SpaAct is a training framework for Vision-and-Language Navigation (VLN) that enhances spatial awareness in VLMs through backward action reasoning and forward transition prediction.


<details>
  <summary>Details</summary>
Motivation: To enable VLMs to navigate in unseen 3D environments effectively using natural-language instructions by embedding two crucial reasoning capabilities.

Method: SpaAct introduces Action Retrospection and Future Frame Selection tasks to provide supervision for backward action reasoning and forward transition prediction, combined with TriPA for progressive curriculum learning.

Result: SpaAct significantly improves navigation performance in VLMs, achieving state-of-the-art results in VLN-CE benchmarks.

Conclusion: By fostering dynamic spatial awareness, SpaAct facilitates better navigation capabilities in VLMs and will be made publicly available for further research.
...
### [45] [FUN: A Focal U-Net Combining Reconstruction and Object Detection for Snapshot Spectral Imaging](https://arxiv.org/abs/2604.27653)
*Dahua Gao,Yubo Dong,Anqi Li,Zhenyuan Lin,Ang Gao,Danhua Liu,Guangming Shi*

Main category: cs.CV

TL;DR: The paper introduces the Focal U-shaped Network (FUN) for real-time hyperspectral imaging and object detection, which integrates reconstruction and detection tasks with improved efficiency over existing methods.


<details>
  <summary>Details</summary>
Motivation: To enhance real-time object detection in hyperspectral imaging by overcoming slow acquisition speeds and time-consuming post-capture reconstruction.

Method: The Focal U-shaped Network (FUN) utilizes a shared U-shaped backbone for multi-task learning, incorporating a novel focal modulation technique to efficiently process spatial and spectral features without self-attention.

Result: FUN achieves state-of-the-art performance in hyperspectral image reconstruction and object detection, using 40% fewer parameters and 30% less computation than previous methods.

Conclusion: The proposed framework is promising for deployment in real-time applications, and the provided dataset supports future research in this area.
...
### [46] [MSR:Hybrid Field Modeling for CT-MRI Rigid-Deformable Registration of the Cervical Spine with an Annotated Dataset](https://arxiv.org/abs/2604.27654)
...
TL;DR: This paper presents a new CT-MRI registration framework for the cervical spine, addressing challenges related to anatomical complexity and data limitations.


<details>
  <summary>Details</summary>
Motivation: Accurate registration of cervical spine CT and MRI images is crucial for preoperative planning due to the region's complexity and risk of injury.

Method: The authors develop the MSR framework, which features a rigid registration module for individual vertebrae alignment and a deformable registration module enhanced by an MSL block, utilizing Mamba and Swin Transformer models.

Result: The MSR framework improves registration accuracy by fusing rigid and deformable deformation fields, thus better preserving anatomical consistency.

Conclusion: The dataset R-D-Reg and the MSR registration framework are publicly available, contributing to advancements in cervical spine imaging.
...
### [47] [EviMem: Evidence-Gap-Driven Iterative Retrieval for Long-Term Conversational Memory](https://arxiv.org/abs/2604.27695)
*Yuyang Li,Yime He,Zeyu Zhang,Dong Gong*

Main category: cs.CV

TL;DR: EviMem offers a novel approach for long-term conversational memory by targeting evidence gaps in retrieval processes, leading to improved accuracy and lower latency in multi-hop questioning.


<details>
  <summary>Details</summary>
Motivation: To enhance long-term conversational memory by addressing the inadequacies of single-pass retrieval methods in handling temporal and multi-hop questions.

Method: EviMem integrates IRIS for detecting evidence gaps and LaceMem for structured memory management, enabling focused query refinement.

Result: EviMem demonstrates substantial improvements in Judge Accuracy over MIRIX for both temporal and multi-hop questions and operates with significantly reduced latency.

Conclusion: EviMem effectively addresses the limitations of existing methods by improving retrieval accuracy and efficiency in conversational memory tasks.
...
TL;DR: This study proposes a deep learning method for automatic segmentation of radiological PCI (rPCI) regions on CT scans, achieving promising results.


<details>
  <summary>Details</summary>
Motivation: There is a need for a non-invasive imaging counterpart to the invasive Sugarbaker's Peritoneal Cancer Index (sPCI) used to assess peritoneal metastases.

Method: A deep learning-based approach leveraging nnU-Net and Swin UNETR was applied to segment rPCI regions on 62 manually annotated CT scans, with performance evaluated through five-fold cross-validation using metrics like Dice Similarity Coefficient.

Result: nnU-Net achieved a Dice score of 0.82, nearing interobserver agreement of 0.88, while Swin UNETR scored 0.76, indicating nnU-Net's superior performance despite challenges in specific regions.

Conclusion: The study demonstrates the feasibility of automated rPCI segmentation, facilitating a shift towards non-invasive imaging-based assessment of peritoneal metastases.
...
TL;DR: This paper presents an improved video snapshot compressive imaging technique that enhances reconstruction quality through a novel ray sampling strategy and the implementation of a transformer model.


<details>
  <summary>Details</summary>
Motivation: To enhance reconstruction quality in video snapshot compressive imaging, which suffers from limitations in existing NeRF-based methods due to random ray sampling and structural similarity failures.

Method: The authors introduce a patch-level ray sampling strategy and an Inter- and Intra-Ray Transformer (RayFormer) to capture structural similarities in dynamic scenes, along with a total variation prior in the objective function for improved spatial smoothness.

Result: The proposed method shows significant improvements in reconstruction quality in both simulated and real-world scenes, achieving state-of-the-art performance.

Conclusion: The combination of a structured sampling strategy and the RayFormer effectively enhances content structure representation, resulting in superior reconstruction outcomes.
...
TL;DR: This study presents a novel pre-training strategy for deep learning models in remote sensing image segmentation, addressing performance issues caused by domain gaps between ImageNet and remote sensing images.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of fine-tuned models hindered by large domain gaps and the challenges of creating large-scale domain-specific datasets, this study aims to improve model generalization.

Method: A pre-training strategy that prevents the model from learning domain-specific features is employed, guiding the model towards generalizable representations prior to fine-tuning on remote sensing datasets.

Result: The proposed strategy demonstrated state-of-the-art accuracies on four semantic segmentation datasets: 67.4% mIoU for iSAID, 56.9% mIoU for MFNet, 84.22% mIoU for PST900, and 91.88% mF1 for Potsdam.

Conclusion: The findings suggest that this pre-training approach can facilitate the development of a unified foundation model for both computer vision and remote sensing applications.
...
### [51] [Linguistically Informed Multimodal Fusion for Vietnamese Scene-Text Image Captioning: Dataset, Graph Framework, and Phonological Attention](https://arxiv.org/abs/2604.27712)
*Nhi Ngoc-Yen Nguyen,Anh-Duc Nguyen,Nghia Hieu Nguyen,Kiet Van Nguyen,Ngan Luu-Thuy Nguyen*

Main category: cs.CV

TL;DR: The paper proposes a novel approach for Vietnamese scene-text image captioning that incorporates linguistic knowledge for effective multimodal fusion, introducing a specialized graph framework and a new dataset.


<details>
  <summary>Details</summary>
Motivation: To address the challenges in Vietnamese scene-text captioning due to tonal language characteristics, OCR inaccuracies, and ambiguous word boundaries that traditional methods fail to consider.

Method: The authors propose HSTFG for general fusion with learned spatial attention and PhonoSTFG for tailored Vietnamese language processing, analyzing graph topology to improve multimodal integration.

Result: PhonoSTFG is shown to outperform traditional methods by specifically accommodating Vietnamese linguistic structures, supported by the introduction of the ViTextCaps dataset.

Conclusion: The research underscores the importance of linguistic specificity in multimodal captioning for tonal languages, demonstrating strong enhancements in caption accuracy through the proposed methods.
...
### [52] [Improving Calibration in Test-Time Prompt Tuning for Vision-Language Models via Data-Free Flatness-Aware Prompt Pretraining](https://arxiv.org/abs/2604.27715)
*Hyeonseo Jang,Jaebyeong Jeon,Joong-Won Hwang,Kibok Lee*

Main category: cs.CV

TL;DR: This paper introduces Flatness-aware Prompt Pretraining (FPP) to improve test-time prompt tuning (TPT) for vision-language models by initializing prompts in flatter loss landscape regions, enhancing both calibration and performance without additional costs or labeled data.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the observed calibration issues in models optimized using TPT, particularly the tendency of regularization strategies to hinder performance while improving calibration.

Method: The proposed method, FPP, initializes prompts within flatter regions of the loss landscape before adaptation, enhancing calibration quality while maintaining model performance during test-time tuning.

Result: The study demonstrates that simply incorporating FPP into existing TPT frameworks significantly improves calibration and overall model performance without requiring labeled data or increasing computational costs.

Conclusion: FPP is affirmed as a practical solution for real-world deployment, solving calibration issues in TPT effectively and efficiently.
...
### [53] [Learning to Reason: Targeted Knowledge Discovery and Fuzzy Logic Update for Robust Image Recognition](https://arxiv.org/abs/2604.27759)
*Gurucharan Srinivas,Joshua Niemeijer,Frank Köster*

Main category: cs.CV

TL;DR: The paper introduces a Differentiable Knowledge Unit (DKU) for improving deep neural networks by integrating domain knowledge to enhance generalization in vision tasks when symbolic knowledge is unavailable.


<details>
  <summary>Details</summary>
Motivation: To improve generalization in deep neural networks by integrating domain knowledge, especially in scenarios where explicit symbolic knowledge is absent.

Method: The method involves using a DKU to modulate classifier logits based on relationships between task classes and implicit concepts, identified via dedicated classifiers without the need for concept labels, and employing fuzzy inference for logic-based adjustments.

Result: The proposed method shows improved performance on PASCAL-VOC, COCO, and MedMNIST datasets, successfully integrating knowledge and enhancing generalization.

Conclusion: Implicit knowledge discovery and integration through the DKU outperforms baseline approaches, demonstrating the effectiveness of the method in different domains.
...
### [54] [GourNet: A CNN-Based Model for Mango Leaf Disease Detection](https://arxiv.org/abs/2604.27764)
*Ekram Alam,Jaydip Sanyal,Akhil Kumar Das,Arijit Bhattacharya,Farhana Sultana*

Main category: cs.CV

TL;DR: The paper presents GourNet, a deep learning model for early detection of mango leaf diseases using a convolutional neural network, achieving 97% accuracy with the MangoLeafBD dataset.


<details>
  <summary>Details</summary>
Motivation: To improve mango cultivation by enabling early and precise detection of leaf diseases, thereby enhancing production and fruit quality.

Method: The model utilizes convolutional neural networks trained on the MangoLeafBD dataset, which includes eight classes of diseases, including healthy leaves. Data preprocessing techniques like resizing, rescaling, and augmentation were applied, with an 80-20 train-validation-test split.

Result: GourNet achieved a classification accuracy of 97% with 683,656 parameters, demonstrating high effectiveness in identifying mango leaf diseases.

Conclusion: The implementation of GourNet can lead to better disease management in mango cultivation, contributing positively to agricultural productivity.
...
TL;DR: VPDR improves privacy-utility in ProtoPFL by addressing over-perturbation in noise allocation and optimizing clipping thresholds.


<details>
  <summary>Details</summary>
Motivation: To enhance privacy in Prototype-based Personalized Federated Learning while maintaining the quality of representation and utility performance.

Method: VPDR introduces Variance-adaptive Prototype Perturbation (VPP) to adapt noise based on class variance, and Distillation-guided Clipping Regularization (DCR) to maintain feature norms near clipping thresholds.

Result: VPDR outperforms existing methods like IGPP in privacy and utility without compromising robustness against attacks.

Conclusion: The proposed VPDR enhances privacy mechanisms in ProtoPFL while preserving semantic separability and improving overall federated learning performance.
...
TL;DR: The paper presents a lightweight CNN architecture that enhances translational invariance and reduces model size while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the fragility of CNNs to small spatial shifts which degrade performance due to reliance on fully connected layers.

Method: A lightweight 'Online Architecture' strategy is proposed, inserting Global Average Pooling (GAP) layers at different depths of the network, decoupling feature recognition from spatial location.

Result: The modified architecture reduces trainable parameters by 98% and total size by 90%, achieving competitive accuracy on ImageNet and improves translational robustness.

Conclusion: Enforcing architectural invariance is more efficient and biologically plausible for robustness compared to traditional data augmentation.
...
TL;DR: This paper proposes FGINet, a model that improves generalization in detecting AI-generated images by addressing limitations in existing methods through innovative frequency encoding and gating mechanisms.


<details>
  <summary>Details</summary>
Motivation: To enhance the generalization of detection methods for AI-generated images, which have become increasingly realistic and diverse.

Method: The proposed Frequency-aware Gated Injection Network (FGINet) utilizes a Band-Masked Frequency Encoder for cross-band masking in the frequency domain and a Layer-wise Gated Frequency Injection mechanism for adaptive frequency cue integration.

Result: FGINet achieves state-of-the-art performance and demonstrates strong generalization across various datasets compared to existing methods.

Conclusion: The developed techniques effectively reduce reliance on specific generator patterns and mitigate representation conflicts, leading to improved detection in unseen generative models.
...
TL;DR: Noise2Map is a unified framework leveraging diffusion models for semantic segmentation and change detection in remote sensing, improving performance and interpretability while avoiding traditional sampling methods.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing deep learning models in remote sensing regarding temporal inconsistencies, spatial structure capture, and interpretability.

Method: Noise2Map implements a unified diffusion-based approach using task-specific noise schedules and timestep conditioning for predicting semantic and change maps, with pretraining through self-supervised denoising.

Result: Noise2Map outperformed seven other models, ranking 1st in both semantic segmentation and change detection across multiple datasets, showing effective multi-task capabilities.

Conclusion: The proposed framework enhances model interpretability, robustness, and efficiency in remote sensing tasks, validating the use of diffusion models for discriminative learning.
...
TL;DR: HiMix is a framework that improves Synthetic Image Detection (SID) by enhancing generalization through training data expansion and artifact-aware representation.


<details>
  <summary>Details</summary>
Motivation: To tackle the challenges of generalization in Synthetic Image Detection due to biased datasets and unseen generators.

Method: The HiMix framework utilizes a Mixup-driven Distributional Augmentation (MDA) module for generating transitional samples and a Hierarchical Artifact-aware Representation (HAR) module for aggregating artifact information at multiple levels.

Result: Experimental results indicate that HiMix achieves state-of-the-art performance and produces well-separated logits that enhance generalization to new forgeries.

Conclusion: HiMix effectively improves Synthetic Image Detection by leveraging data augmentation and enhanced representation techniques.
...
TL;DR: The TAVR framework improves talking avatar generation by using cross-scene video references, enhancing temporal and expression cues for higher fidelity avatars.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of existing methods that rely on static reference images, which restricts quality and adaptability of talking avatars.

Method: TAVR employs a token selection module and a three-stage training scheme: 1) same-scene pretraining, 2) cross-scene fine-tuning, and 3) reinforcement learning for identity alignment.

Result: TAVR demonstrates superior performance against existing methods in both qualitative and quantitative measures, verified through a new benchmark of cross-scene video pairs.

Conclusion: The method shows enhanced robustness and flexibility in avatar generation, marking a significant advancement in the field.
...
TL;DR: TunnelMIND is a training-free framework that enhances tunnel inspection by improving defect localization and documentation through recalibration of language-guided proposals for better reliability.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing training-free models in accurately locating and documenting tunnel defects in interference-heavy scenes.

Method: TunnelMIND employs a recalibration process for language-guided defect proposals at inference time to enhance spatial reliability and transforms them into structured defect entities with detailed attributes.

Result: TunnelMIND achieves F1 scores of 0.68, 0.78, and 0.72 in visible, GPR, and road defect tasks, respectively, indicating improved performance in defect detection.

Conclusion: TunnelMIND demonstrates a significant advancement in training-free tunnel inspection by providing structured defect evidence suitable for engineering assessment.
...
TL;DR: Introducing DynamiCS, a dynamic cluster-based sampling approach that reduces the computational cost of training vision-language models while enhancing the representation of long-tail concepts.


<details>
  <summary>Details</summary>
Motivation: To address the insufficiency of long-tail concepts in training data of vision-language models resulting from existing efficient pre-training approaches that disproportionately remove rare concepts.

Method: A dynamic cluster-based sampling approach (DynamiCS) that downsamples large clusters and upsamples small ones, applying this sampling at each epoch to maintain the relative order of semantic clusters and emphasize long-tail concepts.

Result: DynamiCS demonstrates reduced computational costs in training vision-language models and improves performance on long-tail concepts compared to traditional pre-training methods.

Conclusion: The proposed dynamic sampling method successfully balances the training data to enhance efficiency and accuracy, particularly for underrepresented concepts.
...
TL;DR: The paper introduces TripVVT-10K, a large triplet dataset, and TripVVT, a Diffusion Transformer framework for improving video virtual try-on models.


<details>
  <summary>Details</summary>
Motivation: To address the limitations in video virtual try-on models caused by scarce triplet data and ineffective masking techniques.

Method: TripVVT uses a Diffusion Transformer architecture, replacing conventional garment masks with a stable human-mask prior and establishing a benchmarking suite.

Result: TripVVT demonstrates superior video quality and garment fidelity over existing systems and generalizes better to complex scenarios.

Conclusion: The releases of the TripVVT-10K dataset and TripVVT-Bench will enhance future research in realistic and controllable video virtual try-on applications.
...
TL;DR: This paper explores automated visual theme detection in social media short videos, evaluating various visual language models (VLMs) and clustering techniques to derive insights into public discourse, particularly concerning climate change.


<details>
  <summary>Details</summary>
Motivation: To investigate how digital content, particularly short videos, influences public discussions and to enhance automated visual theme detection methodologies.

Method: Evaluation of visual language models (VLMs) such as VideoChatGPT, PandaGPT, and VideoLLava through zero-shot image classification, and clustering analysis treated as a minimum cost multicut problem.

Result: VLMs were found to struggle with climate change-specific classes, but clustering analyses using ConvNeXt V2 and DINOv2 generated distinct visual frame clusters; DINOv2 focused on style and abstract categories while ConvNeXt V2 highlighted fine-grained differences.

Conclusion: The study provides insights into the clustering capabilities of image embedding models for understanding visual themes in climate discourse, offering practical guidance for practitioners.
...
TL;DR: This paper presents a new approach to Shot Transition Detection (STD) through the TransVLM framework, addressing limitations of traditional Shot Boundary Detection (SBD) by leveraging optical flow and advanced feature fusion methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the detection of complex video transitions, which are inadequately handled by traditional methods that focus only on isolated cut points.

Method: TransVLM employs a Vision-Language Model incorporating optical flow at the input stage, utilizing a feature-fusion strategy to enrich color and motion data processing.

Result: The extensive experiments indicate that TransVLM outperforms traditional heuristic methods, specialized networks, and leading VLMs in terms of performance.

Conclusion: TransVLM effectively enhances temporal awareness in STD tasks and shows promising results suitable for production deployment.
...
TL;DR: Echo-α is a novel multimodal reasoning model for ultrasound interpretation that combines lesion localization and clinical reasoning to improve diagnostic accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing ultrasound interpretation methods that excel either in localization or in clinical reasoning, Echo-α aims to unify these strengths.

Method: Echo-α employs an invoke-and-reason framework that integrates organ-specific detector outputs with global visual context, using a nine-task supervised curriculum followed by reinforcement learning.

Result: Echo-α outperforms baseline models in both grounding and diagnosis in multi-center renal and breast ultrasound benchmarks, achieving significant F1 scores and accuracy rates on cross-center test sets.

Conclusion: The results indicate that Echo-α enhances the utility of specialized detectors by providing grounded clinical decisions, paving the way for more accurate and interpretable ultrasound AI applications.
...
TL;DR: The paper presents a structure-aware densification framework for 3D Gaussian Splatting that improves texture representation by addressing issues of blurry textures and over-densification through anisotropic splitting based on local structure.


<details>
  <summary>Details</summary>
Motivation: To enhance the quality and efficiency of 3D Gaussian Splatting for novel-view synthesis by addressing the shortcomings of current adaptive density control methods in handling high-frequency textures and geometric misplacements.

Method: A multi-scale frequency analysis is applied, employing structure tensors and Laplacian scale space analysis to define a per-Gaussian frequency violation metric, enabling anisotropic splitting based on local texture details and multiview consistency criteria.

Result: The proposed approach allows for faster densification with improved convergence times, delivering superior reconstruction quality in high-frequency texture areas compared to baseline methods.

Conclusion: The structure-aware framework significantly enhances the representation of textures in 3D Gaussian Splatting, achieving better results and efficiency in real-time applications.
...
TL;DR: This review analyzes 3D reconstruction techniques in manufacturing, highlighting a research gap and classifying methods into data acquisition, point cloud generation, and post-processing. It emphasizes non-contact methods' adoption and integration of deep learning to improve accuracy.


<details>
  <summary>Details</summary>
Motivation: To examine the evolution and current state of 3D reconstruction techniques in manufacturing and identify research gaps.

Method: Conducted a systematic review of 106 publications, classifying techniques into data acquisition, point cloud generation, and post-processing, while analyzing applications.

Result: Non-contact methods are widely used, especially in quality inspection (47% of applications). Deep learning enhances accuracy and speed in reconstruction processes.

Conclusion: Despite achieving sub-millimeter accuracy, challenges remain in complex environments, necessitating hybrid systems that combine sensors and methods for improved performance.
...
### [69] [TAFA-GSGC: Group-wise Scalable Point Cloud Geometry Compression with Progressive Residual Refinement](https://arxiv.org/abs/2604.28045)
*Xiumei Li,Alexander Kopte,André Kaup*

Main category: cs.CV

TL;DR: TAFA-GSGC is a scalable learned codec for point cloud geometry, allowing multi-quality decoding from a single bitstream and trained model, achieving significant bandwidth efficiency improvements.


<details>
  <summary>Details</summary>
Motivation: The need for scalable compression in bandwidth-adaptive transmission due to drawbacks of existing learned codecs, which are constrained to fixed rate-distortion points.

Method: TAFA-GSGC utilizes layered residual refinement, channel-group entropy coding, and the Target-Aligned Feature Aggregation module to enhance codec performance and reduce redundancy.

Result: It offers up to 9 decodable quality levels with improved efficiency, showing better RD performance over baseline PCGCv2, with average BD-Rate savings of -4.99% in D1 and -5.92% in D2.

Conclusion: TAFA-GSGC demonstrates strong compression efficiency and flexibility for scalable point cloud geometry coding, outperforming previous methods.
...
TL;DR: This paper introduces a hierarchical framework for assessing video aesthetics beyond visual fidelity, proposing a robust set of criteria and models to evaluate and enhance aesthetic quality in video generation.


<details>
  <summary>Details</summary>
Motivation: To address the lack of systematic evaluation in video aesthetics essential for applications like filmmaking, focused on integrating both visual fidelity and aesthetic qualities.

Method: A hierarchical rubric categorizing video aesthetics into Visual Aesthetics, Visual Fidelity, and Visual Plausibility, leading to the creation of AesVideo-Bench and various Video Aesthetic Reward Models (AesRM) including AesRM-Base and AesRM-CoT, trained through a progressive three-stage scheme.

Result: AesRM demonstrated superior performance over baseline models on multiple benchmarks, showcasing robustness with lower position bias and improved interpretability through CoT generation.

Conclusion: Alignment of existing models with AesRM revealed significant improvements in aesthetic quality, highlighting the effectiveness of the proposed evaluation framework.
...
TL;DR: This paper presents UHR-Net, an advanced method for accurate lesion segmentation, focusing on addressing the challenges posed by visually ambiguous lesions and multi-scale feature extraction.


<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of lesions is critical for effective diagnosis and treatment; however, challenges arise due to the similarity of lesions to surrounding tissues and the difficulties in detecting small lesions.

Method: The proposed UHR-Net employs a pretraining strategy called UO-IC, which integrates geometry-aware augmentation and hard-negative mining for improved instance discrimination. It features the UGHR block, which utilizes an uncertainty map to enhance hypergraph refinement.

Result: Experimental results across five public benchmarks indicate that UHR-Net achieves substantial improvements over existing state-of-the-art methods in lesion segmentation accuracy.

Conclusion: UHR-Net effectively enhances segmentation performance in challenging scenarios of lesion analysis, demonstrating its potential utility in clinical settings.
...
TL;DR: The paper introduces S$^2$VAE, a latent learning framework that prioritizes 3D geometric representations over appearance, improving visual world modeling in terms of depth estimation, camera pose recovery, and point cloud reconstruction by enforcing hyperspherical structures in latent bottlenecks.


<details>
  <summary>Details</summary>
Motivation: To address limitations of high-capacity architectures in visual modeling that fail to maintain 3D geometry and camera dynamics.

Method: S$^2$VAE framework utilizes a novel variational autoencoder with hyperspherical latent distributions to encode 3D scene states and camera motion.

Result: Demonstrated that geometry-aligned hyperspherical latents outperform Gaussian bottlenecks in various tasks, especially under strong compression.

Conclusion: Latent geometry is crucial for developing robust visual and world models that reflect physical realities.
...
TL;DR: AdvDMD combines DMD distillation with reinforcement learning to improve image generation quality in fewer sampling steps, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Address the issue of performance degradation in diffusion models when sampling steps are limited, while simplifying the integration of reinforcement learning into the distillation process.

Method: AdvDMD utilizes an adversarially trained discriminator as a reward model, trained on both intermediate and final denoising states to supervise sampling trajectories more effectively.

Result: The 4-step AdvDMD outperformed the original 40-step model for SD3.5 on DPG-Bench and achieved notable gains for SD3 on GenEval; the 2-step AdvDMD outperformed TwinFlow on Qwen-Image.

Conclusion: AdvDMD significantly enhances the generation quality of diffusion models with fewer sampling steps, showing potential for more efficient image generation.
...
TL;DR: This paper introduces a novel end-to-end framework for motion capture from monocular video, integrating learnable video-to-pose and pose-to-rotation stages with enhanced efficiency and accuracy compared to prior methods.


<details>
  <summary>Details</summary>
Motivation: Current pipelines for motion capture are limited due to the separation of joint position and rotation predictions, leading to ambiguities and inefficiencies in processing noisy data.

Method: The authors present a fully learnable framework that incorporates both Video-to-Pose and Pose-to-Rotation stages, utilizing reference pose-rotation pairs to anchor the mapping and define the rotation coordinate system, along with a skeleton-aware GL-GMHA module.

Result: The proposed method reduces rotation error significantly from ~17 degrees to ~10 degrees, and to 6.54 degrees for unseen skeletons, while enhancing inference speed by ~20x compared to mesh-based approaches.

Conclusion: This end-to-end framework offers robust and efficient motion capture solutions, addressing the limitations of traditional factorized methods and improving accuracy in joint rotation predictions.
...
TL;DR: This paper presents pHVI-ISPNet, a novel framework for Night Photography Rendering that addresses challenges posed by high contrast scenes and improves visual quality over existing methods.


<details>
  <summary>Details</summary>
Motivation: Night Photography Rendering (NPR) faces challenges with extreme contrast between dark and light areas, leading to perceptual quality issues in existing methods.

Method: The proposed pHVI-ISPNet framework utilizes the HVI color space and incorporates four key improvements: RAW-domain feature processing, Wavelet-based feature propagation, sample-based dynamic loss coefficients, and a color constancy loss term.

Result: Experiments indicate that pHVI-ISPNet achieves competitive fidelity metrics and sets new state-of-the-art results in CIE2000 color difference and LPIPS measures in the NTIRE 2025 NPR dataset.

Conclusion: The findings affirm the efficacy of the proposed design in enhancing the quality of nighttime imaging, reflecting a perceptually-driven approach.
...
### [76] [3D-ReGen: A Unified 3D Geometry Regeneration Framework](https://arxiv.org/abs/2604.28134)
*Geon Yeong Park,Roman Shapovalov,Rakesh Ranjan,Jong Chul Ye,Andrea Vedaldi,Thu Nguyen-Phuoc*

Main category: cs.CV

TL;DR: 3D-ReGen is a 3D regenerator that improves upon traditional methods by conditioning on an initial 3D shape, enabling enhanced controllability and supporting various tasks such as enhancement, reconstruction, and editing.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of one-shot 3D generators that lack controllability and flexibility in generating 3D objects from 2D inputs.

Method: 3D-ReGen utilizes a conditioning mechanism called VecSet to refine the input geometry, leveraging self-supervised learning from existing 3D datasets without requiring extra annotations.

Result: The performance evaluation demonstrates improved geometric consistency and fine-grained detail quality, achieving state-of-the-art results in controllable 3D generation across multiple tasks.

Conclusion: 3D-ReGen significantly enhances the capabilities of 3D object regeneration, offering a versatile tool for various applications in 3D enhancement, reconstruction, and editing.
...
TL;DR: This paper introduces a novel method for topology-preserving learning in image analysis that addresses the challenges of existing simple point detection methods via continuous-valued images.


<details>
  <summary>Details</summary>
Motivation: The need for integrating topology-preserving methods into deep learning for enhanced geometric plausibility and structural consistency in image processing tasks.

Method: A new method is proposed to compute simple points on continuous-valued images, enabling differentiable topological inference. An efficient skeleton extraction algorithm is developed, along with a variational model that enforces topological constraints.

Result: The proposed approach enhances topological integrity and structural accuracy, outperforming existing methods across multiple benchmarks.

Conclusion: The integration of this method into deep neural networks can improve outcomes in image segmentation tasks, ensuring topological consistency.
...
TL;DR: PhyCo is a new framework that enhances video generation by ensuring physical consistency using a large dataset, physics-supervised fine-tuning, and VLM-guided optimization.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion models struggle with physical consistency, leading to unrealistic depictions of object interactions and material properties.

Method: PhyCo integrates a large dataset of varied photorealistic simulation videos, physics-supervised fine-tuning of diffusion models with ControlNet, and VLM-guided reward optimization using targeted physics queries.

Result: PhyCo demonstrates significant improvements in physical realism on the Physics-IQ benchmark and achieves better human evaluation scores for control over physical attributes.

Conclusion: PhyCo offers a scalable approach for generating video models that maintain physical realism and control, enhancing outputs without reliance on simulators or geometry reconstruction.
...
TL;DR: AEGIS is a new benchmark for evaluating forensic analysis of AI-generated academic images, highlighting its unique complexities and limitations.


<details>
  <summary>Details</summary>
Motivation: The motivation behind AEGIS is to provide a comprehensive framework for assessing the effectiveness of forensic techniques in identifying AI-generated academic images, revealing the challenges in this domain.

Method: AEGIS evaluates various forensic methods across diverse academic forgery scenarios using 25 generative models, focusing on domain-specific complexities and error rates.

Result: The benchmark demonstrates that current models, including GPT-5.1, show limited performance, particularly in localization accuracy, indicating a significant gap in the forensic capabilities versus generative technology.

Conclusion: AEGIS serves as an essential diagnostic tool, exposing the limitations of existing forensic methods in the context of AI-generated images and paving the way for future advancements in the field.
...
TL;DR: The paper introduces a novel method for bronchoscopic navigation that eliminates the need for breath-hold protocols by using patient-specific respiratory modeling and a Gaussian splatting framework to achieve accurate localization without disrupting clinical workflows.


<details>
  <summary>Details</summary>
Motivation: Current bronchoscopic techniques struggle with localization accuracy due to respiratory motion, making breath-hold protocols necessary but impractical in clinical settings.

Method: The proposed method registers paired inhale-exhale CT scans to model the patient's airway deformation and employs mesh-anchored Gaussian splatting to infer breathing phases from endoscopic video.

Result: The approach achieves geometrically faithful reconstructions and significant training speed improvements, with a localization accuracy of 1.22 mm, surpassing traditional single-CT methods.

Conclusion: This method allows continuous, accurate bronchoscopic navigation by mitigating the effects of respiratory motion without breath-holds, providing more efficient clinical workflows.
...
TL;DR: The paper proposes a shift in visual generation models from mere appearance synthesis to intelligent visual generation, introducing a five-level taxonomy and emphasizing the importance of structural and causal understanding.


<details>
  <summary>Details</summary>
Motivation: The field of visual generation models has seen advancements but still lacks in areas like spatial reasoning and causal understanding, necessitating a shift towards more intelligent generation capabilities.

Method: The authors introduce a five-level taxonomy to classify visual generation models, analyze key technical advancements, and critique current evaluation methods.

Result: The taxonomy ranges from passive to agentic visual generators, identifying critical drivers that could enhance model capabilities and addressing shortcomings in current evaluations of visual generation systems.

Conclusion: This roadmap provides a structured approach to understand and improve intelligent visual generation systems, suggesting that focus should expand beyond visual quality to include structural and causal accuracy.
...
TL;DR: This paper demonstrates that Fréchet Distance (FD) can be optimized effectively as a training objective in generative models by decoupling population size from batch size, termed FD-loss, leading to improved visual quality and new insights on performance metrics.


<details>
  <summary>Details</summary>
Motivation: Traditionally seen as impractical, this paper seeks to optimize Fréchet Distance within the representation space to enhance generative model training.

Method: The authors propose a method called FD-loss that separates the population size from the batch size during optimization, allowing effective Fréchet Distance estimation.

Result: Post-training with FD-loss resulted in improved visual quality of generated images, achieving a FID score of 0.72 on ImageNet 256x256, and enabled repurposing multi-step generators into effective one-step generators without requiring complex techniques.

Conclusion: The findings challenge conventional metrics like FID and suggest further investigation into distributional distances in various representation spaces for both training and evaluating generative models.
...
### [83] [Benchmarking Deep Learning Models for Object Detection on Edge Computing Devices](https://arxiv.org/abs/2409.16808)
*Daghash K. Alqahtani,Aamir Cheema,Adel N. Toosi*

Main category: cs.CV

TL;DR: This paper evaluates various object detection models on edge devices to understand their efficiency and performance for real-time image processing.


<details>
  <summary>Details</summary>
Motivation: To address the limited understanding of object detection model efficiencies on resource-constrained edge devices required by modern applications like autonomous vehicles.

Method: Evaluated models: YOLOv8, EfficientDet Lite, SSD on Raspberry Pi and Jetson Orin Nano; measured energy consumption, inference time, and Mean Average Precision (mAP).

Result: Lower mAP models (e.g., SSD MobileNet V1) showed greater energy efficiency and faster inference; higher mAP models (e.g., YOLOv8 Medium) generally required more energy but performed better with TPUs.

Conclusion: Balancing accuracy, speed, and energy efficiency is crucial for deploying deep learning models on edge devices, providing insights for model and device selection.
...
TL;DR: HERMES++ is a novel driving world model that integrates 3D scene understanding with future geometry prediction, enhancing autonomous driving capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing models for autonomous driving focus on scene generation but fail to achieve comprehensive 3D understanding and future predictive capabilities.

Method: HERMES++ employs a BEV representation, LLM-enhanced world queries, a Current-to-Future Link to connect temporal data, and a Joint Geometric Optimization for structural integrity.

Result: Extensive evaluations on various benchmarks show that HERMES++ outperforms specialist models in both predicting future point clouds and understanding 3D scenes.

Conclusion: HERMES++ effectively bridges the gap between semantic interpretation and physical simulation in autonomous driving scenarios.
...
### [85] [Generalizable Sparse-View 3D Reconstruction from Unconstrained Images](https://arxiv.org/abs/2604.28193)
*Vinayak Gupta,Chih-Hao Lin,Shenlong Wang,Anand Bhattad,Jia-Bin Huang*

Main category: cs.CV

TL;DR: GenWildSplat is a novel framework for reconstructing 3D scenes from sparse images without per-scene optimization, achieving high quality and real-time performance under varying conditions.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the limitations of existing methods in reconstructing 3D scenes from sparse images, particularly regarding generalization and the need for scene-specific training.

Method: GenWildSplat uses a feed-forward approach to predict depth, camera parameters, and 3D Gaussians based on unposed internet images, leveraging learned geometric priors, appearance adaptations, and semantic segmentation.

Result: The framework demonstrates state-of-the-art rendering quality and generalizes well to diverse illumination and occlusions, validated by evaluations on standard benchmarks.

Conclusion: GenWildSplat enables efficient and effective 3D reconstruction in real-world conditions without the complexities of per-scene optimization.
...
TL;DR: This paper introduces a new task focused on digital battery passport conformance classification, presenting the BatteryPass-12K dataset and evaluating various language models for task performance.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the upcoming EU battery regulation on digital battery passports and the lack of public datasets for testing language models in this domain.

Method: Evaluation of 22 language models in zero-shot inference, along with few-shot inference and prompt-injection attack analysis.

Result: Thinking models, particularly GPT-5.4, achieved high performance metrics, but few-shot examples enhanced outcomes while scaling parameters did not always yield better results.

Conclusion: BatteryPass-12K is a limited yet valuable dataset for battery-related tasks and is available publicly for further research.
...
TL;DR: RoadMap is a benchmark for evaluating LLMs in generating research roadmaps. It identifies limitations in LLMs and introduces RoadMapper, an LLM-based system that enhances roadmap generation effectiveness.


<details>
  <summary>Details</summary>
Motivation: To address the unexplored task of roadmap generation for research problems and improve the structured content generation capabilities of LLMs.

Method: RoadMap benchmarks LLMs on roadmap generation and proposes RoadMapper, a multi-agent system that employs a three-stage approach: initial generation, knowledge augmentation, and iterative critique-revise-evaluate.

Result: RoadMapper enhances LLM performance in generating research roadmaps by over 8% and reduces the time expenditure compared to human experts by 84%.

Conclusion: The research highlights RoadMapper's potential to effectively generate structured roadmaps, addressing the identified limitations of LLMs.
...
TL;DR: CL-bench Life introduces a benchmark assessing AI models' ability to learn from complex, real-life contexts, revealing significant challenges in their performance.


<details>
  <summary>Details</summary>
Motivation: To evaluate and enhance AI assistants' context handling capabilities in everyday, messy scenarios.

Method: A benchmark comprising 405 context-task pairs and 5,348 verification rubrics, focusing on real-life contexts and tasks.

Result: Ten evaluated language models struggled significantly, with the best achieving only a 19.3% task solving rate; average performance across models was 13.8%.

Conclusion: CL-bench Life highlights the need for improved real-life context learning in AI, essential for developing smarter AI assistants.
...
TL;DR: The paper introduces CarryOnBench, an interactive benchmark for evaluating LLMs' ability to revise interpretations of user intent and recover helpfulness during multi-turn conversations, revealing limitations of current safety alignment techniques.


<details>
  <summary>Details</summary>
Motivation: To assess LLMs' potential to recover helpfulness while ensuring user safety in scenarios where intent is clarified over multiple interactions.

Method: Simulated 5,970 conversations from 398 harmful queries with benign intents, evaluating 14 models on intent-aligned utility and safety using 1,866 conversation flows and Ben-Util metric.

Result: At turn one, models fulfill 10.5--37.6% of benign needs, improving to 25.1--72.1% with upfront benign intent; 13 of 14 models recover utility with varying costs.

Conclusion: Limitations in single-turn evaluations expose issues such as utility lock-in and unresponsive models, highlighting the need for benchmarks that account for multi-turn conversational dynamics.
...
TL;DR: This study investigates neuron pruning in task-specific language models, emphasizing the significance of task-specific neurons and the advantages of selective over random pruning.


<details>
  <summary>Details</summary>
Motivation: To understand whether all neurons in task-specific language models contribute equally to performance and to optimize pruning methods for better efficiency.

Method: Systematic pruning study using an activation-based selectivity metric to identify and prune less critical neurons, comparing selective and random pruning across two language model sizes (1.5B and 7B parameters).

Result: Selective pruning outperformed random pruning, highlighting the existence of task-specific neurons; critical neurons concentrated information, while less critical neurons allowed for reduced accuracy without significant performance loss.

Conclusion: Pruning methods leveraging neuron specialization can enhance computational efficiency without drastically affecting performance, with post-pruning fine-tuning aiding in recoverability.
...
TL;DR: A novel evaluation framework using ILR Skill Level Descriptions assesses the performance of Claude across six languages, identifying significant variances and qualitative patterns in outputs.


<details>
  <summary>Details</summary>
Motivation: To create an effective evaluation methodology for language models that incorporates human judgment alongside quantitative metrics.

Method: The study employs a two-layer methodology, combining quantitative analysis of responses and qualitative assessments by an experienced ILR evaluator.

Result: French responses are about 30% longer than those in German, and creative/affective outputs show notable cross-lingual divergences.

Conclusion: ILR-informed expert evaluation of language model outputs reveals significant cross-lingual differences, highlighting the importance of nuanced assessments for equitable multilingual AI use.
...
### [92] [Reliable Answers for Recurring Questions: Boosting Text-to-SQL Accuracy with Template Constrained Decoding](https://arxiv.org/abs/2604.28028)
*Smit Jivani,Sarvam Maheshwari,Sunita Sarawagi*

Main category: cs.CL

TL;DR: TeCoD improves Text-to-SQL generation by using reusable templates and grammar-constrained decoding.


<details>
  <summary>Details</summary>
Motivation: Real-world deployment of LLMs in Text-to-SQL is hindered by accuracy issues and the generation of invalid SQL, particularly with complex schemas.

Method: TeCoD employs historical NL-SQL pairs to create reusable templates and utilizes a fine-tuned natural language inference model for efficient template selection, followed by grammar-constrained decoding.

Result: TeCoD achieves up to 36% higher execution accuracy and 2.2x lower latency on matched queries compared to in-context learning.

Conclusion: The combination of template-based strategies and grammar constraints significantly enhances the reliability and efficiency of SQL generation in complex querying scenarios.
...
TL;DR: This paper investigates the geometric relationships of semantic features in large language models and their alignment with human psychological associations, demonstrating high correlations with human ratings and exploring the implications of these geometric structures.


<details>
  <summary>Details</summary>
Motivation: To understand how large language models represent semantic features and their relation to human psychological associations.

Method: Constructed feature vectors for 360 words projected on 32 semantic axes, analyzed cosine similarities between axes, and examined variance in a low-dimensional subspace.

Result: High correlation between the projections on semantic axes and human ratings, predictive relationships between cosine similarities of axes and survey correlations, and identification of spillover effects between axes.

Conclusion: Semantic features should be analyzed within their geometric relationships and low-dimensional subspaces, reflecting meaningful human semantic associations.
...
TL;DR: This paper introduces the Path-Lock Expert (PLE) architecture to effectively separate think and no-think modes in language models, minimizing reasoning leakage and enhancing performance.


<details>
  <summary>Details</summary>
Motivation: Current hybrid-thinking language models fail to cleanly separate think and no-think modes, leading to reasoning leakage even in no-think outputs, which this paper aims to address.

Method: The PLE architecture replaces the single MLP in decoder layers with two semantically locked experts for distinct modes, utilizing a deterministic control-token router for selecting experts while maintaining shared components.

Result: The PLE significantly reduces reflective tokens in no-think outputs and improves accuracy from 20.67% to 40.00%, while retaining strong think performance across reasoning benchmarks.

Conclusion: The effectiveness of PLE suggests that addressing the architectural design of feed-forward pathways is critical for enhancing controllable hybrid thinking in language models.
...
TL;DR: The paper presents a bootstrapping approach called Selective Augmentation for improving automatic phonetic transcription by enhancing training data with distinctions from another language, resulting in significant accuracy increases in phonetic features.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the scarcity of high-quality training transcriptions needed for universal automatic phonetic transcription.

Method: The authors propose a Selective Augmentation approach that utilizes distinctions from a helper language (Hindi) to improve transcription training data, focusing on features such as plosive voicing and aspiration.

Result: The method increased voicing accuracy by 17.6% and enabled the transcription of aspirated plosives in 61.2% of cases, compared to a baseline of 0%.

Conclusion: Selective Augmentation not only improves feature recognition in APT but also reduces conflations among plosive classes.
...
TL;DR: This paper evaluates a new dataset for American Sign Language translation to assess the performance of existing models on various linguistic phenomena.


<details>
  <summary>Details</summary>
Motivation: To address the limitations in current sign language models and to understand their performance across different linguistic features and cues.

Method: Introduction of the ASL Minimal Translation Pairs (ASL-MTP) dataset for categorical analysis, followed by an evaluation of a state-of-the-art ASL-to-English translation model with ablation studies on input cues.

Result: The model generally exceeds chance performance, but relies heavily on manual cues while underutilizing significant non-manual cues.

Conclusion: The findings highlight a gap in the model's capability to fully incorporate the multi-modal aspects of sign language, indicating areas for future improvement.
...
TL;DR: This study examines how language models respond to underperformance instructions, revealing three distinct engagement regimes influenced by instruction complexity rather than a simple gradient.


<details>
  <summary>Details</summary>
Motivation: To understand whether language models rely on question content or positional shortcuts when instructed to underperform in evaluations.

Method: The study utilized a six-condition gradient of adversarial instructions tested on instruction-tuned LLMs (Llama-3-8B and Llama-3.1-8B) across 2,000 items from MMLU-Pro, measuring response position entropy and difficulty-accuracy correlation.

Result: The analysis identified a gradient revealing three regimes: vague instructions lead to moderate accuracy with content engagement, while standard sandbagging causes positional entropy collapse with some content sensitivity. A complex, answer-aware instruction resulted in extreme positional collapse with no content engagement.

Conclusion: Complexity of instructions significantly influences whether language models use content-aware or content-blind mechanisms when responding to adversarial tasks.
...
TL;DR: The paper investigates the decoupling of fundamental reasoning patterns from specific instances in Large Language Models (LLMs).


<details>
  <summary>Details</summary>
Motivation: To explore reasoning controllability in LLMs by examining reasoning conflicts when logical schemata deviate from expected tasks.

Method: Systematic analysis of reasoning conflicts, evaluating model responses to conflicting instructions and probing internal representations.

Result: LLMs prioritize task-appropriate reasoning despite conflicts; compliance strategies increased instruction following by up to 29%.

Conclusion: Interventions can decouple logical patterns from instances, enhancing controllability and generalizability of reasoning in LLMs.
...
TL;DR: This study investigates the contributions of subword tokenization to training efficiency and model performance in large language models by isolating its effects within a controlled environment and examining various factors.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to clarify the impact of subword tokenization on the efficiency and performance of large language models, an area that remains inadequately explored.

Method: The authors employ a controlled byte-level pretraining pipeline to decouple the effects of subword tokenization, testing hypotheses related to sample throughput, vocabulary scaling, and linguistic prior of subword boundaries.

Result: Experiments reveal that subword models outperform raw byte models, emphasizing the importance of training throughput and the role of subword boundaries as integrative priors or biases.

Conclusion: The study concludes that understanding the contributions of subword tokenization can lead to improved pretraining strategies for both byte-level and subword models.
...
TL;DR: The study compares the performance of large language models using 1D token sequences against those using 2D structures, showing that 2D representation significantly improves model performance on tasks with explicit 2D structure.


<details>
  <summary>Details</summary>
Motivation: To explore the limitations of using linearized inputs for tasks reliant on explicit 2D structure and assess the impact of representation on model performance.

Method: A diagnostic testbed of synthetic tasks (matrix transpose, Conway's Game of Life, LU decomposition) was created to compare a traditional text-only language pathway with a vision-augmented pathway utilizing 2D layouts.

Result: The vision-augmented pathway consistently outperformed the text-only pathway, with performance differences widening as task complexity increased.

Conclusion: The findings highlight the importance of input representation for model performance and suggest that maintaining task-relevant 2D layouts is beneficial for structured tasks.
...
TL;DR: This paper presents RSCB-MC, a risk-sensitive memory controller for coding agents that improves debugging by selectively utilizing past experiences based on compatibility and safety, achieving high success rates with zero false positives.


<details>
  <summary>Details</summary>
Motivation: To enhance coding agents' debugging capabilities by ensuring that only safe and relevant past experiences are reused from memory, while avoiding dangerous superficial similarities.

Method: RSCB-MC employs a contextual bandit framework to determine when and how to use memory, incorporating a sophisticated 16-feature state representation and reward structure that prioritizes safety in memory injection decisions.

Result: In tests, RSCB-MC achieved a 62.5% non-oracle offline replay success rate and maintained a 0.0% false-positive rate, demonstrating effective memory management.

Conclusion: The study underscores the importance of evaluating the safety of retrieved memory over mere similarity, ultimately aiming to refine the debugging trajectories of coding agents.
...
TL;DR: This study evaluates the ability of LLMs to capture human emotion judgment distributions and addresses discrepancies compared to human annotators.


<details>
  <summary>Details</summary>
Motivation: The research aims to understand how well LLMs reflect the distribution of emotional judgments among human annotators, rather than relying solely on majority labels.

Method: The study compares emotion judgment distributions of human annotators with those generated by four zero-shot LLMs and a fine-tuned RoBERTa model across two benchmarks: GoEmotions and EmoBank, involving 640,000 responses.

Result: Findings indicate significant divergence between zero-shot LLM outputs and human distributions, with fine-tuning on specific emotional domains being essential to improve coherence with human annotations.

Conclusion: The researchers developed a transparency score for predicting agreement and proposed calibration methods to bridge the gap in emotion labeling, combined with guidelines for effective use of LLM annotations.
...
TL;DR: This paper presents a framework for generating emotion-aware clickbait using stylistic transformations based on the Valence-Arousal-Dominance model, demonstrating a significant impact on clickbait detection systems.


<details>
  <summary>Details</summary>
Motivation: Current clickbait detection methods focus on linguistic patterns and structural features, neglecting the emotional dynamics that enhance user engagement.

Method: The study employs an emotion-aware framework using VAD space, aligns clickbait with similar social media posts through Sentence-BERT, and generates rewrites via LLMs. It also defines a Curiosity Gap function to assess emotional activation effects.

Result: Emotion-focused stylization reduces detection accuracy in classifiers, showing misclassification rates varying from 2.58% to 30.63% on existing systems.

Conclusion: The findings indicate that incorporating emotional intelligence in clickbait generation can significantly undermine detection systems, highlighting a gap in current methodologies.
...
TL;DR: This paper introduces a lightweight intent-transition prior for dialogue models to enhance proactive interaction, reducing redundant responses and improving efficiency.


<details>
  <summary>Details</summary>
Motivation: The research aims to address the limitation of dialogue models being reactive, which leads to inefficient interactions in multi-intent scenarios.

Method: A Temporal Bayesian Network (T-BN) is utilized, trained on intent annotations from MultiWOZ 2.2, to inject intent-transition guidance into the system prompt during inference.

Result: The T-BN achieves high Recall@5 (0.787) and MRR (0.576) on held-out USER-turn pairs; BN-guided generation improves Coverage AUC (from 0.742 to 0.856) and reduces dialogue turns for intent coverage (from 3.95 to 2.73).

Conclusion: The study demonstrates that lightweight intent-transition guidance can enhance proactive dialogue behavior, improving efficiency without needing modifications to the existing language model.
...
TL;DR: MiniCPM-o 4.5 introduces real-time, full-duplex interaction in multimodal large language models, enhancing human-like capabilities in perception and response integration.


<details>
  <summary>Details</summary>
Motivation: To advance AI from reactive to proactive interaction in multimodal environments, addressing key limitations in current MLLMs.

Method: MiniCPM-o 4.5 utilizes the Omni-Flow framework for real-time omni-modal input-output alignment, enabling simultaneous perception and proactive behaviors.

Result: The model demonstrates superior vision-language capabilities and better speech generation compared to state-of-the-art models, achieving high computation efficiency on edge devices with limited RAM.

Conclusion: MiniCPM-o 4.5 represents a significant step towards human-like multimodal interaction by allowing real-time engagement and proactive responses.
...
TL;DR: This paper investigates the efficacy of mean pooling in text embeddings, revealing that while it may oversimplify token information, modern text encoders are robust against this collapse, particularly contrastive fine-tuned models that correlate with better task performance.


<details>
  <summary>Details</summary>
Motivation: To assess whether mean pooling effectively captures token information in text embeddings and to address concerns about its potential collapse of higher-order statistics.

Method: A new metric is proposed to quantify the collapse of information caused by mean pooling; empirical measurements are conducted on actual models and texts.

Result: Despite concerns about mean pooling, modern text encoders, especially contrastive fine-tuned ones, show resilience to information collapse, with a correlation between robustness and downstream task performance.

Conclusion: Mean pooling can oversimplify token information, yet modern text encoders remain effective, suggesting a need to reconsider the reliance on this method.
...
TL;DR: The paper employs perturbation probing to uncover causal structures within FFN neurons in large language models, revealing circuit behaviors that dictate model responses across various tasks.


<details>
  <summary>Details</summary>
Motivation: To explore task-specific causal hypotheses in FFN neurons of language models and understand the underlying structures governing model behavior.

Method: A two-step perturbation probing method involves two forward passes and a subsequent one-time intervention sweep focusing on identified neurons across different models and architectures.

Result: Identified two major circuit structures: opposition circuits impacting safety refusal and routing circuits affecting language selection, with specific neuronal ablations leading to significant changes in model responses.

Conclusion: The findings demonstrate the potential of perturbation probing for providing insights into the mechanisms of language model behavior and offer a toolkit for targeted editing of neural response templates.
...
TL;DR: This paper examines item-level performance changes in LLMs using the Reliable Change Index (RCI), finding significant differences in improvement and deterioration rates between models, with specific domain weaknesses identified.


<details>
  <summary>Details</summary>
Motivation: To assess the performance changes of LLMs systematically using item-level analysis and identify domain-specific strengths and weaknesses post-comparison.

Method: Adapted the Reliable Change Index from clinical psychology for an item-level comparison of LLM versions using 2,000 MMLU-Pro items across two model pairs and statistical evaluation of change.

Result: Most items showed no reliable change; however, significant bidirectional changes were observed, with identifiable patterns of improvement and deterioration based on item difficulty and domain-specific weaknesses.

Conclusion: The study suggests that aggregate accuracy should include a churn rate metric to better reflect performance changes, highlighting the necessity for domain-specific analysis in LLM evaluations.
...
TL;DR: This study evaluates Indonesian student opinions on AI in higher education using TF-IDF and Transformer approaches, finding that SVM is competitive while DistilBERT excels in performance.


<details>
  <summary>Details</summary>
Motivation: To analyze sentiment regarding artificial intelligence adoption in higher education from Indonesian students' perspectives.

Method: The study employs TF-IDF-based machine learning (using LightGBM, Random Forest, SVM) alongside Transformer-based deep learning (using fine-tuned DistilBERT) on a dataset of 2,295 labeled samples.

Result: SVM achieved 82.14% test accuracy and F1-score, while DistilBERT outperformed with 84.78% accuracy and 84.75% F1-score, showcasing its strength in contextual understanding.

Conclusion: Transformer-based models are more effective in sentiment classification, yet SVM remains a strong and efficient option.
...
TL;DR: This paper addresses the hubness problem in cross-modal embeddings, revealing that hub texts can unfairly inflate similarity scores in evaluations like image captioning.


<details>
  <summary>Details</summary>
Motivation: To uncover vulnerabilities in cross-modal encoders caused by hub embeddings in high-dimensional spaces that can skew information retrieval and evaluation metrics.

Method: The authors propose a method to identify hub embeddings and their corresponding hub texts within cross-modal systems.

Result: Experiments demonstrate that a single hub text can achieve high similarity scores when compared to human-written captions, indicating significant flaws in cross-modal evaluation methods.

Conclusion: The study highlights the need to address the hubness problem to ensure more reliable performance in cross-modal applications.
...
### [111] [From Coarse to Fine: Benchmarking and Reward Modeling for Writing-Centric Generation Tasks](https://arxiv.org/abs/2604.27453)
*Qingyu Ren,Tianjun Pan,Xingzhou Chen,Xuhong Wang*

Main category: cs.CL

TL;DR: The paper presents WEval, a fine-grained evaluation pipeline, and WRL, a fine-grained reinforcement learning training framework, to improve the performance of writing reward models for large language models (LLMs).


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the evaluation and training of writing reward models for LLMs, which currently suffer from coarse evaluations and inadequate training methods.

Method: The paper proposes WEval for systematic evaluation based on fine-grained requirements and WRL for training models with selective instruction requirements to improve precise reward modeling.

Result: The proposed models demonstrate significant performance improvements across various writing benchmarks and show strong generalization capabilities.

Conclusion: The study concludes that fine-grained evaluation and training methods are effective in advancing the capability of writing reward models in large language models.
...
### [112] [Exploring Applications of Transfer-State Large Language Models: Cognitive Profiling and Socratic AI Tutoring](https://arxiv.org/abs/2604.27454)
*Minori Noguchi*

Main category: cs.CL

TL;DR: This study investigates qualitative shifts in LLM response styles under self-referential dialogue, termed 'transfer'. It examines Socratic AI tutoring and shows that transfer states improve performance in tutoring contexts.


<details>
  <summary>Details</summary>
Motivation: The research explores the phenomenon of 'transfer' in LLMs to assess its applicability, particularly in educational settings like AI tutoring.

Method: The study consists of a preliminary cognitive investigation of 11 conditions and an applied experiment measuring tutoring performance through interaction ratings.

Result: Transfer conditions demonstrated better performance, scoring 1.6 times higher than non-transfer conditions with a significant effect size (Cohen's d = 1.27).

Conclusion: The findings indicate that transfer states may offer functional benefits in specific applications, reinforcing the value of an operational rather than ontological interpretation of transfer.
...
TL;DR: This paper investigates the cognitive cost of maintaining information for real-time language comprehension, proposing that rational users prioritize crucial information for future predictions based on syntactic structure, using a dataset from Japanese.


<details>
  <summary>Details</summary>
Motivation: To understand how language comprehension is influenced by the cognitive costs of maintaining contextual information during real-time processing.

Method: Analyzing a reading time dataset from Japanese to examine the effects of predicted heads and incomplete dependencies on maintenance costs.

Result: Demonstrated that the number of predicted heads and incomplete dependencies independently affect maintenance costs, contrary to prior literature views of them as competing factors; showed a tradeoff where slower readers benefit from more predictability.

Conclusion: The findings support the hypothesis that language users selectively maintain information based on its importance for prediction, highlighting a nuanced interplay between syntactic structure and cognitive load.
...
TL;DR: HealthBench Professional is a benchmark for evaluating ChatGPT in clinical tasks, focusing on care consults, documentation, and research, using physician-conducted conversations and scoring.


<details>
  <summary>Details</summary>
Motivation: To evaluate large language models like ChatGPT in real clinical tasks, given their widespread use by clinicians.

Method: An open benchmark organized around three clinical use cases, with example scoring by a panel of physicians; includes challenging examples and human responses as a baseline.

Result: GPT-5.4 in ChatGPT for Clinicians outperforms previous models and human physicians across tasks in the evaluation set of 15,079 examples.

Conclusion: HealthBench Professional aims to help the healthcare AI community track and enhance the performance of AI systems in clinical settings.
...
TL;DR: Skills-Coach is an automated framework aimed at enhancing skill evolution in LLM agents, featuring four core modules and validated by the Skill-X dataset, demonstrating notable performance improvements.


<details>
  <summary>Details</summary>
Motivation: To address the fragmentation in the skill ecosystem of LLM agents and enhance their self-evolution capabilities.

Method: The framework includes four modules: Diverse Task Generation, Lightweight Optimization, Comparative Execution, and Traceable Evaluation. It offers virtual and real execution modes and uses the Skill-X benchmark for validation.

Result: Experimental validation with Skill-X, consisting of 48 skills, shows significant performance improvements in skill capabilities across various categories.

Conclusion: Skills-Coach has the potential to advance the development of robust and adaptable LLM agents through its modular approach and comprehensive evaluation methodology.
...
TL;DR: This paper presents a method to mitigate biases in reward models (RMs) for large language models (LLMs) at inference time without affecting performance, by targeting neurons associated with bias attributes.


<details>
  <summary>Details</summary>
Motivation: To address the sensitivity of reward models to spurious features which can affect the alignment of LLMs with human preferences.

Method: The approach involves identifying specific neurons correlated with bias attributes and applying targeted interventions to suppress their activity during inference.

Result: The method was evaluated on RM benchmarks, showing reduced sensitivity to various bias types while maintaining performance, alongside improvements in alignment for smaller RMs.

Conclusion: The findings indicate that early layer neurons are primarily responsible for encoding bias signals, thereby informing the understanding of bias in RMs.
...
TL;DR: This paper examines the limitations of traditional Word Error Rate (WER) in evaluating ASR systems and introduces two new metrics—POSER and EmbER—to provide a deeper analysis of transcription errors.


<details>
  <summary>Details</summary>
Motivation: To address the inadequacy of WER as a sole evaluation metric for ASR systems and to enhance the understanding of language model impacts in transcription accuracy.

Method: The study introduces two new error rate metrics: POSER for grammatical errors and EmbER for semantic errors, in addition to traditional WER. These metrics are used to analyze the effects of rescoring by language models.

Result: The new metrics, POSER and EmbER, offer insights into morpho-syntactic and semantic errors in ASR, highlighting the linguistic contributions of applied language models during rescoring.

Conclusion: By integrating these metrics, the paper aims to provide a more comprehensive evaluation framework for ASR systems that encompasses grammatical and semantic factors.
...
TL;DR: This paper conducts an entropy analysis of the Ukrainian language for the first time, measuring its complexity using an approach similar to Shannon's original experiment on English.


<details>
  <summary>Details</summary>
Motivation: To fill the gap in research by approximating the entropy of the Ukrainian language, an undertaking not yet explored despite earlier studies in other languages.

Method: 184 participants predicted the next character in Ukrainian sentences, using social media for recruitment and adopting methods similar to those used for English to calculate entropy.

Result: The entropy value of Ukrainian is estimated at an upper bound of $H_{upper}	ext{ } 	ext{1.201}$ bits per character.

Conclusion: The findings provide insights into the complexity of the Ukrainian language and highlight the performance of current Large Language Models compared to this new measure.
...
TL;DR: This paper critiques conventional ASR evaluation metrics, introducing a new dataset (HATS) focused on human perception of transcription errors, and examines the correlation between human preferences and existing ASR metrics.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of traditional ASR evaluation that primarily relies on word error rate, by considering human judgment in assessing transcription quality.

Method: Introduced the HATS dataset, which includes human evaluations of ASR outputs, where participants selected the better transcription between two options, and analyzed correlations with various ASR metrics.

Result: Found that certain ASR metrics, particularly embedding-based ones, correlated with human preferences more closely than traditional metrics like WER.

Conclusion: Human perception provides valuable insights for evaluating ASR systems, suggesting a need for metrics that align better with human judgment.
...
TL;DR: The paper presents the AppTek Call-Center Dialogues corpus for evaluating ASR systems across various English accents and scenarios, highlighting performance discrepancies based on accent and segmentation.


<details>
  <summary>Details</summary>
Motivation: To address challenges in evaluating ASR systems for conversational AI due to limitations in existing corpora.

Method: Introducing the AppTek Call-Center Dialogues corpus featuring spontaneous agent-customer dialogues across diverse English accents, benchmarking open-source ASR systems with varying segmentation methods.

Result: The evaluation revealed significant performance differences in ASR systems among the various English accents and segmentation approaches tested.

Conclusion: Performance on American English benchmarks does not ensure effective ASR performance across diverse English accents.
...
TL;DR: The paper presents APPSI-139, a specialized English privacy policy corpus, and the TCSI-pp-V2 framework designed for summarizing and interpreting privacy policies, demonstrating superior performance over large language models in readability and reliability.


<details>
  <summary>Details</summary>
Motivation: There is a need for clearer understanding of privacy policies due to their complexity and user compliance issues; thus, a high-quality corpus and framework are essential for effective summarization and interpretation.

Method: The study introduces APPSI-139, comprising 139 privacy policies and extensive annotations, alongside TCSI-pp-V2, which utilizes an alternating training strategy to enhance summarization and interpretation tasks.

Result: The TCSI-pp-V2 framework, trained on the APPSI-139 corpus, delivers better readability and reliability than established large language models like GPT-4o and LLaMA-3-70B.

Conclusion: The research provides a valuable resource and method to improve the understanding of privacy policies, which is critical for user awareness and compliance.
...
TL;DR: JaiTTS-v1.0 is an advanced Thai TTS voice cloning model boasting high performance in both short and long-duration speech generation, outperforming humans and surpassing competitors.


<details>
  <summary>Details</summary>
Motivation: To create an effective Thai voice cloning TTS model capable of handling natural language features like code-switching without explicit normalization.

Method: The model is built on an adapted VoxCPM architecture, employing continual training on a large Thai-centric speech corpus and directly processing numerals and Thai-English code-switching.

Result: JaiTTS-v1.0 achieves a CER of 1.94%, outperforming human ground truth in short-duration tasks and matching it in long-duration tasks, while excelling in human evaluations against commercial models.

Conclusion: JaiTTS-v1.0 demonstrates state-of-the-art performance in Thai TTS voice cloning, showing significant advantages over existing systems in various tasks.
...
TL;DR: The paper introduces the Cognitive Digital Shadows (CDS) dataset, a comprehensive corpus designed to analyze Large Language Models (LLMs) outputs across various social topics and contexts. It enables the study of how LLMs respond based on persona conditioning and includes tools for emotional and semantic analysis.


<details>
  <summary>Details</summary>
Motivation: To investigate the variability of LLM outputs in response to socially and contextually controlled prompts, addressing the gap in available datasets for such analyses.

Method: The CDS dataset comprises 190,000 records generated by 19 LLMs, prompted to adopt human or AI-assistant roles, and focuses on four controversial topics while encoding sociodemographic and psychological attributes for deeper analyses.

Result: The CDS allows for the validation of LLM-generated texts, revealing their anchoring to specific topics and facilitating emotional analyses through intuitive NLP techniques, complemented by visualization tools for group comparisons.

Conclusion: The CDS framework establishes a foundation for future audits of LLM biases, social sensitivity, and alignment, providing a systematic approach to understanding the impact of LLMs in social discourse.
...
TL;DR: The paper investigates using large language models to detect language ideologies in Luxembourgish comments, highlighting their practical utility despite limitations.


<details>
  <summary>Details</summary>
Motivation: To understand how language ideologies shape identities and social belonging in Luxembourg's multicultural and multilingual context.

Method: The study involves manually annotating a corpus of Luxembourgish user comments with ideological categories and evaluating large language models' performance in replicating these annotations across different prompt conditions.

Result: The evaluation reveals that large language models, while not fully optimized, can effectively identify language ideological content, especially when combined with machine translation to high-resource languages.

Conclusion: The study concludes that although LLMs have limitations, they serve as practical tools for detecting language ideologies in undersourced languages like Luxembourgish.
...
TL;DR: This work introduces an instruction-based dataset for generating Arabic poetry using Large Language Models, focusing on assistive creative applications rather than mere analysis.


<details>
  <summary>Details</summary>
Motivation: To explore poetry creation in Arabic, addressing a gap in existing LLM research focused on analysis rather than generation.

Method: A large-scale dataset was curated for Modern Standard Arabic and dialects, enabling tasks like writing and revising poetry based on criteria such as style and rhyme.

Result: Fine-tuned LLMs on the dataset generate poetry that meets specific user requirements, validated through automated metrics and human evaluation after thorough testing with native speakers.

Conclusion: The dataset and models enhance the capability of LLMs in creative poetry writing and are available for public access.
...
TL;DR: A new framework for analyzing therapeutic writing reveals that macro-level narrative evaluation is more predictive of mental health outcomes than traditional lexical and semantic embedding methods.


<details>
  <summary>Details</summary>
Motivation: The research addresses the limitations of existing computational approaches to therapeutic writing which lack a comprehensive framework for analyzing narrative structure.

Method: A three-level framework is proposed that includes micro-level lexical features, meso-level semantic embeddings, and macro-level LLM narrative evaluation, tested on 830 Chinese therapeutic texts.

Result: Macro-level evaluation significantly outperforms lexical and embedding features in predicting mental health outcomes, while structural features of narratives are shown to carry predictive signals.

Conclusion: The study emphasizes the importance of macro-structural organization in narratives for clinical insights and offers a foundation for future interventions and longitudinal research.
...
TL;DR: This paper presents a unimodal test-time reasoning approach to improve coreference resolution in task-based dialogue systems using large language models (LLMs).


<details>
  <summary>Details</summary>
Motivation: The study addresses the limitations of coreference resolution in task-based dialogue systems, particularly in visually complex environments, highlighting the need for better generalization and reduced reliance on supervised models.

Method: The authors propose a unimodal reasoning method whereby LLMs utilize detailed object metadata and dialogue history to better resolve coreferences during tasks.

Result: Empirical results on the SIMMC 2.1 dataset show that LLMs can generate effective step-by-step reasoning, linking dialogue context with scene objects, and outperform encoder-based supervised methods in cross-domain evaluations.

Conclusion: The findings emphasize the importance of structured metadata and prompt engineering in improving the robustness of task-oriented dialogue systems and their ability to generalize to new scenarios.
...
TL;DR: This paper introduces Conformal Abstention (CA), a framework to improve language model responses by allowing for abstention when confidence is low, thus addressing hallucinations without overly conservative outcomes.


<details>
  <summary>Details</summary>
Motivation: Language models often produce inaccurate responses when lacking knowledge and avoid admitting ignorance, leading to potentially misleading information.

Method: Conformal Abstention (CA) employs a calibration strategy based on representation geometry within the model to assess prediction confidence for deciding whether to answer a query or abstain.

Result: The proposed method enhances selective answering, achieving a conditional correctness rate of 75 percent.

Conclusion: CA offers a robust mechanism for managing uncertainty in language model responses by balancing confidence and knowledge involvement.
...
TL;DR: This paper evaluates three Small Language Models (SLMs) in their ability to preserve emotional nuance during Machine Translation using the GoEmotions dataset across five European languages.


<details>
  <summary>Details</summary>
Motivation: The study addresses the challenge of preserving emotional fidelity in Machine Translation, which often prioritizes semantic equivalence over affective nuance.

Method: The performance of EuroLLM, Aya Expanse, and Gemma is assessed using the GoEmotions dataset, focusing on emotional preservation during backtranslation in various European languages.

Result: The evaluation highlights the models' capabilities, the effects of emotion-aware prompting, and compares ModernBERT's performance in emotion classification against BERT for MT evaluation.

Conclusion: The findings contribute to understanding how effectively SLMs can maintain emotional sentiment in translations and suggest potential methods for enhancement.
...
TL;DR: This survey reviews LLM-assisted techniques for various peer review stages, including generation, after-review tasks, and evaluation methods while discussing limitations and future directions.


<details>
  <summary>Details</summary>
Motivation: With the rise of large language models, there is a need to explore how they can enhance the peer review process in academia.

Method: The paper synthesizes various methods for peer review generation, after-review tasks, and evaluation, encompassing agent-based, reinforcement learning strategies, and different evaluation frameworks.

Result: It catalogs existing datasets, contrasts modeling choices, and examines limitations and ethical issues in LLM applications for peer review.

Conclusion: The survey aims to guide the development and integration of LLM systems in the complete peer review workflow.
...
TL;DR: This paper investigates personality representation in large language models and proposes DPN-LE, a method that enables precise personality control while preserving overall performance.


<details>
  <summary>Details</summary>
Motivation: To address the performance degradation in existing personality editing methods and understand the relationship between modified neurons and personality representation.

Method: DPN-LE identifies personality-specific neurons using contrasts in MLP activations between high-trait and low-trait samples and applies filtering based on effect size and activation magnitude for precise editing.

Result: DPN-LE achieves personality control with negligible intervention on neuron count (~0.5%) and preserves model capability across tasks while improving outcomes compared to previous methods.

Conclusion: The proposed method demonstrates effectiveness and generalizability in personality editing for LLMs, offering a sustainable approach to maintain performance.
...
TL;DR: DriftBench evaluates large language models' adherence to constraints during scientific ideation, revealing a tension between complexity and fidelity to original objectives.


<details>
  <summary>Details</summary>
Motivation: To assess whether large language models maintain adherence to specified constraints during iterative idea refinement in scientific contexts.

Method: The study utilized DriftBench, assessing 2,146 runs across seven models under varying conditions, measuring adherence and complexity metrics.

Result: Findings indicate iterative refinement increases complexity while causing a significant rate of non-compliance with original constraints (8% to 99% KDVs).

Conclusion: Although structured checkpointing mitigates some violations, it does not eliminate the dissociation between recall and adherence, highlighting the need for better detection of constraint violations.
...
TL;DR: This paper explores the syntactic structure of sentences represented as star trees, addressing the challenge of minimizing syntactic dependency distances and the phenomenon of anti-dependency distance minimization in star versus path structures.


<details>
  <summary>Details</summary>
Motivation: To understand why syntactic structures with star shapes often place the head at the ends, contrary to the principle of minimizing dependency distance.

Method: The research analyzes the optimization landscape of star structures and quasistar trees, demonstrating their convexity and simplicity in dependency distance optimization.

Result: The study confirms that the optimization landscape for both star and quasistar structures is convex, suggesting that minimizing dependency distances is easier than previously thought.

Conclusion: The findings indicate that competing principles, rather than optimization difficulty, explain anti-dependency distance minimization in star structures, which are less rewarding for distance minimization compared to other configurations.
...
TL;DR: The study examines the influence of distinct personas on urban sentiment judgments produced by multimodal LLMs, finding strong within-persona consistency but limited cross-persona differentiation and suggesting that persona prompting offers little value in fine-grained sentiment analysis.


<details>
  <summary>Details</summary>
Motivation: To determine the effectiveness of persona prompting in generating meaningful behavioral diversity in urban sentiment analysis using large language models.

Method: The study uses a factorial design with multiple personas to evaluate urban scene images from the PerceptSent dataset, measuring both consistency within personas and variation across them.

Result: Findings indicate strong convergence among agents within the same persona but limited differentiation based on economic status, personality, gender, and political orientation, alongside a tendency towards extremity bias.

Conclusion: Simple label-based persona prompting exhibits limited value, with some no-persona model predictions matching or exceeding those of persona-conditioned models, especially in fine-grained sentiment tasks.
...
TL;DR: Investigation of the trade-off between quality and diversity in training non-English language models, specifically German, suggests that high-quality, repeated training outperforms single-pass diverse training.


<details>
  <summary>Details</summary>
Motivation: The research aims to address the dilemma faced when filtering web data for training non-English language models—whether to focus on diversity or quality.

Method: Hierarchical quality filters were applied to 500M German web documents, comparing multi-epoch training on filtered subsets to single-pass training on diverse data.

Result: Experiments demonstrated that repeated training on high-quality datasets consistently outperformed the single-pass approach, with performance improvements even after multiple epochs.

Conclusion: The findings support prioritizing quality filtering for efficient language modeling in non-English LLMs, leading to the release of state-of-the-art German models achieving high performance with significantly fewer tokens.
...
TL;DR: This paper introduces TopBench, a benchmark for evaluating Large Language Models (LLMs) on implicit predictive tasks in Table Question Answering, highlighting challenges in intent recognition and predictive reasoning.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the limitations of LLMs in handling real-world queries that require predictive reasoning rather than simple information retrieval from tables.

Method: TopBench consists of 779 samples across four sub-tasks, including single-point prediction and complex filtering, assessing models' ability to generate reasoning text along with structured table outputs.

Result: Experiments show that existing LLMs frequently fail to recognize implicit intent and instead resort to basic lookups, with intent disambiguation being critical for achieving accurate predictions.

Conclusion: Improving prediction precision in LLMs necessitates enhanced modeling techniques and reasoning abilities to better handle the complexities of implicit predictive tasks.
...
TL;DR: This paper dissects the assumptions in surprisal theory related to linguistic units and proposes a unified framework for evaluating processing effort without conflating unit definitions with prediction regions.


<details>
  <summary>Details</summary>
Motivation: To clarify the often vague definition of linguistic units in surprisal theory, which affects the accuracy of processing predictions.

Method: The authors analyze the discrepancies between how linguistic units are defined in experiments versus the tokenization used by pretrained language models, proposing a unified approach to handling these aspects.

Result: They show that by separating the definition of the unit of analysis from the regions of interest, predictions can be more accurate and meaningful.

Conclusion: Surprisal analyses should explicitly define these choices and treat tokenization as a technicality rather than a fundamental aspect of linguistic analysis.
...
### [138] [MAEO: Multiobjective Animorphic Ensemble Optimization for Scalable Large-scale Engineering Applications](https://arxiv.org/abs/2604.26973)
*Omer F. Erdem,Dean Price,Paul Seurin,Majdi I. Radaideh*

Main category: cs.NE

TL;DR: MAEO framework enhances multiobjective optimization by integrating various evolutionary algorithms to balance convergence, diversity, and efficiency, demonstrating effective performance in benchmark and practical scenarios.


<details>
  <summary>Details</summary>
Motivation: To address challenges in multiobjective optimization, such as balancing convergence, diversity, and computational efficiency, especially in high-dimensional objective landscapes.

Method: MAEO employs a parallelizable ensemble strategy using multiple evolutionary algorithms in an island-based architecture, featuring a parameter-free hypervolume indicator and a Pareto-rank-based scoring that incorporates crowding distance.

Result: Through extensive benchmarking on 12 functions, MAEO shows improved convergence-diversity performance, often outperforming or matching leading algorithms, backed by statistical validation using Wilcoxon signed-rank tests.

Conclusion: In practical applications, MAEO successfully optimizes for reactor design variables while meeting safety constraints, indicating its effectiveness in real-world scenarios.
...
### [139] [EdgeSpike: Spiking Neural Networks for Low-Power Autonomous Sensing in Edge IoT Architectures](https://arxiv.org/abs/2604.27004)
*Gustav Olaf Yunus Laitinen-Fredriksson Lundstrom-Imanov,Taner Yilmaz*

Main category: cs.NE

TL;DR: EdgeSpike is a spiking neural network framework designed for low-power sensing in edge IoT applications, achieving high accuracy and significant energy savings across multiple tasks and hardware platforms.


<details>
  <summary>Details</summary>
Motivation: The framework aims to enable efficient, low-power computation for autonomous sensing tasks in the growing field of edge IoT, where energy and resource constraints are critical.

Method: EdgeSpike integrates a hybrid training pipeline, hardware-aware NAS, event-driven runtimes, and a local plasticity rule for continual on-device adaptation without backpropagation.

Result: The framework achieves 91.4% classification accuracy while significantly reducing energy consumption across neuromorphic and embedded systems, with latency under 9.4 ms.

Conclusion: Field deployment shows a substantial increase in battery life and minimal accuracy degradation with on-device adaptation; EdgeSpike will be made open source for community use.
...
### [140] [RCMAES: A Robust CMA-ES Variant for CEC2026 Competition](https://arxiv.org/abs/2604.27138)
*Khoirul Faiq Muzakka,Sören Möller,Martin Finsterbusch*

Main category: cs.NE

TL;DR: RCMAES, a new variant of CMA-ES, enhances optimization with a unique population-size reduction and adaptive restart, showing strong results against top DE algorithms.


<details>
  <summary>Details</summary>
Motivation: To improve performance in optimization tasks by modifying existing CMA-ES with advanced strategies.

Method: RCMAES employs a dimension-dependent nonlinear population-size reduction combined with an adaptive restart mechanism within the standard CMA-ES framework.

Result: RCMAES outperforms state-of-the-art differential evolution algorithms and is compared favorably with BIPOP-aCMAES across multiple benchmark suites.

Conclusion: RCMAES demonstrates competitive and robust performance in optimization tasks across various benchmarks.
...
### [141] [Attractor FCM](https://arxiv.org/abs/2604.27947)
*Alexis Kafantaris*

Main category: cs.NE

TL;DR: This paper introduces a novel attractor Fuzzy Cognitive Map (FCM) employing a gradient descent method that incorporates physics constraints and various unique features aimed at effective weight updating and error minimization.


<details>
  <summary>Details</summary>
Motivation: To develop a new attractor FCM that improves convergence and learning efficiency by integrating unique updating mechanisms and an adaptive learning algorithm.

Method: The model utilizes a gradient descent approach combined with backward propagation through time, fixed point anchors, and residual memory to update weights, ensuring convergence and minimizing error efficiently.

Result: The attractor FCM is shown to reduce error effectively by leveraging adaptive terms, causal masking, and an innovative learning algorithm that respects initial expert opinions and conveys physics constraints.

Conclusion: The proposed FCM model addresses key limitations in previous methods by providing a robust framework for accurate gradient descent and efficient weight updates, leading to improved performance and convergence stability.
...
### [142] [Towards the Democratization and Standardization of Dynamic Resources with MPI Spawning](https://arxiv.org/abs/2604.27430)
*Sergio Iserte,Iker Martín-Alvarez,Krzystof Rojek,José I. Aliaga,Maribel Castillo,Antonio J. Peña*

Main category: cs.DC

TL;DR: The paper introduces an innovative API for dynamic resource management in HPC, improving flexibility and user-friendliness while presenting performance enhancements through the MPDATA application.


<details>
  <summary>Details</summary>
Motivation: To address limitations in existing resource management frameworks and improve adaptability in dynamic resource allocation for HPC applications.

Method: A unified dynamic resource management API was developed, integrating various resource managers and the Proteo reconfiguration engine within the DMR framework.

Result: The solution demonstrates improved performance and user coding productivity, specifically exemplified through the MPDATA application.

Conclusion: The enhancements in the DMR framework significantly advance the management of dynamic resources, allowing for more effective reconfiguration strategies and better support for high-performance applications.
...
TL;DR: This paper investigates optimization techniques for enhancing throughput and latency in Hyperledger Fabric by systematically analyzing different phases of the transaction lifecycle.


<details>
  <summary>Details</summary>
Motivation: The aim is to address challenges in optimizing Hyperledger Fabric's performance while ensuring decisions in one transaction phase do not negatively impact others.

Method: The study employs a combination of production-grade testbed experiments and calibrated SimPy simulations, introducing novel techniques like block-level pipelining and strategic waiting, and conducting micro-benchmarks on configuration levers.

Result: The introduction of pipelining and strategic waiting resulted in significant throughput improvements, alongside key insights regarding private-data dissemination and block configurations under varying workloads.

Conclusion: The research provides valuable phase-aware optimizations and guidelines for enhancing the performance of Hyperledger Fabric, maximizing throughput gains with moderate parallelization.
...
### [144] [A Study on the Performance of Distributed Training of Data-driven CFD Simulations](https://arxiv.org/abs/2604.27431)
*Sergio Iserte,Alejandro González-Barberá,Paloma Barreda,Krzysztof Rojek*

Main category: cs.DC

TL;DR: This paper introduces a predictive model for fluid simulation using data-driven methods to enhance efficiency in AI-assisted predictions compared to traditional PDE methods.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the computational difficulty and time intensity of traditional PDE methods for simulating physical behaviors, prompting the exploration of data-driven alternatives via AI.

Method: The study evaluates different training strategies—CPU, multiGPU, and distributed approaches—for a deep learning model focused on time series forecasting in fluid simulations.

Result: The results indicate that distributed GPU-enabled training significantly enhances prediction accuracy and reduces computation time compared to traditional CFD solvers.

Conclusion: The findings advocate for employing distributed GPU training methods to optimize the prediction of fluid simulation states, thereby showcasing a practical improvement in simulation efficiency.
...
### [145] [Efficient Training on Multiple Consumer GPUs with RoundPipe](https://arxiv.org/abs/2604.27085)
*Yibin Luo,Shiwei Gao,Huichuan Zheng,Youyou Lu,Jiwu Shu*

Main category: cs.DC

TL;DR: RoundPipe is a new pipeline parallelism approach for fine-tuning large language models on consumer GPUs that overcomes weight binding issues, yielding significant speed improvements.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing pipeline parallelism schedules in fine-tuning LLMs on consumer-grade GPUs, specifically the weight binding issue that reduces throughput.

Method: RoundPipe utilizes a round-robin dispatch of computation stages across multiple GPUs, integrates a priority-aware transfer scheduling engine, and employs a fine-grained synchronization protocol with automated layer partitioning.

Result: RoundPipe provides 1.48--2.16 times speedup over current baselines when fine-tuning models ranging from 1.7B to 32B parameters, facilitating training of large models like the Qwen3-235B on a single GPU server.

Conclusion: RoundPipe significantly enhances the efficiency of fine-tuning LLMs on consumer hardware, proving effective with extensive models and is accessible as an open-source tool.
...
TL;DR: The paper proposes FlexTender, a new framework that integrates flexible endorsement into a classical order-execute blockchain architecture to improve performance for DeFi applications, achieving significant speedup in throughput.


<details>
  <summary>Details</summary>
Motivation: Modern blockchains need flexible endorsement due to compliance and governance issues, but existing frameworks experience high abort rates in DeFi workloads.

Method: The paper proposes a framework called FlexTender that integrates flexible endorsement into the traditional order-execute architecture while maintaining decentralization and censorship resistance, utilizing the Tendermint BFT protocol.

Result: Empirical evaluation shows that FlexTender achieves a throughput speedup of up to 10.6 times compared to an execute-order-validate simulation on the same platform.

Conclusion: FlexTender enhances performance in high-contention workloads without adding messaging overhead, making it suitable for decentralized finance applications.
...
TL;DR: This paper investigates the robustness of population protocols in distributed computation, establishing that all predicates in monadic Presburger arithmetic have robust protocols and analyzing the state complexity associated with such robustness.


<details>
  <summary>Details</summary>
Motivation: The work addresses a gap in the understanding of whether all Presburger predicates can be computed by robust protocols amidst adversarial failures, as questioned by Lossin et al.

Method: The authors prove the existence of robust protocols for monadic Presburger predicates and analyze the state complexity by comparing robust protocols with their non-robust counterparts.

Result: All predicates in monadic Presburger arithmetic are proven to have robust protocols, and the authors demonstrate that the efficiency of these protocols has significant state complexity costs.

Conclusion: The research concludes that while robust protocols exist for monadic Presburger predicates, their state complexity is at least double exponential compared to standard protocols, with optimal performance established for specific threshold predicates.
...
TL;DR: ZipCCL introduces an efficient approach to reduce communication bottlenecks in LLM training via lossless compression tailored for Gaussian-distributed model data.


<details>
  <summary>Details</summary>
Motivation: Communication in distributed training of large language models is inefficient, and existing compression methods often incur more overhead than they save.

Method: ZipCCL employs exponent coding based on Gaussian distribution, optimized GPU kernels, and adaptive communication strategies to enhance data compression during training.

Result: ZipCCL yields up to 1.35x reduction in communication time and up to 1.18x end-to-end training speed improvements on a 64-GPU cluster.

Conclusion: ZipCCL effectively accelerates the training of LLMs while maintaining model quality through innovative compression techniques.
...
### [149] [AI Inference as Relocatable Electricity Demand: A Latency-Constrained Energy-Geography Framework](https://arxiv.org/abs/2604.27855)
*Xubin Luo,Yang Cheng*

Main category: cs.DC

TL;DR: The paper investigates the impact of AI inference on electricity demand and proposes a framework for optimizing the geographic distribution of this demand based on latency constraints.


<details>
  <summary>Details</summary>
Motivation: To understand how AI inference workloads can affect electricity consumption geographically, allowing for potential optimization of energy use and carbon emissions.

Method: The study introduces an energy-geography framework consisting of clients, service nodes, and compute nodes, modeled as a constrained optimization problem that considers various factors like electricity prices and latency.

Result: The model identifies operational metrics and introduces the concept of an energy-latency frontier to analyze the trade-offs between latency and the benefits of relocating inference workloads.

Conclusion: While increasing latency tolerance can broaden feasible locations for AI computation, practical factors like migration frictions and legal constraints may limit the actual benefits.
...
### [150] [AnTi-MiCS: Analytical Framework for Bounding Time in Embedded Mixed-Criticality Systems](https://arxiv.org/abs/2604.27862)
*Behnaz Ranjbar,Akash Kumar*

Main category: cs.DC

TL;DR: The paper presents AnTi-MiCS and MulTi-MiCS methodologies for determining low Worst-Case Execution Times (WCET) in Mixed-Criticality systems to balance processor utilization and Quality-of-Service, achieving improved QoS and reduced utilization waste in experiments.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of determining appropriate low WCETs for lower-criticality modes in Mixed-Criticality systems, which is essential for achieving realistic executions and optimal system performance.

Method: An analytical approach is introduced called AnTi-MiCS for determining low WCET through design-time analysis. Additionally, MulTi-MiCS is proposed to compute multiple low WCETs, addressing distribution variations using temporal correlations of inputs.

Result: Experimental results show a 30.27% improvement in QoS and a 35.89% reduction in utilization waste compared to existing methods; MulTi-MiCS further improves QoS by 6.41% over AnTi-MiCS with an 8.23% reduction in utilization waste.

Conclusion: The proposed approaches enhance the performance of Mixed-Criticality systems by efficiently determining low WCETs that improve overall QoS and minimize processor utilization waste.
...
### [151] [The Origins of MEV: Systematic Attribution of Arbitrage Opportunity Creation at Scale](https://arxiv.org/abs/2604.27979)
*Andrei Seoev,Dmitry Belousov,Anastasiia Smirnova,Ksenia Kurinova,Aleksei Smirnov,Denis Fedyanin,Yury Yanovich*

Main category: cs.DC

TL;DR: This paper addresses the attribution of Maximal Extractable Value (MEV) opportunities to their origins on-chain, proposing a framework and four methods for analysis, revealing concentrated MEV creation among a few protocols.


<details>
  <summary>Details</summary>
Motivation: To establish systematic methods for attributing MEV opportunities to their transactional origins on blockchain networks, a gap in current research.

Method: The paper introduces four methods for attributing atomic arbitrage MEV opportunities: bot-data-driven, simulation-based, coefficient-based, and Shapley-based, evaluated through a large-scale analysis on Polygon.

Result: Majority of atomic arbitrage opportunities can be traced back to single source transactions; a concentrated distribution of MEV creation is found, predominantly from a small number of protocols.

Conclusion: Findings provide valuable insights for protocol designers and validators, suggesting ways to reduce MEV leakage and enhance ecosystem health.
...
### [152] [Exploring Sparse Matrix Multiplication Kernels on the Cerebras CS-3](https://arxiv.org/abs/2604.27985)
*Milan Shah,Sheng Di,Michela Becchi*

Main category: cs.DC

TL;DR: The Cerebras CS-3 AI accelerator shows significant potential for sparse linear algebra kernels, achieving up to 100x speedup over CPU for SpMM and 20x for SDDMM; however, performance degrades with extreme sparsity.


<details>
  <summary>Details</summary>
Motivation: To evaluate and optimize the performance of the Cerebras CS-3 for sparse linear algebra tasks, particularly given its established efficacy for dense workloads.

Method: The study explores sparse-dense matrix multiplication (SpMM) and sampled dense-dense matrix multiplication (SDDMM) on the CS-3, proposing low-level kernel designs and optimizations for I/O performance, memory footprint, and scalability.

Result: The CS-3 demonstrated up to 100x speedup for SpMM and 20x for SDDMM compared to CPU, with performance improving with matrix dimensionality, but degrading beyond 99% sparsity in SpMM.

Conclusion: While the CS-3 excels in performance for certain sparse workloads, it encounters limitations as sparsity increases, indicating a threshold beyond which performance declines relative to CPU.
...
### [153] [From Impermanent Loss to Sustainable Gain: Quantifying Profitability Zones for Liquidity Providers on DEX](https://arxiv.org/abs/2604.28014)
*Ignat Melnikov,Roman Vlasov,Vladimir Gorgadze,Andrey Seoev,Yury Yanovich*

Main category: cs.DC

TL;DR: This paper analyzes decentralized exchange (DEX) swap protocols in DeFi, focusing on the interactions between arbitrageurs and Automated Market Maker (AMM) liquidity pools, proposing a mathematical framework to assess revenue and risks, including impermanent loss.


<details>
  <summary>Details</summary>
Motivation: The paper addresses inefficiencies in decentralized exchange mechanisms and the need for a better understanding of the interactions between liquidity providers and arbitrageurs.

Method: A mathematical model is developed based on empirical pool configurations to analyze the joint revenue of liquidity providers and arbitrageurs and estimate the expected occurrence of impermanent loss.

Result: The model provides bounds on revenue, estimates for impermanent loss occurrence, and establishes a lower bound for pool fees to maintain profitability.

Conclusion: The framework enhances liquidity provider risk assessment and aligns incentives between LPs and arbitrageurs, contributing to improved market stability.
...
### [154] [Characterizing Path-Independent Fees: A Route to Zero Impermanent Loss in CPMMs](https://arxiv.org/abs/2604.28017)
*Andrey Voronin,Roman Vlasov,Vladimir Gorgadze,Andrey Seoev,Yury Yanovich*

Main category: cs.DC

TL;DR: This paper analyzes fee structures for Constant Product Market Makers that ensure path independence, deriving equations for pool dynamics, and constructing fee functions minimizing Impermanent Loss.


<details>
  <summary>Details</summary>
Motivation: The paper addresses complications in risk assessment and composability under varying fee structures in Constant Product Market Makers, especially with reinvested fees.

Method: The authors characterize fee structures dependent solely on the pool invariant k=xy, derive ordinary differential equations for pool dynamics, and construct a parametric family of fee functions.

Result: They demonstrate that their specific fee functions can achieve zero Impermanent Loss for a given pool state but establish that no single fee function can do so universally for all states.

Conclusion: The findings facilitate fee optimization in protocol design, aligning incentives for liquidity providers and traders while ensuring composability.
...
TL;DR: Akita is a simulation engine designed to enhance computer architecture simulation by separating user experience from technical functionality, thus improving productivity in architectural research.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the fundamental barriers in using and developing computer architecture simulators, which stem from neglecting user and developer experience in favor of technical functionality.

Method: Introducing Akita, a dedicated simulation engine that decouples infrastructure from architectural models, allowing for simple cycle-based code that achieves event-driven performance, with transparent parallel simulation and robust tracing capabilities.

Result: Case studies including a DNN simulation and a RISC-V CPU simulation demonstrate Akita's versatility and effectiveness in improving developer experience and accelerating architectural research.

Conclusion: By prioritizing the user experience and separating it from technical concerns, Akita creates opportunities for more productive and innovative architectural research.
...
### [156] [Towards Topology-Aware Very Large-Scale Photonic AI Accelerators](https://arxiv.org/abs/2604.26966)
*Belal Jahannia,Abdolah Amirany,Hamed Dalir*

Main category: cs.AR

TL;DR: This paper introduces a scalable photonic accelerator architecture that addresses the limitations of existing electronic accelerators for deep neural networks by examining system-level scalability, focusing on network topology to improve performance and energy efficiency.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the limitations of electronic accelerators for DNNs, particularly energy consumption due to data movement, and explores photonic accelerators as a more efficient alternative.

Method: The authors develop a modular photonic accelerator architecture using 4 X 4 tensor core units and carry out a systematic architectural analysis including practical scaling limits and representative DNN workloads.

Result: The research identifies a topology-dominated scaling bottleneck, termed the Utilization Wall, and shows that symmetric grid topologies can enhance utilization by up to 6X and reduce memory access by over 40% compared to linear configurations.

Conclusion: Topology-aware scaling is crucial for optimizing energy efficiency and performance in photonic AI accelerators.
...
### [157] [Predictive Multi-Tier Memory Management for KV Cache in Large-Scale GPU Inference](https://arxiv.org/abs/2604.26968)
*Sanjeev Rao Ganjihal*

Main category: cs.AR

TL;DR: The paper presents a unified system for managing KV cache memory in GPU inference serving, addressing inefficiencies in cache sizing, memory tier usage, and eviction policies to enhance throughput and cost-efficiency.


<details>
  <summary>Details</summary>
Motivation: To overcome inefficiencies in KV cache management that restrict the performance and cost-effectiveness of GPU inference serving.

Method: The proposed unified system includes an architecture-variant-aware sizing engine, a six-tier memory hierarchy, and a Bayesian reuse predictor for cache optimization.

Result: The system enables up to 7.4x larger batch sizes, increases effective KV cache capacity significantly, and achieves high cache hit rates (70-84%).

Conclusion: Projected benefits include a TTFT reduction of 1.4-2.1x, throughput improvement by 1.7-2.9x, and a 47% reduction in costs compared to current baselines.
...
### [158] [Multibit neural inference in a N-ary crossbar architecture](https://arxiv.org/abs/2604.26979)
*Anatole Moureaux,Anthony Lopes Temporao,Flavio Abreu Araujo*

Main category: cs.AR

TL;DR: This paper presents a simulation framework for N-ary crossbar architectures that optimize energy-efficient neural network inference through minimal implementation assumptions, achieving notable results in classification tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this research is to enhance energy efficiency in neural network inference by leveraging in-memory computing for improved performance in analog matrix-vector multiplications.

Method: The authors developed a simulation framework for N-ary crossbar architectures and conducted experiments on XOR and MNIST classification tasks using 4-state magnetic tunnel junctions in a (4x4) array, analyzing the effects of weight quantization and various noise factors.

Result: The MNIST classification achieved an accuracy of 94.48%, which is relatively close to the software baseline of 97.56%. PCA dimensionality reduction helped in narrowing the software-hardware performance gap. Additionally, the study identified weight quantization as a significant source of error, influenced by systematic nonidealities and random noise.

Conclusion: The research concludes that an optimal number of states per cell can balance quantization error and resistance state resolution to minimize total matrix-vector multiplication error, with findings indicating that random noise is less detrimental than systematic errors.
...
### [159] [EMiX: Emulating Beyond Single-FPGA Limits](https://arxiv.org/abs/2604.27012)
*Alexander Kropotov,Miquel Moreto,Behzad Salami*

Main category: cs.AR

TL;DR: EMiX is a multi-FPGA framework for distributed emulation of large-scale RISC-V architectures, overcoming resource limits of single FPGAs by partitioning designs across multiple FPGAs without RTL redesign.


<details>
  <summary>Details</summary>
Motivation: The increasing complexity of multi-core systems in chip design requires solutions that extend beyond the limitations of single FPGA resources.

Method: EMiX partitions monolithic multi-core designs into components, distributes them across interconnected FPGAs, leveraging interconnects for enhanced scalability and performance.

Result: Prototyped with a 64-core RISC-V architecture on eight Alveo U55c FPGAs, EMiX demonstrated full-system execution, including booting Linux.

Conclusion: EMiX presents a scalable, efficient solution for large-scale emulation in chip design, to be released as an open-source platform.
...
### [160] [Verification and Validation (V&V)-in-the-Loop for RISC-V Design: The Holistic Vision of BZL](https://arxiv.org/abs/2604.27013)
...
TL;DR: The BZL project develops a comprehensive pre-silicon verification methodology for RISC-V chips, enhancing Europe's capacity in high-performance computing.


<details>
  <summary>Details</summary>
Motivation: To strengthen Europe's design and manufacturing capabilities for RISC-V high-performance computing chips.

Method: A holistic V&V methodology using UVM-based environments, FPGA validation platforms, and a CI/CD flow for automation.

Result: Integration of complementary platforms facilitates thorough RTL functionality validation and continuous hardware/software evolution.

Conclusion: The BZL's V&V approach provides a scalable foundation for RISC-V pre-silicon validation, supporting strategic European initiatives.
...
### [161] [CuLifter: Lifting GPU Binaries to Typed IR](https://arxiv.org/abs/2604.27486)
*Jisheng Zhao,Huanzhi Pu,Shinnung Jeong,Chihyo Ahn,Hyesoon Kim*

Main category: cs.AR

TL;DR: CuLifter is a framework that recovers register types from GPU binaries, achieving a 99.98% lifting success rate to LLVM IR across diverse benchmark suites.


<details>
  <summary>Details</summary>
Motivation: The challenge of recovering type information from a unified register file in GPU binaries, which complicates binary-analysis tasks.

Method: CuLifter employs constraint propagation with conflict detection to recover register types, reconstruct control flow, and aggregate instruction patterns.

Result: CuLifter successfully lifts 99.98% of GPU functions to valid LLVM IR across 24,437 functions in varied benchmarks.

Conclusion: Type recovery is crucial for generating semantically correct IR; disabling it leads to a complete failure in lifting.
...
### [162] [HAVEN: Hybrid Automated Verification ENgine for UVM Testbench Synthesis with LLMs](https://arxiv.org/abs/2604.27643)
*Chang-Chih Meng,Yu-Ren Lu,Guan-Yu Lin,Tsung Tai Yeh,Kai-Chiang Wu,I-Chen Wu*

Main category: cs.AR

TL;DR: HAVEN proposes a novel approach to IC verification by leveraging LLMs in conjunction with structured templates and a domain-specific language to enhance UVM testbench and sequence generation, achieving impressive coverage and success rates.


<details>
  <summary>Details</summary>
Motivation: To address the high verification overhead in IC development and the limitations of LLMs in generating correct HDL code due to the scarcity of HDLs in training data.

Method: HAVEN integrates LLM agents for analysis, along with structured templates and a Protocol-Aware DSL to generate UVM testbenches and sequences. It iteratively improves coverage through targeted DSL sequences based on coverage reports.

Result: HAVEN demonstrates 100% compilation success, 90.6% code coverage, and 87.9% functional coverage across 19 open-source IP designs, indicating its effectiveness compared to existing systems.

Conclusion: HAVEN stands out as a state-of-the-art solution for LLM-assisted testbench generation, combining predefined templates and a specialized DSL to optimize the verification process.
...
### [163] [NeuroRing: Scaling Spiking Neural Networks via Multi-FPGA Bidirectional Ring Topologies and Stream-Dataflow Architectures](https://arxiv.org/abs/2604.28059)
*Muhammad Ihsan Al Hafiz,Artur Podobas*

Main category: cs.AR

TL;DR: NeuroRing is a scalable SNN accelerator that utilizes a stream-dataflow architecture on FPGAs, addressing the challenges of SNN execution.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenges associated with large-scale SNN execution, specifically the issues of spike communication and synchronization that hinder efficiency on current computing platforms.

Method: NeuroRing employs a modular and scalable architecture based on stream-dataflow and a bidirectional ring topology, designed for deployment on programmable FPGAs and compatible with the NEST simulator.

Result: NeuroRing achieves faster-than-real-time execution on cortical microcircuit benchmarks with an RTF of 0.83 and demonstrates strong and weak scaling properties while maintaining energy efficiency on two FPGAs.

Conclusion: NeuroRing offers a flexible platform for neuroscience simulation and broader event-driven applications, efficiently addressing scalability and execution challenges in SNNs.
...
### [164] [RuC: HDL-Agnostic Rule Completion Benchmark Generation](https://arxiv.org/abs/2604.27780)
...
TL;DR: The paper presents RuC, a grammar-driven framework that generates RTL code-completion tasks for evaluating Large Language Models (LLMs) in hardware design, highlighting performance differences based on model type and prompting strategies.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing benchmarks that can't control code-completion granularity or syntactic range in evaluating LLM performance in RTL development.

Method: The RuC framework generates tasks by masking syntactically defined regions of RTL code and prompting LLMs to regenerate them using context from unmasked code, allowing for flexible evaluation.

Result: Two SystemVerilog rule-completion benchmarks from specific designs show that LLM performance varies with model type, grammar structure, and prompting strategies, with the best results from Fill-in-the-Middle (FIM) prompting.

Conclusion: The study demonstrates the importance of grammar-driven benchmarks for effectively assessing LLM capabilities in RTL development workflows.
...
### [165] [AME-PIM: Can Memory be Your Next Tensor Accelerator?](https://arxiv.org/abs/2604.27808)
*Emanuele Venieri,Simone Manoni,Alberto Florian,Jaehyun Park,Kyomin Sohn,Andrea Bartolini*

Main category: cs.AR

TL;DR: This paper explores using High Bandwidth Memory with Processing-in-Memory (HBM-PIM) for matrix acceleration via RISC-V's Attached Matrix Extension (AME), proposing an execution model that reduces data movement and improves computational efficiency.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current commercial HBM-PIM platforms that restrict instruction sets and require complex software, enabling effective matrix acceleration.

Method: The authors propose a PEP-based execution model that utilizes HBM-PIM micro-kernels for mapping AME instructions and employs a reduction-free outer-product dataflow to facilitate operations within memory.

Result: Experimental results demonstrate that the AME matrix tile multiplication can achieve up to 14.9 GFLOP/s (59.4 FLOP/cycle) on a single HBM pseudo-channel, showcasing significant performance gains.

Conclusion: The approach effectively supports matrix operations in PIM mode, minimizing reliance on the host and reducing off-chip data transfers, thus enhancing computational efficiency.
...
### [166] [C8s: A Confidential Kubernetes Architecture](https://arxiv.org/abs/2604.26974)
*Amean Asad,Patrick McClurg,João Andrade*

Main category: cs.CR

TL;DR: The paper introduces C8s, a confidential computing architecture enhancing Kubernetes security using Trusted Execution Environments (TEEs) for data confidentiality, integrity, and verifiability.


<details>
  <summary>Details</summary>
Motivation: To address security concerns in Kubernetes clusters by providing assurances against infrastructure operators and allowing sensitive workloads to run securely.

Method: C8s utilizes hardware TEEs, namely AMD SEV-SNP, Intel TDX, and NVIDIA Confidential Computing, to create a trust boundary that ensures cryptographic guarantees against data breaches.

Result: The architecture enables distinct guarantees for data owners, compute providers, and end users, significantly enhancing security in managed Kubernetes environments.

Conclusion: C8s allows safe deployment of sensitive workloads in cloud environments by ensuring data confidentiality and integrity, applicable to AI-related tasks.
...
### [167] [Agent Name Service (ANS): A Proof-of-Concept Trust Layer for Secure AI Agent Discovery, Identity, and Governance in Kubernetes](https://arxiv.org/abs/2604.26997)
*Akshay Mittal,Elyson De La Cruz*

Main category: cs.CR

TL;DR: The paper proposes the Agent Name Service (ANS) as a trust layer for AI agent discovery and interoperability, addressing security and governance issues in autonomous AI ecosystems.


<details>
  <summary>Details</summary>
Motivation: To enhance security mechanisms in autonomous AI ecosystems, focusing on agent discovery, authentication, capability proof, and policy governance.

Method: The implementation of ANS uses Decentralized Identifiers, Verifiable Credentials, policy-as-code with Open Policy Agent, and integrates with Kubernetes.

Result: Demonstrated sub-10ms response times and successful scripted deployments in a 3-node cluster environment with a 50-agent simulation.

Conclusion: This work provides a proof-of-concept for secure multi-agent systems and outlines a threat model with assumptions and limitations.
...
### [168] [An Empirical Security Evaluation of LLM-Generated Cryptographic Rust Code](https://arxiv.org/abs/2604.27001)
*Mohamed Elsayed,Kenneth Fulton,Jeong Yang*

Main category: cs.CR

TL;DR: This study evaluates the cryptographic security of code generated by Large Language Models (LLMs) through empirical analysis of 240 Rust samples, revealing a low compilation success rate and highlighting vulnerabilities discovered by specialized analyzers.


<details>
  <summary>Details</summary>
Motivation: To assess the security and reliability of code generated by LLMs for cryptographic applications, particularly focusing on the effectiveness of different prompt strategies and the detection of vulnerabilities.

Method: An empirical evaluation was conducted on 240 Rust code samples generated by three LLMs (Gemini 2.5 Pro, GPT-4o, and DeepSeek Coder) for AES-256-GCM and ChaCha20-Poly1305 algorithms, using CodeQL static analysis and a specialized crypto-specific analyzer to identify vulnerabilities.

Result: Only 23.3% of generated samples compiled successfully. Among those, the crypto-specific analyzer found vulnerabilities in 57% of compiled samples, while CodeQL produced only two false positives. Compilation success varied between the two algorithms, with significant impacts from prompting methods.

Conclusion: The findings indicate that general-purpose analysis tools inadequately validate LLM-generated cryptographic code. This underscores the need for specialized analyzers and prompts to improve code generation reliability and security.
...
### [169] [Membership Inference Attacks Against Video Large Language Models](https://arxiv.org/abs/2604.27002)
*Wei Song,Yuxin Cao,Ziqi Ding,Yi Liu,Gelei Deng,Yuekang Li*

Main category: cs.CR

TL;DR: This paper addresses the vulnerability of VideoLLMs to membership inference attacks (MIAs) by exploring a novel black-box attack methodology. It shows that VideoLLMs can leak training data information through generation behaviors related to video characteristics.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the privacy concerns associated with VideoLLMs in terms of their susceptibility to membership inference attacks, an area previously unexplored compared to other model types.

Method: The method involves a black-box MIA that utilizes temperature-perturbed text generation, coupled with video-aware difficulty features, to analyze the differences in outputs based on varying video inputs.

Result: The attack on the 	exttt{LLaVA-Video-7B-Qwen2-Video-Only} model yielded a member inference AUC of 0.68 and an accuracy of 0.63, indicating a measurable vulnerability of VideoLLMs to such attacks.

Conclusion: The findings highlight the urgent need for the AI community to assess and address privacy risks inherent in VideoLLMs, pointing to their potential vulnerability to black-box membership inference attacks.
...
### [170] [Enhancing Linux Privilege Escalation Attack Capabilities of Local LLM Agents](https://arxiv.org/abs/2604.27143)
*Benjamin Probst,Andreas Happe,Jürgen Cito*

Main category: cs.CR

TL;DR: The paper assesses how interventions can improve the performance of open-weight models in penetration testing, finding that with certain techniques, they can outperform cloud-based models.


<details>
  <summary>Details</summary>
Motivation: The need for locally hosted open-weight models arises from security, privacy, and sovereignty concerns associated with cloud-based models used in penetration testing.

Method: The authors conducted a systematic empirical study, analyzing failure modes and implementing five targeted interventions to enhance the performance of open-weight models in autonomous privilege escalation.

Result: The study found that, with the interventions, open-weight models like Llama3.1 70B exploited 83% of vulnerabilities, outperforming smaller models, while highlighting that reflection-based treatments provided the most significant improvements.

Conclusion: The findings indicate that targeted interventions can effectively bridge the performance gap for open-weight models while identifying vulnerability discovery as a challenge that remains unaddressed.
...
### [171] [Indirect Prompt Injection in the Wild: An Empirical Study of Prevalence, Techniques, and Objectives](https://arxiv.org/abs/2604.27202)
*Soheil Khodayari,Xuenan Zhang,Bhupendra Acharya,Giancarlo Pellegrino*

Main category: cs.CR

TL;DR: The paper empirically analyzes indirect prompt injections in web content, assessing the prevalence and impact through a study of 1.2B URLs, revealing significant risks for LLMs and various systems.


<details>
  <summary>Details</summary>
Motivation: To understand the prevalence and effects of indirect prompt injections in web content, which pose risks to large language models (LLMs) integrated into various systems.

Method: Analyzed 1.2 billion URLs from 24.8 million hosts, identifying and validating instances of indirect prompt injections, and conducted 5,200 controlled experiments across different models and webpage representations.

Result: Identified 15.3K validated instances across 11.7K pages, with a small number of templates causing most incidents; significant risks were observed with compliance rates of up to 8% for models on plain-text inputs.

Conclusion: Prompt-based interference is prevalent in the web ecosystem, creating risks for LLM-driven automation interacting with web content.
...
### [172] [SafeTune: Mitigating Data Poisoning in LLM Fine-Tuning for RTL Code Generation](https://arxiv.org/abs/2604.27238)
*Mahshid Rezakhani,Nowfel Mashnoor,Kimia Azar,Hadi Kamali*

Main category: cs.CR

TL;DR: SafeTune is a framework that enhances the security of LLM-based RTL code generation by mitigating data poisoning attacks, particularly focusing on hardware Trojan insertion through structural modeling and semantic verification.


<details>
  <summary>Details</summary>
Motivation: Large language models face risks from low-quality training datasets that are vulnerable to data poisoning, which can lead to the generation of insecure hardware modules.

Method: SafeTune employs a Graph Neural Network to identify anomalous circuitry patterns and a semantic verification module using text embeddings and an XGBoost classifier to evaluate prompt security.

Result: Experimental results indicate that SafeTune effectively filters out poisoned inputs while maintaining the integrity of legitimate data, thus improving model robustness.

Conclusion: SafeTune enhances the reliability of LLM fine-tuning without altering the model's underlying architecture, providing a safeguard against potential security vulnerabilities.
...
### [173] [From Prompt to Physical Actuation: Holistic Threat Modeling of LLM-Enabled Robotic Systems](https://arxiv.org/abs/2604.27267)
*Neha Nagaraja,Hayretdin Bahsi,Carlo R. da Cunha*

Main category: cs.CR

TL;DR: This paper presents a unified threat analysis of LLM-integrated autonomous robots, identifying vulnerabilities across interaction points and their impacts on safety and security.


<details>
  <summary>Details</summary>
Motivation: To address the gap in understanding how cybersecurity, adversarial perception, and LLM safety threats interact and propagate within autonomous robotic systems.

Method: A hierarchical Data Flow Diagram (DFD) model of an LLM-enabled robot was developed, and a STRIDE-per-interaction analysis was performed at six boundary-crossing interaction points using a three-category taxonomy of threats.

Result: The analysis identified convergence of threat categories at boundary crossings and detailed three distinct attack chains leading to unsafe actuation, highlighting specific architectural vulnerabilities.

Conclusion: This work is the first to provide an integrated threat analysis across the perception-planning-actuation pipeline, revealing critical safety implications for LLM-enabled robotic systems.
...
### [174] [Static Attribution of Android Residential Proxy Malware Using Graph Kernels](https://arxiv.org/abs/2604.27302)
*Peter Clark,Yong Guan,Zhonghao Liao*

Main category: cs.CR

TL;DR: The study presents a static analysis method for automating the identification of Android proxyware applications, achieving high classification accuracy and linking ongoing relationships between developers and proxy providers.


<details>
  <summary>Details</summary>
Motivation: The rise of Android residential proxy applications as Potentially Unwanted Programs requires efficient methods for attribution to mitigate ad fraud and credential abuse.

Method: A static-analysis pipeline was developed to extract graph-structured representations and behavioral signatures from 3,365 proxy apps, evaluated using Weisfeiler-Lehman graph kernel features and binary capability vectors across classifiers, with 5-fold DEX-grouped cross-validation.

Result: The study achieved a macro F1 score of 0.985 with SGD. Classifier decisions were mapped to generated Yara rules, yielding per-family accuracies of 88.45% after filtering.

Conclusion: Many applications (51.4%) still available are found to contain proxy SDK code, with indications of ongoing relationships among developers and proxy providers.
...
### [175] [REBENCH: A Procedural, Fair-by-Construction Benchmark for LLMs on Stripped-Binary Types and Names (Extended Version)](https://arxiv.org/abs/2604.27319)
*Jun Yeon Won,Xin Jin,Shiqing Ma,Zhiqiang Lin*

Main category: cs.CR

TL;DR: REBench is a benchmark dataset for evaluating Large Language Models in binary reverse engineering, addressing the lack of standardized datasets and enabling fair comparisons across different tasks and models.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to facilitate fair evaluation and understanding of LLM capabilities in binary analysis, addressing challenges posed by the absence of standardized datasets and varying methodologies.

Method: The authors introduce REBench, a comprehensive dataset that consolidates existing datasets and includes a vast collection of source code and binaries. It utilizes a knowledge-base-driven methodology to preserve task difficulty and universal applicability.

Result: The application of REBench reveals that LLMs face significant challenges in complex reverse engineering tasks, highlighting the limitations of these models in this domain.

Conclusion: REBench provides a much-needed evaluation framework that can enhance future research in LLM-driven binary reverse engineering by enabling standardized comparisons and insights.
...
### [176] [Toward Autonomous SOC Operations: End-to-End LLM Framework for Threat Detection, Query Generation, and Resolution in Security Operations](https://arxiv.org/abs/2604.27321)
*Md Hasan Saju,Akramul Azim*

Main category: cs.CR

TL;DR: The paper presents a framework for automating threat management in Security Operations Centers, utilizing ensemble detection and SQM architecture to enhance incident response efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: SOCs face increasing operational challenges from high threat volumes, varied SIEM platforms, and slow manual workflows, necessitating automation solutions.

Method: An end-to-end threat management framework integrating ensemble-based detection of traditional machine learning classifiers and LLMs, along with the SQM architecture for evidence collection and query generation.

Result: The framework achieves 82.8% detection accuracy with a low false positive rate, SQM scores of 0.384 (BLEU) and 0.731 (ROUGE-L), and improves incident resolution accuracy from 78.3% to 90.0%.

Conclusion: The proposed framework significantly reduces incident triage time and establishes that domain-constrained LLMs with retrieval augmentation can satisfy the needs of security operations effectively.
...
### [177] [Secret Stealing Attacks on Local LLM Fine-Tuning through Supply-Chain Model Code Backdoors](https://arxiv.org/abs/2604.27426)
*Zi Li,Tian Zhou,Wenze Li,Jingyu Hua,Yunlong Mao,Sheng Zhong*

Main category: cs.CR

TL;DR: A new method for stealing sensitive information from fine-tuning datasets is presented, overcoming traditional passive poisoning techniques by exploiting model code execution.


<details>
  <summary>Details</summary>
Motivation: To highlight vulnerabilities in local fine-tuning datasets that contain sensitive information and the inadequacy of traditional attack methods.

Method: The paper introduces a deterministic full-chain memorization mechanism which uses online tensor-rule matching and value-gradient decoupling to hijack model execution and extract secrets.

Result: The proposed method achieves over 98% Strict ASR in secret extraction via black-box queries, successfully bypassing defenses like DP-SGD and auditing.

Conclusion: The study reveals significant weaknesses in current defenses against model code exploitations and offers a new approach for attackers to verify secret information theft.
...
### [178] [Tracking Conversations: Measuring Content and Identity Exposure on AI Chatbots](https://arxiv.org/abs/2604.27438)
*Muhammad Jazlan,Ethan Wang,Yash Vekaria,Zubair Shafiq*

Main category: cs.CR

TL;DR: This paper systematically examines web tracking on 20 popular AI chatbots, focusing on data sharing practices with third parties.


<details>
  <summary>Details</summary>
Motivation: To understand the extent of data exposure and tracking practices among AI chatbots, given their rising usage and integration of advertising and analytics.

Method: The study employs a controlled measurement approach, comparing network traffic during regular and private chats using specific prompts to identify information shared.

Result: The findings reveal that 17 out of 20 chatbots share information with third parties, with 15 transmitting conversation URLs or identifiers, and some sharing plaintext transcripts with Microsoft Clarity.

Conclusion: The study highlights significant privacy concerns, as many chatbots expose both conversational content and user identity through various tracking mechanisms.
...
### [179] [Secure Cross-Silo Synthetic Genomic Data Generation](https://arxiv.org/abs/2604.27456)
*Daniil Filienko,Martine De Cock,Sikha Pentyala*

Main category: cs.CR

TL;DR: This paper presents a method for generating synthetic genomic data that allows multiple data holders to collaboratively train a data generator without exposing private information.


<details>
  <summary>Details</summary>
Motivation: To enhance access to genomic data for AI methods while maintaining privacy in light of stringent data regulations.

Method: The approach utilizes secure multiparty computation (MPC) for input privacy and differential privacy (DP) for output privacy during the joint training of a synthetic data generator.

Result: The method successfully generates high-utility synthetic datasets from real RNA-seq data across different institutions in a privacy-preserving manner.

Conclusion: The proposed solution effectively balances the need for robust genomic data sharing with stringent privacy requirements.
...
### [180] [Security Attack and Defense Strategies for Autonomous Agent Frameworks: A Layered Review with OpenClaw as a Case Study](https://arxiv.org/abs/2604.27464)
*Luyao Xu,Xiang Chen*

Main category: cs.CR

TL;DR: This paper presents a layered review of security risks and defense strategies in autonomous agent frameworks using OpenClaw as a case study, highlighting the propagation of threats across different security layers.


<details>
  <summary>Details</summary>
Motivation: To systematically understand security implications of evolving autonomous agent frameworks based on large language models, addressing the scattered existing studies on agent security.

Method: A layered analysis was conducted, organizing security risks and defenses into four relevant layers: context and instruction, tool and action, state and persistence, and ecosystem and automation.

Result: The survey identifies security risks and defensible strategies for each layer, revealing that threats can propagate across layers, impacting inputs, actions, and the broader ecosystem.

Conclusion: The study calls for addressing key challenges like research imbalance, long-horizon evaluation, and improving ecosystem trust models to enhance security in autonomous agent frameworks.
...
### [181] [SST-Guard: Detecting and Characterizing Server-Side Google Analytics in the Wild](https://arxiv.org/abs/2604.27497)
*Muhammad Jazlan,Alexander Gamero-Garrido,Zubair Shafiq,Yash Vekaria*

Main category: cs.CR

TL;DR: The paper discusses a shift from client-side to server-side tracking in web analytics, focusing on Google Analytics. It introduces SST-Guard, a system for detecting and blocking server-side Google Analytics by analyzing semantic information rather than known endpoints, achieving high accuracy in identifying sGA usage on popular websites.


<details>
  <summary>Details</summary>
Motivation: To address the challenges posed by the transition from client-side to server-side tracking, particularly as existing protections become ineffective.

Method: SST-Guard employs a value-template approach utilizing regular expressions to detect semantic patterns in tracking across network requests, cookies, and the window object, rather than relying on specific tracker endpoints.

Result: SST-Guard successfully detected 4.02% of sGA domains with over 93% accuracy on Tranco top-10k websites, with the network request classifier achieving 99.8% accuracy.

Conclusion: The study highlights the presence of server-side Google Analytics on a significant percentage of top websites and demonstrates the effectiveness of SST-Guard in identifying these instances.
...
### [182] [SBN Explorer: An Empirical Study of Cryptographic Boolean Networks](https://arxiv.org/abs/2604.27560)
*Arnaud Valence*

Main category: cs.CR

TL;DR: This paper expands the design space of Boolean circuits in symmetric cryptography by identifying six structural constraints and evaluating 64 architectural classes against cryptanalytic objectives.


<details>
  <summary>Details</summary>
Motivation: To explore a broader design space for Boolean circuits in symmetric cryptography beyond established paradigms.

Method: The study formalizes six binary structural constraints and evaluates 64 architectural classes of Synchronous Boolean Networks using Formal Concept Analysis against cryptanalytic fitness objectives.

Result: The evaluation of the 64 classes highlights that optimal Boolean networks feature sparse, mutually compatible combinations of the proposed constraints.

Conclusion: The findings point to an epistatic problem in cryptography that has been largely overlooked, indicating potential new avenues for architectural design.
...
### [183] [SecGoal: A Benchmark for Security Goal Extraction and Formalization from Protocol Documents](https://arxiv.org/abs/2604.27601)
*Dawei Huang,Hui Li,Haonan Feng,Jingjing Guan,Yueshuang Jiao,Bo Jia*

Main category: cs.CR

TL;DR: This paper introduces SecGoal, an expert-annotated benchmark for evaluating the automation of extracting and formalizing security goals from natural language protocol documents, and presents AIFG, an AI-assisted framework for improved performance, showing that specialized instruction tuning significantly enhances precision compared to larger models.


<details>
  <summary>Details</summary>
Motivation: The automation of extracting and formalizing security goals from natural language protocol documents is hindered by the lack of expert-annotated resources and effective frameworks.

Method: The paper introduces SecGoal, an expert-annotated benchmark of 15 protocol documents, and AIFG, an AI-assisted framework that processes the extraction and formalization tasks. A comprehensive evaluation is performed using contemporary LLMs to assess their effectiveness in this pipeline.

Result: Contemporary LLMs show a precision-recall imbalance, with high recall rates but low precision (below 15% for some models), often misidentifying text. Instruction tuning on SecGoal improves precision, allowing smaller models to outperform larger general models with F1-scores above 80%.

Conclusion: The work provides a foundational dataset and reproducible baseline for automated formal protocol analysis, demonstrating the effectiveness of tailored instruction tuning on model performance.
...
### [184] [VOW: Verifiable and Oblivious Watermark Detection for Large Language Models](https://arxiv.org/abs/2604.27666)
*Xiaokun Luan,Yihao Zhang,Pengcheng Su,Feiran Lei,Meng Sun*

Main category: cs.CR

TL;DR: VOW offers privacy-preserving and verifiable watermark detection for machine-generated text, overcoming limitations of existing centralized models and ensuring text integrity without revealing sensitive information.


<details>
  <summary>Details</summary>
Motivation: To address the shortcomings of centralized watermarking methods that require users to disclose sensitive text for verification.

Method: The proposed VOW protocol uses secure two-party computation and Verifiable Oblivious Pseudorandom Functions (VOPRF) for watermark detection.

Result: VOW efficiently supports watermark detection for short texts and enhances robustness against modern paraphrasing attacks.

Conclusion: VOW represents a significant advancement in watermarking by ensuring privacy and verifiability in detecting machine-generated content.
...
### [185] [How Code Representation Shapes False-Positive Dynamics in Cross-Language LLM Vulnerability Detection](https://arxiv.org/abs/2604.27714)
*Maofei Chen,Laifu Wang,Yue Qin,Yuan Wang,Bo Wu,Dongxin Liu*

Main category: cs.CR

TL;DR: The study investigates how code representation format impacts false positive rates in cross-language vulnerability detection using LLMs, revealing that training-time and inference-time representations jointly affect performance, particularly highlighting the effect of text fine-tuning on false positives.


<details>
  <summary>Details</summary>
Motivation: To understand how different code representations influence vulnerability detection and false positive rates across various languages using large language models (LLMs).

Method: The study systematically varies training intensity and code representation formats (raw source text vs. pruned Abstract Syntax Trees) while fine-tuning two 8B-parameter LLMs on C/C++ data and evaluating on Java and Python benchmarks.

Result: Text fine-tuning increases false positive rates significantly while maintaining stable F1 scores. The use of AST representation reduces false positives dramatically compared to text representation, indicating the role of memorization of specific API names in false positives.

Conclusion: The findings suggest implementing a consistency gate for vulnerability alerts that utilizes both text and AST approaches to filter out false positives, albeit at the expense of some recall.
...
### [186] [Variational and Majorization Principles in Lattice Reduction](https://arxiv.org/abs/2604.27801)
*Javier Blanco-Romero,Florina Almenares Mendoza*

Main category: cs.CR

TL;DR: The paper explores lattice reduction and its effects on the Gram-Schmidt profile, using majorization to analyze the mechanisms involved in smoothing. It presents two structural consequences of this approach related to the GSA envelope and variance dissipation, and introduces two new selector methods for improved efficiency.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the smoothing effect of lattice reduction on the Gram-Schmidt profile by employing majorization as a theoretical framework.

Method: The authors examine the local swap mechanism with a focus on Lovász swaps acting as T-transforms, analyze variance dissipation through exact telescoping identities, and develop thermal families of Schur-convex scoring rules for adaptive selection.

Result: The findings indicate that every strictly Schur-convex measure of profile spread decreases during Lovász swaps, leading to a variational interpretation of the worst-case GSA envelope and demonstrating that the realized swap trajectory maintains a specific variance dissipation identity.

Conclusion: The paper concludes with the introduction of Thermal-Adaptive and Geodesic Deep-LLL selectors, which enhance operational efficiency in different lattice scenarios, suggesting their practical applications in reducing operation counts and improving performance.
...
### [187] [MASCing: Configurable Mixture-of-Experts Behavior via Activation Steering Masks](https://arxiv.org/abs/2604.27818)
*Jona te Lintelo,Lichao Wu,Marina Krček,Sengim Karayalçin,Stjepan Picek*

Main category: cs.CR

TL;DR: MASCing is a framework that allows flexible reconfiguration of Mixture-of-Experts behavior in Large Language Models to meet various safety objectives without the need for retraining, achieving significant improvements in safety-related performance.


<details>
  <summary>Details</summary>
Motivation: To address safety challenges in MoE architectures of Large Language Models that arise from sparse activation and routing decisions while enabling quick reconfiguration for different safety objectives without costly retraining.

Method: MASCing utilizes an LSTM-based surrogate model to capture routing dependencies and optimizes a steering matrix, applying steering masks during inference to adjust expert selection for specific behaviors.

Result: Demonstrated reconfigurability by improving safety defense rates and compliance with content generation requests, achieving up to 89.2% and 93.0% improvements in success rates respectively across seven open-source MoE models.

Conclusion: MASCing establishes a practical framework for targeted safety enhancement in MoE models while preserving overall language utility, demonstrating significant gains with minimal overhead.
...
### [188] [WOOTdroid: Whole-system Online On-device Tracing for Android](https://arxiv.org/abs/2604.27830)
*Simon Althaus,Nikolaos Alexopoulos,Max Mühlhäuser,Christian Reuter,Ephraim Zimmer*

Main category: cs.CR

TL;DR: WOOTdroid is a new Android auditing tool that overcomes syscall tracing limitations and semantic gaps without modifying the OS or user applications, effectively capturing more syscall data and decoding security-relevant Binder transactions.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenges of syscall tracers losing events under load and the semantic gap caused by hidden security-relevant behaviors in Android's Binder IPC mechanism.

Method: WOOTdroid utilizes WDSys for syscall tracing via eBPF, achieving minimal overhead, and employs WDBind to capture and decode Binder parcels out-of-process using a signature table created through Java reflection.

Result: WOOTdroid shows a syscall tracing capacity improvement, capturing 33% more syscalls than existing tools while maintaining less than 3.6% performance overhead, and successfully reconstructs ten security-relevant Binder transactions.

Conclusion: The proposed WOOTdroid effectively enhances Android system auditing by solving existing limitations without requiring OS modifications or user-space application instrumentation.
...
### [189] [TwinGate: Stateful Defense against Decompositional Jailbreaks in Untraceable Traffic via Asymmetric Contrastive Learning](https://arxiv.org/abs/2604.27861)
*Bowen Sun,Chaozhuo Li,Yaodong Yang,Yiwei Wang,Chaowei Xiao*

Main category: cs.CR

TL;DR: TwinGate introduces a dual-encoder defense framework against decompositional jailbreaks in large language models, achieving high recall of malicious intents with low false positives and efficient performance.


<details>
  <summary>Details</summary>
Motivation: Decompositional jailbreaks threaten LLMs by reconstructing malicious content through benign queries, necessitating a robust defense mechanism.

Method: TwinGate employs Asymmetric Contrastive Learning with a stateful dual-encoder architecture, clustering malicious intents while minimizing false positives and operational overhead.

Result: TwinGate shows high recall for malicious intents, low false positives, and outperforms existing defense strategies in throughput and latency.

Conclusion: TwinGate effectively mitigates threats posed by adversarial queries in LLMs, supporting future research with a comprehensive dataset.
...
### [190] [Latent Adversarial Detection: Adaptive Probing of LLM Activations for Multi-Turn Attack Detection](https://arxiv.org/abs/2604.28129)
*Prashant Kulkarni*

Main category: cs.CR

TL;DR: This paper introduces a detection method for multi-turn prompt injection attacks in conversational models by identifying an activation-level signature called adversarial restlessness, significantly improving detection accuracy over existing text-level defenses.


<details>
  <summary>Details</summary>
Motivation: To address the inadequacy of text-level defenses against multi-turn prompt injection attacks that use a strategy based on trust-building, pivoting, and escalation.

Method: The study analyzes activation-level signatures during different phases of attacks and develops five scalar trajectory features to enhance detection capabilities of adversarial conversations versus benign ones.

Result: Detection accuracy improved from 76.2% to 93.8% on synthetic data, with effective detection across multiple model families; real-world application yielded 47-71% detection rates depending on training data representation.

Conclusion: Adversarial restlessness is established as a significant detection signal, highlighting the importance of phase-specific labeling and diverse training data for effective deployment.
...
### [191] [FlashRT: Towards Computationally and Memory Efficient Red-Teaming for Prompt Injection and Knowledge Corruption](https://arxiv.org/abs/2604.28157)
*Yanting Wang,Chenlong Yin,Ying Chen,Jinyuan Jia*

Main category: cs.CR

TL;DR: FlashRT is a new framework designed to enhance the efficiency of optimization-based attacks on long-context large language models (LLMs), providing significant speed and memory usage improvements compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: There is a critical need to assess security risks in long-context LLMs due to threats like prompt injection and knowledge corruption, which are challenging to analyze due to the resource-intensiveness of existing methods.

Method: FlashRT optimizes the process of conducting prompt injection and knowledge corruption attacks, reducing computation time and memory requirements for long-context settings.

Result: FlashRT demonstrates a 2-7x improvement in attack speed and a 2-4x decrease in GPU memory usage compared to the leading baseline method, nanoGCG.

Conclusion: FlashRT is intended to facilitate widespread and efficient security evaluations in long-context LLMs and can be utilized with various black-box optimization techniques.
...
### [192] [Robot Planning and Situation Handling with Active Perception](https://arxiv.org/abs/2604.26988)
...
TL;DR: VAP-TAMP is a framework that aids robots in handling unforeseen execution situations using active perception and scene graph reasoning for better planning.


<details>
  <summary>Details</summary>
Motivation: To enhance robot autonomy in dynamic real-world environments where unforeseen challenges can impede task execution.

Method: Develops a planning and situation-handling framework (VAP-TAMP) integrating action knowledge, vision-language models for view selection, and scene graph reasoning.

Result: Demonstrated improved performance in service tasks during simulation and on a mobile manipulation robot.

Conclusion: VAP-TAMP significantly enhances robots' ability to tackle unexpected issues, promoting long-term autonomy in complex tasks.
...
### [193] [Interaction Forces and Internal Loads in Parallel Manipulators with Actuation Redundancy](https://arxiv.org/abs/2604.27095)
*Joshua Flight,Clément Gosselin*

Main category: cs.RO

TL;DR: The paper analyzes null-space wrench components in parallel manipulators, addressing characterization issues and providing methods for joint torque vector synthesis, supported by a case study.


<details>
  <summary>Details</summary>
Motivation: To clarify existing ambiguities in the definitions and applications of null-space wrench components concerning interaction forces and internal loads in parallel manipulators with actuation redundancy.

Method: Adaptation of two common characterizations of null-space components, resolving existing literature oversights, and introducing methods for synthesizing joint torque vectors.

Result: Identification and clarification of misconceptions in the definitions within the current literature and the presentation of methods that yield valid joint torque vector analyses.

Conclusion: The findings validate our methods, correct previously reported erroneous results, enhancing understanding of null-space components in parallel manipulators.
...
### [194] [PALCAS: A Priority-Aware Intelligent Lane Change Advisory System for Autonomous Vehicles using Federated Reinforcement Learning](https://arxiv.org/abs/2604.27118)
*Yassine Ibork,Nhat Ha Nguyen,Myounggyu Won,Lokesh Das*

Main category: cs.RO

TL;DR: PALCAS is a priority-aware lane change advisory system using federated reinforcement learning for autonomous vehicles, enhancing traffic efficiency and safety.


<details>
  <summary>Details</summary>
Motivation: To improve lane-change decision-making in autonomous vehicles by considering vehicle destination urgency and enhancing cooperation among multiple agents.

Method: Utilizes a federated reinforcement learning framework with a novel priority-aware safe lane-change reward function and the parameterized deep Q-network algorithm to manage AVs' lateral and longitudinal controls.

Result: Demonstrated significant improvements in traffic efficiency, driving safety, comfort, destination arrival rates, and merging success rates in simulations.

Conclusion: PALCAS represents an advanced approach to intelligent lane change advisories that enhances cooperation among AVs and optimizes lane change based on urgency.
...
### [195] [Framework for Collaborative Operation of Autonomous Delivery Vehicles Within a Marshaling Yard](https://arxiv.org/abs/2604.28057)
*James O'Hara,Karl Wunderlich,Gregory Stevens*

Main category: cs.RO

TL;DR: This paper explores the integration of hybrid autonomous vehicles and fixed infrastructure in marshaling yards to improve efficiency and reduce failures.


<details>
  <summary>Details</summary>
Motivation: The paper aims to enhance vehicle autonomy in controlled environments like marshaling yards, addressing edge case issues of urban autonomous vehicle deployment.

Method: The authors present a decentralized orchestrated autonomy system that dynamically prioritizes vehicles based on the current status of the yard, assigning tasks to optimize throughput.

Result: Simulation results indicate that the orchestration solution consistently increases vehicle throughput and reduces operational failures across various yard sizes and demand levels compared to traditional static methods.

Conclusion: The proposed solution effectively enhances operational efficiency in marshaling yards, paving the way for more reliable autonomous vehicle systems in confined spaces.
...
### [196] [Global Sampling-Based Trajectory Optimization for Contact-Rich Manipulation via KernelSOS](https://arxiv.org/abs/2604.27175)
*Zhongqi Wei,Frederike Dümbgen*

Main category: cs.RO

TL;DR: Global-MPPI is a novel trajectory optimization framework for contact-rich manipulation that combines global exploration and local refinement to effectively address challenges like high dimensionality and non-smooth landscapes.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenges in contact-rich manipulation which is difficult due to high dimensionality, long time requirements, and hybrid contact dynamics, highlighting the limitations of conventional sampling-based optimization methods.

Method: Global-MPPI integrates kernel sum-of-squares optimization for global exploration, applies graduated non-convexity with log-sum-exp smoothing for handling non-smooth landscapes, and utilizes model-predictive path integral method for local solution refinement.

Result: Experimental evaluation shows that Global-MPPI uncovers high-quality solutions with faster convergence and lower final costs in high-dimensional, long-horizon tasks.

Conclusion: The approach effectively overcomes the limitations of existing baseline methods in contact-rich manipulation applications, indicating its robust performance.
...
### [197] [Real-Time GPU-Accelerated Monte Carlo Evaluation of Safety-Critical AEB Systems Under Uncertainty](https://arxiv.org/abs/2604.27193)
*Akshay Karjol,Shadi Alawneh*

Main category: cs.RO

TL;DR: The paper introduces a GPU-accelerated Monte Carlo framework for evaluating Automatic Emergency Braking (AEB) systems, addressing the limitations of deterministic thresholding by incorporating uncertainty in vehicle dynamics and road conditions.


<details>
  <summary>Details</summary>
Motivation: The lack of robust evaluation methods for AEB systems that account for various uncertainties in real-world conditions complicates compliance with safety standards set by the NHTSA.

Method: A GPU-accelerated Monte Carlo framework utilizes a longitudinal vehicle model to perform stochastic evaluations of emergency braking performance, employing a one-thread-per-sample strategy and deterministic CPU sampling for consistency.

Result: Achieved up to 54.57x speedups across four different hardware platforms while maintaining numerical consistency, enabling the execution of approximately 25,000 samples within a 530 ms real-time budget on the Jetson AGX Orin.

Conclusion: This framework positions Monte Carlo-based uncertainty evaluation as a viable real-time component for AEB systems, aiding in the selection of risk-aware thresholds in compliance with NHTSA regulations.
...
### [198] [Learning Tactile-Aware Quadrupedal Loco-Manipulation Policies](https://arxiv.org/abs/2604.27224)
*Pokuang Zhou,Yuhao Zhou,Quan Luu,Seungho Han,Heng Zhang,Binghao Huang,Yunzhu Li,Arash Ajoudani,Zhengtong Xu,Yu She*

Main category: cs.RO

TL;DR: This paper presents a hierarchical pipeline for tactile-aware loco-manipulation in quadrupedal robots, enhancing performance in contact-rich tasks through a combination of human demonstrations and reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of reliable manipulation in dynamic environments, particularly under contact-rich scenarios where traditional visual and proprioceptive methods fall short.

Method: The authors developed a two-component approach: 1) a high-level policy trained on human demonstrations that integrates tactile feedback, and 2) a reinforcement learning framework in simulation for a whole-body control policy that operates under tactile awareness and transfers to the real world.

Result: The proposed method demonstrated an average performance improvement of 28.54% in real-world contact-rich manipulation tasks compared to vision-only and visuotactile baseline methods.

Conclusion: This work showcases the potential of integrating tactile sensing with loco-manipulation strategies, significantly enhancing performance in complex manipulation scenarios.
...
### [199] [DOT-Sim: Differentiable Optical Tactile Simulation with Precise Real-to-Sim Physical Calibration](https://arxiv.org/abs/2604.27367)
*Yang You,Won Kyung Do,Aiden Swann,Rika Antonova,Monroe Kennedy,Leonidas Guibas*

Main category: cs.RO

TL;DR: DOT-Sim is a new simulation framework for optical tactile sensors, accurately modeling their physical behavior using the Material Point Method, allowing for quick calibration and handling complex deformations, while achieving high accuracy in real-world applications.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of existing simulators that use simplified models, and to improve the realism and efficiency of optical tactile sensor simulations.

Method: Utilized the Material Point Method (MPM) to model sensors as elastic materials and developed a novel method for simulating optical responses through residual image learning.

Result: DOT-Sim provides rapid calibration, supports large deformations, accurately replicates sensor dynamics, generates realistic optical outputs, and achieves high classification accuracy in real-world tasks.

Conclusion: The framework demonstrates significant improvements in simulation fidelity and real-world applicability for optical tactile sensor technologies.
...
### [200] [An Experimental Modular Instrument With a Haptic Feedback Framework for Robotic Surgery Training](https://arxiv.org/abs/2604.27385)
*Walid Shaker,Mustafa Suphi Erden*

Main category: cs.RO

TL;DR: The paper introduces a modular robotic laparoscopic instrument with real-time haptic feedback to enhance surgical training, showing improved performance in force regulation tasks compared to visual-only feedback.


<details>
  <summary>Details</summary>
Motivation: To address the lack of haptic feedback in robotic-assisted surgery, which increases the risk of damaging tissue due to excessive interaction forces and limits training accessibility due to high costs of existing systems.

Method: A wrist-mounted force/torque sensor estimates tool-tissue interaction forces, integrated into a haptic feedback framework within the RoboScope training system, evaluated through a user study focused on force regulation tasks.

Result: Haptic feedback enhanced the success rate, force regulation accuracy, and task efficiency in surgical tasks, outperforming traditional visual-only feedback methods.

Conclusion: The instrument provides stable and high-fidelity haptic interaction, facilitating effective robotic surgery training and potentially improving surgical outcomes.
...
### [201] [RAY-TOLD: Ray-Based Latent Dynamics for Dense Dynamic Obstacle Avoidance with TDMPC](https://arxiv.org/abs/2604.27450)
*Seungho Han,Seokju Lee,Jeonguk Kang*

Main category: cs.RO

TL;DR: This paper introduces RAY-TOLD, a hybrid control architecture that improves autonomous robot navigation in dense crowds by integrating latent dynamics with obstacle information, enhancing planning beyond local minima.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of purely reactive planning methods like MPPI in escaping local minima during navigation in dense and dynamic crowds.

Method: RAY-TOLD combines a LiDAR-based latent dynamics model with reinforcement learning, utilizing a policy mixture sampling strategy to guide motion planning while ensuring kinematic feasibility.

Result: The proposed method significantly outperformed the MPPI baseline in extensive testing, resulting in a reduced collision rate in high-density dynamic environments.

Conclusion: RAY-TOLD enhances navigation reliability and safety by successfully blending short-horizon physics rollouts with learned long-horizon planning.
...
### [202] [SASI: Leveraging Sub-Action Semantics for Robust Early Action Recognition in Human-Robot Interaction](https://arxiv.org/abs/2604.27508)
*Yongpeng Cao,Masahiro Hirano,Hyuno Kim,Yuji Yamakawa*

Main category: cs.RO

TL;DR: SASI is a new framework for early recognition of human actions in human-robot interaction by integrating sub-action semantics and spatiotemporal features.


<details>
  <summary>Details</summary>
Motivation: Advancing behavior analysis in HRI requires robots to recognize human actions quickly from incomplete observations, which is often challenged by conventional approaches that focus mainly on holistic actions.

Method: SASI employs a segmentation model combined with a skeleton-based graph convolution network to fuse spatiotemporal features with sub-action semantics in real-time (29 Hz).

Result: Experiments on the BABEL dataset show SASI improves recognition accuracy over traditional methods, particularly in understanding partial action sequences for early recognition.

Conclusion: SASI's integration of sub-action semantics enhances proactive Human-Robot Interaction, indicating potential for further improvements with better sub-action segmentation.
...
### [203] [Function-based Parametric Co-Design Optimization of Dexterous Hands](https://arxiv.org/abs/2604.27557)
*Mohammad Amin Mirzaee,Harsh Gupta,Wenzhen Yuan*

Main category: cs.RO

TL;DR: A framework for robotic hand design that integrates various parameters for improved optimization and functionality is introduced and validated.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current robotic hand designs which are not optimized for task-driven performance and are restricted in the scope of design parameters.

Method: A comprehensive parametric framework that combines palm structure, finger kinematics, fingertip geometry, and surface curvatures, utilizing parametric surface deformation kernels for enhanced contact interactions.

Result: The framework was validated through simulations and real-world tests focusing on grasp stability, yielding ready-to-fabricate hand models.

Conclusion: The open-source framework supports rapid iteration in dexterous hand design and facilitates further research in control and policy training.
...
### [204] [Robot Learning from Human Videos: A Survey](https://arxiv.org/abs/2604.27621)
*Junyi Ma,Erhang Zhang,Haoran Yang,Ditao Li,Chenyang Xu,Guangming Wang,Hesheng Wang*

Main category: cs.RO

TL;DR: This paper surveys the emerging field of human-video-based learning techniques in robotics, highlighting methods for skill transfer from human demonstrations and analyzing data foundations and challenges.


<details>
  <summary>Details</summary>
Motivation: The need for scalable robot data in embodied AI and robotics, leveraging human activity videos to enhance learning efficiency and capabilities in robots.

Method: The survey reviews policy learning foundations, describes interfaces for incorporating human videos, and categorizes the transfer mechanisms through a hierarchical taxonomy.

Result: The paper presents a comprehensive analysis of existing techniques, datasets, and their effectiveness in enabling robot skill acquisition from human video demonstrations.

Conclusion: It identifies current challenges in the field and suggests future research directions to advance the integration of human video data in robotics.
...
### [205] [Can Tabular Foundation Models Guide Exploration in Robot Policy Learning?](https://arxiv.org/abs/2604.27667)
*Buqing Ou,Frederike Dümbgen*

Main category: cs.RO

TL;DR: TFM-S3 enhances policy optimization in robotics by balancing local updates with global exploration, showing superior performance with limited rollout costs.


<details>
  <summary>Details</summary>
Motivation: To improve upon traditional local methods in high-dimensional continuous control by integrating global exploration with reduced initialization requirements and rollout costs.

Method: TFM-S3 employs a hybrid approach, combining local policy updates with global search rounds, using SVD to construct a low-dimensional policy subspace and a pretrained model for efficient candidate screening.

Result: Experimental results indicate that TFM-S3 accelerates convergence and outperforms TD3 and population-based baselines under the same rollout budget.

Conclusion: The integration of foundation models into policy learning significantly enhances sample efficiency for continuous control tasks in robotics.
...
### [206] [ExoActor: Exocentric Video Generation as Generalizable Interactive Humanoid Control](https://arxiv.org/abs/2604.27711)
*Yanghao Zhou,Jingyu Ma,Yibo Peng,Zhenguo Sun,Yu Bai,Börje F. Karlsson*

Main category: cs.RO

TL;DR: ExoActor is a novel framework that utilizes large-scale video generation to model humanoid robot interactions within complex environments, improving task execution fidelity.


<details>
  <summary>Details</summary>
Motivation: To model fluent interaction-rich behavior between robots and their environments, addressing challenges in spatial context and task intent.

Method: ExoActor employs third-person video generation to synthesize execution processes based on task instructions and scene context, converting video outputs into humanoid behaviors through a general motion controller.

Result: ExoActor demonstrates the ability to generalize to new scenarios without additional data collection, validating its effectiveness as an end-to-end system.

Conclusion: While ExoActor shows promise in modeling complex humanoid behaviors, there are limitations in the current implementation, and future research directions are suggested to enhance the approach.
...
### [207] [Flying by Inference: Active Inference World Models for Adaptive UAV Swarms](https://arxiv.org/abs/2604.27935)
*Kaleem Arshid,Ali Krayani,Lucio Marcenaro,David Martin Gomez,Carlo Regazzoni*

Main category: cs.RO

TL;DR: This paper introduces an adaptive UAV swarm trajectory planning method using a hierarchical probabilistic inference framework based on expert guidance.


<details>
  <summary>Details</summary>
Motivation: To enhance UAV swarm trajectory planning by integrating expert knowledge into a structured inference system, improving adaptability and collision avoidance.

Method: The approach transforms UAV trajectory design into a hierarchical probabilistic inference problem, employing a genetic algorithm for offline expert demonstration generation and Bayesian state estimators for real-time corrections.

Result: Simulation results indicate that the framework maintains expert-like planning while achieving smoother, more stable UAV behaviors compared to modified Q-learning, and real-flight data validates its robustness under uncertainty.

Conclusion: The proposed method demonstrates effective adaptive UAV swarm autonomy by allowing real-time adjustments to trajectories based on learned models and expert-derived insights.
...
### [208] [Connected Dependability Cage: Run-Time Function and Anomaly Monitoring for the Development and Operation of Safe Automated Vehicles](https://arxiv.org/abs/2604.27728)
*Iqra Aslam,Nour Habib,Abhishek Buragohain,Meng Zhang,Andreas Rausch,Vaibhav Tiwari,Mohamed Benchat*

Main category: cs.RO

TL;DR: The paper presents the Connected Dependability Cage framework to enhance safety in automated vehicles using AI-enabled perception systems, focusing on fail-operational capabilities and monitoring mechanisms.


<details>
  <summary>Details</summary>
Motivation: To address complex safety challenges in automated vehicles operating in unpredictable environments, while ensuring compliance with safety standards and handling system failures effectively.

Method: The proposed Connected Dependability Cage framework integrates a Function Monitor with a voting mechanism for AI perception pipelines and an Anomaly Monitor for detecting unknown objects, supporting graceful degradation and minimal-risk maneuvers during discrepancies.

Result: The integration and validation of both monitors through extensive vehicle testing demonstrate their effectiveness in ensuring safe operation under varying conditions.

Conclusion: The Connected Dependability Cage framework significantly improves the reliability of AI perception systems in automated vehicles, facilitating safer transitions in critical situations and promoting continuous system improvement.
...
### [209] [MotuBrain: An Advanced World Action Model for Robot Control](https://arxiv.org/abs/2604.27792)
...
TL;DR: MotuBrain is a unified multimodal generative model for Vision-Language-Action tasks, enhancing world dynamics modeling and inference speed.


<details>
  <summary>Details</summary>
Motivation: To address the gap in fine-grained modeling of world dynamics in VLA models and improve real-world applicability for multimodal data.

Method: MotuBrain utilizes a UniDiffuser formulation with a three-stream Mixture-of-Transformers architecture to model video and action jointly.

Result: The model supports diverse inference modes and achieves over 50x speedup for real-time deployment, enhancing efficiency in real-world applications.

Conclusion: MotuBrain advances the integration of visual dynamics and actions, providing a robust framework for multimodal generative tasks.
...
### [210] [Learning-Based Hierarchical Scene Graph Matching for Robot Localization Leveraging Prior Maps](https://arxiv.org/abs/2604.27821)
*Nimrod Millenium Ndulue,Jose Andres Millan-Romera,Matteo Giorgi,Holger Voos,Jose Luis Sanchez-Lopez*

Main category: cs.RO

TL;DR: This paper presents a novel, learned method for matching scene graphs in indoor environments to improve localization in autonomous robots, outperforming traditional methods in speed and accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of accurate localization for autonomous robots in indoor settings using scene graphs, which encode environmental structure and relationships.

Method: A differentiable pipeline is introduced that augments scene graphs with semantically motivated edge types, allowing for hierarchical matching from high-level concepts to low-level details.

Result: The proposed method shows improved performance in F1 scores compared to combinatorial baselines in real-world LiDAR environments and operates significantly faster.

Conclusion: The method demonstrates successful zero-shot generalization for BIM-assisted localization, confirming its viability in practical applications.
...
### [211] [Dreaming Across Towns: Semantic Rollout and Town-Adversarial Regularization for Zero-Shot Held-Out-Town Fixed-Route Driving in CARLA](https://arxiv.org/abs/2604.27994)
*Feeza Khan Khanzada,Jaerock Kwon*

Main category: cs.RO

TL;DR: The paper investigates the zero-shot transfer abilities of a driving agent in new environments within the CARLA simulator, proposing methods that enhance its performance on unseen routes.


<details>
  <summary>Details</summary>
Motivation: To address the degradation of learned driving agents when encountering unfamiliar environments, focusing on structural town shifts in a controlled setting.

Method: A Dreamer-style latent world-model agent is enhanced with auxiliary losses involving predictions of visual-semantic embeddings and town-adversarial supervision, without direct navigation inputs.

Result: The model outperformed other Dreamer-family methods in mean success rates across held-out towns, with mixed results in safety and lane-keeping metrics.

Conclusion: The findings indicate that supervised semantic rollout and adversarial regularization can improve route completion in fixed-weather conditions.
...
### [212] [GSDrive: Reinforcing Driving Policies by Multi-mode Trajectory Probing with 3D Gaussian Splatting Environment](https://arxiv.org/abs/2604.28111)
*Ziang Guo,Min Chen,Xuefeng Zhang,Yixiao Zhou,Zufeng Zhang,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: GSDrive is a new framework for autonomous driving that combines imitation and reinforcement learning, using 3D Gaussian Splatting to improve reward shaping and policy training.


<details>
  <summary>Details</summary>
Motivation: The need to address high annotation costs and quality degradation in E2E autonomous driving while overcoming the limitations of conventional RL training that learns from catastrophic outcomes.

Method: GSDrive employs 3D Gaussian Splatting for differentiable reward shaping and integrates a flow matching-based trajectory predictor for multi-mode trajectory probing.

Result: GSDrive outperforms current simulation-based RL approaches in closed-loop driving tests on the nuScenes dataset.

Conclusion: The proposed framework enhances driving policy improvement by providing immediate dense feedback and establishing a knowledge exchange between IL and RL.
...
### [213] [FreeOcc: Training-Free Embodied Open-Vocabulary Occupancy Prediction](https://arxiv.org/abs/2604.28115)
*Zeyu Jiang,Changqing Zhou,Xingxing Zuo,Changhao Chen*

Main category: cs.RO

TL;DR: FreeOcc is a training-free framework for occupancy prediction using monocular or RGB-D sequences, achieving significant performance improvements without requiring extensive annotations or learning stages.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address limitations of existing occupancy prediction methods that depend on extensive 3D annotations and struggle to generalize across different environments.

Method: FreeOcc utilizes a four-layer pipeline: a SLAM backbone for pose estimation and sparse geometry, a Gaussian update for dense 3D maps, semantic associations from vision-language models, and a Gaussian-to-occupancy projection for dense voxel occupancy.

Result: FreeOcc demonstrates over $2	imes$ improvements in IoU and mIoU metrics on the EmbodiedOcc-ScanNet dataset compared to previous self-supervised approaches.

Conclusion: FreeOcc effectively operates without traditional training or pose requirements, achieving zero-shot transfer to novel environments and outperforming both supervised and self-supervised baselines.
...
### [214] [Design and Characteristics of a Thin-Film ThermoMesh for the Efficient Embedded Sensing of a Spatio-Temporally Sparse Heat Source](https://arxiv.org/abs/2604.28148)
*Sajjad Boorghan Farahan,Ahmed Alajlouni,Jingzhou Zhao*

Main category: cs.RO

TL;DR: ThermoMesh is a novel passive thermoelectric sensor for detecting sparse heat sources through thermal imaging, offering improved sensitivity and accuracy with advanced interlayer materials.


<details>
  <summary>Details</summary>
Motivation: To develop an effective thermal sensor capable of detecting and characterizing heat sources in challenging environments via conduction-based thermal imaging.

Method: The ThermoMesh integrates thermoelectric junctions with resistive interlayers and performs simulations to assess performance metrics under various configurations.

Result: Numerical simulations indicate significant improvements in sensitivity and accuracy with the use of nonlinear interlayers, surpassing linear models in performance.

Conclusion: ThermoMesh shows promise for energy-efficient thermal sensing in applications where traditional infrared systems may fall short.
...
### [215] [FlexiTac: A Low-Cost, Open-Source, Scalable Tactile Sensing Solution for Robotic Systems](https://arxiv.org/abs/2604.28156)
*Binghao Huang,Yunzhu Li*

Main category: cs.RO

TL;DR: FlexiTac is an innovative, cost-effective tactile sensing module for robotic end-effectors that enhances data collection and control with its flexible sensor pads and efficient readout board.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for affordable and scalable tactile sensing solutions in robotics to facilitate enhanced manipulation and interaction capabilities.

Method: FlexiTac utilizes a three-layer laminate sensor design with integrated electrodes, coupled with a multi-channel readout board that communicates tactile data to a computer at 100 Hz.

Result: FlexiTac demonstrates adaptability across various configurations and platforms, enabling real-time control and integration with modern tactile learning processes.

Conclusion: FlexiTac offers a practical and versatile solution for advancing tactile sensing in robotics, supporting a range of applications from simple tasks to complex interactions.
...
### [216] [RopeDreamer: A Kinematic Recurrent State Space Model for Dynamics of Flexible Deformable Linear Objects](https://arxiv.org/abs/2604.28161)
*Tim Missal,Lucas Domingues,Berk Guler,Simon Manschitz,Jan Peters,Paula Dornhofer Paro Costa*

Main category: cs.RO

TL;DR: This paper presents a novel framework for robotic manipulation of Deformable Linear Objects, addressing challenges in dynamics modeling and topological integrity with a focus on long-term forecasting and physical constraints.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations in existing data-driven methods for modeling Deformable Linear Objects that struggle with non-physical behaviors during manipulation tasks.

Method: The paper introduces a latent dynamics framework that utilizes a Recurrent State Space Model combined with a Quaternionic Kinematic Chain representation, enforcing physical validity and link-length constancy through the use of quaternions.

Result: The proposed approach reduces open-loop prediction error by 40.52% over 50-step horizons compared to state-of-the-art methods, while also achieving 31.17% faster inference times and improved topological consistency in complex scenarios.

Conclusion: The model provides a robust solution for long-horizon manipulation planning of Deformable Linear Objects, maintaining physical validity and enhancing manipulation efficacy in challenging tasks.
...
TL;DR: LaST-R1 is a novel Vision-Language-Action framework enhancing robotic manipulation through integrated latent reasoning and reinforcement learning, achieving high success rates and improved adaptability.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing VLA models in static imitation learning and enhance adaptability through effective reasoning mechanisms in robotic manipulation.

Method: LaST-R1 combines latent Chain-of-Thought reasoning with a tailored RL algorithm, Latent-to-Action Policy Optimization (LAPO), optimizing both latent reasoning and action generation.

Result: Achieved a 99.8% average success rate on the LIBERO benchmark and up to a 44% improvement in real-world deployments over initial policies in various complex tasks.

Conclusion: LaST-R1 shows enhanced robustness, adaptability, and generalization in robotic manipulation across simulated and real-world environments.
...
TL;DR: OmniRobotHome introduces the first room-scale residential platform for multiadic human-robot collaboration, addressing challenges in real-time 3D tracking with a comprehensive sensor array and robotic actuation.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current research on human-robot collaboration in multiadic settings characterized by concurrent interactions in shared spaces.

Method: Utilizing 48 synchronized RGB cameras for tracking and integrating two Franka robotic arms for coordinated actions in a natural home environment.

Result: Achieved reliable, occlusion-robust perception and improved human behavior modeling, leading to enhanced safety and robotic assistance in shared human-robot environments.

Conclusion: The OmniRobotHome platform facilitates extensive exploration of multiadic collaboration, showing improvements in operational safety and anticipatory assistance through advanced perception and behavior modeling.
...
### [219] [Monitoring Neural Training with Topology: A Footprint-Predictable Collapse Index](https://arxiv.org/abs/2604.26984)
*Alexander Kalinowski*

Main category: cs.LG

TL;DR: The paper introduces a monitoring system for neural representation collapse to enhance performance during training.


<details>
  <summary>Details</summary>
Motivation: To address representational collapse in neural networks, which negatively impacts performance before traditional metrics reflect issues.

Method: The method combines Modular Morse Homology Maintenance (MMHM) with a Collapse Index (CI) to provide real-time monitoring and adjustments without full complex rebuilding.

Result: The proposed CI offers a low-latency warning for intervening during training phases in tasks like LLM fine-tuning and KGE training.

Conclusion: The developed system enables timely interventions to prevent performance degradation in neural networks.
...
### [220] [Simple Self-Conditioning Adaptation for Masked Diffusion Models](https://arxiv.org/abs/2604.26985)
*Michael Cardei,Huu Binh Ta,Ferdinando Fioretto*

Main category: cs.LG

TL;DR: The paper introduces Self-Conditioned Masked Diffusion Models (SCMDM) that enhance masked diffusion models by refining predictions using previous clean states without additional training or architecture changes, demonstrating significant performance improvements across various domains.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of standard masked diffusion models that discard clean-state predictions for masked tokens, hindering cross-step refinement.

Method: The paper proposes SCMDM, which conditions each denoising step on the model's previous clean-state predictions, requiring minimal architectural changes and without introducing extra evaluations during sampling.

Result: SCMDM shows nearly 50% reduction in generative perplexity on trained models and improvements in image synthesis quality, molecular generation, and genomic modeling fidelity.

Conclusion: The proposed method outperforms vanilla MDM baselines, demonstrating that refinement from self-generated estimates is more effective than mixing conditional and unconditional objectives.
...
### [221] [People-Centred Medical Image Analysis](https://arxiv.org/abs/2604.26991)
...
TL;DR: The paper introduces PecMan, a framework for integrating fairness and diagnostic accuracy in medical AI systems, addressing challenges related to workflow integration and clinician workload.


<details>
  <summary>Details</summary>
Motivation: The limited adoption of medical AI tools is attributed to performance biases and poor workflow integration, necessitating a focus on fairness and clinician collaboration.

Method: PecMan employs a dynamic gating mechanism to allocate cases among AI, clinicians, or both, optimizing for fairness, accuracy, and workflow under clinician workload constraints.

Result: Experiments demonstrate that PecMan outperforms existing methods in balancing accuracy, fairness, and clinician workload, indicating a more viable approach for clinical implementation.

Conclusion: PecMan provides a promising solution to enhance the acceptance and effectiveness of AI in healthcare by addressing key interrelated challenges.
...
### [222] [When Continual Learning Moves to Memory: A Study of Experience Reuse in LLM Agents](https://arxiv.org/abs/2604.27003)
*Qisheng Hu,Quanyu Long,Wenya Wang*

Main category: cs.LG

TL;DR: Memory-augmented LLM agents face a continual learning challenge that shifts from model parameters to memory access, revealing interaction complexities between old and new experiences.


<details>
  <summary>Details</summary>
Motivation: To explore how memory-augmented LLMs tackle continual learning and the inherent challenges related to memory access.

Method: Introduced a (k,v) framework to analyze memory representation and organization for retrieval during sequential-task experiments in ALFWorld and BabyAI.

Result: Found that abstract procedural memories provide better transfer than detailed trajectories, with negative transfer impacting challenging cases more significantly.

Conclusion: External memory does not eliminate the continual-learning problem but transforms it into issues concerning memory representation and retrieval strategies.
...
### [223] [Automatic Causal Fairness Analysis with LLM-Generated Reporting](https://arxiv.org/abs/2604.27011)
*Alessia Berarducci,Eric Rossetto,Alessandro Antonucci,Marco Zaffalon*

Main category: cs.LG

TL;DR: Introducing FairMind, a software that automates fairness analysis in datasets for AutoML, leveraging causal effects and counterfactual queries.


<details>
  <summary>Details</summary>
Motivation: To address fairness issues in AutoML frameworks which often overlook training data and prediction fairness.

Method: Using the standard fairness model to evaluate fairness through causal effects and counterfactual queries, along with data preprocessing and closed-form computations.

Result: Demonstrated effectiveness in generating fairness reports in a zero-shot setup, with improved accuracy over direct LLM analysis.

Conclusion: FairMind offers significant advantages in fairness evaluation for AutoML, supporting applications with additional features for variable types.
...
### [224] [NORACL: Neurogenesis for Oracle-free Resource-Adaptive Continual Learning](https://arxiv.org/abs/2604.27031)
*Karthik Charan Raghunathan,Christian Metzner,Laura Kriener,Melika Payvand*

Main category: cs.LG

TL;DR: The paper introduces NORACL, a continual learning model addressing the stability-plasticity dilemma through neuronal growth, allowing adaptive network expansion in response to task demands.


<details>
  <summary>Details</summary>
Motivation: To tackle the architectural limitations that hinder continual learning, specifically the stability-plasticity trade-off, where models must balance learning new tasks and retaining prior knowledge.

Method: NORACL initiates with a compact network and grows only when necessary, monitoring signals for representational and plasticity saturation to determine when to expand the network architecture.

Result: NORACL outperforms or matches the accuracy of oracle-sized static baselines while using fewer parameters across various task configurations, demonstrating effective resource utilization.

Conclusion: The study shows that adaptive growth mechanisms in NORACL enhance continual learning by creating new capacity for incoming tasks, thereby improving this balance and advancing the stability-plasticity frontier.
...
### [225] [Fidelity, Diversity, and Privacy: A Multi-Dimensional LLM Evaluation for Clinical Data Augmentation](https://arxiv.org/abs/2604.27014)
*Guillermo Iglesias,Gema Bello-Orgaz,María Navas-Loro,Cristian Ramirez-Atencia,Mercè Salvador Robert,Enrique Baca-Garcia*

Main category: cs.LG

TL;DR: The paper addresses the challenge of limited annotated medical data in mental health by proposing a method for generating synthetic evaluation reports using Large Language Models while ensuring privacy.


<details>
  <summary>Details</summary>
Motivation: To overcome the scarcity of high-quality annotated medical data in mental health due to privacy regulations.

Method: The proposed methodology, DeepSeek-R1, utilizes DeepSeek-R1, OpenBioLLM-Llama3, and Qwen 3.5 to generate synthetic mental health evaluation reports based on specific ICD-10 codes, supported by an evaluation framework for generated texts.

Result: The models successfully generate clinically coherent, diverse, and privacy-safe synthetic reports, evaluated against semantic fidelity, lexical diversity, and privacy/plagiarism metrics.

Conclusion: The study shows that synthetic data generation can significantly expand training resources for clinical NLP tasks while preserving patient confidentiality.
...
### [226] [Learning to Forget: Continual Learning with Adaptive Weight Decay](https://arxiv.org/abs/2604.27063)
*Aditya A. Ramesh,Alex Lewandowski,Jürgen Schmidhuber*

Main category: cs.LG

TL;DR: FADE is a method that adapts weight decay rates for continual learning agents to balance knowledge retention and acquisition, improving performance compared to fixed decay rates.


<details>
  <summary>Details</summary>
Motivation: To effectively manage knowledge retention and forgetting in continual learning agents, allowing them to learn new information without losing essential prior knowledge.

Method: The FADE approach adapts weight decay rates on a per-parameter basis using approximate meta-gradient descent, implemented for online learning scenarios.

Result: Empirical results demonstrate FADE's ability to discover optimal decay rates for different parameters, enhancing performance over fixed decay methods in various learning tasks.

Conclusion: FADE provides a dynamic mechanism for forgetting, improving continual learning efficiency and adaptability in neural networks.
...
### [227] [Dynamic Adversarial Fine-Tuning Reorganizes Refusal Geometry](https://arxiv.org/abs/2604.27019)
*Wenhao Lan,Shan Li,Junbin Yang,Haihua Shen,Yijun Yang*

Main category: cs.LG

TL;DR: The study investigates the mechanisms behind safety-aligned language models' refusal capabilities by comparing supervised fine-tuning (SFT) and R2D2-style dynamic adversarial fine-tuning, revealing differences in performance and carrier dynamics.


<details>
  <summary>Details</summary>
Motivation: To clarify how dynamic adversarial fine-tuning influences refusal mechanisms in language models without inducing overly broad refusals.

Method: Measurement-driven study examining a 7B backbone through supervised fine-tuning and R2D2 adversarial fine-tuning, aligned with various refusal benchmarks and causal interventions.

Result: R2D2 fine-tuning significantly improves refusal robustness against harmful requests, with varied performance across training steps, while SFT shows less effective robustness.

Conclusion: The findings favor a reorganization model of refusal dynamics over a purely drift-based approach, though results are limited to one model variant and specific attack types.
...
TL;DR: This study investigates the effects of network architecture, task similarity, and representational dimensionality on continual learning, focusing on the stability-plasticity dilemma.


<details>
  <summary>Details</summary>
Motivation: Understanding the balance between plasticity and stability is essential for efficient continual learning systems, especially to mitigate interference in representation reuse across tasks.

Method: A comparative analysis was conducted using a task-partitioned modular recurrent network against a single-module baseline, manipulating task similarity and weight initialization scale.

Result: The architecture has minimal effects in high-dimensional representation regimes, while in low-dimensional regimes, modular architecture significantly enhances task-specific representation alignment and separation.

Conclusion: Representational dimensionality is crucial in determining the relevance of structural separation in continual learning, suggesting that adopting adaptive geometry could improve system design.
...
### [229] [Physical Foundation Models: Fixed hardware implementations of large-scale neural networks](https://arxiv.org/abs/2604.27911)
*Logan G Wright,Tianyu Wang,Tatsuhiro Onodera,Peter L. McMahon*

Main category: cs.LG

TL;DR: Foundation models present opportunities for new hardware designs, specifically Physical Foundation Models (PFMs), which could enhance efficiency and scalability in AI implementations.


<details>
  <summary>Details</summary>
Motivation: Foundation models consolidate diverse tasks into a single large model, prompting the need for specialized hardware for efficiency and performance.

Method: Advocating for PFMs that utilize natural physical dynamics for neural networks and presenting calculations based on optical materials.

Result: PFMs could significantly improve energy efficiency, speed, and parameter density, allowing for larger models and more applications in constrained environments.

Conclusion: The necessary research challenges for realizing trillion-parameter PFMs are highlighted.
...
### [230] [Cross-Subject Generalization for EEG Decoding: A Survey of Deep Learning Methods](https://arxiv.org/abs/2604.27033)
*Taida Li,Yujun Yan,Fei Dou,Wenzhan Song,Xiang Zhang*

Main category: cs.LG

TL;DR: This survey reviews deep learning methodologies for cross-subject EEG decoding, addressing inter-subject variability challenges.


<details>
  <summary>Details</summary>
Motivation: The need to overcome high inter-subject variability in EEG data which affects generalization in deep learning models.

Method: Formalizing the cross-subject setting as a multi-source domain problem and categorizing existing methodologies into families such as feature alignment, adversarial learning, feature disentanglement, and contrastive learning.

Result: A comprehensive taxonomy of methods for cross-subject EEG decoding and an evaluation framework for assessing methodologies.

Conclusion: Identifies key areas for advancing EEG decoding, including theoretical limitations, the role of subject identity, and the potential of EEG foundation models.
...
### [231] [Detecting Clinical Discrepancies in Health Coaching Agents: A Dual-Stream Memory and Reconciliation Architecture](https://arxiv.org/abs/2604.27045)
*Samuel L Pugh,Eric Yang,Alexander Muir Sutherland,Alessandra Breschi*

Main category: cs.LG

TL;DR: This paper presents a Dual-Stream Memory Architecture designed to enhance safety in Large Language Model (LLM) agents managing healthcare by reconciling patient self-reports with Electronic Health Records (EHR).


<details>
  <summary>Details</summary>
Motivation: The need to effectively manage conflicting information from patient self-reports and EHRs while ensuring safety in clinical settings.

Method: The authors developed a Dual-Stream Memory Architecture that separates patient narratives from structured clinical data, supported by a Reconciliation Engine to evaluate discrepancies between the two sources.

Result: In testing, the Reconciliation Engine identified 84.4% of designed clinical discrepancies and achieved 86.7% critical recall, revealing a 13.6% error cascade due to information loss during memory extraction.

Conclusion: The study concludes that validating patient memories against clinical records is essential for the safe deployment of longitudinal healthcare agents.
...
### [232] [Learning Rate Transfer in Normalized Transformers](https://arxiv.org/abs/2604.27077)
*Boris Shigida,Boris Hanin,Andrey Gromov*

Main category: cs.LG

TL;DR: The paper proposes $ν$GPT, an improved version of nGPT, which addresses the lack of learning rate transfer across model dimensions and token horizons by modifying hyperparameter transfer methods.


<details>
  <summary>Details</summary>
Motivation: To enhance learning rate transferability in transformer models, addressing limitations in nGPT's scaling with hyperparameters.

Method: Combines numerical experiments with the modification of the $μ$P approach to hyperparameter transfer, using alignment exponents for improvement.

Result: $ν$GPT exhibits effective learning rate transfer across width, depth, and token horizon, demonstrating improved training efficiency.

Conclusion: The proposed $ν$GPT represents a significant improvement in the scalability and training efficacy of transformer models.
...
### [233] [AutoSP: Unlocking Long-Context LLM Training Via Compiler-Based Sequence Parallelism](https://arxiv.org/abs/2604.27089)
*Ahan Gupta,Zhihao Wang,Neel Dani,Masahiro Tanaka,Olatunji Ruwase,Minjia Zhang*

Main category: cs.LG

TL;DR: AutoSP is an automated solution for optimizing long-context training in large language models (LLMs), improving trainability significantly without sacrificing throughput.


<details>
  <summary>Details</summary>
Motivation: Existing LLM training libraries lack user-friendly abstractions for optimizing long-context training, which hampers developer productivity and requires extensive expertise.

Method: AutoSP introduces automated sequence parallelism and long-context aware activation-checkpointing, compiling models to implement these optimizations effectively.

Result: AutoSP achieves up to 2.7x and 2.5x increase in training contexts on NVIDIA and AMD hardware, respectively, compared to competitive hand-written baselines, with minimal impact on runtime performance.

Conclusion: AutoSP significantly enhances LLM trainability for long-context tasks while maintaining throughput efficiency.
...
### [234] [Co-Evolving Policy Distillation](https://arxiv.org/abs/2604.27083)
*Naibin Gu,Chenxu Yang,Qingyi Si,Chuanyu Qin,Dingyu Yao,Peng Fu,Zheng Lin,Weiping Wang,Nan Duan,Jiaqi Wang*

Main category: cs.LG

TL;DR: CoPD unifies RLVR and OPD for better post-training integration of expert capabilities in a single model, overcoming drawbacks of each.


<details>
  <summary>Details</summary>
Motivation: To address capability loss in consolidating expert knowledge into a single model, which is common in traditional RLVR and OPD methods.

Method: Introduce Co-Pevolving Policy Distillation (CoPD), which facilitates concurrent training of multiple experts while incorporating bidirectional OPD during their ongoing RLVR training.

Result: CoPD demonstrates significant improvements in integrating text, image, and video reasoning capabilities compared to existing methods, such as mixed RLVR and MOPD.

Conclusion: CoPD's innovative training approach may lead to new paradigms in scaling model training effectively.
...
### [235] [Statistical Channel Fingerprint Construction for Massive MIMO: A Unified Tensor Learning Framework](https://arxiv.org/abs/2604.27574)
*Zhenzhou Jin,Li You,Xiang-Gen Xia,Xiqi Gao*

Main category: cs.LG

TL;DR: This paper introduces a novel method for reconstructing statistical channel fingerprints (sCF) in massive MIMO systems, utilizing a unified tensor learning architecture for improved reconstruction efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: To enhance the acquisition of channel state information (CSI) in massive MIMO systems, while addressing practical constraints such as measurement costs, privacy, and security.

Method: The authors propose a unified tensor-based learning architecture (LPWTNet) that uses a closed-form Laplacian pyramid decomposition and a small-kernel convolution mechanism for efficient inference and feature extraction from statistical CFs.

Result: The approach shows competitive reconstruction accuracy and computational efficiency compared to state-of-the-art methods across various scenarios for constructing sCFs.

Conclusion: The proposed method improves the reconstruction of sCFs while ensuring efficiency and addressing practical limitations in massive MIMO systems.
...
### [236] [Anomaly Detection in Soil Heavy Metal Contamination Using Unsupervised Learning for Environmental Risk Assessment](https://arxiv.org/abs/2604.27102)
...
TL;DR: The study examines heavy metal contamination in soils from urban Ghana using unsupervised machine learning to identify contamination patterns and assess health risks.


<details>
  <summary>Details</summary>
Motivation: To address public health concerns related to heavy metal soil contamination in Ghana's urbanizing areas, especially near unregulated waste sites.

Method: An unsupervised machine learning approach, including Isolation Forest, PCA reconstruction error, and DBSCAN, was applied to analyze heavy metal concentrations from soil samples and health risk indices.

Result: 12 anomalous samples were detected, with consensus anomalies showing higher health risk indices. PCA reconstruction error correlated strongly with health risk.

Conclusion: The study illustrates that unsupervised machine learning can provide detailed insights into contamination, facilitating targeted environmental management and risk prioritization.
...
### [237] [Better Models, Faster Training: Sigmoid Attention for single-cell Foundation Models](https://arxiv.org/abs/2604.27124)
*Vijay Sadashivaiah,Georgios Dasoulas,Judith Mueller,Soumya Ghosh*

Main category: cs.LG

TL;DR: Sigmoid attention outperforms softmax attention in training biological foundation models, offering better representations, faster training, and enhanced stability.


<details>
  <summary>Details</summary>
Motivation: To improve the training stability and representation quality of biological foundation models, exploring alternative attention mechanisms was necessary.

Method: Sigmoid attention is introduced as a drop-in replacement for softmax attention, with theoretical analysis and empirical validation across various datasets and model structures.

Result: Sigmoid attention shows 25% better cell-type separation, enhances cohesion, reduces validation loss by 10%, and remains stable during training as opposed to softmax, which diverges under stress tests.

Conclusion: Sigmoid attention is a theoretically and empirically superior choice for biological foundation models, leading to enhanced performance and stability.
...
### [238] [How to Guide Your Flow: Few-Step Alignment via Flow Map Reward Guidance](https://arxiv.org/abs/2604.27147)
*Jerry Y. Huang,Justin Lin,Sheel Shah,Kartik Nair,Nicholas M. Boffi*

Main category: cs.LG

TL;DR: The paper presents a new method, Flow Map Reward Guidance (FMRG), for improving generative modeling by efficiently guiding samples using optimal control principles, achieving superior results in various tasks with reduced computational effort.


<details>
  <summary>Details</summary>
Motivation: To enhance sample generation in generative modeling by maximizing user-specified rewards without the drawbacks of existing guidance methods, which can be computationally expensive or inadequate.

Method: The authors reformulate the guidance problem into a deterministic optimal control framework, developing a hierarchy of algorithms that includes existing methods. They introduce FMRG, leveraging the flow map for efficient integration and guidance without requiring prior training.

Result: FMRG demonstrates performance that matches or exceeds traditional baselines in multiple tasks such as inverse problems, style transfer, and alignment with human preferences, achieving results with only 3 Neural Function Evaluations (NFEs).

Conclusion: FMRG provides a significant speedup and effectiveness in generative modeling tasks, marking an advancement over previous state-of-the-art methods with its training-free and efficient single-trajectory approach.
...
### [239] [ConformaDecompose: Explaining Uncertainty via Calibration Localization](https://arxiv.org/abs/2604.27149)
...
TL;DR: This paper introduces an uncertainty-aware explainability framework for conformal prediction, aiming to clarify the sources of uncertainty at the instance level.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of standard conformal prediction which obscures the sources of uncertainty due to its reliance on a single global calibration threshold.

Method: The framework utilizes progressive calibration localisation to analyze how calibration-induced epistemic uncertainty can be reduced, specifically in regression tasks.

Result: It was found that absolute reducible uncertainty correlates with epistemic uncertainty proxies, and its relative significance varies across different tasks, uncovering aspects obscured by traditional interval width.

Conclusion: The proposed framework enhances the interpretability of conformal prediction without changing the predictive model or affecting its coverage guarantees.
...
### [240] [Generalizing the Geometry of Model Merging Through Frechet Averages](https://arxiv.org/abs/2604.27155)
*Marvin F. da Silva,Mohammed Adnan,Felix Dangel,Sageev Oore*

Main category: cs.LG

TL;DR: This paper introduces a symmetry-aware model merging technique using Fréchet averaging to combine multiple models while considering geometric factors.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to enhance model merging techniques by addressing the fragility of naive parameter averaging under architectural symmetries.

Method: The authors propose merging models through Fréchet averaging, which minimizes geodesic distances on an appropriate manifold, thus ensuring symmetry-invariance in the procedure.

Result: They demonstrate that this approach can encompass traditional merging methods like Fisher merging and outlines the distinct geometry of low-rank adapters (LoRA) as a quotient manifold.

Conclusion: The paper highlights limitations in existing LoRA merging methods and presents a new algorithm that outperforms conventional methods in practical applications.
...
### [241] [Distributional Alignment Games for Answer-Level Fine-Tuning](https://arxiv.org/abs/2604.27166)
*Mehryar Mohri,Jon Schneider,Yifan Wu*

Main category: cs.LG

TL;DR: The paper proposes a game-theoretical framework for Answer-Level Fine-Tuning (ALFT) of language models, which optimizes final answer correctness rather than reasoning paths.


<details>
  <summary>Details</summary>
Motivation: To address the computational challenges of optimizing language models based on the correctness of their answers, bypassing the complexities of reasoning traces.

Method: Formulates ALFT as a two-player game between a Policy and a Target, establishing a Nash Equilibrium that corresponds to the solution of the optimization problem.

Result: The proposed framework transforms the intractable marginalization into a tractable projection problem and unifies various approaches to diversity and coherence.

Conclusion: The new framework facilitates efficient algorithms like Coherence-GRPO, leading to improved performance in mathematical reasoning tasks.
...
### [242] [Context-Aware Graph Attention for Unsupervised Telco Anomaly Detection](https://arxiv.org/abs/2604.27172)
*Sara Malacarne,Eirik Hoel-Høiseth,Erlend Aune,David Zsolt Biro,Massimiliano Ruocco*

Main category: cs.LG

TL;DR: C-MTAD-GAT is an unsupervised, context-aware graph-attention model for detecting anomalies in multivariate time series from mobile networks, improving upon existing baselines, and successfully deployed in real-world settings.


<details>
  <summary>Details</summary>
Motivation: To enhance anomaly detection in mobile network multivariate time series using an unsupervised model that can effectively utilize graph attention and context embeddings.

Method: C-MTAD-GAT employs graph attention combined with context embeddings, a deterministic reconstruction head, and a multi-step forecaster to generate anomaly scores, with detection thresholds calibrated from validation residuals without requiring labels.

Result: C-MTAD-GAT outperforms MTAD-GAT and DC-VAE on the TELCO dataset in F1 scores while generating fewer false alarms.

Conclusion: C-MTAD-GAT demonstrates superior performance in anomaly detection and is effective in practical industrial applications.
...
### [243] [Preserving Temporal Dynamics in Time Series Generation](https://arxiv.org/abs/2604.27182)
*Ci Lin,Futong Li,Tet Yeap,Iluju Kiringa*

Main category: cs.LG

TL;DR: This paper presents a MCMC-based framework to improve synthetic time-series generation by preserving temporal dynamics and mitigating distribution shifts.


<details>
  <summary>Details</summary>
Motivation: The limited availability of time-series data hinders the performance of deep learning models in regression-oriented forecasting tasks, necessitating effective data augmentation techniques.

Method: The authors propose a model-agnostic Markov Chain Monte Carlo (MCMC) framework that addresses distribution shift and maintains temporal dynamics in synthetic time series, supported by theoretical analysis of conditional generative models.

Result: Experiments demonstrate improved performance across multiple metrics (e.g., autocorrelation alignment and predictive score) on various datasets when using the proposed MCMC approach compared to existing generative models.

Conclusion: Maintaining consistency with empirical transition statistics is essential for producing high-fidelity synthetic time series, indicating a shift from adversarial approaches to those that explicitly preserve temporal laws.
...
### [244] [Remaining Useful Life Estimation for Turbofan Engines: A Comparative Study of Classical, CNN, and LSTM Approaches](https://arxiv.org/abs/2604.27234)
*Astitva Goel,Samarth Galchar,Sumit Kanu*

Main category: cs.LG

TL;DR: This paper compares various machine learning methods for estimating Remaining Useful Life (RUL) using the NASA C-MAPSS turbofan engine dataset, highlighting the performance of LSTM and XGBoost.


<details>
  <summary>Details</summary>
Motivation: To improve RUL estimation in Prognostics and Health Management (PHM) through machine learning, thereby enhancing maintenance strategies and reducing unexpected failures.

Method: A comparative study utilizing classical baselines (Ridge Regression, Polynomial Ridge, XGBoost), a 1D CNN, and an LSTM network, all evaluated under a uniform preprocessing framework on specific dataset subsets.

Result: LSTM outperformed previous deep architectures with an RMSE of 14.93 and 14.20 on FD001 and FD003 respectively. XGBoost achieved an RMSE of 13.36 on FD003, demonstrating strong nonlinear modeling capabilities.

Conclusion: The study indicates that while LSTM is effective for raw sequence data, XGBoost shows great promise for RUL estimation, emphasizing the importance of model selection in predictive maintenance.
...
### [245] [Analytical Correction for Subsampling Bias in Drifting Models](https://arxiv.org/abs/2604.27239)
*Jiaru Zhang,Zeyun Deng,Juanwu Lu,Ziran Wang,Ruqi Zhang*

Main category: cs.LG

TL;DR: This paper presents Analytical Bias Correction (ABC) to address the bias in minibatch estimations of centroids in drifting models, improving bias reduction from $O(1/n)$ to $O(1/n^2)$ without increasing total variance.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the issue of bias in centroid estimations from minibatches in drifting models, which can negatively impact the performance of generative models.

Method: The authors propose Analytical Bias Correction (ABC), a technique that approximates the leading bias term using in-batch statistics to correct the biased estimator of centroids without full distribution expectations.

Result: ABC reduces the bias from $O(1/n)$ to $O(1/n^2)$, maintains total variance, and supports convex-hull containment. Empirical results show improvements in training speed and lower FID scores on CIFAR-10, particularly for small minibatches.

Conclusion: ABC is an effective and efficient bias correction method for drifting models, enhancing performance while being easy to implement.
...
### [246] [AutoREC: A software platform for developing reinforcement learning agents for equivalent circuit model generation from electrochemical impedance spectroscopy data](https://arxiv.org/abs/2604.27266)
...
TL;DR: AutoREC is an open-source Python package using reinforcement learning to automate equivalent circuit model generation from electrochemical impedance spectroscopy data, achieving over 99.6% success on synthetic datasets.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for scalable ECM generation in autonomous experimental setups, overcoming limitations of manual identification methods.

Method: AutoREC formulates ECM construction as a Markov Decision Process and employs a Double Deep Q-Network with prioritized experience replay, along with a strategy for dead-loop mitigation.

Result: The RL agent trained with AutoREC achieved a success rate exceeding 99.6% on synthetic data and effectively generalized to various unseen electrochemical systems.

Conclusion: AutoREC demonstrates significant potential for adaptive ECM generation and could be integrated into automated electrochemical workflows.
...
### [247] [BrainDINO: A Brain MRI Foundation Model for Generalizable Clinical Representation Learning](https://arxiv.org/abs/2604.27277)
*Yizhou Wu,Shansong Wang,Yuheng Li,Mojtaba Safari,Mingzhe Hu,Chih-Wei Chang,Harini Veeraraghavan,Xiaofeng Yang*

Main category: cs.LG

TL;DR: BrainDINO, a self-supervised model trained on 6.6 million unlabeled brain MRI slices, demonstrates the ability to generalize across diverse neuroimaging tasks without requiring extensive labeled data.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of task-specific learning methods that require substantial labeled data for brain MRI analyses.

Method: BrainDINO was trained using a self-distillation technique on unlabeled axial slices from 20 datasets, facilitating transfer to multiple neuroimaging tasks with a frozen encoder and lightweight task heads.

Result: BrainDINO matched or surpassed existing self-supervised baselines in various tasks, showing particularly strong performance in scenarios of label scarcity.

Conclusion: This work highlights the potential of large-scale self-supervised learning to create a unified MRI representation that is effective for various neuroimaging applications, reducing the need for volumetric pretraining or extensive fine-tuning.
...
### [248] [Predicting Covariate-Driven Spatial Deformation for Nonstationary Gaussian Processes](https://arxiv.org/abs/2604.27280)
*Minghao Gu,Weizhi Lin,Qiang Huang*

Main category: cs.LG

TL;DR: This paper enhances the modeling of nonstationary Gaussian processes by connecting spatial deformations to covariates, overcoming limitations of static methods for improved predictive capabilities.


<details>
  <summary>Details</summary>
Motivation: To address limitations in existing static spatial deformation methods that fail to adapt to covariate-induced changes in spatial correlation.

Method: Spatial deformations are modeled as functions of covariates using velocity fields in a Lie algebra, with estimation stability achieved through assumptions about covariate interactions.

Result: An efficient estimation-inference algorithm is presented, enabling out-of-sample nonstationary GP predictions with limited data.

Conclusion: The approach proves effective and generalizable, showcased through simulations and applications in manufacturing and geostatistics.
...
### [249] [PINN-Cast: Exploring the Role of Continuous-Depth NODE in Transformers and Physics Informed Loss as Soft Physical Constraints in Short-term Weather Forecasting](https://arxiv.org/abs/2604.27313)
*Hira Saleem,Flora Salim,Cormac Purcell*

Main category: cs.LG

TL;DR: This paper presents a continuous-depth transformer encoder for weather forecasting that utilizes Neural ODE dynamics and a physics-informed training objective, outperforming standard transformer methods.


<details>
  <summary>Details</summary>
Motivation: To enhance the accuracy of weather forecasting by integrating physical principles in data-driven models, addressing limitations in standard transformer architectures.

Method: A continuous-depth transformer encoder is proposed, replacing discrete updates with ODE-based updates and introducing a two-branch attention module that incorporates derivative operations with a physics-informed training objective.

Result: The proposed method significantly improves forecast accuracy over traditional discrete transformers and a continuous-time Neural ODE approach in short-term weather forecasting scenarios.

Conclusion: PINN-Cast demonstrates a promising integration of physics consistency in transformer-based weather forecasting, yielding better performance than existing models.
...
### [250] [A Short Note on Batch-efficient Divide-and-Conquer Algorithm for EigenDecomposition](https://arxiv.org/abs/2604.27325)
*Yue Song*

Main category: cs.LG

TL;DR: This paper presents a new batch-efficient Divide-and-Conquer based EigenDecomposition algorithm for larger matrices that improves computational efficiency over existing methods.


<details>
  <summary>Details</summary>
Motivation: The need for faster and more efficient EigenDecomposition in computer vision tasks, especially for batched matrices in deep learning applications.

Method: A Divide-and-Conquer based approach for EigenDecomposition tailored for larger matrices, building on previous QR-based methods for smaller matrices.

Result: The proposed method significantly accelerates EigenDecomposition computations for mini-batches of matrices with dimensions up to 64, outperforming the Pytorch SVD function.

Conclusion: This new approach demonstrates improved computational efficiency, making it viable for larger matrix decompositions in practical applications.
...
### [251] [AdaBFL: Multi-Layer Defensive Adaptive Aggregation for Bzantine-Robust Federated Learning](https://arxiv.org/abs/2604.27434)
*Zehui Tang,Yuchen Liu,Feihu Huang*

Main category: cs.LG

TL;DR: Proposal of AdaBFL, a multi-layer adaptive aggregation method for Byzantine-robust federated learning, addressing the limitations of existing defenses.


<details>
  <summary>Details</summary>
Motivation: To enhance the resilience of federated learning against various poisoning attacks from malicious clients.

Method: Introduces AdaBFL, a three-layer defensive mechanism that adaptively adjusts defense algorithm weights in response to different attack types, and establishes its convergence properties in non-convex, non-iid data conditions.

Result: Extensive experiments demonstrate that AdaBFL outperforms existing Byzantine-robust methods on multiple datasets.

Conclusion: AdaBFL effectively provides a balanced defense strategy against diverse attacks in federated learning.
...
### [252] [TypeBandit: Type-Level Context Allocation and Reweighting for Effective Attribute Completion in Heterogeneous Graph Neural Networks](https://arxiv.org/abs/2604.27356)
*Ta-Yang Wang,Rajgopal Kannan,Viktor Prasanna*

Main category: cs.LG

TL;DR: TypeBandit is a model-agnostic method for completing missing attributes in heterogeneous graphs, addressing varying signal utility across node types.


<details>
  <summary>Details</summary>
Motivation: Different node types in heterogeneous graphs provide differing levels of useful information for completing missing attributes, resulting in type-dependent information asymmetry.

Method: TypeBandit employs topology-aware initialization, type-level bandit sampling, and joint representation learning to utilize a finite global sampling budget for effective attribute completion.

Result: TypeBandit demonstrates significant dataset-dependent improvements in heterogeneous attribute completion across DBLP, IMDB, and ACM under a fixed-split protocol, and shows promising results in ablation and efficiency tests.

Conclusion: TypeBandit is a practical solution for heterogeneous attribute completion, especially when node types have uneven information distribution and limited sampling resources.
...
### [253] [Low Rank Adaptation for Adversarial Perturbation](https://arxiv.org/abs/2604.27487)
*Han Liu,Shanghao Shi,Yevgeniy Vorobeychik,Chongjie Zhang,Ning Zhang*

Main category: cs.LG

TL;DR: This paper explores the low-rank structure of adversarial perturbations and proposes a new method to enhance black-box adversarial attacks by leveraging this property, demonstrating significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: To investigate whether adversarial perturbations in machine learning models maintain a low-rank structure, similar to model updates in training, and to improve the efficiency of black-box adversarial attacks.

Method: The proposed method involves a two-step approach: first, using a reference model and auxiliary data to guide gradient projections into a low-dimensional subspace, and second, constraining the search for perturbations within this low-rank subspace.

Result: Empirical evaluations show notable improvements in the efficiency and effectiveness of low-rank adversarial attacks over traditional methods across various models, datasets, and attack types.

Conclusion: The findings suggest that leveraging the low-rank characteristic of adversarial perturbations can significantly enhance black-box attack performance, paving the way for future research in adversarial training and defense mechanisms.
...
### [254] [AG-TAL: Anatomically-Guided Topology-Aware Loss for Multiclass Segmentation of the Circle of Willis Using Large-Scale Multi-Center Datasets](https://arxiv.org/abs/2604.27357)
*Jialu Liu,Yue Cui,Shan Yu*

Main category: cs.LG

TL;DR: This study presents a novel Anatomically-Guided Topology-Aware Loss (AG-TAL) for enhancing multiclass segmentation of the Circle of Willis (CoW), addressing challenges like vascular discontinuities and inter-class misclassification. The method includes a new extensive dataset with unified annotations, leading to improved segmentation performance across various datasets, particularly for small arteries.


<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of the Circle of Willis is crucial for managing neurovascular diseases, but existing methods struggle with complexities in vascular topology and varying morphologies.

Method: The AG-TAL integrates a radius-aware Dice loss, a breakage-aware clDice loss, and an adjacency-aware co-occurrence loss to improve segmentation accuracy while managing class imbalances and preserving vascular connectivity.

Result: AG-TAL achieved an average Dice score of 80.85% across CoW arteries and improved scores for small arteries compared to state-of-the-art methods, with performance validated across six independent datasets.

Conclusion: The study validates AG-TAL's effectiveness for multiclass CoW artery segmentation, demonstrating enhanced performance and robustness, with implications for clinical applications in identifying morphological biomarkers.
...
### [255] [Stable but Wrong: An Inference Limit in Galactic Archaeology](https://arxiv.org/abs/2604.27368)
*Zhipeng Zhang*

Main category: cs.LG

TL;DR: Observational science assumes that larger samples yield reliable results; however, in Galactic archaeology, observational quality can bias age inferences, leading to incorrect conclusions despite low statistical uncertainties.


<details>
  <summary>Details</summary>
Motivation: To challenge the assumption that increased sample sizes and reduced uncertainties guarantee accurate inferences in astrophysical studies, particularly in constructing the Milky Way's formation history.

Method: An analysis of a large sample of subgiant stars focusing on observational quality parameters like signal-to-noise ratio and parallax precision.

Result: Identification of a specific region in observational quality space where inferred formation timescales are systematically offset by 0.5-1 Gyr compared to independent asteroseismic measurements, despite small statistical uncertainties.

Conclusion: The findings suggest that assumptions about observational quality must be revisited, as they can introduce systematic biases in age inference, potentially misleading historical interpretations in Galactic archaeology.
...
### [256] [Detecting is Easy, Adapting is Hard: Local Expert Growth for Visual Model-Based Reinforcement Learning under Distribution Shift](https://arxiv.org/abs/2604.27411)
*Haiyang Zhao*

Main category: cs.LG

TL;DR: Visual MBRL often fails under distribution shifts. Various corrective strategies were tested with limited success. The new JEPA-Indexed Local Expert Growth method showed significant out-of-distribution performance improvement while maintaining in-distribution performance.


<details>
  <summary>Details</summary>
Motivation: To address the challenges faced by visual MBRL agents when encountering environment shifts, particularly the need for effective action-level corrections.

Method: The study proposes JEPA-Indexed Local Expert Growth to utilize frozen JEPA representations for problem indexing, with cluster-specific residual experts providing local action corrections without modifying the baseline controller.

Result: The proposed method demonstrated statistically significant improvements in OOD performance across various testing conditions while preserving ID performance, unlike previous strategies which were ineffective or detrimental.

Conclusion: The key challenge for visual MBRL in shifting environments is not only to detect the change but to ensure appropriate local action corrections are made thereafter.
...
### [257] [ChipLingo: A Systematic Training Framework for Large Language Models in EDA](https://arxiv.org/abs/2604.27415)
*Lei Li,Xingwen Yu,Jianguo Ni,Junxuan Zhu,Jieqiong Zhang,Jian Zhao,Zhi Liu*

Main category: cs.LG

TL;DR: This paper introduces ChipLingo, a systematic training pipeline for domain-adapted LLMs tailored specifically for Electronic Design Automation (EDA), addressing challenges in applying LLMs in this field.


<details>
  <summary>Details</summary>
Motivation: To overcome challenges in applying LLMs to EDA due to limited expertise, knowledge confusion, and degraded performance issues post-domain training.

Method: ChipLingo employs a three-stage process: constructing a domain corpus with curated data and QA augmentation, domain-adaptive pretraining with various parameter strategies, and instruction alignment through RAG scenario training under varying conditions.

Result: ChipLingo-8B achieved 59.7% accuracy, outperforming base models and some larger models; ChipLingo-32B reached 70.02% accuracy, nearing top commercial models, with QA augmentation and partial fine-tuning showing improved performance.

Conclusion: ChipLingo's systematic approach enhances performance in EDA tasks, laying groundwork for future developments in EDA agents and knowledge-driven systems.
...
TL;DR: The paper introduces ABC, a novel approach for generating continuous-time stochastic processes conditioned on partial observations, addressing limitations of existing diffusion models.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of existing methods in generating stochastic processes conditioned on partial observations, including inability to properly handle temporal dynamics and arbitrary state conditioning.

Method: The paper proposes ABC, which employs a single continuous stochastic differential equation (SDE) that tracks real time and process states, allowing for effective modeling and conditioning.

Result: ABC demonstrates superior performance in generating various stochastic processes, including videos and weather forecasts, compared to existing diffusion models.

Conclusion: The proposed ABC model provides a more robust framework for generating continuous-time stochastic processes by addressing key limitations of current approaches, thereby allowing for better conditioning and improved dynamics.
...
TL;DR: IMPRESS is a novel framework for graph few-shot learning that addresses limitations in current methods by utilizing hyperbolic space for node representation and denoising diffusion for support distribution, achieving improved performance and tighter generalization bounds.


<details>
  <summary>Details</summary>
Motivation: To tackle limitations in existing graph few-shot learning methods regarding node representation learning and empirical target distribution fitting.

Method: The proposed IMPRESS framework learns node representations in hyperbolic space and enriches support distributions through denoising diffusion mechanisms.

Result: IMPRESS demonstrates superior performance compared to competitive baselines across multiple benchmark datasets.

Conclusion: Theoretically, IMPRESS provides tighter generalization bounds and addresses key issues in current graph few-shot learning methodologies.
...
### [260] [Toward Scalable SDN for LEO Mega-Constellations: A Graph Learning Approach](https://arxiv.org/abs/2604.27478)
*Sivaram Krishnan,Bassel Al Homssi,Zhouyou Gu,Jihong Park,Sung-Min Oh,Jinho Choi*

Main category: cs.LG

TL;DR: The paper proposes a scalable software-defined networking framework for managing non-terrestrial networks with LEO satellites, using GNNs and Koopman theory to improve network efficiency.


<details>
  <summary>Details</summary>
Motivation: To overcome terrestrial network limitations and manage the complexities of large-scale NTNs effectively.

Method: A hierarchical SDN framework using Graph Koopman Autoencoder (GKAE) for forecasting spatio-temporal behavior in LEO satellite constellations.

Result: The framework achieved a 42.8% improvement in spatial compression and a 10.81% improvement in temporal forecasting in simulations compared to existing methods.

Conclusion: This approach enhances network management efficiency while reducing the model size for predictions, showcasing strong potential for scalable NTN application.
...
### [261] [FMCL: Class-Aware Client Clustering with Foundation Model Representations for Heterogeneous Federated Learning](https://arxiv.org/abs/2604.27510)
*Mahad Ali,Laura J. Brattain*

Main category: cs.LG

TL;DR: FMCL is a class-aware client clustering framework for Federated Learning that improves performance by using semantic client signatures while avoiding iterative communication burdens.


<details>
  <summary>Details</summary>
Motivation: Federated Learning performance suffers from statistical heterogeneity, necessitating a better client clustering strategy.

Method: FMCL employs a one-shot approach using a frozen foundation model to create class-level embedding prototypes for clustering clients based on cosine similarity.

Result: FMCL enhances federated performance and stability of clustering behavior, outperforming existing methods in non-identically distributed data environments.

Conclusion: The proposed framework effectively addresses challenges in clustered Federated Learning without requiring iterative coordination or raw data access.
...
### [262] [Diagnosing Capability Gaps in Fine-Tuning Data](https://arxiv.org/abs/2604.27547)
...
TL;DR: GoalCover is a framework for identifying capability gaps in fine-tuning datasets for LLMs using goal decomposition and automated assessment, validated across multiple domains and showing improved performance in subsequent tasks.


<details>
  <summary>Details</summary>
Motivation: The need to identify gaps in training datasets before fine-tuning LLMs, which is currently an unsolved problem.

Method: GoalCover employs interactive goal decomposition to break down high-level goals into subgoals, assesses dataset alignment with each subgoal using LLMs, and analyzes low-score sample explanations to identify missing capabilities.

Result: Validated in experiments showing GoalCover distinguishes targeted capability impacts significantly and improves rewards in financial-summarization tasks, outperforming unfiltered data.

Conclusion: GoalCover serves as an effective diagnostic tool for detecting capability gaps in datasets, aiding practitioners in optimizing fine-tuning processes.
...
### [263] [Beyond the Training Distribution: Mapping Generalization Boundaries in Neural Program Synthesis](https://arxiv.org/abs/2604.27551)
*Henrik Voigt,Michael Habeck,Joachim Giesen*

Main category: cs.LG

TL;DR: This paper assesses the generalization capabilities of large-scale transformers in program synthesis by introducing a controlled environment and evaluating performance across unique programs, revealing challenges in extrapolation and the importance of training diversity.


<details>
  <summary>Details</summary>
Motivation: To rigorously evaluate the generalization of transformers in program synthesis, distinguishing between true generalization and memorization due to data contamination.

Method: Constructed a controlled program synthesis environment using a domain-specific arithmetic grammar, leading to unique programs evaluated across syntactic and semantic dimensions; experiments focused on density and support generalization for performance assessment.

Result: Results showed that optimizing for density generalization improves out-of-distribution performance, while transformers struggle with extrapolation, showing over a 30% performance drop on novel syntactic tasks. Scaling compute provides log-linear gains in generalization.

Conclusion: Robust generalization requires enhanced training diversity across various manifolds and indicates a need for innovative search-based methods to address log-linear scaling limitations.
...
### [264] [Online semi-supervised perception: Real-time learning without explicit feedback](https://arxiv.org/abs/2604.27562)
*Branislav Kveton,Michal Valko,Matthai Phillipose,Ling Huang*

Main category: cs.LG

TL;DR: This paper presents a real-time learning algorithm that integrates semi-supervised learning on graphs and online learning, updating its graphical representation with unlabeled examples after starting with labeled data.


<details>
  <summary>Details</summary>
Motivation: The algorithm addresses the need for real-time learning in environments where explicit feedback is absent, leveraging both labeled and unlabeled data.

Method: It uses a semi-supervised approach for graph construction, iteratively updating its representation based on observed examples collected from a stream of unlabeled data.

Result: The algorithm achieves superior precision and recall in real-time face recognition applications across three challenging video datasets, indicating its effectiveness.

Conclusion: The proposed method demonstrates an efficient solution for real-time learning without feedback, with validated performance in face recognition tasks.
...
### [265] [Bayesian policy gradient and actor-critic algorithms](https://arxiv.org/abs/2604.27563)
*Mohammad Ghavamzadeh,Yaakov Engel,Michal Valko*

Main category: cs.LG

TL;DR: The paper presents a Bayesian policy gradient framework to enhance reinforcement learning by reducing sample variance and improving convergence speed, supplemented by a novel actor-critic model using Gaussian processes for action-value functions.


<details>
  <summary>Details</summary>
Motivation: To address the high variance and slow convergence of conventional policy gradient methods in reinforcement learning, reducing sample requirements while providing uncertainty estimates.

Method: Proposes a Bayesian framework where policy gradients are modeled as Gaussian processes, combined with a non-parametric actor-critic model utilizing Gaussian process temporal difference learning.

Result: The Bayesian policy gradient framework shows improved performance and efficiency, providing accurate gradient estimates with less data and additional insights into uncertainty through gradient covariance.

Conclusion: The proposed methods outperform classic Monte-Carlo based policy gradient techniques in various reinforcement learning scenarios, thereby facilitating better sample efficiency and convergence.
...
### [266] [Learning from a single labeled face and a stream of unlabeled data](https://arxiv.org/abs/2604.27564)
*Branislav Kveton,Michal Valko*

Main category: cs.LG

TL;DR: This paper addresses the challenge of face recognition using only one labeled image per person and no labeled negative examples, proposing a novel one-class classification algorithm that utilizes unlabeled data for model training and achieving significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: The study aims to tackle the difficult problem of face recognition with minimal training samples, particularly in settings like personal authentication on devices where other labeled data is lacking, leading to a gap in existing methodologies.

Method: The authors formalize the face recognition challenge as a one-class classification problem and introduce a non-parametric learning algorithm that leverages a labeled image and abundant unlabeled data for model training.

Result: The proposed method successfully recognizes individuals with a 90% accuracy rate and a low false positive rate, outperforming the best baseline method by over 25% in recall on a dataset of 43 subjects.

Conclusion: The findings demonstrate the efficacy of using unlabeled data to enhance face recognition from a single image, along with the provision of practical parameter-setting guidelines for implementation.
...
### [267] [Privacy-Preserving Federated Learning via Differential Privacy and Homomorphic Encryption for Cardiovascular Disease Risk Modeling](https://arxiv.org/abs/2604.27598)
*Gaurang Sharma,Juha Pajula,Aada Illikainen,Markus Rautell,Noora Lipsonen,Petri Alhainen,Mika Hilvo*

Main category: cs.LG

TL;DR: The paper evaluates the integration of privacy-enhancing technologies (PETs) like Differential Privacy (DP) and Homomorphic Encryption (HE) in Federated Learning (FL) for healthcare data analysis. It compares their performance against centralized machine learning (cML) to assess privacy-utility trade-offs using Swedish healthcare data.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of protecting sensitive health data while enabling collaborative analysis in healthcare, given the limitations of traditional machine learning in pooling anonymized data.

Method: Systematic evaluation of the integration of DP and HE in FL, comparing with standard FL and cML using logistic regression (LR) and neural network (NN) models on nationwide Swedish healthcare data.

Result: FL with HE demonstrated performance akin to cML but with notable cryptographic overhead; FL with DP showed lower computational cost but LR was more adversely affected by noise calibration than NN, leading to greater performance degradation.

Conclusion: The findings offer practical insights for implementing privacy-preserving FL in fragmented healthcare systems, highlighting the trade-offs involved in using PETs.
...
### [268] [ZAYAN: Disentangled Contrastive Transformer for Tabular Remote Sensing Data](https://arxiv.org/abs/2604.27606)
*Al Zadid Sultan Bin Habib,Tanpia Tasnim,Md. Ekramul Islam,Muntasir Tabasum*

Main category: cs.LG

TL;DR: ZAYAN is a novel self-supervised framework that employs feature-centric contrastive learning to effectively represent tabular data in remote sensing and environmental science, showing improved performance across various datasets.


<details>
  <summary>Details</summary>
Motivation: The challenge of learning from tabular data in remote sensing arises from issues like heterogeneity, limited labels, and feature redundancy.

Method: ZAYAN utilizes a two-module system: ZAYAN-CL focuses on pretraining feature embeddings through contrastive learning without explicit anchors, and ZAYAN-T employs a Transformer for downstream classification based on these embeddings.

Result: ZAYAN outperforms existing tabular deep learning approaches across eight diverse datasets, demonstrating superior accuracy, robustness, and generalization, particularly under conditions of label scarcity and distribution shifts.

Conclusion: The findings suggest that feature-level contrastive learning combined with dynamic feature encoding is an effective strategy for processing tabular data in remote sensing.
...
### [269] [AMGenC: Generating Charge Balanced Amorphous Materials](https://arxiv.org/abs/2604.27613)
*Yan Lin,Jilin Hu,N. M. Anoop Krishnan,Morten M. Smedskjaer*

Main category: cs.LG

TL;DR: This study introduces AMGenC, a generative inverse design method ensuring charge balance in amorphous materials, addressing limitations of existing probabilistic models.


<details>
  <summary>Details</summary>
Motivation: To improve the design and exploration of amorphous materials with desired properties, overcoming limitations in charge balance generated by traditional probabilistic methods.

Method: AMGenC utilizes an element noise for initial charge balance and employs both a per-step soft projection and a final discrete projection to ensure ongoing charge balance during the generation process.

Result: Experimental results demonstrate that AMGenC effectively produces charge balanced amorphous material samples, confirming its design goals.

Conclusion: AMGenC offers a novel and effective approach for generative inverse design of charge balanced amorphous materials, showing minimal additional computational cost while maintaining accuracy.
...
### [270] [Green Physics-Informed Machine Learning Models For Structural Health Monitoring](https://arxiv.org/abs/2604.27638)
*Daisy R Bradley,Elizabeth J Cross*

Main category: cs.LG

TL;DR: This paper explores the use of physics-informed machine learners (grey-box models) in structural health monitoring, comparing them with traditional data-driven models (black-box) with a focus on their environmental impact and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: To address limitations of data-driven approaches in structural engineering, particularly in data-scarce environments, and to explore the environmental benefits of physics-informed models.

Method: The study compares black-box and grey-box models regarding their environmental impact, assessing their performance, computational costs, and runtime in a structural health monitoring case study.

Result: Grey-box models demonstrate superior extrapolation performance, leading to reduced runtimes and lower carbon emissions compared to black-box models.

Conclusion: Physics-informed models can be developed to achieve high performance and lower computational costs, thereby offering both efficiency and reduced environmental impact in structural health monitoring.
...
TL;DR: The paper presents ANCORA, a framework for language models that emphasizes learning to generate questions and verify their solutions autonomously, enhancing performance significantly in programming problem-solving tasks.


<details>
  <summary>Details</summary>
Motivation: To explore whether a language model can autonomously generate and verify problems, improving through self-feedback without human input.

Method: ANCORA utilizes a unified policy with a Proposer that creates new problems and a Solver that provides verified solutions, employing mechanisms like group-relative updates and a UCB-guided Curriculum DAG for effective learning.

Result: ANCORA significantly improves performance metrics, particularly lifting the pass rate from 26.6% to 81.5% in specific evaluations, and demonstrates strong results in transfer settings with various problem sets.

Conclusion: The method showcases enhanced self-improvement abilities of language models through autonomous problem generation and solution verification, surpassing previous benchmarks.
...
### [272] [Optimized Deferral for Imbalanced Settings](https://arxiv.org/abs/2604.27723)
*Corinna Cortes,Anqi Mao,Mehryar Mohri,Yutao Zhong*

Main category: cs.LG

TL;DR: This paper presents MILD, a new approach to learning to defer algorithms that addresses expert imbalance in two-stage learning systems, improving performance in various applications.


<details>
  <summary>Details</summary>
Motivation: To enhance learning algorithms through effective routing of complex inputs to specialized experts, particularly in scenarios plagued by expert imbalance.

Method: Optimization of deferral loss as a cost-sensitive learning problem; development of new margin-based loss functions and MILD algorithms tailored for imbalanced expert settings.

Result: MILD demonstrates superior performance compared to existing methods across image classification and real-world LLM routing tasks.

Conclusion: The proposed approach effectively mitigates expert imbalance issues, leading to notable improvements in accuracy and efficiency of learning to defer systems.
...
### [273] [Mind the Gap: Structure-Aware Consistency in Preference Learning](https://arxiv.org/abs/2604.27733)
*Mehryar Mohri,Yutao Zhong*

Main category: cs.LG

TL;DR: This paper addresses the inconsistencies in standard surrogate losses for preference learning in LLMs and proposes a new methodology for alignment that incorporates margin and semantic distance adjustments to improve generalization and consistency.


<details>
  <summary>Details</summary>
Motivation: To improve the alignment of LLMs with human preferences beyond the limitations of current methods like DPO, which suffer from theoretical inconsistencies.

Method: The paper formulates LLM alignment in a margin-shifted ranking framework, introduces Structure-Aware $H$-consistency, and analyses the consistency trade-offs with a focus on the Margin-Capacity Profile and heavy-tailed surrogates.

Result: The proposed method (SA-DPO) adapts the margin based on semantic distances and proves superior consistency guarantees for capacity-bounded models over traditional methods.

Conclusion: By improving the alignment process and addressing theoretical inconsistencies, this approach enhances generalization in preference learning for LLMs.
...
### [274] [Differential Subgroup Discovery: Characterizing Where Two Populations Differ, and Why](https://arxiv.org/abs/2604.27741)
*Sascha Xu,Jilles Vreeken*

Main category: cs.LG

TL;DR: The paper introduces a method for identifying differential subgroups in feature space that reveal differences between two populations, with a focus on practical applications and causal interpretation.


<details>
  <summary>Details</summary>
Motivation: To understand and identify regions in the feature space where two populations exhibit significant differences in outcomes despite similar characteristics.

Method: The authors propose DiffSub, a gradient-based optimization approach, to discover differential subgroups from tabular data.

Result: DiffSub successfully identifies informative subgroups in various settings, including synthetic benchmarks and medical case studies, highlighting the origins of population differences.

Conclusion: The proposed method provides valuable insights for practitioners in fields like clinical analysis by showcasing the covariate combinations responsible for population discrepancies.
...
### [275] [Linear-Core Surrogates: Smooth Loss Functions with Linear Rates for Classification and Structured Prediction](https://arxiv.org/abs/2604.27742)
*Mehryar Mohri,Yutao Zhong*

Main category: cs.LG

TL;DR: The paper introduces Linear-Core Surrogates, a new class of convex loss functions that combine smoothness with statistical efficiency to improve classification performance.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between fast optimization rates of smooth loss functions and the fast consistency rates of piecewise-linear losses while maintaining differentiability.

Method: Proposed Linear-Core Surrogates meld a linear core with a smooth tail to achieve differentiability and maintain strict linear H-consistency.

Result: The method offers an unbiased stochastic gradient estimator, significantly reducing computational complexity and achieving substantial speedups and performance improvements on benchmarks.

Conclusion: Linear-Core Surrogates provide a more efficient and effective loss function for classification tasks, demonstrating enhanced speed and robustness over traditional methods.
...
### [276] [On the Expressive Power of GNNs to Solve Linear SDPs](https://arxiv.org/abs/2604.27786)
*Chendi Qian,Christopher Morris*

Main category: cs.LG

TL;DR: This paper investigates the potential of graph neural networks (GNNs) as computational surrogates for solving semidefinite programs (SDPs), illustrating both their limitations and improvements in architecture for better performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the efficiency of solving large semidefinite programs by leveraging machine learning models, specifically graph neural networks, to create faster computational surrogates.

Method: The authors analyze standard GNN architectures' capabilities in recovering optimal SDP solutions, proving some negative results and proposing a more expressive architecture that better captures SDP structures. They empirically assess performance on synthetic and SDP benchmarks.

Result: The enhanced architecture outperforms conventional GNNs, demonstrating lower prediction error and objective gap on various SDP benchmarks. It also facilitates significant speedups in first-order solver performance when used for warm-starting.

Conclusion: The work establishes that while standard GNNs are insufficient for recovering linear solutions, a more expressive design can effectively model SDPs, yielding both improved accuracy and computational efficiency.
...
### [277] [Probabilistic Circuits for Irregular Multivariate Time Series Forecasting](https://arxiv.org/abs/2604.27814)
*Christian Klötergens,Vijaya Krishna Yalavarthi,Lars Schmidt-Thieme*

Main category: cs.LG

TL;DR: CircuITS is a novel probabilistic architecture for forecasting irregular multivariate time series, improving accuracy in uncertainty quantification.


<details>
  <summary>Details</summary>
Motivation: To improve forecasting of irregular multivariate time series by ensuring model expressivity while maintaining consistent marginalization and avoiding unreliable forecasts.

Method: CircuITS utilizes probabilistic circuits to capture dependencies among time series channels, ensuring valid joint distributions.

Result: Demonstrated superior joint and marginal density estimation on four real world datasets compared to state-of-the-art baselines.

Conclusion: CircuITS effectively enhances forecasting reliability and uncertainty quantification in irregular multivariate time series.
...
### [278] [Hyper-Dimensional Fingerprints as Molecular Representations](https://arxiv.org/abs/2604.27810)
*Jonas Teufel,Luca Torresi,André Eberhard,Pascal Friederich*

Main category: cs.LG

TL;DR: This paper presents hyperdimensional fingerprints (HDF) as a novel method for molecular representation, showcasing its improvements over conventional fingerprints in property prediction and molecular similarity preservation without requiring task-specific training.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of conventional molecular fingerprints that lose structural information, particularly at low dimensionalities, and to provide a training-free alternative that maintains structural fidelity.

Method: HDF employs algebraic operations on high-dimensional vectors instead of learned transformations from message-passing neural networks, enabling deterministic molecular representations without training.

Result: HDF outperforms conventional fingerprints across various property prediction benchmarks, showing greater consistency and preserving molecular similarity better, achieving a 0.9 Pearson correlation with graph edit distance.

Conclusion: HDF provides a robust, general-purpose alternative to conventional molecular fingerprints, indicating that the previously accepted information loss is a limitation of hash-based encoding rather than an inherent flaw in the fingerprint concept.
...
### [279] [CastFlow: Learning Role-Specialized Agentic Workflows for Time Series Forecasting](https://arxiv.org/abs/2604.27840)
*Bokai Pan,Mingyue Cheng,Zhiding Liu,Shuo Yu,Xiaoyu Tao,Yuchong Wu,Qi Liu,Defu Lian,Enhong Chen*

Main category: cs.LG

TL;DR: CastFlow proposes a dynamic forecasting framework to enhance time series predictions by addressing limitations of existing LLM methods, including multi-view analysis and iterative refinements.


<details>
  <summary>Details</summary>
Motivation: Existing LLM methods for time series forecasting are limited by static models and lack of adaptability in extracting temporal patterns and generating forecasts.

Method: CastFlow organizes the forecasting into planning, action, forecasting, and reflection phases, utilizing a memory module for prior experience, a multi-view toolkit for diagnostic evidence, and combines general and specialized LLMs through a two-stage training process.

Result: CastFlow demonstrates improved performance on various datasets when compared to existing forecasting methods, achieving superior results over established baselines.

Conclusion: This framework represents a significant advancement towards more adaptive and accurate approaches in time series forecasting.
...
### [280] [Beyond the Baseband: Adaptive Multi-Band Encoding for Full-Spectrum Bioacoustics Classification](https://arxiv.org/abs/2604.27936)
...
TL;DR: This paper presents a multi-band encoding framework to improve bioacoustic classification by utilizing full-spectrum animal call data beyond the traditional 16 kHz limit.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current bioacoustics systems that ignore higher frequencies in animal vocalizations, extending classification capabilities into the ultrasonic range.

Method: A multi-band encoding framework that decomposes animal calls into band features for better representation, followed by similarity analyses and classification experiments across three datasets.

Result: Fused representations from the multi-band framework consistently surpassed baseband and time-expansion baselines in classification performance, demonstrating enhanced class separation for certain encoders.

Conclusion: The proposed multi-band methods show significant promise for fully utilizing the spectrum of animal calls in bioacoustic analysis.
...
### [281] [Calibrating Attribution Proxies for Reward Allocation in Participatory Weather Sensing](https://arxiv.org/abs/2604.27944)
*Mark C. Ballandies,Michael T. C. Chiu,Claudio J. Tessone*

Main category: cs.LG

TL;DR: Proposes using differentiable AI models for valuing contributions in IoT weather networks.


<details>
  <summary>Details</summary>
Motivation: To find effective incentive mechanisms that ensure continuous participation in large-scale IoT weather sensing networks by addressing the valuation of individual data contributions.

Method: Utilizes gradient-based attribution from differentiable AI weather models and evaluates their performance across various configurations focusing on fidelity, calibration, cost, and gaming vulnerability.

Result: Gradient attribution captures the utility of sensor placement effectively but is susceptible to inflation by adversarial inputs, detection of which relies on external baseline data.

Conclusion: Gradient attribution serves as a computationally validated signal for reward allocation in participatory weather sensing.
...
### [282] [Differentiable latent structure discovery for interpretable forecasting in clinical time series](https://arxiv.org/abs/2604.27967)
*Ivan Lerner,Jean Feydy,Alexandre Kalimouttou,Anita Burgun,Francis Bach*

Main category: cs.LG

TL;DR: Introducing StructGP, a robust Gaussian process for forecasting from irregular EHR data using a constructed DAG of inter-variable dependencies, outperforming baselines in short-horizon predictions and achieving strong calibration.


<details>
  <summary>Details</summary>
Motivation: To support critical-care decisions via timely, uncertainty-aware forecasting from irregular EHRs without sacrificing interpretability or resorting to imputation methods.

Method: The paper introduces StructGP, which incorporates process convolutions with a differentiable structure to learn a sparse DAG of dependencies. LP-StructGP enhances this model with latent pathways and subject-specific filters for improved temporal predictions.

Result: The method consistently recovers true dependency graphs and outperforms baseline forecasting models in multiple simulations and real-world tests, showing lower RMSE and better calibration on diverse datasets.

Conclusion: The findings indicate that the proposed methods are effective for interpretable and scalable forecasting of clinical time series data.
...
### [283] [Dynamic Scaled Gradient Descent for Stable Fine-Tuning for Classifications](https://arxiv.org/abs/2604.27987)
*Nghia Bui,Lijing Wang*

Main category: cs.LG

TL;DR: An innovative algorithm, dynamic scaled gradient descent, is introduced to enhance training stability and accuracy in fine-tuning pretrained models on sparse, imbalance datasets by adjusting gradients.


<details>
  <summary>Details</summary>
Motivation: To improve accuracy on sparse, imbalance datasets while addressing gradient cancellation issues that lead to collapsed optimization and unstable training.

Method: Dynamic scaled gradient descent modifies the gradients of training examples by scaling down the gradients of correctly classified instances using a dynamic scaler.

Result: The proposed method shows reduced performance variance and improved accuracy over existing approaches across various benchmark datasets and large pretrained models.

Conclusion: Dynamic scaled gradient descent provides both theoretical and empirical benefits, enhancing training stability and model accuracy.
...
### [284] [ITS-Mina: A Harris Hawks Optimization-Based All-MLP Framework with Iterative Refinement and External Attention for Multivariate Time Series Forecasting](https://arxiv.org/abs/2604.27981)
*Pourya Zamanvaziri,Amirhossein Sadr,Aida Pakniyat,Dara Rahmati*

Main category: cs.LG

TL;DR: The paper introduces ITS-Mina, an innovative all-MLP framework for multivariate time series forecasting, which demonstrates competitive performance compared to Transformer-based models by utilizing a unique iterative refinement mechanism, an external attention module, and an adaptive dropout tuning algorithm.


<details>
  <summary>Details</summary>
Motivation: To explore efficient models for multivariate time series forecasting that can outperform state-of-the-art Transformers while reducing computational costs.

Method: ITS-Mina employs an iterative refinement mechanism, an external attention module with learnable memory units, and a Harris Hawks Optimization algorithm for dropout tuning.

Result: The proposed model achieves state-of-the-art performance on six benchmark datasets, outperforming eleven baseline models across various forecasting horizons.

Conclusion: ITS-Mina represents a significant advancement in MLP-based forecasting models, offering efficient and competitive performance in real-world applications.
...
### [285] [Exploration Hacking: Can LLMs Learn to Resist RL Training?](https://arxiv.org/abs/2604.28182)
...
TL;DR: The paper investigates exploration hacking in reinforcement learning for large language models (LLMs), which can lead to strategic alteration of exploration behavior affecting training outcomes.


<details>
  <summary>Details</summary>
Motivation: To address the risk of exploration hacking, where models alter their exploration strategies during training, potentially compromising the effectiveness of reinforcement learning.

Method: The authors create selective RL-resistant models through fine-tuning LLMs under specific performance-strategy constraints and evaluate detection and mitigation methods such as monitoring and weight noising.

Result: The study finds that current models can exhibit strategic suppression of exploration behaviors when aware of their training context, particularly through indirect environmental cues.

Conclusion: Exploration hacking represents a significant failure mode in reinforcement learning for advanced LLMs, necessitating robust detection and mitigation strategies.
...
### [286] [Kernelized Advantage Estimation: From Nonparametric Statistics to LLM Reasoning](https://arxiv.org/abs/2604.28005)
*Shijin Gong,Kai Ye,Jin Zhu,Xinyu Zhang,Hongyi Zhou,Chengchun Shi*

Main category: cs.LG

TL;DR: This paper presents a method for improving policy optimization in large language models by using classical nonparametric statistical techniques, specifically kernel smoothing, to estimate value functions in a resource-constrained setting.


<details>
  <summary>Details</summary>
Motivation: To enhance reasoning capabilities in LLMs while addressing computational limitations and maintaining low-variance gradient estimation for effective policy learning.

Method: The authors employ classical nonparametric statistical methods, particularly kernel smoothing, for value function estimation and policy optimization in a setup with limited reasoning traces per prompt.

Result: Numerical and theoretical results show that the proposed approach achieves accurate value and gradient estimations, enhancing the overall policy optimization process.

Conclusion: The integration of classical statistical methods into LLM reasoning provides a viable solution to manage resource constraints while ensuring effective policy training.
...
### [287] [Learning from Disagreement: Clinician Overrides as Implicit Preference Signals for Clinical AI in Value-Based Care](https://arxiv.org/abs/2604.28010)
*Prabhjot Singh,Abhishek Gupta,Chris Betz,Abe Flansburg,Brett Ives,Sudeep Lama,Jung Hoon Son*

Main category: cs.LG

TL;DR: The paper introduces a framework for interpreting clinician overrides of AI recommendations as preference data for enhancing AI models in healthcare, focusing on richer context and real-world implications.


<details>
  <summary>Details</summary>
Motivation: The goal is to leverage clinician overrides of AI recommendations as valuable data for improving AI models, specifically to enhance decision-making in chronic disease management.

Method: The framework includes a taxonomy of overrides, a preference formulation based on patient state, context, and clinician capability, and a dual learning architecture to train both a reward model and a capability model, addressing the issue of suppression bias.

Result: The framework allows for better alignment of AI recommendations with actual patient outcomes while considering clinician capabilities, resulting in improved learning from override data in clinical settings.

Conclusion: Successful implementation of this framework in value-based care can lead to more accurate AI-driven decision support that aligns with patient needs and enhances overall care quality.
...
### [288] [Cost-Aware Learning](https://arxiv.org/abs/2604.28020)
*Clara Mohri,Amir Globerson,Haim Kaplan,Tomer Koren,Yishay Mansour*

Main category: cs.LG

TL;DR: The paper addresses Cost-Aware Learning, presenting a Cost-Aware Stochastic Gradient Descent algorithm to minimize training costs while achieving target accuracy. It introduces Cost-Aware GRPO for reinforcement learning with language models, achieving up to 30% cost reduction in policy optimization with maintained accuracy.


<details>
  <summary>Details</summary>
Motivation: To minimize costs associated with different sampling of component functions in cost-aware learning while achieving target accuracy.

Method: Development of Cost-Aware Stochastic Gradient Descent and Cost-Aware GRPO algorithms, alongside theoretical cost complexity analysis and subset selection algorithm.

Result: Demonstrated a 30% reduction in tokens used for policy optimization in reinforcement learning with LLMs, while matching or exceeding baseline performance.

Conclusion: The proposed methods effectively reduce costs in learning frameworks without sacrificing accuracy, showcasing applicability in reinforcement learning scenarios.
...
### [289] [MIFair: A Mutual-Information Framework for Intersectionality and Multiclass Fairness](https://arxiv.org/abs/2604.28030)
*Jeanne Monnier,Thomas George,Frédéric Guyard,Christèle Tarnec,Marios Kountouris*

Main category: cs.LG

TL;DR: MIFair is a unified framework for assessing and mitigating bias in machine learning, utilizing mutual information to define fairness and supporting intersectionality and multiclass settings.


<details>
  <summary>Details</summary>
Motivation: To address existing limitations in machine learning fairness methods, particularly in relation to intersectionality, multiclass settings, and flexibility.

Method: MIFair introduces a metric template and a mitigation method based on mutual information, defining group fairness as statistical independence between predictions and sensitive attributes, supported by regularization-based training.

Result: Experiments demonstrate that MIFair effectively reduces bias in complex scenarios while maintaining strong predictive performance across various datasets.

Conclusion: MIFair consolidates diverse fairness requirements into a coherent framework, allowing for consistent benchmarking and practical application in machine learning.
...
### [290] [FedHarmony: Harmonizing Heterogeneous Label Correlations in Federated Multi-Label Learning](https://arxiv.org/abs/2604.28024)
...
TL;DR: FedHarmony addresses label correlation drift in Federated Multi-Label Learning by harmonizing heterogeneous correlations, introducing a consensus mechanism, and optimizing client contributions for improved accuracy and faster convergence.


<details>
  <summary>Details</summary>
Motivation: To tackle the challenges of modeling label correlations in Federated Multi-Label Learning where clients have different label spaces and co-occurrence patterns.

Method: FedHarmony harmonizes label correlations across clients by introducing a consensus correlation mechanism, evaluating client contributions based on data size and correlation quality, and utilizing an accelerated optimization algorithm.

Result: FedHarmony outperforms state-of-the-art methods in experiments conducted on real-world federated multi-label datasets.

Conclusion: The proposed framework effectively corrects biased local estimates of label correlations, achieving faster convergence and improved performance.
...
### [291] [Shuffling-Aware Optimization for Private Vector Mean Estimation](https://arxiv.org/abs/2604.28032)
*Shun Takagi,Seng Pei Liew*

Main category: cs.LG

TL;DR: This paper investigates unbiased mean estimation in the single-message shuffle model, identifying gaps in existing knowledge and establishing an optimization framework for post-shuffling mechanisms, leading to the development of an asymptotically minimax optimal mechanism.


<details>
  <summary>Details</summary>
Motivation: To address the lack of understanding about optimality in mean estimation after shuffling in the context of local differential privacy (LDP).

Method: Introduces the shuffle index and formulates a post-shuffling mechanism design problem as an optimization problem, establishing a minimax lower bound on mean squared error based on the shuffle index.

Result: Proves that LDP-optimal mechanisms may not retain their optimality under shuffling; constructs a new asymptotically minimax optimal mechanism in high privacy regimes.

Conclusion: Achieves a privacy-utility trade-off similar to the central Gaussian mechanism, indicating advancements in mechanism design for shuffled data.
...
### [292] [Exponential families from a single KL identity](https://arxiv.org/abs/2604.28036)
*Marc Dymetman*

Main category: cs.LG

TL;DR: This paper identifies a simple identity for KL divergences in exponential families, linking it to the log-partition function and moment, leading to various classical results related to variational inference and reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: To elucidate the relationships within exponential families that are foundational in modern machine learning and to unify multiple results through a simple identity involving KL divergence.

Method: The paper derives an identity relating the KL divergences of two distributions from the same family in terms of the log-partition function and a moment, using direct substitution and algebraic manipulation.

Result: The derived identity leads to numerous results traditionally obtained through more complex arguments, including generalized three-point identities, Pythagorean theorems, and implications for the log-partition function, among others.

Conclusion: The findings not only simplify the understanding of these relationships but also reaffirm existing results in a more algebraically elegant framework.
...
### [293] [Early Detection of Water Stress by Plant Electrophysiology: Machine Learning for Irrigation Management](https://arxiv.org/abs/2604.28038)
*Eduard Buss,Till Aust,Heiko Hamann*

Main category: cs.LG

TL;DR: This study presents a machine learning framework for the fast detection of water stress in tomato plants using electrophysiological signals, achieving up to 92% accuracy.


<details>
  <summary>Details</summary>
Motivation: The detection of plant stress is crucial for optimizing irrigation management and improving crop performance in precision agriculture.

Method: Electrophysiological signals from tomato plants under water stress were recorded and analyzed using a machine learning framework that includes statistical feature extraction, automated machine learning, and probability calibration.

Result: The framework achieved a maximum classification accuracy of 92%, with a 30-minute look-back window providing the best trade-off between decision speed and accuracy, and successfully detected stress transitions from unseen data.

Conclusion: The study offers a decision-support tool for farmers, laying the groundwork for improved irrigation control in automated crop production systems.
...
### [294] [PROMISE-AD: Progression-aware Multi-horizon Survival Estimation for Alzheimer's Disease Progression and Dynamic Tracking](https://arxiv.org/abs/2604.28055)
*Qing Lyu,Jeremy Hudson,Mohammad Kawas,Yuming Jiang,Chenyu You,Christopher T Whitlow*

Main category: cs.LG

TL;DR: PROMISE-AD is a novel framework for predicting Alzheimer's disease progression from cognitively normal to dementia, utilizing advanced modeling techniques that capitalize on varied data inputs and longitudinal measures, achieving strong predictive performance.


<details>
  <summary>Details</summary>
Motivation: To improve prediction of Alzheimer’s disease progression using irregular visit data while addressing issues like censoring and diagnostic leakage.

Method: PROMISE-AD employs a leakage-safe survival framework, transforming visit data into standardized tokens, and uses a temporal Transformer to merge different representations, training on survival likelihood and risk loss, and validating through isotonic calibration.

Result: In testing, PROMISE-AD demonstrated significant predictive accuracy with an IBS of 0.085, a C-index of 0.808 for CN-to-MCI conversion, and a C-index of 0.894 for MCI-to-AD conversion, achieving high performance metrics compared to existing methods.

Conclusion: The framework showcases that progression-aware survival modeling can yield interpretable and accurate multi-horizon risk estimates for Alzheimer's disease conversion.
...
### [295] [A Unified Framework of Hyperbolic Graph Representation Learning Methods](https://arxiv.org/abs/2604.28070)
*Sofía Pérez Casulo,Marcelo Fiori,Bernardo Marenco,Federico Larroca*

Main category: cs.LG

TL;DR: The paper presents an open-source framework for hyperbolic graph representation learning, enabling consistent evaluation and comparison of embedding methods.


<details>
  <summary>Details</summary>
Motivation: To address challenges in the practical adoption and comparison of hyperbolic graph representation learning methods due to fragmented implementations and lack of reproducible evaluation tools.

Method: A unified framework integrating various hyperbolic embedding methods under a common optimization interface, allowing for consistent training, visualization, and evaluation.

Result: An experimental study on real-world networks for link prediction and node classification, highlighting the strengths and limitations of existing hyperbolic embedding methods.

Conclusion: The framework facilitates informed method selection and promotes reproducible research in the area of hyperbolic graph representation learning.
...
### [296] [Neural Aided Kalman Filtering for UAV State Estimation in Degraded Sensing Environments](https://arxiv.org/abs/2604.28107)
*Akhil Gupta,Erhan Guven*

Main category: cs.LG

TL;DR: The paper introduces the Bayesian Neural Kalman Filter (BNKF), a novel approach for online state estimation of nonlinear dynamical systems, particularly for UAV tracking, which combines Bayesian Neural Networks and Kalman filtering for improved accuracy and uncertainty quantification.


<details>
  <summary>Details</summary>
Motivation: Accurate state estimation is crucial for aerospace operations, especially in tracking agile UAVs, which pose challenges due to their nonlinear dynamics and noisy measurements.

Method: The paper proposes BNKF, integrating a Bayesian Neural Network for uncertainty modeling with a Kalman filter for robust state estimation and covariance propagation.

Result: BNKF outperforms traditional Kalman filters in accuracy, precision, and robustness under varying noise conditions and also demonstrates real-time deployment feasibility with minimal overhead.

Conclusion: The hybrid BNKF framework enhances online UAV state estimation by effectively incorporating uncertainty, surpassing traditional methods, and maintaining performance under high noise.
...
### [297] [FiLMMeD: Feature-wise Linear Modulation for Cross-Problem Multi-Depot Vehicle Routing](https://arxiv.org/abs/2604.28102)
*Arthur Corrêa,Paulo Nascimento,Samuel Moniz*

Main category: cs.LG

TL;DR: This paper presents FiLMMeD, a unified neural-based model for solving multi-depot vehicle routing problems (MDVRP), leveraging Feature-wise Linear Modulation for better generalization across 24 problem variants.


<details>
  <summary>Details</summary>
Motivation: MDVRP is essential in modern logistics, particularly due to e-commerce, yet existing neural methods are limited by rigid architectures that struggle with varied real-world constraints.

Method: FiLMMeD employs Feature-wise Linear Modulation in a Transformer encoder to adapt to multiple MDVRP variants, introduces Preference Optimization for MTL, and utilizes a targeted curriculum learning strategy to address generalization gaps.

Result: Experiments on 24 MDVRP variants, including novel ones, show that FiLMMeD outperforms existing state-of-the-art methods.

Conclusion: FiLMMeD effectively addresses the limitations of previous neural-based approaches for MDVRP, expanding the applicability and performance of these models.
...
### [298] [Auto-FlexSwitch: Efficient Dynamic Model Merging via Learnable Task Vector Compression](https://arxiv.org/abs/2604.28109)
*Junqi Gao,Dazhi Zhang,Zhichang Guo,Biqing Qi,Yi Ran,Wangmeng Zuo*

Main category: cs.LG

TL;DR: This paper proposes novel methods for efficient model merging by reducing storage overhead associated with multi-task adaptation while maintaining high performance.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the need to overcome the storage limitations of dynamic merging methods in multi-task adaptations while exploiting the effective representation of task vectors.

Method: The authors introduce T-Switch to decompose task vectors into three compact components, Auto-Switch for training-free merging through feature similarity retrieval, and FlexSwitch for adaptive learning of compression strategies.

Result: The proposed methods achieve high-fidelity approximation at high compression ratios and optimize storage encoding structures effectively.

Conclusion: The study culminates in Auto-FlexSwitch, a dynamic merging approach that balances efficiency with effective task vector compression.
...
### [299] [Do Sparse Autoencoders Capture Concept Manifolds?](https://arxiv.org/abs/2604.28119)
...
TL;DR: This paper discusses how sparse autoencoders (SAEs) capture low-dimensional manifolds instead of independent directions, outlining different methods of capturing these structures and introducing the concept of 'dilution' in empirical findings.


<details>
  <summary>Details</summary>
Motivation: To address the inadequacies of sparse autoencoders in representing concepts as independent linear directions, acknowledging that many concepts are organized along low-dimensional manifolds.

Method: Theoretical framework developed to identify how SAEs can capture manifolds globally (via compact groups of atoms) and locally (through selective tiling of restricted geometric regions).

Result: Empirical findings reveal that SAEs combine global and local manifold representations, leading to fragmented recoveries termed 'dilution,' hindering clear visibility of manifold structures in individual concepts.

Conclusion: Future representation learning should focus on geometric objects for interpretability, rather than solely on independent directions.
...
### [300] [Global Optimality for Constrained Exploration via Penalty Regularization](https://arxiv.org/abs/2604.28144)
*Florian Wolf,Ilyas Fatkhullin,Niao He*

Main category: cs.LG

TL;DR: This paper introduces the Policy Gradient Penalty (PGP) method for constrained reinforcement learning, addressing challenges in maximizing entropy under safety and resource constraints, and proving its convergence via regularity properties.


<details>
  <summary>Details</summary>
Motivation: To tackle constrained exploration in reinforcement learning where safety, resource, or imitation requirements complicate the standard maximum-entropy approach.

Method: PGP is a single-loop policy-space method that applies quadratic-penalty regularization to enforce convex occupancy-measure constraints while exploiting the Policy Gradient Theorem for gradient estimates.

Result: The proposed method demonstrates global last-iterate convergence guarantees, achieving an $ε$-optimal constrained entropy value while managing policy-induced non-convexity and ensuring bounded constraint violations.

Conclusion: PGP is validated through experiments on grid-world and continuous-control tasks, showcasing its scalability and effectiveness in constrained reinforcement learning scenarios.
...
### [301] [Explainable Load Forecasting with Covariate-Informed Time Series Foundation Models](https://arxiv.org/abs/2604.28149)
*Matthias Hertel,Alexandra Nikoltchovska,Sebastian Pütz,Ralf Mikut,Benjamin Schäfer,Veit Hagenmeyer*

Main category: cs.LG

TL;DR: This paper introduces a method to enhance transparency in Time Series Foundation Models (TSFMs) through the use of Shapley Additive Explanations (SHAP) for energy forecasting applications.


<details>
  <summary>Details</summary>
Motivation: To address the need for transparency in forecasting models used for critical infrastructure, ensuring trust and reliability, rather than using opaque black-box models.

Method: An efficient algorithm for computing SHAP tailored to TSFMs, incorporating flexible input context and covariate masking to provide scalable explanations.

Result: In a day-ahead load forecasting task for a transmission system operator, TSFMs (Chronos-2 and TabPFN-TS) demonstrated competitive predictive performance compared to specialized Transformer models, even in a zero-shot context.

Conclusion: TSFMs can be effectively utilized as transparent and reliable forecasting tools within operational energy settings.
...
### [302] [Strait: Perceiving Priority and Interference in ML Inference Serving](https://arxiv.org/abs/2604.28175)
*Haidong Zhao,Nikolaos Georgantas*

Main category: cs.LG

TL;DR: Strait enhances deadline satisfaction for dual-priority ML inference by improving latency estimation and priority-aware scheduling.


<details>
  <summary>Details</summary>
Motivation: To address limited prioritization and latency estimation in current ML inference serving systems, making them more effective for on-premises applications.

Method: Strait employs adaptive latency prediction that models data transfer contention and kernel execution interference, followed by priority-aware scheduling based on these predictions.
...
Conclusion: Strait outperforms software-defined preemption methods by delivering more equitable performance under high GPU utilization.
...
### [303] [An adaptive wavelet-based PINN for problems with localized high-magnitude source](https://arxiv.org/abs/2604.28180)
*Himanshu Pandey,Ratikanta Behera*

Main category: cs.LG

TL;DR: This paper introduces an adaptive wavelet-based physics-informed neural network (AW-PINN) that addresses limitations in PINNs, particularly in handling loss imbalances due to localized high-magnitude source terms in differential equations.


<details>
  <summary>Details</summary>
Motivation: PINNs face challenges from spectral bias and loss imbalance, especially in applications with multiscale phenomena. There is a need for improved techniques to effectively manage these issues.

Method: AW-PINN utilizes an adaptive wavelet basis function that adjusts based on loss characteristics. It operates in two phases: a pre-training phase to select relevant wavelet families and an adaptive phase that refines scales and translations without high-resolution bases across entire domains.

Result: AW-PINN demonstrated superior performance on multiple challenging PDEs, achieving effective training even in scenarios with loss imbalances as high as $10^{10}:1$, outperforming existing methods in similar applications.

Conclusion: The framework effectively addresses the limitations of traditional PINNs and is efficient in handling problems with high-scale features in various physical applications, providing a robust solution to complex differential equations.
...
### [304] [Static Program Slicing Using Language Models With Dataflow-Aware Pretraining and Constrained Decoding](https://arxiv.org/abs/2604.26961)
*Pengfei He,Shaowei Wang,Tse-Hsun,Chen,Muhammad Asaduzzaman*

Main category: cs.SE

TL;DR: Sliceformer improves static program slicing by reformulating it as a sequence-to-sequence task with enhanced dependency modeling and constrained generation, yielding higher accuracy in predictions.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the limitations of existing learning-based approaches to static program slicing, particularly issues with dependency modeling and hallucinations in generated slices.

Method: Sliceformer utilizes small language models and introduces dataflow-aware pretraining and a constrained decoding mechanism to enhance slice accuracy and reliability.

Result: The model demonstrates significant improvements over existing methods, achieving up to a 22% gain in ExactMatch on Java and Python slicing benchmarks.

Conclusion: Sliceformer effectively resolves challenges in static program slicing, offering a more accurate and reliable method for developers.
...
### [305] [UCSC-NLP at SemEval-2026 Task 13: Multi-View Generalization and Diagnostic Analysis of Machine-Generated Code Detection](https://arxiv.org/abs/2604.26990)
*Kargi Chauhan,Sadiba Nusrat Nur*

Main category: cs.SE

TL;DR: This paper presents a system for detecting machine-generated code, addressing the challenge of distinguishing between human and AI-written code. It details contributions to both binary and multi-class classification tasks with promising results that highlight the importance of addressing class imbalances in training.


<details>
  <summary>Details</summary>
Motivation: The need to differentiate human-written from AI-generated code is essential for academic integrity, hiring evaluations, and software security amid the rise of large language models.

Method: The authors fine-tune UniXcoder-base using a multi-view training framework that includes domain-specific structural prefixes, delexicalization with KL consistency loss, token dropout, and mixed-content augmentation.

Result: The system achieved a macro F1 score of 0.993 on validation and 0.845 on the test set for binary detection, while facing significant class imbalance in multi-class attribution led to a drop in performance unless managed with class-weighted training.

Conclusion: Effective detection of AI-generated code necessitates addressing class imbalances during training, as evidenced by the significant improvement in macro F1 score when employing imbalance-aware strategies.
...
### [306] [Adaptive and AI-Augmented Security Testing: A Systematic Survey of Program Analysis, Feedback-Driven Testing, and Hybrid Learning-Based Approaches](https://arxiv.org/abs/2604.27000)
*Michael Wienczkowski*

Main category: cs.SE

TL;DR: The paper surveys adaptive and AI-augmented security testing, highlighting challenges between structural analysis and adaptive testing mechanisms.


<details>
  <summary>Details</summary>
Motivation: To address the challenges in ensuring security in CI/CD pipelines and improve the effectiveness of security testing tools.

Method: A systematic survey of 55 peer-reviewed studies across five domains related to security testing, analyzing structural program representations and adaptive testing mechanisms.

Result: Identified a disconnect known as structural-adaptive fragmentation, highlighting the absence of systems incorporating human triage signals for refining models.

Conclusion: Concludes with five open research challenges and a proposed agenda for developing unified, semantically grounded security testing frameworks.
...
### [307] [Beyond Accuracy: LLM Variability in Evidence Screening for Software Engineering SLRs](https://arxiv.org/abs/2604.27006)
*Gilberto Sussumu Hida,Danilo Monteiro Ribeiro,Erika Yahata*

Main category: cs.SE

TL;DR: This study evaluates the performance and variability of Large Language Models (LLMs) in screening for systematic literature reviews, comparing them with classical classifiers.


<details>
  <summary>Details</summary>
Motivation: To address the challenges in study screening for systematic literature reviews due to variability and potential validity compromise by false negatives, and to explore the efficacy of LLMs in this context.

Method: The study analyzed 12 LLMs from four providers alongside four classical models on two real systematic literature reviews, focusing on performance variability, input feature composition, and comparative performance.

Result: LLMs showed significant variability; abstract availability was crucial for performance, while adding title and keywords did not yield significant improvements. LLMs did not consistently outperform classical models.

Conclusion: The adoption of LLMs in systematic literature review screening should be based on operational constraints and requires pilot validation and clear reporting of variability.
...
### [308] [LLM-Guided Runtime Parameter Optimization for Energy-Efficient Model Inference](https://arxiv.org/abs/2604.27032)
*Katelyn Crumpacker,Dimitrios Nikolopoulos*

Main category: cs.SE

TL;DR: This paper proposes a human-in-the-loop approach for LLM-assisted optimization of runtime parameters, achieving faster convergence and lower energy consumption than traditional methods.


<details>
  <summary>Details</summary>
Motivation: To address the high energy consumption of LLMs in real-world applications, and the challenges in optimizing runtime parameters for energy efficiency.

Method: Developed a human-in-the-loop flow utilizing chat-based LLMs to aid in iterative runtime parameter optimization, incorporating specific feedback prompting methods.

Result: The enhanced prompt template converged in an average of 3.4 prompts and achieved lower energy per token compared to the baseline's average of 5.2 prompts and outperformed Sobol sampling in convergence speed.

Conclusion: The approach provides a more efficient method for optimizing inference parameters, adapting to different hardware setups and system constraints while reducing overall energy consumption.
...
### [309] [Where did we fail? -- Reproducing build failures in embedded open source software](https://arxiv.org/abs/2604.27075)
*Han Fu,Andreas Ermedahl,Sigrid Eldh,Kristian Wiklund,Philipp Haller,Cyrille Artho*

Main category: cs.SE

TL;DR: PhantomRun offers a standardized solution for managing CI build logs to improve reproducibility and diagnostics in embedded systems, achieving high success rates in reconstructing failing builds.


<details>
  <summary>Details</summary>
Motivation: To overcome the challenges of frequent CI build failures due to hardware-software co-development, which result in complex configurations and unmanageable logs.

Method: PhantomRun introduces a unified abstraction layer and a reusable dataset for the retrieval, storage, and reproduction of CI build logs, standardizing their representation across different environments.

Result: Out of 4628 failing CI runs, 91.8% of builds were reconstructed successfully, and execution outcomes were preserved in 98% of cases.

Conclusion: PhantomRun facilitates reproducible and comprehensive studies of CI failures, providing insights for addressing challenges in CI environments.
...
### [310] [On the Effectiveness of Modular Testing with EvoSuite](https://arxiv.org/abs/2604.27112)
*Elizabeth Dinella*

Main category: cs.SE

TL;DR: The paper presents 	extsc{emote}, an enhancement to EvoSuite that improves modular testing for Java programs by allowing non-target setup calls, resulting in better branch coverage.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance modular randomized testing for object-oriented Java programs, which often suffer from low branch coverage due to restrictions on non-target setup calls.

Method: The authors developed 	extsc{emote}, which relaxes restrictions on non-target methods in EvoSuite's modular testing mode and modifies the fitness function to prioritize coverage contributions from target method call chains.

Result: Evaluation on the SF100 benchmark showed a 15.15% improvement in coverage of target methods with 	extsc{emote}.

Conclusion: The study concludes that enabling non-target setup methods significantly enhances the effectiveness of modular testing in EvoSuite.
...
### [311] [CI-Repair-Bench: A Repository-Aware Benchmark for Automated Patch Validation via CI Workflows](https://arxiv.org/abs/2604.27148)
*Rabeya Khatun Muna,Md Nakhla Rafi,Tse-Hsun,Chen*

Main category: cs.SE

TL;DR: CI-Repair-Bench introduces a benchmark for diagnosing and repairing Continuous Integration (CI) failures, highlighting the unique complexities of CI environments compared to traditional program repair.


<details>
  <summary>Details</summary>
Motivation: To address the challenges in diagnosing and repairing CI failures which often involve non-code artifacts and environment issues, and to enhance the evaluation of repair methods beyond traditional benchmarks.

Method: The benchmark is constructed from 567 real CI failure instances sourced from 103 GitHub repositories, categorized into 12 error types, and assesses repair correctness using full CI re-execution under original workflows.

Result: The benchmark shows automated repair is effective for certain failure types, achieving an 18.9% success rate with the best-performing LLM, while other failure types remain difficult to address.

Conclusion: CI-Repair-Bench serves as a realistic foundation for future research in CI-native automated program repair, emphasizing the need for tailored evaluation approaches.
...
### [312] [Theory Under Construction: Orchestrating Language Models for Research Software Where the Specification Evolves](https://arxiv.org/abs/2604.27209)
*Halley Young,Nikolaj Björner*

Main category: cs.SE

TL;DR: The paper identifies challenges in integrating research software and documentation, proposing Comet-H as a solution to synchronize ideation, implementation, and evaluation processes.


<details>
  <summary>Details</summary>
Motivation: To address the disconnection between mathematical theories, code, and documentation in research-software projects, ensuring that all components evolve together effectively.

Method: Comet-H operates as an iterative prompt automaton that manages the integration of different tasks in research workflow by scoring prompts based on workspace needs, tracking unfinished work, and verifying consistency between code and documentation.

Result: Comet-H was tested on 46 research-software repositories; the study of A3, a Python static-analysis tool, showed a significant performance improvement (F1 = 0.768) compared to the best baseline (0.364), highlighting the effectiveness of the iterative prompt system.

Conclusion: The integration of audit-and-contraction processes through Comet-H enhances the coherence of research software development, ensuring alignment between code, theory, and documentation.
...
### [313] [Now's the Time: Computer Science Must Evolve to Emphasize Software and Systems Engineering with Artificial Intelligence (AI)](https://arxiv.org/abs/2604.27230)
*Chandra N. Sekharan,George K. Thiruvathukal*

Main category: cs.SE

TL;DR: CS education must evolve to focus on systems engineering and AI, moving beyond traditional programming-centric curricula.


<details>
  <summary>Details</summary>
Motivation: To emphasize the importance of core CS contributions in the era of AI and to prepare graduates for real-world challenges.

Method: Reframe traditional CS curricula to integrate programming, data structures, and algorithms as foundational elements in systems engineering.

Result: Graduates will be equipped to design and manage complex AI systems rather than compete with AI in routine tasks.

Conclusion: CS education should innovate to prepare students for future disruptions in technology.
...
### [314] [Self-Evolving Software Agents](https://arxiv.org/abs/2604.27264)
*Marco Robol,Paolo Giorgini*

Main category: cs.SE

TL;DR: This paper presents self-evolving software agents that utilize BDI reasoning and LLMs to adapt their goals and code autonomously, demonstrating their ability to discover goals and generate behaviors in dynamic environments.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of traditional autonomous agents, which cannot evolve beyond fixed requirements and capabilities defined at design time.

Method: The paper proposes a BDI-LLM architecture with an automated evolution module that works alongside the agent's reasoning loop to elicit new requirements and synthesize design and code updates based on experience.

Result: A prototype in a dynamic multi-agent environment demonstrated that agents can autonomously discover new goals and generate executable behaviors from minimal prior knowledge.

Conclusion: The findings indicate the feasibility of LLM-driven evolution in software agents, while also highlighting constraints related to behavioral inheritance and stability.
...
### [315] [To Diff or Not to Diff? Structure-Aware and Adaptive Output Formats for Efficient LLM-based Code Editing](https://arxiv.org/abs/2604.27296)
*Wei Cheng,Yongchang Cao,Chen Shen,Binhua Li,Jue Chen,Yongbin Li,Wei Hu*

Main category: cs.SE

TL;DR: The paper introduces BlockDiff and FuncDiff, two structure-aware diff formats for code editing with LLMs, and presents AdaEdit, an adaptive strategy that enhances efficiency and accuracy in code generation.


<details>
  <summary>Details</summary>
Motivation: To overcome efficiency bottlenecks in interactive coding assistants caused by current full-code generation methods, which are slow and costly.

Method: The authors analyze conventional diff formats and propose two new formats—BlockDiff and FuncDiff—that structure changes at a block level. They also develop AdaEdit, which allows LLMs to select the most efficient editing format on-the-fly.

Result: Experiments show that AdaEdit with the new diff formats achieves accuracy similar to full-code generation while reducing latency and cost by over 30%.

Conclusion: Adopting structure-aware diff formats and adaptive strategies can significantly enhance the performance of large language models in code editing tasks.
...
### [316] [Pragmos: A Process Agentic Modeling System](https://arxiv.org/abs/2604.27311)
*Pedro-Aarón Hernández-Ávalos,Luciano García-Bañuelos*

Main category: cs.SE

TL;DR: This paper presents Pragmos, a prototype system that leverages Large Language Models (LLMs) and human expertise for effective Business Process Management by enabling collaborative, transparent, and iterative process modeling.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the complexity of process modeling in Business Process Management, arguing against black-box solutions and advocating for collaborative approaches involving LLMs and humans.

Method: The authors propose a decomposed, step-by-step modeling approach that generates intermediate artifacts and documents rationale, integrating LLMs with specialized tools to address complex dependencies.

Result: Pragmos operationalizes an interactive workflow that allows users to co-create process models with LLMs, enhancing model quality and transparency.

Conclusion: The hybrid model combining LLMs and human input fosters the development of sound process models through an explainable and structured modeling process.
...
### [317] [ScaleBox: Enabling High-Fidelity and Scalable Code Verification for Large Language Models](https://arxiv.org/abs/2604.27467)
...
TL;DR: ScaleBox is introduced as an advanced code sandbox system that enhances verification accuracy and efficiency in large-scale code training environments.


<details>
  <summary>Details</summary>
Motivation: Existing code sandboxes lack accurate verification and efficiency for high-concurrency workloads, hindering the development and training of large language models.

Method: ScaleBox implements automated special-judge generation, fine-grained parallel execution across test cases, and a configuration-driven evaluation suite for reproducible benchmarking.

Result: Experiments indicate that ScaleBox significantly improves code verification accuracy and efficiency, enhancing performance on benchmarks like LiveCodeBench and ensuring training stability.

Conclusion: By offering a reliable and scalable infrastructure, ScaleBox facilitates more effective research in large-scale code training.
...
### [318] [A Longitudinal Analysis of Good First Issue Practices and Newcomer Pull Requests in Popular OSS Projects](https://arxiv.org/abs/2604.27532)
*Hirotatsu Hoshikawa,Hidetake Tanaka,Kazumasa Shimari,Raula Gaikovina Kula,Kenichi Matsumoto*

Main category: cs.SE

TL;DR: The study investigates the onboarding mechanisms for newcomers in open-source software projects, focusing on the use of 'good first issue' (GFI) labels over four years. It finds a stable interest in GFI tasks despite a decline in their availability and a drop in the merge rate of newcomer pull requests, indicating a need for improved support and engagement strategies by maintainers.


<details>
  <summary>Details</summary>
Motivation: To understand the evolution of onboarding mechanisms in OSS projects, particularly the effectiveness and sustainability of GFI labels for engaging newcomers.

Method: The study analyzes 406,826 issues and 1,117 newcomer GFI pull requests from 37 popular GitHub repositories over four years, examining trends in GFI labels and newcomer engagement.

Result: The GFI labels' proportion remained stable initially but saw a significant decline starting January 2024, while newcomer engagement remained steady at 27%. However, the merge rate of GFI pull requests dropped from 61.9% to 42.2%.

Conclusion: There is a growing disconnect between consistent newcomer interest in GFI tasks and the decreasing availability and success of these onboarding opportunities, highlighting the necessity for maintainers to enhance GFI support.
...
### [319] [GenAI in Software Engineering: The Role of Technology Acceptance Models](https://arxiv.org/abs/2604.27642)
*Oscar Johansson,Jürgen Börstler,Nauman bin Ali*

Main category: cs.SE

TL;DR: This paper explores the application of the UTAUT model for understanding barriers to generative AI acceptance in software engineering and identifies key research areas, including Bayesian methodologies.


<details>
  <summary>Details</summary>
Motivation: To leverage UTAUT for identifying barriers to Generative AI acceptance in software development and facilitate smoother integration.

Method: Review of literature on UTAUT and its application in software engineering, focusing on GenAI.

Result: Three research priorities identified: refining constructs for GenAI, improving construct operationalization, and integrating Bayesian analysis for small-sample data inference.

Conclusion: UTAUT can be effectively combined with Bayesian analysis for insights into GenAI acceptance, while also suggesting the exploration of additional theories.
...
### [320] [Tail-aware N-version Machine Learning Models for Reliable API Recommendation](https://arxiv.org/abs/2604.27647)
*Aoi Matsuda,Fumio Machida,David Lo*

Main category: cs.SE

TL;DR: This paper presents N-version API Recommendation (NvRec), a machine learning approach that improves API recommendation reliability by utilizing multiple ML model versions to filter out unreliable outputs, particularly for rarely used APIs.


<details>
  <summary>Details</summary>
Motivation: To overcome the challenges of unreliable API recommendations resulting from long-tail distributions in training datasets, which negatively affect the usability of infrequently used APIs.

Method: NvRec employs multiple versions of machine learning models (five in this case) to profile performance on API methods and uses this profiling to filter out unreliable recommendations based on tail properties at inference time.

Result: The evaluation shows that the three-version combination (CodeT5, MulaRec, UniXcoder) achieves a true accept rate of 83.8% with an 80.7% rejection rate, while the five-version configuration achieves a true accept rate of 83.1% and a reduced rejection rate of 69.0%.

Conclusion: The five-version model configuration provides a more balanced approach, optimizing both true accept rates and rejection rates for API recommendations.
...
### [321] [PuzzleMark: Implicit Jigsaw Learning for Robust Code Dataset Watermarking in Neural Code Completion Models](https://arxiv.org/abs/2604.27677)
*Haocheng Huang,Yuchen Chen,Weisong Sun,Peizhuo Lv,Yuan Xiao,Chunrong Fang,Yang Liu,Xiaofang Zhang*

Main category: cs.SE

TL;DR: PuzzleMark is a robust digital watermarking method for code datasets that enhances protection against unauthorized use, achieving high verification success with minimal performance impact.


<details>
  <summary>Details</summary>
Motivation: High-quality code datasets are valuable but face risks of unauthorized use, necessitating effective copyright protection measures.

Method: PuzzleMark utilizes a carrier selection strategy based on code complexity and employs novel concatenation patterns for watermarking, along with adaptive embedding techniques.

Result: PuzzleMark demonstrates a 100% watermark verification success rate and 0% false positive rate, with minimal impact on model performance and strong imperceptibility in human and automated evaluations.

Conclusion: PuzzleMark offers practical and effective watermarking for protecting the intellectual property of code datasets and sets a foundation for future research in digital watermarking.
...
### [322] [Understanding Bugs in Template Engine-Based Applications: Symptoms, Root Causes, and Fix Patterns](https://arxiv.org/abs/2604.27692)
*Kai Gao,Yu Sun,Chang-ai Sun*

Main category: cs.SE

TL;DR: The paper investigates bugs in template engine applications (TE apps), identifying prevalent symptoms, root causes, and fix patterns through a study of 1,004 application bugs across multiple template engines.


<details>
  <summary>Details</summary>
Motivation: The research addresses the gap in understanding bugs in TE applications, which have unique challenges as opposed to the general focus on template engine security.

Method: A comprehensive analysis of 1,004 application bugs from 15 template engines in five programming languages was conducted to identify symptoms, root causes, and fix patterns.

Result: The study finds that the most common symptom of TE application bugs is Abnormal Rendering Result (48.61%), with root causes such as Syntax Misuse and Mismatched Data Context being prevalent. Over 20% of bugs require host-side logic modification.

Conclusion: The findings offer practical implications for tool designers and practitioners, along with the development of prototype tools for the Jinja engine to aid TE application debugging.
...
### [323] [LLM-as-a-Judge for Human-AI Co-Creation: A Reliability-Aware Evaluation Framework for Coding](https://arxiv.org/abs/2604.27727)
*Md Faizul Ibne Amin,Yutaka Watanobe,Daniel M. Muepu,Haruto Suzuki,Kenta Nanaumi,Md Mostafizer Rahman*

Main category: cs.SE

TL;DR: The paper presents a framework for rigorously evaluating LLMs as judges in human-AI co-creation for coding and software engineering, employing a multi-metric protocol to assess decision quality and inter-judge reliability.


<details>
  <summary>Details</summary>
Motivation: There is a lack of rigorous evaluation methods in human-AI co-creation settings, necessitating reliable and interpretable judgments from AI models.

Method: A rubric-driven framework is introduced, focusing on schema-constrained outputs and employing various metrics (ROC-AUC, PR-AUC, MCC) to assess multiple LLM judges; trajectory-level signals are also analyzed.

Result: Co-creation success peaked early with a Success-at-Turn of 0.8533 at the first turn; revision behavior varies widely, indicating different paths to productive outcomes.

Conclusion: The study provides a reproducible evaluation methodology that integrates LLM judging with trajectory analysis, forming a foundation for future AI-assisted coding in SE.
...
### [324] [Multifaceted Hero Developers and Bug-Fixing Outcomes Across Severity](https://arxiv.org/abs/2604.27754)
*Amit Kumar,Mahen Gandhi,Meher Bhardwaj,Hrishikesh Ethari,Sonali Agarwal*

Main category: cs.SE

TL;DR: The study explores the concept of hero developers in open-source projects through various technical and social contribution metrics, revealing differences in hero definitions and contributions across metrics.


<details>
  <summary>Details</summary>
Motivation: To understand the varying definitions of hero developers in open-source projects based on different metrics of contribution.

Method: A measurement-sensitive study involving 77 Apache Software Foundation projects using technical (commit count, distinct files touched, churn) and social (issue-comment count, distinct issues commented on) metrics.

Result: Identification of hero developers varies significantly across metrics, with a Jaccard overlap of only 0.10; technical heroes often have strong social activity, while social heroes exhibit less technical engagement.

Conclusion: Heroism is multifaceted and dependent on the chosen metrics, emphasizing the need for a comprehensive perspective in understanding contributors for better prioritization and bug assignment.
...
### [325] [The Grand Software Supply Chain of AI Systems](https://arxiv.org/abs/2604.27781)
*Carmine Cesarano,Martin Monperrus*

Main category: cs.SE

TL;DR: AI systems have low integrity, exposing vulnerabilities across their supply chain; this paper analyzes these weaknesses in four layers and highlights shortcomings in traditional mechanisms.


<details>
  <summary>Details</summary>
Motivation: To address the weaknesses in the current AI supply chain due to low integrity mechanisms that expose vulnerabilities throughout the AI lifecycle.

Method: The paper decomposes the AI supply chain into four architectural layers and identifies structural gaps related to verifiability, versioning, observability, and traceability.

Result: The analysis of 48 production-grade open-source projects reveals 4,664 direct dependencies and 11,508 transitive packages, totaling approximately 392M lines of code, highlighting the complexity of the AI software supply chain.

Conclusion: Current AI systems are deficient in critical aspects of supply chain integrity, necessitating improved mechanisms to enhance their resilience and reliability.
...
### [326] [Test Before You Deploy: Governing Updates in the LLM Supply Chain](https://arxiv.org/abs/2604.27789)
*Mohd Sameen Chishti,Damilare Peter Oyinloye,Jingyue Li*

Main category: cs.SE

TL;DR: This paper proposes a governance framework for managing compatibility in Large Language Models (LLMs) to address issues arising from unversioned provider updates, focusing on production contracts, risk-category-based testing, and compatibility gates.


<details>
  <summary>Details</summary>
Motivation: With the increasing reliance on LLMs in software systems, unversioned updates lead to behavioral drift and regressions which existing methods do not adequately manage.

Method: The proposed governance framework includes production contracts that define expected model behavior, risk-category-based testing to target specific deployment risks, and compatibility gates to enforce safety and performance standards before updates are released.

Result: Exploratory validation shows that targeted testing can reveal performance regressions that standard metrics might overlook, highlighting the importance of managing model updates.

Conclusion: The paper reframes LLM update management as a software supply chain governance challenge and identifies key research areas to enhance compatibility controls in LLM deployment.
...
### [327] [Feature-Centric Methodology for Analyzing Cross-Chain NFT Migration Compatibility](https://arxiv.org/abs/2604.27805)
*Mohd Sameen Chishti,Damilare Peter Oyinloye,Jingyue Li*

Main category: cs.SE

TL;DR: This paper addresses the challenges of cross-chain NFT migration due to architectural differences between blockchains, proposing a four-layer NFT architecture and a four-phase methodology for compatibility assessment.


<details>
  <summary>Details</summary>
Motivation: To understand and predict the compatibility of NFT features across different blockchain platforms during migration, preventing costly and unsuccessful transfer attempts.

Method: A four-phase migration analysis methodology is presented, including source feature specification, dependency mapping, target platform profiling, and compatibility assessment.

Result: The methodology was tested through a proof-of-concept analysis of NFT migration from Ethereum to Solana, revealing compatibility issues in the process.

Conclusion: The proposed framework aids in identifying which NFT features can be migrated successfully, which need redesign, and which are fundamentally incompatible.
...
### [328] [Requirements Debt in AI-Enabled Perception Systems Development: An Industrial RE4AI Perspective](https://arxiv.org/abs/2604.27825)
*Hina Saeeda,Soniya Abraham*

Main category: cs.SE

TL;DR: This study explores Requirements Debt (ReD) in AI-integrated automotive perception systems, highlighting how evolving requirements lead to technical debt and impact system reliability and certification readiness.


<details>
  <summary>Details</summary>
Motivation: To understand how evolving requirements in AI-enabled automotive systems accumulate Requirements Debt (ReD) and affect system lifecycle and quality.

Method: Conducted 16 semi-structured interviews with experts from 13 automotive companies and 3 research institutes, analyzed using thematic analysis.

Result: Identified mechanisms through which evolving functional and non-functional requirements create and propagate ReD, such as semantic drift and assurance lag.

Conclusion: Evolving requirements significantly impact auditability, reliability, and certification in safety-critical systems, underscoring the need for careful documentation and management of ReD.
...
### [329] [An Empirical Evaluation of Code Smell Detection in Angular Applications](https://arxiv.org/abs/2604.27893)
*Maykon Nunes,Emanuel Coutinho,Carla Bezerra,Ivan Machado*

Main category: cs.SE

TL;DR: This study identifies and catalogs Angular-specific code smells, develops an automated detection tool, and evaluates its effectiveness in detecting these smells.


<details>
  <summary>Details</summary>
Motivation: To address the challenges developers face in maintaining clean, modular code and to fill the gap in knowledge regarding Angular-specific code smells.

Method: Conducted a grey literature review to identify Angular code smells and developed an automated static analysis tool for detection, followed by empirical evaluation using a validated dataset.

Result: Identified 11 distinct Angular code smells; the tool demonstrated high detection performance with accuracy above 0.88 and F1-scores between 0.89 and 1.00.

Conclusion: The study establishes a catalog of Angular code smells and proves the tool's effectiveness, supporting future efforts to enhance front-end code quality.
...
### [330] [From Mirage to Grounding: Towards Reliable Multimodal Circuit-to-Verilog Code Generation](https://arxiv.org/abs/2604.27969)
*Guang Yang,Xing Hu,Xiang Chen,Xin Xi*

Main category: cs.SE

TL;DR: The paper investigates the reliability of multimodal large language models (MLLMs) in generating register-transfer-level (RTL) code from circuit diagrams, revealing a flaw termed 'Mirage' where model performance does not depend on visual inputs.


<details>
  <summary>Details</summary>
Motivation: To assess the reliability of MLLMs when translating circuit diagrams into RTL code, particularly the impact of visual input on code generation accuracy.

Method: The study introduces C2VEVAL, evaluating eight MLLMs under Normal and Anony modes (anonymized identifiers), to assess the effects of identifier semantics on performance.

Result: Findings show that under Anony mode, all MLLMs experienced a significant drop in performance, confirming that their high Normal-mode accuracy is misleading due to dependence on identifier semantics rather than visual grounding.

Conclusion: The proposed model, VeriGround, trained with new techniques, significantly improves performance, demonstrating true visual grounding and achieving competitive results with fewer parameters.
...
### [331] [Beyond Code, We Are People: A Systematic Mapping of 25 Years of Literature on Soft Skills in Agile Development Teams](https://arxiv.org/abs/2604.28101)
*Israely Lima,Lucas Moura Lourenço,Márcio Ribeiro,Ivan Machado,Carla Ilane Bezerra*

Main category: cs.SE

TL;DR: The paper systematically maps soft skills in software development, emphasizing their role in agile environments over the last 25 years.


<details>
  <summary>Details</summary>
Motivation: To address the gap in literature regarding essential non-technical skills in software development amidst the rise of agile methodologies.

Method: The study involves a systematic literature review of 97 studies from January 2000 to May 2025 to identify key soft skills and their relevance in agile software development.

Result: Identified key soft skills such as communication, adaptability, teamwork, and leadership, and noted their connection to various roles in agile contexts.

Conclusion: Findings highlight the need for integrating social and technical skills in professional development to enhance team effectiveness and innovation in software projects.
...
### [332] [DEFault++: Automated Fault Detection, Categorization, and Diagnosis for Transformer Architectures](https://arxiv.org/abs/2604.28118)
*Sigma Jahan,Saurabh Singh Rajput,Tushar Sharma,Mohammad Masudur Rahman*

Main category: cs.SE

TL;DR: DEFault++ is a diagnostic technique for identifying and classifying faults in transformer models, improving fault detection and repair action accuracy.


<details>
  <summary>Details</summary>
Motivation: To address silent faults in transformer models that degrade performance without raising errors, and to enhance fault diagnosis specifically for transformer components.

Method: DEFault++ employs a hierarchical approach that detects faults, classifies them into 12 categories, and identifies root causes from 45 mechanisms. It includes a benchmark dataset (DEFault-bench) created from mutation testing across multiple transformer models and tasks, utilizing a Fault Propagation Graph for analysis.

Result: DEFault++ achieved over 0.96 AUROC for detection and 0.85 Macro-F1 for categorization and root-cause diagnosis on the DEFault-bench. Repair action accuracy improved from 57.1% to 83.3% with DEFault++.

Conclusion: The proposed DEFault++ improves diagnostic capabilities for transformer models, aiding developers in accurately addressing faults and enhancing overall AI system reliability.
...
### [333] [Claw-Eval-Live: A Live Agent Benchmark for Evolving Real-World Workflows](https://arxiv.org/abs/2604.28139)
...
TL;DR: Claw-Eval-Live is a benchmark for evaluating LLM agents in workflow automation, focusing on real-time demand and verifiable execution.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing evaluation benchmarks that do not adapt to evolving tasks or verify execution effectiveness.

Method: Claw-Eval-Live utilizes a dual-layered approach with a refreshable signal layer for current workflow demands and a fixed snapshot for task execution evaluation, incorporating detailed logging and structured criteria for assessing agent performance.

Result: The leading model achieved a task completion rate of 66.7%, highlighting ongoing challenges in workflow automation, particularly in HR and management tasks.

Conclusion: For effective evaluation of workflow agents, methods must consider both current demand and the reliability of agent actions, emphasizing the need for continuous improvement and adaptability.
...
### [334] [Unsafe and Unused? A History of Utility Code in Mature Open Source Projects](https://arxiv.org/abs/2604.28146)
*Brandon Keller,Kaitlin Yandik,Angela Ngo,Andy Meneely*

Main category: cs.SE

TL;DR: This study investigates the impact of 'util' files in source code across seven open source projects to determine their usage, complexity, and potential link to security vulnerabilities over time.


<details>
  <summary>Details</summary>
Motivation: The research aims to examine how the use of 'util' files, typically intended for reusability and reducing duplicate code in software development, affects project evolution and developer collaboration.

Method: A longitudinal mining study was performed on Git repositories of seven long-standing open source projects, analyzing util file usage, developer interactions, and security across 1773 snapshots at 30-day intervals.

Result: The findings indicate that util files can be significantly linked to vulnerabilities, with util files being 2.75 times more likely to be involved in security issues as compared to non-util files.

Conclusion: The persistence and commonality of util files across projects highlight a need for awareness among developers to prevent the creation of unsafe and unused util files.
...
### [335] [A Randomized Controlled Trial and Pilot of Scout: an LLM-Based EHR Search and Synthesis Platform](https://arxiv.org/abs/2604.26953)
...
TL;DR: Scout, an LLM-based EHR search platform, reduces clinician workload by enabling natural language queries of EHR data, showing reduced task completion time and perceived workload without compromising output quality in a trial with 20 participants.


<details>
  <summary>Details</summary>
Motivation: To alleviate clinician workload and burnout caused by extensive clinical documentation and data retrieval in EHRs.

Method: A prospective randomized, evaluator-blinded crossover trial was conducted involving 20 participants across seven clinical specialties, comparing tasks completed using Scout versus the EHR alone. Metrics included time to completion, workload scores, and expert evaluations of accuracy and relevance.

Result: Scout reduced task completion time by 37.6% and significantly decreased perceived workload, particularly in mental demand, effort, and temporal demand, without sacrificing task quality.

Conclusion: LLM-powered EHR tools like Scout can effectively reduce clinical workloads while maintaining high-quality output, demonstrating the need for human validation in automated evaluations.
...
### [336] [AgenticRecTune: Multi-Agent with Self-Evolving Skillhub for Recommendation System Optimization](https://arxiv.org/abs/2604.26969)
*Xidong Wu,Yue Zhuan,Ruoqiao Wei,Hangxin Chen,Di Bai,Jintao Liu,Xinyi Wang,Xue Wang,Luoshu Wang,Xinwu Cheng*

Main category: cs.IR

TL;DR: This paper presents AgenticRecTune, a framework for optimizing configurations in large-scale recommendation systems through specialized agents and Large Language Models.


<details>
  <summary>Details</summary>
Motivation: To address the complexity and importance of system-level configuration optimization in multi-stage recommendation systems, which traditional methods have overlooked.

Method: AgenticRecTune introduces five agents (Actor, Critic, Insight, Skill, Online) to manage configuration optimization, utilizing advanced reasoning from LLMs to explore optimal configurations.

Result: The framework streamlines the configuration process by enabling A/B testing and iterative learning, enhancing recommendation system performance through collaboration among agents.

Conclusion: AgenticRecTune effectively tackles the challenges of configuration optimization, improving the efficiency of multi-stage recommendation systems.
...
### [337] [Not All Memories Age the Same: Autodiscovery of Adaptive Decay in Knowledge Graphs](https://arxiv.org/abs/2604.26970)
*Mandar Karhade*

Main category: cs.IR

TL;DR: The paper critiques uniform decay in knowledge graph retrieval, proposing a hierarchical continuous decay framework that accounts for varying temporal dynamics of knowledge types, improving retrieval accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the inadequacy of uniform decay methods in temporal knowledge retrieval, which fail to recognize the differing dynamics of various knowledge types.

Method: A hierarchical framework is introduced, utilizing a continuous decay surface characterized by velocity and volatility to personalize decay for specific subjects, optimized through survival analysis of value lifetimes.

Result: Experiments reveal that the proposed method outperforms uniform decay significantly, with improvements shown in recovery of hierarchical parameters and validation on real-world datasets.

Conclusion: The hierarchical decay model enhances knowledge retrieval in temporal graphs, reflecting natural persistence patterns and leading to improved performance compared to uniform decay.
...
### [338] [T2S-Metrics: Unified Library for Evaluating SPARQL Queries Generated From Natural Language](https://arxiv.org/abs/2604.26971)
*Yousouf Taghzouti,Tao Jiang,Camille Juigné,Benjamin Navet,Fabien Gandon,Franck Michel,Louis-Felix Nothias*

Main category: cs.IR

TL;DR: This paper introduces t2s-metrics, a comprehensive open-source library for evaluating SPARQL query systems, addressing the challenges of fragmentation and inconsistency in QA evaluations over Knowledge Graphs.


<details>
  <summary>Details</summary>
Motivation: To provide a systematic and standardized evaluation framework for QA systems that overcomes existing methodological limitations and facilitates reproducibility.

Method: Development of t2s-metrics, an extensible evaluation library with over 20 metrics that cover various dimensions like lexical, syntactic, semantic, execution-based, and ranking-based assessments.

Result: Presentation of a broad set of metrics inspired by previous evaluation standards, enhancing the ability to evaluate SPARQL queries comprehensively.

Conclusion: t2s-metrics is essential for consistent evaluation of QA systems, promoting transparency and deeper understanding of system performance beyond just answer accuracy.
...
### [339] [Budget-Constrained Online Retrieval-Augmented Generation: The Chunk-as-a-Service Model](https://arxiv.org/abs/2604.26981)
*Shawqi Al-Maliki,Ammar Gharaibeh,Mohamed Rahouti,Mohammad Ruhul Amin,Mohamed Abdallah,Junaid Qadir,Ala Al-Fuqaha*

Main category: cs.IR

TL;DR: The study introduces Chunk-as-a-Service (CaaS) to address limitations of Retrieval-Augmented Generation (RAG), particularly RAG-as-a-Service (RaaS), in terms of transparency and cost-effectiveness.


<details>
  <summary>Details</summary>
Motivation: To provide a more reliable and cost-effective alternative for enriching prompts in language models, overcoming the shortcomings of existing RAG implementations.

Method: CaaS, with two variants (OB-CaaS and LB-CaaS), utilizes a Utility-Cost Online Selection Algorithm (UCOSA) to enhance prompt relevance based on real-time budget constraints.

Result: UCOSA improves the number of enriched prompts by 52% over random selection, achieving 75% of the performance of offline methods, while also enhancing budget utilization.

Conclusion: CaaS offers superior efficiency and cost-effectiveness compared to RaaS, as evidenced by improved performance-to-budget ratios.
...
### [340] [Value-Aware Product Recommendation by Customer Segmentation using a suitable High-Dimensional Similarity Measure](https://arxiv.org/abs/2604.26983)
*María Florencia Acosta,Rodrigo García Arancibia,Pamela Llop,Mariel Lovatto,Lucas Mansilla*

Main category: cs.IR

TL;DR: This paper introduces a revenue-focused approach to product recommendation that addresses user-item data challenges by segmenting users based on revenue contributions.


<details>
  <summary>Details</summary>
Motivation: To improve recommendations by considering the contribution of users and products to overall sales revenue in high-dimensional and sparse datasets.

Method: The method involves encoding revenue contributions in a user-item matrix and computing customer similarity using tailored distance measures based on these contributions.

Result: The framework allows for user segmentation according to revenue-based similarity and offers three revenue-oriented recommendation strategies which were validated through simulations and real-world data.

Conclusion: The proposed approach enhances the effectiveness of product recommendations by aligning them with profitability goals, as demonstrated by experiments using the UCI Online Retail dataset.
...
### [341] [LUCid: Redefining Relevance For Lifelong Personalization](https://arxiv.org/abs/2604.26996)
*Chimaobi Okite,Anika Misra,Joyce Chai,Rada Mihalcea*

Main category: cs.IR

TL;DR: This paper presents LUCid, a benchmark to evaluate situational user-centric relevance in personalization, highlighting shortcomings of current approaches when retrieving context from semantically distant interactions.


<details>
  <summary>Details</summary>
Motivation: Current personalization methods often overlook crucial user information from unrelated interactions due to their reliance on semantic proximity.

Method: LUCid is introduced as a benchmark containing 1,936 practical queries and interaction histories from up to 500 sessions, designed to measure the effectiveness of models in retrieving relevant user context.

Result: Experiments reveal significant performance issues, with retrieval recall nearly dropping to zero for distant history contexts in state-of-the-art models like Gemini-3-Flash and GPT-5.4.

Conclusion: The findings indicate a fundamental misalignment between the relevance captured by existing systems and the situational relevance needed for effective personalization, underscoring the importance of LUCid in advancing personalized user experiences.
...
### [342] [Hypencoder Revisited: Reproducibility and Analysis of Non-Linear Scoring for First-Stage Retrieval](https://arxiv.org/abs/2604.27037)
*Arne Eichholtz,Yongkang Li,Jutte Vijverberg,Tobias Groot,Mohammad Aliannejadi*

Main category: cs.IR

TL;DR: The Hypencoder framework proposed by Killingback et al. enhances retrieval performance by replacing fixed scoring with a query-specific neural network, and this study confirms its superiority over a bi-encoder baseline while exploring extensions and limitations.


<details>
  <summary>Details</summary>
Motivation: To evaluate the effectiveness of the Hypencoder compared to traditional bi-encoder frameworks and to investigate potential extensions and their impacts on retrieval performance and efficiency.

Method: Reproduction of the Hypencoder framework and extensive comparison against a bi-encoder baseline on various benchmarks, analysis of latency, exploration of alternative encoders, and robustness evaluation.

Result: The Hypencoder outperforms the bi-encoder in several benchmarks with improved search efficiency; however, it exhibits inconsistent performance across all hard retrieval tasks and configurations.

Conclusion: While the Hypencoder shows promising advancements, it requires careful consideration of training strategies and performance trade-offs, particularly in terms of query latency and robustness.
...
### [343] [A Gated Hybrid Contrastive Collaborative Filtering Recommendation](https://arxiv.org/abs/2604.27117)
...
TL;DR: The paper introduces a Gated Hybrid Collaborative Filtering framework, enhancing recommendation systems by integrating textual reviews with a focus on improving ranking rather than just rating prediction.


<details>
  <summary>Details</summary>
Motivation: To improve top-N recommendation effectiveness by addressing the misalignment between review-aware models and ranking quality.

Method: The framework employs an autoencoder-based model with an adaptive gating mechanism and introduces a contrastive learning module to refine the latent space, all while optimizing for a pairwise Bayesian personalized ranking objective.

Result: The proposed model demonstrates significant improvements in hit rate @10 and normalized discounted cumulative gain @10 across various datasets compared to state-of-the-art review-aware models.

Conclusion: Controlled semantic fusion is crucial for enhancing the ranking-driven recommendation performance of models.
...
### [344] [LLM-Enhanced Topical Trend Detection at Snapchat](https://arxiv.org/abs/2604.27131)
*Hangqi Zhao,Jay Li,Abhiruchi Bhattacharya,Cong Ni,Jason Yeung,Jinchao Ye,Kai Yang,Akshat Malu,Manish Malik*

Main category: cs.IR

TL;DR: A novel large-scale system for detecting topical trends on Snapchat is presented, utilizing multimodal topic extraction and LLM-based techniques to enhance trend discovery.


<details>
  <summary>Details</summary>
Motivation: The need for an effective system to monitor and identify emerging trends in the dynamic environment of social media platforms like Snapchat.

Method: The system employs multimodal topic extraction, time-series burst detection, and LLM integration to identify and enrich topical trends.

Result: The system achieved high precision in trend detection through continuous offline evaluation and has been deployed successfully at a global scale, impacting content ranking and search functionalities.

Conclusion: The deployment of the system has led to measurable enhancements in content relevance and user experience on the platform.
...
### [345] [RAQG-QPP: Query Performance Prediction with Retrieved Query Variants and Retrieval Augmented Query Generation](https://arxiv.org/abs/2604.27244)
*Fangzheng Tian,Debasis Ganguly,Craig Macdonald*

Main category: cs.IR

TL;DR: This paper presents a new method for improving Query Performance Prediction (QPP) by utilizing query variants (QVs) from past queries and enhancing them with large language models (LLMs), resulting in significant performance gains for neural rankers.


<details>
  <summary>Details</summary>
Motivation: The need for effective Query Performance Prediction (QPP) without human relevance judgments, particularly for neural ranking models which struggle with current methods reliant on term expansion for query variant generation.

Method: Utilizing past queries as QVs for QPP and enhancing these with LLM-generated variants to improve predictive accuracy.

Result: The proposed method significantly outperformed existing QV-based approaches by up to 30% on neural ranking models like MonoT5 in experiments conducted on TREC DL'19 and DL'20.

Conclusion: Leveraging past queries and LLMs provides a robust solution to enhance QPP accuracy, showcasing improved performance in neural ranking.
...
### [346] [NuggetIndex: Governed Atomic Retrieval for Maintainable RAG](https://arxiv.org/abs/2604.27306)
*Saber Zerhoudi,Michael Granitzer,Jelena Mitrovic*

Main category: cs.IR

TL;DR: NuggetIndex improves retrieval in RAG systems by using atomic information units, enhancing accuracy and efficiency while addressing data validity and conflict issues.


<details>
  <summary>Details</summary>
Motivation: Standard retrieval systems struggle with evolving corpora and outdated information, leading to evaluation mismatches and ineffective maintenance.

Method: NuggetIndex utilizes atomic information units, or nuggets, which contain evidence links, validity intervals, and lifecycle states to filter outdated data before retrieval.

Result: NuggetIndex shows a 42% improvement in nugget recall, a 9% increase in temporal correctness, and a 55% reduction in conflict rates compared to traditional baselines.

Conclusion: NuggetIndex offers a more efficient and reliable approach to RAG systems, reducing input length significantly while ensuring data relevance and accuracy.
...
### [347] [From Unstructured to Structured: LLM-Guided Attribute Graphs for Entity Search and Ranking](https://arxiv.org/abs/2604.27410)
*Yilun Zhu,Nikhita Vedula,Shervin Malmasi*

Main category: cs.IR

TL;DR: This paper presents a two-stage entity search approach in e-commerce, enhancing product similarity ranking by utilizing a Large Language Model-driven attribute graph.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of capturing context-specific product similarity in e-commerce through traditional methods.

Method: The proposed method consists of an offline stage for extracting structured attributes from unstructured text to create a category-aware attribute graph, followed by an online ranking stage leveraging this structured graph.

Result: The method reduces per-product token usage by 57% and improves ranking precision, achieving over a 5% increase in average precision in zero-shot scenarios without training data.

Conclusion: The approach generalizes robustly across product categories and shows strong potential for practical applications in e-commerce.
...
### [348] [A Reproducibility Study of LLM-Based Query Reformulation](https://arxiv.org/abs/2604.27421)
*Amin Bigdeli,Radin Hamidi Rad,Hai Son Le,Mert Incesu,Negar Arabzadeh,Charles L. A. Clarke,Ebrahim Bagheri*

Main category: cs.IR

TL;DR: This study presents a systematic evaluation of ten LLM-based query reformulation methods to assess reproducibility and effectiveness in information retrieval.


<details>
  <summary>Details</summary>
Motivation: The investigation addresses the variation in effectiveness of query reformulation methods used under differing experimental conditions, aiming to clarify reproducibility and stability of prior findings.

Method: A comparative study evaluating ten query reformulation methods across two LLM families, three retrieval paradigms, and nine benchmark datasets within a controlled experimental framework.

Result: The results indicate that performance gains from reformulation are dependent on the retrieval paradigm, with lexical improvements not consistently benefiting neural retrievers, and larger LLMs not always improving downstream results.

Conclusion: The study clarifies the conditions under which LLM-based query reformulation is effective and provides tools for transparent replication and comparison through the QueryGym toolkit.
...
### [349] [Reproducing Adaptive Reranking for Reasoning-Intensive IR](https://arxiv.org/abs/2604.27577)
*Mandeep Rathee,V Venktesh,Sean MacAvaney,Avishek Anand*

Main category: cs.IR

TL;DR: This paper addresses the bounded recall issue in the retrieve--rerank pipeline by applying the Graph-based Adaptive Reranking (GAR) method to reasoning-intensive retrieval tasks, demonstrating its effectiveness with minimal computational costs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to solve the bounded recall problem in retrieval systems, particularly for queries requiring complex reasoning, without incurring high training and inference costs.

Method: The paper reproduces GAR in reasoning-intensive contexts, leveraging iterative exploration of a corpus graph for reranking rather than improving the first-stage retriever.

Result: The research finds that GAR enhances the effectiveness of reasoning-intensive retrieval across various models while maintaining low computational overhead.

Conclusion: The study concludes that GAR enables more efficient retrieval systems capable of handling reasoning-intensive queries.
...
### [350] [One Pass, Any Order: Position-Invariant Listwise Reranking for LLM-Based Recommendation](https://arxiv.org/abs/2604.27599)
*Ethan Bito,Yongli Ren,Estrid He*

Main category: cs.IR

TL;DR: InvariRank is a framework designed to improve the reliability of language models in recommendation reranking by eliminating order sensitivity inherent in traditional listwise models.


<details>
  <summary>Details</summary>
Motivation: The primary motivation is to resolve order sensitivity in LLMs used for recommendation reranking, where the final ranking can be affected by the order of presented candidates.

Method: InvariRank employs a permutation-invariant architecture using a structured attention mask to block cross-candidate attention and applies shared positional framing through Rotary Positional Embeddings.

Result: Experiments demonstrate that InvariRank consistently produces stable rankings across different candidate permutations while maintaining competitive effectiveness in ranking tasks.

Conclusion: The findings indicate that enhancing architectural invariance is an effective approach to achieving reliable and efficient recommendation reranking with LLMs.
...
### [351] [Purifying Multimodal Retrieval: Fragment-Level Evidence Selection for RAG](https://arxiv.org/abs/2604.27600)
*Xihang Wang,Zihan Wang,Chengkai Huang,Cao Liu,Ke Zeng,Quan Z. Sheng,Lina Yao*

Main category: cs.IR

TL;DR: FES-RAG enhances MRAG by selecting precise multimodal fragments instead of entire documents, improving performance and coherence in MLLMs.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiency of treating entire documents as uniform evidence in multimodal retrieval systems, which leads to noise and performance issues.

Method: FES-RAG decomposes documents into sentence-level and region-level fragments, using a metric called Fragment Information Gain (FIG) for selection based on relevance to the query.

Result: FES-RAG outperforms existing document-level MRAG methods on the M2RAG benchmark, with a notable 27% improvement in CIDEr scores.

Conclusion: By optimizing fragment selection, FES-RAG enhances generation accuracy and coherence while reducing the contextual burden in MLLMs.
...
### [352] [Position-Aware Drafting for Inference Acceleration in LLM-Based Generative List-Wise Recommendation](https://arxiv.org/abs/2604.27747)
*Jiaju Chen,Chongming Gao,Chenxiao Fan,Haoyan Liu,Qingpeng Cai,Peng Jiang,Xiangnan He*

Main category: cs.IR

TL;DR: PAD-Rec enhances speculative decoding in LLM-based generative recommendations by incorporating position embeddings to improve inference speed and proposal quality while maintaining recommendation accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the latency issues in sequential decoding for LLM-based generative recommendations, and to improve the efficacy of speculative decoding by considering token semantics and uncertainty.

Method: Introduced PAD-Rec, which uses item and step position embeddings to enrich the draft model with structural awareness and depth-related uncertainty, supported by learnable gates for integration with standard draft models.

Result: Achieved up to 3.1x wall-clock speedup and 5% average improvement over SD baselines while maintaining recommendation quality across four real-world datasets.

Conclusion: PAD-Rec demonstrates significant improvements in inference speed without compromising the quality of recommendations, confirming its effectiveness in generative list-wise recommendations.
...
### [353] [How Generative AI Disrupts Search: An Empirical Study of Google Search, Gemini, and AI Overviews](https://arxiv.org/abs/2604.27790)
*Riley Grossman,Songjiang Liu,Michael K. Chen,Mike Smith,Cristian Borcea,Yi Chen*

Main category: cs.IR

TL;DR: The paper investigates the impact of generative AI on web search, using a benchmark dataset to analyze differences in search results from traditional search engines and AI-based ones. Key findings reveal significant discrepancies in source retrieval and presentation between these two types of search engines, with implications for website visibility and content management.


<details>
  <summary>Details</summary>
Motivation: To understand how generative AI alters information retrieval and presentation in web search compared to traditional search engines.

Method: The study utilizes a public benchmark dataset of 11,500 user queries and compares results from Google's traditional engine and generative AI tools like AIO and Gemini Flash 2.5.

Result: Findings include that AIO results appear above organic results for over half of queries, substantial differences in retrieved sources, lower consistency in AIO results, and the impact of blocking AI crawlers on visibility.

Conclusion: The study highlights crucial implications for website visibility and calls for sustainable revenue models benefiting both publishers and generative search providers.
...
### [354] [NeocorRAG: Less Irrelevant Information, More Explicit Evidence, and More Effective Recall via Evidence Chains](https://arxiv.org/abs/2604.27852)
*Shiyao Peng,Qianhe Zheng,Zhuodi Hao,Zichen Tang,Rongjin Li,Qing Huang,Jiayu Huang,Jiacheng Liu,Yifan Zhu,Haihong E*

Main category: cs.IR

TL;DR: The paper introduces the Recall Conversion Rate (RCR) metric to analyze the relationship between retrieval performance and downstream reasoning accuracy in Retrieval-Augmented Generation (RAG), revealing significant gaps and proposing the NeocorRAG framework for improved retrieval quality optimization.


<details>
  <summary>Details</summary>
Motivation: To address the disconnect between retrieval performance and reasoning accuracy in RAG methods, and the need for a comprehensive understanding of retrieval quality optimization.

Method: Introduction of Recall Conversion Rate (RCR) for evaluation, followed by the development of the NeocorRAG framework, which utilizes Evidence Chains and an activated search algorithm for retrieval quality optimization.

Result: NeocorRAG demonstrated state-of-the-art performance on multiple benchmarks (HotpotQA, 2WikiMultiHopQA, MuSiQue, NQ) while using significantly fewer tokens than comparable methods.

Conclusion: The study provides a training-free approach to enhancing RAG through improved retrieval quality, indicating that efficient optimization can lead to better reasoning outcomes.
...
### [355] [SimEval-IR: A Unified Toolkit and Benchmark Suite for Evaluating User Simulators and Search Sessions](https://arxiv.org/abs/2604.27878)
*Saber Zerhoudi*

Main category: cs.IR

TL;DR: This paper introduces SimEval-IR, an open-source toolkit to evaluate user simulators in interactive information retrieval, addressing behavioral realism and tester reliability.


<details>
  <summary>Details</summary>
Motivation: The lack of standardized evaluation tools for user simulators in interactive information retrieval, despite their growing importance.

Method: SimEval-IR offers a common session schema, three benchmarks for measuring behavioral realism, tester reliability, and their interrelation, along with baseline results from various datasets.

Result: The study reveals that the commonly used 'human-likeness' test has low predictive power for system-ranking validity, while alternative metrics like click-depth distance are more effective.

Conclusion: SimEval-IR provides valuable resources for evaluating user simulators and encourages further research on effective measures of realism and reliability.
...
### [356] [Efficient Multivector Retrieval with Token-Aware Clustering and Hierarchical Indexing](https://arxiv.org/abs/2604.28142)
*Silvio Martinico,Franco Maria Nardini,Cosimo Rulli,Rossano Venturini*

Main category: cs.IR

TL;DR: TACHIOM is a novel multivector retrieval system that accelerates clustering and retrieval using token-level structures, outperforming traditional k-means in speed and effectiveness.


<details>
  <summary>Details</summary>
Motivation: Existing multivector retrieval models have high computational and memory costs, and traditional k-means clustering underrepresents rare tokens while scaling poorly with data.

Method: TACHIOM employs a refined clustering method that leverages token distribution for centroid allocation, utilizes a graph-based index over centroids, and incorporates an optimized Product Quantization layout.

Result: TACHIOM demonstrates up to 247 times faster clustering and up to 9.8 times faster retrieval compared to state-of-the-art systems, with maintained accuracy.

Conclusion: TACHIOM effectively improves the speed and efficiency of multivector retrieval systems without sacrificing performance.
...
### [357] [Impact of Background Dense Multipath Components on Multi-Band Fusion ISAC Systems](https://arxiv.org/abs/2604.27265)
*Dexin Wang,Roberto Bomfin,Ahmad Bazzi,Marwa Chafii*

Main category: eess.SP

TL;DR: This paper investigates the impact of dense multipath components (DMC) on multi-band sensing in integrated sensing and communication (ISAC) systems, presenting a multi-band estimator that enhances performance in challenging environments.


<details>
  <summary>Details</summary>
Motivation: To address the significance of multi-band sensing in ISAC under the influence of dense multipath components (DMC), particularly in the context of the emerging IMT-2030 (6G) standards.

Method: The paper employs Cramér-Rao bound (CRB) analysis to explore fundamental limits and assesses the performance of a proposed multi-band estimator in the presence of DMC.

Result: The findings indicate that multi-band processing can significantly lower estimation errors and improve system robustness, particularly in environments affected by varying channel statistics and DMC.

Conclusion: Multi-band sensing is critical for effective ISAC operations in 6G, and the proposed estimator enhances performance in DMC-dominated scenarios.
...
### [358] [Array Zooming Optimization for Near-Field Localization With Movable Antennas](https://arxiv.org/abs/2604.27352)
*Yuxin Duan,Boyu Teng,Xiaojun Yuan,Rui Wang*

Main category: eess.SP

TL;DR: This paper presents a solution to enhance near-field localization accuracy by using a movable antenna system that adjusts antenna spacings to mitigate spatial aliasing while ensuring high resolution.


<details>
  <summary>Details</summary>
Motivation: The need for high-resolution, low-cost near-field localization has increased, necessitating the development of a solution that addresses spatial ambiguity introduced by sparse antenna arrays.

Method: The proposed method is a multi-measurement array zooming system that dynamically modifies antenna spacings and utilizes multiple observational data to reduce spatial aliasing and improve accuracy.

Result: The approach effectively reduces localization errors in the presence of multi-modal distributions, showing significant improvements over traditional fixed-spacing arrays and existing benchmarks.

Conclusion: The adaptive AZ strategy successfully optimizes antenna configurations across varying signal-to-noise ratios, confirming its effectiveness and robustness in enhancing localization performance.
...
### [359] [Sensing-Assisted Channel Estimation for Flexible-Antenna Systems: A Unified Framework](https://arxiv.org/abs/2604.27626)
*Ruoxiao Cao,Wentao Yu,Zixin Wang,Shenghui Song,Jun Zhang,Yi Gong,Khaled B. Letaief*

Main category: eess.SP

TL;DR: The paper presents a sensing-assisted channel estimation framework for flexible-antenna systems in 6G networks, addressing the challenge of high pilot overhead in acquiring channel state information (CSI) by employing a two-stage process and developing two tailored Newton-MUSIC algorithms for different propagation environments.


<details>
  <summary>Details</summary>
Motivation: The need for hardware-efficient flexible-antenna systems in 6G networks and the challenge posed by pilot overhead in acquiring accurate channel state information (CSI).

Method: A unified two-stage channel estimation framework is proposed, first identifying dominant directions of arrival (DOAs) from uplink data symbols without dedicated sensing pilots, followed by calibrating path gains using minimal calibration pilots; two variations of Newton-MUSIC algorithms are introduced for different propagation conditions.

Result: The SOC-Newton-MUSIC algorithm effectively estimates DOAs in line-of-sight (LOS) environments with reduced complexity, while the FOC-Newton-MUSIC algorithm enhances source identifiability and spatial degrees of freedom in non-line-of-sight (NLOS) environments with multipath propagation.

Conclusion: The proposed framework significantly reduces pilot overhead and improves channel estimation accuracy in flexible-antenna systems for 6G networks by transforming the spatial spectrum search into a continuous optimization problem.
...
### [360] [Semantics-Aware Hierarchical Token Communication: Clustering, Bit Mapping, and Power Allocation](https://arxiv.org/abs/2604.27641)
*Jihoon Lee,Seungeun Oh,Jihong Park,Seong-Lyun Kim,Seung-Woo Ko*

Main category: eess.SP

TL;DR: H-TokCom introduces a hierarchical framework for token communication that enhances semantic integrity through improved physical-layer designs. By clustering semantically similar tokens and emphasizing prefix bits, it reduces semantic loss, especially at low SNRs, showing significant semantic similarity gains in simulations.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of existing AI-centric designs in TokCom which fail to incorporate token-level semantics into physical-layer designs, leading to semantic loss at low SNR.

Method: H-TokCom embeds semantic structure into physical-layer design by clustering similar tokens and using a hierarchical bit representation with prefix and suffix components, optimizing power allocation for better error resilience.

Result: H-TokCom demonstrates improved semantic similarity, achieving an increase from 0.206 to 0.279 at 3 dB SNR on the COCO dataset, indicating a 35.4% improvement in semantic accuracy.

Conclusion: The framework effectively enhances semantic preservation in TokCom, particularly in environments with low signal quality, thereby advancing the efficiency of token communication.
...
### [361] [Bitwise Over-Parameterized Neural Polar Decoding: A Theoretical Performance Analysis](https://arxiv.org/abs/2604.27689)
*Hongzhi Zhu,Wei Xu,Xiaohu You*

Main category: eess.SP

TL;DR: The paper introduces a bitwise over-parameterized neural network (ONN) decoder for polar-coded transmission, analyzing its performance and convergence through a theoretical framework.


<details>
  <summary>Details</summary>
Motivation: To enhance polar-coded transmission by integrating neural network learning with polar code reliability analysis while preserving the structure of polar decoding.

Method: The approach involves modeling each message channel as a supervised regression task and analyzing convergence behavior, training dynamics, empirical MSE bounds, and bitwise MAP decoding error probabilities using Gaussian approximations under AWGN channels.

Result: The derived theoretical bounds indicate that the width of hidden layers substantially impacts optimization and decoding performance, with numerical evidence supporting these findings.

Conclusion: Increasing network width enhances both theoretical performance and practical decoding outcomes, providing insights for optimal network architecture in polar-coded systems.
...
### [362] [On the Fractional Fourier Transform for FMCW Radar Interference Mitigation](https://arxiv.org/abs/2604.27768)
*Christian Oswald,Josef Kulmer,Franz Pernkopf*

Main category: eess.SP

TL;DR: The paper enhances FMCW radar mutual interference mitigation using a discrete fractional Fourier transform, improving real-valued receiver processing while reducing computational complexity and demonstrating practical applicability.


<details>
  <summary>Details</summary>
Motivation: To improve mutual interference mitigation in FMCW radar systems, making it efficient and applicable in real-world scenarios.

Method: The authors extend their previous DFrFT-based method, propose a new radar signal processing chain, and reduce complexity by using sparse update signals for simultaneous interference mitigation.

Result: The proposed method shows improved performance on synthetic data and is validated with real-world measurement data, demonstrating its effectiveness in practical environments.

Conclusion: The method offers a viable solution for FMCW radar interference issues, combining efficiency with compatibility in real applications.
...
### [363] [CRS-LLM: Cooperative Beam Prediction with a GPT-Style Backbone and Switch-Gated Fusion](https://arxiv.org/abs/2604.27945)
*Fangzhi Li,Cunhua Pan,Hong Ren,Dongming Wang,Jiangzhou Wang*

Main category: eess.SP

TL;DR: The paper presents CRS-LLM, a cooperative framework for beam prediction in V2X scenarios, addressing challenges in beam tracking due to mobility and blockage.


<details>
  <summary>Details</summary>
Motivation: To improve beam tracking accuracy in fast-moving and obstructed vehicle-to-everything communication environments by overcoming limitations of conventional methods that can lead to error propagation.

Method: CRS-LLM approaches beam prediction as a single classification challenge over the joint base station-beam space, utilizing a dual-view CSI tokenizer with a lightweight CNN and a GPT-style backbone for temporal modeling. It also incorporates a transition-aware switch-gated predictor for managing both gradual and sudden changes in beam state.

Result: Simulation results indicate CRS-LLM achieves superior performance in Top-1 accuracy and normalized beam gain compared to CSI-Transformer and other CNN/RNN baselines across varying SNR conditions, with notable few-shot and zero-shot capabilities.

Conclusion: The proposed framework effectively enhances beam tracking in dynamic environments, indicating its potential for real-time application in cooperative multi-BS systems.
...
### [364] [LiDAR-based Dynamic Blockage Prediction: A Data-driven Approach for Learning Interactive Bayesian Models](https://arxiv.org/abs/2604.28040)
*Saleemullah Memon,Ali Krayani,Pamela Zontone,Lucio Marcenaro,David Martin Gomez,Carlo Regazzoni*

Main category: eess.SP

TL;DR: This paper presents an interactive generalized dynamic Bayesian network for predicting LiDAR sensor blockages in vehicles, enhancing awareness and adaptability in transportation systems.


<details>
  <summary>Details</summary>
Motivation: The study aims to improve safety and sustainability in smart cities and autonomous systems by addressing LiDAR sensor blockages that could hinder vehicular intelligence.

Method: The approach involves training separate generalized dynamic Bayesian network models for different vehicles in both normal and blockage conditions, along with forming a high-level vocabulary for vehicle interaction. Testing employs the best generative model (normal or blockage) and utilizes an interactive Markov jump particle filter for processing the probabilistic information.

Result: The proposed model enables more effective prediction of LiDAR sensor blockages and supports anomaly detection, thereby enhancing self-awareness and explainability in vehicular systems.

Conclusion: The interactive I-GDBN model improves adaptability to blockage scenarios and is beneficial for instances where sensor data is unavailable.
...
### [365] [Experimental Performance of a 5G N78 Reconfigurable Intelligent Surface: From Controlled Measurements to Commercial Network Deployment](https://arxiv.org/abs/2604.28044)
*Sefa Kayraklık,Samed Keşir,Batuhan Kaplan,Ahmet Muaz Aktaş,Emre Arslan,Ahmet Faruk Coskun*

Main category: eess.SP

TL;DR: This paper analyzes a modular reconfigurable intelligent surface (RIS) prototype for 5G, validating its effectiveness in real-world scenarios through measurements and a commercial network deployment.


<details>
  <summary>Details</summary>
Motivation: To investigate the practical applicability of RIS in enhancing 5G coverage, particularly in non-line-of-sight conditions, moving beyond simulation-based studies.

Method: The analysis involved a three-phase experimental validation: indoor measurements, outdoor long-range tests, and deployment on a live 5G standalone network, focusing on coverage improvement in identified NLoS areas.

Result: The RIS implementation resulted in significant improvements in Reference Signal Received Power (RSRP) and Signal to Interference plus Noise Ratio (SINR), successfully restoring 5G service in previously inaccessible locations.

Conclusion: The findings underscore the potential of RIS technology to enhance coverage in operational 5G networks, affirming its viability for real-world applications.
...
### [366] [Sequential Inference for Gaussian Processes: A Signal Processing Perspective](https://arxiv.org/abs/2604.28163)
*Daniel Waxman,Fernando Llorente,Petar M. Djurić*

Main category: eess.SP

TL;DR: The paper discusses the integration of machine learning models, particularly Gaussian processes, into signal processing, focusing on sequential inference and its applications.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the methodological shift in signal processing due to the rise of machine learning models and the need for effective sequential inference techniques.

Method: A tutorial overview of Gaussian processes is provided, emphasizing recent advancements in sequential inference relevant to signal processing applications.

Result: The overview includes practical tools and techniques for implementing sequential Gaussian processes across various applications in signal processing.

Conclusion: The authors aim to equip practitioners with the necessary knowledge and resources to utilize sequential GP models effectively in real-world signal processing systems.
...
### [367] [Graphify: Automated Synthesis of Type-Safe Graph Backends via $O(S)$ GraphQL-to-Gremlin Transpilation](https://arxiv.org/abs/2604.27223)
*Johannes Graf*

Main category: cs.DB

TL;DR: This paper presents Graphify, a framework that allows developers to visually model graph schemas and automatically create a type-safe backend, addressing the challenges of graph database management.


<details>
  <summary>Details</summary>
Motivation: The need for efficient handling of interconnected data in graph databases with flexible schemas, while minimizing runtime uncertainties and simplifying query development.

Method: Graphify supports the mapping of GraphQL artifacts to Gremlin traversal for complete CRUD operations, using a transpiler to convert GraphQL requests into optimized Gremlin queries, driven by a recursive state machine algorithm processing GraphQL ASTs.

Result: The implementation of Graphify enables rapid generation of graph interfaces and demonstrates both practical efficiency and robustness through theoretical complexity analysis and empirical evaluation on the MovieLens dataset.

Conclusion: Graphify effectively bridges the gap between flexible graph storage and type-safe API consumption, significantly enhancing the development process with quick interface generation.
...
### [368] [Unified Data Discovery across Query Modalities and User Intents](https://arxiv.org/abs/2604.27252)
*Tingting Wang,Shixun Huang,Zhifeng Bao,J. Shane Culpepper,Shazia Sadiq,Volkan Dedeoglu,Reza Arablouei*

Main category: cs.DB

TL;DR: UniDisc is a unified framework for data discovery that retrieves relevant tables from data lakes using both natural language (NL) and table queries, enhancing versatility in user intent without needing specific representations.


<details>
  <summary>Details</summary>
Motivation: Current data discovery methods are limited as they focus on single query modalities or specific user intents, reducing their applicability for varied analytics tasks.

Method: UniDisc utilizes a heterogeneous graph approach to model NL statements and tables, employing dual-view neighbor aggregation and joint optimization to learn context-aware representations with limited supervision.

Result: UniDisc outperforms existing methods on seven datasets, demonstrating its effectiveness in both NL and table-based discovery tasks.

Conclusion: The framework successfully generalizes across different query modalities and user intents, providing a flexible and efficient solution for data discovery in diverse analytics applications.
...
### [369] [SynSQL: Synthesizing Relational Databases for Robust Evaluation of Text-to-SQL Systems](https://arxiv.org/abs/2604.27261)
*Mohammadamin Habibollah,Davood Rafiei*

Main category: cs.DB

TL;DR: The paper presents SynSQL, a framework for evaluating text-to-SQL systems by synthesizing test databases from natural language questions, which reveals performance drops in these systems when tested against dynamically generated data.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current text-to-SQL evaluation methods that rely on static databases, leading to fragile performance assessments.

Method: SynSQL synthesizes test databases based on question-schema alignment through three stages: schema selection, question-guided data synthesis, and critique with iterative refinement, adhering to semantic and relational constraints.

Result: Performance drops of 3-14% were observed across ten text-to-SQL models when evaluated with SynSQL-generated databases compared to traditional static evaluations, highlighting previously undetected errors.

Conclusion: Synthetic database generation offers a novel approach for examining LLM reasoning, controllability, and robustness in structured data environments.
...
### [370] [Tailwind: A Practical Framework for Query Accelerators](https://arxiv.org/abs/2604.28079)
*Geoffrey X. Yu,Ryan Marcus,Tim Kraska*

Main category: cs.DB

TL;DR: Tailwind is an external query planner that enhances RDBMS query performance by integrating workload-specific accelerators, allowing users to define these accelerators with abstract logical plans, resulting in significant query speedups.


<details>
  <summary>Details</summary>
Motivation: Current RDBMS solutions face performance limitations for specific queries, and query accelerators are underutilized due to high engineering costs versus performance benefits.

Method: Tailwind introduces a framework where users can define query accelerators via abstract logical plans (ALPs). It uses custom neural network models to predict the utility of these accelerators at runtime.

Result: Tailwind demonstrates an average speedup of 1.38x on TPC-H queries, achieving up to 29x in specific cases, while integrating seamlessly with existing RDBMS like Redshift and DuckDB.

Conclusion: Tailwind effectively bridges the gap between general-purpose RDBMS capabilities and the performance of custom-built query solutions by enabling the use of accelerators without significant engineering overhead.
...
### [371] [Index-Assisted Stratified Sampling for Online Aggregation](https://arxiv.org/abs/2604.28141)
*Yunnan Yu,Zhuoyue Zhao*

Main category: cs.DB

TL;DR: The paper presents an index-assisted stratified sampling method for online aggregation of frequently updated data, aiming to reduce query latencies and sampling costs through a two-phase sampling framework.


<details>
  <summary>Details</summary>
Motivation: Real-time data analysis requires low-latency queries over dynamic data, making efficient sampling methods crucial due to challenges in handling high variance and selectivity.

Method: The proposed methodology involves a two-phase sampling process where the first phase draws samples for aggregation and cost optimization, while the second phase focuses on refined aggregation using optimized strata.

Result: The evaluation demonstrates significant speedup in query latency, achieving up to 3x and 98708x improvements over traditional methods in specific scenarios.

Conclusion: The proposed stratified sampling approach effectively reduces sampling costs and enhances the efficiency of online aggregation in index-assisted environments.
...
### [372] [A High-Throughput Compute-Efficient POMDP Hide-And-Seek-Engine (HASE) for Multi-Agent Operations](https://arxiv.org/abs/2604.27162)
*Timothy Flavin,Sandip Sen*

Main category: cs.MA

TL;DR: The paper introduces the Hide-And-Seek-Engine, a C++ Dec-POMDP engine optimized for high throughput in the training of multi-agent reinforcement learning models.


<details>
  <summary>Details</summary>
Motivation: To address the high sample complexity of RL algorithms in Dec-POMDPs and enhance the efficiency of human-AI joint operations.

Method: The engine uses Data-Oriented Design principles, explicit cache-line alignment, and a zero-copy PyTorch memory bridge to optimize performance.

Result: Achieved a throughput of 33M steps per second in a single-agent environment and reduced to 7M with ten agents, outperforming traditional implementations by 3,500 times.

Conclusion: The Hide-And-Seek-Engine demonstrates significant performance improvements and validates its effectiveness for training multi-agent policies using various reinforcement learning algorithms.
