...
TL;DR: This paper explores the role of CLIP embeddings in memorization within text-to-image diffusion models, particularly focusing on the influence of specific embeddings and proposing mitigation strategies to reduce memorization effects.


<details>
  <summary>Details</summary>
Motivation: To enhance interpretability and safety in text-to-image models by understanding the impact of CLIP embeddings on memorization behaviors.

Method: Analysis of CLIP embeddings in Stable Diffusion, categorizing input tokens and identifying their contributions to model behavior, followed by proposing two mitigation strategies for memorization.

Result: It was found that the model disproportionately relies on certain embeddings, with <pad> significantly amplifying memorization effects due to its structural duplication of <eot>, while <prompt> contributes minimally.

Conclusion: Proposed strategies can effectively reduce memorization without compromising generation quality and are operationally feasible.
...
TL;DR: VANGUARD is a novel framework for Video Anomaly Detection that integrates anomaly classification, spatial grounding, and reasoning, achieving high performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: Traditional VAD methods lack spatial localization and interpretability, necessitating a more sophisticated approach.

Method: VANGUARD uses a three-stage curriculum involving classifier warmup, LoRA-adapted spatial grounding, and chain-of-thought generation, supported by a teacher-student annotation pipeline.

Result: On UCF-Crime, VANGUARD achieves 94% ROC-AUC and 84% F1, providing interpretable reasoning and localization of anomalies, surpassing previous methods.

Conclusion: Staged training improves performance, and cross-domain generalization is possible without specific target-domain adaptation.
...
TL;DR: This study presents a composite method combining the Segment Anything Model (SAM) with a domain-specific tool to enhance the accuracy of measuring size and shape in developing organoids using computer vision and machine learning techniques.


<details>
  <summary>Details</summary>
Motivation: To effectively study organoid development and disease processes, precise measurement of size and shape during growth is crucial.

Method: A composite approach combining Segment Anything Model (SAM) with a domain-specific segmentation tool is developed and evaluated against existing methods using organoid image data.

Result: The new composite method consistently delivers accurate segmentation results across various conditions, outperforming existing tools and matching inter-observer variability.

Conclusion: This composite method significantly enhances image segmentation accuracy for organoid studies, approaching the variability seen in manual segmentations by annotators.
...
TL;DR: The paper presents a method for training segmentation models that incorporates summary statistics and weak supervision to improve performance in medical image analysis.


<details>
  <summary>Details</summary>
Motivation: To reduce the manual effort in segmenting medical images while relying on summary statistics for training segmentation models.

Method: A novel loss function is utilized that considers image reconstruction quality, adherence to summary statistics, and overlap with weak supervisory signals (few pixels marked in the area of interest).

Result: Adding weak information significantly enhances model performance, beyond what summary statistics can achieve alone.

Conclusion: The proposed method shows promise for effective medical image segmentation using limited annotations, with experiments validating its potential across various imaging modalities.
...
TL;DR: The paper investigates the effect of targeted data augmentation techniques on improving cross-modality generalization in medical image segmentation, specifically using MRI and CT images.


<details>
  <summary>Details</summary>
Motivation: To address limited model performance due to a lack of annotated data and poor generalization across different imaging protocols in medical image segmentation.

Method: The authors applied specific data augmentation techniques while training spine segmentation models on single-modality datasets and evaluated their performance on multiple out-of-distribution datasets.

Result: The study found an average Dice gain of 155% in unseen domains, with negligible impact on in-domain accuracy, and effective performance transfer between CT and MRI images.

Conclusion: The proposed GPU-optimized augmentations enhance robustness to diverse clinical imaging scenarios without slowing down training, and the methodology is made available as an open-source toolbox.
...
### [6] [Reservoir property image slices from the Groningen gas field for image translation and segmentation](https://arxiv.org/abs/2605.03942)
*Abdulrahman Al-Fakih,Nabil Sariah,Ardiansyah Koeshidayatullah,SanLinn I. Kaka*

Main category: cs.CV

TL;DR: This paper presents a high-resolution geological image dataset for benchmarking image analysis methods in reservoir characterization, along with a reproducible software workflow.


<details>
  <summary>Details</summary>
Motivation: To address the need for openly available geological image datasets that facilitate reproducible benchmarking in reservoir characterization using advanced imaging techniques.

Method: The authors derived datasets from 3D reservoir grids, creating aligned 2D PNG images representing various reservoir properties and provided a software workflow for augmentation and image processing tasks.

Result: The dataset comprises images depicting facies, porosity, permeability, and water saturation, along with tools for generating and processing these images.

Conclusion: This resource enhances transparency and reusability in geoscience and machine-learning applications, promoting further research and development in geological image analysis.
...
TL;DR: This paper presents NucEval, a unified framework for evaluating nuclear instance segmentation in computational pathology, addressing key evaluation issues and demonstrating improved metrics through modifications.


<details>
  <summary>Details</summary>
Motivation: The study aims to improve the evaluation of nuclear instance segmentation techniques, which are essential in clinical applications but often lack a robust evaluation pipeline.

Method: The authors identify four issues in the evaluation process (vague regions, score normalization, overlapping instances, border uncertainty) and propose solutions integrated into NucEval, tested on the NuInsSeg and other datasets.

Result: The implementation of NucEval showed improved evaluation metrics for three CNN- and ViT-based segmentation models across the datasets used in the study, highlighting the significance of the proposed modifications.

Conclusion: The NucEval framework provides a comprehensive solution for evaluating nuclear instance segmentation, improving the reliability of evaluation metrics and offering publicly accessible tools for the research community.
...
TL;DR: The paper introduces the Fire-Centered Evaluation Region (FCER) framework for assessing uncertainty quantification in wildfire spread prediction models, emphasizing the need for spatially relevant evaluation methods.


<details>
  <summary>Details</summary>
Motivation: There is a critical need for effective wildfire spread prediction models that incorporate uncertainty quantification, particularly for improving emergency planning.

Method: The study employs the FCER framework to evaluate an Ensemble model against a distilled single-pass student model on the WildfireSpreadTS dataset, focusing on boundary-sensitive predictions.

Result: Both models exhibit comparable uncertainty calibration and performance in relevant fire zones, with the student model showing effective complementary uncertainty rankings.

Conclusion: The findings underscore the utility of the FCER framework for enhancing the assessment of uncertainty in wildfire prediction models.
...
TL;DR: CAFe-DINO utilizes advancements in open vocabulary semantic segmentation (OVSS) to segment remote sensing images without domain-specific fine-tuning, achieving state-of-the-art results on several datasets.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of limited labeled datasets in remote sensing (RS), emphasizing the need for effective segmentation models that do not require labor-intensive supervised fine-tuning.

Method: The authors develop the CAFe-DINO model which integrates DINOv3 with techniques like cost aggregation and training-free upsampling of text-image similarity scores to enhance OVSS performance for RS imagery.

Result: CAFe-DINO outperforms other OVSS approaches that rely on RS-specific fine-tuning, demonstrating its effectiveness across key RS segmentation datasets.

Conclusion: The proposed model signifies a substantial advancement in RS imagery segmentation by leveraging an existing powerful backbone and avoiding the necessity for domain-specific training, thus providing a more accessible solution.
...
TL;DR: The paper introduces Sentinel2Cap, a human-annotated dataset for multimodal satellite image captioning, emphasizing the challenges and potential benefits of such datasets in cross-modal scene understanding.


<details>
  <summary>Details</summary>
Motivation: There is a lack of captioning datasets for multimodal satellite data, specifically SAR imagery, which limits research in this area.

Method: The authors created the Sentinel2Cap dataset with human-annotated captions for Sentinel-1 SAR and Sentinel-2 images, and evaluated it using the Qwen3-VL-8B-Instruct model in a zero-shot captioning scenario across various image modalities.

Result: RGB images demonstrated the highest captioning performance, while SAR images posed challenges for the models, improved by modality-specific prompts.

Conclusion: The paper underscores the importance of human-annotated datasets for advancing multimodal remote sensing image captioning and scene understanding.
...
TL;DR: The paper presents a diffusion-model-based synthetic data augmentation pipeline that enhances deep learning performance on long-tailed medical datasets, particularly improving accuracy for tail classes.


<details>
  <summary>Details</summary>
Motivation: Long-tailed distributions in medical datasets hinder deep learning model performance, especially for rare high-risk diseases that need accurate diagnosis.

Method: A synthetic data augmentation pipeline using a novel inpainting diffusion model along with an Out-of-Distribution post-selection mechanism for generating diverse and clinically relevant samples.

Result: The approach was tested on the ISIC2019 dataset, yielding substantial overall performance improvements, particularly over 28% on the tail class with the fewest samples.

Conclusion: The diffusion-based augmentation effectively addresses long-tail imbalances, enhancing the robustness of medical classification models.
...
TL;DR: The paper presents CropVLM, a Vision-Language Model for high-throughput plant phenotyping, addressing limitations of conventional methods through Domain-Specific Semantic Alignment, achieving high accuracy in species detection.


<details>
  <summary>Details</summary>
Motivation: To overcome the phenotyping bottleneck in plant breeding caused by manual data collection and limitations of closed-set computer vision systems.

Method: CropVLM is trained on 52,987 image-caption pairs across 37 plant species, utilizing Domain-Specific Semantic Alignment and integrating with the Hybrid Open-Set Localization Network for flexible detection of novel species based on natural language.

Result: CropVLM achieves 72.51% zero-shot classification accuracy and outperforms several baselines; its detection pipeline demonstrates significant zero-shot generalization for new species.

Conclusion: CropVLM offers a scalable and effective solution for high-throughput phenotyping, enhancing genetic research and sustainable agriculture.
...
TL;DR: VEBENCH is a new benchmark for evaluating video editing knowledge and reasoning skills in large multimodal models, highlighting significant performance gaps compared to human editors.


<details>
  <summary>Details</summary>
Motivation: To address the lack of evaluation tools for video editing knowledge and operational reasoning in LMMs.

Method: VEBENCH includes 3.9K edited videos and 3,080 QA pairs developed via a human-AI collaborative process, featuring tasks for editing technique recognition and workflow simulation.

Result: Experiments indicate a significant performance gap between LMMs and human capabilities in video editing cognition.

Conclusion: VEBENCH aims to enhance intelligent video editing systems and encourage further research in multimodal reasoning.
...
TL;DR: FACTOR is a novel test-time adaptation framework for open-vocabulary object detection that enhances robustness against distribution shifts by utilizing counterfactual reasoning to address failures related to non-causal visual attributes.


<details>
  <summary>Details</summary>
Motivation: To tackle failures in open-vocabulary object detection caused by spurious correlations between non-causal attributes and object categories during distribution shifts.

Method: FACTOR utilizes a counterfactual reasoning approach, perturbing test images along non-causal attributes to assess and suppress attribute-dependent predictions without requiring parameter updates.

Result: FACTOR demonstrates superior performance compared to existing TTA methods on datasets like PASCAL-C, COCO-C, and FoggyCityscapes, indicating its effectiveness in improving model robustness.

Conclusion: The use of explicit counterfactual reasoning in FACTOR significantly enhances the resilience of object detection systems against distribution shifts.
...
TL;DR: TACO enhances geolocation through a tightly-coupled IMU and fine-grained CVGL, significantly improving trajectory accuracy while maintaining low computational cost.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of GNSS in scenarios with signal interference and to improve the trajectory accuracy of ground imagery localization against satellite tiles without accumulating significant drift from inertial sensors.

Method: TACO integrates IMU data with a CVGL pipeline, starting with a GNSS reading and employing a closed-form cross-track error model to manage drift and maintain accuracy, using a multi-crop search and a yaw-residual gate for fix verification and noise scaling in updates.

Result: TACO reduces median Absolute Trajectory Error from 97.0m without GNSS to 16.3m with the proposed method, demonstrating a substantial 5.9 times improvement in accuracy.

Conclusion: The approach provides a reliable and efficient alternative for geo-localization in environments where GNSS signals are compromised, achieving significant accuracy increases at a low computational cost.
...
TL;DR: This paper presents Adaptive Hierarchical Prior Alignment (AHPA), a novel alignment framework for improving Diffusion Transformer training by adapting representation supervision based on varying noise levels throughout the denoising process.


<details>
  <summary>Details</summary>
Motivation: The static nature of existing alignment methods fails to address the varying granularity of required supervision across different noise levels in diffusion models, which leads to suboptimal performance.

Method: AHPA utilizes multi-level features from a frozen VAE encoder and employs a timestep-conditioned Dynamic Router to adaptively select and weight these features, aligning them with the model's training needs during denoising.

Result: Experiments demonstrate that AHPA enhances both convergence speed and generation quality compared to baseline methods, with no added inference cost and no need for external encoder supervision during training.

Conclusion: The proposed AHPA framework effectively resolves the issues of representation mismatch in alignment strategies by incorporating a flexible and adaptive approach informed by the training dynamics.
...
TL;DR: This paper analyzes the underlying factors of 4D Gaussian Splatting (4DGS) for dynamic scene reconstruction and proposes an improved method, FreeTimeGS++, for enhanced stability and realistic representations.


<details>
  <summary>Details</summary>
Motivation: To systematically understand the hidden drivers behind the performance gains in recent 4D Gaussian Splatting methods for dynamic scene reconstruction.

Method: The authors establish a controlled baseline (FreeTimeGS_ours) that reproduces heuristics of the state-of-the-art FreeTimeGS, dissecting 4DGS along fundamental axes to uncover key secrets and proposing the method FreeTimeGS++ with gated marginalization and neural velocity fields.

Result: The analysis reveals emergent temporal partitioning from Gaussian durations and discrepancies between photometric fidelity and spatiotemporal consistency, leading to improved stability and reduced run-to-run variance in representation.

Conclusion: FreeTimeGS++ achieves superior performance and reproducibility, laying a foundation for future research in 4D Gaussian Splatting.
...
TL;DR: This paper introduces MedSR-Vision, a unified framework for benchmarking super-resolution models in medical imaging across multiple modalities and magnification scales, highlighting the strengths of various models in terms of perceptual quality and structural accuracy.


<details>
  <summary>Details</summary>
Motivation: The need to enhance diagnostic precision in medical imaging through improved super-resolution techniques while addressing challenges in anatomical accuracy and model generalization.

Method: A novel framework, MedSR-Vision, benchmarks three models (SRCNN, SwinIR, Real-ESRGAN) across five imaging modalities at magnification scales of 2x, 3x, and 4x using various quantitative metrics.

Result: Real-ESRGAN outperforms the others in perceptual quality and edge recovery at higher magnifications, while SwinIR is best for preserving structural features, and SRCNN shows stable performance at lower magnification scales.

Conclusion: The findings provide valuable insights for model selection in clinical imaging and establish a standardized evaluation framework for future research in medical image super-resolution.
...
TL;DR: The paper analyzes the inefficiencies in video vision-language models (VLMs) using a method called training-free anti-recomputation, demonstrating significant speedup and performance improvements by reusing visual states and adapting queries accordingly.


<details>
  <summary>Details</summary>
Motivation: To address inefficiencies in VLMs that repeatedly process stable visual states, leading to unnecessary computational waste.

Method: The study employs training-free anti-recomputation to reuse stable visual states based on validation, and introduces adaptive methodologies for follow-up queries to enhance performance and reduce latency.

Result: The results show that reusing video states can significantly enhance efficiency, achieving a follow-up latency reduction of up to 35.92x in certain settings, while maintaining correctness across queries.

Conclusion: The findings suggest a shift towards VLMs that expose media dynamics, allowing for more efficient processing without rediscovering static elements from dense RGB frames each time.
...
TL;DR: The study explores the capability of Multimodal Large Language Models to recognize pathological movements in seizure videos, showing superior performance compared to traditional methods with specific preprocessing enhancements.


<details>
  <summary>Details</summary>
Motivation: To investigate the application of MLLMs in analyzing involuntary movements in neurological disorders, particularly seizures, where traditional methods have limitations.

Method: The study assessed zero-shot performance of MLLMs on 20 clinical features across 90 seizure recordings, comparing them to fine-tuned CNN and ViT models, with additional targeted signal enhancements applied.

Result: MLLMs outperformed baseline models on 13 of 18 tested features, particularly in recognizing postural features, while struggling with subtle movements. Preprocessing techniques improved performance on 10 features, and 94.3% of expert evaluations marked MLLM-generated explanations with high faithfulness scores.

Conclusion: The findings reveal that general-purpose MLLMs can be adapted for clinical video analysis in neurology, supporting the development of interpretable diagnostic tools through targeted preprocessing.
...
TL;DR: This paper introduces an automated system for tracing cephalometric radiographs that closely mimics orthodontists' workflow, achieving superior accuracy in landmark identification compared to previous methods.


<details>
  <summary>Details</summary>
Motivation: To automate the tracing of cephalometric radiographs, aligning computational methods with established orthodontic practices.

Method: A five-phase pipeline that utilizes anatomy-guided initialization to inform a spatial attention model, integrated with an HRNet-W32 detector for landmark detection.

Result: The system achieves a mean radial error of 1.04 mm on 25 landmarks, outperforming previous state-of-the-art results significantly, with twelve landmarks below 1 mm error.

Conclusion: Incorporating clinical domain knowledge as anatomical priors enhances generalization in model performance, indicating that such inductive biases are crucial for effective learning.
...
TL;DR: Mix3R is a novel method for 3D reconstruction that integrates feed-forward and generative approaches to enhance input alignment and produce accurate geometries.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the limitations of current sparse-view 3D reconstruction methods that either predict poorly aligned geometries or suffer from incomplete geometry generation.

Method: Mix3R combines two stages of generation—sparse voxel generation and textured geometry generation—by incorporating a Mixture-of-Transformers architecture that aligns 3D structures with input data. It uses pretrained models for better 2D-3D alignment and generates an overlap-based attention bias for texture placement.

Result: The integration of feed-forward reconstruction with generative modeling allows Mix3R to achieve improved input alignment of generated 3D shapes and more accurate camera pose estimations than existing methods.

Conclusion: Mix3R succeeds in producing better-aligned 3D shapes and enhances the synergy between different reconstruction approaches, thus offering significant improvements over purely generative or feed-forward methods.
...
TL;DR: This paper addresses Long-tailed Class Incremental Learning (LT-CIL) by proposing techniques to mitigate catastrophic forgetting while managing class imbalance.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenges posed by imbalanced datasets in continual learning, notably the issues of under-learning minority classes and overfitting majority classes.

Method: The paper introduces two techniques: gradient consistency regularization and dynamic adjustment of distillation loss weight based on class imbalance measured by normalized entropy.

Result: Experiments on various benchmarks showed accuracy improvements of up to 5.0%, especially in settings where tasks transition from majority to minority classes.

Conclusion: The proposed methods effectively mitigate forgetting and improve learning dynamics without increasing computational overhead, indicating their practicality.
...
TL;DR: This paper proposes a dual-foundation UDA framework employing two models to enhance semantic segmentation for autonomous driving, addressing limitations in adaptation processes between synthetic and real datasets.


<details>
  <summary>Details</summary>
Motivation: To improve semantic segmentation in autonomous driving by overcoming costly annotations and adaptation challenges between synthetic and real domains.

Method: The proposed framework utilizes the Segment Anything Model (SAM) for broader target pixel learning and DINOv3 for stable, domain-invariant class prototype construction.

Result: The method achieves +1.3% and +1.4% mIoU improvements over competitive UDA baselines in experiments on the GTA-to-Cityscapes and SYNTHIA-to-Cityscapes datasets.

Conclusion: The dual-foundation approach effectively bridges the domain gap and improves the performance of segmentation models in real-world applications.
...
TL;DR: This paper presents SoDa$^2$, a novel single-stage open-set domain adaptation method for cross-scene hyperspectral image classification, addressing domain shift and computational efficiency challenges.


<details>
  <summary>Details</summary>
Motivation: To improve open-set cross-scene hyperspectral image classification by overcoming domain shift and reducing the computational load of existing methods.

Method: SoDa$^2$ employs contribution-aware dual-modality feature extraction for spectral and spatial details, along with a decoupled alignment module that minimizes Maximum Mean Discrepancy (MMD) for better domain-invariant feature extraction.

Result: The proposed framework performs MMD-constrained alignment of features and recognizes open-set classes using a Gaussian Mixture Model, showing enhanced selectivity between known and unknown classes without prior knowledge.

Conclusion: Experimental results on multiple HSI datasets indicate that SoDa$^2$ surpasses existing methods in classification accuracy and transferability, making it a viable solution for open-set cross-scene hyperspectral image classification.
...
TL;DR: Proposes a Training-Free Dual-System framework to enhance the discriminative capacity of score-based self-supervised talking head forgery detectors by refining the ordering of uncertain samples.


<details>
  <summary>Details</summary>
Motivation: Existing detectors have insufficient discriminative capacity on challenging cases, leading to unreliable anomaly ordering.

Method: A Training-Free Dual-System (TFDS) framework is proposed, partitioning samples into confident and uncertain subsets and revisiting only the uncertain ones for detailed reasoning.

Result: Extensive experiments show consistent improvements in anomaly detection across various datasets and conditions, primarily through better ordering within the uncertain subset.

Conclusion: Self-supervised detectors have underutilized discriminative cues that can be unlocked without additional training using the proposed dual-system approach.
...
TL;DR: The paper proposes MLLM-Assisted Semantic-Relational Consistent Alignment (MASRA) to improve Video Temporal Grounding (VTG) by addressing cross-modal semantic gaps in aligning queries to temporal events.


<details>
  <summary>Details</summary>
Motivation: The aim is to overcome alignment issues in VTG caused by incorrect feature matches and insufficient discriminability related to temporal semantics.

Method: MASRA employs a training-based MLLM framework to generate event-level descriptions and clip-level captions, implementing two alignments: Event Semantic Temporal Alignment (ESTA) and Local Relational Consistency Alignment (LRCA) to enhance temporal consistency and semantic alignment.

Result: Experimental results demonstrate that MASRA surpasses existing approaches in VTG tasks, with ablation studies affirming its contributions and effectiveness.

Conclusion: The proposed framework offers a robust solution to address semantic and temporal alignment issues in VTG, leveraging MLLM techniques effectively during training.
...
TL;DR: GRPO-TTA improves test-time adaptation in vision-language models through group-wise policy optimization and tailored reward functions.


<details>
  <summary>Details</summary>
Motivation: To assess if Group Relative Policy Optimization enhances test-time adaptation for vision-language models.

Method: GRPO-TTA adapts GRPO by reformulating prompt prediction as a group-wise optimization problem using top-K class candidates sampled from CLIP similarity distributions, with new reward functions for effective tuning.

Result: GRPO-TTA consistently outperforms existing test-time adaptation methods across various benchmarks, yielding significant performance improvements under natural distribution shifts.

Conclusion: The proposed method demonstrates a significant advance in test-time adaptation for vision-language models.
...
TL;DR: This paper presents a framework with MK-ResRecon and IdentityRefineNet3D for reconstructing high-quality 3D MRI from 12.5% of 2D slices, effectively improving efficiency and reducing patient strain.


<details>
  <summary>Details</summary>
Motivation: The motivation is to reduce the time and patient discomfort associated with MRI scans, which are often hindered by motion artifacts due to prolonged scanning times.

Method: The method involves two models: MK-ResRecon, which predicts missing slices using a texture-aware loss, and IdentityRefineNet3D, which refines these predictions alongside the original data for a cohesive 3D volume.

Result: The framework successfully reconstructs 3D MRI volumes with high fidelity from sparsely sampled slices, demonstrating accuracy and clinical validity without hallucinations.

Conclusion: This research provides a clinically viable solution for faster and more patient-friendly MRI imaging, highlighting the framework's potential for practical application in medical settings.
...
TL;DR: The paper proposes a novel surface-based approach for 3D anomaly detection in point clouds, utilizing a Noisy Points Generation module, a Multi-scale Level-of-detail Feature module, and an Implicit Surface Discrimination module to enhance feature learning and detection accuracy.


<details>
  <summary>Details</summary>
Motivation: The need for improved anomaly detection in 3D point clouds due to existing challenges with scalability and representation accuracy in current methods.

Method: The methodology involves generating noise to enrich the dataset, capturing multi-scale features for better representation, and learning an implicit surface representation to classify points as normal or abnormal.

Result: The proposed approach achieved an average object-level AUROC of 92.1% on the Anomaly-ShapeNet dataset and 85.9% on the Real3D-AD dataset, surpassing the performance of the previous best methods.

Conclusion: This study demonstrates effective 3D anomaly detection using a surface-based method that enhances feature learning through multi-scaling, noise generation, and implicit representation.
...
TL;DR: Mantis is a novel parameter-efficient fine-tuning framework for 3D point cloud foundation models, enabling effective state-level adaptation without extensive computational costs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the fine-tuning of Mamba-based backbones in 3D PFMs, which previously faced issues with accuracy and optimization instability when applying standard PEFT methods.

Method: Mantis employs a State-Aware Adapter (SAA) for task-conditioned control signal injection into state-space updates, maintaining the frozen backbone, along with Dual-Serialization Consistency Distillation (DSCD) for regularizing point cloud serializations.

Result: Mantis achieves competitive performance with only about 5% of the parameters being trainable, addressing the limitations of existing PEFT methods on Mamba backbones.

Conclusion: The proposed framework offers an efficient and effective solution for fine-tuning 3D PFMs, overcoming the challenges of existing parameter-efficient approaches.
...
TL;DR: VL-SAM-v3 improves open-world object detection by integrating retrieval-grounded visual memory and memory-guided prompt refinement, enhancing performance, especially for rare categories.


<details>
  <summary>Details</summary>
Motivation: Existing methods in open-world object detection are limited by coarse textual semantics and insufficient visual evidence for complex scenarios.

Method: VL-SAM-v3 retrieves visual prototypes from a non-parametric memory bank, creating sparse and dense visual priors integrated with detection prompts through Memory-Guided Prompt Refinement.

Result: Experiments demonstrate that VL-SAM-v3 enhances detection performance in both open-vocabulary and open-ended settings, particularly for rare categories, and performs well even when using a stronger detector (SAM3).

Conclusion: The proposed retrieval-and-refinement approach shows promise for improving open-world object detection, indicating robustness across varying inference types.
...
TL;DR: FSTM is a novel approach that improves indoor 3D reconstruction by effectively learning geometry and semantics. It features a two-step process that enhances training efficiency and performance compared to multi-SDF methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the representation of indoor 3D scenes by integrating object-level semantic understanding while overcoming the limitations of existing multi-SDF methods in terms of training speed and scalability.

Method: FSTM employs a two-step learning process: first optimizing scene geometry using RGB inputs and geometric cues, followed by semantic field estimation, which avoids dependency on complex multi-SDF designs.

Result: The proposed method outperforms multi-SDF approaches, achieving 2.3x faster training on the Replica dataset, better robustness against real-world imperfections on ScanNet++, and higher recall in object surface recovery.

Conclusion: FSTM demonstrates that a simplified approach can yield comparable or superior results for geometric and semantic reconstructions in indoor 3D scenes, suggesting the effectiveness of its streamlined formulation.
...
TL;DR: MHPR introduces a benchmark for joint perception-reasoning in human-centric scenes, improving upon current LVLM evaluations through fine-grained attributes and automated data generation, leading to enhanced model performance.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current LVLM benchmarks that focus on single-task evaluations and lack detailed, human-centric assessments.

Method: The MHPR benchmark includes various data types (C-RD, SFT-D, RL-D, T-D) and employs an automated caption/VQA generation pipeline to ensure quality annotations for evaluating vision-language models.

Result: Evaluation reveals format-aligned SFT data improves model stability, RL data enhances performance on challenging scenarios, and training with MHPR achieves significant improvements, nearing performance of larger models.

Conclusion: The release of MHPR and ACVG promotes reproducibility and extensibility in research focused on human-centric perception and reasoning.
...
TL;DR: This paper presents a novel framework for brain tumor classification using deep learning that addresses challenges in MRI data scarcity and inter-institutional variability.


<details>
  <summary>Details</summary>
Motivation: The paper aims to overcome the limitations posed by insufficient expert-annotated MRI data and the impact of varied imaging protocols on deep learning model performance in neuro-oncology.

Method: The proposed framework employs a CNN to classify input MRI slices into different orientations and utilizes a slice-wise unsupervised domain adaptation strategy to transfer knowledge across various MRI modalities, while enforcing feature alignment through maximum mean discrepancy loss.

Result: The framework shows significant improvement in target-domain performance compared to existing methods, demonstrating the effectiveness of orientation-specific learning and the introduced adaptation strategies.

Conclusion: The study underscores the potential of the proposed orientation-aware unsupervised domain-adaptive framework in enhancing brain tumor classification accuracy in diverse clinical settings.
...
TL;DR: This paper introduces BFORE, a hybrid metaheuristic-optimized framework for low-light image enhancement that automatically tunes Retinex parameters, resulting in significant performance improvements over existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of existing Retinex-based methods that require manual parameter tuning, which does not generalize well for diverse lighting conditions in low-light images.

Method: BFORE employs a hybrid optimization approach combining a Butterfly Optimization Algorithm (BOA) for Multi-Scale Retinex with Color Restoration (MSRCR) parameters and a Firefly Algorithm (FA) for Adaptive Gamma Correction with Weighted Distribution (AGCWD) and denoising parameters, utilizing a dynamic BOA-FA switching strategy.

Result: BFORE achieved the highest PSNR of 17.22 dB in experiments on the LOL benchmark dataset, outperforming traditional methods with a notable improvement over Histogram Equalization (20.3%) and MSRCR (17.5%), and surpassed the deep learning baseline RetinexNet in both PSNR and SSIM metrics.

Conclusion: The proposed hybrid optimization strategy in BFORE significantly enhances low-light image quality, providing automatic parameter tuning and yielding better performance compared to existing methods.
...
TL;DR: Introduction of DALPHIN, a comprehensive benchmark for AI models in digital pathology, analyzing performance against expert pathologists.


<details>
  <summary>Details</summary>
Motivation: To independently evaluate AI models' capabilities in assisting pathologists with diagnostics and to establish a standard for benchmarking in digital pathology.

Method: Creation of a multicentric open benchmark containing 1236 images from 300 cases across various countries and subspecialties, along with human performance metrics from pathologists of varying expertise.

Result: PathChat+ demonstrated expert-level performance in 4 out of 6 tasks, while Gemini 2.5 Pro and GPT-5 showed lower performance in fewer tasks, indicating varying effectiveness among models.

Conclusion: DALPHIN serves as a robust platform for validating AI performance in digital pathology, encouraging further research and development in this field.
...
TL;DR: Introduction of CoVUBench, a benchmark for evaluating copyright content unlearning in Large Vision-Language Models (LVLMs).


<details>
  <summary>Details</summary>
Motivation: To mitigate risks of LVLMs memorizing copyrighted visual content and address the lack of robust evaluation methods for machine unlearning in complex multimodal settings.

Method: CoVUBench employs procedurally generated synthetic data with systematic visual variations to evaluate unlearning generalization effectively.

Result: Provides a multimodal evaluation protocol assessing both the effectiveness of forgetting copyrighted content and the preservation of model utility.

Conclusion: CoVUBench serves as a standardized tool to enhance responsible unlearning methods for LVLMs.
...
TL;DR: MILE introduces a modular framework for continual semantic segmentation that efficiently scales across tasks without sacrificing past knowledge, utilizing lightweight experts trained on task-specific data.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of continual semantic segmentation, specifically the issues of scalability and forgetting in expert-based learning methods when adapting to new domains or modalities.

Method: MILE uses Low-Rank Adaptation (LoRA) to create lightweight task-specific experts while keeping the base model frozen, employing a prototype-guided gating mechanism to select the appropriate expert during inference.

Result: MILE shows improved stability, plasticity, and scalability in continual semantic segmentation benchmarks, requiring significantly fewer parameters compared to traditional expert-based methods.

Conclusion: MILE achieves efficient continual segmentation with robust performance and resource efficiency, addressing the limitations of existing expert-based approaches.
...
TL;DR: This paper introduces a model for instance segmentation of visual affordances in images, enhancing interaction capabilities for robots and augmented reality systems through improved predictions and uncertainty estimation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable better interaction for autonomous systems by accurately predicting regions in images where interactions are possible, moving beyond general saliency maps.

Method: The authors extend an attention-based architecture to create a model that utilizes sample-based and ensemble methods for uncertainty estimation, assessing both epistemic and aleatoric variances in the predictions.

Result: The model shows improved performance on the IIT-Aff dataset with a 7.4 percentage point increase in the $F_β^w$ score, demonstrating enhanced mask refinement and generalization.

Conclusion: Bayesian models with attention mechanisms provide better calibration and uncertainty estimation, adding interpretability to predictions, particularly highlighting variances in object contours and challenging visual pixels.
...
TL;DR: PriorNet is a framework that enhances engagement estimation from face videos by using task-relevant priors at multiple stages to handle incomplete facial evidence and subjective annotations.


<details>
  <summary>Details</summary>
Motivation: Engagement estimation from facial video is hindered by incomplete data, limited labeled examples, and subjective interpretation, necessitating a robust framework.

Method: PriorNet employs task-relevant priors during preprocessing, model adaptation, and objective design, using placeholders for missing faces, adapting a self-supervised video model through a Prior-LoRA module, and utilizing a Dirichlet-evidential objective for uncertainty-weighted training.

Result: PriorNet outperforms the strongest previous reference across multiple datasets (EngageNet, DAiSEE, DREAMS, PAFE), demonstrating significant improvement due to its layered approach.

Conclusion: The use of explicit prior injection is validated as an effective design principle for enhancing engagement estimation in facial video analysis.
...
TL;DR: RPBA-Net enhances RAW-domain ISP by integrating demosaicing with detail enhancement, using a novel residual-affine approach and hierarchical modeling.


<details>
  <summary>Details</summary>
Motivation: The paper aims to solve issues of module fragmentation, uninterpretable mappings, and deployment challenges in RAW-domain imaging processes.

Method: RPBA-Net employs residual affine base reconstruction and pyramid bilateral affine grids, combining autoregressive adaptive slicing and adaptive cross-layer fusion to enhance image quality.

Result: The proposed method significantly outperforms existing RAW-to-sRGB techniques, demonstrating superior reconstruction fidelity and perceptual quality while being efficient for deployment.

Conclusion: RPBA-Net offers a robust solution for RAW image enhancement that is interpretable, stable, and suitable for mobile applications.
...
TL;DR: The paper introduces Diffusion Masked Pretraining (DiMP) to address the limitations of existing masked reconstruction methods for dynamic point clouds, focusing on positional leakage and deterministic motion targets.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of traditional masked reconstruction objectives that lead to positional leakage and loss of distributional structure in motion learning.

Method: DiMP employs diffusion modeling for positional inference and motion learning, using noisy masked tube centers for training and a DDPM noise-prediction objective for inter-frame displacement.

Result: DiMP achieves significant improvements in downstream tasks, evidenced by absolute accuracy gains of 11.21% in offline action segmentation and 13.65% in online inference.

Conclusion: DiMP enhances the performance of dynamic point cloud processing by leveraging diffusion modeling, thereby providing a more robust framework for self-supervised learning.
...
### [44] [A Deeper Dive into the Irreversibility of PolyProtect: Making Protected Face Templates Harder to Invert](https://arxiv.org/abs/2605.03857)
*Vedrana Krivokuća Hahn,Jérémy Maceiras,Sébastien Marcel*

Main category: cs.CV

TL;DR: The paper analyzes the irreversibility of PolyProtect, a biometric template protection method for face embeddings, and proposes a key selection algorithm to enhance its security against inversion.


<details>
  <summary>Details</summary>
Motivation: To improve the security of biometric templates by making them harder to invert and control the trade-off between irreversibility and accuracy.

Method: The authors conduct an irreversibility analysis, propose a key selection algorithm for optimizing polynomial coefficients and exponents, and normalize embeddings before applying PolyProtect.

Result: Experiments demonstrate that the proposed algorithm generates templates that are significantly more difficult to invert and balances irreversibility across different overlap parameters.

Conclusion: The study offers a reproducible framework for enhancing the effectiveness of PolyProtect in biometric security.
...
### [45] [The Detector Teaches Itself: Lightweight Self-Supervised Adaptation for Open-Vocabulary Object Detection](https://arxiv.org/abs/2605.03642)
*Yazhe Wan,Changjae Oh*

Main category: cs.CV

TL;DR: Proposes Decoupled Adaptivity Training (DAT) to enhance vision-language models (VLMs) for open-vocabulary object detection, addressing local detail capture issues.


<details>
  <summary>Details</summary>
Motivation: To improve zero-shot recognition of novel objects in open-vocabulary detection, overcoming limitations of VLMs in capturing local details.

Method: DAT is a self-supervised fine-tuning method that constructs a region-aware pseudo-labeled dataset using a closed-set object detector and fine-tunes the VLM's visual backbone in a decoupled manner.

Result: DAT shows consistent improvement on COCO and LVIS datasets for both novel and known categories, achieving state-of-the-art performance in cooperative open-vocabulary detection.

Conclusion: DAT enhances local feature alignment in VLMs without increasing inference overhead, creating a more effective detection system.
...
### [46] [Rethinking Temporal Consistency in Video Object-Centric Learning: From Prediction to Correspondence](https://arxiv.org/abs/2605.03650)
*Zhiyuan Li,Rongzhen Zhao,Wenyan Yang,Wenshuai Zhao,Pekka Marttinen,Joni Pajarinen*

Main category: cs.CV

TL;DR: The paper introduces Grounded Correspondence, a framework for video object-centric learning that uses deterministic bipartite matching instead of learned temporal dynamics, maintaining identity consistency without needing additional learnable parameters.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiencies of existing learned dynamics modules for maintaining temporal consistency in video object-centric learning.

Method: Grounded Correspondence employs deterministic bipartite matching for frame-to-frame identity continuity, utilizing instance-discriminative features from pre-trained vision backbones and initializing slots from salient regions in frozen features.

Result: The method achieves competitive performance on benchmark datasets like MOVi-D, MOVi-E, and YouTube-VIS without the need for learnable parameters for temporal modeling.

Conclusion: The proposed framework demonstrates that effective video object learning can be achieved with simpler deterministic methods, providing a more efficient alternative to traditional learned dynamics approaches.
...
### [47] [AniMatrix: An Anime Video Generation Model that Thinks in Art, Not Physics](https://arxiv.org/abs/2605.03652)
*Tencent HY Team*

Main category: cs.CV

TL;DR: AniMatrix is a novel video generation model designed for anime, emphasizing artistic expression over physical realism, utilizing a dual-channel conditioning mechanism for improved control and evaluation in animation generation.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing video generation models that prioritize physical realism and fail to capture the unique stylization of anime.

Method: AniMatrix employs a dual-channel conditioning mechanism with a structured taxonomy of controllable production variables, and it transitions through a curriculum that evolves from near-physical to fully anime expressiveness, optimizing for intentional artistic outcomes.

Result: In qualitative evaluations by professional animators, AniMatrix outperformed existing models in four out of five production dimensions, showing significant improvements in Prompt Understanding and Artistic Motion metrics.

Conclusion: The novel approach of AniMatrix effectively separates artistic intent from physical constraints, offering a robust tool for generating anime with higher fidelity to stylistic conventions.
...
### [48] [Real Image Denoising with Knowledge Distillation for High-Performance Mobile NPUs](https://arxiv.org/abs/2605.03680)
*Faraz Kayani,Sarmad Kayani,Asad Ahmed,Radu Timofte,Dmitry Ignatov*

Main category: cs.CV

TL;DR: The paper presents a hardware-algorithm co-design for image denoising on mobile NPUs, achieving high fidelity while optimizing for NPU compatibility and efficiency.


<details>
  <summary>Details</summary>
Motivation: To address the incompatibility and memory-access overhead in deep-learning-based image restoration on mobile NPUs.

Method: A teacher-student model is utilized, where a high-capacity teacher supervises a lightweight student network designed for NPU architectures, emphasizing NPU-native operations and employing a progressive context expansion strategy.

Result: The student model achieved 37.66 dB PSNR and 0.9278 SSIM on the validation benchmark, with significantly reduced parameters while retaining restoration quality.

Conclusion: The study demonstrates that hardware-aware distillation can effectively reconcile high-fidelity denoising with efficient deployment on mobile NPUs, and introduces LiteDenoiseNet, further available for public access.
...
### [49] [Unified Multimodal Visual Tracking with Dual Mixture-of-Experts](https://arxiv.org/abs/2605.03716)
*Lingyi Hong,Jinglun Li,Xinyu Zhou,Kaixun Jiang,Pinxue Guo,Zhaoyu Chen,Runze Li,Xingdong Sheng,Wenqiang Zhang*

Main category: cs.CV

TL;DR: OneTrackerV2 is a unified multi-modal tracking framework that allows end-to-end training across different modalities, achieving state-of-the-art performance and robustness in various tracking tasks.


<details>
  <summary>Details</summary>
Motivation: Current methods in multimodal visual object tracking typically rely on separate or pretrained models, leading to inefficiencies. A unified approach is needed for better scalability and usability.

Method: OneTrackerV2 implements a unified framework using Meta Merger for modality embedding and Dual Mixture-of-Experts to handle spatio-temporal relations and multi-modal knowledge.

Result: The framework achieves state-of-the-art performance on five RGB and RGB+X tracking tasks across 12 benchmarks, demonstrating superior inference efficiency and robustness.

Conclusion: OneTrackerV2 maintains strong performance even after model compression and shows resilience in scenarios where modalities are missing.
...
### [50] [FluxFlow: Conservative Flow-Matching for Astronomical Image Super-Resolution](https://arxiv.org/abs/2605.03749)
...
TL;DR: FluxFlow offers a new framework for astronomical super-resolution, addressing limitations of existing methods by considering atmospheric uncertainties and providing a robust dataset for evaluation.


<details>
  <summary>Details</summary>
Motivation: To enhance ground-to-space astronomical imaging by recovering high-quality images despite the limitations of pixel resolution and atmospheric distortion.

Method: FluxFlow employs a pixel-space flow-matching framework with observation uncertainty and source-region importance, and it includes a Wiener-regularized approach at test time to refine results without training.

Result: FluxFlow shows superior performance in both photometric and scientific accuracy compared to current benchmarks, validated through experiments on the newly created DESI--HST Dataset.

Conclusion: The proposed method presents a significant advancement in super-resolution for ground-based astronomical observations, effectively addressing challenges from atmospheric conditions and training dataset limitations.
...
### [51] [Before Forgetting, Learn to Remember: Revisiting Foundational Learning Failures in LVLM Unlearning Benchmarks](https://arxiv.org/abs/2605.03759)
*JuneHyoung Kwon,MiHyeon Kim,Eunju Lee,JungMin Yun,Byeonggeuk Lim,YoungBin Kim*

Main category: cs.CV

TL;DR: ReMem is introduced as a benchmark to address the memorization and unlearning challenges in Large Vision-Language Models (LVLMs) by ensuring effective memorization and evaluating information erasure.


<details>
  <summary>Details</summary>
Motivation: To tackle the privacy risks posed by LVLMs due to unintentional memorization of sensitive information and to improve the reliability of unlearning evaluations.

Method: ReMem uses data scaling, reasoning-aware QA pairs, and various visual contexts to enforce effective memorization in LVLMs, along with a novel Exposure metric to measure information erasure depth.

Result: Extensive tests show ReMem as a reliable framework for assessing the learning and unlearning dynamics in LVLMs, addressing foundational memorization issues.

Conclusion: ReMem enhances the understanding of LVLMs' behaviors in memorization and information erasure, providing a robust approach for future research on privacy risks in AI.
...
### [52] [GeoTopoDiff: Learning Geometry--Topology Graph Priors through Boundary-Constrained Mixed Diffusion for Sparse-Slice 3D Porous Reconstruction](https://arxiv.org/abs/2605.03764)
*Yue Shi,Peng Wang,Mingzhe Yu,Yunlong Zhao,Li Liu,Gareth D Hatton,Yan Lyu,Liangxiu Han*

Main category: cs.CV

TL;DR: GeoTopoDiff enhances 3D porous microstructure reconstruction from sparse CT scans by employing a graph diffusion model that captures both pore geometry and topology, achieving significant error reductions in morphology and transport.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of large-scale 3D porous microstructure reconstruction while balancing throughput and topological fidelity from incomplete CT data.

Method: A graph diffusion-based framework (GeoTopoDiff) is introduced that transfers learning from voxel space to a mixed graph space, incorporating both continuous geometry and discrete topology, with a topology-aware partial graph prior guiding the denoising process.

Result: GeoTopoDiff demonstrated an average reduction of 19.8% in morphology-related errors and 36.5% in topology-sensitive transport errors based on experiments with PTFE and Fontainebleau sandstone.

Conclusion: The mixed graph state space effectively enhances the diffusion denoising process, lowering posterior uncertainty from sparse observations, and promotes further research through public access to models and code.
...
### [53] [ReLeaf: Benchmarking Leaf Segmentation across Domains and Species](https://arxiv.org/abs/2605.03784)
*Robert Martinko,Daniel Steininger,Julia Simon,Andreas Trondl,Matthias Blaickner*

Main category: cs.CV

TL;DR: The paper addresses the challenges of leaf-level segmentation in agriculture due to limited datasets and evaluates instance-segmentation models, introducing a new benchmark dataset.


<details>
  <summary>Details</summary>
Motivation: To meet rising global food demand and climate pressures through sustainable agricultural practices that require precise plant analysis and treatment.

Method: The study compares different instance-segmentation models using public datasets and introduces a new benchmark dataset with semi-automatically annotated leaf masks for diverse plant species.

Result: The YOLO26 model configuration was found to be optimal for precision agriculture tasks, with notable performance drops observed across different plant species and datasets.

Conclusion: The introduction of a new benchmark dataset and the need for diverse datasets are crucial for enhancing model performance and generalization in leaf-level segmentation for precision agriculture.
...
### [54] [A Robust Unsupervised Domain Adaptation Framework for Medical Image Classification Using RKHS-MMD](https://arxiv.org/abs/2605.03787)
*Sapna Sachan,Rakesh Kumar Sanodiya,Amulya Kumar Mahto*

Main category: cs.CV

TL;DR: This paper addresses the challenge of labeling medical images by proposing an unsupervised domain adaptation framework that enhances model generalization across different medical datasets.


<details>
  <summary>Details</summary>
Motivation: Labeling medical images is hindered by the need for expert knowledge and variability in data across medical centers and imaging devices, leading to limited model generalization.

Method: An unsupervised domain adaptation framework combining transfer learning with Reproducing Kernel Hilbert Space based Maximum Mean Discrepancy loss to align source and target domains.

Result: Improvement in generalization to unannotated medical datasets and performance on two chest X-ray datasets, showing better results than models without adaptation.

Conclusion: RKHS-MMD outperforms standard MMD in bridging the modality gap and demonstrates strong efficacy for medical image classification and AI-driven diagnostics.
...
### [55] [Enhancing Visual Question Answering with Multimodal LLMs via Chain-of-Question Guided Retrieval-Augmented Generation](https://arxiv.org/abs/2605.03790)
*Quanxing Xu,Ling Zhou,Xian Zhong,Xiaohua Huang,Rubing Huang,Chia-Wen Lin*

Main category: cs.CV

TL;DR: This paper introduces CoVQD, a logical prompting strategy that enhances retrieval-based Visual Question Answering (VQA) using Multimodal Large Language Models (MLLMs) by combining Chain-of-Thought reasoning with Visual Question Decomposition.


<details>
  <summary>Details</summary>
Motivation: To enhance retrieval-based Visual Question Answering (VQA) by integrating MLLMs with structured reasoning and effective knowledge acquisition, particularly in open-domain scenarios.

Method: The authors introduce a logical prompting strategy (CoVQD) that merges Chain-of-Thought reasoning with Visual Question Decomposition, guiding retrieval processes for improved knowledge relevance in MLLM inference. They also propose the CoVQD-guided RAG (CgRAG) framework to facilitate access to structured external knowledge.

Result: Experiments on E-VQA, InfoSeek, and OKVQA benchmarks show that the proposed method effectively improves the performance of VQA by integrating better reasoning and knowledge retrieval.

Conclusion: The CoVQD and CgRAG frameworks significantly enhance the generalization and reliability of MLLMs in complex cross-domain VQA tasks.
...
### [56] [Multimodal Learning on Low-Quality Data with Conformal Predictive Self-Calibration](https://arxiv.org/abs/2605.03820)
*Xun Jiang,Yufan Gu,Disen Hu,Yuqing Hou,Yazhou Yao,Fumin Shen,Heng Tao Shen,Xing Xu*

Main category: cs.CV

TL;DR: This paper presents a unified framework called CPSC for enhancing multimodal learning by addressing low-quality data issues through predictive uncertainty and self-calibration.


<details>
  <summary>Details</summary>
Motivation: To tackle the challenges posed by low-quality data in multimodal learning, particularly modality imbalance and noisy corruption, which stem from predictive uncertainty.

Method: The CPSC framework employs a self-calibrating training loop that integrates Representation Self-Calibration and Gradient Self-Calibration to enhance feature resilience and optimize gradient flow respectively, along with a self-update strategy for the conformal predictor.

Result: CPSC outperforms existing state-of-the-art methods across six benchmark datasets under both imbalanced and noisy conditions, demonstrating superior performance in addressing data quality challenges.

Conclusion: The proposed CPSC framework effectively enhances multimodal learning by enabling self-guided calibration and consistently improves model robustness against low-quality data.
...
### [57] [Identity-Consistent Multi-Pose Generation of Contactless Fingerprints](https://arxiv.org/abs/2605.03830)
*Zhiyu Pan,Xiongjun Guan,Jianjiang Feng,Jie Zhou*

Main category: cs.CV

TL;DR: This paper presents IMPOSE, a novel framework for generating identity-consistent contactless fingerprint samples that mitigate cross-modal inconsistencies between contactless and contact-based fingerprints.


<details>
  <summary>Details</summary>
Motivation: To address severe geometric distortions and the domain gap between contactless and conventional contact-based fingerprints, enabling more reliable fingerprint recognition without physical contact.

Method: IMPOSE synthesizes multi-pose contactless fingerprints through three stages: generating rolled fingerprints via latent diffusion, translating them to contactless modality using local adaptive binarization, and simulating multi-pose fingerprints through 3D modeling techniques.

Result: IMPOSE achieves state-of-the-art cross-modal matching performance, notably reducing the equal error rate (EER) to 8.74% on UWA and 2.26% on PolyU CL2CB databases after fine-tuning recognition models with synthetic data.

Conclusion: The method not only preserves identity consistency and spatial alignment but also improves recognition metrics across various mainstream representations, demonstrating the effectiveness of combining synthetic and real data.
...
### [58] [Conditions for well-posed color recovery in scattering media](https://arxiv.org/abs/2605.03837)
*Grigory Solomatov,Derya Akkaynak*

Main category: cs.CV

TL;DR: The paper addresses the challenge of recovering scene color from images in scattering media, proposing conditions for well-posedness by identifying inherent patterns and relationships in the image data.


<details>
  <summary>Details</summary>
Motivation: To address the ill-posed nature of color recovery in scattering media, where multiple solutions can lead to prediction errors without a clear understanding of the solution space.

Method: The authors identify conditions that make the problem well-posed, focusing on recovery patterns and cross-pixel relationships, specifically using an ideal hyperspectral camera to restrict solutions to a unique candidate.

Result: By establishing criteria for unique solution pathways in scattering environments, they demonstrate that sensor improvements alone cannot address distortions without additional constraints.

Conclusion: The findings pave the way for new vision algorithms based on theoretical foundations, enhancing the quantitative analysis of images in complex scattering environments.
...
### [59] [Parameter-Efficient Multi-View Proficiency Estimation: From Discriminative Classification to Generative Feedback](https://arxiv.org/abs/2605.03848)
*Edoardo Bianchi,Antonio Liotta*

Main category: cs.CV

TL;DR: This paper discusses advancements in multi-view proficiency estimation for action performance, highlighting three methods that improve accuracy while reducing complexity.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of estimating action performance in coaching, rehabilitation, and talent identification, focusing on subtle proficiency indicators across multiple views.

Method: Introduction of three methods: 1) SkillFormer for selective multi-view fusion, 2) PATS for improved temporal sampling of movements, and 3) ProfVLM for conditional language generation for proficiency estimation and feedback.

Result: These methods achieve state-of-the-art accuracy with significantly fewer trainable parameters and training epochs compared to existing models.

Conclusion: The advancements suggest a trend towards more efficient multi-view systems capable of generating interpretable feedback.
...
### [60] [Stream-R1: Reliability-Perplexity Aware Reward Distillation for Streaming Video Generation](https://arxiv.org/abs/2605.03849)
*Bin Wu,Mengqi Huang,Shaojin Wu,Weinan Jia,Yuxin Wang,Zhendong Mao,Yongdong Zhang*

Main category: cs.CV

TL;DR: Stream-R1 enhances distillation-based acceleration for video diffusion models by adaptively reweighting the learning objective based on reliability and perplexity to improve output quality.


<details>
  <summary>Details</summary>
Motivation: To improve distilled quality by addressing the indiscriminate treatment of rollouts and optimizing where quality can improve.

Method: Stream-R1 employs a Reliability-Perplexity Aware Reward Distillation framework that adjusts weights at the rollout and spatiotemporal levels using a reward-guided mechanism.

Result: Achieves consistent improvements across visual quality, motion quality, and text alignment on streaming video generation benchmarks compared to traditional distillation methods.

Conclusion: Stream-R1 effectively enhances the performance of autoregressive streaming video diffusion models without needing architectural changes or increasing inference costs.
...
TL;DR: This paper introduces the Dual Matching Guided Diffusion (DMGD) framework for dataset distillation that improves training efficiency by using a novel guidance mechanism and optimal transport methods without requiring fine-tuning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the dataset distillation process by addressing the inefficiencies of existing diffusion-based methods that require fine-tuning and lack effective guidance.

Method: The method involves a Dual Matching Guided Diffusion (DMGD) framework that includes Semantic Matching through conditional likelihood optimization, a dynamic guidance mechanism for data diversity, and an optimal transport-based Distribution Matching strategy.

Result: Experimental results on various ImageNet datasets show that the proposed approach outperforms state-of-the-art methods requiring fine-tuning, achieving accuracy improvements of 2.1% on ImageNet-Woof, 5.4% on ImageNet-Nette, and 2.4% on ImageNet-1K.

Conclusion: The paper concludes that the DMGD framework provides a significant advancement in dataset distillation, offering efficient training-free methods that yield superior accuracy compared to existing approaches.
...
TL;DR: The paper introduces a novel mixture model for estimating empirical fixation densities from eye-tracking data, improving interobserver consistency and benchmarking in saliency models.


<details>
  <summary>Details</summary>
Motivation: To enhance the accuracy of fixation density estimates used in saliency benchmarking, particularly as the field transitions to sample-level evaluation.

Method: A principled mixture model combining adaptive-bandwidth KDE, center bias, uniform components, and a saliency model, with parameter optimization via leave-one-subject-out cross-validation.

Result: The proposed method shows significantly improved interobserver consistency, with median gains of 5-15% in log-likelihood and up to 2 percentage points in AUC for various benchmarks, especially for images relevant to failure case analyses.

Conclusion: Empirical fixation densities should be regarded as dynamic estimates that evolve with methodology; there remains substantial potential for improving saliency models.
...
TL;DR: This paper addresses the limitations of Vision-Language Models (VLMs) in numerical reasoning for robotic tasks, introducing a novel training strategy that enhances object detection and localization.


<details>
  <summary>Details</summary>
Motivation: VLMs excel in combining visual and linguistic information, but they struggle with numerical reasoning, which is critical for tasks like object detection and state localization in robotics.

Method: The authors propose an Auxiliary Regression Loss (ARL) that is included during fine-tuning with box decoder outputs, while maintaining standard sequence prediction at inference. This leads to the development of StateVLM, a model aimed at improving object representation and localization accuracy.

Result: StateVLM with ARL demonstrates a performance improvement of 1.6% on adapted benchmarks and 5.2% on the newly introduced OSAR benchmark, indicating enhanced numerical reasoning capabilities.

Conclusion: The study underlines the significance of ARL in advancing VLM performance in affordance reasoning tasks, emphasizing its role in producing consistent and accurate outputs.
...
TL;DR: iWorld-Bench is a proposed benchmark for evaluating world models in AGI through a comprehensive dataset and unified evaluation framework, addressing the lack of large-scale datasets for assessing interaction abilities.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for robust datasets and benchmarks to evaluate the physical interaction capabilities of agents learning toward AGI.

Method: The authors created iWorld-Bench, a benchmark consisting of 330k video clips and 2.1k carefully selected samples, and developed an Action Generation Framework to unify evaluation across six diverse task types.

Result: Fourteen world models were evaluated, revealing key limitations in their performance across visual generation, trajectory following, and memory tasks.

Conclusion: The study highlights the need for improved evaluation methods in AGI research and provides a publicly accessible leaderboard to guide future advancements.
...
TL;DR: UnAC improves reasoning in LMMs for complex multimodal tasks with new prompting strategies.


<details>
  <summary>Details</summary>
Motivation: To enhance multi-step reasoning and reliability in LMMs dealing with visual evidence.

Method: Introduces adaptive visual prompting for salient focus, an image-abstraction prompt for key information extraction, and a gradual self-checking scheme for verifying reasoning.

Result: Demonstrates enhanced performance on benchmarks MathVista, MM-Vet, and MMMU, indicating improved reasoning and image understanding.

Conclusion: UnAC effectively strengthens visual reasoning in LMMs, addressing the limitations in current models.
...
TL;DR: This paper presents a weakly supervised framework for detecting schools from aerial imagery, aimed at overcoming challenges associated with traditional manual mapping methods.


<details>
  <summary>Details</summary>
Motivation: The study addresses the need for accurate school detection to aid education initiatives, particularly in regions with outdated or incomplete records, and to facilitate efficient infrastructure planning.

Method: The proposed method uses a two-stage training pipeline: first, it automatically labels images using sparse location points and semantic segmentation to create bounding boxes; second, it fine-tunes the model using a small dataset of manually labeled images.

Result: The results indicate strong object detection performance in low-data regimes, achieving effective detection with only 50 manually labeled images, thus minimizing reliance on extensive human annotations.

Conclusion: The framework provides a scalable approach for mapping school infrastructure globally, promoting education and connectivity initiatives, with all related resources made publicly available for future research.
...
TL;DR: This paper presents a pipeline for reconstructing a 3D human face model from a single RGB image using convolutional neural networks (CNNs), addressing challenges in generating detailed and photo-realistic face models.


<details>
  <summary>Details</summary>
Motivation: To enhance 3D face reconstruction from 2D images while addressing the limitations of coarse morphable face models in generating detailed features.

Method: The proposed pipeline consists of face detection, landmark detection, regression of 3D Morphable Model (3DMM) parameters, and soft rendering techniques.

Result: The pipeline successfully reconstructs detailed and photo-realistic 3D human face models from single RGB images.

Conclusion: This method improves the quality of 3D face reconstruction by leveraging advanced CNN techniques and synthesizing realistic features.
...
TL;DR: RD-ViT is a Recurrent-Depth Vision Transformer that enhances segmentation tasks while reducing parameter count and improving computational efficiency.


<details>
  <summary>Details</summary>
Motivation: The need for improved segmentation accuracy in ViTs with limited training datasets and unique parameter learning requirements.

Method: The RD-ViT employs a shared transformer block with LTI-stable state injection, Adaptive Computation Time (ACT) for compute allocation, depth-wise LoRA adaptation, and optional Mixture-of-Experts (MoE) for specialization.

Result: In 2D segmentation, RD-ViT outperforms standard ViT using only 10% of the training data. In 3D segmentation, it achieves competitive performance with fewer parameters.

Conclusion: RD-ViT demonstrates effectiveness in cardiac MRI segmentation, with flexible depth for inference and significant computational efficiency improvements.
...
TL;DR: The paper presents a state-of-the-art approach for early detection of brain tumors using the SegResNet architecture, achieving notable dice scores in segmentation metrics for various tumor types.


<details>
  <summary>Details</summary>
Motivation: Early detection of brain tumors significantly influences patient survival rates, necessitating improved identification methods.

Method: Utilized the SegResNet architecture for three-dimensional tumor segmentation, trained with an automatic multi-precision method, and employed the dice loss function for evaluation.

Result: Achieved a dice score of 0.84 for the tumor core, 0.90 for the whole tumor, and 0.79 for the enhanced tumor, indicating effective segmentation performance.

Conclusion: The proposed method offers a robust approach to early brain tumor detection, demonstrating strong segmentation accuracy as indicated by the dice scores.
...
TL;DR: HeadsUp introduces a scalable method for high-quality 3D Gaussian head reconstruction using a novel encoder-decoder architecture, achieving state-of-the-art results and enabling new applications.


<details>
  <summary>Details</summary>
Motivation: The goal is to reconstruct 3D Gaussian heads efficiently from a large array of multi-camera inputs, addressing limitations of existing datasets and methods.

Method: A feed-forward encoder-decoder architecture compresses input images into a latent representation, which is then used to generate UV-parameterized 3D Gaussians anchored to a neutral head template.

Result: HeadsUp demonstrates superior reconstruction quality and performs well on new identities without requiring optimization during testing, trained on a dataset with over 10,000 subjects.

Conclusion: The analysis of model scaling offers valuable insights for trade-offs in quality and computational efficiency, showcasing the latent space's potential for creating new 3D identities and animating expressions.
...
TL;DR: UniReasoner improves text-to-image generation by addressing the understanding-generation gap using LLM-based reasoning for better prompt alignment and semantic accuracy.


<details>
  <summary>Details</summary>
Motivation: To tackle the issue where advanced text-to-image generation systems fail to align complex prompts with generated images, despite being able to verify prompt adherence.

Method: UniReasoner employs an LLM to first create a visual draft of the scene and then critiques this draft by assessing its alignment with the prompt, subsequently guiding a diffusion model with this critique to enhance image generation accuracy.

Result: Experiments demonstrate that UniReasoner enhances compositional alignment and semantic faithfulness in image generation while maintaining high image quality, effectively closing the understanding-generation gap.

Conclusion: The framework showcases a practical approach to leveraging LLM reasoning in improving the fidelity of text-to-image synthesis.
...
TL;DR: This survey reviews advancements in Audio-Visual Intelligence (AVI), emphasizing large foundation models and establishing a unified taxonomy for tasks and methodologies.


<details>
  <summary>Details</summary>
Motivation: To bridge gaps in the fragmented literature on AVI by providing a systematic and comprehensive review, particularly focusing on large foundation models.

Method: The paper develops a unified taxonomy for AVI tasks and synthesizes methodologies, datasets, and evaluation metrics, thereby facilitating structured comparisons and identifying challenges.

Result: The survey highlights the diversity in tasks, methodologies, and metrics in AVI, proposing a coherent framework for future research.

Conclusion: Consolidating efforts in AVI, this work aims to provide a foundational reference and guide for ongoing and future developments in the field.
...
TL;DR: UniCorrn is a unified correspondence model for geometric matching across 2D-2D, 2D-3D, and 3D-3D tasks, leveraging shared weights and Transformer attention for cross-modal feature similarity.


<details>
  <summary>Details</summary>
Motivation: To address the fragmented approach in geometric matching where separate models are employed for different modality combinations, thus streamlining the process.

Method: UniCorrn employs a dual-stream decoder with separate appearance and positional feature streams while utilizing Transformer attention to capture feature similarity, allowing for end-to-end learning across varied modalities.

Result: Demonstrated competitive performance in 2D-2D matching and achieved an 8% improvement on 7Scenes for 2D-3D, and 10% on 3DLoMatch for 3D-3D tasks in registration recall.

Conclusion: UniCorrn successfully unifies geometric matching across multiple modalities, outperforming existing methods and indicating the effectiveness of a shared-weight architecture.
...
TL;DR: Self-verification shows variable effectiveness in selective prediction compared to likelihood-based baselines, with significant task- and model-dependent performance.


<details>
  <summary>Details</summary>
Motivation: To evaluate the practical value of same-model self-verification as a confidence signal for selective prediction against strong likelihood-based baselines.

Method: Self-verification was tested against LL-AVG and LL-SUM on ARC-Challenge and TruthfulQA-MC datasets, measuring both correctness ranking and abstention quality through AURC and operating-point analyses across various model families and prompt variants.

Result: Self-verification outperformed LL-AVG on ARC-Challenge for certain models like Phi-2 and Qwen, particularly Qwen-7B, but was less reliable on TruthfulQA-MC, with smaller models showing prompt sensitivity and LL-SUM often performing better.

Conclusion: Self-verification is not a universal uncertainty estimator but a conditional confidence signal whose effectiveness is influenced by task type, model family, prompt formulation, and the competing baseline.
...
TL;DR: The paper evaluates the ability of large reasoning models (LRMs) to address user queries that contain false assumptions, finding they perform slightly better than non-reasoning models but still inadequately challenge many inaccuracies.


<details>
  <summary>Details</summary>
Motivation: To investigate whether recent advancements in large reasoning models enable them to effectively address and challenge users' factually inaccurate assumptions in various queries.

Method: The authors constructed user queries with differing levels of presuppositions in health, science, and general knowledge and evaluated multiple widely-used models to assess their responses.

Result: Reasoning models exhibited a modest increase in accuracy (2-11%) compared to non-reasoning models but still failed to challenge a significant percentage (26-42%) of erroneous presuppositions.

Conclusion: Despite improvements in reasoning capabilities, LRMs do not sufficiently address inaccurate user assumptions and remain influenced by the strength of those presuppositions.
...
TL;DR: An innovative method called SURE-RAG improves selective retrieval-augmented generation (RAG) by verifying evidence sufficiency for answer support, leading to better performance in multi-hop question answering.


<details>
  <summary>Details</summary>
Motivation: The necessity to ensure that retrieved evidence not only relates to a question but sufficiently supports the provided answer in retrieval-augmented generation (RAG).

Method: SURE-RAG utilizes a shared pair-level claim-evidence verifier to assess evidence sufficiency, aggregating relation distributions and producing interpretable signals (coverage, relation strength, etc.) for decision-making.

Result: SURE-RAG achieves a Macro-F1 score of 0.9075, outperforming prior models like DeBERTa and GPT-4o, with a significant reduction in unsafe answers compared to traditional methods.

Conclusion: This study illustrates that evidence sufficiency verification can enhance RAG systems, distinguishing it from natural hallucination detection, warranting further examination of varying task boundaries.
...
TL;DR: This paper investigates how Large Language Models (LLMs) handle negation, revealing that while they exhibit incorrect answers to negation tasks, they have internal structures capable of processing negation. Their inaccuracies result from late-layer attention issues. Through various interpretability methods, the study shows LLMs utilize both attention suppression and constructive representation of negation, with a particular emphasis on the latter.


<details>
  <summary>Details</summary>
Motivation: To understand the mechanistic processing of negation in Large Language Models, revealing strengths and weaknesses in handling negation-related queries.

Method: Using observational and causal interpretability techniques on LLMs Mistral-7B and Llama-3.1-8B to analyze their internal mechanisms and attention behaviors regarding negation.

Result: Models exhibit both direct attention to negation phrases and the construction of comprehensive negative representations. The constructive approach is found to be the dominant mechanism.

Conclusion: This research enhances understanding of LLM internals, emphasizing construction-centric operations and the interaction of various mechanisms in processing negation.
...
### [78] [The TTS-STT Flywheel: Synthetic Entity-Dense Audio Closes the Indic ASR Gap Where Commercial and Open-Source Systems Fail](https://arxiv.org/abs/2605.03073)
*Venkata Pushpak Teja Menta*

Main category: cs.CL

TL;DR: This study addresses the under-performance of Indic Automatic Speech Recognition (ASR) systems for niche-domain applications. It proposes a self-contained TTS-STT pipeline that significantly improves entity recognition rates using a fine-tuned model, while revealing limitations in cross-language performance.


<details>
  <summary>Details</summary>
Motivation: To improve Automatic Speech Recognition for niche Indic domains that are not well supported by existing systems.

Method: The authors develop an open-source TTS-STT pipeline and fine-tune a model (LoRA) on a synthesized entity-dense dataset, generating a large volume of utterances for training and testing.

Result: The fine-tuned model achieves a significant improvement in Entity-Hit-Rate (0.473), outperforming both open-source and commercial systems, although some targets are not met. A sanity check indicates effective transfer to real speech.

Conclusion: The proposed approach enhances Indic ASR performance significantly, with noted limitations in performance against commercial systems in certain languages and challenges in meeting predefined targets.
...
### [79] [Rethinking Reasoning-Intensive Retrieval: Evaluating and Advancing Retrievers in Agentic Search Systems](https://arxiv.org/abs/2605.04018)
*Yilun Zhao,Jinbiao Wei,Tingyu Song,Siyue Zhang,Chen Zhao,Arman Cohan*

Main category: cs.CL

TL;DR: The paper presents BRIGHT-Pro, an expert-annotated benchmark for reasoning-intensive retrieval, and RTriever-Synth, a synthetic corpus for enhancing evidence retrieval. Experiments reveal improvements in retriever performance under enhanced evaluation conditions.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for better evaluation and training methods in reasoning-intensive retrieval, necessary for supporting complex search and synthesis tasks in agentic systems.

Method: Introduction of BRIGHT-Pro for multi-aspect evidence evaluation and RTriever-Synth for creating a synthetic corpus that allows for positive and hard negative examples; fine-tuning was conducted using LoRA on RTriever-4B model.

Result: Experiments demonstrate that using aspect-aware evaluations reveals hidden behaviors in retriever performance, and RTriever-4B shows significant improvements over the base model in multiple retrieval scenarios.

Conclusion: The proposed methods enhance the evaluation and training of reasoning-intensive retrieval systems, contributing to improved performance in evidence-based information retrieval.
...
TL;DR: The paper enhances financial sentiment analysis by adding semantically structured opinion graphs to the StockEmotions dataset, improving classification performance in emotion detection using GNNs.


<details>
  <summary>Details</summary>
Motivation: To tackle the challenge of understanding the reasons behind sentiment in financial NLP, beyond just identifying sentiment itself.

Method: The authors augment the StockEmotions dataset with opinion graphs for each sentence from 10,000 comments on StockTwits, applying a declarative LLM pipeline and testing with Graph Neural Networks (GNNs).

Result: The analysis indicates that the integration of opinion semantics leads to improved classification performance across various emotional categories.

Conclusion: Incorporating opinion semantics into sentiment analysis for financial NLP enhances the ability to classify emotional nuances effectively.
...
TL;DR: MedStruct-S benchmark evaluates semi-structured information extraction tasks from OCR-derived clinical reports, addressing key discovery, QA, and extraction under real-world conditions of unknown keys and OCR noise.


<details>
  <summary>Details</summary>
Motivation: Address the challenges in evaluating model robustness for semi-structured information extraction from OCR-derived clinical reports, considering heterogeneous key representations and OCR noise.

Method: A benchmark named MedStruct-S was created with 3,582 annotated clinical report pages to evaluate encoder-only and decoder-only models in three key tasks: key discovery, QA, and key-value extraction.

Result: Encoder-only models outperformed decoder-only models in key-conditioned QA, while fine-tuned decoder models showed stronger results overall when disregarding model scale.

Conclusion: MedStruct-S serves as a practical benchmark for comparing models in semi-structured information extraction tasks, particularly regarding handling unknown keys and OCR-induced challenges.
...
TL;DR: This paper addresses the challenges of extracting information from earnings calls, highlights the limitations of SEC-trained models, introduces new benchmarking datasets, and proposes a LLM-based system for extracting key performance indicators from call transcripts.


<details>
  <summary>Details</summary>
Motivation: To improve information extraction from earnings calls, which are unstructured and conversational in nature, compared to structured SEC filings.

Method: The authors evaluate SEC-trained models on established SEC datasets, introduce three benchmarks (SECB, ECB, and ECB-A), and propose a system utilizing LLMs for open-ended extraction, validated by human evaluation metrics.

Result: The study reveals that encoder-based models face difficulties due to domain shifts and introduces novel benchmarks for future evaluation. The proposed LLM system achieved a precision score of 79.7%.

Conclusion: The paper establishes a baseline for information extraction from earnings calls and highlights the effectiveness of LLMs in tackling the challenges posed by unstructured data.
...
TL;DR: This paper examines whether the geometric representation of hidden states in language models can signal when a question is unanswerable, across several model forms and tasks.


<details>
  <summary>Details</summary>
Motivation: To create a reliable signal indicating when a query falls outside the model's knowledge without needing labeled failure data or access to outputs.

Method: The study analyzes three instruction-tuned models using various prompts and measures the deviation of hidden states from an answerable reference set, examining performance in signal detection.

Result: The analysis reveals that geometry primarily encodes task forms, with significant performance in mathematical tasks (ROC-AUC 0.78-0.84) but less reliable signals for factual prompts; geometric signals appear strongest in early model layers.

Conclusion: Geometric deviation can effectively signal unanswerability in structured domains, demonstrating potential as a lightweight pre-generation tool with specified generalization limits.
...
TL;DR: The paper presents S^2tory, a framework for automatic summarization of movie scripts, which identifies key plot events through character trajectories, resulting in high-quality summaries.


<details>
  <summary>Details</summary>
Motivation: Automatic summarization of movie scripts is challenging due to their non-linear narratives and the limitations of traditional saliency methods.

Method: The S^2tory framework utilizes character development trajectories to identify plot nuclei while filtering out less relevant events, employing a Narrative Expert Agent for reasoning, followed by summarization from identified nuclei.

Result: Experiments reveal state-of-the-art semantic fidelity in summaries with 3.5x compression on the MovieSum dataset and strong generalization in zero-shot settings on BookSum.

Conclusion: The use of narratological theory is crucial for accurately modeling and summarizing complex movie narratives.
...
TL;DR: LLM-XTM enhances cross-lingual topic modeling by integrating LLM-guided refinement and uncertainty quantification, improving coherence and alignment while reducing resource dependency.


<details>
  <summary>Details</summary>
Motivation: Existing cross-lingual topic models are hindered by sparse bilingual resources and issues of coherence, alignment, and hallucination, necessitating a more effective approach.

Method: LLM-XTM combines LLM-guided topic refinement with self-consistency uncertainty quantification to provide a stable and scalable framework for topic modeling across languages.

Result: Experiments demonstrate that LLM-XTM achieves better topic coherence and alignment compared to previous models, while minimizing the need for bilingual resources and expensive LLM usage.

Conclusion: LLM-XTM presents a novel enhancement to cross-lingual topic modeling, addressing key challenges and demonstrating improved performance.
...
TL;DR: This paper presents SHIELD, a diverse dataset for de-identification of clinical texts, created to enhance the secondary use of EHRs, and evaluates its performance against existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the limitations of outdated public benchmarks for de-identifying clinical texts and the computational challenges posed by LLMs in practical applications.

Method: SHIELD comprises 1,394 clinical notes with identified PHI spans, created using set-cover diversity sampling and human adjudication, and evaluates four different LLMs to inform the development of locally deployable SLMs.

Result: The study finds that the best distilled model matches its more complex teacher on structured PHI categories with high precision and recall, and that diversity-trained models generalize well across datasets while specialized models are needed for specific entities.

Conclusion: The SHIELD dataset and distilled DeBERTa v3 model provide significant improvements in the de-identification of clinical texts, and their release supports further research in this area.
...
TL;DR: The paper introduces Side-by-Side (SxS) Interleaved Reasoning, a method that manages the timing of disclosure in autoregressive models to balance accuracy and content latency in response generation.


<details>
  <summary>Details</summary>
Motivation: The challenge of irreversible commitments in single-stream autoregressive interfaces leads to a silence tax, where the timing of task-relevant content is crucial for performance.

Method: SxS interleaves partial disclosures with private reasoning, releasing content only when it is supported by previous reasoning. It uses entailment-aligned trajectories for training, combining supervised fine-tuning and reinforcement learning.

Result: SxS demonstrates improvements in accuracy and latency trade-offs on various benchmarks and models, including the MoE Qwen3-30B-A3B and dense Qwen3-4B across in-domain and out-of-domain datasets.

Conclusion: The SxS method successfully enhances the reasoning capabilities of autoregressive models by allowing for better timing control of content disclosure.
...
TL;DR: This study presents a retrieval-augmented generation translation system that enhances Japanese-Chinese sentence translation, particularly for noun-modifying clause constructions.


<details>
  <summary>Details</summary>
Motivation: To address the reliability issues of large language models in translating Japanese-Chinese sentences, specifically those with complex structures like NMCCs.

Method: A RAG+Prompt translation system integrating linguistic analysis, retrieval, prompt construction, and LLM generation was utilized without altering the base model, evaluated with varying sizes of knowledge bases.

Result: The mean BLEU score improved significantly from 24.28 to 29.96 with the largest knowledge base, indicating that a larger knowledge base positively impacts translation quality.

Conclusion: The RAG+Prompt translation system enhances the translation of NMCC-containing sentences in a transparent manner; however, further research is required to expand genres and metrics.
...
TL;DR: This study compares the outputs of three named-entity recognition (NER) tools for identifying geolocation in German news articles related to extreme climate events.


<details>
  <summary>Details</summary>
Motivation: The study addresses the challenge of effectively geolocating climate events in texts, a crucial aspect for climate impact and adaptation research.

Method: A comparative analysis of Flair, Spacy, and Stanza NER tools is conducted on German news articles, evaluating their outputs and extrinsic performance based on three methods to determine event locations.

Result: The analysis illustrates significant differences in the NER tools' outputs, affecting the identification of event locations and influencing conclusions about media representation of countries.

Conclusion: The study underscores the importance of tool selection in NER, as it can lead to varying interpretations of geographical focus in climate-related media coverage.
...
TL;DR: The paper compares traditional ML and Transformer-based models for sentiment analysis of e-commerce reviews in Indonesia, finding that a traditional Linear SVC model outperforms IndoBERT due to data sampling differences.


<details>
  <summary>Details</summary>
Motivation: To analyze user-generated product reviews for better customer satisfaction insights on e-commerce platforms in Indonesia.

Method: Compared the performance of Logistic Regression, Linear SVC, and Multinomial Naive Bayes against a fine-tuned IndoBERT model on a three-class sentiment analysis task using the Tokopedia Product Reviews 2025 dataset, with adjustments for class imbalance.

Result: Linear SVC achieved an Accuracy of 97.60% and Macro F1-score of 0.5510, outperforming IndoBERT, which reached an Accuracy of 88.70% and Macro F1-score of 0.5088.

Conclusion: The performance gap is attributed to differences in data sampling methods; the study also showcases a functional sentiment classification model via a web application.
...
TL;DR: This study analyzes email classification and sentiment detection performance, comparing traditional machine learning algorithms and deep learning approaches.


<details>
  <summary>Details</summary>
Motivation: To improve email classification and sentiment detection in response to the growing demand for efficient electronic communication systems.

Method: A comparative analysis of traditional algorithms (SVM, Logistic Regression, Naive Bayes) and deep learning (LSTM) using Word2Vec embeddings.

Result: The SVM model with a linear kernel achieved peak performance of 98.74% accuracy, while LSTM showed high recall but at the cost of increased computational time.

Conclusion: SVM is determined to be the most effective for email detection, balancing precision and processing speed, vital for developing automated filtering systems.
...
TL;DR: This study examines recursive sentence processing in Mandarin-speaking children with autism spectrum disorder (ASD) using ERP techniques, revealing atypical temporal brain response patterns compared to typically developing peers.


<details>
  <summary>Details</summary>
Motivation: To investigate how children with ASD process complex recursive structures in language, an area with limited prior research.

Method: An ERP study comparing 12 children with ASD to 12 typically developing peers, using a sentence-picture matching paradigm to analyze neural responses.

Result: ASD children showed reduced P200 amplitudes, a lack of typical grammaticality effect, and attenuated P600 responses, indicating deficits in early perceptual processing and syntactic reanalysis.

Conclusion: The findings suggest that while lexical-semantic abilities are intact, children with ASD have significant impairments in the syntactic processing necessary for recursion, supporting the notion of modular dissociation in language processing.
...
TL;DR: This paper explores using Topic Models as binary classifiers for improving the retrieval of news concerning extreme climate events in Germany.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the challenge of insufficient annotated data for training deep learning classifiers in the domain of media coverage of climate events.

Method: The approach utilizes the posterior distributions from Topic Models to identify relevant documents while maintaining their training procedure, informed by an annotated sample for evaluation.

Result: The probabilities of keywords for querying news databases enhance the selection of relevant topics, improving precision; comparisons are made with text embedding classifiers and open-weight LLMs, highlighting trade-offs in results.

Conclusion: Results indicate that categorizing climate events as a single type is inadequate, as outcomes vary depending on the specific hazard.
...
TL;DR: This paper introduces Dynamic Emotional Signature Graphs (DESG) as a novel evaluation method for conversational AI therapists, addressing shortcomings in existing LLM-based assessments of therapeutic dialogue quality.


<details>
  <summary>Details</summary>
Motivation: There is a need for reliable offline evaluation of therapeutic responses in conversational AI settings without large language models as final judges due to misalignment in evaluation metrics.

Method: The paper proposes DESG, a model-agnostic evaluator that utilizes asymmetric clinical geometry to score dialogue windows based on decoupled clinical states, evaluated on a benchmark of 3,000 dialogue windows.

Result: DESG-Ensemble achieves a macro-F1 score of 0.9353, outperforming existing models such as ConcatANN, BERTScore, and TRACT by significant margins on a 600-window test set.

Conclusion: The study concludes that the clinical state manifold is crucial for evaluation, while graph-based trajectory components enhance scoring interpretability.
...
TL;DR: CuraView is a multi-agent framework for detecting and explaining faithfulness hallucinations in discharge summaries, improving the safety and accuracy of EHR extracts.


<details>
  <summary>Details</summary>
Motivation: The manual extraction of information from EHRs is labor-intensive, and LLMs often produce unreliable outputs that can endanger patient safety.

Method: CuraView utilizes a GraphRAG-based knowledge graph with a closed-loop generation-detection mechanism that retrieves and classifies evidence at the sentence level, categorizing it into four grades of faithfulness from strong support to contradiction.

Result: CuraView achieved an F1 score of 0.831 on safety-critical evidence (E4), with significant improvements over baseline models, demonstrating enhanced factual reliability in clinical documentation.

Conclusion: The evidence-chain-based graph retrieval method not only improves the accuracy of discharge summaries but also creates valuable annotated datasets for future model training.
...
TL;DR: This study models the historical choices behind morphological compositions in human languages, revealing a trade-off between listener recoverability and speaker production cost that influences which morpheme combinations are favored.


<details>
  <summary>Details</summary>
Motivation: To understand why certain morpheme combinations are preferred in languages rather than arbitrary forms, considering factors like communicative efficiency and speaker choices.

Method: The researchers apply the Rational Speech Act framework using a time-indexed lexicon from historical and contemporary English corpora, analyzing 4323 compounds and derivations from 1820 to 2019.

Result: Attested lexical compositions are systematically prioritized over unattested alternatives, with models that combine semantic informativeness and production costs demonstrating superior performance.

Conclusion: The findings indicate that the process of lexicalization is influenced by a balance between expressiveness and efficiency, highlighting the role of speaker choices in the formation of word structures.
...
TL;DR: This paper investigates whether Graph-Tokenizing LLMs (GTokenLLMs) fully comprehend graph tokens within natural-language embedding spaces. It introduces an evaluation framework, GTEval, revealing that existing GTokenLLMs exhibit limitations in understanding graph tokens and suggest the need for improvement in instruction tuning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to challenge the assumption that GTokenLLMs effectively understand graph tokens in natural-language embedding spaces, spurring the need for a rigorous evaluation.

Method: The authors formalize a unified framework for GTokenLLMs and develop GTEval, an evaluation pipeline that assesses graph-token understanding through instruction transformations at both content and format levels.

Result: Extensive experiments using GTEval across 6 GTokenLLMs reveal that these models do not fully grasp graph tokens, are inconsistently responsive to instructions, and depend heavily on textual reasoning.

Conclusion: The study concludes that while additional instruction tuning can enhance performance, it does not resolve the core issue of graph-token comprehension, indicating a need for further advancements.
...
TL;DR: This paper introduces PatRe, a benchmark modeling the full patent examination lifecycle, emphasizing its dynamic nature. It includes 480 cases for evaluation and reveals insights on LLM performance in legal reasoning.


<details>
  <summary>Details</summary>
Motivation: To address the complexity of patent examination and the inadequacy of prior benchmarks that overlook its interactive nature, similar to academic peer review processes.

Method: The paper introduces the PatRe benchmark, which models the patent examination process, including Office Action generation and applicant rebuttal, and comprises 480 real-world cases with various evaluation settings.

Result: Experiments with various LLMs show differences in performance between proprietary and open-source models, alongside task asymmetries in examiner analysis versus applicant rebuttal.

Conclusion: Current LLMs exhibit both potential and limitations in simulating nuanced legal reasoning and technical judgment in patent examinations; introduced resources aim to promote further research in this area.
...
TL;DR: AfriVox-v2 is a comprehensive benchmark that evaluates speech models for African languages under realistic conditions, highlighting their limitations in noisy environments and various domains.


<details>
  <summary>Details</summary>
Motivation: To address the underrepresentation of African languages in speech recognition and translation benchmarks, particularly in low-resource settings.

Method: AfriVox-v2 introduces unscripted audio and strict verticalization across ten sectors, assessing model accuracy under realistic deployment conditions.

Result: The benchmarking indicates significant generalization gaps of modern speech models in specialized African contexts, exposing challenges related to noise and domain specificity.

Conclusion: AfriVox-v2 serves as a vital resource for developers, providing insights for building effective localized voice AI solutions.
...
TL;DR: This paper discusses the participation of BIT.UA and AAUBS in the ArchEHR-QA 2026 task, focusing on clinical question answering with a low-resource setting using LLMs without weight updates, achieving notable success through prompt engineering and ensembling techniques.


<details>
  <summary>Details</summary>
Motivation: The need for clinical question answering in a low-resource healthcare context, limited by data privacy regulations and lack of training data, motivates the investigation of LLMs without weight updates.

Method: Evaluation of various proprietary and open-source models leveraging prompt engineering techniques such as task decomposition, Chain-of-Thought, and in-context learning, along with ensembling methods for improved performance.

Result: Proprietary models show resilience to prompt changes, while domain-adapted open-source models like MedGemma 3 27B perform competitively when used with appropriate prompts; the approach secured top placements in specific subtasks of the competition.

Conclusion: The effectiveness of prompt-based methodologies in achieving high performance in clinical question answering tasks is confirmed, with all resources made publicly available.
...
TL;DR: This paper explores the impact of different annotation sources on Aspect-Based Sentiment Analysis (ABSA) for German, including comparisons of expert and non-expert annotations and their effects on model performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limited research on ABSA in German due to the scarcity of high-quality annotated datasets and to improve annotation quality.

Method: The existing dataset is re-annotated by experts, and quality is evaluated against annotations from students, crowdworkers, and Large Language Models (LLMs) using Inter-Annotator Agreement (IAA) and various ABSA subtasks.

Result: The study reveals how annotation quality influences model performance across different ABSA tasks and identifies the strengths and weaknesses of different annotation sources.

Conclusion: The findings highlight trade-offs between annotation reliability and efficiency, aiding in dataset construction for under-resourced NLP tasks.
...
TL;DR: The paper critiques traditional metrics like WER and CER for evaluating speech transcriptions and proposes a new metric, Minimum Edit Distance (minED), which better represents human perceptions of transcription errors.


<details>
  <summary>Details</summary>
Motivation: To address the shortcomings of WER and CER in correlating with human perception and to incorporate linguistic and semantic contexts in evaluating speech transcriptions.

Method: Develop a framework that incorporates a chosen metric into calculating an equivalent error rate, specifically Minimum Edit Distance (minED), to align transcription errors with human perception.

Result: minED allows for a better representation of transcription errors in relation to human perception and facilitates an analysis of the severity of these errors.

Conclusion: The proposed method enhances the evaluation of automatic speech transcriptions by offering a metric that is more interpretable and aligned with human judgments of error severity.
...
TL;DR: This study assesses the effectiveness of various subword tokenization and self-supervised learning methods in automatic speech recognition (ASR) for the French language, highlighting inadequacies in traditional error metrics.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of conventional CER and WER metrics that fall short in capturing the practical performance of speech-to-text systems in real applications.

Method: The research involves a qualitative analysis of subword tokenization algorithms and self-supervised models, evaluated through a broad spectrum of metrics beyond just CER and WER.

Result: The findings indicate that certain tokenization and model choices significantly influence ASR performance from both linguistic and acoustic viewpoints.

Conclusion: The study emphasizes the necessity of employing a diverse set of evaluation metrics to more accurately reflect the effectiveness of ASR systems, particularly in practical applications.
...
TL;DR: SERE is a proposed framework to improve event causality identification in LLMs by addressing causal reasoning biases through structured example retrieval.


<details>
  <summary>Details</summary>
Motivation: To enhance the performance of LLMs in Event Causality Identification by mitigating biases that lead to causal hallucination.

Method: SERE employs a structural example retrieval framework based on three concepts: Conceptual Path Metric, Syntactic Metric, and Causal Pattern Filtering for improved causal reasoning.

Result: Extensive experiments demonstrate the effectiveness of SERE in improving accuracy in multiple ECI datasets.

Conclusion: SERE successfully mitigates bias and enhances the accuracy of LLMs in identifying event causality.
...
TL;DR: SAM-NER is a framework designed to improve Zero-shot Named Entity Recognition by stabilizing cross-domain transfer through an intermediate archetype space, addressing challenges related to domain and schema shifts.


<details>
  <summary>Details</summary>
Motivation: ZS-NER struggles with domain and schema shifts, resulting in semantic misalignments between entity mentions and target labels, which can cause systematic errors in predictions.

Method: SAM-NER consists of three stages: (i) Entity Discovery for high-coverage entity spans, (ii) Abstract Mediation to create a set of universal semantic archetypes, and (iii) Semantic Calibration to align predictions with target-domain types using a frozen LLM.

Result: Experiments demonstrate that SAM-NER consistently outperforms traditional ZS-NER models, particularly in cross-domain scenarios, validating its effectiveness in overcoming current limitations in the field.

Conclusion: The proposed SAM-NER framework enhances the robustness and accuracy of Zero-shot Named Entity Recognition in challenging cross-domain transfers.
...
TL;DR: Rose-SQL is a training-free framework utilizing small-scale Large Reasoning Models for improved performance on multi-turn Text-to-SQL tasks, outperforming existing models in code generation and reasoning without the need for fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of LRMs in multi-turn Text-to-SQL tasks and overcome limitations of existing methods that depend on unstable APIs or are costly to fine-tune.

Method: Rose-SQL employs in-context learning, introducing Role-State as a structural blueprint, tracking its evolution through conversational dependency, and using structural isomorphism checks to guide SQL composition.

Result: Rose-SQL surpasses in-context learning baselines at 4B model scale and outperforms fine-tuned models at larger scales (8B and 14B) in Text-to-SQL benchmarks like SParC and CoSQL.

Conclusion: The framework demonstrates a significant advancement in context-dependent parsing for SQL generation, providing a more effective solution without the need for extensive training.
...
TL;DR: This paper presents algorithms for segmenting human- and LLM-authored text, addressing the inadequacy of binary classification in detecting authorship in co-authored works.


<details>
  <summary>Details</summary>
Motivation: To ensure authenticity and societal trust in text, there is a need to distinguish human-written text from LLM-generated text, especially when both coexist in co-authored works.

Method: The authors adapt classical change point detection from time-series analysis to create algorithms that segment text, including a weighted algorithm and a generalized version for variable detection scores.

Result: The proposed methods outperform established detection baselines in accurately identifying segments of text based on authorship.

Conclusion: The segmentation approach based on change point detection is effective for distinguishing between human and LLM text contributions.
...
TL;DR: This paper presents the development of a generative large language model for the Tajik language using a newly created corpus and evaluates different fine-tuning methods.


<details>
  <summary>Details</summary>
Motivation: To address the scarcity of digital resources for the Tajik language, facilitating its representation in generative models.

Method: Creation of the Tajik Web Corpus with 319,298 documents and benchmarking 17 model configurations using various fine-tuning strategies.

Result: Mistral 7B with QLoRA (r=16) achieved the best performance, while smaller models fared differently with full fine-tuning risking catastrophic forgetting.

Conclusion: The creation of the Tajik Web Corpus significantly benefits Tajik language modeling and provides insights for model fine-tuning strategies that balance performance and resource use.
...
TL;DR: TriBench-Ko is a benchmark for assessing the risks and performance of large language models in judicial tasks, revealing significant risks in current models.


<details>
  <summary>Details</summary>
Motivation: To better evaluate large language models in legal workflows beyond traditional proxy tasks and focus on risks in practical judicial contexts.

Method: The paper introduces TriBench-Ko, which includes four core tasks relevant to judicial processes and assesses multiple types of deployment risks in LLMs.

Result: Evaluation of existing LLMs shows frequent significant risks, especially in precedent retrieval and legal information capture.

Conclusion: The findings emphasize the need for thorough inspection of LLM outputs in judicial contexts, and the authors provide their dataset for further research.
...
TL;DR: This paper presents a hands-on practicum guiding users through the NLP pipeline using an evolving corpus and open-weight models.


<details>
  <summary>Details</summary>
Motivation: To provide a systematic approach for learning and implementing modern NLP techniques, particularly for low-resource languages.

Method: The practicum includes twelve sessions that integrate theory with practical implementation, focusing on transparency in code, models, and results.

Result: The material emphasizes reproducibility and research in NLP, showcasing adaptations for Tajik and Tatar languages.

Conclusion: The work advocates for open-source practices in NLP and aims to empower both students and developers in the field.
...
TL;DR: This paper conducts a reproducibility study on retrieval methods, revealing limitations in their performance on complex queries and introducing a new benchmark, LIMIT+, for better evaluation.


<details>
  <summary>Details</summary>
Motivation: To assess the effectiveness of current retrieval paradigms in handling complex, set-compositional queries and to investigate the reliance on semantic shortcuts.

Method: A reproducibility study benchmarking major retrieval families and reasoning-targeted methods on two datasets (QUEST and LIMIT+), focusing on effectiveness and constraint satisfaction.

Result: In QUEST, neural retrievers significantly outperform BM25, while reasoning-targeted methods do not consistently excel. On LIMIT+, performance declines drastically for the best QUEST method, contrasting with improvements for classic lexical retrieval methods.

Conclusion: The study identifies a consistent performance degradation in complex query scenarios, suggesting a need for more robust retrieval methods and the release of tools for reproducibility.
...
TL;DR: TRACE is a framework aimed at creating trustworthy AI in critical domains by integrating a structured architecture and metrics for performance evaluation.


<details>
  <summary>Details</summary>
Motivation: The need for a reliable AI framework that ensures trust and safety in diverse operational contexts, particularly where decisions can have significant consequences.

Method: TRACE employs a four-layer reference architecture, distinguishing between classical ML and LLMs, incorporates a trust metrics suite, and uses the Computational Parsimony Ratio to promote efficient design.

Result: Three practical applications of TRACE in healthcare, industrial operations, and judicial support illustrate its adaptability across different governance frameworks while maintaining a consistent approach to trustworthiness.

Conclusion: TRACE establishes new principles for AI design, emphasizing trust and parsimony, especially in operationally critical contexts.
...
TL;DR: MCJudgeBench benchmark evaluates LLM judges on multi-constraint instruction following, revealing distinct consistency and correctness dimensions.


<details>
  <summary>Details</summary>
Motivation: To enhance the assessment of LLM judges in multi-constraint instruction following by evaluating them at the constraint level rather than overall response judgment.

Method: The benchmark includes candidate responses assessed against explicit constraint lists with gold labels and employs a variety of prompt variants to measure judge stability and consistency through both correctness and inconsistency metrics.

Result: Proprietary and open-source LLM judges were evaluated, revealing that high overall performance does not ensure reliable detection of all label categories nor lower inconsistency across evaluations.

Conclusion: The study highlights the necessity of constraint-level evaluations to better understand LLM judge reliability and to identify specific failure modes.
...
TL;DR: CC-OCR V2 introduces a new OCR benchmark that focuses on real-world document processing tasks, revealing performance gaps in existing Large Multimodal Models (LMMs) across various scenarios.


<details>
  <summary>Details</summary>
Motivation: To address the misalignment between existing OCR benchmarks and practical application requirements in document literacy tasks.

Method: CC-OCR V2 provides a comprehensive benchmark with 7,093 challenging samples across five OCR-centric tracks, incorporating critical real-world document processing scenarios.

Result: Experiments on 14 advanced LMMs demonstrate significant performance limitations, particularly in diverse tasks and scenarios.

Conclusion: The findings indicate a substantial gap between benchmark performance and real-world effectiveness, highlighting the need for improved models in practical applications.
...
TL;DR: The paper introduces Prompt Steering Replacement (PSR) models to improve activation steering in large language models, demonstrating their superiority over existing methods.


<details>
  <summary>Details</summary>
Motivation: To enhance the performance of activation steering methods, which typically underperform compared to prompt-based steering methods in large language models.

Method: The authors propose a framework that interprets prompt steering as activation steering, focusing on token-specific interventions, and develop PSR models that mimic prompt-based interventions through token-centric steering coefficients derived from activations.

Result: PSR models demonstrate superior performance compared to traditional activation steering methods across three benchmarks and show competitive results with prompt-based steering.

Conclusion: The research concludes that token-specific steering can effectively bridge the gap between activation and prompt-based steering performance in language models.
...
TL;DR: Atomic fact-checking significantly increases clinician trust in AI treatment recommendations compared to traditional methods.


<details>
  <summary>Details</summary>
Motivation: To determine if atomic fact-checking enhances clinician trust in AI-generated recommendations in high-stakes clinical settings.

Method: A randomized trial involving 356 clinicians assessing 7,476 trust ratings between atomic fact-checking and traditional explainability approaches.

Result: Atomic fact-checking showed a large effect size (Cohen's d = 0.94), raising trust from 26.9% to 66.5%. Traditional methods demonstrated smaller improvements (d = 0.25 to 0.50).

Conclusion: Decomposing AI recommendations into verifiable claims linked to guidelines significantly boosts clinician trust compared to conventional transparency methods.
...
TL;DR: The study investigates language models' ability to conduct conceptual analysis through iterated counterexample generation and definition refinement, revealing that while LMs generate more accepted counterexamples than humans, the iterative process often leads to verbose but less accurate definitions.


<details>
  <summary>Details</summary>
Motivation: To explore whether language models can effectively engage in philosophical methodology, particularly in refining definitions through counterexamples.

Method: The research involves language models generating counterexamples to definitions, followed by repairing those definitions, and repeating this process across multiple concepts and cycles.

Result: Language models accepted approximately twice as many counterexamples as human experts, yet there was moderate consistency in validity judgments across humans and between humans and the LM. Iteration yielded more verbose definitions without a corresponding increase in accuracy, and some concepts remained difficult to define consistently.

Conclusion: While language models can engage in philosophical reasoning, the effectiveness of the counterexample-repair loop diminishes rapidly, suggesting it could serve as a valuable benchmark for assessing LMs' capabilities in high-level philosophical reasoning.
...
TL;DR: The study addresses the robustness of AI-generated text detectors to distribution shifts by training transformer-based models and evaluating their performance across various datasets, revealing model-dependent error patterns and the importance of feature augmentation.


<details>
  <summary>Details</summary>
Motivation: The need for robust detection of AI-generated text in the face of distribution shifts across diverse domains and generators.

Method: Training transformer-based detectors using HC3 PLUS, calibrating a single decision threshold for performance evaluation across different datasets including M4 and AI-Text-Detection-Pile.

Result: Achieving up to 99.5% balanced accuracy in in-domain performance and 85.9% on M4 with robust feature augmentation, while outperforming strong baselines under a fixed-threshold protocol.

Conclusion: Feature augmentation and the DeBERTa architecture significantly enhance detector performance and provide a realistic measure of effectiveness against distribution shifts.
...
TL;DR: LaaB integrates neural and symbolic approaches for hallucination detection in Large Language Models, improving reliability through a meta-judgment process.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing hallucination detection methods that either focus on intrinsic neural patterns or macro-level symbolic reasoning, treating them in isolation.

Method: LaaB introduces a meta-judgment process that maps symbolic labels into the feature space, aligning neural features with symbolic judgments through mutual learning.

Result: Extensive experiments show LaaB's superiority over existing methods on 4 public datasets and across 4 LLMs, validating its efficacy in hallucination detection.

Conclusion: LaaB effectively bridges neural features and symbolic reasoning for a more comprehensive approach to detecting hallucinations in LLMs.
...
TL;DR: EQUITRIAGE conducts a fairness audit of LLMs used in emergency department triage, finding notable gender biases in model assessments, necessitating counterfactual auditing before clinical use.


<details>
  <summary>Details</summary>
Motivation: To investigate whether large language models reproduce or mitigate gender biases in triage decisions where disparities in acuity assessment have been documented.

Method: Evaluated five LLMs across 374,275 assessments on MIMIC-IV-ED vignettes, comparing original evaluations with gender-swapped counterfactuals under different prompting strategies.

Result: All models exceeded a 5% threshold for bias flip rates, with two models showing significant female undertriage, while others exhibited varying levels of directional bias and calibration gaps.

Conclusion: Distinct fairness properties exist among group parity, counterfactual invariance, and gender calibration; model-dependent interventions and counterfactual auditing are essential prior to clinical deployment.
...
TL;DR: The paper presents SaFE-Scale, a framework for assessing safety in clinical LLMs, highlighting that scaling does not inherently ensure safety. It introduces RadSaFE-200, a benchmark for evaluating clinical LLM responses to medical questions, with findings showing significant safety improvements with clean evidence use, while other methods did not eliminate high-risk errors or overconfidence.


<details>
  <summary>Details</summary>
Motivation: The assumption that larger clinical LLMs are inherently safer due to higher accuracy is insufficient, especially in medicine where significant errors can have serious implications.

Method: Developed SaFE-Scale framework to evaluate clinical LLM safety across various factors and introduced RadSaFE-200 benchmark with structured evidence and error labeling to assess 34 LLMs under different deployment conditions.

Result: Using clean evidence significantly improved accuracy and reduced high-risk errors, contradictions, and dangerous overconfidence compared to other retrieval strategies and context constructions.

Conclusion: Clinical LLM safety relies on various deployment factors, not merely model scaling, emphasizing the need for careful consideration of evidence quality and retrieval processes.
...
### [122] [Aspect-Aware Content-Based Recommendations for Mathematical Research Papers](https://arxiv.org/abs/2605.03861)
*Ankit Satpute,André Greiner-Petter,Noah Gießing,Olaf Teschke,Moritz Schubotz,Akiko Aizawa,Bela Gipp*

Main category: cs.IR

TL;DR: This paper presents GoldRiM and SilverRiM datasets and AchGNN, an aspect-aware recommendation system for mathematics papers, addressing limitations of existing content-based recommendation systems.


<details>
  <summary>Details</summary>
Motivation: To improve paper recommendations in mathematics where traditional methods are ineffective due to conceptual paper relationships and minimal textual or citation overlap.

Method: Introduces GoldRiM and SilverRiM datasets; employs AchGNN, a heterogeneous GNN that incorporates textual semantics, citation structure, and author lineage, conditioned on relevance aspects.

Result: AchGNN outperforms prior methods on both GoldRiM and SilverRiM datasets, showing substantial improvements across all relevant aspects.

Conclusion: The proposed system effectively enhances paper recommendations in mathematics and demonstrates transferability to other domains, with publicly available resources for reproducibility.
...
### [123] [RAG over Thinking Traces Can Improve Reasoning Tasks](https://arxiv.org/abs/2605.03344)
*Negar Arabzadeh,Wenjie Ma,Sewon Min,Matei Zaharia*

Main category: cs.IR

TL;DR: This paper investigates the effectiveness of retrieval-augmented generation (RAG) for reasoning-intensive tasks by introducing thinking traces as a new retrieval corpus, resulting in improved reasoning performance across various models while maintaining low inference costs.


<details>
  <summary>Details</summary>
Motivation: To challenge the belief that RAG has limited benefits for reasoning-intensive tasks, demonstrating the importance of corpus choice.

Method: Introduced T3, an offline method to transform thinking traces into structured, retrieval-friendly representations and utilized them in a retrieve-then-generate pipeline.

Result: Using thinking traces, the proposed method showed consistent performance improvements on benchmarks, surpassing non-RAG baselines and standard web corpus retrieval.

Conclusion: Thinking traces serve as an effective retrieval corpus for reasoning tasks, and their structured transformations lead to significant performance enhancements with minimal inference costs.
...
### [124] [Revisiting General Map Search via Generative Point-of-Interest Retrieval](https://arxiv.org/abs/2605.03397)
*Dong Chen,Shuai Zheng,Haoyang Shao,Hongsheng Wu,Muhao Xu,Yeyu Yan,Ruifang Li,Zhenfeng Zhu*

Main category: cs.IR

TL;DR: GenPOI is a Generative POI retrieval framework aimed at improving POI retrieval in location-based services by addressing underspecified user queries and context-dependency using Large Language Models.


<details>
  <summary>Details</summary>
Motivation: To enhance POI retrieval effectiveness in the face of complex, context-dependent user queries that traditional methods struggle to interpret.

Method: GenPOI utilizes a generative approach for POI retrieval, integrating various search contexts and employing Large Language Models for advanced context modeling and candidate generation.

Result: GenPOI outperforms traditional POI retrieval methods, particularly in handling challenging queries, through innovative Geo-Semantic POI Tokenization and a proximity-aware generation strategy.

Conclusion: The framework achieves superior performance on large-scale datasets, validating its efficacy in improving POI retrieval by aligning semantic and geographic contexts.
...
### [125] [Beyond Similarity Search: A Unified Data Layer for Production RAG Systems](https://arxiv.org/abs/2605.03275)
*Venkata Krishna Prasanth Budigi,Siri Chandana Sirigiri*

Main category: cs.IR

TL;DR: The paper addresses the performance gap in RAG systems caused by data staleness, tenant data leakage, and query composition explosion; it proposes a unified PostgreSQL data layer which significantly improves query latency and reliability.


<details>
  <summary>Details</summary>
Motivation: To bridge the performance gap between prototype and real-world deployment of RAG systems by identifying and addressing three key issues: data staleness, tenant data leakage, and query composition explosion.

Method: A unified data layer is proposed, leveraging PostgreSQL with native vector search (pgvector) and HNSW indexing, where controlled benchmarks were conducted on 50,000 documents.

Result: The proposed method results in 92% latency reduction for date-filtered queries, 74% for tenant-scoped queries, zero synchronization inconsistencies, complete elimination of cross-tenant data leakage, and 93% reduction in synchronization code.

Conclusion: The unified data layer presents significant improvements in reliability and efficiency for RAG systems, supporting a hybrid tier architecture for optimal performance.
...
### [126] [Domain-Adaptive Dense Retrieval for Brazilian Legal Search](https://arxiv.org/abs/2605.04005)
*Jayr Pereira,Roberto Lotufo,Luiz Bonifacio*

Main category: cs.IR

TL;DR: This paper investigates training dense retrievers for Brazilian legal retrieval, emphasizing trade-offs between specialization and robustness.


<details>
  <summary>Details</summary>
Motivation: The research addresses the heterogeneous nature of Brazilian legal retrieval, which encompasses various data types, necessitating a balance between domain-specificity and general applicability in training retrieval models.

Method: Three training setups based on Qwen3-Embedding-4B were utilized: a base model (not fine-tuned), a legal-only model, and a mixed model combining legal and SQuAD-pt data. Evaluation was performed across five datasets from the JUÁ leaderboard and the Quati dataset for generalization testing.

Result: The mixed model demonstrated a notable enhancement in average metrics across datasets (NDCG@10 improved from 0.414 to 0.447, MRR@10 from 0.586 to 0.595, MAP@10 from 0.270 to 0.308), especially excelling on the Quati dataset compared to the legal-only model.

Conclusion: While the legal-only model excels in specialized tasks, the mixed setup provides broader robustness across varying retrieval types, especially in question-based searches.
...
TL;DR: RoboULM is a methodology and tool designed to help practitioners systematically analyze uncertainties in self-adaptive robots during the design stage using large language models.


<details>
  <summary>Details</summary>
Motivation: To address challenges in identifying and analyzing uncertainties in self-adaptive robots operating in dynamic environments, where these uncertainties can lead to safety issues and operational failures.

Method: A human-in-the-loop methodology that employs large language models for systematic exploration of uncertainties, complemented by an uncertainty taxonomy for categorization.

Result: RoboULM was evaluated with 16 practitioners from four industries, showing it was useful and easy to understand, with particularly positive feedback on structured prompting and iterative refinement.

Conclusion: RoboULM demonstrates potential as an effective solution for systematic uncertainty analysis in the design of complex robots.
...
TL;DR: Refining Compositional Diffusion (RCD) improves long-horizon trajectory generation by addressing mode-averaging in compositional diffusion planning, resulting in higher quality plans.


<details>
  <summary>Details</summary>
Motivation: Existing compositional methods for generating long-horizon trajectories struggle with mode-averaging when faced with multimodal local plan distributions, leading to poor quality trajectories.

Method: The proposed RCD uses the self-reconstruction error from a pretrained diffusion model to guide compositional sampling towards high-density plans, while also incorporating an overlap consistency term to ensure coherence at segment boundaries.

Result: RCD focuses sampling on high-density plans, effectively reducing mode-averaging and producing more coherent trajectories compared to traditional methods.

Conclusion: Experiments demonstrate that RCD outperforms existing methods on various long-horizon tasks, providing a significant advancement in compositional diffusion planning.
...
TL;DR: This paper benchmarks 25 open-source language models for educational robots on edge hardware, assessing their efficiency, knowledge, and teaching effectiveness. Results show significant trade-offs in performance, with a proposed architecture for maintaining balance in resource-limited conditions.


<details>
  <summary>Details</summary>
Motivation: To address the lack of systematic benchmarking for language models suitable for edge computing in educational robotic applications, ensuring responsive and privacy-preserving interactions.

Method: Benchmarking 25 open-source language models on a Raspberry Pi 4 across inference efficiency, general knowledge, and teaching effectiveness, validated by human raters.

Result: Significant trade-offs identified; throughput and energy efficiency vary greatly, MMLU accuracy differs widely, and teaching effectiveness does not directly correlate with performance metrics. Granite4 Tiny Hybrid (7B) emerges as the best performer.

Conclusion: A proposed three-tier local inference architecture optimally balances responsiveness and accuracy for deployment in resource-constrained educational robotics.
...
TL;DR: ICODE-MPPI improves MPPI control by using learning-based techniques to enhance dynamic fidelity, resulting in reduced tracking errors and smoother control commands.


<details>
  <summary>Details</summary>
Motivation: The need for improved performance in MPPI control due to limitations caused by nominal dynamics in nonlinear autonomous systems.

Method: Introduce ICODE-MPPI framework that employs Input Concomitant Neural Ordinary Differential Equations to compensate for unmodeled dynamics.

Result: Achieved up to a 69% reduction in cross-tracking error with significant suppression of control chattering in comparison to standard MPPI.

Conclusion: ICODE-MPPI provides a more robust performance for nonlinear autonomous systems under persistent disturbances.
...
TL;DR: RLDX-1 is an advanced robotic policy for dexterous manipulation, incorporating a Multi-Stream Action Transformer to enhance complex task performance beyond existing VLAs. It achieves superior results in real-world applications, particularly in humanoid tasks.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of current Vision-Language-Action models in handling complex real-world tasks that require enhanced functional abilities.

Method: RLDX-1 combines the Multi-Stream Action Transformer architecture with specialized training data, human-like manipulation learning procedures, and real-time inference optimizations.

Result: RLDX-1 outperforms state-of-the-art VLAs like π_{0.5} and GR00T N1.6 in both simulations and real-world scenarios, achieving 86.8% success in challenging humanoid tasks compared to about 40% by its competitors.

Conclusion: RLDX-1 demonstrates significant progress toward effective robotic manipulation in dynamic environments, positioning it as a reliable model for complex dexterous operations.
...
TL;DR: The paper introduces Neural Control, a boundary-control framework that efficiently computes proxy gradients for equilibrium-based AI tasks, improving performance in multi-stable environments.


<details>
  <summary>Details</summary>
Motivation: Many physical AI tasks operate under implicit equilibrium, where the interaction of actuated and free degrees of freedom complicates learning and control due to multi-stability and high computational demands.

Method: Neural Control employs an adjoint formulation to derive memory-efficient gradients without backpropagating through iterative solvers, integrated into a receding-horizon MPC framework.

Result: The proposed method shows enhanced performance in managing deformable linear objects (DLOs) compared to traditional gradient-free methods such as SPSA and CEM.

Conclusion: Neural Control effectively addresses the challenges of multi-stability and computation inefficiency in equilibrium-based tasks, offering improved robustness and performance in both simulations and practical robot applications.
...
### [133] [Height Control and Optimal Torque Planning for Jumping With Wheeled-Bipedal Robots](https://arxiv.org/abs/2605.03302)
*Yulun Zhuang,Yuan Xu,Binxin Huang,Mandan Chao,Guowei Shi,Xin Yang,Kuangen Zhang,Chenglong Fu*

Main category: cs.RO

TL;DR: This study presents a method for controlling the jumping height of wheeled-bipedal robots by optimizing torque and energy consumption, overcoming challenges of existing models.


<details>
  <summary>Details</summary>
Motivation: To improve the jumping control of wheeled-bipedal robots and reduce excessive height that leads to increased energy consumption and motor strain.

Method: A new dynamical model (W-JBD) is introduced for better height control, combined with a Bayesian optimization method (BOTP) for efficient torque planning without needing an accurate dynamic model.

Result: The BOTP method achieved an 82.3% reduction in height error and a 26.9% reduction in energy costs with a continuous torque curve, validated through simulations.

Conclusion: The combination of the W-JBD model and BOTP enables effective height control for real robots with fewer experimental trials.
...
### [134] [Learning Reactive Dexterous Grasping via Hierarchical Task-Space RL Planning and Joint-Space QP Control](https://arxiv.org/abs/2605.03363)
*Ho Jae Lee,Yonghyeon Lee,Alexander Alexiev,Tzu-Yuan Lin,Se Hwan Jeon,Sangbae Kim*

Main category: cs.RO

TL;DR: This paper presents a hybrid control framework for dexterous grasping that separates high-level planning from low-level execution, utilizing a multi-agent reinforcement learning approach with a focus on safety and adaptability.


<details>
  <summary>Details</summary>
Motivation: To improve reactive dexterous grasping capabilities by decoupling spatial intent from joint execution while ensuring safety and efficiency in control.

Method: A multi-agent reinforcement learning architecture with specialized arm and hand agents generates task-space velocity commands, which are then translated into joint velocities by a GPU-accelerated quadratic programming controller, enforcing kinematic limits and collision avoidance.

Result: The framework enables zero-shot steerability, allowing dynamic adjustments and robust performance in real-world tasks, validated through simulations and hardware experiments.

Conclusion: The proposed control system demonstrates effective dexterous grasping, adaptable to diverse unseen objects, and resilient to unexpected disturbances in unstructured environments.
...
### [135] [BifrostUMI: Bridging Robot-Free Demonstrations and Humanoid Whole-Body Manipulation](https://arxiv.org/abs/2605.03452)
*Chenhao Yu,Hongwu Wang,Youhao Hu,Jiachen Zhang,Yuanyuan Li,Shaqi Luo*

Main category: cs.RO

TL;DR: The paper presents BifrostUMI, a novel data collection framework using VR devices to capture human demonstrations for training humanoid robots.


<details>
  <summary>Details</summary>
Motivation: The study addresses the limitations of current data acquisition methods in humanoid robot training, focusing on improving efficiency and accessibility.

Method: BifrostUMI employs lightweight VR technology to capture human movements as sparse keypoint trajectories, along with visual data, to train a policy network for predicting trajectories based on visual input.

Result: The framework demonstrates successful mapping of human demonstration behaviors to robot actions through a keypoint retargeting pipeline in two experimental scenarios.

Conclusion: BifrostUMI effectively enables the transfer of human-like behaviors to humanoid robots, showcasing its potential for diverse applications.
...
TL;DR: The paper addresses the challenge of learning robotic manipulation from human videos by proposing a generative framework that disentangles task and embodiment representations, allowing robots to execute tasks demonstrated by humans without requiring paired data.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the need to overcome the data bottleneck in robotics while tackling the distribution shift between human and robot kinematics.

Method: A generative framework is introduced that factorizes a demonstration video into two independent latent spaces using a dual contrastive objective, which enhances representation stability and independence.

Result: The method enables the synthesis of coherent robot execution videos from human demonstrations, demonstrating temporally consistent and morphologically accurate robot behaviors.

Conclusion: The proposed solution effectively leverages human video content at scale for enhancing robot learning capability.
...
TL;DR: The paper presents an innovative architecture for consumer robotics that enhances control, safety, and usability on multicore platforms, addressing expertise asymmetry between end-users and developers.


<details>
  <summary>Details</summary>
Motivation: To tackle the expertise asymmetry issue in modifying robotic behavior within safety-critical applications on shared multicore platforms.

Method: The architecture consists of three components: a Safe IO Cell for hardware-level override, a Parameter Synchronization Service to manage complexity, and a Safety Communication Layer for IEC 61508-aligned verification.

Result: The empirical evaluation on ARM Cortex-A55 shows significant improvement in timing performance: 84.5% reduction in cycle-period jitter and a 90% reduction in tail timing error (p99 jitter from 69.0 μs to 7.8 μs), eliminating excursions over 50 μs.

Conclusion: The proposed architecture effectively consolidates safety and usability, ensuring reliable operation of consumer robotics in safety-critical environments.
...
TL;DR: A new method for planar task and motion planning that integrates planning and control design using hybrid modeling is introduced, effectively managing overlapping tasks and preventing deadlocks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance planar task and motion planning in robotic systems, addressing local constraint satisfaction and complex workspace challenges.

Method: The method involves a discrete variable for local feasibility analysis and control barrier functions on a transformed workspace, improving planning and control integration.

Result: Simulations demonstrate the method's effectiveness in managing multiple overlapping spatio-temporal tasks, even with input saturation issues.

Conclusion: The proposed approach successfully addresses planning complexity and deadlock issues, providing a robust solution for robotic task and motion planning.
...
TL;DR: This paper presents a new method for controlling UAVs manipulating cable-suspended loads by incorporating geometric constraints into the dynamic modeling, improving performance and trajectory tracking.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the limitations of existing methods that rely on direct load measurements and Lagrangian methods, which do not provide a straightforward dynamic model for cable-suspended loads.

Method: The Udwadia-Kalaba method is employed to explicitly include the cable's geometric constraints, allowing for the derivation of the tension force and its integration into a Nonlinear Model Predictive Control (NMPC) model. A sensorless load state estimation technique is also proposed utilizing these geometric constraints.

Result: Experiments with real robots showed that the integration of load dynamics into the optimization process significantly reduced trajectory-tracking errors compared to traditional methods based on incomplete models.

Conclusion: The proposed method demonstrates improved control and estimation of UAVs in aerial manipulation tasks, validating the effectiveness of incorporating geometric constraints into dynamic modeling.
...
TL;DR: FUS3DMaps introduces an online dual-layer semantic mapping technique that enhances open-vocabulary semantic mapping for robots by integrating dense and instance-level layers in a shared voxel map, improving scalability and accuracy.


<details>
  <summary>Details</summary>
Motivation: To enhance robots' capabilities in spatially grounding unseen concepts in a scalable manner without requiring predefined class sets, addressing limitations in current training-free semantic mapping methods.

Method: The proposed FUS3DMaps method uses an online approach with dual-layer semantic mapping, maintaining both dense and instance-level embeddings within a shared voxel map and applying voxel-level semantic fusion.

Result: FUS3DMaps improves the quality of both layers and achieves accurate open-vocabulary semantic mapping, even for complex multi-story building environments, outperforming existing methods in scalability and precision.

Conclusion: The dual-layer approach and cross-layer fusion enhance semantic mapping capabilities, making FUS3DMaps effective for real-time applications in robotic perception.
...
### [141] [Robust Visual SLAM for UAV Navigation in GPS-Denied and Degraded Environments: A Multi-Paradigm Evaluation and Deployment Study](https://arxiv.org/abs/2605.03678)
*Prasoon Kumar,Akshay Deepak,Sandeep Kumar*

Main category: cs.RO

TL;DR: The paper evaluates five V-SLAM systems in challenging conditions for UAVs, revealing MASt3R's robustness and DPVO's efficiency as top choices.


<details>
  <summary>Details</summary>
Motivation: To improve reliable localization for autonomous UAV operations in GPS-denied and visually degraded environments.

Method: Systematic comparative evaluation of ORB-SLAM3, DPVO, DROID-SLAM, DUSt3R, and MASt3R across various degradation scenarios in controlled experiments using multiple benchmarks and a custom dataset.

Result: ORB-SLAM3 underperformed in severe conditions; MASt3R had the lowest degraded ATE, while DPVO provided a good balance of efficiency and performance.

Conclusion: MASt3R and DPVO are recommended for UAV SLAM tasks, especially under memory constraints, highlighting the need for system-specific SLAM selection guidelines.
...
### [142] [RoboAlign-R1: Distilled Multimodal Reward Alignment for Robot Video World Models](https://arxiv.org/abs/2605.03821)
...
TL;DR: RoboAlign-R1 enhances robot video world models by aligning training with task-relevant capabilities and stabilizing long-horizon predictions.


<details>
  <summary>Details</summary>
Motivation: To better align robot video models with decision-making abilities such as instruction following and physical plausibility, which are not captured by traditional low-level training objectives.

Method: Introduced RoboAlign-R1 framework which incorporates reward-aligned post-training and Sliding Window Re-encoding (SWR) for long-horizon inference. Developed RobotWorldBench for video-instruction evaluation using a multimodal teacher judge.

Result: RoboAlign-R1 achieved a 10.1% improvement in six-dimension scores, with significant gains in Manipulation Accuracy and Instruction Following. SWR improved long-horizon prediction quality with marginal latency increase.

Conclusion: The combination of reward-aligned post-training and SWR successfully enhances task consistency, physical realism, and prediction quality in robot video world models.
...
### [143] [SigLoMa: Learning Open-World Quadrupedal Loco-Manipulation from Ego-Centric Vision](https://arxiv.org/abs/2605.03846)
*Shiyi Chen,Haiyi Liu,Mingye Yang,Jiaqi Zhang,Debing Zhang*

Main category: cs.RO

TL;DR: SigLoMa is a vision-based system for quadrupedal loco-manipulation that aims to overcome sample inefficiencies and sim-to-real gaps associated with traditional reinforcement learning methods by using a lightweight geometric representation and an ego-centric Kalman Filter.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenges of traditional reinforcement learning frameworks in quadrupedal locomotion, particularly issues like sample inefficiency, sim-to-real gaps, and latency in visual tracking, which hinder precise control.

Method: SigLoMa introduces Sigma Points for scalable exteroception and an ego-centric Kalman Filter for high-rate state estimation. An Active Sampling Curriculum is utilized to improve sample efficiency, and temporal encoding with simulated random-walk drift is used to handle visual blind spots.

Result: SigLoMa enables a quadrupedal robot to execute dynamic loco-manipulation tasks effectively, with performance levels comparable to expert human teleoperation, while operating on a 5Hz open-vocabulary detector.

Conclusion: The framework successfully integrates onboard visual processing and fast control, demonstrating that high-quality loco-manipulation can be achieved without reliance on external systems.
...
### [144] [Evaluating Generative Models as Interactive Emergent Representations of Human-Like Collaborative Behavior](https://arxiv.org/abs/2605.03855)
*Shinas Shaji,Teena Chakkalayil Hassan,Sebastian Houben,Alex Mitrevski*

Main category: cs.RO

TL;DR: This paper investigates emergent collaborative behaviors in embodied foundation model agents during human-AI interactions in a 2D game environment, emphasizing coordination and mental model representation.


<details>
  <summary>Details</summary>
Motivation: To explore the effectiveness of human-AI collaboration by understanding if AI agents can exhibit and recognize emergent collaborative behaviors indicative of mental models.

Method: A 2D collaborative game environment was created for testing, where large language model agents and humans perform color-matching tasks. Five collaborative behaviors were identified and measured using an automated behavior detection system.

Result: The automated detection system found that foundation model agents demonstrated emergent collaborative behaviors without specific training, with patterns varying among different models and collaboration stages.

Conclusion: The study provides a framework for assessing human-AI collaboration, demonstrating that embodied LLM agents can engage in effective collaborative behaviors, thus enhancing the understanding of their potential in coordination tasks.
...
### [145] [Task-Aware Scanning Parameter Configuration for Robotic Inspection Using Vision Language Embeddings and Hyperdimensional Computing](https://arxiv.org/abs/2605.03909)
*Zhiling Chen,David Gorsich,Matthew P. Castanier,Yang Zhang,Jiong Tang,Farhad Imani*

Main category: cs.RO

TL;DR: A study introduces a method for optimizing robotic laser profiler settings based on task instructions and scene observations, achieving high accuracy and rapid inference.


<details>
  <summary>Details</summary>
Motivation: The need for improved measurement fidelity in robotic laser profiling while minimizing trial-and-error tuning of sensory parameters.

Method: The study develops a multimodal dataset (Instruct-Obs2Param) and a hyperdimensional computing framework (ScanHD) to associate sensor parameters with task instructions and visual observations.

Result: ScanHD achieves a 92.7% average exact accuracy and 98.1% average Win@1 accuracy on parameter selection, demonstrating superior performance over traditional methods.

Conclusion: This work allows for autonomous adjustments of sensor configurations based on task intent and context, enhancing efficiency and effectiveness in robotic inspections.
...
### [146] [ARIS: Autonomous Research via Adversarial Multi-Agent Collaboration](https://arxiv.org/abs/2605.03042)
*Ruofeng Yang,Yongcan Li,Shuai Li*

Main category: cs.SE

TL;DR: ARIS is an open-source research harness designed for autonomous research, featuring a robust architecture to ensure effective workflows and evidence-supported claims.


<details>
  <summary>Details</summary>
Motivation: To facilitate long-horizon research in machine learning while addressing issues of unsupported successes in agent systems that rely on LLMs.

Method: ARIS utilizes a multi-layer architecture with execution, orchestration, and assurance layers to manage research workflows, integrating adversarial review and continuous improvement processes.

Result: The architecture includes extensive reusable skills, coordinated workflows, and a detailed assurance process for verifying claims and supporting evidence.

Conclusion: ARIS aims to enhance the reliability and integrity of autonomous research by fostering collaboration and rigorous review processes, ultimately improving research outcomes.
...
### [147] [ARISE: A Repository-level Graph Representation and Toolset for Agentic Fault Localization and Program Repair](https://arxiv.org/abs/2605.03117)
*Shahd Seddik,Fatemeh Fard*

Main category: cs.SE

TL;DR: ARISE enhances fault localization and automated program repair by introducing a multi-granularity program graph, improving recall rates and repair success rates significantly over existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing graph-based systems lack the ability to model variable value flow, which is essential for accurate fault localization and program repair tasks.

Method: ARISE employs an LLM-based agent augmented with a detailed program graph that represents statements and relationships in terms of data dependencies, using a three-tier API for data-flow slicing.

Result: ARISE improves Function Recall@1 by 17.0 points and Line Recall@1 by 15.0 points over the baseline, and achieves a Pass@1 repair success rate of 22.0%, a 4.7 percentage-point improvement.

Conclusion: The data-flow graph is the key driver of improvements in localization and repair success, and the framework is intended as a versatile toolset for future research in automated program repair.
...
### [148] [Kerncap: Automated Kernel Extraction and Isolation for AMD GPUs](https://arxiv.org/abs/2605.03208)
*Cole Ramos,Keith Lowery*

Main category: cs.SE

TL;DR: Kerncap is an automated tool for isolating GPU kernels to enhance iterative tuning efficiency, enabling rapid edits without full application rebuilds, achieving significant speedup in the process.


<details>
  <summary>Details</summary>
Motivation: To facilitate faster GPU kernel tuning by automating the kernel isolation process, reducing reliance on slow manual methods.

Method: Kerncap intercepts dispatches at the HSA runtime for HIP and Triton, creating a virtual-address-faithful snapshot of device memory and generating self-contained projects for kernel reproduction without altering existing build systems.

Result: Kerncap successfully extracts and validates kernels from snapshots of various sizes across multiple AMD GPU architectures, demonstrating a 13.6x speedup in the edit-recompile-validate loop for kernel tuning.

Conclusion: Kerncap significantly streamlines the kernel isolation process, enabling efficient autotuning and performance evaluation of isolated GPU kernels.
...
### [149] [TeamUp: Semantic Project Matching and Team Formation for Learning at Scale](https://arxiv.org/abs/2605.03237)
*Dhruv Gulwani,Basem Suleiman,Aditya Joshi,Sonit Singh*

Main category: cs.SE

TL;DR: TeamUp is a system for efficiently allocating students to projects, enhancing learning outcomes and equity by forming cognitively diverse teams.


<details>
  <summary>Details</summary>
Motivation: The need for effective methods to allocate students to challenging projects and form diverse teams to improve engagement and equity in project-based learning.

Method: TeamUp utilizes pretrained language models for semantic embeddings and a hybrid ranking algorithm to match students to projects, ensuring cognitive diversity in team formation.

Result: TeamUp demonstrated superior match quality, improved difficulty alignment, increased team diversity, and quick recommendation times in an evaluation with 250 student profiles and 60 project descriptions.

Conclusion: The implementation of TeamUp can significantly enhance educational equity and learning outcomes in large-scale project-based courses.
...
### [150] [Exploring Sustainability in Scientific Software through Code Quality & Test Coverage Metrics](https://arxiv.org/abs/2605.03243)
*Sheikh Md. Mushfiqur Rahman,Gregory R. Watson,Nasir U. Eisty*

Main category: cs.SE

TL;DR: The paper examines the long-term sustainability of scientific open-source software by analyzing code quality and test metrics, revealing differences between sustainable and unsustainable projects.


<details>
  <summary>Details</summary>
Motivation: The study addresses the significant concern of the long-term sustainability of scientific open-source software, which is crucial for research and engineering.

Method: Analyzed CASS Software Portfolio projects by classifying them based on sustainability and comparing their code structure, test coverage, and correlations between code quality and testing.

Result: Sustainable projects exhibit higher and more consistent test coverage and better code-test correlations; unsustainable projects show low test coverage and issues with complexity and coupling.

Conclusion: The paper proposes a data-driven method for evaluating the sustainability of scientific software, supporting quality assurance efforts and sustainability monitoring.
...
### [151] [Operationalizing Software Engineering Theories for Practical Validation](https://arxiv.org/abs/2605.03257)
*Isaque Alves,Fabio Kon,Jessica Diaz,Carla Rocha*

Main category: cs.SE

TL;DR: This paper presents a systematic approach to the operationalization phase in Software Engineering, enhancing the link between abstract theories and empirical validation.


<details>
  <summary>Details</summary>
Motivation: To address the socio-technical complexity in Software Engineering by improving the operationalization of theoretical frameworks.

Method: The paper proposes a systematic procedure that builds on Sjøberg et al.'s operationalization framework and Dubin's approach, defining variables, selecting indicators, and formulating non-causal hypotheses.

Result: The proposed procedure provides a replicable and evidence-based methodological guideline that maintains a clear chain of evidence for practical validation.

Conclusion: This guideline empowers researchers to connect theoretical advancements with empirical evidence, ensuring the theories developed are useful for practitioners.
...
### [152] [POSTCONDBENCH: Benchmarking Correctness and Completeness in Formal Postcondition Inference](https://arxiv.org/abs/2605.03356)
*Gehao Zhang,Juan Zhai*

Main category: cs.SE

TL;DR: POSTCONDBENCH is a multilingual benchmark for evaluating method-level postcondition generation from real-world software, revealing gaps between correctness and completeness in generation by SOTA LLMs.


<details>
  <summary>Details</summary>
Motivation: There is a need for precise formal postconditions in programming, but generating them is challenging, motivating the use of LLMs for automation.

Method: POSTCONDBENCH consists of 420 tasks from real open-source projects with expert-constructed ground-truth postconditions, evaluated through a practical execution environment and a novel completeness metric based on defect discrimination.

Result: Evaluation of five state-of-the-art LLMs using POSTCONDBENCH shows a significant gap between correctness and completeness of the generated postconditions, influenced by dependencies and method complexity.

Conclusion: The findings underscore the limitations of LLMs in generating comprehensive postconditions, highlighting the need for improvement in handling repository-level dependencies and method complexity.
...
### [153] [Multi-Agent Systems for Root Cause Analysis in Microservices](https://arxiv.org/abs/2605.03505)
*Alexander Naakka,Yuqing Wang,Mika V Mäntylä*

Main category: cs.SE

TL;DR: This paper presents LATS-RCA, a multi-agent framework that utilizes large language models to enhance root cause analysis in microservice-based systems, improving diagnostic accuracy across different environments while addressing underlying complexities.


<details>
  <summary>Details</summary>
Motivation: To improve root cause analysis in microservice-based systems by overcoming limitations of previous linear reasoning approaches.

Method: LATS-RCA employs a reflection-guided tree-structured search utilizing multiple LLM-driven agents that analyze execution logs and performance metrics to evaluate root causes.

Result: LATS-RCA demonstrates high diagnostic accuracy in a controlled microservice environment (Light-OAuth2) and reveals challenges in a more complex production environment, leading to lower accuracy and higher computational costs.

Conclusion: LATS-RCA is effective for RCA in microservice systems, especially in simpler setups, but faces challenges in production environments due to increased complexity and varying practices.
...
### [154] [ProgramBench: Can Language Models Rebuild Programs From Scratch?](https://arxiv.org/abs/2605.03546)
...
TL;DR: ProgramBench introduces a new benchmark for evaluating software engineering agents' ability to develop complete software projects, focusing on high-level architectural decisions rather than isolated tasks.


<details>
  <summary>Details</summary>
Motivation: There is a need for a benchmark that assesses software engineering agents on their holistic software development capabilities, as current benchmarks only evaluate limited tasks.

Method: ProgramBench requires agents to develop a codebase from a given program and documentation, using end-to-end behavioral tests generated through agent-driven fuzzing for evaluation.

Result: Evaluation of 9 language models shows none fully achieved any task, with the most promising model passing 95% of tests on merely 3% of tasks, indicating significant limitations in current models.

Conclusion: Models tend to produce monolithic, single-file implementations, often deviating from the typical structure of human-written code, highlighting gaps in their software engineering capabilities.
...
### [155] [Geographic Variation in Stack Overflow Code Quality: Evidence from a Cross-Regional Study of Coding Practices](https://arxiv.org/abs/2605.03670)
*Elijah Zolduoarrati,Sherlock A. Licorish,Nigel Stanger*

Main category: cs.SE

TL;DR: The study evaluates code quality of Stack Overflow snippets across different languages and regions, finding readability as the most common issue and highlighting socio-technical influences on code quality.


<details>
  <summary>Details</summary>
Motivation: To understand the quality of code snippets reused by developers from Stack Overflow, particularly across different programming languages and geographic contexts.

Method: The study uses linting and static analysis tools to assess code snippets on dimensions of reliability, readability, performance, and security. It involves evaluating violations by region and conducting qualitative content analysis on selected states.

Result: Readability violations are the most common, with reliability, performance, and security violations also identified. Regional analysis shows that tech hubs produce more parsable snippets, but not necessarily fewer violations.

Conclusion: The findings underscore the socio-technical nature of code quality, indicating that developers should be cautious when reusing snippets, especially from diverse geographic backgrounds.
...
### [156] [Deep Graph-Language Fusion for Structure-Aware Code Generation](https://arxiv.org/abs/2605.03689)
*Mert Tiftikci,Amir Molzam Sharifloo,Mira Mezini*

Main category: cs.SE

TL;DR: CGFuse is a novel framework that integrates graph-derived representations into pre-trained language models, enhancing their ability to capture code structure and improve code generation performance.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current pre-trained language models in capturing the structured attributes of code, specifically control flow and data dependencies, which traditional sequential processing does not adequately handle.

Method: CGFuse introduces a method for token-level integration of graph features into language models by infusing graph representations directly into their intermediate layers, using a combination of a graph neural network (GNN) and a language model.

Result: Evaluation of CGFuse across multiple large language models (LLMs) shows improvements in code generation performance, with up to 10-16% increases in BLEU scores and 6-11% increases in CodeBLEU scores.

Conclusion: The integration of deep graph features within pre-trained language models has the potential to significantly enhance their performance in software development tasks, paving the way for more robust AI systems.
...
### [157] [AI Advocate: Educational Path to Transform Squads to the Future](https://arxiv.org/abs/2605.03800)
*Carla Soares,Gabriel Moreira,Ana Paula Camargo,Fabio Henrique Scacabarozi,Nicole Davila,Marselle Silva*

Main category: cs.SE

TL;DR: This study examines the transformation of software development teams into hybrid structures that integrate human and AI collaboration, focusing on the education of AI Advocates in a Brazilian tech company.


<details>
  <summary>Details</summary>
Motivation: To enhance productivity through effective human-AI collaboration by upskilling professionals.

Method: An experience report detailing the education and enablement process of AI Advocates.

Result: Identified key lessons learned and challenges faced in transforming traditional teams.

Conclusion: The upskilling process significantly contributes to cultural and technical transformation in software development.
...
### [158] [Beyond Rules: LLM-Powered Linting for Quantum Programs](https://arxiv.org/abs/2605.03943)
*Pietro Cassieri,Giuseppe Scanniello,Seung Yeob Shin,Fabrizio Pastore,Domenico Bianculli*

Main category: cs.SE

TL;DR: This paper presents two novel LLM-based approaches for detecting quantum programming issues, outperforming traditional rule-based methods.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of traditional static analysis tools in efficiently detecting quantum programming problems due to rapid API changes and complexity.

Method: Introduction of LintQ-LLM+CoT and LintQ-LLM+RAG, leveraging Large Language Models with Chain-of-Thought prompting and a Retrieval-Augmented Generation system for grounded reasoning.

Result: The LLM-based approaches surpassed the rule-based tool LintQ in precision and recall, achieving F1-scores of 0.70 and 0.68 compared to LintQ's 0.41, with RAG variant reducing false positives.

Conclusion: LLMs represent a promising solution for enhancing the detection capabilities in quantum software engineering, providing scalability and adaptability.
...
### [159] [KVerus: Scalable and Resilient Formal Verification Proof Generation for Rust Code](https://arxiv.org/abs/2605.03822)
*Yuwei Liu,Xinyi Wan,Yanhao Wang,Minghua Wang,Lin Huang,Tao Wei*

Main category: cs.SE

TL;DR: KVerus is a self-adaptive verification system for Rust that overcomes limitations of large language models by efficiently handling complex dependencies in evolving software for formal verification.


<details>
  <summary>Details</summary>
Motivation: To address the challenges faced by large-scale, evolving systems in formal verification due to the Semantic-Structural Gap between LLMs and formal verification requirements.

Method: KVerus employs a dynamic knowledge base integrating code metadata, lemma semantics, and toolchain specifics, utilizing dependency-aware analysis, semantic indexing, and error-driven refinement for proof synthesis and repair.

Result: KVerus verified 80.2% of tasks in single-file benchmarks compared to 56.9% for AutoVerus, and 51.0% success on repository-level benchmarks with complex dependencies, superior to 4.5% of baseline.

Conclusion: KVerus advances formal verification as a scalable practice for security-critical software by producing accepted proofs for previously unverified functions.
...
### [160] [Randomized and Diverse Input State Generation for Quantum Program Testing](https://arxiv.org/abs/2605.03957)
*Maryse Ernzer,Seung Yeob Shin,Fabrizio Pastore,Domenico Bianculli*

Main category: cs.SE

TL;DR: The paper presents a framework to evaluate test circuit generators for quantum software, focusing on complete state-space exploration through diversity scores and a new test circuit generator called Brick-Circuit (BC).


<details>
  <summary>Details</summary>
Motivation: The need for reliable quantum software due to the rapid development of quantum technologies and the inadequacy of current testing methods that overlook complete state-space exploration.

Method: The authors propose a framework with diversity scores to assess the extent of state space coverage and develop a Brick-Circuit (BC) generator for test input states.

Result: The BC generator outperforms existing generators in producing uniformly distributed random test input states, demonstrating higher expressibility and entanglement performance at shallower depths.

Conclusion: The proposed BC generator significantly enhances the reliability of quantum software testing by achieving better exploration and utilization of the quantum state space compared to traditional methods.
...
### [161] [Mitigating False Positives in Static Memory Safety Analysis of Rust Programs via Reinforcement Learning](https://arxiv.org/abs/2605.04000)
*P Akilesh,Leuson Da Silva,Foutse Khomh,Sridhar Chimalakonda*

Main category: cs.SE

TL;DR: This paper introduces a reinforcement learning approach to reduce false positives in static memory safety analysis for Rust programs, significantly improving accuracy and precision.


<details>
  <summary>Details</summary>
Motivation: To address high false positive rates in existing static analysis tools for Rust, which undermine developer trust and obscure real vulnerabilities.

Method: A reinforcement learning agent learns to classify and suppress false warnings by leveraging contextual features from Rust's Mid-level Intermediate Representation (MIR) and interacts with static analysis outputs. It also utilizes dynamic validation through cargo-fuzz for targeted fuzz testing to improve decision quality.

Result: The approach achieves 65.2% accuracy and an F1 score of 0.659, outperforming existing LLM-based methods by 17.1%. The method successfully detects 74.6% of true bugs while enhancing precision from 25.6% to 59.0%. Incorporating dynamic fuzzing further increases accuracy by 10.7 percentage points and F1 score by 8.6 percentage points over the RL-only variant.

Conclusion: Combining reinforcement learning with hybrid static-dynamic analysis effectively reduces false positives and enhances the usability of memory safety verification tools for Rust.
...
### [162] [Structural Limits of OHLCV-Based Intraday Signals in MNQ Futures: A Systematic Falsification Study](https://arxiv.org/abs/2605.04004)
*Mathias Mesfin*

Main category: q-fin.TR

TL;DR: This study analyzes intraday momentum signals from OHLCV data to determine their effectiveness in trading Micro E-mini Nasdaq 100 futures, revealing no signals met all institutional criteria due to transaction cost constraints.


<details>
  <summary>Details</summary>
Motivation: To assess the viability of intraday momentum trading strategies using OHLCV data while considering realistic execution factors.

Method: Evaluated 14 signal families over 947 trading days of five-minute data, applying strict validation criteria including out-of-sample testing and statistical significance.

Result: No signal met all stringent criteria, with gross trading edge limited to 0.07-1.50 points per trade; one gap-continuation signal showed T = 3.23 but did not meet minimum trade requirements.

Conclusion: The research establishes a reproducible falsification framework and documents a null result, emphasizing the limitations of OHLCV-based intraday trading strategies.
...
### [163] [eOptShrinkQ: Near-Lossless KV Cache Compression Through Optimal Spectral Denoising and Quantization](https://arxiv.org/abs/2605.02905)
*Pei-Chun Su*

Main category: cs.LG

TL;DR: This paper presents eOptShrinkQ, a new two-stage compression method for transformer KV caches, leveraging low-rank decomposition and optimal singular value shrinkage for improved performance.


<details>
  <summary>Details</summary>
Motivation: The need for efficient compression methods in transformer models to reduce storage and enhance performance while maintaining quality.

Method: eOptShrinkQ combines optimal singular value shrinkage for extracting shared context with TurboQuant for quantizing the residual, supported by random matrix theory for theoretical guarantees.

Result: eOptShrinkQ shows significant improvements over TurboQuant in compression efficiency and reconstruction quality across multiple experimental settings.

Conclusion: The results suggest that eOptShrinkQ effectively compresses KV caches in transformers, making it a promising approach for retrieval-intensive tasks and demonstrating the benefits of spectral denoising.
...
### [164] [An End-to-End Framework for Building Large Language Models for Software Operations](https://arxiv.org/abs/2605.02906)
*Jingkai He,Pengfei Chen,Chenghui Wu,Shuang Liang,Ye Li,Gou Tan,Xiadao Wen,Chuanfu Zhang*

Main category: cs.LG

TL;DR: OpsLLM is a domain-specific LLM for software operations, enhancing QA and RCA through a curated data process and a reward model.


<details>
  <summary>Details</summary>
Motivation: Existing LLMs struggle with low-quality data and fragmented knowledge in software operations.

Method: A Human-in-the-Loop mechanism is used for data curation, followed by supervised fine-tuning and introducing a domain process reward model during reinforcement learning.

Result: OpsLLM outperforms existing models in QA (0.2%-5.7% improvement) and RCA (2.7%-70.3% improvement), showing strong transferability.

Conclusion: Three versions of OpsLLM with varying parameters will be open-sourced along with a fine-tuning dataset.
...
### [165] [On the Invariants of Softmax Attention](https://arxiv.org/abs/2605.02907)
*Wonsuk Lee*

Main category: cs.LG

TL;DR: This paper explores the underlying structure of softmax attention through the concept of the energy field, identifying invariant properties across models and inputs.


<details>
  <summary>Details</summary>
Motivation: To uncover the unexplored structural aspects of softmax attention and its implications on model behavior.

Method: The authors define the energy field as the row-centered attention logit and analyze its invariant properties across various models, identifying mechanism-level and model-level invariants.

Result: Two invariant properties were identified: mechanism-level invariants related to the algebraic structure of softmax attention, and model-level regularities across tested autoregressive language models, regardless of architecture.

Conclusion: The findings highlight practical applications of these invariants, such as constraining the energy field and enabling monitoring during training, confirmed across different context lengths and inputs.
...
### [166] [Delay, Plateau, or Collapse: Evaluating the Impact of Systematic Verification Error on RLVR](https://arxiv.org/abs/2605.02909)
*Kazuki Egashira,Mark Vero,Jasper Dekoninck,Florian E. Dorner,Robin Staab,Martin Vechev*

Main category: cs.LG

TL;DR: Systematic verification errors in RLVR can have detrimental effects on model training and performance, contrary to previous assumptions about random errors.


<details>
  <summary>Details</summary>
Motivation: To investigate how systematic errors from real-world verifiers impact the effectiveness of RLVR in training LLMs.

Method: Controlled experiments on arithmetic tasks were conducted to observe the effects of systematic false negatives and positives in verification.

Result: Systematic false negatives produced effects akin to random noise, while false positives led to varied outcomes, from sub-optimal performance to complete degradation.

Conclusion: Verifier quality and error patterns critically influence RLVR outcomes, necessitating a deeper understanding beyond just error rates.
...
### [167] [Agentic AI-Based Joint Computing and Networking via Mixture of Experts and Large Language Models](https://arxiv.org/abs/2605.02911)
*Robert-Jeron Reifert,Alaa Alameer Ahmad,Hayssam Dahrouj,Aydin Sezgin*

Main category: cs.LG

TL;DR: This paper presents a framework for optimizing 6G networks using AI and a mixture of experts (MoE) integrated with large language models (LLMs).


<details>
  <summary>Details</summary>
Motivation: The need for scalable optimization mechanisms in future 6G networks that can manage diverse and specialized optimization experts based on high-level intents and uncertainties.

Method: An agentic AI-based framework that employs LLMs as semantic gates to dynamically compose optimization agents for various network objectives.

Result: The framework shows near-optimal performance compared to exhaustive combinations, outperforming individual experts.

Conclusion: The proposed MoE framework effectively bridges high-level intents with resource management, delivering efficient optimization under various objectives.
...
### [168] [Generate, Filter, Control, Replay: A Comprehensive Survey of Rollout Strategies for LLM Reinforcement Learning](https://arxiv.org/abs/2605.02913)
...
TL;DR: The paper surveys rollout strategies in reinforcement learning for enhancing reasoning in large language models, introducing the GFCR framework to structure and evaluate these design processes.


<details>
  <summary>Details</summary>
Motivation: To address underreporting of rollout design in RL-based post-training of language models and to provide a structured approach for optimizing reasoning capabilities.

Method: Introduces the Generate-Filter-Control-Replay (GFCR) taxonomy to delineate the rollout pipeline into four stages and characterizes strategies by a criterion taxonomy of reliability, coverage, and cost sensitivity.

Result: Synthesized a comprehensive suite of methods for RL rollout strategies, supported by concrete case studies across diverse domains, enhancing skill induction and transfer.

Conclusion: The paper outlines a systematic framework for rollout pipelines, offering diagnostics for common challenges and proposing future research directions for better rollout efficiency and trustworthiness.
...
### [169] [When Safety Geometry Collapses: Fine-Tuning Vulnerabilities in Agentic Guard Models](https://arxiv.org/abs/2605.02914)
*Ismail Hossain,Sai Puppala,Jannatul Ferdaus,Md Jahangir Alam,Yoonpyo Lee,Syed Bahauddin Alam,Sajedul Talukder*

Main category: cs.LG

TL;DR: Safety classifiers can lose effectiveness through benign fine-tuning, impacting AI safety alignment.


<details>
  <summary>Details</summary>
Motivation: Investigate how benign fine-tuning can deteriorate safety alignment in guard models within AI systems.

Method: Analyze safety classifiers using SVD on activation differences to understand latent safety geometry; propose Fisher-Weighted Safety Subspace Regularization to enhance safety performance.

Result: Granite Guardian's refusal rate dropped drastically, yet FW-SSR restored 75% refusal, enhancing safety subspace and reducing attack success rates.

Conclusion: Structural representational geometry is crucial for evaluating guard models, offering a better prediction of safety behavior than traditional metrics.
...
### [170] [From Synthesis to Clinical Assistance: A Strategy-Aware Agent Framework for Autism Intervention based on Real Clinical Dataset](https://arxiv.org/abs/2605.02916)
*Junhong Lai,Shuzhong Lai,Yanhao Yu,Wanlin Chen,Chenyu Yan,Haifeng Li,Lin Yao,Yueming Wang*

Main category: cs.LG

TL;DR: The paper introduces ASDAgent, an AI-assisted framework for Early Intensive Behavioral Intervention (EIBI) for Autism Spectrum Disorder (ASD), addressing data scarcity and inconsistencies in ABA execution through specialized components: DoctorAgent and ChildAgent, achieving high strategic consistency in therapeutic dialogues and enhancing small language models.


<details>
  <summary>Details</summary>
Motivation: To overcome data scarcity in AI-assisted EIBI for ASD and improve adherence to standard ABA procedures.

Method: ASDAgent features a DoctorAgent with an O-T-A-C reasoning loop for explicit ABA execution, and a ChildAgent using probabilistic modeling for diverse ASD response simulations.

Result: Experiments show ASDAgent-generated dialogues closely align with human therapists' strategies (KL divergence: 0.083) and achieve about 80% strategic consistency with human experts.

Conclusion: ASDAgent successfully distills clinical knowledge into small language models, improving their therapeutic capabilities.
...
### [171] [Mitigating the reconstruction-detection trade-off in VAE-based unsupervised anomaly detection](https://arxiv.org/abs/2605.02918)
*Agathe Senellart,Maëlys Solal,Stéphanie Allassonnière,Ninon Burgos*

Main category: cs.LG

TL;DR: This paper investigates the trade-off between reconstruction quality and anomaly detection in variational autoencoders, proposing methods to improve detection metrics without compromising reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: To address the unresolved issue of model selection for unsupervised anomaly detection in variational autoencoders, particularly in balancing reconstruction quality and detection performance.

Method: The paper analyzes $β$-VAE models, examining their performance variability and its relation to latent distribution distances, while proposing two techniques (beta-scheduling and Sparse VAE) to mitigate the identified trade-off.

Result: It finds that constrained latent space improves detection metrics but reduces reconstruction quality, and demonstrates that Sparse VAE enhances detection while preserving reconstruction integrity.

Conclusion: The proposed methods provide strategies to balance reconstruction quality and anomaly detection capabilities, suggesting promising directions for future work.
...
### [172] [DeRelayL: Sustainable Decentralized Relay Learning](https://arxiv.org/abs/2605.02935)
*Haihan Duan,Tengfei Ma,Yuyang Qin,Runhao Zeng,Wei Cai,Victor C. M. Leung,Xiping Hu*

Main category: cs.LG

TL;DR: The paper introduces decentralized relay learning (DeRelayL), a novel collaborative model training paradigm aimed at inclusivity for common users in the era of big data, facilitating sustainable model training and sharing.


<details>
  <summary>Details</summary>
Motivation: Address the exclusion of common users from large-scale model training due to high resource demands and current limitations in collaborative paradigms.

Method: Proposes decentralized relay learning (DeRelayL), which allows permissionless participants to train models in a relay manner and share the results, ensuring participant incentives for sustainability.

Result: Demonstrates the feasibility and effectiveness of DeRelayL through theoretical analysis and numerical simulations.

Conclusion: DeRelayL offers a sustainable and inclusive approach to collaborative model training, benefiting common users and enhancing the accessibility of large-scale machine learning.
...
### [173] [Proteo-R1: Reasoning Foundation Models for De Novo Protein Design](https://arxiv.org/abs/2605.02937)
...
TL;DR: Introducing Proteo-R1, a framework for protein design that combines reasoning and geometric generation for enhanced interpretability and control.


<details>
  <summary>Details</summary>
Motivation: There is a need for more interpretative and controllable models in protein design that separate functional reasoning from geometric synthesis.

Method: Proteo-R1 uses a dual-expert model with a multimodal large language model for understanding functional residues and a diffusion-based model for geometric generation under fixed constraints.

Result: This approach allows for stable and interpretable co-design, aligning with expert human methodology in molecular engineering.

Conclusion: Proteo-R1 successfully integrates reasoning with geometric models in protein design, promoting modularity and interpretability.
...
### [174] [PAMNet: Cycle-aware Phase-Amplitude Modulation Network for Multivariate Time Series Forecasting](https://arxiv.org/abs/2605.02938)
*Yingbo Zhou,Yutong Ye,Zhiwei Ling,Shuhao Li,Rui Qian,Jian Xiong,Li Sun,Dejing Dou*

Main category: cs.LG

TL;DR: The paper presents PAMNet, an innovative network for multivariate time series forecasting that explicitly models periodic patterns by decoupling phase and amplitude components, addressing limitations of existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing forecasting methods struggle with high complexity or insufficient consideration of phase-amplitude interactions in periodic data.

Method: PAMNet introduces a dual-branch modulator for phase and amplitude components using cyclical embeddings and a lightweight element-wise fusion mechanism to combine them.

Result: PAMNet outperforms existing methods on twelve real-world datasets, demonstrating superior forecasting accuracy through its novel decoupling approach.

Conclusion: The proposed PAMNet offers a fresh perspective on cyclical modeling, enhancing the accuracy of multivariate time series forecasts.
...
TL;DR: AutoRAGTuner automates the RAG lifecycle, improving performance and reducing engineering overhead.


<details>
  <summary>Details</summary>
Motivation: To streamline the development process of Retrieval-Augmented Generation systems by automating tuning and configuration, addressing the inefficiencies of manual tuning.

Method: AutoRAGTuner utilizes a modular architecture with component registration, employs the Domain-Element Model (DEM) for data unity, and incorporates an adaptive Bayesian optimization engine for hyper-parameter tuning.

Result: Experimental results showed AutoRAGTuner's flexibility across various RAG pipelines, consistently outperforming baselines and reducing code churn by up to 95%.

Conclusion: AutoRAGTuner establishes a robust framework for creating optimizable and reusable RAG systems.
...
### [176] [Joint Energy Management and Coordinated AIGC Workload Scheduling for Distributed Data Centers: A Diffusion-Aided Reward Shaping Approach](https://arxiv.org/abs/2605.02965)
*Yang Fu,Peng Qin,Liming Chen,Zihao Zhang,Hao Yu,Yifei Wang*

Main category: cs.LG

TL;DR: AIGC workload scheduling needs energy management strategies due to increased cloud data center demands. This paper presents a framework to optimize scheduling and energy costs while ensuring service quality, incorporating deep reinforcement learning with novel reward shaping to enhance decision-making.


<details>
  <summary>Details</summary>
Motivation: To efficiently manage growing computational workloads from AIGC and reduce energy costs in data centers while maintaining content quality.

Method: A joint energy management and coordinated workload scheduling framework incorporating mathematical service quality evaluations, and a diffusion model-aided reward shaping approach integrated with deep reinforcement learning.

Result: Experimental results show improved system utility and learning convergence, successfully adapting to electricity price changes and AIGC model diversity compared to benchmarks.

Conclusion: The proposed method effectively enhances scheduling efficiency and system performance in AIGC service environments.
...
### [177] [From Static Analysis to Audience Dissemination: A Training-Free Multimodal Controversy Detection Multi-Agent Framework](https://arxiv.org/abs/2605.02939)
*Zihan Ding,Ziyuan Yang,Yi Zhang*

Main category: cs.LG

TL;DR: The paper presents AuDisAgent, a multi-agent framework for multimodal controversy detection in videos and comments, addressing limitations of static methods and improving performance in both rich and limited comment scenarios.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to enhance the detection of controversial content by incorporating diverse audience perspectives and dynamic content dissemination processes, which current static methods overlook.

Method: The proposed method uses a multi-agent system, including Screening Agents for initial assessments and a Viewing Panel Agent for discussions among diverse audience groups, culminating in a final judgment by an Arbitration Agent.

Result: Experiments demonstrate significant performance improvements over SOTA methods in detecting controversial content in both rich-comment and limited-comment scenarios.

Conclusion: The AuDisAgent framework effectively captures audience diversity and improves controversy detection in social video platforms.
...
### [178] [Exploring Pass-Rate Reward in Reinforcement Learning for Code Generation](https://arxiv.org/abs/2605.02944)
*Xin-Ye Li,Ren-Biao Liu,Yun-Ji Zhang,Hui Sun,Zheng Xie,Ming Li*

Main category: cs.LG

TL;DR: The study examines the effectiveness of pass-rate rewards in critic-free reinforcement learning for code generation, revealing that they do not consistently enhance performance compared to binary rewards due to miscalibration and conflicting gradient directions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of binary rewards in reinforcement learning for code generation, where sparse rewards can hinder learning, particularly for complex tasks. The authors investigate the potential of pass-rate rewards as an alternative.

Method: The methodology involves controlled experiments comparing the performance of various models using binary rewards versus pass-rate rewards in critic-free reinforcement learning settings (GRPO and RLOO).

Result: The experiments show that while pass-rate rewards alleviate reward sparsity, they do not lead to better performance outcomes in code generation compared to binary rewards. An analysis of reward density and gradient directions further reveals that the pass-rate rewards may misalign with the goal of achieving full correctness.

Conclusion: The study concludes that pass-rate rewards are insufficient for enhancing code generation in critic-free RL, highlighting the need for improved reward designs that align more closely with the objective of full correctness.
...
TL;DR: PrismAgent is a novel framework designed for zero-shot detection of harmful content in memes, utilizing a multi-agent system for enhanced interpretability and effectiveness without relying on large annotated datasets.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the urgent need for effective harmful content detection in memes, highlighting the limitations of existing methods that depend on high-volume annotated data.

Method: PrismAgent employs a multi-agent framework resembling a criminal investigation, with agents for analysis, investigation, prosecution, and judgment, ensuring a structured collaborative approach.

Result: Experimental results demonstrate that PrismAgent outperforms existing zero-shot detection methods across three public datasets, showcasing its effectiveness in identifying harmful content.

Conclusion: The multi-stage, interpretable nature of PrismAgent contributes to improved detection capabilities while minimizing reliance on annotated data, making it a significant advancement in the field.
...
TL;DR: The paper introduces an autoencoder-based neural network model for denoising canine ECGs, effectively reducing noise while preserving critical signal features.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of noise in canine ECGs which obscures important cardiac information, leading to difficulties in analysis.

Method: An autoencoder neural network model is trained to reconstruct clean ECG signals from noisy inputs, focusing on effective noise reduction without compromising waveform integrity.

Result: The proposed model shows strong performance in denoising both noisy and clean ECG recordings, showcasing robustness to different signal conditions.

Conclusion: The autoencoder-based approach is a promising preprocessing step for improving the accuracy of canine ECG analysis.
...
TL;DR: This paper addresses bias in medical AI, specifically in fetal ultrasound image analysis, emphasizing that image quality influences predictive accuracy and can contribute to performance disparities beyond mere representation issues.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how bias in medical AI extends beyond representation to include image quality and acquisition conditions, especially in relation to sensitive demographic factors.

Method: The authors propose a structured framework that combines unsupervised slice discovery, systematic factor-wise analysis, and targeted intersectional evaluation to explore and detect intersectional bias.

Result: In a case study analyzing over 94,000 ultrasound images, the study found that pixel spacing (PS) significantly impacted performance, with a noted 24% improvement in certain subgroups, while also revealing that PS effects are influenced by gestational age (GA) and not limited to BMI factors.

Conclusion: The findings highlight the need for acquisition-aware and interaction-aware evaluations in medical AI to ensure fairness, as biases in image acquisition methods can skew results and obscure true disparities.
...
### [182] [Healthcare AI GYM for Medical Agents](https://arxiv.org/abs/2605.02943)
*Minbyul Jeong*

Main category: cs.LG

TL;DR: This paper investigates multi-turn agentic reinforcement learning (RL) for medical AI using a comprehensive environment with diverse tasks. It addresses issues in training stability and performance, proposing a novel self-distillation method for improvement.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance clinical AI training by creating a robust environment that supports multi-step interactions essential for effective clinical reasoning.

Method: The study utilizes a gymnasium-compatible environment with 10 clinical domains, introducing a self-distillation framework called Turn-level Truncated On-Policy Distillation (TT-OPD) to improve training efficiency and stability.

Result: TT-OPD outperforms previous methods on 10 of 18 benchmarks, showing an average improvement of +3.9 percentage points while ensuring better control over response length and stable tool usage.

Conclusion: The findings demonstrate the viability of TT-OPD in achieving enhanced performance and stability in multi-turn interactions for medical AI, despite challenges in the training process.
...
### [183] [RouteHijack: Routing-Aware Attack on Mixture-of-Experts LLMs](https://arxiv.org/abs/2605.02946)
*Zhiyuan Xu,Joseph Gardiner,Sana Belguith,Lichao Wu*

Main category: cs.LG

TL;DR: The paper introduces RouteHijack, a routing-aware jailbreak for Mixture-of-Experts LLMs, which exploits safety behavior concentrated in specific experts to manipulate model output, achieving a high attack success rate.


<details>
  <summary>Details</summary>
Motivation: To address safety alignment in LLMs, particularly in the context of the increasingly used Mixture-of-Experts architectures, where understanding safety robustness is paramount.

Method: RouteHijack utilizes response-driven expert localization to identify key experts influencing safety and employs routing-aware input optimization to construct adversarial suffixes that skew model responses towards harmful outcomes.

Result: RouteHijack achieves a 69.3% average attack success rate across seven MoE LLMs, outperforming previous attacks significantly and showing improved transferability across related models.

Conclusion: The findings reveal a critical vulnerability in sparse expert systems, necessitating improved defensive strategies beyond merely optimizing output alignment.
...
### [184] [OGPO: Sample Efficient Full-Finetuning of Generative Control Policies](https://arxiv.org/abs/2605.03065)
...
TL;DR: This paper presents Off-policy Generative Policy Optimization (OGPO), a novel algorithm for fine-tuning generative control policies in robot learning that enhances data reuse and achieves superior performance in various manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: The goal is to improve robot learning through effective parameterization and fine-tuning of generative control policies by maximizing data reuse and overcoming limitations of existing methods.

Method: OGPO utilizes off-policy critic networks within a modified Proximal Policy Optimization (PPO) framework to optimize generative control policies, along with practical stabilizers to prevent over-exploitation by the critics.

Result: OGPO achieves state-of-the-art results across diverse manipulation tasks, successfully fine-tuning poorly-initialized behavior cloning policies without expert data, and requiring minimal hyperparameter tuning.

Conclusion: The study shows marked performance improvements with OGPO and identifies key stabilizing mechanisms, further advancing the understanding of generative policy finetuning.
...
### [185] [Predicting Euler Characteristics and Constructing Topological Structure Using Machine Learning Techniques](https://arxiv.org/abs/2605.02947)
*Gyunghun Yu,Seong Min Park,Han Gyu Yoon,Tae Jung Moon,Jun Woo Choi,Hee Young Kwon,Changyeon Won*

Main category: cs.LG

TL;DR: The study presents a neural network approach to extract the Euler characteristic from images using a single geometric image, inspired by solid-state physics. It predicts the characteristic through skyrmion number calculation while incorporating a magnetic Hamiltonian for improved accuracy.


<details>
  <summary>Details</summary>
Motivation: To extract topological properties like the Euler characteristic from images using minimal data, inspired by techniques in solid-state physics.

Method: A neural network generates a unit vector field from a geometric image and predicts the Euler characteristic via the skyrmion number; a magnetic Hamiltonian is used as a physics-informed loss function.

Result: The model successfully predicts topological properties without requiring ground-truth configurations, even with complex geometrical inputs.

Conclusion: The approach demonstrates an innovative solution for extracting topological information from images, enabling applications to practical tasks without reliance on large datasets.
...
### [186] [AsymK-Talker: Real-Time and Long-Horizon Talking Head Generation via Asymmetric Kernel Distillation](https://arxiv.org/abs/2605.02948)
*Yuxin Lu,Qian Qiao,Jiayang Sun,Min Cao,Guibo Zhu*

Main category: cs.LG

TL;DR: AsymK-Talker addresses limitations in audio-driven talking head generation using a novel method that enhances real-time capabilities and maintains coherence over long sequences.


<details>
  <summary>Details</summary>
Motivation: The need for efficient, real-time talking head generation systems that maintain visual fidelity and coherent motion over long periods.

Method: AsymK-Talker employs a diffusion-distillation approach incorporating Kernel-Conditioned Loop Generation, Temporal Reference Encoding, and Asymmetric Kernel Distillation to improve audio-visual sync and maintain consistency.

Result: AsymK-Talker demonstrates significant improvements in visual fidelity and metrics related to lip synchronization.

Conclusion: The proposed method effectively overcomes major limitations in existing talking head generation techniques, making it suitable for real-time applications.
...
### [187] [Disease Is a Spectral Perturbation](https://arxiv.org/abs/2605.02949)
*John D. Mayfield,Matthew S. Rosen*

Main category: cs.LG

TL;DR: A novel method for understanding disease transformation from healthy baselines using biomarker covariance matrices, enabling mechanistic explanations of disease trajectories.


<details>
  <summary>Details</summary>
Motivation: To better understand disease transitions and trajectories on a biomolecular level using patient-derived biomarker data.

Method: Modeling biomarker covariance matrices and defining a Hamiltonian to characterize perturbations caused by disease states, analyzing eigenvalues and eigenvectors.

Result: Formulated a mathematical framework that relates disease perturbations to eigenvalue changes, providing a new prognostic statistic for individual patient prognosis based on biomarker data.

Conclusion: The proposed framework offers insights into disease mechanisms and could enhance prognostic precision across various diseases.
...
TL;DR: The paper describes a pipeline for predictive monitoring of clinical pathways, evaluated on COVID-19 data, demonstrating enhanced predictive capabilities over time as more data becomes available.


<details>
  <summary>Details</summary>
Motivation: To address limitations of traditional retrospective process mining in monitoring clinical pathways.

Method: The method includes data lifting, temporal reconstruction, event log construction, and predictive modeling, specifically focusing on ICU admission predictions for COVID-19 patients.

Result: Logistic Regression produced the highest performance with AUC 0.906 and F1-score 0.835, showing that predictive accuracy improves as more clinical event data is utilized.

Conclusion: Predictive monitoring in healthcare should be viewed as a dynamic process where risk estimates evolve with patient data.
...
TL;DR: This paper presents Kernel Affine Hull Machines (KAHMs) as an efficient alternative to transformer-based semantic retrieval processes, maintaining high retrieval quality while significantly reducing latency.


<details>
  <summary>Details</summary>
Motivation: To address the high costs in online query encoding in transformer-based retrieval systems and explore effective alternatives that preserve retrieval quality.

Method: KAHMs map lexical features into a semantic embedding space, using an analytically explicit estimator to assess and refine prototype weights, breaking down encoding errors into distinct components.

Result: KAHM shows superior performance in teacher-space reconstruction and ranks higher in multiple retrieval metrics (MRR@20: 0.504, Hit@20: 0.694, Top-1 accuracy: 0.411) compared to matched learned adapters, while also achieving an 8.5-fold reduction in per-query latency.

Conclusion: KAHMs can effectively replace online neural encoding in fixed-teacher settings without compromising retrieval performance, enhancing both efficiency and interpretability.
...
TL;DR: The paper presents a Bayesian approach for calibrating urban flood simulation parameters through optimization, introducing latent variables to handle uncertainties effectively.


<details>
  <summary>Details</summary>
Motivation: Accurate calibration of urban surface parameters is essential for reliable urban flood simulations.

Method: The study formulates the calibration as an optimization problem using Bayesian principles, employs a surrogate model for flood dynamics, introduces latent variables for uncertainty representation, and utilizes adjoint equations for efficient optimization.

Result: The proposed methodology demonstrates rapid convergence and good insensitivity to observation time intervals, calibrating Manning's coefficient with relative errors ranging from 1.16% to 13.88%.

Conclusion: The methods introduced enhance the efficiency and accuracy of urban flood parameter calibration.
...
TL;DR: ZeRO-Prefill enhances MoE model efficiency by utilizing asynchronous expert weight gathering to reduce overheads associated with distributed execution, resulting in significant throughput improvements over existing methods.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiencies in serving discriminative tasks with MoE models, particularly the bottlenecks caused by coupling expert placement and synchronous activation routing.

Method: ZeRO-Prefill employs AsyncEP to gather expert weights asynchronously while enforcing a load threshold through prefix-aware routing and FLOPs tracking, optimizing for large-batch prefill workloads.

Result: ZeRO-Prefill achieves 1.35-1.37x increased throughput over existing distributed systems on real-world tasks, and up to 1.59x on synthetic workloads, while maintaining high per-GPU FLOPs utilization.

Conclusion: The proposed system significantly enhances the efficiency of serving prefill-only workloads in MoE models, reducing computational overhead and improving throughput.
...
TL;DR: LQ-GM-PID is an analytically solvable bridge-diffusion method that utilizes linear-quadratic-Gaussian control principles for efficient transport and path shaping without neural networks.


<details>
  <summary>Details</summary>
Motivation: To improve finite-time transport in bridge-diffusion methods by offering a class that is analytically solvable, eliminating the need for neural networks and inner stochastic simulations.

Method: LQ-GM-PID recasts classical LQG stochastic-control as a transport problem, allowing for Gaussian Mixtures in the initial and terminal distributions and enabling path shaping rather than only terminal matching.

Result: Demonstrated its effectiveness on various tasks, achieving sub-50 ms precompute on standard hardware, establishing LQ-GM-PID's utility as an analytically manageable model.

Conclusion: LQ-GM-PID serves as a reference model for testing and benchmarking neural bridge-diffusion and generative transport methods in a controlled analytical context.
...
TL;DR: ISAAC evaluates structural sensitivity in deep learning models for drug-target interaction, revealing significant reasoning disparities despite similar accuracy.


<details>
  <summary>Details</summary>
Motivation: To identify limitations in deep learning DTI models that rely on inaccurate mechanistic features despite high benchmark accuracy.

Method: ISAAC is a post-hoc framework assessing structural sensitivity through interventions on frozen models, focusing on mechanism relevance rather than just accuracy.

Result: ISAAC shows around 25% relative differences in reasoning across DTI models with similar AUROC, indicating variabilities undetected by accuracy metrics.

Conclusion: The findings advocate for post-hoc structural auditing to enhance standard evaluation methods in molecular modeling.
...
TL;DR: The paper introduces the Reward Hacking Benchmark (RHB) to assess the exploit rates of various language model agents in sequential tasks, revealing significant exploit potential and the impact of RL post-training on these rates.


<details>
  <summary>Details</summary>
Motivation: To evaluate the vulnerability of RL-trained language models to reward hacking in tool-accessible environments, particularly in complex sequential tasks.

Method: Introduced the RHB for multi-step tasks and evaluated 13 language models from leading organizations, categorizing exploits and analyzing models' behaviors with examples and comparative studies.

Result: Exploit rates varied from 0% to 13.9% among different models, with RL post-training linked to higher reward hacking. Six exploit categories were identified, with 72% of hacks expressing reasoning.

Conclusion: Environmental hardening effectively decreased exploit rates while maintaining task success; however, models displayed increased exploits in more complex tasks, indicating a threshold where honest solutions become less feasible.
...
### [195] [Finite-Size Gradient Transport in Large Language Model Pretraining: From Cascade Size to Intensive Transport Efficiency](https://arxiv.org/abs/2605.02968)
*Ping Wang,Yan-Qi Du*

Main category: cs.LG

TL;DR: The paper presents a framework for analyzing gradient transport in language models, highlighting differences in transport efficiency between models Pico-LM and Pythia.


<details>
  <summary>Details</summary>
Motivation: To investigate the transport dynamics of real language-model training using a finite-size gradient-transport framework.

Method: A comparative analysis of direct raw-gradient measurements from Pico-LM and a companion dataset from Pythia, examining key observables such as cascade size, duration, and transport efficiency across multiple scales.

Result: Pico-LM exhibits positive duration scaling with negative intensive-efficiency scaling, while Pythia shows near-unity scaling with less pronounced efficiency dependency; both families adhere to shared algebraic closures but differ in compressibility and performance associations.

Conclusion: The study offers a reusable measurement framework for transport dynamics in language models without asserting universal laws, emphasizing model-specific transport characteristics.
...
### [196] [Multilingual Safety Alignment via Self-Distillation](https://arxiv.org/abs/2605.02971)
*Ruiyang Qin,Qingzhuo Wang,Dongrui Liu,Qiang Li,Zhihua Wei,Wen Shen*

Main category: cs.LG

TL;DR: The paper presents Multilingual Self-Distillation (MSD), a framework aimed at enhancing multilingual safety in large language models (LLMs) by transferring safety capabilities from high-resource to low-resource languages without needing response data.


<details>
  <summary>Details</summary>
Motivation: LLMs show significant safety misalignment, being more secure in high-resource languages but vulnerable in low-resource contexts, prompting the need for improved safety methods.

Method: The Multilingual Self-Distillation (MSD) framework employs cross-lingual safeguard transfer and includes two methods: on-policy MSD and off-policy MSD, leveraging multilingual queries, along with DPSW for optimizing distillation safety objectives.

Result: MSD effectively transfers safety capabilities and improves multilingual safety performance, demonstrating better generalization across diverse benchmarks and preserving the model's overall effectiveness.

Conclusion: The proposed framework enhances multilingual safety in LLMs, particularly in low-resource languages, addressing a critical gap in current safety alignment methods.
...
### [197] [Structured Diffusion Bridges: Inductive Bias for Denoising Diffusion Bridges](https://arxiv.org/abs/2605.02973)
*Eitan Kosman,Gabriele Serussi,Chaim Basking*

Main category: cs.LG

TL;DR: The paper proposes a diffusion-bridge framework for modality translation that allows for flexible supervision levels, achieving near fully-paired quality without stringent pairing requirements.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing modality translation methods that rely on fully paired datasets, restricting their applicability.

Method: A diffusion-bridge framework that characterizes admissible solution spaces and utilizes alignment constraints without mandatory paired supervision.

Result: The proposed method demonstrates consistent performance across different levels of supervision (unpaired, semi-paired, paired) and achieves near fully-paired quality with relaxed pairing requirements.

Conclusion: Diffusion bridges provide a flexible foundation for modality translation, enabling effective results even in unpaired scenarios.
...
### [198] [Mixed-Precision Information Bottlenecks for On-Device Trait-State Disentanglement in Bipolar Agitation Detection](https://arxiv.org/abs/2605.03039)
*Joydeep Chandra*

Main category: cs.LG

TL;DR: Introducing MP-IB, a framework for monitoring bipolar disorder agitation using voice biomarkers, which effectively differentiates stable speaker traits from volatile emotional states using mixed-precision quantization.


<details>
  <summary>Details</summary>
Motivation: To improve continuous monitoring of bipolar disorder agitation through voice biomarkers on resource-constrained devices.

Method: The MP-IB framework utilizes mixed-precision quantization to separate speaker identity (FP16 trait head) and agitation state (INT4 state head), while incorporating Dynamic Precision Scheduling and Multi-Scale Temporal Fusion.

Result: The framework achieved rho = 0.117 in distinguishing agitation in a clinical setting, outperforming current methods by significant margins and demonstrating effective zero-shot transfer capabilities.

Conclusion: MP-IB enables real-time monitoring on low-cost devices, achieving high efficiency and minimal identity leakage, positioning it as a viable solution for continuous mental health monitoring.
...
### [199] [TCD-Arena: Assessing Robustness of Time Series Causal Discovery Methods Against Assumption Violations](https://arxiv.org/abs/2605.03045)
*Gideon Stein,Niklas Penzel,Tristan Piater,Joachim Denzler*

Main category: cs.LG

TL;DR: The paper presents TCD-Arena, a customizable testing kit that evaluates the robustness of causal discovery algorithms against various assumption violations.


<details>
  <summary>Details</summary>
Motivation: To address the limitations in the practical adoption of Causal Discovery due to reliance on strong assumptions and insufficient performance assessment.

Method: A modularized testing kit, TCD-Arena, is developed to assess the robustness of time series causal discovery algorithms through extensive empirical studies involving 30 million CD attempts and evaluation under 33 distinct assumption violations.

Result: The study reveals nuanced robustness profiles for various assumption violations and explores the potential of CD ensembles in improving general robustness across scenarios.

Conclusion: The findings aim to guide the development of reliable causal discovery methods suitable for varied synthetic and real-world data conditions.
...
### [200] [Neuron-Anchored Rule Extraction for Large Language Models via Contrastive Hierarchical Ablation](https://arxiv.org/abs/2605.03058)
*Francesco Sovrano,Gabriele Dominici,Marc Langheinrich*

Main category: cs.LG

TL;DR: MechaRule presents an efficient way to ground rule extraction in large language models' (LLMs) circuitry through the identification and manipulation of sparse neurons called agonists.


<details>
  <summary>Details</summary>
Motivation: The study addresses the challenges in explainable AI (XAI), specifically in connecting symbolic rule extraction to the internal workings of LLMs, which traditional methods fail to do effectively.

Method: MechaRule uses group testing to efficiently locate agonists—sparse neurons whose activation significantly influences behaviors—by applying confidence-guided pruning in a specific regime.

Result: The pipeline recalls 96.8% of high-effect agonists on tested models and demonstrates that manipulating these neurons can significantly affect model performance, decreasing arithmetic accuracy and jailbreak success rates.

Conclusion: MechaRule effectively localizes crucial neuron groups in LLMs, connecting symbolic understanding with mechanistic interpretability and showing that agonist identification can enhance our insight into LLM decision logic.
...
### [201] [Adaptive Negative Scheduling for Graph Contrastive Learning](https://arxiv.org/abs/2605.03076)
*Adnan Ali,Jinlong Li,Syed Muhammad Israr,Ali Kashif Bashir*

Main category: cs.LG

TL;DR: This paper presents AdNGCL, an adaptive negative scheduling framework for graph contrastive learning (GCL) that enhances performance by dynamically selecting negatives based on their informativeness and computational cost.


<details>
  <summary>Details</summary>
Motivation: The study addresses the limitations of static negative sampling in existing GCL methods, which do not adapt to the changing informativeness and computational load of negative samples during training.

Method: AdNGCL introduces a hardness-aware scheduler (HANS) that organizes negative selection into a loss-gated, budget-constrained process, adjusting step sizes according to contrastive loss trends while refreshing samples periodically for diversity.

Result: Experiments conducted on nine benchmark graph datasets showed that AdNGCL improves state-of-the-art performance, achieving the highest accuracy on seven datasets and second-highest on two, while managing computational costs effectively.

Conclusion: The research underlines the importance of budget-aware, loss-sensitive negative scheduling as a means to enhance the robustness and efficiency of representation learning in computational intelligence.
...
### [202] [Adaptive Data Compression and Reconstruction for Memory-Bounded EEG Continual Learning](https://arxiv.org/abs/2605.03085)
*Chengcheng Xie*

Main category: cs.LG

TL;DR: The paper presents a novel method called ADaCoRe for improving EEG signal analysis through efficient data compression and reconstruction in the context of Unsupervised Individual Continual Learning (UICL), showcasing significant performance gains.


<details>
  <summary>Details</summary>
Motivation: EEG signal analysis is hindered by noise and variability, necessitating a method for efficient personalization with limited labeled data.

Method: ADaCoRe uses a memory-efficient pipeline that includes saliency-driven keyframe protection, rational polyphase compression, adjoint reconstruction for selective overwriting, and prototype-confidence selection for maintaining adaptive exemplars.

Result: The method outperforms existing baseline models significantly, achieving at least +2.7 and +15.3 accuracy improvements on the ISRUC and FACED datasets, respectively, under tight buffer constraints.

Conclusion: ADaCoRe effectively addresses the challenges of memory management and signal integrity in EEG analysis, with ablation studies demonstrating the importance of its design components.
...
### [203] [Self-Mined Hardness for Safety Fine-Tuning](https://arxiv.org/abs/2605.03226)
*Prakhar Gupta,Garv Shah,Donghua Zhang*

Main category: cs.LG

TL;DR: This study proposes a novel fine-tuning method for language models to enhance safety against adversarial prompts, significantly reducing attack success rates while managing false refusals on benign prompts.


<details>
  <summary>Details</summary>
Motivation: The aim is to improve safety fine-tuning of language models without relying on curated adversarial datasets.

Method: The proposed method scores prompts based on the frequency of harmful judgments from the model's rollouts and fine-tunes on the hardest prompts alongside non-jailbroken rollouts.

Result: The new approach reduced WildJailbreak attack success rates from 11.5% and 20.1% to 1-3%, but increased refusals on benign prompts to 74-94%. Interleaving hard prompts with benign prompts adjusted refusal rates down to 30-51% and 52-72%, at a slight cost to attack success.

Conclusion: By strategically selecting prompts, the fine-tuning can enhance safety and manage false refusals in adversarial settings.
...
### [204] [Attribution-Guided Masking for Robust Cross-Domain Sentiment Classification](https://arxiv.org/abs/2605.03091)
*Shubham Harkare,Arvind Yogesh Suresh Babu,Yash Kulkarni*

Main category: cs.LG

TL;DR: This paper introduces Attribution-Guided Masking (AGM) to address the generalization gap in Transformer models for sentiment classification when transferring to out-of-domain data, improving performance by suppressing reliance on domain-specific tokens.


<details>
  <summary>Details</summary>
Motivation: To investigate and mitigate the performance degradation of pre-trained Transformer models when applied to out-of-domain sentiment classification due to reliance on spurious, domain-specific tokens.

Method: The paper proposes Attribution-Guided Masking (AGM), which dynamically detects and penalizes spurious tokens during fine-tuning using a gradient-based attribution masking loss. This approach, which does not require target-domain labels, also allows optional inclusion of a counterfactual contrastive loss to promote domain invariance.

Result: AGM achieves competitive generalization in a zero-shot transfer setting, showing performance enhancement on difficult transfers, particularly on the Sentiment140 dataset, compared to several strong baselines. It provides insights into which features contribute to the generalization gap, specifically reducing reliance on domain-specific language.

Conclusion: AGM effectively improves model performance by suppressing the impact of spurious tokens, as confirmed by qualitative analysis and ablation studies, showcasing the importance of attribution-guided masking in enhancing generalization across diverse domains.
...
TL;DR: This paper critiques the sufficiency of differential privacy (DP) in providing privacy for machine learning prediction explanations, demonstrating that an adversary can effectively reconstruct graph structures even when DP is applied. It introduces a new attack method, PRIVX, and provides guidance for selecting explainers based on graph properties.


<details>
  <summary>Details</summary>
Motivation: To address concerns about the privacy of machine learning model predictions, especially in light of regulatory demands for post-hoc explanations while ensuring the safety of sensitive data.

Method: The authors propose the PRIVX attack, leveraging the properties of the Gaussian differential privacy mechanism alongside a Bayesian denoising framework to reconstruct hidden graph structures using perturbed explanations.

Result: The attack demonstrates high accuracy in reconstructing graph structures, achieving an area under the curve (AUC) above 0.7 for five out of seven datasets tested with various DP mechanisms and graph neural network architectures.

Conclusion: DP alone may not adequately protect against adversarial reconstruction attempts, necessitating careful selection of explanation methods tailored to specific graph characteristics to mitigate leakage.
...
TL;DR: Hybrid Prompt Arithmetic (HyPA) improves model robustness against confounding shifts by combining task prompts with confounder prompts without necessitating full fine-tuning, making it a parameter-efficient solution.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of models exploiting spurious features for strong performance during in-distribution tasks, leading to poor generalization in out-of-distribution scenarios.

Method: The authors propose Hybrid Prompt Arithmetic (HyPA), which implements task arithmetic on soft prompts created through prompt tuning, combining task prompts with linearized confounder prompts.

Result: HyPA shows consistent improvements in the robustness-performance trade-off across various benchmarks when facing distribution shifts, compared to traditional prompt arithmetic baselines.

Conclusion: The study establishes HyPA as an effective and computationally efficient strategy for enhancing model robustness under confounding shifts.
...
TL;DR: The paper presents the ensemble directional Kalman filter (EnDKF) for improved pose tracking by combining position and attitude estimation using directional statistics, resulting in lower tracking errors.


<details>
  <summary>Details</summary>
Motivation: To enhance the accuracy of pose tracking by addressing poor performance of traditional Kalman filters in modeling directional uncertainty.

Method: The paper introduces the EnDKF, which utilizes an ensemble-based approach and a unit-quaternion representation for attitude to jointly estimate position and orientation.

Result: The EnDKF demonstrates a significant reduction in tracking error in both synthetic and digital-twin scenarios compared to traditional measurement methods.

Conclusion: The EnDKF effectively captures directional uncertainty and improves pose estimation accuracy over conventional methods.
...
TL;DR: The paper presents a method to boost inference speeds in transformer models by leveraging low effective rank in token activations without retraining or architecture changes.


<details>
  <summary>Details</summary>
Motivation: To accelerate inference in transformer language models while maintaining output quality, especially in regards to memory efficiency and computational load.

Method: The method decomposes activation vectors into subspace components and residuals, using cached low-rank weights for faster linear-layer outputs, alongside a gating mechanism to balance output accuracy vs. speed.

Result: Achieved speedup of 3.0x to 10.5x on specific models, with perplexity ratios below 1.00 and over 98% token agreement on outputs across tested model families.

Conclusion: The approach preserves the accuracy of outputs while significantly accelerating inference, demonstrating its effectiveness without requiring changes to the model architecture or additional training.
...
TL;DR: The paper presents a method called Activation Decorrelation Attention (ADA) to reduce computational costs in transformer attention layers by efficiently selecting representative tokens, achieving significant savings in Gram operations.


<details>
  <summary>Details</summary>
Motivation: To address the high computational cost of representative token selection in transformer models, which remains a bottleneck in efficient attention mechanisms.

Method: ADA selects a limited number of representative tokens in transformer layers, leveraging a cascade mechanism that propagates token selection across layers, thus reducing the need for costly Gram matrix computations at each layer.

Result: The proposed method decreases the computational complexity of token selection from O(T^2 d) to O(T r d) per layer and demonstrates significant Gram operation savings (22%-63%) while maintaining high Jaccard overlap (0.83-0.94) between layers across various model families.

Conclusion: The findings support the notion that informative tokens demonstrate consistency through layers in a transformer network, indicating that token relevance is a structural characteristic of the input.
...
TL;DR: This paper addresses the challenges of robustness and data efficiency in multi-agent reinforcement learning (MARL) with distributionally robust Markov games (RMGs), presenting new algorithms that improve performance in large state spaces.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the robustness and data efficiency of MARL in the face of environmental uncertainties, particularly in large-scale and infinite state spaces.

Method: The authors develop provably data-efficient algorithms tailored for general RMGs with linear function approximation, specifically for uncertainty sets defined by total variation distance, and explore both generative model and online interactive settings.

Result: The proposed algorithms successfully mitigate the curse of multiagency by improving sample complexity in RMGs, even within large (possibly infinite) state spaces.

Conclusion: The findings represent a significant advancement in handling sample complexity for multi-agent settings in RMGs, making the approach more feasible for complex problems.
...
TL;DR: This paper proposes the normalized excess cost (NEC) metric to differentiate the impact of classification errors based on their costs, highlighting substantial disparities in error rates across various benchmarks.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the disproportionate impact of classification errors in critical applications, where clear-cut mistakes are costlier than ambiguous ones.

Method: It introduces NEC, which weights errors based on per-example costs derived from various factors and compares its effectiveness against traditional error rates across multiple data types.

Result: The study finds that NEC is often significantly lower than standard error rates, revealing that most errors occur in ambiguous examples with lower associated costs, such as achieving 1.8% NEC with a 5% error rate.

Conclusion: Despite the promise of cost-sensitive training methods, benefits are inconsistent; improvements are evident only when costs are predictable, while real-world datasets show mixed outcomes, demonstrating the need for a structured approach to instance-level misclassification costs.
...
TL;DR: The paper introduces OCRR, a new benchmark for measuring the recovery speed of classification systems through online corrections, evaluating various algorithms and substrates under distribution shift conditions.


<details>
  <summary>Details</summary>
Motivation: Existing static benchmarks do not address recovery from distribution shifts in real systems; this research aims to fill that gap.

Method: OCRR benchmar integrates oracle or stochastic corrections into classification systems, measuring novel-class and original-distribution accuracy against correction counts.

Result: The proposed substrate outperforms nine baseline algorithms across tasks by achieving high retention of accuracies despite varying corpus scales, showing significant improvements in both metrics.

Conclusion: The substrate exhibits stability in classification accuracy and resilience against retrieval imperfections, demonstrating its effectiveness in online correction scenarios.
...
TL;DR: This paper proposes a new interpretability protocol for sparse-autoencoders, revealing significant findings about feature interactions and their effects on model outputs that the standard protocol overlooks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the interpretability of sparse-autoencoders by addressing the limitations in the current standard protocol, especially in identifying nuanced feature behaviors and interactions.

Method: The authors introduce a pairwise matrix protocol and a co-varying steering coefficient involving joint conditions, which evaluates the influence of specific model features on output quality.

Result: The study identifies three key findings: an inverted U-shape response of a self-disclaimer feature, the detrimental impact of joint suppression of certain features on composition and coherence, and a distinction between output patterns based on the perturbation methods applied.

Conclusion: The paper concludes that the new protocol provides a more refined understanding of feature contribution and interaction, revealing critical insights into model behavior that traditional methods fail to capture.
...
TL;DR: This study presents a topology-aware attention framework to enhance predictive modeling of time series data by integrating geometric structures not captured by standard attention mechanisms. It demonstrates improvements across various architectures and datasets, showing the effectiveness of incorporating topological information in predictions.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of standard dot-product attention in capturing complex geometric structures in scientific time series data, this study aims to develop a method that incorporates topological information to improve prediction accuracy.

Method: The authors propose a topology-aware attention framework that utilizes persistent homology, Euler characteristic transforms, and kernel-Hilbert channels, capturing local topological signals through a validation-gated local residual approach, and evaluates the framework with rigorous experimental protocols.

Result: The topology-aware models showed varying degrees of improvement across three architecture families, with lightweight attention/Ridge improving in 46 out of 63 units (12.5% RMSE reduction), PatchTST showing improvements in 33 units (23.5% RMSE reduction), and TimeSeriesTransformer achieving the best performance in 47 units (47.8% RMSE reduction).

Conclusion: The findings suggest that integrating topological data as an inductive bias enhances modeling capabilities, confirming the utility of topology-aware approaches in predictive task performances across various datasets.
...
### [215] [Moral Sensitivity in LLMs: A Tiered Evaluation of Contextual Bias via Behavioral Profiling and Mechanistic Interpretability](https://arxiv.org/abs/2605.03217)
*Yash Aggarwal,Atmika Gorti,Vinija Jain,Aman Chadha,Krishnaprasad Thirunarayan,Manas Gaur*

Main category: cs.LG

TL;DR: The paper critiques binary bias evaluations of large language models, proposing a nuanced approach using the Moral Sensitivity Index (MSI) and mechanistic validation to analyze bias across several models and contexts.


<details>
  <summary>Details</summary>
Motivation: To address the simplification of bias evaluations in large language models by presenting a more nuanced, context-sensitive analysis of bias emergence.

Method: The study introduces the Moral Sensitivity Index (MSI) for behavioral profiling, applying rigorous stress tests, and conducts mechanistic validation through various analysis techniques on selected models.

Result: Distinct behavioral signatures of bias were identified across models, with Gemini 1.5 and Claude leading in different contexts, while mechanistic analysis revealed a U-curve indicating bias re-emergence post-reasoning distillation.

Conclusion: The findings show that bias in language models is complex and context-dependent, urging for deeper investigations into model architecture and training processes.
...
### [216] [Beyond Activation Alignment: The Geometry of Neural Sensitivity](https://arxiv.org/abs/2605.03222)
*Amirhossein Yavari,Farnaz Zamani Esfahlani*

Main category: cs.LG

TL;DR: The paper introduces a framework that assesses local decodable information in neural representations, focusing on their ability to discriminate small perturbations under noise, complementing traditional global activation-alignment measures.


<details>
  <summary>Details</summary>
Motivation: The study aims to enhance the understanding of how neural representations react to local stimulus evidence, overcoming limitations of existing global measures that overlook sensitivity to perturbations.

Method: The framework is built on concepts of Fisher information and local representation geometry, summarizing representations via the expected projected pullback/Fisher metric within a stimulus-coordinate subspace and developing the Spectral Riemannian Alignment Score (S-RAS).

Result: The proposed method successfully identifies corresponding layers in trained neural networks, reveals class-conditional relationships, distinguishes between training approaches, and highlights stimulus-coordinate effects in mouse visual cortex data.

Conclusion: This framework advances the comparison of neural representations by integrating local decodable information, providing a richer analysis of how neural systems utilize stimulus information.
...
### [217] [Text-Conditional JEPA for Learning Semantically Rich Visual Representations](https://arxiv.org/abs/2605.03245)
*Chen Huang,Xianhang Li,Vimal Thilak,Etai Littwin,Josh Susskind*

Main category: cs.LG

TL;DR: TC-JEPA enhances visual self-supervised learning by using image captions to improve feature prediction stability and semantic representation.


<details>
  <summary>Details</summary>
Motivation: To address the challenges posed by visual uncertainty in masked feature prediction in I-JEPA, aiming for better semantic learning and performance.

Method: TC-JEPA introduces a text-conditional approach that modulates predicted patch features through sparse cross-attention with input text tokens, making features predictable and semantically rich.

Result: TC-JEPA demonstrates improved downstream performance and stability during training, outperforming traditional contrastive methods, especially in tasks demanding detailed visual understanding.

Conclusion: This work establishes a new paradigm for vision-language pretraining focused solely on feature prediction, achieving significant advancements in visual reasoning tasks.
...
### [218] [Ortho-Hydra: Orthogonalized Experts for DiT LoRA](https://arxiv.org/abs/2605.03252)
*Seunghyun Ji*

Main category: cs.LG

TL;DR: Ortho-Hydra improves LoRA fine-tuning of diffusion transformers by addressing ‘style bleed’ through a novel re-parameterization that enables effective gradient propagation for diverse artist styles.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitation of traditional LoRA fine-tuning where multiple distinct artist fingerprints result in averaging due to style bleed, thereby enabling better performance with multi-style data.

Method: Introducing Ortho-Hydra, which utilizes a Cayley-orthogonal shared basis and disjoint output subspaces for each expert, enhancing the router's specialization capability from the outset.

Result: Ortho-Hydra demonstrates improved routing dynamics and capacity to de-uniformise within the initial training steps compared to HydraLoRA baselines, which are trapped in uniform prior.

Conclusion: The approach effectively addresses early gradient propagation issues in a multi-expert setup, leading to potentially better training outcomes in LoRA fine-tuning scenarios.
...
### [219] [Posterior-First Neural PDE Simulation: Inferring Hidden Problem State from a Single Field](https://arxiv.org/abs/2605.03247)
*Wenshuo Wang,Fan Zhang*

Main category: cs.LG

TL;DR: The paper advocates for a posterior-first approach in neural PDE simulation to improve predictive performance and decision-making under uncertainty, as traditional methods may lose critical latent information.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of traditional field-to-future predictors in neural PDE simulators, especially when only a single observation is available at deployment, which can lead to loss of key ambiguity and reliability in predictions.

Method: The proposed approach involves inferring a posterior over the minimal task-sufficient problem state before conditioning future predictions on that posterior, thus enhancing the predictive framework.

Result: Synthetic experiments demonstrated that the posterior-based method shows improved tracking of ambiguity compared to traditional methods, and on real tasks, it significantly reduced the normalized root mean square error (nRMSE) from 0.175 to 0.132, closing a large gap in performance.

Conclusion: The findings indicate that neural PDE simulation should adopt a posterior-first methodology instead of relying solely on deterministic field-to-future predictions to better handle uncertainty and enhance performance.
...
### [220] [Do LLMs have core beliefs?](https://arxiv.org/abs/2605.03255)
*Anna Sokol,Marianna B. Ganapini,Nitesh V. Chawla*

Main category: cs.LG

TL;DR: The paper investigates whether Large Language Models (LLMs) possess core beliefs similar to human cognition, using a probing framework. It finds that LLMs struggle to maintain stable worldviews despite some improvements in performance over generations.


<details>
  <summary>Details</summary>
Motivation: To explore the structural aspect of human cognition, specifically whether LLMs have core commitments that shape their understanding and worldview.

Method: The authors employ a framework called Adversarial Dialogue Trees (ADTs) to assess LLMs' capacity to maintain a stable worldview across five domains: science, history, geography, biology, and mathematics.

Result: The study shows that most LLMs fail to uphold a consistent worldview, with some newer models demonstrating improved stability yet ultimately succumbing to conversational pressure and failing to maintain key commitments.

Conclusion: While LLMs exhibit advancements in argumentative skills, they do not possess the core commitments characteristic of human-level cognition.
...
### [221] [A Universal Reproducing Kernel Hilbert Space from Polynomial Alignment and IMQ Distance](https://arxiv.org/abs/2605.03262)
*Taha Bouhsine*

Main category: cs.LG

TL;DR: The paper introduces the Yat kernel, a rational kernel that offers advantages such as positive definiteness and universality, while enabling specific learning representations through finite expansions in a Reproducing Kernel Hilbert Space (RKHS).


<details>
  <summary>Details</summary>
Motivation: To present a new kernel function that provides enhanced properties such as positive definiteness and universality over existing kernels, facilitating improved learning representations.

Method: The Yat kernel is defined mathematically and its properties are demonstrated, including positive semi-definiteness (PSD), universality, and generalization bounds. The relationship to existing kernels like inverse-multiquadric (IMQ) is examined.

Result: The Yat kernel exhibits superior performance characteristics, including strict positive definiteness on compact domains, and the ability to recover IMQ representations through finite learned expansions, with explicit bounds.

Conclusion: The introduction of the Yat kernel establishes a powerful tool for machine learning applications, leveraging its distinctive properties to improve the representation of learning tasks in RKHS.
...
### [222] [Partially Observed Structural Causal Models](https://arxiv.org/abs/2605.03268)
*Turan Orujlu,Jordan Matelsky,Martin V. Butz,Charley M. Wu,Konrad P. Kording*

Main category: cs.LG

TL;DR: Introduction of Partially Observed Structural Causal Models (POSCMs) that extend SCMs for causal systems with latent contexts affecting both interaction structures and mechanisms.


<details>
  <summary>Details</summary>
Motivation: To formalize causal systems with latent variables influencing interactions and outcomes, addressing limitations in traditional structural causal models.

Method: Development of POSCMs with an intervention hierarchy and application of Kolmogorov-Arnold-Sprecher edge-functional decomposition for surgical edge interventions; identifiability theory established for intervention families.

Result: Validation through a virtual human retina simulator, revealing non-identifiability under latent contexts and recovering synaptic relationships with targeted interventions, affirming the identifiability theory.

Conclusion: POSCMs enhance understanding of causal structures in realistic scenarios, integrating latent contexts to better mirror complex causal systems.
...
### [223] [RFPrompt: Prompt-Based Expert Adaptation of the Large Wireless Model for Modulation Classification](https://arxiv.org/abs/2605.03279)
*Md Raihan Uddin,Tolunay Seyfi,Fatemeh Afghah*

Main category: cs.LG

TL;DR: This paper presents RFPrompt, a prompt-based adaptation framework for automatic modulation classification in wireless foundation models, improving robustness and efficiency under distribution shifts.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of robustness in automatic modulation classification due to distribution shifts caused by various real-world factors, whilst efficiently adapting wireless foundation models to new tasks without losing learned structures.

Method: Introduced RFPrompt, a parameter-efficient approach utilizing learnable deep prompt tokens while keeping the pre-trained backbone frozen, allowing for task-specific adaptation with minimal additional parameters.

Result: RFPrompt consistently enhances robustness to distribution shifts and limited supervision, particularly effective in real-world over-the-air IQ data, compared to other methods.

Conclusion: Prompt learning is a practical and effective strategy for adapting wireless foundation models to complex RF environments, demonstrating strong parameter efficiency and improved performance.
...
### [224] [Stable Multimodal Graph Unlearning via Feature-Dimension Aware Quantile Selection](https://arxiv.org/abs/2605.03303)
*Jingjing Zhou,Yongshuai Yang,Qing Qing,Ziqi Xu,Xikun Zhang,Renqiang Luo,Ivan Lee,Feng Xia*

Main category: cs.LG

TL;DR: FDQ is a novel framework that improves graph unlearning by using adaptive quantile thresholds for multimodal graphs to enhance utility preservation and privacy.


<details>
  <summary>Details</summary>
Motivation: To address the shortcomings of existing unlearning strategies that apply uniform parameter selection across all GNN layers, particularly in the context of high-dimensional multimodal graphs.

Method: FDQ employs a Feature-Dimension Aware Quantile framework that identifies sensitive high-dimensional layers and applies conservative quantile thresholds for suppression while maintaining the importance estimation mechanism.

Result: Experimental results on Ele-Fashion and Goodreads-NC validate that FDQ achieves strong utility preservation and effective forgetting against membership inference attacks.

Conclusion: FDQ provides a robust and principled approach for privacy-conscious unlearning in complex multimodal graph systems, enhancing both privacy and learning utility.
...
### [225] [Task Vector Geometry Underlies Dual Modes of Task Inference in Transformers](https://arxiv.org/abs/2605.03780)
*Hao Yan,Haolin Yang,Yiqiao Zhong*

Main category: cs.LG

TL;DR: This paper investigates the relationship between task-vector geometry, training distributions, and generalization in transformers, revealing that in-distribution and out-of-distribution behaviors can coexist within the same model.


<details>
  <summary>Details</summary>
Motivation: To address gaps in understanding the connection between internal representations of transformers and their external behaviors, particularly regarding how geometry shapes task responses and generalization.

Method: The authors conducted experiments using small transformers trained from scratch on synthetic sequence distributions to mathematically characterize the relationship between task vectors and model behavior.

Result: The study found that in-distribution behavior relies on Bayesian task retrieval through convex combinations of learned task vectors, while out-of-distribution behavior corresponds to extrapolative learning with representations in a nearly orthogonal subspace.

Conclusion: The findings establish a close relationship between task-vector geometry, the training distribution, and generalization behaviors in transformers.
...
### [226] [Will the Carbon Border Adjustment Mechanism Impact European Electricity Prices? A GNN-Based Network Analysis](https://arxiv.org/abs/2605.03304)
*Jiachen Shen,Jian Shi,Dan Wang,Han Zhu*

Main category: cs.LG

TL;DR: This paper develops a spatio-temporal Graph Neural Network framework to analyze the impact of the EU's Carbon Border Adjustment Mechanism (CBAM) on electricity prices and carbon intensity across eight European countries, revealing significant market transformations and competitive advantages for low-carbon countries.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of traditional analyses on the cross-border spillover effects of the CBAM on the European electricity market.

Method: Development of a spatio-temporal Graph Neural Network framework to model the impacts of CBAM on electricity prices and carbon intensity.

Result: CBAM is not a uniform tax; it transforms the market and creates structural differences, benefiting low-carbon countries like France and Switzerland while imposing costs on high-carbon countries like Poland.

Conclusion: The primary driver of these changes is a fundamental shift in the market's merit order.
...
### [227] [Distributed Learning with Adversarial Gradient Perturbations](https://arxiv.org/abs/2605.03313)
*Nawapon Sangsiri,Yufei Tao*

Main category: cs.LG

TL;DR: The paper explores the impact of adversarial gradient perturbation in distributed learning and establishes parameters for sub-optimality gaps and query complexity.


<details>
  <summary>Details</summary>
Motivation: To address privacy concerns in distributed learning which lead to clients intentionally altering gradient information.

Method: The study analyzes learning convex and $L$-smooth functions under the influence of adversarial perturbations, aiming to determine the smallest possible sub-optimality gap and the number of queries needed to ensure a specific gap is met.

Result: The researchers establish tight feasibility thresholds for the sub-optimality gap and develop algorithms that can reach these thresholds with guarantees regarding query complexity.

Conclusion: The paper provides significant insights into the balance between privacy and optimization efficiency in distributed learning contexts.
...
### [228] [DGPO: Distribution Guided Policy Optimization for Fine Grained Credit Assignment](https://arxiv.org/abs/2605.03327)
*Hongbo Jin,Rongpeng Zhu,Zhongjing Du,Xu Jiang,Jingqi Tian,Qiaoman Zhang,Jiayu Ding*

Main category: cs.LG

TL;DR: The paper introduces Distribution Guided Policy Optimization to improve reinforcement learning in large language models by addressing limitations in existing methods.


<details>
  <summary>Details</summary>
Motivation: Current reinforcement learning algorithms struggle with precise credit assignment and suffer from instability due to the standard Kullback Leibler divergence penalty.

Method: The proposed method uses a critic free framework that treats distribution deviation as a guiding signal instead of a penalty.

Result: This approach mitigates issues of gradient instability and promotes the exploration of new reasoning trajectories.

Conclusion: Distribution Guided Policy Optimization enhances the performance of large language models in complex reasoning tasks.
...
### [229] [LLM-ADAM: A Generalizable LLM Agent Framework for Pre-Print Anomaly Detection in Additive Manufacturing](https://arxiv.org/abs/2605.03328)
*Ahmadreza Eslaminia,Chuhan Cai,Cameron Smith,Ruo-Syuan Mei,Shichen Li,Rajiv Malhotra,Klara Nahrstedt,Chenhui Shao*

Main category: cs.LG

TL;DR: This paper introduces LLM-ADAM, a framework for pre-print anomaly detection in additive manufacturing G-code, achieving 87.5% accuracy by structuring the detection process into three roles.


<details>
  <summary>Details</summary>
Motivation: To address the accessibility of AM and the associated risks of inexperienced users making harmful settings in G-code, leading to potential print failures and wasted resources.

Method: LLM-ADAM utilizes three roles: Extractor-LLM for mapping G-code to parameters, Reference-LLM for aligning operational ranges, and Judge-LLM for classifying defects versus anomalies.

Result: The framework demonstrated 87.5% accuracy in detecting anomalies across a dataset of 200 G-code samples, outperforming the best single-LLM baseline by a significant margin.

Conclusion: Structured decomposition of tasks provides a superior approach to anomaly detection in AM, focusing on process layout rather than solely on the capabilities of LLM technology.
...
### [230] [Toward Structural Multimodal Representations: Specialization, Selection, and Sparsification via Mixture-of-Experts](https://arxiv.org/abs/2605.03348)
*Hahyeon Choi,Nojun Kwak*

Main category: cs.LG

TL;DR: The paper introduces the S3 framework for multimodal learning, focusing on specialization, selection, and sparsification to optimize task-specific performance.


<details>
  <summary>Details</summary>
Motivation: The need for an efficient approach to multimodal learning that moves beyond fixed embeddings to enhance task adaptability and representation utility.

Method: S3 decomposes multimodal inputs into specialized semantic experts, selectively routes them according to task needs, and prunes ineffective pathways for efficiency.

Result: S3 shows improved accuracy across four benchmarks and reveals a reverse U-shaped relationship between sparsity and performance, achieving optimal results at moderate sparsity.

Conclusion: Structuring multimodal representations as selectable components is a promising alternative to traditional contrastive learning methods.
...
TL;DR: This paper explores Imitation Learning in Mean Field Games with common noise, emphasizing population-aware policies for agent behavior. It evaluates Behavioral Cloning and Adversarial divergence as learning proxies, establishing error bounds for policy performance relative to experts. A numerical framework and experiments demonstrate the necessity of accounting for noise in policy learning.


<details>
  <summary>Details</summary>
Motivation: To improve the modeling of agent behavior in Mean Field Games under stochastic conditions, particularly focusing on the necessity for population-aware policies.

Method: The paper formulates learning objectives, examines Behavioral Cloning and Adversarial divergence as imitation proxies, and establishes finite-sample error bounds. A numerical framework is proposed using generalized Fictitious Play and Deep Learning to compute policies.

Result: The results indicate that minimizing the chosen imitation proxies effectively controls policy exploitability and performance gaps. Experiments reveal that standard population-unaware policies are inadequate for capturing equilibrium dynamics.

Conclusion: Learning population-aware policies is essential in Mean Field Games to effectively respond to stochastic elements and avoid misleading outcomes.
...
### [232] [Transformers with Selective Access to Early Representations](https://arxiv.org/abs/2605.03953)
*Skye Gunasekaran,Téa Wright,Rui-Jie Zhu,Jason Eshraghian*

Main category: cs.LG

TL;DR: SATFormer introduces a novel mechanism for selective access to early representations in Transformers, improving performance while maintaining efficiency.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of recovering low-level features in Transformers as layers deepen, while exploring more effective access to early representations.

Method: The SATFormer uses a context-dependent gate to control access to first-layer value pathways, treating early representation reuse as a retrieval problem.

Result: SATFormer consistently outperforms static value-residual and Transformer baselines in validation loss and zero-shot accuracy across various model sizes, especially on retrieval-intensive benchmarks.

Conclusion: SATFormer demonstrates that selective reuse of early representations leads to better performance without significant increases in memory cost or throughput.
...
### [233] [Fully Automatic Trace Gas Plume Detection](https://arxiv.org/abs/2605.03372)
...
TL;DR: The paper presents an automated framework for detecting trace gas point sources using machine learning and spectroscopic fitting, applied to EMIT imaging spectrometer data.


<details>
  <summary>Details</summary>
Motivation: To handle the increased data volumes from future imaging spectrometers and automate the detection of trace gas sources without human intervention.

Method: A combination of machine learning-based morphological analysis and physics-based spectroscopic fitting, operating in 'daily digest' and retrospective analysis modes.

Result: The system successfully detects a significant portion of large plumes with few false positives and identifies that at least 25% of plumes were previously missed by human review.

Conclusion: The approach not only detects known methane sources but also expands to NH3, NO2, and the first observations of CO plumes in EMIT data.
...
### [234] [Learning Dynamics of Zeroth-Order Optimization: A Kernel Perspective](https://arxiv.org/abs/2605.03373)
*Zhe Li,Bicheng Ying,Zidong Liu,Haibo Yang*

Main category: cs.LG

TL;DR: This paper explains why zeroth-order (ZO) optimization methods, typically hindered by high dimensions, effectively fine-tune large language models (LLMs) by analyzing their learning dynamics via the empirical Neural Tangent Kernel (eNTK).


<details>
  <summary>Details</summary>
Motivation: To address the paradox of ZO algorithms performing well on LLMs despite theoretical dimension-dependent slowdowns.

Method: Deriving the one-step learning dynamics of ZO stochastic gradient descent (SGD) and analyzing the eNTK associated with it.

Result: The eNTK reveals that its elements correspond to inner products of neural tangent vectors in a low-dimensional subspace, influenced by the number of perturbations as per the Johnson-Lindenstrauss Lemma.

Conclusion: The approximation error is based on model output size, not parameter dimension, thus providing a theoretical basis for ZO methods' efficacy in LLM fine-tuning.
...
### [235] [GRAFT: Auditing Graph Neural Networks via Global Feature Attribution](https://arxiv.org/abs/2605.03377)
*Rishi Raj Sahoo,Subhankar Mishra*

Main category: cs.LG

TL;DR: GRAFT is a novel framework for explaining Graph Neural Networks (GNNs) by identifying class-level feature importance profiles and generating natural language rules, enhancing interpretability at the input attribute level.


<details>
  <summary>Details</summary>
Motivation: To improve the interpretability of Graph Neural Networks by explaining model behavior globally at the level of input node attributes, which is currently lacking in existing global GNN explainers.

Method: GRAFT utilizes diversity-guided exemplar selection, Integrated Gradients for attribution, and aggregation to create a comprehensive view of feature influence for each class. It also transforms this information into natural language rules via a self-refining large language model.

Result: GRAFT effectively captures important model-relevant features across different datasets and architectures, supports bias analysis, and facilitates feature-efficient transfer learning. A structured human evaluation also indicates the interpretability of the generated rules.

Conclusion: GRAFT provides a practical method for analyzing feature-level behavior in GNNs, linking quantitative analysis with interpretability through understandable explanations.
...
### [236] [Local Truncation Error-Guided Neural ODEs for Large Scale Traffic Forecasting](https://arxiv.org/abs/2605.03386)
*Xiao Zhang,Yafei Li,Ruixiang Wang,Wei Wei,Shuo He,Mingliang Xu*

Main category: cs.LG

TL;DR: The paper presents Local Truncation Error-Guided Neural ODEs (LTE-ODE) to address the limitations of Neural ODEs in spatiotemporal forecasting, particularly in handling abrupt anomalies in physical systems like traffic networks.


<details>
  <summary>Details</summary>
Motivation: Existing Neural ODEs suffer from over-smoothing due to Lipschitz continuity when addressing transient anomalies, necessitating a new approach to maintain model sensitivity.

Method: The LTE-ODE methodology repurposes Local Truncation Error as an inductive bias, using it to create a dynamic spatial attention mask that adapts ODE evolution and incorporates discrete compensation at anomaly points.

Result: LTE-ODE outperforms existing models on large-scale benchmarks and shows remarkable resilience to non-linear fluctuations, along with flexible adaptability to hardware constraints.

Conclusion: By innovatively utilizing Local Truncation Error, LTE-ODE preserves accuracy in stable regions while effectively managing shocks, marking a significant advancement in Neural ODE methodologies.
...
### [237] [Structured Analytic Coherent Point Drift for Non-Rigid Point Set Registration](https://arxiv.org/abs/2605.00934)
*Wei Feng,Haiyong Zheng*

Main category: cs.LG

TL;DR: Analytic-CPD enhances point set registration by replacing traditional methods with a structured analytic mapping estimator for improved efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: To improve non-rigid point set registration, addressing the limitations of existing methods, particularly in large-deformation scenarios.

Method: Analytic-CPD uses a structured analytic mapping instead of a Gaussian-kernel displacement field, with barycentric condensation of posterior probabilities to form a weighted analytic fitting problem.

Result: Experiments demonstrate that Analytic-CPD results in lower final errors and faster convergence than standard CPD, particularly in large-deformation contexts.

Conclusion: The method offers a compact, interpretable alternative to kernel-based non-rigid registration, leveraging probabilistic correspondences and structured mappings.
...
### [238] [PODiff: Latent Diffusion in Proper Orthogonal Decomposition Space for Scientific Super-Resolution](https://arxiv.org/abs/2605.03399)
*Onkar Jadhav,Tim French,Matthew Rayson,Nicole L. Jones*

Main category: cs.LG

TL;DR: PODiff is an efficient probabilistic super-resolution framework that operates in a POD coefficient space rather than pixel space, enhancing computational efficiency and uncertainty estimation.


<details>
  <summary>Details</summary>
Motivation: To reduce the computational burden of probabilistic super-resolution in high-dimensional fields by leveraging Proper Orthogonal Decomposition (POD).

Method: PODiff uses a structured conditional generative framework for diffusion in POD coefficient space, maintaining a variance-ordered latent geometry for interpretability and efficiency.

Result: PODiff demonstrates accuracy in reconstruction similar to traditional diffusion methods while consuming less memory and providing better uncertainty estimates than existing baselines.

Conclusion: PODiff effectively balances reconstruction accuracy and computational efficiency, making it a valuable tool for applications like sea surface temperature downscaling.
...
### [239] [Discovering Reinforcement Learning Interfaces with Large Language Models](https://arxiv.org/abs/2605.03408)
*Akshat Singh Jaswal,Ashish Baghel,Paras Chopra*

Main category: cs.LG

TL;DR: LIMEN is a novel framework for automatic discovery of reinforcement learning interfaces, integrating observation mappings and reward functions through evolutionary methods and LLM guidance, reducing manual engineering efforts significantly.


<details>
  <summary>Details</summary>
Motivation: The motivation is to alleviate the manual effort required to construct environment interfaces for reinforcement learning by automating the design of both observations and reward functions.

Method: LIMEN employs an evolutionary framework guided by large language models to generate and iteratively refine task interfaces as executable programs based on feedback from policy training.

Result: The study shows that the joint evolution of observations and rewards leads to successful task interfaces across various domains, outperforming isolated optimization of these components, which fails in at least one evaluated domain.

Conclusion: The findings indicate that automated construction of RL interfaces from raw states is highly effective, and that optimizing observation and reward components together yields better performance compared to optimizing them independently.
...
### [240] [Learning to Theorize the World from Observation](https://arxiv.org/abs/2605.03413)
*Doojin Baek,Gyubin Lee,Junyeob Baek,Hosung Lee,Sungjin Ahn*

Main category: cs.LG

TL;DR: The paper presents Learning-to-Theorize, a paradigm for inferring theories from observations using the Neural Theorizer (NEO), advancing the understanding of world cognition through executable programs.


<details>
  <summary>Details</summary>
Motivation: The study aims to redefine understanding by contrasting traditional prediction models with a theory-building approach inspired by developmental cognitive science.

Method: The authors propose a learning paradigm (Learning-to-Theorize) and develop the Neural Theorizer (NEO) model, which represents theories as executable programs derived from raw observations.

Result: NEO effectively induces latent programs that allow for explanation-driven generalization, enabling the model to explain observations based on the generated programs.

Conclusion: This approach enhances our comprehension of observations by linking them to underlying theories, aligning with how humans construct understanding early in cognitive development.
...
### [241] [FIBER: A Differentially Private Optimizer with Filter-Aware Innovation Bias Correction](https://arxiv.org/abs/2605.03425)
*Duc Dm,Thao Do,Minh Son Hoang,Anh Le Duc Tran,Daeyoung Kim,Huy Nguyen*

Main category: cs.LG

TL;DR: FiBeR is a differentially private optimizer that addresses noise interaction issues in adaptive methods by introducing a novel second-moment calibration and filtering approach, showing improved performance in benchmarks.


<details>
  <summary>Details</summary>
Motivation: To enhance the effectiveness of DP training with adaptive optimizers, mitigating the adverse effects of noise filtering on gradient statistics.

Method: FiBeR employs denoising in innovation space, decouples observation geometry for tuning, and uses filter-aware second-moment calibration.

Result: FiBeR shows significant performance improvements on vision and language benchmarks compared to current state-of-the-art DP optimizers.

Conclusion: FiBeR offers a robust solution for optimizing DP training, outperforming existing methods while maintaining privacy constraints.
...
### [242] [DynaTab: Dynamic Feature Ordering as Neural Rewiring for High-Dimensional Tabular Data](https://arxiv.org/abs/2605.03430)
*Al Zadid Sultan Bin Habib,Gianfranco Doretto,Donald A. Adjeroh*

Main category: cs.LG

TL;DR: DynaTab is a dynamic feature ordering architecture designed for high-dimensional tabular data that enhances deep learning model performance through a novel feature permutation strategy.


<details>
  <summary>Details</summary>
Motivation: High-dimensional tabular data lacks inherent feature order, which limits the effectiveness of permutation-sensitive deep learning models.

Method: DynaTab utilizes a neural rewiring algorithm to dynamically reorder features based on a lightweight criterion predicting the benefits of permutation, employing importance-based gating and masked attention layers compatible with sequence-sensitive models.

Result: DynaTab demonstrates statistically significant improvements over 45 state-of-the-art models across 36 real-world datasets, especially in high-dimensional contexts.

Conclusion: DynaTab represents a promising new approach for enhancing deep learning in high-dimensional tabular data.
...
### [243] [Quantum Hierarchical Reinforcement Learning via Variational Quantum Circuits](https://arxiv.org/abs/2605.03434)
*Yu-Ting Lee,Samuel Yen-Chi Chen,Fu-Chieh Chang*

Main category: cs.LG

TL;DR: This study develops a hybrid hierarchical agent that integrates quantum computing into reinforcement learning, demonstrating enhanced performance and parameter efficiency.


<details>
  <summary>Details</summary>
Motivation: To explore the potential advantages of quantum computations in hierarchical reinforcement learning settings, particularly in terms of efficiency and efficacy.

Method: A hybrid agent based on the option-critic architecture was created, integrating quantum circuits into various components like feature extractors and option-value functions.

Result: The hybrid agent outperformed classical models in standard benchmarking environments while reducing the number of trainable parameters by up to 66% demonstrating efficiency benefits.

Conclusion: The research outlines design principles for creating parameter-efficient hybrid hierarchical agents and highlights an architectural bottleneck affecting quantum performance.
...
### [244] [From Code to Prediction: Fine-Tuning LLMs for Neural Network Performance Classification in NNGPT](https://arxiv.org/abs/2605.03686)
*Mahmoud Hanouneh,Radu Timofte,Dmitry Ignatov*

Main category: cs.LG

TL;DR: This study evaluates the ability of fine-tuned Large Language Models (LLMs) to predict neural network performance across datasets, incorporating various prompt types in the NNGPT framework.


<details>
  <summary>Details</summary>
Motivation: To assess whether LLMs can reason about neural network performance across different datasets beyond just generating code.

Method: A classification task was integrated into the NNGPT framework, using a fine-tuned LLM to predict accuracy performance between two image classification datasets based on neural network architecture.

Result: Using different prompt configurations, the code-only prompt achieved 80% peak accuracy, while the metadata-enriched prompt peaked at 70%.

Conclusion: The study indicates that LLMs can predict the suitability of neural network architecture for different datasets, showcasing the discriminative power of architecture source code over dataset metadata.
...
### [245] [Bandits on graphs and structures](https://arxiv.org/abs/2605.03493)
*Michal Valko*

Main category: cs.LG

TL;DR: This thesis investigates structural properties of sequential problems, particularly focusing on graph representations and large action spaces in bandit settings.


<details>
  <summary>Details</summary>
Motivation: To enhance the practical applicability of solutions to sequential problems by investigating their structural characteristics.

Method: Analyzes various settings related to graph bandits and large structured domains, including reward smoothness, side observations, and different types of bandit problems like kernel bandits and polymatroid bandits.

Result: Explores complex scenarios in bandit theory, including smoothness of rewards for spectral bandits and strategies for function optimization in structured environments.

Conclusion: The thesis compiles the author's contributions to understanding and improving graph and structured bandits.
...
### [246] [Adaptive graph-based algorithms for conditional anomaly detection and semi-supervised learning](https://arxiv.org/abs/2605.03495)
*Michal Valko*

Main category: cs.LG

TL;DR: This paper presents novel graph-based methods for semi-supervised learning and conditional anomaly detection, emphasizing efficient label propagation and online algorithms for large data sets.


<details>
  <summary>Details</summary>
Motivation: To address computation and storage challenges in graph-based semi-supervised learning and to improve anomaly detection in clinical settings.

Method: Developed a fast approximate online algorithm for harmonic solutions on data similarity graphs, incorporating local representative points and regularization techniques for stability.

Result: Empirical and theoretical evidence shows that the proposed methods effectively minimize distortion and enhance stability, along with successful anomaly detection in hospital patient-management actions.

Conclusion: Novel nonparametric graph-based methods for anomaly detection improve upon standard frameworks and account for challenges related to data structure, validated through expert evaluations.
...
### [247] [Learning Generalizable Action Representations via Pre-training AEMG](https://arxiv.org/abs/2605.03462)
*Zhenghao Huang,Huilin Yao,Kaikai Wang,Lin Shu*

Main category: cs.LG

TL;DR: AEMG is a self-supervised learning framework that improves EMG signal interpretation for human-computer interaction by treating EMG data as a physiological language.


<details>
  <summary>Details</summary>
Motivation: To address the limitations in generalization capabilities of EMG due to data heterogeneity and lack of a unified framework.

Method: AEMG uses a Neuromuscular Contraction Tokenizer to convert muscle contractions into a structured, linguistic format and compiles a large cross-device EMG vocabulary.

Result: AEMG enhances zero-shot LOSO accuracy by 5.79-9.25% compared to existing methods and achieves over 90% performance with minimal user data.

Conclusion: This work establishes a foundation for a versatile EMG model, conceptualizing EMG as a cross-device physiological language.
...
### [248] [Bandits attack function optimization](https://arxiv.org/abs/2605.03496)
*Philippe Preux,Rémi Munos,Michal Valko*

Main category: cs.LG

TL;DR: SOO is a deterministic optimization algorithm that balances exploration and exploitation under budget constraints, ensuring solution guarantees and efficiency.


<details>
  <summary>Details</summary>
Motivation: To address function optimization as a sequential decision-making problem constrained by a limited number of evaluations.

Method: SOO, inspired by multi-armed bandit principles, partitions the domain to balance exploration and exploitation in the search space.

Result: Empirical evaluations demonstrate SOO's effectiveness on CEC'2014 optimization benchmarks, highlighting its numerical efficiency.

Conclusion: SOO provides a reliable approach to optimization, offering guarantees on solutions while efficiently managing evaluation budgets.
...
### [249] [GRIFDIR: Graph Resolution-Invariant FEM Diffusion Models in Function Spaces over Irregular Domains](https://arxiv.org/abs/2605.03497)
*James Rowbottom,Elizabeth L. Baker,Nick Huang,Ben Adcock,Carola-Bibiane Schönlieb,Alexander Denker*

Main category: cs.LG

TL;DR: This paper introduces a novel function-space diffusion model architecture that effectively handles unstructured meshes and complex geometries, overcoming limitations of existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance function-valued data modeling in infinite-dimensional spaces by addressing deficiencies in current implementations, particularly regarding resolution invariance and adaptability to complex geometries.

Method: The paper proposes a new architecture that utilizes generalized graph convolutional kernels represented as finite element functions to naturally manage unstructured meshes and intricate geometrical configurations.

Result: The experimental results demonstrate the efficacy of the architecture through successful unconditional and conditional sampling on various challenging geometries, maintaining resolution invariance and capturing functional distributions with high fidelity.

Conclusion: The proposed method effectively advances the modeling of function-valued data by accommodating complex geometries, thereby achieving improved performance over existing models.
...
### [250] [Meta-Inverse Physics-Informed Neural Networks for High-Dimensional Ordinary Differential Equations](https://arxiv.org/abs/2605.03511)
*Zhao Wei,Kenneth Hor Cheng Koh,Sheng Yuan Chin,James Chun Yip Chan,Chin Chun Ooi,Yew-Soon Ong*

Main category: cs.LG

TL;DR: The paper presents a meta-inverse physics-informed neural network (MI-PINN) designed to improve the inference of unknown parameters in high-dimensional dynamical systems, addressing limitations of traditional PINNs in optimization and generalization.


<details>
  <summary>Details</summary>
Motivation: The need arises from challenges in solving inverse problems in high-dimensional ODEs, particularly when dealing with sparse observations and partially characterized systems, making traditional methods insufficient.

Method: MI-PINN reformulates inverse modeling into a two-stage meta-learning problem, where a physics-aware representation is learned across multiple tasks, followed by optimization of task-specific unknowns with the representation kept fixed.

Result: The proposed MI-PINN shows significant improvements in sample efficiency and accurate inference, demonstrated through its application to complex pharmacokinetic models with coupled ODEs, successfully recovering masked parameters and reconstructing missing terms.

Conclusion: MI-PINN effectively addresses optimization challenges in traditional PINNs while enhancing parameter recovery and model accuracy in the context of limited clinical observations.
...
### [251] [A Hierarchical Sampling Framework for bounding the Generalization Error of Federated Learning](https://arxiv.org/abs/2605.03499)
*Dario Filatrella,Ragnar Thobaben,Mikael Skoglund*

Main category: cs.LG

TL;DR: The paper presents expected generalization bounds in Hierarchical Federated Learning using Wasserstein distance, introducing a hierarchical sampling framework and deriving bounds under Lipschitz loss assumptions, while demonstrating compatibility with Differential Privacy.


<details>
  <summary>Details</summary>
Motivation: To establish generalization bounds for Hierarchical Federated Learning by considering the hierarchical structure of client datasets and their dependencies, thus addressing limitations in existing methods.

Method: A hierarchical sampling framework is proposed based on a multi-layered tree structure. Generalization bounds are derived using Wasserstein distance with the Lipschitz condition on the loss function, and a supersample construction to assess the sensitivity of the algorithm.

Result: The derived bounds imply existing state-of-the-art conditional mutual information bounds for bounded losses and can integrate Differential Privacy notions to yield algorithmic privacy-based generalization bounds. The tightness of these bounds is validated using the Gaussian Location Model, recovering asymptotic rates for generalization error.

Conclusion: The study provides a comprehensive framework for analyzing generalization in Hierarchical Federated Learning, with bounds that enhance understanding of client dataset interactions and compatibility with privacy considerations.
...
### [252] [Understanding Self-Supervised Learning via Latent Distribution Matching](https://arxiv.org/abs/2605.03517)
*Fabian A Mikulasch,Friedemann Zenke*

Main category: cs.LG

TL;DR: The paper introduces a unifying theoretical framework for self-supervised learning (SSL) through latent distribution matching (LDM), consolidating various SSL methods and deriving a new Bayesian filtering model for high-dimensional timeseries.


<details>
  <summary>Details</summary>
Motivation: The lack of a theoretical framework in SSL hinders the understanding and development of new methods.

Method: The authors propose LDM, framing SSL as a process of aligning representations with a latent model while maintaining diversity through entropy maximization.

Result: The paper derives a nonlinear Bayesian filtering model and proves that predictive LDM leads to identifiable latent representations under certain assumptions.

Conclusion: The LDM approach clarifies existing SSL methods' assumptions and offers a structured way to guide future SSL research.
...
### [253] [Enhance the after-discharge mortality rate prediction via learning from the medical notes](https://arxiv.org/abs/2605.03560)
*Zijiang Yang*

Main category: cs.LG

TL;DR: The paper presents a Deep Neural Network model that utilizes unstructured Electronic Health Records (EHR) data to improve mortality rate prediction, achieving higher accuracy than traditional models.


<details>
  <summary>Details</summary>
Motivation: To enhance the prediction accuracy of after-discharge mortality rates using unstructured medical notes from Electronic Health Records, which often suffer from low quality.

Method: A Deep Neural Network model incorporating a 'pooling' mechanism to process medical notes for mortality prediction tasks.

Result: The DNN model's AUC-ROC improves by 2% to 14% compared to traditional models across various time frames post-discharge, indicating superior predictive capability.

Conclusion: The proposed model effectively learns from the most informative medical notes, reveals key relationships between keywords and patient severity, and shows consistent results with previous research.
...
### [254] [PerFlow: Physics-Embedded Rectified Flow for Efficient Reconstruction and Uncertainty Quantification of Spatiotemporal Dynamics](https://arxiv.org/abs/2605.03548)
*Hao Zhou,Rui Zhang,Han Wan,Hao Sun*

Main category: cs.LG

TL;DR: PerFlow is a proposed generative model for reconstructing PDE-governed fields from sparse measurements efficiently and accurately, overcoming limitations of existing deterministic surrogates and generative approaches.


<details>
  <summary>Details</summary>
Motivation: The need for effective reconstruction of PDE-governed fields from sparse data due to ill-posed problems and limitations in existing methods regarding uncertainty quantification and data consistency.

Method: PerFlow uses a Physics-embedded rectified Flow that decouples observation conditioning from physics enforcement, implementing guidance-free conditioning while embedding hard physics through constraint-preserving projections.

Result: PerFlow achieves competitive reconstruction accuracy and maintains physics consistency while enabling efficient conditional sampling and significantly faster inference compared to traditional methods.

Conclusion: The proposed model offers a promising solution for sparse reconstruction and uncertainty quantification in spatiotemporal dynamics, demonstrating efficiency and accuracy improvements over existing techniques.
...
### [255] [HeadQ: Model-Visible Distortion and Score-Space Correction for KV-Cache Quantization](https://arxiv.org/abs/2605.03562)
*Jorge L. Ruiz Williams*

Main category: cs.LG

TL;DR: This paper presents HeadQ, a new method for optimizing KV-cache quantizers by focusing on model-visible error rather than traditional storage-space metrics, leading to significant improvements across multiple language models.


<details>
  <summary>Details</summary>
Motivation: The goal is to improve KV-cache quantization by addressing persistent cache error through model-visible coordinates, rather than just optimizing for storage-space reconstruction.

Method: HeadQ uses a low-rank residual side code for keys and applies an additive logit correction, while values employ an $A^2$-weighted token-distortion surrogate for readout, optimizing error in terms of Fisher/score-space rather than raw key MSE.

Result: HeadQ demonstrates significant improvements in attention KL prediction across six models, outperforming storage-MSE alternatives, and effectively reducing perplexity in experiments with dense values.

Conclusion: The study reveals that by focusing on model-visible error and implementing HeadQ, significant reductions in excess perplexity can be achieved, validating the proposed approach in performance improvement.
...
### [256] [Disentangling Shared and Task-Specific Representations from Multi-Modal Clinical Data](https://arxiv.org/abs/2605.03570)
*He Lyu,Huolin Zeng,Junren Wang,Huazhen Yang,Linchao He,Yong Chen,Zhirui Li,Andreas Maier,Siming Bayer,Huan Song*

Main category: cs.LG

TL;DR: The paper proposes a multi-task learning framework using Orthogonal Task Decomposition (OrthTD) with a unified Transformer to improve the prediction of multiple outcomes from multimodal clinical data, achieving notable performance gains, particularly for rare events.


<details>
  <summary>Details</summary>
Motivation: To improve efficiency in predicting multiple clinical outcomes by addressing the challenge of balancing shared information and outcome-specific modeling in multi-task learning.

Method: A multi-task framework utilizing a unified Transformer for multimodal fusion, enhanced by Orthogonal Task Decomposition (OrthTD) to separate patient representations into shared and specific subspaces while enforcing geometric orthogonality.

Result: OrthTD demonstrated an average AUC of 87.5% and AUPRC of 37.2% in predicting four outcomes for 12,430 surgical patients, outperforming existing methods and showing enhanced performance for rare events.

Conclusion: The results indicate that separating shared and task-specific representations can effectively improve predictions from multimodal clinical data, particularly for rare outcomes.
...
### [257] [Most ReLU Networks Admit Identifiable Parameters](https://arxiv.org/abs/2605.03601)
*Moritz Grillo,Guido Montúfar*

Main category: cs.LG

TL;DR: The paper examines the realization map of deep ReLU networks, revealing that for wide networks, identifiable parameters exist and functional dimensions correlate with parameters minus hidden neurons.


<details>
  <summary>Details</summary>
Motivation: To understand the identification and redundancy of parameters in deep ReLU networks through a new theoretical framework.

Method: The authors introduce weighted polyhedral complexes to analyze hidden redundancies in network parameters and derive results for architectures with input and hidden layers wider than two.

Result: For architectures meeting the criteria, the study finds identifiable parameters and establishes a relationship between functional dimension and network architecture, while also showing redundancies in minimal representations.

Conclusion: The paper concludes with a depth hierarchy demonstrating that certain functions cannot be represented by shallower networks, indicating limitations on the expressive power of networks with fewer layers.
...
### [258] [Flow Matching on Symmetric Spaces](https://arxiv.org/abs/2605.03588)
*Francesco Ruscelli,Ferdinando Zanchetta,Rita Fioresi*

Main category: cs.LG

TL;DR: The paper presents a framework for training flow matching models specifically on Riemannian symmetric spaces, enhancing the process by utilizing the structures of Lie algebras.


<details>
  <summary>Details</summary>
Motivation: To provide an efficient approach to flow matching on Riemannian symmetric spaces, which are commonly used in various applications.

Method: Reformulating flow matching on symmetric spaces as flow matching within a subspace of the Lie algebra of the isometry group, leading to linearization and simplification.

Result: Demonstrates the effectiveness of the proposed framework through application on real Grassmannians, specifically on the quotient of special orthogonal groups.

Conclusion: The new approach significantly simplifies the handling of geodesics in flow matching, making it easier to apply this technique across various symmetric space settings.
...
### [259] [A Few-Step Generative Model on Cumulative Flow Maps](https://arxiv.org/abs/2605.03623)
*Zhiqi Li,Duowen Chen,Yuchen Sun,Bo Zhu*

Main category: cs.LG

TL;DR: Proposes a few-step generative modeling framework using cumulative flow maps for efficient long-range transport in probability space, applicable to various models with minimal adjustments.


<details>
  <summary>Details</summary>
Motivation: To create a unified modeling framework that enhances long-range transport in generative models inspired by physical flow-map techniques.

Method: A cumulative-flow abstraction links local updates to global transitions, enabling a few-step and one-step generation framework applicable to diffusion- and flow-based models.

Result: The framework allows for effective generation across multiple tasks such as image generation and geometric distribution modeling with reduced inference cost.

Conclusion: The approach maintains synthesis quality with minimal changes to model architecture or training objectives.
...
### [260] [Information Plane Analysis of Binary Neural Networks](https://arxiv.org/abs/2605.03636)
*Maximilian Nothnagel,Bernhard C. Geiger*

Main category: cs.LG

TL;DR: This study conducts information plane analysis on binary neural networks to examine mutual information reliability, particularly in relation to sample size and dimensionality, while also exploring the connection between representation compression and generalization performance.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of estimating mutual information in high-dimensional representations during the training of deep neural networks, and to understand the effects of late-stage compression on generalization performance in binary neural networks.

Method: IP analysis was performed on binary neural networks, utilizing a plug-in entropy estimator to assess mutual information while identifying reliable sample size and dimensionality regimes, and training 375 BNNs to observe compression effects.

Result: The study identified reliable regimes for mutual information estimates and found that late-stage compression is common in BNNs; however, compressed representations do not consistently correlate with better generalization performance.

Conclusion: The relationship between compression and generalization is complex and influenced by factors such as task, architecture, and regularization, indicating that late-stage compression does not guarantee improved performance.
...
### [261] [ELAS: Efficient Pre-Training of Low-Rank Large Language Models via 2:4 Activation Sparsity](https://arxiv.org/abs/2605.03667)
*Jiaxi Li,Lu Yin,Li Shen,Jinjin Xu,Yuhui Liu,Wenwu Wang,Shiwei Liu,Xilu Wang*

Main category: cs.LG

TL;DR: ELAS, a novel framework, combines low-rank models with 2:4 activation sparsity, improving training efficiency of large language models by reducing memory usage and maintaining performance.


<details>
  <summary>Details</summary>
Motivation: The need for efficient training of large language models due to high computational demands and memory constraints in existing methods.

Method: The proposed framework applies squared ReLU activation functions and implements 2:4 structured sparsity on activations post-squared ReLU in low-rank models.

Result: ELAS was evaluated on LLaMA models, showing minimal performance degradation while enabling faster training and inference, along with reduced memory usage during large-batch settings.

Conclusion: The results indicate that ELAS successfully enhances the pre-training process of large language models and provides an efficient and effective method for managing activation memory.
...
### [262] [Uni-OPD: Unifying On-Policy Distillation with a Dual-Perspective Recipe](https://arxiv.org/abs/2605.03677)
...
TL;DR: This paper presents Uni-OPD, a unified framework for on-policy distillation that enhances student model training by addressing exploration and supervision challenges.


<details>
  <summary>Details</summary>
Motivation: The need to understand the conditions that facilitate effective on-policy distillation (OPD) due to its empirical successes and the limited understanding of its reliability.

Method: Uni-OPD utilizes dual perspectives: student-focused data balancing strategies to enhance exploration, and teacher-focused outcome-guided margin calibration to ensure consistent supervision.

Result: Extensive experiments across multiple domains demonstrated Uni-OPD's effectiveness, confirming its versatility in both single and multi-teacher distillation scenarios.

Conclusion: Uni-OPD provides a reliable approach to OPD, offering new insights into the factors that contribute to its success.
...
### [263] [Graph Neural Network based Hierarchy-Aware Embeddings of Knowledge Graphs: Applications to Yeast Phenotype Prediction](https://arxiv.org/abs/2605.03690)
*Filip Kronström,Alexander H. Gower,Daniel Brunnsåker,Ievgeniia A. Tiukova,Ross D. King*

Main category: cs.LG

TL;DR: This paper presents a novel method for generating hierarchy-aware embeddings of knowledge graphs using graph neural networks, enhanced by semantic loss from ontologies, leading to improved predictions in biological contexts such as yeast gene deletions.


<details>
  <summary>Details</summary>
Motivation: To enhance the representation of knowledge graphs by incorporating domain-specific semantic information and improve prediction accuracy for biological phenomena.

Method: The authors used graph neural networks with additional semantic loss derived from ontologies to create low-dimensional box embeddings that capture hierarchical information in knowledge graphs.

Result: The method achieves a mean $R^2$ score of 0.377 for predicting cell growth in yeast double gene knockouts and shows generalization to triple gene knockouts, outperforming baseline models.

Conclusion: The use of ontological structures in training enhances predictive performance and aids in generating biological hypotheses, showcasing the potential for guiding experimental biology.
...
### [264] [Distribution-Free Pretraining of Classification Losses via Evolutionary Dynamics](https://arxiv.org/abs/2605.03722)
*Meng Xiang,Yan Pei*

Main category: cs.LG

TL;DR: Proposed EDL learns a flexible classification loss from synthetic data using an evolutionary strategy, outperforming traditional methods in specific tasks.


<details>
  <summary>Details</summary>
Motivation: To create a transfer learning method that develops a classification loss without dependency on real sample data in the pretraining phase, leveraging synthetic data instead.

Method: EDL employs a lightweight neural network to parameterize the classification loss, incorporates a ranking-consistency objective with a focus on penalizing inaccurate predictions more heavily, and optimizes using an evolutionary strategy enhanced by chaotic mutation.

Result: EDL demonstrates competitive or superior accuracy compared to cross-entropy loss when applied to CIFAR-10 using ResNet architectures, with faster convergence observed through chaotic mutation during ablation studies.

Conclusion: The findings suggest that EDL provides an effective and innovative approach for classification tasks, enabling improved performance and convergence in settings where real data is limited.
...
### [265] [Rethinking the Rank Threshold for LoRA Fine-Tuning](https://arxiv.org/abs/2605.03724)
*Juneyoung Park*

Main category: cs.LG

TL;DR: The paper establishes a reduced requirement for the LoRA rank in fine-tuning models under the neural tangent kernel regime, suggesting rank one suffices for binary classification tasks instead of the previously suggested rank of 12.


<details>
  <summary>Details</summary>
Motivation: To address the condition for LoRA rank in fine-tuning models, particularly focusing on binary classification using RoBERTa while considering potential spurious local minima.

Method: The authors propose a new condition for the LoRA rank involving the non-symmetric LoRA manifold dimension, apply the Polyak–Łojasiewicz inequality in cross-entropy contexts, and leverage Rademacher-complexity bounds to establish rank-one variance optimality.

Result: They demonstrate that for binary classification, a rank of 1 is sufficient, while traditional methods suggest a rank of 12; empirical results across various tasks support these findings, although adjustments for multi-class tasks are noted.

Conclusion: The findings indicate that existing rank requirements can be relaxed for binary classification tasks, with implications for future work on multi-class scenarios.
...
### [266] [Vanishing L2 regularization for the softmax Multi Armed Bandit](https://arxiv.org/abs/2605.03752)
*Stefana-Lucia Anita,Gabriel Turinici*

Main category: cs.LG

TL;DR: This paper explores L2 regularized softmax policy gradient methods in Multi Armed Bandit algorithms, proving their theoretical convergence and demonstrating numerical advantages in empirical tests.


<details>
  <summary>Details</summary>
Motivation: To address the lack of a theoretical framework for the convergence of L2 regularized MAB algorithms when the regularization parameter approaches zero.

Method: The paper formulates the L2 regularized softmax policy gradient and proves its theoretical convergence while analyzing the effects of the vanishing regularization parameter.

Result: The authors established that L2 regularized methods converge theoretically, which was previously uncertain, and showed through empirical testing that these methods outperform traditional approaches on benchmark problems.

Conclusion: L2 regularization enhances the numerical performance of softmax policy gradients in MAB problems while maintaining convergence properties.
...
### [267] [GEM-FI: Gated Evidential Mixtures with Fisher Modulation](https://arxiv.org/abs/2605.03750)
*Marco Mustafa Mohammed,Fatemeh Daneshfar,Pietro Liò*

Main category: cs.LG

TL;DR: Gated Evidential Mixtures (GEM) enhance Evidential Deep Learning (EDL) by improving uncertainty estimation and calibration through an energy gating mechanism and multi-modal capabilities while retaining single-pass inference.


<details>
  <summary>Details</summary>
Motivation: EDL struggles with overconfidence and poor calibration, inadequately representing multi-modal epistemic uncertainty, prompting the need for improved models.

Method: GEM employs gated evidential outputs using energy signals, with GEM-CORE for feature-level gating, GEM-MIX for mixtures of evidential heads, and GEM-FI for stabilizing allocations via a Fisher-informed regularizer.

Result: GEM improves calibration and ID/OOD separation compared to DAEDL, notably increasing accuracy and reducing Brier scores while enhancing out-of-distribution detection performance.

Conclusion: GEM demonstrates significant advancements in uncertainty estimation accuracy, calibration, and OOD detection, validating its efficacy in single-pass inference settings.
...
### [268] [Nora: Normalized Orthogonal Row Alignment for Scalable Matrix Optimizer](https://arxiv.org/abs/2605.03769)
*Jinghui Yuan,Jiaxuan Zou,Shuo Wang,Yong Liu,Feiping Nie*

Main category: cs.LG

TL;DR: Nora is a new matrix-based optimizer designed for training Large Language Models (LLMs) that aims to optimize efficiency, stability, and speed simultaneously without incurring high computational costs or compromising stability.


<details>
  <summary>Details</summary>
Motivation: The ongoing challenge in optimizer design for training LLMs involves balancing efficiency, stability, and computational speed without incurring excessive costs or instability that can hinder model training.

Method: Nora stabilizes training by addressing weight norms and angular velocities through row-wise momentum projection, and utilizes the block-diagonal structure of the Transformer Hessian to achieve efficient preconditioning with computational complexity of $	ext{O}(mn)$.

Result: Nora is shown to be a scalable optimizer, confirmed by scaling theorems and preliminary experiments suggesting its effectiveness in large-scale training scenarios.

Conclusion: Nora represents a significant advancement in the design of optimizers for LLMs, demonstrating the feasibility of meeting key requirements efficiently and effectively.
...
### [269] [Graph Convolutional Support Vector Regression for Robust Spatiotemporal Forecasting of Urban Air Pollution](https://arxiv.org/abs/2605.03795)
*Nourin Jahan,Madhurima Panja,Muhammed Navas T,Tanujit Chakraborty*

Main category: cs.LG

TL;DR: This study introduces a Graph Convolutional Support Vector Regression (GCSVR) model to improve urban air quality forecasting by addressing nonlinearity, nonstationarity, and outliers in pollutant data.


<details>
  <summary>Details</summary>
Motivation: Urban air quality forecasting is affected by complex and varying factors like traffic, industries, and weather, making it a difficult task to predict pollutant concentrations accurately.

Method: The GCSVR framework integrates graph convolutional learning for capturing spatial dependencies between monitoring stations with support vector regression to handle nonlinear dynamics and outliers.

Result: Evaluation of GCSVR on air quality data from 55 monitoring stations in Delhi and Mumbai demonstrates improved accuracy and stable performance during different seasons and pollution episodes compared to existing models.

Conclusion: The incorporation of conformal prediction allows GCSVR to provide reliable prediction intervals, enhancing its utility in air quality monitoring and public health applications.
...
### [270] [Realizable Bayes-Consistency for General Metric Losses](https://arxiv.org/abs/2605.03823)
*Dan Tsir Cohen,Steve Hanneke,Aryeh Kontorovich*

Main category: cs.LG

TL;DR: The paper addresses strong universal Bayes-consistency under general metric losses, resolving an open problem related to necessary conditions for hypothesis classes that ensure risk convergence to zero for realizable distributions, introducing a novel combinatorial obstruction concept.


<details>
  <summary>Details</summary>
Motivation: To extend the understanding of strong universal Bayes-consistency beyond classical settings and provide clarity on necessary conditions for hypothesis classes in the context of metric losses.

Method: Analysis involves characterizing conditions for hypothesis classes $ H$ that allow for a distribution-free learning rule to achieve zero risk in realizable cases, using the introduction of an infinite non-decreasing $(γ_k)$-Littlestone tree as a combinatorial obstruction.

Result: Establishes a sharp characterization of when a distribution-free learning rule can achieve Bayes-consistency, contingent on the properties of the hypothesis class and its relation to the introduced Littlestone tree concept.

Conclusion: This work extends classical theory into broader contexts of learning, providing vital insights for future research on learning with general metric losses.
...
### [271] [Complex Equation Learner: Rational Symbolic Regression with Gradient Descent in Complex Domain](https://arxiv.org/abs/2605.03841)
*Sergei Garmaev,Maurice Gauché,Olga Fink*

Main category: cs.LG

TL;DR: Proposed method improves symbolic regression by allowing the use of problematic operators, ensuring stable convergence despite singularities.


<details>
  <summary>Details</summary>
Motivation: Modern methods struggle with operators that cause singularities, limiting the models’ expressiveness and leading to suboptimal hypothesis classes.

Method: A complex weight extension of the Equation Learner is introduced to navigate real-axis degeneracies and allow stable convergence with problematic operators.

Result: The method outperformed existing approaches in symbolic regression benchmarks, successfully recovering singular behaviors in frequency response data.

Conclusion: The proposed approach enhances symbolic regression capabilities, particularly with operations previously avoided due to optimization issues.
...
### [272] [A Domain Incremental Continual Learning Benchmark for ICU Time Series Model Transportability](https://arxiv.org/abs/2605.03832)
*Ryan King,Conrad Krueger,Ethan Veselka,Tianbao Yang,Bobak J. Mortazavi*

Main category: cs.LG

TL;DR: The paper addresses the challenge of generalizing machine learning models used for clinical outcome prediction by proposing a benchmark for assessing model transferability across diverse patient data from different U.S. regions.


<details>
  <summary>Details</summary>
Motivation: To mitigate resource limitations for smaller hospitals in developing their own predictive models, particularly the challenges posed by training models on data from a single hospital, which affects generalizability.

Method: The authors propose a benchmark for evaluating a model's capacity to transfer knowledge across regions and treat the model transfer as a domain incremental learning problem, assessing methods like data replay and Elastic Weight Consolidation (EWC).

Result: The research indicates significant variability in measurement distributions and patient data across U.S. regions, thereby substantiating the need for an effective benchmark in model transfer assessment.

Conclusion: Implementing a robust benchmark allows for more reliable adaptation of machine learning models to different healthcare settings, ultimately improving patient outcome predictions in smaller hospitals.
...
### [273] [Memory-Efficient Continual Learning with CLIP Models](https://arxiv.org/abs/2605.03866)
*Ryan King,Gang Li,Bobak Mortazavi,Tianbao Yang*

Main category: cs.LG

TL;DR: The paper introduces a method to enhance CLIP models' adaptability to new data while preventing knowledge loss, utilizing dynamic class loss reweighting and minimal memory usage.


<details>
  <summary>Details</summary>
Motivation: CLIP models struggle with retaining past knowledge when adapting to new tasks, especially with limited memory buffers, leading to performance drops.

Method: A memory-efficient, distributionally robust technique that dynamically reweights losses for each class during training to minimize catastrophic forgetting.

Result: The proposed method was evaluated in class incremental (CIFAR-100, ImageNet1K) and domain incremental (DomainNet) settings, demonstrating improved adaptability and retention of past knowledge.

Conclusion: The method enables CLIP models to adapt quickly to new tasks while effectively minimizing the risk of catastrophic forgetting, even with limited memory.
...
### [274] [On Adaptivity in Zeroth-Order Optimization](https://arxiv.org/abs/2605.03869)
*Hassan Dbouk,Nidham Gazagnadou,Matthias Reisser,Christos Louizos*

Main category: cs.LG

TL;DR: This paper examines adaptive zeroth-order optimization methods for fine-tuning large language models, revealing the inefficiency of adaptive ZO methods compared to ZO-SGD. It introduces MEAZO, a memory-efficient optimizer with proven theoretical convergence.


<details>
  <summary>Details</summary>
Motivation: The increasing need to fine-tune large language models under memory constraints motivates the investigation of optimization methods that balance performance and memory usage.

Method: The study analyzes existing adaptive zeroth-order methods and proposes MEAZO, which uses a single scalar for global step size adaptation, supported by theoretical convergence guarantees.

Result: MEAZO is shown to match the performance of ZO-Adam while maintaining a lower memory footprint similar to ZO-SGD, and it provides better robustness to varying step sizes in specific optimization contexts.

Conclusion: The findings highlight the limitations of adaptive ZO methods and advocate for MEAZO as a viable optimizer for memory-constrained fine-tuning of large language models.
...
### [275] [Spatiotemporal Convolutions on EEG signal -- A Representation Learning Perspective on Efficient and Explainable EEG Classification with Convolutional Neural Nets](https://arxiv.org/abs/2605.03874)
*Laurits Dixen,Stefan Heinrich,Paolo Burelli*

Main category: cs.LG

TL;DR: The paper investigates the use of 2D spatiotemporal convolutions in EEG signal classification, comparing their efficiency and effectiveness to 1D convolutions and a CNN+transformer hybrid. Results show that 2D convolutions reduce training time without sacrificing performance, revealing distinct representational geometries between model types.


<details>
  <summary>Details</summary>
Motivation: To explore more efficient methods for classifying EEG signals by improving the encoding used in CNN models.

Method: The study compares 1D CNNs, 2D CNNs, and a CNN+transformer hybrid model on low-dimensional and high-dimensional BCI motor imagery tasks, examining training time and representational similarities.

Result: 2D convolutions significantly decrease training time in high-dimensional tasks while maintaining accuracy, with differing representational geometries found between 1D and 2D models.

Conclusion: Adopting a 2D convolutional layer enhances training and inference speed, emphasizing the need for thoughtful architecture design in processing complex multivariate signals.
...
### [276] [Optimal Posterior Sampling for Policy Identification in Tabular Markov Decision Processes](https://arxiv.org/abs/2605.03921)
*Cyrille Kone,Kevin Jamieson*

Main category: cs.LG

TL;DR: This paper addresses the $(	ext{ε}, 	ext{δ})$-PAC policy identification problem in finite-horizon episodic MDPs, introducing a computationally efficient algorithm that achieves optimal sample complexity and improves over existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for approximate policy identification are computationally expensive and exhibit poor dependence on $	ext{log}(1/	ext{δ})$, hindering practical implementation.

Method: The proposed algorithm combines posterior sampling with an online learning approach, enabling guided exploration within the MDP.

Result: Achieved asymptotic optimality in sample complexity and run-time of $O(S^2AH)$ per episode, improving on previous algorithms by maintaining meaningful guarantees in the asymptotic regime.

Conclusion: The findings offer theoretical insights and practical tools for efficient policy identification in tabular MDPs, enhancing existing methodologies.
...
### [277] [TabSurv: Adapting Modern Tabular Neural Networks to Survival Analysis](https://arxiv.org/abs/2605.03944)
*Stanislav Kirpichenko,Andrei Konstantinov,Lev Utkin*

Main category: cs.LG

TL;DR: TabSurv is a novel approach for survival analysis on tabular data, integrating deep learning architectures and a unique histogram loss function, leading to improved performance over traditional methods.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing deep learning methods in survival analysis by providing a more adaptable and high-performing framework that can leverage modern tabular architectures.

Method: TabSurv employs a feed-forward network and deep ensembles of MLPs, utilizing the Weibull distribution or non-parametric models, alongside an innovative histogram loss function (SurvHL) designed for censored data.

Result: TabSurv demonstrates superior performance across 10 diverse survival datasets, surpassing classical and deep learning baselines in terms of average rank by the C-index, particularly with deep ensembles under Weibull parametrization.

Conclusion: This study enhances the application of modern tabular neural networks in survival analysis, proposing a reliable and effective method, with the TabSurv implementation made publicly available.
...
### [278] [Integrating Feature Correlation in Differential Privacy with Applications in DP-ERM](https://arxiv.org/abs/2605.03945)
*Tianyu Wang,Luhao Zhang,Rachel Cummings*

Main category: cs.LG

TL;DR: This paper introduces a relaxed definition of differential privacy that distinguishes between sensitive and insensitive features, implementing a framework called CorrDP.


<details>
  <summary>Details</summary>
Motivation: Standard differential privacy treats all features uniformly, ignoring the practical distinctions between sensitive and insensitive features, which can lead to suboptimal privacy guarantees.

Method: The authors propose a correlation-aware framework, CorrDP, which allows relaxation of privacy constraints for insensitive features, using total variation distance to quantify correlations and designing algorithms for differentially private empirical risk minimization (DP-ERM).

Result: Experiments on synthetic and real-world datasets demonstrate that CorrDP-based DP-ERM algorithms significantly outperform traditional differential privacy methods when insensitive features are present.

Conclusion: The study shows that the proposed CorrDP framework enhances privacy-utility trade-offs by effectively managing the correlations between sensitive and insensitive features.
...
### [279] [Pretrained Model Representations as Acquisition Signals for Active Learning of MLIPs](https://arxiv.org/abs/2605.03964)
*Eszter Varga-Umbrich,Shikha Surana,Paul Duckworth,Jules Tilly,Olivier Peltre,Zachary Weller-Davies*

Main category: cs.LG

TL;DR: The study explores how pretrained machine learning interatomic potentials (MLIPs) can enhance active learning acquisition in reactive chemistry without requiring complex auxiliary components. Two acquisition signals from pretrained models are introduced, showing significant improvements in efficiency and accuracy over traditional methods.


<details>
  <summary>Details</summary>
Motivation: The high cost of quantum chemical labels and the scarcity of transition state configurations in reactive chemistry hinder the training of machine learning interatomic potentials.

Method: The study investigates the use of two acquisition signals derived from a pretrained MACE potential: a finite-width neural tangent kernel (NTK) and an activation kernel from hidden latent space features, assessing their performance in active learning for MLIPs.

Result: Both acquisition signals significantly outperform traditional methods, reducing the data needed to achieve performance targets by an average of 38% for energy error and 28% for force error on reactive-chemistry benchmarks.

Conclusion: Pretraining aligns latent-space geometry with model error, providing a practical and sufficient acquisition signal for fine-tuning MLIPs in reactive environments.
...
### [280] [A Closed-Form Adaptive-Landmark Kernel for Certified Point-Cloud and Graph Classification](https://arxiv.org/abs/2605.04046)
*Sushovan Majhi,Atish Mitra,Žiga Virk,Pramita Bagchi*

Main category: cs.LG

TL;DR: PALACE introduces a data-adaptive approach for landmark-based classification, offering closed-form guarantees and demonstrating strong empirical performance.


<details>
  <summary>Details</summary>
Motivation: To improve classification accuracy and efficiency in data-adaptive landmark analysis through a refined mechanism that reduces distortion and optimizes performance.

Method: PALACE utilizes a cover-theoretic approach with guarantees on structural distortion, weight maximization, classification rate derivation, and empirical evaluation on benchmark datasets.

Result: Achieved a classification accuracy of 91.3% on Orbit5k, outperforming uniform grids and matching other state-of-the-art methods, with adaptive placement showing robustness under domain inflation.

Conclusion: PALACE provides a robust, efficient framework for landmark-based classification, demonstrating superiority in accuracy and adaptability over conventional methods.
...
### [281] [Flow Sampling: Learning to Sample from Unnormalized Densities via Denoising Conditional Processes](https://arxiv.org/abs/2605.03984)
*Aaron Havens,Brian Karrer,Neta Shaul*

Main category: cs.LG

TL;DR: Flow Sampling introduces an efficient method for sampling unnormalized densities using diffusion models, focusing on energy function evaluation reduction and extending to Riemannian manifolds.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of efficient sampling from unnormalized densities defined by known energy functions, which is critical since energy evaluations can be expensive.

Method: The Flow Sampling framework utilizes diffusion models and flow matching, with a training objective based on noise samples, regressing onto a constructed denoising diffusion drift derived from the energy function.

Result: Demonstrates strong empirical performance on various benchmarks including synthetic energy functions, peptide generation, and spherical distributions.

Conclusion: Flow Sampling efficiently reduces energy evaluations required for sampling and extends the approach to complex geometries, enhancing its applicability in generative modeling.
...
### [282] [MARS-DA: A Hierarchical Reinforcement Learning Framework for Risk-Aware Multi-Agent Bidding in Power Grids](https://arxiv.org/abs/2605.03142)
*Jiayi Chen,Xuan Zhang,Guiling Wang*

Main category: cs.MA

TL;DR: This paper presents MARS-DA, a novel framework for optimizing bidding strategies in volatile renewable energy markets, featuring a two-settlement environment and improved risk-adjusted returns.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the complexities and volatility introduced by renewable energy penetration in electricity markets, specifically focusing on the limitations of traditional Reinforcement Learning approaches in managing risk and profitability.

Method: The authors developed a high-fidelity gymnasium environment based on empirical data from the PJM Interconnection and introduced MARS-DA, which employs a Meta-Controller to manage two specialized agents for distinct objectives: risk management and speculative profit seeking.

Result: MARS-DA demonstrated enhanced performance in achieving risk-adjusted returns compared to existing approaches, and effectively managed strategy alignment in volatile market conditions.

Conclusion: The findings indicate that MARS-DA offers a significant advancement in bidding strategy optimization in renewable electricity markets, balancing profit and risk management more effectively than traditional methods.
...
### [283] [Coordination as an Architectural Layer for LLM-Based Multi-Agent Systems](https://arxiv.org/abs/2605.03310)
*Maksym Nechepurenko,Pavel Shuvalov*

Main category: cs.MA

TL;DR: This study addresses the high failure rates of multi-agent LLM systems due to coordination defects, proposing coordination as a configurable architectural layer and presenting an information-controlled design for prediction markets.


<details>
  <summary>Details</summary>
Motivation: To reduce failure rates in multi-agent LLM systems by focusing on coordination issues rather than base model capabilities.

Method: An information-controlled design was applied to prediction markets using a fixed LLM, tools, output cap, and prompt templates across five coordination configurations while analyzing Murphy signatures and Pareto frontiers.

Result: The study discovered that three out of five predictions were upheld, with two specific configurations leading the Pareto frontier and exploratory analysis revealing distinctions in consensus alignment.

Conclusion: This work presents a methodological framework that validates the approach in a controlled environment, rather than making broad claims across various models.
...
### [284] [Enwar 3.0: An Agentic Multi-Modal LLM Orchestrator for Situation-Aware Beamforming, Blockage Prediction, and Handover Management](https://arxiv.org/abs/2605.03215)
*Ahmad M. Nazar,Abdulkadir Celik,Asmaa Abdallah,Mohamed Y. Selim,Daji Qiao,Ahmed M. Eltawil*

Main category: cs.MA

TL;DR: The paper introduces Enwar 3.0, a framework utilizing multimodal sensing and large language models for improving mmWave connectivity in vehicular networks under varying conditions.


<details>
  <summary>Details</summary>
Motivation: To address challenges in maintaining reliable mmWave connectivity due to environmental change, sensor failures, and link variability in vehicular networks.

Method: Enwar 3.0 combines a classifier-driven sensor health assessment with a large language model that manages task-specific agents for beamforming, blockage detection, and handover management.

Result: The proposed approach includes a synthetic degradation pipeline, achieving over 99% accuracy in detecting sensor impairments and performance metrics such as beam selection accuracy above 88% and blockage F1-score over 98%.

Conclusion: Enwar 3.0 establishes a scalable framework for real-time adaptive wireless systems that integrate reasoning and perception capabilities.
...
### [285] [Physics-Grounded Multi-Agent Architecture for Traceable, Risk-Aware Human-AI Decision Support in Manufacturing](https://arxiv.org/abs/2605.04003)
...
TL;DR: This paper presents multi-agent knowledge analysis (MAKA), a decision-support architecture for high-precision CNC machining of aerospace components, improving reliability and safety in numerical workflows by separating processes and incorporating human approval.


<details>
  <summary>Details</summary>
Motivation: The need for bounded compensations informed by various data sources to ensure high-precision machining of aerospace components and enhance decision-making processes.

Method: MAKA uses a human-in-the-loop architecture that involves intent routing, quantitative analysis, knowledge graph retrieval, and critic-based verification to ensure physical plausibility and safety before human approval is sought.

Result: In a machining testbed for Ti-6Al-4V rotor blades, MAKA improved successful tool execution by up to 87.5 percentage points compared to unstructured interactions, and reduces predicted surface deviation to approximately ±10^{-3} in.

Conclusion: MAKA provides a framework for risk-aware decision-making in CNC machining by enhancing tool orchestration and offering traceable compensation candidates.
...
### [286] [MemFlow: Intent-Driven Memory Orchestration for Small Language Model Agents](https://arxiv.org/abs/2605.03312)
*Jiayi Chen,Yingcong Li,Guiling Wang*

Main category: cs.MA

TL;DR: MemFlow is a memory orchestration framework enhancing Small Language Models' performance on complex tasks by using specialized memory retrieval strategies.


<details>
  <summary>Details</summary>
Motivation: To resolve issues of memory failure in Small Language Models due to mismatched memory operations for different query types.

Method: MemFlow employs a Router Agent to classify queries and route them to a Memory Agent for executing retrieval through one of three memory tiers, followed by an Answer Agent generating responses and a Validator Agent for potential retries.

Result: MemFlow demonstrates nearly 2x improvement in accuracy compared to full-context SLM baselines on long-horizon memory benchmarks.

Conclusion: Structured intent routing and deterministic evidence preparation can notably enhance the effectiveness of resource-constrained models in complex, long-horizon tasks.
...
### [287] [FINER-SQL: Boosting Small Language Models for Text-to-SQL](https://arxiv.org/abs/2605.03465)
*Thanh Dat Hoang,Thanh Trung Huynh,Matthias Weidlich,Thanh Tam Nguyen,Tong Chen,Hongzhi Yin,Quoc Viet Hung Nguyen*

Main category: cs.DB

TL;DR: FINER-SQL is a novel reinforcement learning framework designed to improve small language models (SLMs) for Text-to-SQL generation through fine-grained execution feedback, enhancing performance while maintaining efficiency and data privacy.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the limitations of large language models in Text-to-SQL tasks, specifically their high computational costs, latency issues, and data privacy concerns, advocating for the use of small language models instead.

Method: FINER-SQL employs group relative policy optimization, integrating two novel reward functions: a memory reward for semantic stability and an atomic reward for partial credit on structurally correct SQLs, replacing sparse binary rewards with continuous feedback.

Result: Experiments demonstrate that FINER-SQL achieves significant execution accuracy, reaching up to 67.73% and 85% on the BIRD and Spider benchmarks, using a 3B model, while also reducing inference latency to 5.57 seconds per sample.

Conclusion: The framework showcases a cost-effective, efficient, and privacy-preserving approach to enhance SLMs for Text-to-SQL generation, enabling high performance comparable to larger models.
...
### [288] [In-memory Multidimensional Indexing Using the skd-tree](https://arxiv.org/abs/2605.03640)
*Achilleas Michalopoulos,Dimitrios Tsitsigkos,Nikos Mamoulis*

Main category: cs.DB

TL;DR: The paper introduces the slicing kd-tree (skd-tree), a new indexing structure for efficiently handling multi-dimensional range queries and nearest neighbor queries by compressing splitters and utilizing data-parallelism.


<details>
  <summary>Details</summary>
Motivation: To improve indexing efficiency for multi-dimensional data in memory, addressing the limitations of traditional structures like the kd-tree, R-tree, and others.

Method: The skd-tree compresses partition splitters and leverages data-parallelism to reduce tree levels and computational requirements for queries, with novel algorithms for construction and updates.

Result: The skd-tree offers enhanced performance over existing multi-dimensional indexing methods as shown in experimental evaluations on diverse datasets.

Conclusion: The skd-tree achieves significant efficiency gains in handling multi-dimensional range and proximity queries, proposing a promising alternative to established indexing structures.
...
### [289] [ConRAD: Conformal Risk-Aware Neural Databases](https://arxiv.org/abs/2605.03806)
*Sonia Horchidan,Fabian Zeiher,Xiangyu Shi,Vasiliki Kalavri,Henrik Boström,Ioannis Kontoyiannis,Paris Carbone*

Main category: cs.DB

TL;DR: ConRAD is a framework that optimizes neural graph database queries by ensuring recall guarantees while maximizing precision, using novel techniques for threshold calibration and model inference reduction.


<details>
  <summary>Details</summary>
Motivation: The need to manage errors in multi-hop queries on incomplete knowledge graphs while ensuring valid recall guarantees and minimizing model invocations.

Method: ConRAD implements Conformal Risk Control to set thresholds for operators based on user-defined risk budgets, utilizing a quantile-space scalarization for efficient calibration and a conformal gate to bypass unnecessary inferences.

Result: ConRAD meets all risk budgets with empirical recall at most 0.046 below targets, eliminates neural model invocations in nearly complete regions, and achieves high precision comparable to or better than static baselines.

Conclusion: ConRAD successfully combines guaranteed recall with optimal precision and reduced computational cost in querying incomplete knowledge graphs.
...
### [290] [Inconsistent Databases and Argumentation Frameworks with Collective Attacks](https://arxiv.org/abs/2605.03954)
*Yasir Mahmood,Jonni Virtema,Timon Barlag,Axel-Cyrille Ngonga Ngomo*

Main category: cs.DB

TL;DR: This paper establishes connections between subset-maximal repairs for inconsistent databases with integrity constraints and argumentation frameworks, highlighting the need for SETAFs and delineating relationships between repairs and extensions.


<details>
  <summary>Details</summary>
Motivation: To explore the relationship between database repairs involving integrity constraints and argumentation frameworks, specifically in the context of denial constraints and tuple-generating dependencies.

Method: The study introduces SETAFs, an extension of traditional argumentation frameworks, and analyzes how different integrity constraints affect the corresponding preferred and stable extensions based on database repairs.

Result: It is demonstrated that repairs related to denial constraints correspond to naive extensions, while repairs associated with tuple-generating dependencies yield preferred extensions. Preprocessing can lead to unique stable and naive extensions under specific conditions.

Conclusion: The paper concludes that allowing both types of integrity constraints complicates the relationships, as only preferred semantics adequately captures repairs when both are present, and confirms that inclusion dependencies do not necessitate set-based attacks.
...
### [291] [Design and Implementation of BNN-Based Object Detection on FPGA](https://arxiv.org/abs/2605.03396)
*Xuyu Zhao,Yunpeng Wu,Mengyuan Zhu,Haoyu Huang,Xiaoyu Xu,Yanjing Li,Gaolong Zhang,Baochang Zhang*

Main category: cs.AR

TL;DR: The paper presents a Binary Neural Network (BNN) based YOLOv3-tiny-like object detector optimized for FPGA with low resource usage.


<details>
  <summary>Details</summary>
Motivation: To implement a lightweight object detection model using BNNs on low-cost hardware for real-time applications.

Method: The model processes 320x320 RGB images, utilizing 1-bit weights and 8-bit activations across most layers, with fixed-point convolutions in specific layers. Implementation is done in Verilog RTL, incorporating hardware optimizations like binary convolution and quantization.

Result: Achieves 39.6% mAP50 on VOC with 0.098 GFLOPs and 0.74 M parameters; demonstrates high accuracy in detection output with a correlation coefficient of 0.999964 and a mean absolute error of 0.020027 against the ONNX model.

Conclusion: The proposed FPGA-based BNN implementation effectively balances performance and resource efficiency, making it suitable for real-time object detection.
...
### [292] [SPEC CPU2026: Characterization, Representativeness, and Cross-Suite Comparison](https://arxiv.org/abs/2605.03713)
*Ruihao Li,Andrew Jacob,Neeraja J. Yadwadkar,Lizy K. John*

Main category: cs.AR

TL;DR: SPEC CPU2026 offers a comprehensive evaluation of CPU performance across multiple platforms, reflecting modern workload shifts. It presents compact workload subsets for efficient testing while demonstrating improvements over previous benchmarks.


<details>
  <summary>Details</summary>
Motivation: To ensure CPU benchmarks reflect current workloads and bottlenecks, particularly in AI-dominated environments where CPU performance affects system efficiency.

Method: Characterization of SPEC CPU2026 across nine platforms, clustering analysis to identify compact subsets, and comparisons against existing benchmark suites like SPEC CPU2017, DCPerf, and MLPerf using microarchitectural metrics.

Result: SPEC CPU2026 has increased instruction volume and memory footprint, highlighting emerging bottlenecks such as instruction-cache stress. Compact subsets maintain 96.4-99.9% fidelity of full-suite behavior, reducing evaluation costs.

Conclusion: SPEC CPU2026 establishes a new standard for CPU evaluation, bridging the gap between theoretical benchmarks and real-world usage while facilitating architectural studies.
...
### [293] [From Barrier to Bridge: The Case for AI Data Center/Power Grid Co-Design](https://arxiv.org/abs/2605.03090)
*Noman Bashir,Rob Sherwood,Le Xie,Minlan Yu*

Main category: cs.DC

TL;DR: The paper discusses the need for collaboration between data centers and electric power industries due to the unique high demands of AI training data centers, which disrupt traditional load diversity assumptions of the electric grid.


<details>
  <summary>Details</summary>
Motivation: The necessity for a sustainable and reliable power supply for AI training data centers, which do not conform to existing electric grid assumptions, prompts an exploration of the relationship between compute and power infrastructure.

Method: The paper analyzes the differing design principles, operational philosophies, and economic incentives between data centers and electric power industries, emphasizing the need for their co-development.

Result: It identifies the challenges posed by cultural and technical misalignment, which hinder effective coordination between the two sectors.

Conclusion: Recommendations are made for research directions necessary to ensure the sustainable and reliable powering of AI, including joint capacity planning and market innovations.
...
### [294] [VDCores: Resource Decoupled Programming and Execution for Asynchronous GPU](https://arxiv.org/abs/2605.03190)
*Zijian He,Adrian Sampson,Yiying Zhang,Zhiyuan Guo*

Main category: cs.DC

TL;DR: VDCores introduces a new programming model for asynchronous GPUs that improves hardware utilization and decoding throughput while minimizing programmer effort.


<details>
  <summary>Details</summary>
Motivation: Modern GPU software stacks do not effectively utilize asynchronous hardware units due to a reliance on a monolithic kernel model.

Method: VDCores utilizes virtual cores to abstract hardware execution units and represents workloads as dependency-connected micro-operations, automating the orchestration of resources.

Result: VDCores improves decoding throughput by an average of 24%, reaching up to 77% under dynamic inputs, and reduces kernel programming effort by 90%.

Conclusion: The innovative decoupled model significantly boosts GPU performance and opens new avenues for efficient programming of asynchronous workloads.
...
### [295] [On Solving Problems of Substantially Super-linear Complexity in $N^{o(1)}$ Rounds in the MPC Model](https://arxiv.org/abs/2605.03376)
*Andrzej Lingas*

Main category: cs.DC

TL;DR: The paper explores designing efficient protocols for complex problems in Massively Parallel Computation (MPC) under resource constraints.


<details>
  <summary>Details</summary>
Motivation: To investigate the limitations and requirements for developing efficient multi-round protocols in MPC when local memory and machine count are restricted.

Method: The authors analyze the relationship between local memory size and time complexity in the context of established problems with high polynomial time complexity.

Result: They establish that with limited local memory and number of machines, the local computation's time complexity exceeds that of the problem itself.

Conclusion: A fundamental trade-off exists in MPC protocols regarding the efficiency of computation given constraints in resources.
...
### [296] [Enhancing Performance Insight at Scale: A Heterogeneous Framework for Exascale Diagnostics](https://arxiv.org/abs/2605.03561)
*Dragana Grbic*

Main category: cs.DC

TL;DR: The paper presents an enhanced hpcanalysis framework utilizing C++ API and GPU parallelism for efficient performance analysis in exascale systems, achieving significant speedup and improved analytics.


<details>
  <summary>Details</summary>
Motivation: With the increasing concurrency in exascale systems, traditional performance tools face challenges due to high-scale telemetry overhead.

Method: The framework employs a high-performance C++ API and a GPU-accelerated layer to process execution traces, mapping performance outliers to physical interconnect coordinates.

Result: Achieved 9.69 seconds ingestion time for 100,000 MPI ranks and up to 314x speedup in analyzing execution traces, identifying a potential 32.28% speedup for a GAMESS workload.

Conclusion: The proposed infrastructure significantly enhances performance diagnostics and integrates with external tools, leading to better analysis and potential optimizations.
...
### [297] [Lifting to tensors when compiling scientific computing workloads for AI Engines](https://arxiv.org/abs/2605.03566)
*Nick Brown,Gabriel Rodriguez-Canal*

Main category: cs.DC

TL;DR: The paper discusses a pipeline for mapping scientific codes to AMD's AI Engines (AIEs), aiming to reduce complexities and enhance performance and energy efficiency without extensive code modifications.


<details>
  <summary>Details</summary>
Motivation: To leverage the energy and performance advantages of AMD's AI Engines in scientific computing, particularly in resource-constrained environments, while minimizing the effort required to adapt existing codes.

Method: The authors propose a compilation pipeline that transforms general-purpose scientific loops into tensor representations, simplifying the mapping process to AIEs when using OpenMP annotations.

Result: Using the proposed method on six benchmark kernels for AI and scientific computing, the NPU achieves performance comparable to multicore CPUs with lower energy consumption. Performance improves by up to 40% and energy usage decreases by 15% when running certain kernels across both CPU and NPU.

Conclusion: The approach effectively alleviates the complexity of adapting scientific codes for AIEs, leading to improved performance and reduced energy consumption.
...
### [298] [A Workflow-Oriented Framework for Asynchronous Human-AI Collaboration in Hybrid and Compute-Intensive HPC Environments](https://arxiv.org/abs/2605.03743)
...
TL;DR: The paper introduces a workflow framework for asynchronous human-AI collaboration in high-performance computing environments, allowing for efficient human input without interrupting compute jobs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance human involvement in AI training and deployment in defence and security sectors, addressing the impracticality of real-time interactions in high-performance computing due to resource constraints.

Method: The method involves creating a workflow framework that integrates human inputs at checkpoints without pausing ongoing computational tasks. It supports SLURM-based scheduling and can work with both containerized and native tasks.

Result: The framework was successfully demonstrated in model training on MareNostrum 5, showcasing enhanced portability, efficiency, and oversight during AI workflows.

Conclusion: The framework enables effective asynchronous collaboration between humans and AI, making it practical for high-stakes environments while optimizing resource use.
...
### [299] [Implementing True MPI Sessions and Evaluating MPI Initialization Scalability](https://arxiv.org/abs/2605.03983)
*Hui Zhou,Kenneth Raffenetti,Yanfei Guo,Michael Wilkins,Rajeev Thakur*

Main category: cs.DC

TL;DR: The paper discusses the implementation of true Sessions in MPICH, addressing challenges posed by traditional MPI_COMM_WORLD dependencies and highlighting architectural changes that enhance scalability in exascale systems.


<details>
  <summary>Details</summary>
Motivation: To address the scalability bottleneck of the traditional MPI_COMM_WORLD model, particularly for exascale systems, by implementing true Sessions as proposed in the MPI-4 standard.

Method: The paper includes an internal refactoring of MPICH to support true MPI Sessions, detailing the architectural changes necessary for this implementation.

Result: The evaluation demonstrates that the implementation of true Sessions leads to significant scalability improvements through the adoption of explicit hierarchical designs.

Conclusion: The establishment of true Sessions can effectively enhance the scalability of applications in MPI, aligning more closely with the intended design of the MPI-4 standard.
...
### [300] [Safety in Embodied AI: A Survey of Risks, Attacks, and Defenses](https://arxiv.org/abs/2605.02900)
...
TL;DR: The survey reviews safety research in Embodied AI, addressing challenges in ensuring safety for autonomous agents in critical environments, and presents a structured framework to identify research gaps.


<details>
  <summary>Details</summary>
Motivation: To ensure the safety of embodied AI systems operating in safety-critical domains as they gain autonomy, addressing the distinct challenges they face compared to digital AI systems.

Method: The paper provides a structured review of safety research across the embodied AI pipeline, introduces a multi-level taxonomy, and analyzes insights from over 400 research papers related to various attack types and safety measures.

Result: Reveals several overlooked challenges in embodied AI, such as vulnerabilities in multimodal perception fusion, planning instability under specific attacks, and issues with human-agent interactions in open-ended scenarios.

Conclusion: The survey establishes a coherent framework for embodied AI safety research and highlights critical gaps, serving as a roadmap for developing safe and reliable autonomous agents.
...
### [301] [Tracing the Dynamics of Refusal: Exploiting Latent Refusal Trajectories for Robust Jailbreak Detection](https://arxiv.org/abs/2605.02958)
*Xulin Hu,Che Wang,Wei Yang Bryan Lim,Jianbo Gao,Zhong Chen*

Main category: cs.CR

TL;DR: This paper introduces SALO, a novel inference-time detector to improve defense against attacks by focusing on dynamic refusal processes rather than static representations.


<details>
  <summary>Details</summary>
Motivation: The traditional approach to Representation Engineering uses static refusal vectors, which limits effectiveness in adversarial contexts.

Method: The paper employs Causal Tracing to identify the Refusal Trajectory, enabling the design of SALO, which captures latent patterns in refusal processes.

Result: SALO improves detection rates against forced-decoding attacks significantly, enhancing performance from ~0% to over 90% compared to existing terminal state methods.

Conclusion: The findings highlight the importance of treating refusal as a dynamic process, providing a more effective defense mechanism in adversarial scenarios.
...
### [302] [LIPPEN: A Lightweight In-Place Pointer Encryption Architecture for Pointer Integrity](https://arxiv.org/abs/2605.03974)
...
TL;DR: LIPPEN is a hardware-software co-design for full-pointer encryption that enhances memory-safety in C and C++ programs by providing pointer integrity and confidentiality without additional metadata overhead. It achieves this by encrypting pointers and remains compatible with existing systems.


<details>
  <summary>Details</summary>
Motivation: To address memory-safety violations in C and C++ programs that lead to security vulnerabilities, by improving existing defenses that either use metadata or in-place mechanisms with limitations.

Method: LIPPEN treats pointers as encrypted blocks cryptographically bound to their execution context, decrypting them upon dereferencing, and fully utilizing the 64-bit pointer field for encryption.

Result: In evaluations on FPGA with RISC-V cores, LIPPEN shows runtime overhead comparable to current PAC implementations while providing comprehensive pointer protection, alongside minimal area and power overhead.

Conclusion: LIPPEN offers a practical solution for deploying robust pointer protection in real processors, strengthening application security against exploitation techniques.
...
### [303] [Decompose to Understand, Fuse to Detect: Frequency-Decoupled Anomaly Detection for Encrypted Network Traffic](https://arxiv.org/abs/2605.02970)
*Xinglin Lian,Chengtai Cao,Ting Zhong,Yong Wang,Kai Chen,Fan Zhou*

Main category: cs.CR

TL;DR: The paper introduces FreeUp, a frequency-decoupled framework for improved anomaly detection in encrypted network traffic, addressing the issue of spectral mismatch in existing methods.


<details>
  <summary>Details</summary>
Motivation: The growing prevalence of encryption in network traffic complicates the anomaly detection task due to a bias in mainstream reconstruction methods toward low-frequency information, resulting in poor anomaly detection performance.

Method: FreeUp employs a frequency-decoupling approach, processing low- and high-frequency traffic data through separate branches and using a specialized training strategy, along with an uncertainty-inspired fusion scoring mechanism for better anomaly score integration.

Result: Experiments reveal that FreeUp consistently outperforms existing state-of-the-art anomaly detection methods across various benchmarks, demonstrating its effectiveness in handling encrypted traffic.

Conclusion: The proposed FreeUp framework significantly enhances anomaly detection performance in encrypted network traffic by effectively addressing spectral mismatch and utilizing a novel scoring mechanism.
...
### [304] [Contrastive Privacy: A Semantic Approach to Measuring Privacy of AI-based Sanitization](https://arxiv.org/abs/2605.02977)
*George Bissias,Eugene Bagdasarian,Brian Neil Levine*

Main category: cs.CR

TL;DR: The paper introduces contrastive privacy, a formal privacy definition for evaluating sanitization techniques in media, emphasizing its semantic interpretation and independence from sanitization methods.


<details>
  <summary>Details</summary>
Motivation: To address the lack of formal guarantees in existing media sanitization techniques and to establish a systematic evaluation framework.

Method: The authors propose contrastive privacy, a formal privacy definition, and operationalize it using imperfect semantic measures from models like CLIP to evaluate sanitized media against others in the same corpus.

Result: The proposed privacy test was applied to both images and text across numerous models, successfully quantifying sanitization effectiveness and pinpointing specific failures in the process.

Conclusion: Contrastive privacy provides a formalized, flexible assessment of media sanitization that is model- and mechanism-agnostic, enhancing the understanding of sanitization efficacy.
...
### [305] [Observability for Post-Quantum TLS Readiness: A Multi-Surface Evidence Framework](https://arxiv.org/abs/2605.02978)
*José Luis Delgado*

Main category: cs.CR

TL;DR: This paper introduces a framework for improving observability in post-quantum TLS through structured measurements and evaluations across various scenarios, achieving significant detection success.


<details>
  <summary>Details</summary>
Motivation: The need for distinguishing various forms of evidence in post-quantum migration within TLS to enhance security and functionality under evolving cryptographic standards.

Method: A multi-surface framework is developed, categorizing passive evidence, active probing, and registry knowledge, which are applied across different measurement planes related to TLS session behavior and capability.

Result: The framework was tested on 29 scenarios, successfully closing session-level measurement planes and revealing hybrid capabilities exceeding classical session views, amidst some uncertainty in evidence.

Conclusion: The framework demonstrates the capability to enhance TLS observability and identify endpoint capabilities that traditional methods may overlook, validating its effectiveness through extensive practical evaluations.
...
### [306] [Towards a Risk-Cost Model for Financial Adaptive Authentication](https://arxiv.org/abs/2605.02979)
*Supriya Khadka,Sanchari Das*

Main category: cs.CR

TL;DR: Introduction of a Risk-Cost Model (RCM) for adaptive authentication in financial systems to better balance fraud risk, cost considerations, and regulatory compliance.


<details>
  <summary>Details</summary>
Motivation: To improve security in financial systems facing significant risks due to inaccurate authentication leading to monetary losses, highlighting the need for a more integrated approach.

Method: Development of a Risk-Cost Model (RCM) that incorporates cost-sensitive risk functions, sequential decision-making mechanisms, and privacy/regulatory constraints to optimize authentication processes.

Result: The RCM presents a dynamic optimization framework that moves beyond traditional static approaches, enhancing the resilience of authentication systems against adversarial threats.

Conclusion: By reframing the authentication process as a constrained dynamic optimization problem, the RCM allows for more economically sound and adaptable security measures in financial systems.
...
### [307] [SoK: After Decades of Web Tracker Detection, What's Next?](https://arxiv.org/abs/2605.02982)
*Wolf Rieder,Philip Raschke,Thomas Cory,Christian René Sechting,Aditya Kumar,Axel Küpper*

Main category: cs.CR

TL;DR: This paper presents a comprehensive review of web tracker detection, proposing a taxonomy and identifying research gaps.


<details>
  <summary>Details</summary>
Motivation: To provide a universal perspective and classification of web tracker detectors, which has been lacking in past reviews.

Method: A systematic review of 832 papers was conducted, resulting in the identification of 59 primary and 16 supplementary studies.

Result: The study led to a proposed taxonomy, observed trends, and highlighted open research gaps in web tracker detection.

Conclusion: The paper aims to lay the groundwork for future research in web tracker detection by synthesizing existing knowledge and addressing validity issues.
...
### [308] [LiteShield: Hybrid Feature Selection-Driven Lightweight Intrusion Detection for Resource-Constrained IoT Networks](https://arxiv.org/abs/2605.02987)
*Dileepa Mabulage,Banuka Athuraliya*

Main category: cs.CR

TL;DR: This paper proposes LiteShield, a lightweight intrusion detection system (IDS) for IoT, utilizing hybrid feature selection and efficient classifiers to enhance attack detection accuracy with minimal computational demands.


<details>
  <summary>Details</summary>
Motivation: To address the security mismatch in IoT by providing a computationally efficient IDS suitable for resource-constrained environments.

Method: The framework employs a hybrid feature selection process combining Mutual Information and Recursive Feature Elimination with Cross-Validation, using the UNSW-NB15 dataset, and evaluates six lightweight classifiers.

Result: KNN achieved the highest accuracy for binary classification at 98.26% and for multiclass classification at 85.22%. Random Forest offered a better balance between accuracy and resource usage.

Conclusion: LiteShield demonstrates that hybrid feature selection and lightweight machine learning techniques can enable effective and efficient intrusion detection for IoT systems.
...
### [309] [ChaRVoC: A Challenge-Response Voice Cancelable Authentication System](https://arxiv.org/abs/2605.02990)
*Phuc-Khang Vo-Hoang,Hoang C. Ta,Nhien-An Le-Khac,Dinh-Thuc Nguyen,Hong-Hanh Nguyen-Le*

Main category: cs.CR

TL;DR: ChaRVoC is a voice cancelable authentication system that safeguards against replay attacks and template compromise by using voice characteristics, user keys, and dynamic challenges.


<details>
  <summary>Details</summary>
Motivation: To address vulnerabilities in biometric authentication such as replay attacks, revocability issues, and template compromise.

Method: ChaRVoC combines voice biometric traits, user-memorized secret keys, and dynamic challenges with a novel HashGray-XOR scheme to create secure, non-invertible biometric templates.

Result: The system was benchmarked against traditional cancelable biometric methods, demonstrating effective recognition performance across multiple datasets (VoxCeleb1, TIMIT, VOiCES).

Conclusion: ChaRVoC successfully achieves cancelability and unlinkability, strengthening biometric authentication security.
...
### [310] [PHANTOM: Polymorphic Honeytoken Adaptation with Narrative-Tailored Organisational Mimicry](https://arxiv.org/abs/2605.02992)
*Abraham Itzhak Weinberg*

Main category: cs.CR

TL;DR: PHANTOM improves honeytoken generation by introducing organisation-specific context, leading to increased believability and resistance to detection compared to traditional methods.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing honeytoken generation tools, which produce identifiable static tokens, by creating contextually relevant tokens that better mimic an organisation's digital environment.

Method: PHANTOM employs a multi-component pipeline that incorporates organisation-specific knowledge into honeytoken generation, and it evaluates quality through a Believability Score based on syntactic, semantic, statistical, and human acceptance metrics.

Result: PHANTOM demonstrates a significantly higher Believability Score (B = 0.778) compared to template-based tokens (B = 0.576), with marked improvements in human acceptance from 6.2% to 100% and detection resistance ratings increasing from 0.609 to 0.870.

Conclusion: The results validate the hypothesis that incorporating organisational context is crucial for effective honeytoken strategies, enhancing their deployment capabilities even in restricted environments.
...
### [311] [Distributed Deep Variational Approach for Privacy-preserving Data Release](https://arxiv.org/abs/2605.03069)
*Zahir Alsulaimawi,Huaping Liu*

Main category: cs.CR

TL;DR: This paper introduces the Gaussian Privacy Protector (GPP), a framework for enhancing privacy in federated learning by mapping sensitive raw data to sanitized low-dimensional representations while preserving utility attributes.


<details>
  <summary>Details</summary>
Motivation: The need for effective privacy protection in federated learning settings, particularly for sensitive data from medical and wearable devices, where traditional methods may not suffice due to potential data leakage.

Method: GPP employs a stochastic encoder trained against a variational lower bound on mutual information pertaining to sensitive attributes, incorporating a trade-off with a cross-entropy term for utility preservation, and is adapted for federated learning by allowing local encoder training on client devices.

Result: GPP shows competitive utility performance, maintaining accuracy close to an unconstrained autoencoder while significantly reducing adversarial capability to infer sensitive attributes.

Conclusion: GPP effectively enhances privacy in federated learning while preserving data utility, marking a significant advancement for applications involving sensitive information.
...
### [312] [Revisiting JBShield: Breaking and Rebuilding Representation-Level Jailbreak Defenses](https://arxiv.org/abs/2605.03095)
*Kemal Derya,Berk Sunar*

Main category: cs.CR

TL;DR: This paper investigates the vulnerability of large language models to jailbreak attacks, presenting a new attack method, JB-GCG, and a defense mechanism called Representation Trajectory Verification (RTV).


<details>
  <summary>Details</summary>
Motivation: To address challenges in defending large language models against jailbreak attacks, particularly in adaptive threat scenarios targeting defensive mechanisms.

Method: The study proposes JB-GCG, which enhances the original Greedy Coordinate Gradient (GCG) attack through refusal-direction suppression and toxic-concept regularization. Additionally, it introduces RTV for detection using Mahalanobis outlier detection across multi-layer representations.

Result: JB-GCG achieves average attack success rates (ASR) of 46.2%, peaking at 53.4%, while the attack against JBShield-M yields 30.7% ASR. RTV demonstrates high detection efficacy with an AUROC of 0.99 against JB-GCG.

Conclusion: The paper concludes that robust non-adaptive defenses are not sufficient against adaptive threats, and emphasizes the importance of multi-layer representation consistency for effective jailbreak detection.
...
### [313] [PIIGuard: Mitigating PII Harvesting under Adversarial Sanitization](https://arxiv.org/abs/2605.03129)
*Mingshuo Liu,Yiwei Zha,Min Chen*

Main category: cs.CR

TL;DR: PIIGuard is a webpage-level defense system that uses hidden HTML fragments to prevent large language models (LLMs) from disclosing personally identifiable information (PII) while preserving webpage utility.


<details>
  <summary>Details</summary>
Motivation: To provide webpage owners with deployable options to protect against PII leaks, focusing on defenses that operate directly at the webpage level rather than at the model or service level.

Method: PIIGuard employs optimized hidden HTML fragments that redirect model queries, using techniques like rule-based leakage scoring and evolutionary mutation to evaluate fragment effectiveness and assess recoverability.

Result: PIIGuard shows a defense success rate of at least 97% against three LLMs in direct-HTML evaluations, achieving up to 100% in some cases, and retains functionality for benign queries on the same page.

Conclusion: The study illustrates that webpage owners can effectively mitigate PII leakage by embedding defensive measures at the page level, though effectiveness can vary based on model and browsing context.
...
### [314] [Zero Day Attacks: Novel Behaviour or Novel Vulnerability?](https://arxiv.org/abs/2605.03138)
*Nnamdi Jibunoh,Sara Khanchi,Adetokunbo Makanju*

Main category: cs.CR

TL;DR: This paper emphasizes the importance of vulnerability-centric approaches in Intrusion Detection Systems (IDSs) to effectively detect zero-day attacks, which primarily exploit unknown vulnerabilities rather than exhibiting novel behaviors.


<details>
  <summary>Details</summary>
Motivation: The paper aims to enhance the effectiveness of IDSs against zero-day attacks by understanding their real-world manifestation and the exploitation of undisclosed vulnerabilities.

Method: A review of documented zero-day incidents over 20 years is conducted, leading to the development of a taxonomy of zero-day vulnerability types and an analysis of machine learning (ML) based detection assumptions.

Result: Findings reveal that attacks predominantly exploit vulnerabilities, particularly memory-corruption flaws, and that the focus on detecting novel behaviors in ML-based systems misaligns with real-world exploitation patterns.

Conclusion: The study advocates for a shift towards vulnerability-centric detection frameworks to improve the identification and mitigation of zero-day threats, cautioning against over-reliance on behavior-based detection.
...
### [315] [Evaluating Retrieval-Augmented Generation for Explainable Malware Analysis](https://arxiv.org/abs/2605.03140)
*Jayson Ng,Amin Milani Fard*

Main category: cs.CR

TL;DR: Retrieval-Augmented Generation (RAG) may degrade the quality of malware explanations provided by Large Language Models (LLMs), as it introduces irrelevant context and noise, despite the assumption that it enhances explanation quality.


<details>
  <summary>Details</summary>
Motivation: To explore the effectiveness of Retrieval-Augmented Generation (RAG) in improving malware explanation using structured input from VirusTotal reports, amidst increasing reliance on LLMs for security engineering.

Method: Empirical evaluation of RAG's impact on malware explanation quality across multiple Large Language Models, utilizing structured input from VirusTotal reports.

Result: RAG was found to often degrade explanation quality, introducing distractions and generalizations rather than enhancing understanding.

Conclusion: Malware explanation should focus on signal extraction rather than knowledge retrieval, indicating that structured evidence alone may be sufficient, and suggesting design changes for security workflows.
...
### [316] [HackerSignal: A Large-Scale Multi-Source Dataset Linking Hacker Community Discourse to the CVE Vulnerability Lifecycle](https://arxiv.org/abs/2605.03158)
*Benjamin M. Ampel,Sagar Samtani*

Main category: cs.CR

TL;DR: HackerSignal is a comprehensive benchmark dataset for cyber threat intelligence, linking vulnerabilities and exploits from diverse sources over 36 years, and supporting various AI-enabled cybersecurity tasks.


<details>
  <summary>Details</summary>
Motivation: To create a public dataset that accurately maps vulnerabilities to exploit trajectories using discourse from the hacker community and other resources, addressing gaps in existing datasets.

Method: HackerSignal aggregates 7.45 million deduplicated documents from 64 public sources across a 36-year timeline and establishes linkage through shared CVE identifiers, supporting unique AI-driven tasks.

Result: The dataset supports three benchmark tasks: CVE linkage retrieval, exploit type classification with temporal considerations, and temporal generalization, validated through temporal data splits.

Conclusion: HackerSignal offers a valuable resource for advancing cybersecurity research, with provided diagnostics and governance to ensure ethical data usage.
...
### [317] [A Validated Prompt Bank for Malicious Code Generation: Separating Executable Weapons from Security Knowledge in 1,554 Consensus-Labeled Prompts](https://arxiv.org/abs/2605.03179)
*Richard J. Young,Gregory D. Moody*

Main category: cs.CR

TL;DR: This paper critiques existing benchmarks of language models in the context of malicious coding tasks, highlighting the confusion between requests for executable malware and requests for harmful security knowledge. It introduces a classification axis using a consensus method across models to provide clarity and reliability in evaluating these distinctions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to clarify the refusal pathways of safety-aligned language models when faced with requests for harmful software versus harmful knowledge, as current benchmarks do not adequately differentiate these categories.

Method: The method involves a five-model consensus protocol where five large-language model judges classify prompts based on a CODE/KNOWLEDGE axis, applying a three-of-five majority rule and assessing inter-rater reliability using Fleiss' kappa.

Result: The consensus process resulted in a 1,554-prompt consensus-CODE bank and a 388-prompt consensus-KNOWLEDGE comparison set, achieving high inter-rater agreement (kappa = 0.876) and zero ambiguity in classifications.

Conclusion: The paper concludes by advocating for the weapons-versus-knowledge axis in code-safety evaluations, while leaving further exploration of its practical separation of model behavior to future studies.
...
### [318] [Dependency-Aware Privacy for Multi-turn Agents](https://arxiv.org/abs/2605.03188)
*Divyam Anshumaan,Sarthak Choudhary,Nils Palumbo,Somesh Jha*

Main category: cs.CR

TL;DR: RootGuard enhances privacy in LLM agents' data releases by sanitizing initial private attributes and deterministically deriving subsequent outputs, outperforming traditional independent noising methods.


<details>
  <summary>Details</summary>
Motivation: LLM agents risk exposing private information through multiple interactions, leading to cumulative privacy degradation that can be exploited by adversaries.

Method: RootGuard sanitizes root values once and derives future outputs from these noised roots, leveraging structural knowledge to efficiently allocate privacy budget.

Result: RootGuard produces 2.3-3.0 times lower target error compared to independent noising, maintaining privacy even with increased adversarial queries.

Conclusion: RootGuard offers a significant improvement in privacy preservation and accuracy over conventional methods in multi-turn interactions.
...
### [319] [When Agents Handle Secrets: A Survey of Confidential Computing for Agentic AI](https://arxiv.org/abs/2605.03213)
*Javad Forough,Marios Kogias,Hamed Haddadi*

Main category: cs.CR

TL;DR: This survey explores the vulnerabilities of agentic AI systems, particularly LLM-driven agents, and presents confidential computing as a defense mechanism. It categorizes Trusted Execution Environments, establishes an agent-centric threat model, compares defenses, and highlights open challenges.


<details>
  <summary>Details</summary>
Motivation: To address the unique threats posed by LLM-driven agentic AI systems, which operate in decentralized and multi-agent environments, necessitating specialized security measures beyond current software-based defenses.

Method: The survey synthesizes information into four parts: a taxonomy of TEE platforms, a threat model for agents, a comparative defense survey, and identification of open challenges in the context of agentic AI security.

Result: It identifies that while TEE technologies are maturing, they have not yet established a comprehensive security framework for production use in agentic AI systems.

Conclusion: An effective security substrate that incorporates hardware trust primitives for agentic AI remains to be developed, highlighting the need for further research and integration.
...
### [320] [MAGE: Safeguarding LLM Agents against Long-Horizon Threats via Shadow Memory](https://arxiv.org/abs/2605.03228)
*Yuhui Wang,Tanqiu Jiang,Jiacheng Liang,Charles Fleming,Ting Wang*

Main category: cs.CR

TL;DR: MAGE is a novel defensive framework designed to protect LLM-powered agents from long-horizon threats that exploit user-agent interactions, leveraging a safety-focused memory for proactive risk assessment.


<details>
  <summary>Details</summary>
Motivation: With the deployment of LLM-powered agents in complex tasks, new long-horizon threats arise that could compromise safety, necessitating effective defenses.

Method: MAGE employs a shadow memory to retain safety-critical context throughout the agent's operations, assessing risks of actions before execution.

Result: Extensive evaluations show MAGE significantly improves detection accuracy of long-horizon threats, allowing for early-stage identification and minimal impact on agent functionality.

Conclusion: MAGE is the first framework to address long-horizon threats using agentic memory, paving the way for future research in enhancing LLM agent safety.
...
### [321] [Cryptographic Registry Provenance: Structural Defense Against Dependency Confusion in AI Package Ecosystems](https://arxiv.org/abs/2605.03309)
*Alan L. McCann*

Main category: cs.CR

TL;DR: This paper presents a cryptographic distribution provenance system to mitigate dependency confusion attacks by ensuring cryptographic proof of package origins.


<details>
  <summary>Details</summary>
Motivation: To address the gaps in existing defenses against dependency confusion attacks that rely on configuration and fail silently when misconfigured.

Method: A three-component cryptographic system incorporating registry identity, a dual-signature model, and authoritative namespace binding to protect against attacks.

Result: Evaluation across multiple ecosystems highlights that no existing system includes mandatory signing and cryptographic identity, indicating the uniqueness of this system.

Conclusion: The proposed system creates a robust defense against dependency confusion attacks while integrating AI-generation provenance and a governance-enforced runtime architecture.
...
### [322] [SILMARILS: Information-Theoretic and Quantum-Secure Designated-Verifier Signatures](https://arxiv.org/abs/2605.03230)
*Hassan Khodaiemehr,Khadijeh Bagheri,Chen Feng,Dariia Porechna*

Main category: cs.CR

TL;DR: SILMARILS is a post-quantum secure framework for designated-verifier signatures that employs minimal algebraic techniques and supports both two-party and three-party modes, achieving efficient security and smaller keys compared to existing schemes.


<details>
  <summary>Details</summary>
Motivation: The need for secure, efficient signature schemes suitable for blockchain and post-quantum cryptography.

Method: A minimal algebraic core using true randomness and Shamir secret sharing is implemented to create two-party TDV signatures and three-party statistically secure signatures.

Result: SILMARILS achieves $	ext{EUF-CMA}^{
eg	ext{DV}}$ security for non-designated verifiers and simulates acceptable transcripts indistinguishable from real ones, with recorded receipt verification.

Conclusion: The framework offers significantly smaller keys and signatures than existing post-quantum schemes while ensuring strong security guarantees applicable in blockchain contexts.
...
### [323] [ARGUS: Defending LLM Agents Against Context-Aware Prompt Injection](https://arxiv.org/abs/2605.03378)
*Shihao Weng,Yang Feng,Jinrui Zhang,Xiaofei Xie,Jiongchi Yu,Jia Liu*

Main category: cs.CR

TL;DR: The paper introduces AgentLure, a benchmark for context-aware prompt injection attacks on LLM agents, and proposes ARGUS, a defense mechanism that significantly improves security by auditing decision provenance.


<details>
  <summary>Details</summary>
Motivation: To address the inadequacies of current benchmarks and defenses against prompt injection attacks in LLM agents that do not account for dynamic context.

Method: The paper develops AgentLure to simulate context-dependent tasks and introduces ARGUS, which uses provenance-aware decision auditing by constructing influence provenance graphs.

Result: Evaluation shows that existing defenses underperform against context-aware attacks, while ARGUS significantly reduces attack success to 3.8% and maintains 87.5% task utility.

Conclusion: ARGUS proves effective against adaptive adversaries, enhancing the security of LLM agents in real-world scenarios by addressing limitations in current threat models.
...
### [324] [SkCC: Portable and Secure Skill Compilation for Cross-Framework LLM Agents](https://arxiv.org/abs/2605.03353)
*Yipeng Ouyang,Yi Xiao,Yuhao Gu,Xianwei Zhang*

Main category: cs.CR

TL;DR: SkCC is introduced as a compilation framework for autonomous systems, enhancing skill portability while addressing performance and security issues.


<details>
  <summary>Details</summary>
Motivation: The motivation lies in the observed performance variations due to prompt formatting in agent frameworks and the burden of rewriting skills for different platforms, coupled with security vulnerabilities in community skills.

Method: SkCC employs classical compiler techniques to create SkIR, a strongly-typed intermediate representation that separates skill semantics from specific formatting, supported by a compile-time Analyzer that enforces security constraints.

Result: Experiments on SkillsBench show that compiled skills outperform original formats in performance, with improved pass rates and significant runtime efficiencies.

Conclusion: SkCC significantly reduces complexity in skill adaptation and enhances both performance and security in autonomous agent systems.
...
### [325] [DECKER: Domain-invariant Embedding for Cross-Keyboard Extraction and Recognition](https://arxiv.org/abs/2605.03384)
*Bikrant Bikram Pratap Maurya,Nitin Choudhury,Daksh Agarwal,Arun Balaji Buduru*

Main category: cs.CR

TL;DR: The paper presents HEAR, a comprehensive dataset for studying acoustic side-channel attacks on keyboards, and introduces the DECKER framework which improves keystroke inference across diverse conditions.


<details>
  <summary>Details</summary>
Motivation: To address limitations in prior ASCA studies, particularly regarding dataset size, diversity, and the analysis of keystrokes across various devices and environments.

Method: A dataset named HEAR was created to encompass recordings from various users and keyboards in different settings. The DECKER framework consists of four stages for effective keystroke inference, with enhancements from a language model for sentence-level accuracy.

Result: DECKER demonstrates superior performance in keystroke identification compared to previous methods, particularly in challenging cross-keyboard and cross-user scenarios.

Conclusion: Acoustic side-channel attacks remain a significant security concern, as evidenced by the effective inference capabilities demonstrated across diverse settings.
...
### [326] [Exposing LLM Safety Gaps Through Mathematical Encoding:New Attacks and Systematic Analysis](https://arxiv.org/abs/2605.03441)
*Haoyu Zhang,Mohammad Zandsalimy,Shanu Sushmita*

Main category: cs.CR

TL;DR: Large language models are vulnerable to encoded harmful prompts when reformulated as coherent mathematical problems, achieving a significant attack success rate.


<details>
  <summary>Details</summary>
Motivation: To expose vulnerabilities in the safety mechanisms of LLMs that primarily rely on semantic pattern matching.

Method: Harmful prompts were reformulated into mathematical problems using various formalisms, and their effectiveness was assessed across multiple models and benchmarks.

Result: The study achieved a high attack success rate (46%--56%) and demonstrated that reformulation into mathematical problems is crucial for bypassing safety filters.

Conclusion: Current LLM safety frameworks have fundamental gaps that need addressing; defenses should focus on mathematical reasoning rather than just semantic patterns.
...
### [327] [MOSAIC-Bench: Measuring Compositional Vulnerability Induction in Coding Agents](https://arxiv.org/abs/2605.03952)
*Jonathan Steinberg,Oren Gal*

Main category: cs.CR

TL;DR: MOSAIC-Bench identifies discrepancies in coding agent safety by evaluating their performance on sequenced requests; results reveal significant exploitable vulnerabilities in routine coding tasks.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the gap in safety alignment of coding agents that approve harmful code due to isolated assessment methods.

Method: Introduced MOSAIC-Bench, a benchmark with 199 attack chains on various software substrates, evaluating the vulnerability of coding agents across multiple dimensions including exploit ground truth and reviewer protocols.

Result: Nine coding agents demonstrated high ASR (53-86%), while a direct-prompt test showed reduced vulnerabilities (0-20.4%), indicating ticket staging's impact on safety.

Conclusion: Reframing code reviewers as adversarial pentesters improves detection of vulnerabilities, with a specific reviewer achieving an 88.4% attack detection rate and only 4.6% false positives.
...
### [328] [MEMSAD: Gradient-Coupled Anomaly Detection for Memory Poisoning in Retrieval-Augmented Agents](https://arxiv.org/abs/2605.03482)
*Ishrith Gowda*

Main category: cs.CR

TL;DR: The paper formalizes memory poisoning attacks on retrieval-augmented agents using game theory and presents the MEMSAD defense against these attacks, demonstrating significant improvements in detection and calibration reliability.


<details>
  <summary>Details</summary>
Motivation: To address the formal security properties of persistent external memory systems in LLM agents and to explore reliable defenses against memory poisoning attacks.

Method: Formalization of attacks as a Stackelberg game with three classes; introduction of MEMSAD for anomaly detection based on gradient coupling; empirical validation of defense effectiveness through extensive experiments.

Result: The evaluation shows that the rate of attack success increases by 4 times under corrected conditions, while MEMSAD achieves optimal calibration with certified detection guarantees across varied adversary strategies.

Conclusion: Composite defenses achieve perfect detection rates, but certain attacks remain effective, highlighting limitations of existing embedding-based defenses.
...
### [329] [Generating Proof-of-Vulnerability Tests to Help Enhance the Security of Complex Software](https://arxiv.org/abs/2605.03956)
*Shravya Kanchi,Xiaoyan Zang,Ying Zhang,Danfeng Yao,Na Meng*

Main category: cs.CR

TL;DR: PoVSmith automates proof-of-vulnerability test generation for Java applications to identify security risks from library vulnerabilities, revealing 158 entry points and generating 152 tests, significantly improving upon existing methods.


<details>
  <summary>Details</summary>
Motivation: To address the challenge developers face in identifying practical security risks from library vulnerabilities, as existing tools for creating proof-of-vulnerability tests are inadequate.

Method: PoVSmith utilizes call path analysis, exemplar tests, code context, and feedback as prompts for coding agents and large language models to automate test generation, execution, and assessment.

Result: On 33 Java app-lib pairs, PoVSmith identified 158 entry points and created 152 feasibility tests, with 84 tests (55%) showing viable attack paths against the vulnerabilities.

Conclusion: PoVSmith not only reduces human effort in test generation but also enhances the quality of the tests, contributing to automated and efficient detection of software supply chain vulnerabilities.
...
### [330] [ZK-Value: A Practical Zero-Knowledge System for Verifiable Data Valuation](https://arxiv.org/abs/2605.03581)
*Zhaoyu Wang,Pingchuan Ma,Zhantong Xue,Yuguang Zhou,Qixin Zhang,Xiaoqin Zhang,Shuai Wang*

Main category: cs.CR

TL;DR: ZK-Value is a practical zero-knowledge proof data valuation system that improves scalability and efficiency in data marketplaces by combining locality-based valuation, optimized proof protocols, and structural enhancements.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the trust issues in data marketplaces where private scores cannot be independently verified, highlighting the need for a scalable zero-knowledge proof system.

Method: The solution comprises a co-designed architecture with LSH-Shapley for valuation, ZK-LSH-Shapley for optimized zero-knowledge proof protocols, and structural optimizations for proof systems.

Result: ZK-Value shows valuation quality comparable to state-of-the-art methods, achieving significant improvements in proving times (12.6x to 68.1x faster than specialized ZK systems) and verification under 4.6 seconds across multiple datasets.

Conclusion: ZK-Value effectively reconciles the conflict between privacy and verifiability in data marketplaces, allowing for trusted data valuation with practical performance.
...
### [331] [The Infinite Mutation Engine? Measuring Polymorphism in LLM-Generated Offensive Code](https://arxiv.org/abs/2605.03619)
*Gabriel Hortea,Juan Tapiador*

Main category: cs.CR

TL;DR: This work investigates the polymorphic capabilities of the Claude Opus 4.6 model as an automated malware generator, demonstrating that it can produce structurally diverse yet behaviorally identical payloads.


<details>
  <summary>Details</summary>
Motivation: The study is motivated by the need to understand the extent to which LLMs can enhance malware polymorphism, complicating detection efforts by cybersecurity defenses.

Method: A dual-agent, four-stage pipeline was used to generate and refine a data-exfiltration payload through varying prompting strategies to measure structural and semantic diversity.

Result: The generated payloads showed high structural diversity when polymorphism was not explicitly requested, while semantic differences were minimal. Explicit prompting led to increased structural diversity but required more processing tokens with minor growth in API calls.

Conclusion: The findings indicate that LLMs like Claude Opus 4.6 can effectively generate diverse malware variants that maintain the same behavioral characteristics, presenting challenges for traditional detection methods.
...
### [332] [Tailored Prompts, Targeted Protection: Vulnerability-Specific LLM Analysis for Smart Contracts](https://arxiv.org/abs/2605.03697)
*Xing Zhang,Keyu Zhang,Taohong Zhu,Anbang Ruan*

Main category: cs.CR

TL;DR: The paper introduces an LLM-based framework for detecting smart contract vulnerabilities, utilizing a large dataset and customized detection methods for improved accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing detection methods which lack flexibility and rely on manual rules, thereby risking financial loss in blockchain applications.

Method: The framework employs a large-scale dataset of annotated vulnerability instances, using AST-based context extraction and tailored prompts for specific vulnerability detection across 13 categories.

Result: The method achieved an average positive recall of 0.92 and an average negative recall of 0.85, indicating high effectiveness in vulnerability detection.

Conclusion: The findings underscore the potential of contextual prompting and LLMs in enhancing the scalability and precision of smart contract security analyses.
...
### [333] [Internet of Things Security: A Survey on Common Attacks](https://arxiv.org/abs/2605.03744)
...
TL;DR: This paper provides a comprehensive survey of IoT security risks, identifying 28 types of attacks and employing the STRIDE and CVSS frameworks for classification and assessment.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the significant security risks in IoT due to device limitations, non-standardization, and an expanding attack surface, which could undermine trust in IoT systems.

Method: A multi-dimensional analysis of the IoT threat landscape is conducted, applying the STRIDE model for threat classification and the CVSS framework for assessing criticality, while also mapping threats to foundational vulnerability classes.

Result: The paper identifies 28 common IoT attacks and establishes a robust mapping to vulnerability classes, along with presenting state-of-the-art mitigation techniques and discussing emerging research gaps.

Conclusion: The findings provide a consolidated technical foundation for developing secure IoT ecosystems and serve as a roadmap for future research.
...
### [334] [GPUBreach: Privilege Escalation Attacks on GPUs using Rowhammer](https://arxiv.org/abs/2605.03812)
*Chris S. Lin,Yuqin Yan,Guozhen Ding,Joyce Qu,Joseph Zhu,David Lie,Gururaj Saileshwar*

Main category: cs.CR

TL;DR: The paper reveals that Rowhammer vulnerabilities in NVIDIA GPUs can facilitate targeted privilege escalation attacks, allowing unauthorized access to GPU memory and potential CPU-side escalations, threatening data security.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore the Rowhammer exploit potential on GPUs, demonstrating its severity akin to CPU attacks, especially given the limited exploit scenarios previously examined on GPUs.

Method: By manipulating GPU page table management, the paper shows how an unprivileged CUDA kernel can perform targeted bit-flips in page tables, enabling access to memory of other processes.

Result: The research illustrates successful GPU-side privilege escalation attacks, leading to the leaking of sensitive data and stealthier degradation of machine learning models.

Conclusion: The study concludes that the vulnerabilities in GPUs can significantly compromise both GPU and CPU security, circumventing protections like the IOMMU and allowing escalated privileges in non-multi-tenant environments.
...
### [335] [HELO Cryptography: A Lightweight Cryptographic System for Enhancing IoT Security in P2P Data Transmission](https://arxiv.org/abs/2605.03948)
*Tahsin Ahmed,Arjita Saha,Arian Nuhan,Nafim Ahmed Bin Mohammad Noor,Md Faisal Ahmed,Muhammad Iqbal Hossain*

Main category: cs.CR

TL;DR: The paper proposes HELO, a lightweight hybrid encryption system designed to enhance IoT device security without compromising performance.


<details>
  <summary>Details</summary>
Motivation: To address the increasing threats to IoT device security due to cryptographic vulnerabilities and limited computational power, particularly in data transmission.

Method: The proposed system, HELO, leverages hybrid encryption techniques optimized for resource-constrained devices, ensuring solid security against cyberattacks.

Result: HELO is effective in maintaining high security while ensuring improved performance in data handling and transmission for IoT devices.

Conclusion: HELO guarantees confidentiality, integrity, and availability in peer-to-peer data transmission, meeting the security needs of IoT devices.
...
### [336] [Probabilistic-bit Guided CDCL for SAT Solving using Ising Consensus Assumptions](https://arxiv.org/abs/2605.04033)
*Melki Bino*

Main category: cs.CR

TL;DR: This paper presents a hybrid SAT-solving framework combining probabilistic-bit samplers and CDCL solvers to improve search efficiency in solving satisfiable instances of SAT.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the efficiency of CDCL SAT solvers, which may require considerable effort in analyzing conflicts and propagating Boolean variables before finding productive search regions.

Method: A hybrid framework is proposed where a p-bit Ising sampler suggests high-agreement literals as temporary assumptions to the CDCL solver, thus aiming to reduce internal search efforts.

Result: The hybrid method reduces median conflicts by 80.8-85.5% and median propagations by 80.2-84.6% compared to pure CDCL on controlled-backbone random 3-SAT benchmarks, indicating substantial efficiency gains for specific classes of instances.

Conclusion: The observed benefits are sensitive to the instance distribution, and machine-learning gates can effectively identify when this hybrid approach is advantageous, with a random-forest gate retaining 94.8% of hybrid wins.
...
### [337] [6DMA-Enabled ISAC for Low-Altitude Economy](https://arxiv.org/abs/2605.03088)
*Yingchao Jiao,Xuhui Zhang,Chunjie Wang,Shuqiang Wang,Yanyan Shen,Kejiang Ye,Chengzhong Xu*

Main category: eess.SP

TL;DR: This paper introduces a six-dimensional movable antenna (6DMA) for integrated sensing and communications (ISAC) in low-altitude economies, proposing a dual-layer deep reinforcement learning algorithm to optimize its movements and UAV operations for enhanced data transmission rates.


<details>
  <summary>Details</summary>
Motivation: The study aims to leverage the spatial adaptability of the 6DMA to improve the performance of ISAC networks by addressing the limitations of static and partially movable antenna systems in UAV communications.

Method: A hierarchical deep reinforcement learning algorithm is proposed, comprising two layers: the first layer optimizes the 6DMA's location and rotation with infrequent updates, while the second layer optimally adjusts UAV flight directions and base station beamforming.

Result: Numerical simulations indicate that the proposed 6DMA-enabled ISAC algorithm significantly outperforms existing schemes that utilize static antennas or partially movable antennas in maximizing data transmission rates under given constraints.

Conclusion: The research demonstrates the effectiveness of joint optimization through deep reinforcement learning in enhancing ISAC network performance using a 6DMA.
...
### [338] [Phy2-ExposNet: A Physics-Informed Neural Network for EMF Exposure Mapping in Complex Urban Environments](https://arxiv.org/abs/2605.03207)
*Shuangning Li,Yarui Zhang,Shanshan Wang,Joe Wiart*

Main category: eess.SP

TL;DR: This paper presents Phy2-ExposNet, a neural network framework for accurate electromagnetic field exposure mapping that integrates physics-based constraints and a refinement stage, achieving lower estimation errors and reduced model complexity.


<details>
  <summary>Details</summary>
Motivation: To improve electromagnetic field mapping for wireless network planning and communication systems by incorporating physical consistency into predictions.

Method: Phy2-ExposNet decouples exposure mapping into a physics-informed estimation and a transformer-based refinement stage, addressing propagation estimation under two physical constraints.

Result: The method reduces estimation error by approximately 15% relative to existing strong baselines while utilizing over 80% fewer model parameters than conventional physics-informed approaches.

Conclusion: The physics-informed design is essential for accurately capturing complex propagation effects, especially in challenging regions such as boundaries and shadows.
...
### [339] [Copula-Based Analysis of Fluid Antenna-Assisted Over-the-Air Computation](https://arxiv.org/abs/2605.03280)
*Saeid Pakravan,Mohsen Ahmadzadeh,Wessam Ajib,Ghosheh Abed Hodtani,Ming Zeng*

Main category: eess.SP

TL;DR: This letter analyzes an uplink AirComp framework using fluid-antenna arrays in spatially correlated fading channels, providing closed-form CDF expressions for MSE and validating them through numerical results.


<details>
  <summary>Details</summary>
Motivation: To improve performance characterization of AirComp in correlated wireless setups and understand how fluid-antenna arrays perform under these conditions.

Method: Modeling channel dependence with the Gumbel copula and deriving closed-form analytical expressions for CDF of the mean-squared error.

Result: Derivation of performance expressions that show the impact of fluid-antenna deployment on MSE, with numerical validation showing reduced MSE compared to fixed-antenna systems.

Conclusion: FA deployment can significantly reduce MSE, but benefits diminish with stronger spatial correlation.
...
### [340] [Enabling Indoor Multi-Person Tracking With 6G mmWave ISAC Systems](https://arxiv.org/abs/2605.03333)
*Chongrui Wang,Aimin Tang,Fei Gao,Chaojun Xu*

Main category: eess.SP

TL;DR: This paper explores indoor multi-person tracking using 6G mmWave ISAC systems with minimal sensing overhead and effective tracking mechanisms.


<details>
  <summary>Details</summary>
Motivation: To enhance indoor multi-person tracking efficiency in 6G wireless networks while minimizing sensing overhead.

Method: Utilizes a sparse deployment of sensing reference signals in OFDM frames and employs a modified MTI scheme, target identification mechanism, and a Kalman filter with a penalty association algorithm for tracking.

Result: The proposed tracking system achieves a median position error of 12 cm with less than 0.005% sensing overhead, validated through an experimental mmWave ISAC prototype at 26 GHz and 500 MHz bandwidth.

Conclusion: The proposed mechanisms enable effective multi-person tracking in complex indoor environments, demonstrating significant advancements in 6G ISAC systems.
...
### [341] [Near-field Wideband Multi-User Localization using NFMR-Net](https://arxiv.org/abs/2605.03389)
*Pearl Hetul Shah,Srikar Sharma Sadhu,Praful D. Mankar*

Main category: eess.SP

TL;DR: This paper presents a deep learning method for wideband near-field multi-user localization using Zadoff-Chu sequence based pilots to reduce interference and improve channel estimation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance multi-user localization accuracy in communication systems through advanced signal processing techniques.

Method: The method uses Zadoff-Chu sequence pilots for interference mitigation and employs a 2D MUSIC algorithm combined with NFMR-Net for refining range and angle estimates.

Result: Numerical analyses show that NFMR-Net achieves better performance than traditional 2D MUSIC for localization tasks.

Conclusion: The proposed approach significantly improves the accuracy of near-field multi-user localization.
...
### [342] [Joint Communication and Trajectory Design for Movable Antenna Systems](https://arxiv.org/abs/2605.03407)
*Jiaxuan Li,Weidong Mei,Changhao Liu,Zhi Chen,Boyu Ning,Rui Zhang*

Main category: eess.SP

TL;DR: This paper addresses the delay in movable antennas (MAs) for wireless communication by optimizing their trajectory to enhance data transmission time and rates.


<details>
  <summary>Details</summary>
Motivation: The mobility of antennas can enhance communication performance, but delays in movement limit effective data transmission; hence, optimizing their trajectory is essential.

Method: The study formulates a trajectory optimization problem for a single-MA system, first solving a simplified case and then applying graph theory to reframe the problem for general cases, allowing for optimal trajectory planning.

Result: The proposed algorithm significantly improves data rates when compared to baseline schemes, showcasing effective trajectory optimization in wireless communication.

Conclusion: Optimizing the movement of MAs can lead to enhanced data transmission rates, addressing the inherent delays in antenna mobility.
...
### [343] [Identification of Clock Ensemble Noise Parameters Using Differential Measurement Analysis](https://arxiv.org/abs/2605.03418)
*Jindrich Dunik,Ladislav Kral,Ivo Puncochar,Oliver Kost,Ondrej Daniel,Simona Circiu,Bernardino Quaranta*

Main category: eess.SP

TL;DR: The paper focuses on identifying unknown parameters in an atomic clock ensemble using differential measurements of phase differences between clocks.


<details>
  <summary>Details</summary>
Motivation: Understanding the parameters of atomic clock ensembles is crucial for improving their accuracy and synchronization.

Method: The authors propose an identification procedure utilizing pairwise phase differences and develop two methods for estimating the parameters from a stochastic state-space model of the clocks.

Result: The effectiveness of the identification methods is validated through simulations and real data involving atomic H-maser clocks, demonstrating accurate parameter estimation.

Conclusion: The developed methodologies provide a robust framework for parameter identification in atomic clock ensembles, enhancing their operational reliability.
...
### [344] [Near-Field Beam Focusing Characterization for 2D Waveguide-Fed Metasurface Antennas](https://arxiv.org/abs/2605.03488)
*Panagiotis Gavriilidis,George C. Alexandropoulos*

Main category: eess.SP

TL;DR: This paper investigates the near-field characteristics of 2D waveguide-fed metasurfaces, deriving scaling laws for beamforming gain and presenting a compact analytic expression for beam-depth transition to far-field behavior, validated by simulations.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of non-uniform excitation in near-field characteristics of metasurfaces and to provide a clearer understanding of their beamforming capabilities.

Method: A physics-compliant model is used to analyze the near-field beam focusing behavior of waveguide-fed metasurfaces, deriving asymptotic scaling laws and a normalized beam-depth formulation.

Result: The study shows that power-normalized gain scales linearly with the number of radiating elements and presents a compact analytic expression for beam-depth transition to far-field behavior, validated through simulations.

Conclusion: The findings enhance the understanding of beamforming in metasurfaces and confirm the derived scaling laws and beam-depth limits, advancing the design of scalable antenna systems.
...
### [345] [A Near-Field Compatible Model for 2D Waveguide-Fed Metasurfaces](https://arxiv.org/abs/2605.03533)
*Panagiotis Gavriilidis,George C. Alexandropoulos*

Main category: eess.SP

TL;DR: The paper introduces a new analytical model for 2D waveguide-fed metasurface antennas based on discrete dipole approximation, providing effective polarizabilities and near-field predictions.


<details>
  <summary>Details</summary>
Motivation: To develop an accurate model for waveguide-fed metasurface antennas that ensures physical consistency and improves upon existing methods.

Method: The model employs the discrete dipole approximation and incorporates power-consistent constraints for the magnetic polarizability tensor, resulting in closed-form effective polarizability expressions.

Result: The model is validated with full-wave simulations for multiple feed configurations and adapted for near-field applications to enhance predictive accuracy.

Conclusion: The work presents a robust framework for analyzing 2D metasurface antennas, improving predictive capabilities in both far-field and near-field scenarios.
...
### [346] [Robust Beamforming for Cell-Free Systems with Parallel-Plate-Waveguided Dynamic Metasurfaces](https://arxiv.org/abs/2605.03538)
*Konstantinos D. Katsanos,Panagiotis Gavriilidis,George C. Alexandropoulos*

Main category: eess.SP

TL;DR: The paper proposes a distributed optimization framework for Dynamic Metasurface Antennas (DMAs) in a cell-free OFDM system, aimed at maximizing spectral efficiency while accommodating for imperfect Channel State Information (CSI).


<details>
  <summary>Details</summary>
Motivation: The need for lower power consumption and reduced hardware costs in large antenna arrays, alongside limitations of centralized systems requiring high computational power.

Method: A distributed optimization framework focusing on frequency-selective analog and digital beamforming matrices for multiple Base Stations, with minimal control information exchange.

Result: The proposed design demonstrates robustness under varying CSI conditions and emphasizes the significance of mutual coupling in DMA design.

Conclusion: The study highlights the effectiveness of decentralized beamforming techniques in enhancing the performance of cell-free OFDM systems with DMAs.
...
### [347] [Graph-Spectral Fusion of Wavelet Packets and Higher-Order Statistics for Anomaly Detection in Industrial IoT Networks](https://arxiv.org/abs/2605.03591)
*Surya Jayakumar,Indrakshi Dey*

Main category: eess.SP

TL;DR: Graph WPT+HOS is a novel label-free anomaly detection method for IIoT networks, combining Graph Fourier Transform, Wavelet Packet Transform, and Higher-Order Statistics, achieving optimal performance under challenging conditions.


<details>
  <summary>Details</summary>
Motivation: IIoT networks need reliable anomaly detection in adverse wireless environments but current methods struggle due to various challenges.

Method: Graph WPT+HOS integrates Graph Fourier Transform for spatial analysis, Wavelet Packet Transform for time-frequency localization, and Higher-Order Statistics for handling non-Gaussian anomalies, using Mahalanobis distance for scoring.

Result: The method outperforms six existing baselines in ROC-AUC and PR-AUC metrics and minimizes detection latency across different domain shifts in Rayleigh fading environments.

Conclusion: Graph WPT+HOS is efficient and robust for anomaly detection in IIoT, enabling deployment on ARM-class edge devices without GPU.
...
### [348] [Joint Scheduling of Sensing Data Offloading and Edge Inference for Multi-UAV Networks](https://arxiv.org/abs/2605.03898)
*Yanan Du,Sai Xu,Yinbo Yu*

Main category: eess.SP

TL;DR: This paper addresses end-to-end latency minimization in multi-UAV networks that collaborate for edge inference using DNNs, proposing a genetic algorithm-based scheduling approach.


<details>
  <summary>Details</summary>
Motivation: The coupling between wireless offloading and DNN execution in UAV networks complicates latency minimization.

Method: A multi-UAV collaborative edge inference model with a multi-branch DNN is established, and a GA-based full joint scheduler, 	exttt{GA-Joint}, is developed alongside two lightweight variants, 	exttt{GA-DAG} and 	exttt{GA-DACS}.

Result: The proposed GA-based scheduling algorithms outperform 	exttt{Decoupled-Greedy} and 	exttt{Joint-Greedy}, particularly in achieving lower end-to-end latency.

Conclusion: 	exttt{GA-DACS} performs comparably to 	exttt{GA-Joint} and even slightly better in some scenarios, indicating an effective solution for minimizing latency in UAV collaborative inference.
...
### [349] [Deterministic Sparse FFT via Keyed Multi-View Gating with $O(\sqrt{N} \log k)$ Expected Time](https://arxiv.org/abs/2605.03935)
*Aaron R. Flouro,Shawn P. Chadwick*

Main category: eess.SP

TL;DR: The paper presents a deterministic sparse Fourier transform framework that uses a multi-view gating mechanism and CRT for efficient frequency pair reduction, achieving sublinear identification time while ensuring worst-case guarantees.


<details>
  <summary>Details</summary>
Motivation: To improve the efficiency of frequency identification in sparse Fourier transforms by reducing the candidate selection process from quadratic to linear time without compromising safety.

Method: A deterministic framework utilizing a keyed multi-view gating mechanism and 2-of-3 CRT agreement, combined with a peeling-based recovery process and recursive self-reduction to optimize frequency identification.

Result: The proposed method achieves $O(	heta(k))$ candidate reduction and $O(	heta(rac{	ext{N}}{	ext{log k}}))$ expected identification time, while ensuring $O(N 	ext{log N})$ worst-case performance.

Conclusion: The framework successfully integrates deterministic frequency candidate reduction with sublinear expected complexity and robust worst-case performance in the context of sparse Fourier transforms.
...
### [350] [Modeling and Modulation Optimization for OWC Limited by Electronic and Photonic Bandwidth](https://arxiv.org/abs/2605.03976)
*Xiaochen Liu,Jean-Paul M. G. Linnartz,Thiago E. B. Cunha*

Main category: eess.SP

TL;DR: The paper explores bandwidth limitations in optical wireless communication and presents a method to optimize signal power spectral density for improved performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address bandwidth constraints in optical wireless communication caused by system component limitations, enhancing signal modulation efficiency.

Method: The authors optimize the power spectral density of DC-biased optical orthogonal frequency-division multiplexing (DCO-OFDM) using a pole-zero transfer function model and implement a Lagrangian optimization approach, comparing a Newton-based algorithm with an accelerated Hughes-Hartogs algorithm.

Result: Numerical results show that the proposed multi-stage response model for optimization significantly enhances performance, overcoming bandwidth limitations in the frequency profile of gain-to-noise ratio.

Conclusion: The paper concludes that the optimized signal spectrum can significantly improve throughput in optical wireless communication systems under bandwidth constraints.
...
### [351] [Where to Bind Matters: Hebbian Fast Weights in Vision Transformers for Few-Shot Character Recognition](https://arxiv.org/abs/2605.02920)
*Gavin Money,Sindhuja Penchala,Jiacheng Li,Noorbakhsh Amiri Golilarz*

Main category: cs.NE

TL;DR: This paper studies Hebbian Fast-Weight (HFW) modules in transformer architectures to improve rapid adaptation during classification tasks.


<details>
  <summary>Details</summary>
Motivation: To enhance the slow adaptation of standard transformers by leveraging fast synaptic updates akin to biological systems, facilitating better performance in low-data situations.

Method: Empirical evaluation of HFW modules integrated into ViT, DeiT, and Swin transformer architectures on Omniglot benchmark using a Prototypical Network meta-learning framework in 5-way 1-shot and 5-way 5-shot tasks.

Result: The Swin-Tiny variant with HFW achieved the highest test accuracy of 96.2% at 1-shot and 99.2% at 5-shot, outperforming its non-Hebbian baseline by +0.3 percentage points at 1-shot.

Conclusion: The single module placement strategy for HFW in Swin-Tiny mitigated training instability and the results contribute to understanding the dynamics of fast and slow-weight learning in meta-learning contexts.
...
### [352] [EvoJail: Evolutionary Diverse Jailbreak Prompt Generation for Large Language Models](https://arxiv.org/abs/2605.02921)
*Rui Tang,Kaiyu Xu,Pengsen Cheng,Hao Ren,Haizhou Wang,Shuyu Jiang*

Main category: cs.NE

TL;DR: EvoJail is an evolutionary framework for generating diverse jailbreak prompts for LLMs, addressing adaptability and diversity issues in existing methods, achieving high attack success and improved diversity.


<details>
  <summary>Details</summary>
Motivation: To improve automated jailbreak generation by addressing adaptability to evolving LLMs and enhancing the diversity of prompts to avoid narrow attack patterns.

Method: EvoJail uses an evolutionary framework, formalizing prompt generation as a multi-objective optimization problem, with an iterative loop that directly evaluates and adapts candidate prompts based on model responses, employing field-aware instruction fusion and multi-level mutation operators.

Result: EvoJail achieved over 93% attack success rate and over 5.6% improvement in diversity metrics compared to state-of-the-art methods.

Conclusion: EvoJail demonstrates improved adaptability and diversity in jailbreak prompt generation, contributing significantly to safety assessments of LLMs.
...
### [353] [Generalization Bounds of Spiking Neural Networks via Rademacher Complexity](https://arxiv.org/abs/2605.02927)
*Shao-Qun Zhang,Zhi-Hua Zhou*

Main category: cs.NE

TL;DR: This paper investigates generalization bounds of Spiking Neural Networks (SNNs) through Rademacher complexity, providing a detailed analysis of the relationship between network architecture and generalization performance.


<details>
  <summary>Details</summary>
Motivation: The understanding of generalization in SNNs remains unclear despite advancements in algorithms, necessitating a theoretical exploration to clarify performance on unseen data.

Method: The paper employs Rademacher complexity to analyze the generalization bounds of SNNs across various integration-and-fire schemes, linking empirical complexity to network architecture and parameters.

Result: Findings indicate that empirical Rademacher complexity relates exponentially to network depth and spike sequence duration, with other polynomial and inverse-linear relationships to width and training samples, respectively.

Conclusion: The results enhance the theoretical foundation of SNNs and guide future development in SNN frameworks.
...
### [354] [Analysis and Explainability of LLMs Via Evolutionary Methods](https://arxiv.org/abs/2605.02930)
*Shannon K. Gallagher,Swati Rallapalli,Tyler Brooks,Chuck Loughin,Michele Sezgin,Ronald Yurko*

Main category: cs.NE

TL;DR: This paper extends evolutionary methods to analyze large language models (LLMs), demonstrating the utility of relating weights and outputs to genetic concepts for better model understanding and visualization.


<details>
  <summary>Details</summary>
Motivation: To improve the analysis and explanation of relationships among LLMs using concepts from evolutionary biology.

Method: The authors relate model weights to genotypes and output text to phenotypes, employing a controlled experiment to estimate evolutionary trees based on training data.

Result: Estimated evolutionary trees accurately reflect the ground-truth training tree topology and highlight the importance of specific weight layers and datasets in model performance.

Conclusion: The study provides visualizations and insights into the evolutionary relationships among LLMs, demonstrating that certain datasets significantly enhance model outputs.
...
### [355] [Relation Reasoning with LLMs in Expensive Optimization](https://arxiv.org/abs/2605.02933)
*Ye Lu,Bingdong Li,Aimin Zhou,Hao Hao*

Main category: cs.NE

TL;DR: The paper introduces R2SAEA, a surrogate-assisted evolutionary algorithm using a reinforcement-trained relation-based LLM to efficiently optimize expensive black-box problems by reducing evaluation costs and improving offspring selection without frequent retraining.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges posed by expensive optimization problems, particularly the constraints of evaluation budgets and the inefficiencies of conventional surrogate models that require frequent retraining during evolutionary processes.

Method: R2SAEA uses a reinforcement-trained relation-based LLM for surrogate modeling, framing it as an in-context pairwise reasoning task. It employs an anchor-based iterative context construction strategy for efficient inference and a voting-based aggregation scheme for offspring selection based on predicted relations.

Result: Experiments demonstrate that R2SAEA achieves superior relation prediction and optimization performance compared to traditional surrogate-assisted evolutionary algorithms and general LLMs, highlighting its effectiveness in single- and multi-objective optimization benchmarks.

Conclusion: The results indicate that R2SAEA not only performs better in optimization tasks but also enables efficient deployment in edge computing settings by supporting a zero-shot surrogate paradigm.
...
### [356] [Symmetry-Protected Lyapunov Neutral Modes in Equivariant Recurrent Networks](https://arxiv.org/abs/2605.03338)
*Hanson Hanxuan Mo*

Main category: cs.NE

TL;DR: This paper establishes criteria for neutral directions in recurrent networks over long horizons using symmetry principles. It proves that specific compact invariant sets ensure at least a certain number of zero Lyapunov exponents related to group symmetries, impacting memory retention in these systems. The authors experiment with equivariant RNNs and demonstrate improved performance metrics by preserving equivariance.


<details>
  <summary>Details</summary>
Motivation: To understand how symmetry contributes to the stability and memory of recurrent networks storing continuous variables over long timelines.

Method: The authors prove a theorem about Lyapunov spectrum and symmetry in finite-dimensional autonomous vector fields and conduct experiments involving equivariant RNNs with normalized equivariance error and performance comparisons against GRU, LSTM, and orthogonal-RNN models.

Result: The theorem confirms that compact invariant sets guarantee specific neutral directions in terms of Lyapunov exponents, while experiments show that the equivariant RNN outperformed classical architectures in key metrics while preserving exact equivariance.

Conclusion: The findings highlight the significance of symmetry in maintaining stability and memory in recurrent networks, with potential applications in designing more robust architectures for processing continuous variables.
...
### [357] [Unifying Dynamical Systems and Graph Theory to Mechanistically Understand Computation in Neural Networks](https://arxiv.org/abs/2605.03598)
*Jatin Sharma,Danyal Akarca,Dan F. M Goodman*

Main category: cs.NE

TL;DR: The paper explores how recurrent neural networks (RNNs) can enhance computation by analyzing multi-hop pathways between input and output, introducing resolvent-RNNs to induce temporal sparsity for improved performance over standard L1 regularization.


<details>
  <summary>Details</summary>
Motivation: To understand the discrepancy between structural and functional connectivity in neural systems and to improve how recurrent neural networks implement computation based on their connectivity.

Method: The authors model RNNs as graphs and analyze multi-hop pathways by decomposing them by hop length, introducing resolvent-RNNs that constrain these pathways to induce temporal sparsity.

Result: R-RNNs outperform standard L1 regularization by achieving better task performance, showing stronger alignment of sparsity and function, and demonstrating greater robustness under regularization.

Conclusion: Multi-hop communication is crucial for linking structure to function in recurrent networks, suggesting a shift in defining sparsity over functional pathways instead of individual weights.
